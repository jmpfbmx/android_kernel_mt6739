...
@mkdir -p $(dir $@)
if [ -f $(JRD_CUSTOM_RES)/$< ]; then \
  $(AAPT2) compile -o $(dir $@) $(PRIVATE_AAPT2_CFLAGS) --legacy $(JRD_CUSTOM_RES)/$< ;\
else \
  $(AAPT2) compile -o $(dir $@) $(PRIVATE_AAPT2_CFLAGS) --legacy $<; \
fi
if [ -n $(findstring /values/,$<) ]; then \
  for file in `find $(JRD_CUSTOM_RES)/$(subst values/,,$(dir $<)) -name $(notdir $<) 2> /dev/null`;do \
    $(AAPT2) compile -o $(dir $@) $(PRIVATE_AAPT2_CFLAGS) --legacy $$file; \
  done \
fi
endef
...
$(2) : $(1) $(AAPT2) $(JRD_CUSTOM_RES)/flag/strings_flag
	@echo "AAPT2 compile $$@ <- $$<"
...
endef
#BEGIN add plf res to aapt2 package-res.apk
define aapt2-link
@mkdir -p $(dir $@)
...
$(call touch-flat-isdm-file,$(dir $@)aapt2-flat-isdm-list)
$(hide) $(AAPT2) link -o $@ \
  $(PRIVATE_AAPT_FLAGS) \
  ...
  \@$(dir $@)aapt2-flat-isdm-list \
  \@$(dir $@)aapt2-flat-list
endef
# END add plf res to aapt2 package-res.apk
...
define dump-words-to-file
        @rm -f $(2)
        @touch $(2)
	@for f in `find $(dir $(2)) -name values-*string*.arsc.flat 2> /dev/null`; do\
		if [ -z $(findstring $$f,$(1)) ] && [ $(findstring aapt2-flat-overlay-list,$(2)) ]; then \
			echo -n $$f' ' >> $(2);\
		fi\
	done        
...
#BEGIN add plf res to aapt2 package-res.apk
define touch-flat-isdm-file
        @if [ ! -s $(1) ]; then touch $(1);fi
endef
#END add plf res to aapt2 package-res.apk
