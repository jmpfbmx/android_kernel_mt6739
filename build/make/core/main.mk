...
# Added by bin.wei.hz for Modem Filter begin
subdir_makefiles := $(filter-out ./modem/LWTG%,$(subdir_makefiles))
# Added by bin.wei.hz for Modem Filter end
...
#Add by yaosen.lin for task:5327087 2017-09-20  begin
include packages/apps/JrdMMITest/mmitest_skip_modules.mk
$(warning MMITEST_SKIP_MODULES:$(MMITEST_SKIP_MODULES))
$(warning TARGET_BUILD_MMITEST:$(TARGET_BUILD_MMITEST))
#Add by yaosen.lin for task:5327087 2017-09-20  End
...
ifdef FULL_BUILD
  ...
  #added by ruihuai.wu for task 5932706 on 2018.02.02 begin
   ifeq ($(TARGET_BUILD_MMITEST),true)
      ifeq ($(strip $(TARGET_BUILD_U5APLUS4G)), yes)
       $(warning skip_before: $(product_MODULES))
        #Start add by wingtech weili.gao for Op00 20180403
        ifeq ($(T96739_WTMINIBASE_SUPPORT),true)
        NEW_MMITEST_SKIP_MODULES := $(filter-out OP00Plugin,$(MMITEST_SKIP_MODULES))
        $(warning skip_before: $(NEW_MMITEST_SKIP_MODULES))
        product_MODULES := $(filter-out $(NEW_MMITEST_SKIP_MODULES),$(product_MODULES))
        else
        product_MODULES := $(filter-out $(MMITEST_SKIP_MODULES),$(product_MODULES))
        endif
        #End add by wingtech weili.gao for Op00 20180403
       $(warning skip_after: $(product_MODULES))
      endif
   endif
#added by ruihuai.wu for task 5932706 on 2018.02.02 end
...
droidcore: files \
	...
	$(INSTALLED_TCTPERSISTIMAGE_TARGET)
