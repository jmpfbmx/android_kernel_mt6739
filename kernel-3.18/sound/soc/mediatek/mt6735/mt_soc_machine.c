				     ...
				     size_t count, loff_t *pos)
{
	...
	n += scnprintf(buffer + n, size - n, "TOP_CKDIVSEL_CON  = 0x%x\n",
		       Ana_Get_Reg(TOP_CKDIVSEL_CON));
	n += scnprintf(buffer + n, size - n, "TOP_CKHWEN_CON  = 0x%x\n",
	...
	n += scnprintf(buffer + n, size - n, "LDO_CON1  = 0x%x\n", Ana_Get_Reg(LDO_CON1));
	n += scnprintf(buffer + n, size - n, "LDO_CON2  = 0x%x\n", Ana_Get_Reg(LDO_CON2));

	n += scnprintf(buffer + n, size - n, "LDO_VCON1  = 0x%x\n", Ana_Get_Reg(LDO_VCON1));
