...
static void copysinewavetohdmi(unsigned int channels)
{
	...
	if (channels == 2) {
		arraybytes = ARRAY_SIZE(table_sgen_golden_values) * sizeof(
				     table_sgen_golden_values[0]);
		SinewaveArr = table_sgen_golden_values;
	}
	if (channels == 4) {
		arraybytes = ARRAY_SIZE(table_sgen_4ch_golden_values) * sizeof(
				     table_sgen_4ch_golden_values[0]);
		SinewaveArr = table_sgen_4ch_golden_values;
	}
	if (channels == 8) {
		arraybytes = ARRAY_SIZE(table_sgen_8ch_golden_values) * sizeof(
				     table_sgen_8ch_golden_values[0]);
...
#if 0 /* 6752 removed */
		Afe_Set_Reg(AFE_TDM_CON2, 0,
			    0x0000000f); /* tmp    0: Channel starts from O30/O31. */
		Afe_Set_Reg(AFE_TDM_CON2, 1 << 4,
			    0x000000f0); /* tmp    1: Channel starts from O32/O33. */
		Afe_Set_Reg(AFE_TDM_CON2, 2 << 8,
			    0x00000f00); /* tmp    2: Channel starts from O34/O35. */
		Afe_Set_Reg(AFE_TDM_CON2, 3 << 12,
			    0x0000f000); /* tmp    3: Channel starts from O36/O37. */
#endif
					      ...
					      *substream)
{
	...
	if (pMemControl->interruptTrigger == 1) {
		...
		PRINTK_AUD_HDMI("[%s] HW_Cur_ReadIdx =0x%x HW_memory_index = 0x%x pointer = 0x%x\n",
				__func__, HW_Cur_ReadIdx, HW_memory_index, (HW_memory_index >> 2));
...
static int mtk_pcm_hdmi_start(struct snd_pcm_substream *substream)
{
	...
	Afe_Set_Reg(AFE_I2S_CON, Soc_Aud_I2S_WLEN_WLEN_16BITS << 1 ,
		    0x00000002); /* I2S wlen */
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) { /* copy once */
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				PRINTK_AUD_HDMI("[mtk_pcm_hdmi_copy] 0ptr invalid data_w_ptr=%p, size=%d",
						data_w_ptr, copy_size);
				PRINTK_AUD_HDMI("[mtk_pcm_hdmi_copy] u4BufferSize=%d, u4DataRemained=%d",
			...
			PRINTK_AUD_HDMI2("[%s] finish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x, Remained:%x \r\n",
			...
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				PRINTK_AUD_HDMI("HDMI_write 1ptr invalid data_w_ptr=%p, size_1=%d", data_w_ptr,
						size_1);
			...
			if (!access_ok(VERIFY_READ, data_w_ptr + size_1, size_2)) {
				PRINTK_AUD_HDMI("HDMI_write 2ptr invalid data_w_ptr=%p, size_1=%d, size_2=%d",
			...
			PRINTK_AUD_HDMI2("[%s] finish2, size:%x, WriteIdx:%x,ReadIdx=%x DataRemained:%x \r\n",
					 __func__, copy_size, Afe_Block->u4WriteIdx, Afe_Block->u4DMAReadIdx,
					 Afe_Block->u4DataRemained);
		}
	}
