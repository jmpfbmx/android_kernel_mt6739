...
{
	pr_debug("mtk_i2s0_awb_pcm_prepare substream->rate = %d  substream->channels = %d\n",
			 ...
			 struct snd_pcm_hw_params *hw_params)
{
	...
	Afe_Set_Reg(AFE_AWB_END  , pblock->pucPhysBufAddr + (pblock->u4BufferSize - 1),
		    0xffffffff);
				      ...
				      struct snd_pcm_hw_params *hw_params)
{
	...
	pr_debug("mtk_i2s0_awb_pcm_hw_params dma_bytes = %zu dma_area = %p dma_addr = 0x%lx\n",
				 ...
				 void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Awb_Block->u4DataRemained >  Awb_Block->u4BufferSize) {
		pr_debug("%s u4DataRemained=%x > u4BufferSize=%x" , __func__, Awb_Block->u4DataRemained,
	...
	PRINTK_AUD_AWB("%s finish0, read_count:0x%x, size:0x%x, Remained:0x%x, ReadIdx:0x%x, u4WriteIdx:0x%x \r\n",
	...
	if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			pr_warn("%s 1, size:%zu, Remained:0x%x, Ptr:%zu, ReadIdx:0x%x \r\n",
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_warn("%s Fail 1 copy to user Ptr:%p, Addr:%p, DMAReadIdx:0x%x, Read_Ptr:%zu,read_size:%zu",
		...
		PRINTK_AUD_AWB("%s finish1, copy size:0x%x, ReadIdx:0x%x, WriteIdx:0x%x, Remained:0x%x \r\n",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			pr_warn("%s 2, read_size1:0x%x, Remained:0x%x, Read_Ptr:%zu, ReadIdx:0x%x \r\n",
			       __func__, size_1, Awb_Block->u4DataRemained, DMA_Read_Ptr, Awb_Block->u4DMAReadIdx);
		}
		if (copy_to_user((void __user *)Read_Data_Ptr,
				 (Awb_Block->pucVirtBufAddr + DMA_Read_Ptr), size_1)) {

			pr_warn("%s Fail 2 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x, Read_Ptr:%zu,read_size:%zu",
			       __func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr,
			       Awb_Block->u4DMAReadIdx, DMA_Read_Ptr,
			       read_size);
			return 0;
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			pr_warn("%s 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:%zu, DMAReadIdx:%x \r\n",
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_warn("%s Fail 3 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x , Read_Ptr:%zu, read_size:%zu",
			       __func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr,
			       Awb_Block->u4DMAReadIdx, DMA_Read_Ptr,
			       read_size);
			return read_count << 2;
		...
		PRINTK_AUD_AWB("%s finish3, copy size_2:0x%x, ReadIdx:0x%x, WriteIdx:0x%x Remained:0x%x \r\n",
