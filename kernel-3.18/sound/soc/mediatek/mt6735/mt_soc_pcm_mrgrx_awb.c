...
{
	pr_debug("mtk_mrgrx_awb_pcm_prepare substream->rate = %d  substream->channels = %d\n",
				       ...
				       struct snd_pcm_hw_params *hw_params)
{
	...
	pr_debug("mtk_mgrrx_awb_pcm_hw_params dma_bytes = %zu dma_area = %p dma_addr = 0x%lx\n",
				  ...
				  void __user *dst, snd_pcm_uframes_t count)
{
	...
	DMA_Read_Ptr = Awb_Block->u4DMAReadIdx + Get_Mem_CopySizeByStream(
			       Soc_Aud_Digital_Block_MEM_AWB, substream);
	...
	if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			pr_warn("%s 1, read_size:%zu, Remained:0x%x, Ptr:%zu, DMAReadIdx:0x%x \r\n",
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_warn("%s Fail 1 copy to user Ptr:%p, BufAddr:%p, uReadIdx:0x%x, Read_Ptr:%zu,read_size:%zu",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			pr_warn("AudDrv_MEMIF_Read 2, read_size1:0x%x, DataRemained:0x%x, DMA_Read_Ptr:%zu, DMAReadIdx:0x%x \r\n",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			PRINTK_AUD_AWB("%s 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_warn("%s Fail 3 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x , DMA_Read_Ptr:0x%zu, read_size:%zu",
			       __func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr,
			       Awb_Block->u4DMAReadIdx, DMA_Read_Ptr,
			       read_size);
			return read_count << 2;
...
static int mtk_asoc_mrgrx_awb_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	pr_debug("mtk_asoc_mrgrx_awb_pcm_new\n");
	return 0;
}
...
static int mtk_afe_mrgrx_awb_probe(struct snd_soc_platform *platform)
{
	pr_debug("mtk_afe_mrgrx_awb_probe\n");
