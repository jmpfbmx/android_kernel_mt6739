...
#if 0	/* Manually check AudioSramPlaybackSize must be smaller than SRAM size */
...
#if 0	/* Manually check AudioSramCaptureSize must be smaller than SRAM size */
	if (Sramsize > AFE_INTERNAL_SRAM_SIZE)
		Sramsize = AFE_INTERNAL_SRAM_SIZE;
#endif
	return Sramsize;
...
int AudDrv_Allocate_DL2_Buffer(struct device *pDev, kal_uint32 Afe_Buf_Length)
{
...
#ifdef AUDIO_MEMORY_SRAM
	if (Afe_Buf_Length > AFE_INTERNAL_SRAM_SIZE) {
		PRINTK_AUDDRV("Afe_Buf_Length > AFE_INTERNAL_SRAM_SIZE\n");
		return -1;
	}
#endif
	/* allocate memory */
	...
	/* check 32 bytes align */
	if ((pblock->pucPhysBufAddr & 0x1f) != 0) {
		PRINTK_AUDDRV("%s(), is not aligned (0x%x)\n",
