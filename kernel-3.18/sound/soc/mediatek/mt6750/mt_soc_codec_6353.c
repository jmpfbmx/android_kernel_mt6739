...
static void HeadsetVoloumeRestore(int target_index);
...
static unsigned int Speaker_pga_gain = SPK_D_TRIM_INDEX;	/* default 6dB */
static bool mSpeaker_Ocflag;
#endif
static uint32 mInTrimming, mHPSPKon, mHPon;
	...
	if (enable) {
		if (audck_buf_Count == 0) {
			...
			if (mClkBufferfromPMIC) {
				Ana_Set_Reg(MT6353_DCXO_CW01, 0x8000, 0x8000);
...
#ifndef CONFIG_FPGA_EARLY_PORTING
			if (mClkBufferfromPMIC) {
				Ana_Set_Reg(MT6353_DCXO_CW01, 0x0000, 0x8000);
...
static void ClsqEnable(bool enable)
{
	...
	if (enable) {
		if (ClsqCount == 0) {
			Ana_Set_Reg(MT6353_TOP_CLKSQ, 0x0001, 0x0001);
			/* Enable CLKSQ 26MHz */
			Ana_Set_Reg(MT6353_TOP_CLKSQ_SET, 0x0001, 0x0001);
		...
		if (ClsqCount == 0) {
			...
			Ana_Set_Reg(MT6353_TOP_CLKSQ, 0x0000, 0x0001);
			/* Disable CLKSQ 26MHz */
		}
	}
...
{
	/* pr_warn("Topck_Enable, TopCkCount = %d, enable = %d\n", TopCkCount, enable); */
...
static void NvregEnable(bool enable)
{
	...
	if (enable == true) {
		if (NvRegCount == 0)
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON8, 0x0000, 0x0002);	/* Enable AUDGLB */
...
static void Apply_Speaker_Gain(void)
{
	...
	Ana_Set_Reg(MT6353_SPK_ANA_CON0, Speaker_pga_gain << 11, 0x7800);
}
#else
static void Apply_Speaker_Gain(void)
{
...
static int mHplOffset_CompenBydB[MAX_HP_GAIN_LEVEL], mHprOffset_CompenBydB[MAX_HP_GAIN_LEVEL],
	mHplSpkOffset_CompenBydB[MAX_HP_GAIN_LEVEL], mHprSpkOffset_CompenBydB[MAX_HP_GAIN_LEVEL];

void CalculateDCCompenForEachdB(void)
{
	int i;

	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		mHplOffset_CompenBydB[i] = (mHplTrimOffset - TrimOffset) * dBFactor_Nom[i] / dBFactor_Den;
		mHprOffset_CompenBydB[i] = (mHprTrimOffset - TrimOffset) * dBFactor_Nom[i] / dBFactor_Den;
		pr_warn("mHplOffset_CompenBydB[%d]=%d, mHprOffset_CompenBydB[%d]=%d\n", i, mHplOffset_CompenBydB[i],
			i, mHprOffset_CompenBydB[i]);
	}
}
...
void CalculateSPKHP_DCCompenForEachdB(void)
{
	...
	for (i = 0; i < MAX_HP_GAIN_LEVEL; i++) {  /* 0:8dB, 1:7dB, ...,  18:-10dB */
		mHplSpkOffset_CompenBydB[i] = (mHplSpkTrimOffset[i] - TrimOffset) * dBFactor_Nom[i] / dBFactor_Den;
		mHprSpkOffset_CompenBydB[i] = (mHprSpkTrimOffset - TrimOffset) * dBFactor_Nom[i] / dBFactor_Den;
		pr_warn("mHplSpkOffset_CompenBydB[%d]=%d, mHprSpkOffset_CompenBydB[%d]=%d\n", i,
			mHplSpkOffset_CompenBydB[i], i, mHprSpkOffset_CompenBydB[i]);
	}
}
...
void SetHplTrimOffset(int Offset)
{
	...
	if ((Offset > 2252) || (Offset < 1844)) {
		mHplTrimOffset = 2048;
		pr_err("[contact ShonHang] SetHplTrimOffset offset may be invalid offset = %d\n", Offset);
	}
}
...
void SetHprTrimOffset(int Offset)
{
	...
	if ((Offset > 2252) || (Offset < 1844)) {
		mHprTrimOffset = 2048;
		pr_err("[contact ShonHang] SetHprTrimOffset offset may be invalid offset = %d\n", Offset);
	}
}

void SetHplSpkTrimOffset(void *Offset)
{
	int i;

	for (i = 0 ; i < MAX_HP_GAIN_LEVEL ; i++) {
		mHplSpkTrimOffset[i] = *((int *)Offset + i);
		if ((mHplSpkTrimOffset[i] > 2696) || (mHplSpkTrimOffset[i] < 1400)) { /* adjust by hp -10dB gain */
			mHplSpkTrimOffset[i] = 2048;
			pr_err("SetHplSpkTrimOffse[%d] offset may be invalid offset = %d\n", i, *((int *)Offset + i));
		}
	}
}
...
void SetHprSpkTrimOffset(int Offset)
{
	...
	if ((Offset > 2696) || (Offset < 1400)) { /* adjust by hp -10dB gain */
...
{
	if (benable == true) {
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0001, 0x0001);
		/* Enable HP & HS drivers bias circuit    //trim buffer enable bit MT6328 */

		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON3, 0x1, 0x1);
		/* Audio offset trimming buffer enable */
	} else {
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0000, 0x0001);
		/* Disable drivers bias circuit */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON3, 0x0, 0x1);
...
void OpenTrimBufferHardware(bool enable)
{
	...
	if (enable) {
		...
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON8, 0x0000, 0x0002);
		/* Enable AUDGLB */
		...
		Ana_Set_Reg(MT6353_ZCD_CON0, 0x0400, 0xffff);
		/* Disable AUD_ZCD */
		...
		/* Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x1f00, 0xffff);
		//Enable HP & HS drivers bias circuit    //trim */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x1f01, 0xffff);
		/* Enable HP & HS drivers bias circuit    //trim buffer enable bit MT6328 */

		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON5, 0x5490, 0xffff);
		/* HP/HS ibias & DR bias current optimization */
		udelay(50);
		Ana_Set_Reg(MT6353_ZCD_CON2, 0x0489, 0xffff);	/* Set HPR/HPL gain as -1dB */
		...
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xE090, 0xffff);	/* Enable voice driver */
		...
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0009, 0x0009);	/* Enable Audio DACL */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0006, 0x0006);	/* Enable Audio DACR */
...
void OpenAnalogTrimHardware(bool enable)
{
	if (enable) {
		...
		/* Enable pre-charge buffer */
		udelay(50);
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x2AC2, 0xfeeb);
		/* Enable AUD_CLK */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0009, 0x0009);
		/* Enable Audio DACL */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0006, 0x0006);
		/* Enable Audio DACR */
...
{
	pr_warn("%s\n", __func__);
	Ana_Set_Reg(MT6353_ZCD_CON1, 0x8 << 7, 0x0f80);
	Ana_Set_Reg(MT6353_ZCD_CON1, 0x8, 0x001f);
}
...
void setIntSpkGainZero(void)
{
	...
	Ana_Set_Reg(MT6353_SPK_ANA_CON0, 0x1 << 11, 0x7800); /* restore to 1: 0dB */
...
{
	uint32 cur_spk_gain;
	int oldindex = 0, offset = 0, count = 1;
	...
	Ana_Set_Reg(MT6353_SPK_ANA_CON0, index << 11, 0x7800); /* set to index */
}

void setHpGain(int target_index)
{
	HeadsetVoloumeRestore(target_index);
	pr_warn("%s(), target_index = %d\n", __func__, target_index);
}
...
static void SetHplOffset(int OffsetTrimming, bool EnableRamp)
{
#define DCOFFSET_RAMP_STEP_HPL 3
	...
	if (EnableRamp == 1) {
		...
		/* Lch needs to ramp up/down due to spk using Lch */
		if ((RegValue & 0x8000) == 0x0) {
			offset = 0;
			while ((RegValue - offset) > DCOFFSET_RAMP_STEP_HPL) {
				Ana_Set_Reg(AFE_DL_DC_COMP_CFG0, offset + DCOFFSET_RAMP_STEP_HPL, 0xffff);
				offset += DCOFFSET_RAMP_STEP_HPL;
				pr_warn("%s(), L offset = 0x%x\n", __func__, offset);
				usleep_range(100, 200);
			}
		} else {
			offset = 0xffff;
			while ((offset - RegValue) > DCOFFSET_RAMP_STEP_HPL) {
				Ana_Set_Reg(AFE_DL_DC_COMP_CFG0, offset - DCOFFSET_RAMP_STEP_HPL, 0xffff);
				offset -= DCOFFSET_RAMP_STEP_HPL;
				pr_warn("%s(), L offset = 0x%x\n", __func__, offset);
				usleep_range(100, 200);
...
#define DCOFFSET_RAMP_STEP 3
static void RestoreDcOffsetToZero(void)
{
	uint32 aud_dc_comp_lch, aud_dc_comp_rch;
	uint32 offset;

	aud_dc_comp_lch = Ana_Get_Reg(AFE_DL_DC_COMP_CFG0); /* Get HP L dc offset */
	aud_dc_comp_rch = Ana_Get_Reg(AFE_DL_DC_COMP_CFG1); /* Get HP R dc offset */

	pr_warn("%s(), aud_dc_comp_lch = 0x%x, aud_dc_comp_rch = 0x%x\n", __func__, aud_dc_comp_lch, aud_dc_comp_rch);

	offset = aud_dc_comp_lch;

	/* Lch needs to ramp up/down due to spk using Lch */
	if ((offset & 0x8000) == 0x0) {
		while (offset > DCOFFSET_RAMP_STEP) {
			Ana_Set_Reg(AFE_DL_DC_COMP_CFG0, offset - DCOFFSET_RAMP_STEP, 0xffff);
			offset -= DCOFFSET_RAMP_STEP;
			usleep_range(100, 200);
		}
	} else {
		while (offset < (0xffff - DCOFFSET_RAMP_STEP)) {
			Ana_Set_Reg(AFE_DL_DC_COMP_CFG0, offset + DCOFFSET_RAMP_STEP, 0xffff);
			offset += DCOFFSET_RAMP_STEP;
			usleep_range(100, 200);
		}
...
static void SetHpLOffsetTrim(void)
{
	...
	pr_warn("%s(), mHplTrimOffset = %d, TrimOffset = %d\n", __func__, mHplTrimOffset, TrimOffset);
	SetHplOffset(OffsetTrimming, 0);
}
#endif
	...
	case AUDIO_ANALOGUL_MODE_DMIC:
		{
			if (MicBias == AUDIO_MIC_BIAS0)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0000, 0x0006);
			else if (MicBias == AUDIO_MIC_BIAS1)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0000, 0x0060);
		}
		break;
	case AUDIO_ANALOGUL_MODE_DCCECMDIFF:
		{
			if (MicBias == AUDIO_MIC_BIAS0)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0006, 0x0006);
			else if (MicBias == AUDIO_MIC_BIAS1)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0060, 0x0060);
		}
		break;
	case AUDIO_ANALOGUL_MODE_DCCECMSINGLE:
		{
			if (MicBias == AUDIO_MIC_BIAS0)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0002, 0x0006);
			else if (MicBias == AUDIO_MIC_BIAS1)
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0020, 0x0060);
		}
		break;
	default:
		break;
...
static void TurnOffDacPower(void)
{
	...
	Ana_Set_Reg(MT6353_AUDNCP_CLKDIV_CON0, 0x0000, 0xffff);	/* Toggle RG_DIVCKS_CHG */
	Ana_Set_Reg(MT6353_AUDNCP_CLKDIV_CON1, 0x0000, 0xffff);	/* Turn off DA_600K_NCP_VA18 */
...
static void HeadsetVoloumeRestore(int target_index)
{
	...
	Ana_Set_Reg(MT6353_ZCD_CON2, (target_index << 7) | target_index, 0xf9f); /* restore to target_index */
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL] = target_index;
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR] = target_index;
}
...
static void Audio_Amp_Change(int channels, bool enable)
{
	pr_warn("Audio_Amp_Change(), enable = %d, mInTrimming = %d\n", enable, mInTrimming);
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x2AC2, 0xfeeb);
			/* Enable AUD_CLK */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xF, 0xf);
			/* Enable Audio DAC */

			/* Apply digital DC compensation value to DAC */
			if (mInTrimming == 0) {
				HeadsetVoloumeSet();
				SetHplOffset(mHplOffset_CompenBydB[mCodec_data->mAudio_Ana_Volume
					[AUDIO_ANALOG_VOLUME_HPOUTL]], false);
				SetHprOffset(mHprOffset_CompenBydB[mCodec_data->mAudio_Ana_Volume
					[AUDIO_ANALOG_VOLUME_HPOUTR]]);
				EnableDcCompensation(true);
			}

			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x1480, 0x1f80);
			/* Switch HP MUX to audio DAC */
			/* here may cause pop */
			/* msleep(1); //6752 removed */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0060, 0x0060);
			/* Enable HPR/HPL */
			udelay(50);
			...
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0x0000, 0x0190);
			...
			if (mInTrimming == 0) /* Not do when triming to avoid pop */
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xF40F, 0xffff);
			/* Disable HPR/HPL */
...
static void Voice_Amp_Change(bool enable)
{
	if (enable) {
		...
		if (GetDLStatus() == false) {
			...
			Ana_Set_Reg(MT6353_AUDNCP_CLKDIV_CON3, 0x0000, 0xffff);
			/* Enable NCP */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x0A40, 0xfeeb);
			...
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x2AC0, 0xfeeb);
			/* Enable IBIST */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0400, 0xffff);
			...
			Ana_Set_Reg(MT6353_ZCD_CON3, 0x001F, 0xffff);
			/* Set voice gain as minimum (~ -40dB) */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON1, 0x1000, 0xffff);
			/* De_OSC of voice, enable output STBENH */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x2AC2, 0xfeeb);
			/* Enable AUD_CLK */
			...
			/* Apply digital DC compensation value to DAC */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xE109, 0xffff);
...
static void Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		Ana_Set_Reg(MT6353_ZCD_CON0, 0x0400, 0xffff);
		/* Disable IVBUF_ZCD */
		Ana_Set_Reg(MT6353_ZCD_CON4, 0x0705, 0xffff);
		/* Set 0dB IV buffer gain */
		Ana_Set_Reg(MT6353_SPK_ANA_CON3, 0x0100, 0xffff);
		/* Set IV buffer MUX select */
		Ana_Set_Reg(MT6353_SPK_ANA_CON3, 0x0110, 0xffff);
		/* Enable IV buffer */
/*		Ana_Set_Reg(MT6353_CLK_CKPDN_CON2_CLR, 0x0003, 0xffff); MT6328 */
		Ana_Set_Reg(MT6353_CLK_CKPDN_CON3_CLR, 0x00C0, 0xffff);

		/* Enable ClassAB/D clock */

#ifdef CONFIG_MTK_SPEAKER
		if (Speaker_mode == AUDIO_SPEAKER_MODE_D)
			Speaker_ClassD_Open(0);
...
#endif
		/*Ana_Set_Reg(MT6353_CLK_CKPDN_CON2_SET, 0x0003, 0xffff); MT6328*/
		Ana_Set_Reg(MT6353_CLK_CKPDN_CON3_SET, 0x00C0, 0xffff); /*MT6353*/

		/* Disable ClassAB/D clock */


		Ana_Set_Reg(MT6353_SPK_ANA_CON3, 0x0, 0xffff);
		/* Disable IV buffer, Set IV buffer MUX select open/open */
		Ana_Set_Reg(MT6353_ZCD_CON4, 0x0707, 0xffff);
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	pr_warn("Headset_Speaker_Amp_Change(), enable = %d, mInTrimming = %d\n", enable, mInTrimming);
	if (enable) {
		if (GetDLStatus() == false)
			TurnOnDacPower();
		...
		Ana_Set_Reg(MT6353_ZCD_CON0, 0x0400, 0xffff);
		/* Disable IVBUF_ZCD */
		Ana_Set_Reg(MT6353_ZCD_CON4, 0x0505, 0xffff);
		/* Set 0dB IV buffer gain */
		Ana_Set_Reg(MT6353_SPK_ANA_CON3, 0x0100, 0xffff);
		/* Set IV buffer MUX select */
		Ana_Set_Reg(MT6353_SPK_ANA_CON3, 0x0110, 0xffff);
		...
		if (Speaker_mode == AUDIO_SPEAKER_MODE_D)
			Speaker_ClassD_Open(mInTrimming);
		else if (Speaker_mode == AUDIO_SPEAKER_MODE_AB)
			Speaker_ClassAB_Open();
		else if (Speaker_mode == AUDIO_SPEAKER_MODE_RECEIVER)
			Speaker_ReveiverMode_Open();
		...
		if (mInTrimming == 0) {
			/* Apply digital DC compensation value to DAC */
			HeadsetVoloumeSet();
			SetHplOffset(mHplSpkOffset_CompenBydB[mCodec_data->mAudio_Ana_Volume
				[AUDIO_ANALOG_VOLUME_HPOUTL]], false);
			SetHprOffset(mHprSpkOffset_CompenBydB[mCodec_data->mAudio_Ana_Volume
				[AUDIO_ANALOG_VOLUME_HPOUTR]]);
			EnableDcCompensation(true);
		}

		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xF29F, 0xffff);
		/* R hp input mux select "Audio playback", L hp input mux select "LoudSPK playback" */
		/* here may cause pop? */
		/* msleep(1); //6752 removed */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xF2FF, 0xffff);
		/* Enable HPR/HPL */
		udelay(50);
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON1, 0x1480, 0xffff);
		/* Disable pre-charge buffer */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON1, 0x0480, 0xffff);
		/* Disable De_OSC of voice */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xF26F, 0xffff);
		...
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xF20F, 0xffff);	/* Disable HPR/HPL */
		/* Disable HPR/HPL */
		Ana_Set_Reg(MT6353_AUDDEC_ANA_CON4, 0x0000, 0xffff);
		/* Disable drivers bias circuit */
		if (mInTrimming == 0)
			RestoreDcOffsetToZero();
#ifdef CONFIG_MTK_SPEAKER
		if (Speaker_mode == AUDIO_SPEAKER_MODE_D) {
			setIntSpkGainToIndex(SPK_D_TRIM_INDEX); /* Set to spk trimmed level before close */
			Speaker_ClassD_close(mInTrimming);
		}
		else if (Speaker_mode == AUDIO_SPEAKER_MODE_AB)
			Speaker_ClassAB_close();
		else if (Speaker_mode == AUDIO_SPEAKER_MODE_RECEIVER)
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if ((ucontrol->value.integer.value[0] == true)
		...
		mCodec_data->mAudio_Ana_DevicePower
		    [AUDIO_ANALOG_DEVICE_OUT_SPEAKER_HEADSET_L] = ucontrol->value.integer.value[0];
	} else if ((ucontrol->value.integer.value[0] == false)
		   &&
		   (mCodec_data->mAudio_Ana_DevicePower
					     ...
					     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0])
		Ana_Set_Reg(MT6353_SPK_CON12, 0x9300, 0xff00);
	else
		Ana_Set_Reg(MT6353_SPK_CON12, 0x1300, 0xff00);
...
static int Audio_Speaker_Current_Sensing_Get(struct snd_kcontrol *kcontrol,
					     struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = (Ana_Get_Reg(MT6353_SPK_CON12) >> 15) & 0x01;
	return 0;
}
							   ...
							   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0])
		Ana_Set_Reg(MT6353_SPK_CON12, 1 << 14, 1 << 14);
	else
		Ana_Set_Reg(MT6353_SPK_CON12, 0, 1 << 14);
...
static int Audio_Speaker_Current_Sensing_Peak_Detector_Get(struct snd_kcontrol *kcontrol,
							   struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = (Ana_Get_Reg(MT6353_SPK_CON12) >> 14) & 0x01;
	return 0;
}
...
static int Lineout_PGAL_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	Ana_Set_Reg(MT6353_ZCD_CON1, index, 0x001f);
...
static int Lineout_PGAR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	Ana_Set_Reg(MT6353_ZCD_CON1, index << 7, 0x0f80);
...
static int Handset_PGA_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	Ana_Set_Reg(MT6353_ZCD_CON3, index, 0x001f);
...
static int Headset_PGAL_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == true) {
		...
		Ana_Set_Reg(MT6353_ZCD_CON2, index, 0x001f);
	}
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL] =
			ucontrol->value.integer.value[0];
...
static int Headset_PGAR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETR] == true) {
		...
		Ana_Set_Reg(MT6353_ZCD_CON2, index << 7, 0x0f80);
	}
	mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR] =
			ucontrol->value.integer.value[0];
...
static bool TurnOnADcPowerACC(int ADCType, bool enable)
{
	...
	if (enable) {
		...
		if (GetMicbias == 0) {
			MicbiasRef = Ana_Get_Reg(MT6353_AUDENC_ANA_CON9) & 0x0700;
		...
		if (GetAdcStatus() == false) {
			...
			/*Ana_Set_Reg(LDO_CON2, 0x8102, 0xffff);  MT6328 */
			/* LDO enable control by RG_VAUD28_EN, Enable AVDD28_LDO (Default on) */

			/*Ana_Set_Reg(MT6353_LDO3_VAUD28_CON0, 0x1042, 0xffff);*/
			/* LDO enable control by LDO_VAUD28_EN Enable AVDD28_LDO (Default on) */

			NvregEnable(true);
			/* ClsqAuxEnable(true); */
			ClsqEnable(true);

			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x0004, 0x0004);
			/* Enable audio ADC CLKGEN */

			Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
			/* ADC CLK from CLKGEN (13MHz) */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x0104, 0x0104);
			/* Enable  LCLDO_ENC 1P8V */

			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON7, 0x0006, 0x0006);
			/* LCLDO_ENC remote sense */
			/* Ana_Set_Reg(AUDENC_ANA_CON6, 0x1515, 0xffff); //default value */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON6, 0x0555, 0xffff);
			/* default value MT6328 */

			Ana_Set_Reg(MT6353_AUDENC_ANA_CON4, 0x0800, 0xffff);
			/* default value */
		}
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0711, 0xff19);
				/* Enable MICBIAS0, MISBIAS0 = 1P9V,
				...
				/* micbias1 DCCopuleNP */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0710, 0xff90);
				/* Enable MICBIAS1, MISBIAS1 = 2P5V ?// or 2P7V George? */
			}
			/* Ana_Set_Reg(AUDENC_ANA_CON15, 0x0003, 0x000f); //Audio L PGA 18 dB gain(SMT) */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0003, 0x000f);	/* Audio L PGA 18 dB gain(SMT) MT6328 */
			...
			if (refmic_using_ADC_L == false) {
				...
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0033, 0x00ff);
				/* Audio R PGA 18 dB gain(SMT) MT6328 */
			} else {
				/* Ana_Set_Reg(AUDENC_ANA_CON15, 0x0003, 0x000f);
				//Audio L PGA 18 dB gain(SMT) */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0003, 0x000f);
				/* Audio L PGA 18 dB gain(SMT) MT6328 */
			}
		}

		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0800, 0xf900);	/* PGA stb enhance */

			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				/* "ADC1", main_mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0041, 0x00C1);
				/* Audio L preamplifier input sel : AIN0. Enable audio L PGA */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0541, 0xffff);
				/* Audio L ADC input sel : L PGA. Enable audio L ADC */
			} else if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 1) {
				/* "ADC2", headset mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0500, 0xffff);
				/* Audio L ADC input sel : L PGA. Enable audio L ADC */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0581, 0xffff);
				/* Audio L preamplifier input sel : AIN1. Enable audio L PGA */

			}
		} else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) {
			/* ref mic */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0800, 0xf900);
			/* PGA stb enhance */
			if (refmic_using_ADC_L == false) {
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C1, 0x00C1);
				/* Audio R preamplifier input sel : AIN2. Enable audio R PGA */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x05C1, 0xffff);
				/* Audio R ADC input sel : R PGA. Enable audio R ADC */
			} else {
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x00c1, 0x00C1);
				/* Audio L preamplifier input sel : AIN2. Enable audio L PGA */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x05c1, 0xffff);
				/* Audio L ADC input sel : L PGA. Enable audio L ADC */
			}
		}
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				/* "ADC1", main_mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0041, 0xffff);
				/* Audio L ADC input sel : off, disable audio L ADC */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
				/* PGA stb enhance off */
				/* Ana_Set_Reg(AUDENC_ANA_CON15, 0x0000, 0x000f); //Audio L PGA 0 dB gain */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0000, 0x000f);
				/* MT6328 Audio L PGA 0 dB gain */

				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0000, 0xffff);
			...
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON4, 0x0000, 0xffff);	/*  */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON6, 0x2020, 0xffff);	/*  */

			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				/* "ADC1", main_mic */
				/* Ana_Set_Reg(AUDENC_ANA_CON9, (MicbiasRef|0x0000), 0xff09);
				//disable MICBIAS0, restore to micbias set by accdet */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, (MicbiasRef | 0x0000), 0xff19);
				/* disable MICBIAS0 and MICBIAS1, restore to micbias set by accdet */
			} else if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 1) {
				/* "ADC2", headset mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, (MicbiasRef | 0x0000), 0xff90);
				/* disable MICBIAS1, restore to micbias set by accdet */
			}
		} else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) {	/* ref mic */
			if (refmic_using_ADC_L == false) {
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C1, 0xffff);
				/* Audio R ADC input sel : off, disable audio R ADC */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
				/* PGA stb enhance off */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0000, 0x00ff);
				/* Audio R PGA 0 dB gain //MT6328 */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0000, 0xffff);
				/* Audio R preamplifier input sel : off, disable audio R PGA */
			} else {
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x00c1, 0xffff);
				/* Audio L ADC input sel : off, disable audio L ADC */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
				/* PGA stb enhance off */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0000, 0x000f);
		...
		if (GetAdcStatus() == false) {
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON7, 0x0000, 0x0006);
			/* LCLDO_ENC remote sense off */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x0004, 0x0104);
			/* disable LCLDO_ENC 1P8V */

			Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
			/* disable ADC CLK from CLKGEN (13MHz) */
			Ana_Set_Reg(MT6353_AUDDEC_ANA_CON6, 0x0000, 0x0104);
...
static bool TurnOnADcPowerDmic(int ADCType, bool enable)
{
	...
	if (enable) {
		...
		if (GetAdcStatus() == false) {
			...
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0211, 0xff19);
			/* Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON8, 0x0005, 0xffff);
			/* DMIC enable */
		...
		if (GetAdcStatus() == false) {
			...
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON8, 0x0000, 0xffff);
			/* DMIC disable */
...
static bool TurnOnADcPowerDCC(int ADCType, bool enable, int ECMmode)
{
	...
	if (enable) {
		...
		if (GetAdcStatus() == false) {
			...
			Ana_Set_Reg(MT6353_TOP_CLKSQ_SET, 0x0003, 0x0003);	/*  */
			/* Ana_Set_Reg(TOP_CKPDN_CON0, 0x0000, 0x2000);
			//bit[13] AUD_CK power down=0 */
			Ana_Set_Reg(MT6353_CLK_CKPDN_CON0, 0x0000, 0x6000);
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {	/* "ADC1", main_mic */
				...
				/* Ana_Set_Reg(AUDENC_ANA_CON9, 0x0201, 0xff09);
				//Enable MICBIAS0, MISBIAS0 = 1P9V */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0701, 0xff09);
				...
				if (ECMmode == 1) {	/* differential */
					...
					Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0717, 0xff1f);
					...
					//Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V, [6752] ALPS01824949 */
					Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0713, 0xff1f);
					...
					//Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 1P9V , [6752] ALPS01824949 */
					Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0711, 0xff1f);
					/* Enable MICBIAS0 and MICBIAS1, MISBIAS0 = 2P5V */
				}
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0004, 0xffff);
				/* Audio L preamplifier DCC precharge */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0045, 0xffff);
				/* Audio L preamplifier input sel : AIN0. Enable audio L PGA */
				...
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0047, 0xffff);
				...
				/* micbias1 DCCopuleNP */
				if (ECMmode == 1) {	/* differential */
					Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0770, 0xfff0);
					/* Enable MICBIAS1, SwithP/N MISBIAS1 = 2P5V */
				} else if (ECMmode == 2) {	/* single end */
					Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0730, 0xfff0);
					...
					Ana_Set_Reg(MT6353_AUDENC_ANA_CON9, 0x0710, 0xfff0);
					/* MICBIAS1 DCC  on //DCC, bit 5 to 1 to match ACCDET */

				}
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0004, 0xffff);
				/* Audio L preamplifier DCC precharge */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0085, 0xffff);
				/* Audio L preamplifier input sel : AIN1. Enable audio L PGA */
				/* Ana_Set_Reg(AUDENC_ANA_CON15, 0x0002, 0x000f);
				//Audio L PGA 12 dB gain */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0003, 0x000f);
				/* Audio L PGA 18 dB gain MT6328 */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0087, 0xffff);
				/* Audio L preamplifier DCCEN */
			}
		} else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) {	/* ref mic */
			SetDCcoupleNP(AUDIO_MIC_BIAS0, mAudio_Analog_Mic2_mode);
			...
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0004, 0xffff);
			/* Audio R preamplifier DCC precharge */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C5, 0xffff);
			/* Audio R preamplifier input sel : AIN2. Enable audio R PGA */
			/* Ana_Set_Reg(AUDENC_ANA_CON15, 0x0020, 0x00f0); //Audio R PGA 12 dB gain */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0033, 0x00ff);
			/* Audio R PGA 18 dB gain MT6328 */

			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C7, 0xffff);
			/* Audio R preamplifier DCCEN */
		}

		Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0800, 0x0900);
		/* PGA stb enhance */
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				/* "ADC1", main_mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0547, 0xffff);
				/* Audio L ADC input sel : L PGA. Enable audio L ADC */
				udelay(100);
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0543, 0xffff);
				/* Audio L preamplifier DCC precharge off */
			} else if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 1) {
				/* "ADC2", headset mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0587, 0xffff);
				/* Audio L ADC input sel : L PGA. Enable audio L ADC */
				udelay(100);
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0583, 0xffff);
				/* Audio L preamplifier DCC precharge off */
			}
		} else if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC2) {
			/* ref mic */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x05C7, 0xffff);
			/* Audio R ADC input sel : R PGA. Enable audio R ADC */
			udelay(100);
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x05C3, 0xffff);
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				/* "ADC1", main_mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0043, 0xffff);
				/* Audio L ADC input sel : off, disable audio L ADC */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
				/* PGA stb enhance off */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0041, 0xffff);
				...
				/* "ADC2", headset mic */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0083, 0xffff);
				/* Audio L preamplifier input sel : off, disable audio L PGA */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
				/* PGA stb enhance off */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0081, 0xffff);
				/* Audio L preamplifier DCCEN disable */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0000, 0x000f);
				/* Audio L PGA 0 dB gain */
				Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0000, 0xffff);
			...
			/* ref mic */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C3, 0xffff);
			/* Audio R ADC input sel : off, disable audio R ADC */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON3, 0x0000, 0xffff);
			/* PGA stb enhance off */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C1, 0xffff);
			/* Audio R preamplifier DCCEN disable */
			Ana_Set_Reg(MT6353_AUDENC_ANA_CON10, 0x0000, 0x00f0);
...
static int Audio_ADC1_Sel_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == 0)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, (0x0000 << 9), 0x0600);	/* pinumx sel */
	else if (ucontrol->value.integer.value[0] == 1)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, (0x0001 << 9), 0x0600);	/* AIN0 */
	else if (ucontrol->value.integer.value[0] == 2)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, (0x0002 << 9), 0x0600);	/* Left preamp */
	else
		pr_warn("%s() warning\n ", __func__);
...
static int Audio_ADC2_Sel_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] == 0)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, (0x0000 << 9), 0x0600);	/* pinumx sel */
	else if (ucontrol->value.integer.value[0] == 1)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, (0x0001 << 9), 0x0600);	/* AIN2 */
	else if (ucontrol->value.integer.value[0] == 2)	/* Right preamp */
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, (0x0002 << 9), 0x0600);
	else
		pr_warn("%s() warning\n ", __func__);
...
static bool AudioPreAmp1_Sel(int Mul_Sel)
{
	...
	if (Mul_Sel == 0)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0000, 0x00C0);	/* pinumx open */
	else if (Mul_Sel == 1)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0040, 0x00C0);	/* AIN0 */
	else if (Mul_Sel == 2)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x0080, 0x00C0);	/* AIN1 */
	else if (Mul_Sel == 3)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON0, 0x00C0, 0x00C0);	/* AIN2 */
	else
...
{
	pr_warn
	    ("%s() mCodec_data->mAudio_Ana_Mux[AUDIO_ANALOG_MUX_IN_PREAMP_1]; = %d\n",
...
static bool AudioPreAmp2_Sel(int Mul_Sel)
{
	...
	if (Mul_Sel == 0)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0000, 0x00C0);	/* pinumx open */
	else if (Mul_Sel == 1)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0040, 0x00C0);	/* AIN0 */
	else if (Mul_Sel == 2)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x0080, 0x00C0);	/* AIN1 */
	else if (Mul_Sel == 3)
		Ana_Set_Reg(MT6353_AUDENC_ANA_CON1, 0x00C0, 0x00C0);	/* AIN2 */
	else
		pr_warn("AudioPreAmp1_Sel warning");
...
{
	pr_warn
	    ("%s() mCodec_data->mAudio_Ana_Mux[AUDIO_ANALOG_MUX_IN_PREAMP_2]; = %d\n",
...
{
	pr_warn("%s\n", __func__);
	/*Ana_Set_Reg(MT6353_TOP_CLKSQ, 0x0, 0x0001);*/
	pmic_set_register_value(PMIC_RG_CLKSQ_EN_AUD , 0x0);
	/* Disable CLKSQ 26MHz */

	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON8, 0x0002, 0x0002);
	/* disable AUDGLB */
	/*Ana_Set_Reg(MT6353_CLK_CKPDN_CON0_SET, 0x0070, 0x0070);*/
	...
	Ana_Set_Reg(MT6353_AUDDEC_ANA_CON0, 0xe000, 0xe000);
