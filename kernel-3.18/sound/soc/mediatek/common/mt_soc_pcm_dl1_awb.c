...
static void StopAudioDl1AWBHardware(struct snd_pcm_substream *substream)
{
	...
	SetIntfConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_AFE_IO_Block_MEM_DL1, Soc_Aud_AFE_IO_Block_MEM_AWB);

	/* for DL2 echoref*/
	SetIntfConnection(Soc_Aud_InterCon_DisConnect, Soc_Aud_AFE_IO_Block_MEM_DL2, Soc_Aud_AFE_IO_Block_MEM_AWB);
...
static void StartAudioDl1AWBHardware(struct snd_pcm_substream *substream)
{
	...
	/* for DL2 echoref*/
	SetIntfConnection(Soc_Aud_InterCon_Connection, Soc_Aud_AFE_IO_Block_MEM_DL2, Soc_Aud_AFE_IO_Block_MEM_AWB);
				...
				void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (Awb_Block->u4DataRemained >  Awb_Block->u4BufferSize) {
		pr_warn("%s u4DataRemained=%x > u4BufferSize=%x" ,
			__func__, Awb_Block->u4DataRemained, Awb_Block->u4BufferSize);
	...
	if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize) {
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_err("%s Fail 1 copy to user Ptr:%p, BufAddr:%p, ReadIdx:0x%x, Read_Ptr:%zu,read_size:%zu",
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			pr_warn("%s 2, read_size1:0x%x, DataRemained:0x%x, Ptr:%zu, ReadIdx:0x%x \r\n",
		...
		if (copy_to_user((void __user *)Read_Data_Ptr, (Awb_Block->pucVirtBufAddr + DMA_Read_Ptr), size_1)) {
			...
			pr_err("%s Fail 2 copy to user Ptr:%p, BufAddr:%p, ReadIdx:0x%x, Read_Ptr:%zu,read_size:%zu",
			       __func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr,
			       Awb_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
			return 0;
		}
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			...
			PRINTK_AUD_AWB("%s 3, read_size2:%x, Remained:%x, Ptr:0x%x, DMAReadIdx:%x \r\n",
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_warn("%s Fail 3 copy to user Ptr:%p, BufAddr:%p, ReadIdx:0x%x , Read_Ptr:%zu, read_size:%zu",
				__func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr,
				Awb_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
