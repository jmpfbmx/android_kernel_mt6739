						 ...
						 *substream)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_DL1) == true) {
		...
		Afe_consumed_bytes = Align64ByteSize(Afe_consumed_bytes);	/* TODO: KC: is this necessary? */
		...
		if (Afe_Block->u4DataRemained < 0) {
			...
			/*pr_warn("[AudioWarn] HW_Cur_ReadIdx =0x%x
			HW_memory_index = 0x%x Afe_consumed_bytes  = 0x%x\n",
			       HW_Cur_ReadIdx,
