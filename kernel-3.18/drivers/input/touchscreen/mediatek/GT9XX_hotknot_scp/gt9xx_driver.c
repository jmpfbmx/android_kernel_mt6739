...
static ssize_t store_scp_ctrl(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case 3:
		...
		ipi_pkt.cmd = IPI_COMMAND_AS_GESTURE_SWITCH, ipi_pkt.param.data = 1;
		md32_ipi_send(IPI_TOUCH, &ipi_pkt, sizeof(ipi_pkt), 0);
		break;
	case 4:
		/* emulate in-pocket off*/
		ipi_pkt.cmd = IPI_COMMAND_AS_GESTURE_SWITCH, ipi_pkt.param.data = 0;
		md32_ipi_send(IPI_TOUCH, &ipi_pkt, sizeof(ipi_pkt), 0);
		break;
	case 5:
		{
	...
	default:
		GTP_INFO("[SCP_CTRL] Unknown command");
		break;
	}
...
void tpd_enter_doze(void)
{
	...
	if (tpd_scp_doze_en) {
		...
		if (ret < 0) {
			...
			if (retry <= 0)
				GTP_ERROR("########################## md32_ipi_send failed retry=%d", retry);
