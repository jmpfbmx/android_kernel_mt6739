...
#define VDO_FPGAPORTING
#endif

#ifdef VDO_FPGAPORTING
#define enable_clock(...)
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (request_irq(VDEC_IRQ_ID , (irq_handler_t)video_intr_dlr, IRQF_TRIGGER_NONE , VCODEC_DEVNAME, NULL) < 0) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
	...
	if (request_irq(VENC_IRQ_ID , (irq_handler_t)video_intr_dlr2, IRQF_TRIGGER_NONE , VCODEC_DEVNAME, NULL) < 0) {
		/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
...
static const struct of_device_id vcodec_of_match[] = {
	{ .compatible = "mediatek,mt6757-vdec_gcon", },
	{/* sentinel */}
};
...
static int __init vcodec_driver_init(void)
{
...

	{
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt6757-venc");
...

	{
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt6757-vdec");
...
MODULE_DESCRIPTION("Everest Vcodec Driver");
MODULE_LICENSE("GPL");
