...
INT32 ppg_control_process(struct ppg_control_t *ppg_control_input)
{
	...
	pr_err("MT6381_AGC start, timer%d=%d, state=%d, led=%d, idx=%d, fs=%d, l=%d\n", ch,
...
#if defined(LOG_PPG_CONTROL_ENABLE)
			pr_err("MT6381_AGC input, PPG%d = %d, LED = %d, AMB = %d, idx = %d\n", ch,
...
#if defined(LOG_PPG_CONTROL_ENABLE)
		pr_err("MT6381_AGC write, state%d = %d->%d, led = %d, step = %d\n", ch,
