...
static long AKECS_GetData(char *rbuf, int size)
{
	...
	if (size < SENSOR_DATA_SIZE) {
		MAG_PR_ERR("buff size is too small %d!\n", size);
		return -1;
	}
	...
	for (loop_i = 0; loop_i < AKM09915_RETRY_COUNT; loop_i++) {
		...
		if (ret) {
			MAG_PR_ERR("read ST1 resigster failed!\n");
			return -1;
		}
	...
	if (loop_i >= AKM09915_RETRY_COUNT) {
		MAG_INFO("Data read retry larger the max count!\n");
		if (factory_mode == 0)
...
#endif
	if (ret < 0) {
		MAG_PR_ERR("AKM8975 akm8975_work_func: I2C failed\n");
		return -1;
	}
	rbuf[0] = temp;
#ifdef AKM_Device_AK8963
...
int FST_AK09915(void)
{
	...
	/* TEST */
	TEST_DATA(TLIMIT_NO_RST_WIA1_09915, TLIMIT_TN_RST_WIA1_09915, (int)i2cData[0],
		  TLIMIT_LO_RST_WIA1_09915, TLIMIT_HI_RST_WIA1_09915, &pf_total);
	TEST_DATA(TLIMIT_NO_RST_WIA2_09915, TLIMIT_TN_RST_WIA2_09915, (int)i2cData[1],
		  TLIMIT_LO_RST_WIA2_09915, TLIMIT_HI_RST_WIA2_09915, &pf_total);
	...
	/* TEST */
	/* i2cData[0] &= 0x7F; */
	TEST_DATA(TLIMIT_NO_SNG_ST1_09915, TLIMIT_TN_SNG_ST1_09915, (int)i2cData[0],
		  TLIMIT_LO_SNG_ST1_09915, TLIMIT_HI_SNG_ST1_09915, &pf_total);

	/* TEST */
	TEST_DATA(TLIMIT_NO_SNG_HX_09915, TLIMIT_TN_SNG_HX_09915, hdata[0], TLIMIT_LO_SNG_HX_09915,
		  TLIMIT_HI_SNG_HX_09915, &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HY_09915, TLIMIT_TN_SNG_HY_09915, hdata[1], TLIMIT_LO_SNG_HY_09915,
		  TLIMIT_HI_SNG_HY_09915, &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HZ_09915, TLIMIT_TN_SNG_HZ_09915, hdata[2], TLIMIT_LO_SNG_HZ_09915,
		  TLIMIT_HI_SNG_HZ_09915, &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_ST2_09915, TLIMIT_TN_SNG_ST2_09915, (int)i2cData[8],
		  TLIMIT_LO_SNG_ST2_09915, TLIMIT_HI_SNG_ST2_09915, &pf_total);
	...
	/* TEST */
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SLF_ST1_09915, TLIMIT_TN_SLF_ST1_09915, (int)i2cData[0],
		  TLIMIT_LO_SLF_ST1_09915, TLIMIT_HI_SLF_ST1_09915, &pf_total);
	...
	/* TEST */
	TEST_DATA(TLIMIT_NO_SLF_RVHX_09915,
		  TLIMIT_TN_SLF_RVHX_09915,
		  hdata[0], TLIMIT_LO_SLF_RVHX_09915, TLIMIT_HI_SLF_RVHX_09915, &pf_total);

	TEST_DATA(TLIMIT_NO_SLF_RVHY_09915,
		  TLIMIT_TN_SLF_RVHY_09915,
		  hdata[1], TLIMIT_LO_SLF_RVHY_09915, TLIMIT_HI_SLF_RVHY_09915, &pf_total);

	TEST_DATA(TLIMIT_NO_SLF_RVHZ_09915,
		  TLIMIT_TN_SLF_RVHZ_09915,
		  hdata[2], TLIMIT_LO_SLF_RVHZ_09915, TLIMIT_HI_SLF_RVHZ_09915, &pf_total);

	TEST_DATA(TLIMIT_NO_SLF_ST2_09915,
		  TLIMIT_TN_SLF_ST2_09915,
		  (int)i2cData[8], TLIMIT_LO_SLF_ST2_09915, TLIMIT_HI_SLF_ST2_09915, &pf_total);
...
static ssize_t store_layout_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret != 0) {
		...
		if (!hwmsen_get_convert(layout, &data->cvt))
			MAG_PR_ERR("HWMSEN_GET_CONVERT function error!\r\n");
		else if (!hwmsen_get_convert(data->hw->direction, &data->cvt))
			MAG_PR_ERR("invalid layout: %d, restore to %d\n", layout, data->hw->direction);
		else {
			MAG_PR_ERR("invalid layout: (%d, %d)\n", layout, data->hw->direction);
			hwmsen_get_convert(0, &data->cvt);
		}
	} else
		MAG_PR_ERR("invalid format = '%s'\n", buf);
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		MAG_PR_ERR("akm09915_i2c_data is null!!\n");
		return 0;
	}
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "0x%x", &trace) == 1)
		...
		MAG_PR_ERR("invalid content: '%s', length = %zu\n", buf, count);
...
static int akm09915_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err) {
			MAG_PR_ERR("driver_create_file (%s) = %d\n",
...
static long akm09915_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MSENSOR_IOCTL_READ_CHIPINFO:
		if (argp == NULL) {
			MAG_PR_ERR("IO parameter pointer is NULL!\r\n");
			break;
		}
	...
	default:
		MAG_PR_ERR("%s not supported = 0x%04x", __func__, cmd);
		return -ENOIOCTLCMD;
	}
...
static long akm09915_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_ECS_IOCTL_WRITE:
		...
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_WRITE, (unsigned long)arg32);
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_RESET:
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_RESET, (unsigned long)arg32);
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_READ:
		...
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_READ, (unsigned long)arg32);
		if (ret) {
			MAGN_LOG("ECS_IOCTL_WRITE unlocked_ioctl failed.");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_INFO:
		...
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_INFO, (unsigned long)(arg32));
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_GET_CONF:
		...
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_CONF, (unsigned long)(arg32));
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_SET_MODE:
		...
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_SET_MODE, (unsigned long)(arg32));
		if (ret) {
	...
	case COMPAT_ECS_IOCTL_GETDATA:
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GETDATA, (unsigned long)(arg32));
		if (ret) {
		     ...
		     void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
			...
			if (atomic_read(&data->trace) & AMK_HWM_DEBUG) {
				MAGN_LOG
				    ("Hwm get m-sensor data: %d, %d, %d. divide %d, status %d!\n",
				     ...
				     msensor_data->values[2], msensor_data->value_divide,
				     msensor_data->status);
			}
#endif
		}
		break;
	default:
		MAG_PR_ERR("msensor operate function no this parameter %d!\n", command);
		err = -1;
		break;
	}
				 ...
				 void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
			...
			if (atomic_read(&data->trace) & AMK_HWM_DEBUG) {
				MAGN_LOG
				    ("Hwm get o-sensor data: %d, %d, %d. divide %d, status %d!\n",
				     ...
				     osensor_data->values[2], osensor_data->value_divide,
				     osensor_data->status);
			}
#endif
		}
		break;
	default:
		MAG_PR_ERR("gsensor operate function no this parameter %d!\n", command);
		err = -1;
		break;
	}
			...
			if (atomic_read(&data->trace) & AMK_HWM_DEBUG) {
				MAGN_LOG
				    ("Hwm get gyro-sensor data: %d, %d, %d. divide %d, status %d!\n",
				     gyrosensor_data->values[0], gyrosensor_data->values[1],
				     gyrosensor_data->values[2], gyrosensor_data->value_divide,
			...
			if (atomic_read(&data->trace) & AMK_HWM_DEBUG) {
				MAGN_LOG
				    ("Hwm get gravity-sensor data: %d, %d, %d. divide %d, status %d!\n",
				     gravity_data->values[0], gravity_data->values[1],
				     gravity_data->values[2], gravity_data->value_divide,
				     gravity_data->status);
			}
#endif
		}
...
static int akm09915_enable(int en)
{
	...
	if (value == 1) {
		...
		if (err < 0) {
			MAG_PR_ERR("%s:AKECS_SetMode Error.\n", __func__);
			return err;
		}
	} else {
		atomic_set(&m_flag, 0);
...
static int akm09915_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		MAG_PR_ERR("AKM09915 akm09915_probe: check device connect error\n");
	...
	if (err) {
		MAG_PR_ERR("create attribute err = %d\n", err);
		goto exit_sysfs_create_group_failed;
	}

	err = misc_register(&akm09915_device);
	if (err) {
		MAG_PR_ERR("akm09915_device register failed\n");
...
static int akm09915_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		MAG_PR_ERR("akm09915_delete_attr fail: %d\n", err);
...
static int akm09915_local_init(void)
{
	if (i2c_add_driver(&akm09915_i2c_driver)) {
		MAG_PR_ERR("i2c_add_driver error\n");
		return -1;
	}
