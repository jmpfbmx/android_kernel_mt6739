...
static void ccci_aed_v1(struct md_ee *mdee, unsigned int dump_flag, char *aed_str, int db_opt)
{
...
static void mdee_dumper_info_dump_v1(struct md_ee *mdee)
{
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
	int md_id = mdee->md_id;
	...
	case MD_EX_TYPE_ASSERT:
		CCCI_NORMAL_LOG(md_id, KERN, "filename = %s\n", debug_info->assert.file_name);
		CCCI_NORMAL_LOG(md_id, KERN, "line = %d\n", debug_info->assert.line_num);
		CCCI_NORMAL_LOG(md_id, KERN, "para0 = %d, para1 = %d, para2 = %d\n",
	...
	case MD_EX_TYPE_C2K_ERROR:
		CCCI_NORMAL_LOG(md_id, KERN, "fatal error code 1 = %d\n", debug_info->fatal_error.err_code1);
		CCCI_NORMAL_LOG(md_id, KERN, "fatal error code 2 = %d\n", debug_info->fatal_error.err_code2);
		CCCI_NORMAL_LOG(md_id, KERN, "fatal error offender %s\n", debug_info->fatal_error.offender);
	...
	case MD_EX_TYPE_EMI_CHECK:
		CCCI_NORMAL_LOG(md_id, KERN, "md_emi_check: %08X, %08X, %02d, %08X\n",
	...
	case DSP_EX_TYPE_ASSERT:
		...
		CCCI_NORMAL_LOG(md_id, KERN, "exec unit = %s\n", debug_info->dsp_assert.execution_unit);
		CCCI_NORMAL_LOG(md_id, KERN, "para0 = %d, para1 = %d, para2 = %d\n",
	...
	case DSP_EX_TYPE_EXCEPTION:
		CCCI_NORMAL_LOG(md_id, KERN, "exec unit = %s, code1:0x%08x\n",
	...
	case DSP_EX_FATAL_ERROR:
		...
		CCCI_NORMAL_LOG(md_id, KERN, "err_code0 = 0x%08x, err_code1 = 0x%08x\n",
...
err_exit:
	...
	if ((md_id == MD_SYS3) || (dumper->more_info == MD_EE_CASE_NORMAL && md_state == BOOT_WAITING_FOR_HS1))
		ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP, &dumper->ex_info, sizeof(EX_LOG_T));
	else if (md_dbg_dump_flag & (1 << MD_DBG_DUMP_SMEM))
		ccci_mem_dump(md_id, smem_layout->ccci_exp_smem_base_vir, smem_layout->ccci_exp_dump_size);
	...
	if (debug_info->type == MD_EX_TYPE_C2K_ERROR && debug_info->fatal_error.err_code1 == MD_EX_C2K_FATAL_ERROR)
		CCCI_ERROR_LOG(md_id, KERN, "C2K EE, No need trigger DB\n");
	else if (debug_info->type == CC_MD1_EXCEPTION)
		CCCI_ERROR_LOG(md_id, KERN, "MD1 EE, No need trigger DB\n");
...
static void mdee_dumper_info_prepare_v1(struct md_ee *mdee)
{
	EX_LOG_T *ex_info;
	int ee_type, ee_case;
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
	...
	if ((md_id == MD_SYS3) ||
	(dumper->more_info == MD_EE_CASE_NORMAL && ccci_md_get_state(mdee->md_obj) == BOOT_WAITING_FOR_HS1)) {
		ex_info = &dumper->ex_info;
		CCCI_DEBUG_LOG(md_id, KERN, "Parse ex info from ccci packages\n");
	} else {
		ex_info = (EX_LOG_T *) smem_layout->ccci_exp_smem_mdss_debug_vir;
		CCCI_DEBUG_LOG(md_id, KERN, "Parse ex info from shared memory\n");
	}
	...
	debug_info->md_image = (void *)mem_layout->md_region_vir;
	debug_info->md_size = MD_IMG_DUMP_SIZE;
}
static void mdee_dumper_v1_set_ee_pkg(struct md_ee *mdee, char *data, int len)
{
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
...
static void mdee_dumper_v1_dump_ee_info(struct md_ee *mdee, MDEE_DUMP_LEVEL level, int more_info)
{
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
	int md_id = mdee->md_id;
	struct ccci_smem_layout *smem_layout = ccci_md_get_smem(mdee->md_obj);
	...
	if (level == MDEE_DUMP_LEVEL_BOOT_FAIL) {
		if (md_state == BOOT_WAITING_FOR_HS1) {
			...
			/* Handshake 1 fail */
			ccci_aed_v1(mdee, CCCI_AED_DUMP_CCIF_REG | CCCI_AED_DUMP_MD_IMG_MEM, ex_info, DB_OPT_DEFAULT);
			...
			ccci_aed_v1(mdee, CCCI_AED_DUMP_CCIF_REG | CCCI_AED_DUMP_EX_MEM, ex_info, DB_OPT_FTRACE);
		}
		...
		if (md_dbg_dump_flag & (1 << MD_DBG_DUMP_SMEM)) {
			...
			ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP,
				smem_layout->ccci_exp_smem_base_vir, smem_layout->ccci_exp_dump_size);
		}
...
int mdee_dumper_v1_alloc(struct md_ee *mdee)
{
	...
	if (dumper == NULL) {
		CCCI_ERROR_LOG(md_id, KERN, "%s:alloc mdee_parser_v1 fail\n", __func__);
		return -1;
	}
