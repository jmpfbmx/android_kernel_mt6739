...
static void _ext_disp_vsync_lock(void)
{
	mutex_lock(&(pgc->vsync_lock));
}

static void _ext_disp_vsync_unlock(void)
{
...
#ifdef CONFIG_SINGLE_PANEL_OUTPUT
	_ext_disp_vsync_lock();
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC);
	_ext_disp_vsync_unlock();
#else
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC);
#endif
...
#ifdef CONFIG_SINGLE_PANEL_OUTPUT
	extd_mutex_init(&(pgc->vsync_lock));
#endif
	_ext_disp_path_lock();
...
int ext_disp_wait_for_vsync(void *config, unsigned int session)
{
	...
	_ext_disp_path_lock();
#ifdef CONFIG_SINGLE_PANEL_OUTPUT
		if ((pgc->state != EXTD_INIT && pgc->state != EXTD_RESUME) || !is_hdmi_video_on()) {
			EXT_DISP_ERR("ext display path not ready yet\n");
			_ext_disp_path_unlock();
			msleep(20);
			return -1;
		}
	...
	if (pgc->dpmgr_handle == NULL) {
		EXT_DISP_ERR("vsync for ext display path not ready yet\n");
		_ext_disp_vsync_unlock();
		return -1;
	}
...
#endif
	if (ret == -2) {
		EXT_DISP_ERR("vsync for ext display path not enabled yet\n");
		_ext_disp_vsync_unlock();
		return -1;
	}
