...
/* Internal Functions                                                    */
/* ===================================================================== */
static int rt5081a_set_aicr(struct charger_device *chg_dev, u32 uA);
static int rt5081a_get_aicr(struct charger_device *chg_dev, u32 *uA);
static int rt5081a_set_ichg(struct charger_device *chg_dev, u32 uA);
static int rt5081a_get_ichg(struct charger_device *chg_dev, u32 *uA);
static int rt5081a_enable_charging(struct charger_device *chg_dev, bool en);
	...
	enum rt5081a_adc_sel adc_sel, int *adc_val)
{
	...
	if (adc_sel == RT5081A_ADC_IBUS) {
		...
		ret = rt5081a_get_aicr(chg_data->chg_dev, &aicr);
		if (ret < 0) {
		...
		ret = rt5081a_get_ichg(chg_data->chg_dev, &ichg);
		if (ret < 0) {
...
#ifdef CONFIG_RT5081A_PMU_CHARGER_TYPE_DETECT
static int rt5081a_inform_psy_changed(struct rt5081a_pmu_charger_data *chg_data)
{
	int ret = 0;
...
out:
	...
	if (inform_psy)
		rt5081a_inform_psy_changed(chg_data);
	...
	bool en)
{
	...
	ret = rt5081a_set_aicr(chg_data->chg_dev, 800000);
	if (ret < 0)
		return ret;

	ret = rt5081a_set_ichg(chg_data->chg_dev, 2000000);
	if (ret < 0)
		return ret;

	ret = rt5081a_enable_charging(chg_data->chg_dev, true);
	if (ret < 0)
		return ret;
...
static int __rt5081a_set_ieoc(struct rt5081a_pmu_charger_data *chg_data, u32 ieoc)
{
	...
	u8 reg_ieoc = rt5081a_find_closest_reg_value(
		RT5081A_IEOC_MIN,
		RT5081A_IEOC_MAX,
		RT5081A_IEOC_STEP,
		RT5081A_IEOC_NUM,
...
static int _rt5081a_set_aicr(struct rt5081a_pmu_charger_data *chg_data, u32 uA)
{
	int ret = 0;
	u8 reg_aicr = 0;
	...
	u32 *ichg)
{
	...
	reg_ichg = (ret & RT5081_MASK_ICHG) >> RT5081A_SHIFT_ICHG;
	*ichg = rt5081a_find_closest_real_value(RT5081A_ICHG_MIN, RT5081A_ICHG_MAX,
		RT5081A_ICHG_STEP, reg_ichg);
...
static int rt5081a_enable_hz(struct rt5081a_pmu_charger_data *chg_data, bool en)
{
	int ret = 0;
...
static int rt5081a_set_ircmp_resistor(struct rt5081a_pmu_charger_data *chg_data,
	u32 uohm)
{
	int ret = 0;
	u8 reg_resistor = 0;
...
static int rt5081a_set_ircmp_vclamp(struct rt5081a_pmu_charger_data *chg_data,
	u32 uV)
{
	int ret = 0;
	u8 reg_vclamp = 0;
...
static int rt5081a_enable_safety_timer(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	ret = __rt5081a_enable_safety_timer(chg_data, en);

	return ret;
}

static int rt5081a_enable_te(struct charger_device *chg_dev, bool en)
{
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	return __rt5081a_enable_te(chg_data, en);
}

static int rt5081a_reset_eoc_state(struct charger_device *chg_dev)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static int rt5081a_is_safety_timer_enable(struct charger_device *chg_dev,
	bool *en)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
	...
	ret = rt5081a_pmu_reg_test_bit(chg_data->chip, RT5081A_PMU_REG_CHGCTRL12,
		RT5081A_SHIFT_TMR_EN, en);

	return ret;
}

static int rt5081a_enable_power_path(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static int rt5081a_is_power_path_enable(struct charger_device *chg_dev, bool *en)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	...
	ret = rt5081a_get_mivr(chg_data, &mivr);
	*en = (mivr == RT5081A_MIVR_MAX ? false : true);

	return ret;
}

static int rt5081a_get_ichg(struct charger_device *chg_dev, u32 *ichg)
{
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	return __rt5081a_get_ichg(chg_data, ichg);
}
...
static int rt5081a_set_ieoc(struct charger_device *chg_dev, u32 uA)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	...
	mutex_lock(&chg_data->ichg_access_lock);
	mutex_lock(&chg_data->ieoc_lock);
	ret = __rt5081a_set_ieoc(chg_data, uA);
	mutex_unlock(&chg_data->ieoc_lock);
	mutex_unlock(&chg_data->ichg_access_lock);
...
static int rt5081a_get_aicr(struct charger_device *chg_dev, u32 *aicr)
{
	int ret = 0;
	u8 reg_aicr = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_set_aicr(struct charger_device *chg_dev, u32 uA)
{
	...
	mutex_lock(&chg_data->aicr_access_lock);
	ret = _rt5081a_set_aicr(chg_data, uA);
	mutex_unlock(&chg_data->aicr_access_lock);

	return ret;
}

static int rt5081a_set_mivr(struct charger_device *chg_dev, u32 uV)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	...
	ret = rt5081a_is_power_path_enable(chg_dev, &en);
	if (!en) {
...
static int rt5081a_get_cv(struct charger_device *chg_dev, u32 *cv)
{
	int ret = 0;
	u8 reg_cv = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_set_cv(struct charger_device *chg_dev, u32 uV)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);

	ret = _rt5081a_set_cv(chg_data, uV);

	return ret;
}

static int rt5081a_set_otg_current_limit(struct charger_device *chg_dev, u32 uA)
{
	int ret = 0;
	u8 reg_ilimit = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_enable_otg(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	bool en_otg = false;
	struct rt5081a_pmu_charger_data *chg_data =
	...
	/* Set OTG_OC to 500mA */
	ret = rt5081a_set_otg_current_limit(chg_dev, 500000);
	if (ret < 0) {
...
static int rt5081a_enable_discharge(struct charger_device *chg_dev, bool en)
{
	int ret = 0, i = 0;
...
static int rt5081a_set_pep_current_pattern(struct charger_device *chg_dev,
	bool is_increase)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_set_pep20_reset(struct charger_device *chg_dev)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	...
	mutex_lock(&chg_data->pe_access_lock);
	ret = rt5081a_set_mivr(chg_dev, 4500000);
	if (ret < 0)
		goto out;
	...
	ret = rt5081a_set_aicr(chg_dev, 100000);
	if (ret < 0)
		goto out;

	msleep(250);

	ret = rt5081a_set_aicr(chg_dev, 700000);
...
static int rt5081a_set_pep20_current_pattern(struct charger_device *chg_dev,
	u32 uV)
{
	int ret = 0;
	u8 reg_volt = 0;
...
static int rt5081a_set_pep20_efficiency_table(struct charger_device *chg_dev)
{
...
static int rt5081a_enable_cable_drop_comp(struct charger_device *chg_dev,
	bool en)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_is_charging_done(struct charger_device *chg_dev, bool *done)
{
	int ret = 0;
...
static int rt5081a_kick_wdt(struct charger_device *chg_dev)
{
...
static int rt5081a_enable_direct_charge(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static int rt5081a_set_dc_vbusov(struct charger_device *chg_dev, u32 uV)
{
	int ret = 0;
	u8 reg_vbusov = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_set_dc_ibusoc(struct charger_device *chg_dev, u32 uA)
{
	int ret = 0;
	u8 reg_ibusoc = 0;
	struct rt5081a_pmu_charger_data *chg_data =
...
static int rt5081a_kick_dc_wdt(struct charger_device *chg_dev)
{
...
static int rt5081a_get_tchg(struct charger_device *chg_dev, int *tchg_min,
	int *tchg_max)
{
...
static int rt5081a_get_ibus(struct charger_device *chg_dev, u32 *ibus)
{
...
static int rt5081a_plug_out(struct charger_device *chg_dev)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	...
	/* Disable charger */
	ret = rt5081a_enable_charging(chg_dev, false);
	if (ret < 0) {
...
static int rt5081a_plug_in(struct charger_device *chg_dev)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
	...
	/* Enable charger */
	ret = rt5081a_enable_charging(chg_dev, true);
	if (ret < 0) {
...
static int rt5081a_run_aicl(struct charger_device *chg_dev, u32 *uA)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static int rt5081a_get_min_ichg(struct charger_device *chg_dev, u32 *uA)
{
	*uA = 500000;
	return 0;
}

static int rt5081a_get_min_aicr(struct charger_device *chg_dev, u32 *uA)
{
	*uA = 100000;
	return 0;
}

static int rt5081a_dump_register(struct charger_device *chg_dev)
{
	int i = 0, ret = 0;
	...
	ret = rt5081a_get_ichg(chg_dev, &ichg);
	ret = rt5081a_get_aicr(chg_dev, &aicr);
	ret = rt5081a_get_charging_status(chg_data, &chg_status);
	ret = rt5081a_get_ieoc(chg_data, &ieoc);
	ret = rt5081a_get_mivr(chg_data, &mivr);
	ret = rt5081a_get_cv(chg_dev, &cv);
...
static int rt5081a_enable_chg_type_det(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
...
static int rt5081a_get_zcv(struct charger_device *chg_dev, u32 *uV)
{
	struct rt5081a_pmu_charger_data *chg_data =
		dev_get_drvdata(&chg_dev->dev);
...
static int rt5081a_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
	switch (event) {
	case EVENT_EOC:
...
static irqreturn_t rt5081a_pmu_chg_wdtmri_irq_handler(int irq, void *data)
{
	...
	dev_err(chg_data->dev, "%s\n", __func__);
	ret = rt5081a_kick_wdt(chg_data->chg_dev);
	if (ret < 0)
...
static int rt5081a_chg_init_setting(struct rt5081a_pmu_charger_data *chg_data)
{
	...
	/* Disable HZ */
	ret = rt5081a_enable_hz(chg_data, false);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: disable hz failed\n", __func__);

	ret = rt5081a_set_ircmp_resistor(chg_data, chg_desc->ircmp_resistor);
	if (ret < 0)
		dev_err(chg_data->dev,
			"%s: set IR compensation resistor failed\n", __func__);

	ret = rt5081a_set_ircmp_vclamp(chg_data, chg_desc->ircmp_vclamp);
	if (ret < 0)
		dev_err(chg_data->dev,
...
static struct charger_ops rt5081a_chg_ops = {
	/* Normal charging */
	.plug_out = rt5081a_plug_out,
	.plug_in = rt5081a_plug_in,
	.dump_registers = rt5081a_dump_register,
	.enable = rt5081a_enable_charging,
	.get_charging_current = rt5081a_get_ichg,
	.set_charging_current = rt5081a_set_ichg,
	.get_input_current = rt5081a_get_aicr,
	.set_input_current = rt5081a_set_aicr,
	.get_constant_voltage = rt5081a_get_cv,
	.set_constant_voltage = rt5081a_set_cv,
	.kick_wdt = rt5081a_kick_wdt,
	.set_mivr = rt5081a_set_mivr,
	.is_charging_done = rt5081a_is_charging_done,
	.get_zcv = rt5081a_get_zcv,
	.run_aicl = rt5081a_run_aicl,
	.set_eoc_current = rt5081a_set_ieoc,
	.enable_termination = rt5081a_enable_te,
	.reset_eoc_state = rt5081a_reset_eoc_state,
	.get_min_charging_current = rt5081a_get_min_ichg,
	.get_min_input_current = rt5081a_get_min_aicr,

	/* Safety timer */
	.enable_safety_timer = rt5081a_enable_safety_timer,
	.is_safety_timer_enabled = rt5081a_is_safety_timer_enable,

	/* Power path */
	.enable_powerpath = rt5081a_enable_power_path,
	.is_powerpath_enabled = rt5081a_is_power_path_enable,

	/* Charger type detection */
	.enable_chg_type_det = rt5081a_enable_chg_type_det,

	/* OTG */
	.enable_otg = rt5081a_enable_otg,
	.set_boost_current_limit = rt5081a_set_otg_current_limit,
	.enable_discharge = rt5081a_enable_discharge,

	/* PE+/PE+20 */
	.send_ta_current_pattern = rt5081a_set_pep_current_pattern,
	.set_pe20_efficiency_table = rt5081a_set_pep20_efficiency_table,
	.send_ta20_current_pattern = rt5081a_set_pep20_current_pattern,
	.set_ta20_reset = rt5081a_set_pep20_reset,
	.enable_cable_drop_comp = rt5081a_enable_cable_drop_comp,

	/* ADC */
	.get_tchg_adc = rt5081a_get_tchg,
	.get_ibus_adc = rt5081a_get_ibus,

	/* Event */
	.event = rt5081a_do_event,
};


static struct charger_ops rt5081a_ls_ops = {
	/* Direct charging */
	.enable_direct_charging = rt5081a_enable_direct_charge,
	.set_direct_charging_vbusov = rt5081a_set_dc_vbusov,
	.set_direct_charging_ibusoc = rt5081a_set_dc_ibusoc,
	.kick_direct_charging_wdt = rt5081a_kick_dc_wdt,
	.get_tchg_adc = rt5081a_get_tchg,
	.get_ibus_adc = rt5081a_get_ibus,
};
...
static int rt5081a_pmu_charger_probe(struct platform_device *pdev)
{
	...
	chg_data->chg_dev = charger_device_register(
		...
		chg_data->dev, chg_data, &rt5081a_chg_ops, &chg_data->chg_props);
	...
	chg_data->ls_dev = charger_device_register(
		...
		chg_data->dev, chg_data, &rt5081a_ls_ops, &chg_data->ls_props);
	...
	rt5081a_dump_register(chg_data->chg_dev);
