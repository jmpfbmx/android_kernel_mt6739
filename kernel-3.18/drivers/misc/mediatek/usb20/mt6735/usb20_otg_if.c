...
void musb_h_in_status(void)
{
	...
	if (csr0 & MUSB_CSR0_RXPKTRDY) {
		csr0 &= ~MUSB_CSR0_RXPKTRDY;
		/* whether this bit will be cleared auto, need to clear by sw?? */
...
TD6_13:do {
		...
		if (intrtx || (intrusb & 0xf7)) {
			...
			DBG(0, "power=0x%x,devctl=0x%x,intrtx=0x%x,intrusb=0x%x\n",
			    musb_readb(mtk_musb->mregs, MUSB_POWER), musb_readb(mtk_musb->mregs,
										MUSB_DEVCTL),
			    musb_readw(mtk_musb->mregs, MUSB_INTRTX), musb_readb(mtk_musb->mregs,
										 MUSB_INTRUSB));
			wait_for_completion_timeout(&stop_event, 1);
		}
	} while (g_exec);
	...
	/* DBG(0,"power=0x%x,devctl=0x%x,intrusb=0x%x\n",
	   musb_readb(mtk_musb->mregs,MUSB_POWER),
	   musb_readb(mtk_musb->mregs,MUSB_DEVCTL),musb_readb(mtk_musb->mregs,MUSB_INTRUSB)); */
	if (TEST_IS_STOP == ret)
		return TEST_IS_STOP;
...
static long musb_otg_test_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
