...
#define CMDQ_REG_SET32(addr, val)    mt65xx_reg_sync_writel(val, (unsigned long)(addr))
#endif
#define CMDQ_REG_SET32(addr, val)    mt_reg_sync_writel(val, (addr))
#if 1
#define CMDQ_REG_GET32(addr)         ((*((volatile uint32_t *)((unsigned long)addr))) & 0xFFFFFFFF)
#define CMDQ_REG_GET16(addr)         ((*((volatile uint32_t *)((unsigned long)addr))) & 0x0000FFFF)
#else
