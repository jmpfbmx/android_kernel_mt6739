					       ...
					       void *_pIwc)
{
	...
	if (CMDQ_INVALID_THREAD != thread) {
		...
		list_for_each_entry(cmd_buffer, &pTask->cmd_buffer_list, listEntry) {
			...
			uint32_t *end_va = (uint32_t *)(current_va + copy_size);
					       ...
					       void *data)
{
	...
	} else if (CMD_CMDQ_TL_PATH_RES_ALLOCATE == iwcCommand
		   || CMD_CMDQ_TL_PATH_RES_RELEASE == iwcCommand) {
		/* do nothing */
	} else {
					    ...
					    cmdqSecCancelTaskResultStruct *pResult)
{
	...
	if ((NULL == pTask) || (false == cmdq_get_func()->isSecureThread(thread))
	    || (NULL == pResult)) {
