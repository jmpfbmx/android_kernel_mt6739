...
static struct mt_idle_block idle_block[NR_TYPES] = {
	[IDLE_TYPE_DP] = {
		.name = "dpidle",
...
{
#ifdef SPM_SODI3_PROFILE_TIME
	idle_prof_ver("SODI3: cpu_freq:%u, 1=>2:%u, 2=>3:%u, 3=>4:%u\n",
...
{
#ifdef SPM_SODI_PROFILE_TIME
	idle_prof_ver("SODI: cpu_freq:%u, 1=>2:%u, 2=>3:%u, 3=>4:%u\n",
			mt_cpufreq_get_cur_phy_freq(0),
...
void mt_idle_twam_enable(u32 event)
{
	...
	spm_twam_register_handler(mt_idle_twam_callback);
...
static void mt_idle_dump_cnt(int type)
{
	static struct mt_idle_buf buf;
...
void mt_idle_dump_cnt_in_interval(void)
{
	...
	append_log("DP: ");
	mt_idle_dump_cnt(IDLE_TYPE_DP);

	append_log("SODI3: ");
	mt_idle_dump_cnt(IDLE_TYPE_SO3);
