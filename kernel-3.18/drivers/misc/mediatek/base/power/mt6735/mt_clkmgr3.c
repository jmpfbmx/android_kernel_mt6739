...
static int dis_sys_enable_op(struct subsys *sys)
{
	...
	clk_writel(MMSYS_DUMMY_1, 0xFFFFFFFF);
...
int clkmux_sel(int id, unsigned int clksrc, char *name)
{
	...
	} else if (id == MT_MUX_MSDC30_0) {
		reg = clk_readl(CLK_CFG_3);
		reg &= ~(0x0000000f);
		reg |= (clksrc << 0) & 0x0000000F;
		clk_writel(CLK_CFG_3, reg);
	}
	return 0;
#endif
