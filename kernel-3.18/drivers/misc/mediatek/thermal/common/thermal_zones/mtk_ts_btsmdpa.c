...
static int mtkts_btsmdpa_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "[mtkts_btsmdpa_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,\n",
				   ...
				   loff_t *data)
{
	...
	if (sscanf
		...
		mtkts_btsmdpa_dprintk("g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,",
			g_THERMAL_TRIP[2], g_THERMAL_TRIP[3], g_THERMAL_TRIP[4]);
		mtkts_btsmdpa_dprintk("g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,",
			g_THERMAL_TRIP[5], g_THERMAL_TRIP[6], g_THERMAL_TRIP[7]);
		mtkts_btsmdpa_dprintk("g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
			g_THERMAL_TRIP[8], g_THERMAL_TRIP[9]);

		mtkts_btsmdpa_dprintk("[mtkts_btsmdpa_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,",
			g_bind0, g_bind1, g_bind2, g_bind3);
		mtkts_btsmdpa_dprintk("cooldev4=%s,cooldev5=%s,cooldev6=%s,cooldev7=%s,cooldev8=%s,cooldev9=%s\n",
		...
		mtkts_btsmdpa_dprintk("trip_3_temp=%d,trip_4_temp=%d,trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,",
			trip_temp[3], trip_temp[4], trip_temp[5], trip_temp[6], trip_temp[7]);
		mtkts_btsmdpa_dprintk("trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
...
void mtkts_btsmdpa_prepare_table(int table_num)
{
	...
	case 7:		/* NCP15WF104F03RC */
		...
		BUG_ON(sizeof(BTSMDPA_Temperature_Table) != sizeof(BTSMDPA_Temperature_Table7));
		break;
	default:		/* AP_NTC_10 */
		mtkts_btsmdpa_copy_table(BTSMDPA_Temperature_Table, BTSMDPA_Temperature_Table4);
		BUG_ON(sizeof(BTSMDPA_Temperature_Table) != sizeof(BTSMDPA_Temperature_Table4));
...
static ssize_t mtkts_btsmdpa_param_write(struct file *file, const char __user *buffer,
...
void mtkts_btsmdpa_cancel_thermal_timer(void)
{
	/* cancel timer */
...
void mtkts_btsmdpa_start_thermal_timer(void)
{
...
static int __init mtkts_btsmdpa_init(void)
{
	...
	if (!mtkts_btsmdpa_dir) {
		...
		entry =
		    proc_create("tzbtspa", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_btsmdpa_dir,
				&mtkts_btsmdpa_fops);
		if (entry)
			proc_set_user(entry, uid, gid);

		entry =
		    proc_create("tzbtspa_param", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_btsmdpa_dir,
