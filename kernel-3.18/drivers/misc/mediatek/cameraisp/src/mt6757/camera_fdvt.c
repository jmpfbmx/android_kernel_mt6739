...
static inline void FD_Prepare_ccf_clock(void)
{
	...
	ret = clk_prepare(fd_clk.CG_IMGSYS_LAB5);
	if (ret)
...
static inline void FD_Enable_ccf_clock(void)
{
	...
	ret = clk_enable(fd_clk.CG_IMGSYS_LAB5);
	if (ret)
...
static inline void FD_Prepare_Enable_ccf_clock(void)
{
	...
	ret = clk_prepare_enable(fd_clk.CG_IMGSYS_LAB5);
	if (ret)
...
static inline void FD_Unprepare_ccf_clock(void)
{
	...
	clk_unprepare(fd_clk.CG_IMGSYS_LAB5);
...
static inline void FD_Disable_ccf_clock(void)
{
	...
	clk_disable(fd_clk.CG_IMGSYS_LAB5);
...
static inline void FD_Disable_Unprepare_ccf_clock(void)
{
	...
	clk_disable_unprepare(fd_clk.CG_IMGSYS_LAB5);
...
static int FDVT_probe(struct platform_device *dev)
{
	...
	fd_clk.CG_IMGSYS_LAB5 = devm_clk_get(&dev->dev, "FD_CG_IMG_LARB5");
	...
	if (IS_ERR(fd_clk.CG_IMGSYS_LAB5)) {
		LOG_ERR("cannot get CG_IMGSYS_LAB5 clock\n");
		return PTR_ERR(fd_clk.CG_IMGSYS_LAB5);
	}
