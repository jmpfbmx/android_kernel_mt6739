...
#define CAM_REG_CTL_SPARE0(module)  (isp_devs[module].regs + 0x0050)
...
#endif
	/* Disable CAMSYS_HALT1_EN: LSCI & BPCI & LSC3I, To avoid ISP halt keep arise */
...
static MINT32 ISP_MARK_IRQ(ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->EventInfo.St_type == SIGNAL_INT) {
		...
		IspInfo.IrqInfo.MarkedFlag[irqinfo->Type][irqinfo->EventInfo.UserKey][irqinfo->EventInfo.St_type] |=
...
static MINT32 ISP_probe(struct platform_device *pDev)
{
	...
	if (isp_dev->irq > 0) {
		...
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if ((0 == strcmp(pDev->dev.of_node->name, IRQ_CB_TBL[i].device_name)) &&
...
)
{
	...
	for (IrqType = 0; IrqType < ISP_IRQ_TYPE_AMOUNT; IrqType++) {
		if (0 == strcmp(moduleName, IRQ_CB_TBL[IrqType].device_name))
			break;
	}
		...
		for (i = 0; i < _cam_max_; i++) {
			...
			if (fbc_ctrl1[dma_arry_map[i]].Raw != 0) {
				...
				if (0 == delayCheck) {
					...
					if (0 == fbc_ctrl2[dma_arry_map[i]].Bits.FBC_CNT)
						product *= 1;
					else
	...
	if (bQueMode) {
		...
		if (fbc_ctrl1.Raw != 0) {
			...
			if (0 == delayCheck) {
				...
				/* Prevent *0 for SOF ISR delayed after P1_DON */
				if (0 == (fbc_ctrl1.Bits.FBC_NUM - fbc_ctrl2.Bits.FBC_CNT))
					product *= 1;
				else
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (CAM_FST_DROP_FRAME != FrameStatus[module]) {
				...
				#if (TSTMP_SUBSAMPLE_INTPL == 1)
				if ((frmPeriod > 1) && (MFALSE == g1stSwP1Done[module])) {
					if (IspInfo.TstpQInfo[module].DmaEnStatus[_imgo_])
						ISP_PatchTimestamp(module, _imgo_, frmPeriod,
							cur_timestp, gPrevSofTimestp[module]);
