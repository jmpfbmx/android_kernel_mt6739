...
static spinlock_t	   SpinLockImemDump;
...
static volatile MUINT32 IrqFlush[ISP_IRQ_USER_MAX];
static volatile MUINT32 IrqFlush_v3[IRQ_USER_NUM_MAX];
...
typedef struct {
	...
	/* latest     occuring time for each interrupt */
	...
	volatile ISP_EIS_META_STRUCT Eismeta[ISP_IRQ_TYPE_INT_STATUSX][EISMETA_RINGSIZE];
} ISP_IRQ_INFO_STRUCT;
...
#define	LOG_PPNUM 2
/*volatile static MUINT32 m_CurrentPPB;*/ /* ERROR */
...
static inline void Prepare_ccf_clock(void)
{
	...
	ret = clk_prepare(isp_clk.CG_SCP_SYS_DIS);
	if (ret)
	...
	ret = clk_prepare(isp_clk.CG_SCP_SYS_ISP);
	if (ret)
	...
	ret = clk_prepare(isp_clk.CG_IMAGE_CAM_CAM);
	if (ret)
		LOG_ERR("cannot prepare CG_IMAGE_CAM_CAM clock\n");

	ret = clk_prepare(isp_clk.CG_IMAGE_SEN_TG);
	if (ret)
		LOG_ERR("cannot prepare CG_IMAGE_SEN_TG clock\n");

	ret = clk_prepare(isp_clk.CG_IMAGE_SEN_CAM);
	if (ret)
		LOG_ERR("cannot prepare CG_IMAGE_SEN_CAM clock\n");

	ret = clk_prepare(isp_clk.CG_IMAGE_CAM_SV);
	if (ret)
...
static inline void Prepare_Enable_ccf_clock(void)
{
	...
	ret = clk_prepare_enable(isp_clk.CG_DISP0_SMI_COMMON);
	if (ret)
	...
	ret = clk_prepare_enable(isp_clk.CG_IMAGE_CAM_CAM);
	if (ret)
		LOG_ERR("cannot get CG_IMAGE_CAM_CAM clock\n");

	ret = clk_prepare_enable(isp_clk.CG_IMAGE_SEN_TG);
	if (ret)
		LOG_ERR("cannot get CG_IMAGE_SEN_TG clock\n");

	ret = clk_prepare_enable(isp_clk.CG_IMAGE_SEN_CAM);
	if (ret)
		LOG_ERR("cannot get CG_IMAGE_SEN_CAM clock\n");

	ret = clk_prepare_enable(isp_clk.CG_IMAGE_CAM_SV);
	if (ret)
...
static inline void Enable_ccf_clock(void)
{
	int ret;
	/* must keep this clk open order: CG_SCP_SYS_DIS-> CG_DISP0_SMI_COMMON -> CG_SCP_SYS_ISP -> ISP clk */
	ret = clk_enable(isp_clk.CG_SCP_SYS_DIS);
	if (ret)
		LOG_ERR("cannot get CG_SCP_SYS_DIS clock\n");
	...
	ret = clk_enable(isp_clk.CG_SCP_SYS_ISP);
	if (ret)
		LOG_ERR("cannot get CG_SCP_SYS_ISP clock\n");
	...
	ret = clk_enable(isp_clk.CG_IMAGE_CAM_CAM);
	if (ret)
		LOG_ERR("cannot get CG_IMAGE_CAM_CAM clock\n");

	ret = clk_enable(isp_clk.CG_IMAGE_SEN_TG);
	if (ret)
		LOG_ERR("cannot get CG_IMAGE_SEN_TG clock\n");

	ret = clk_enable(isp_clk.CG_IMAGE_SEN_CAM);
	if (ret)
		LOG_ERR("cannot get CG_IMAGE_SEN_CAM clock\n");
...
static inline void Disable_ccf_clock(void)
{
	...
	clk_disable(isp_clk.CG_IMAGE_CAM_SMI);
	clk_disable(isp_clk.CG_IMAGE_CAM_CAM);
	...
	clk_disable(isp_clk.CG_SCP_SYS_ISP);
	clk_disable(isp_clk.CG_DISP0_SMI_COMMON);
	clk_disable(isp_clk.CG_SCP_SYS_DIS);
}

static inline void Unprepare_ccf_clock(void)
{
	/* must keep this clk close order: ISP clk -> CG_SCP_SYS_ISP -> CG_DISP0_SMI_COMMON -> CG_SCP_SYS_DIS */
	clk_unprepare(isp_clk.CG_IMAGE_CAM_SMI);
	clk_unprepare(isp_clk.CG_IMAGE_CAM_CAM);
	...
	clk_unprepare(isp_clk.CG_SCP_SYS_ISP);
	clk_unprepare(isp_clk.CG_DISP0_SMI_COMMON);
	clk_unprepare(isp_clk.CG_SCP_SYS_DIS);
}

static inline void Disable_Unprepare_ccf_clock(void)
{
	/* must keep this clk close order: ISP clk -> CG_SCP_SYS_ISP -> CG_DISP0_SMI_COMMON -> CG_SCP_SYS_DIS */
	clk_disable_unprepare(isp_clk.CG_IMAGE_CAM_SMI);
	clk_disable_unprepare(isp_clk.CG_IMAGE_CAM_CAM);
...
static void ISP_BufWrite_WriteToHw(void)
{
	...
	wake_up_interruptible_all((wait_queue_head_t *)&(IspInfo.WaitQueueHead));
	/*      */
...
static volatile MUINT32 lost_pass1_done_cnt;
static volatile MUINT32 lost_pass1_d_done_cnt;
/* record lost p1_done or not, 1 for lost p1_done. 0 for normal , 2 for last working buffer. */
				...
				/**/		/**/			else {
										...
										IRQ_LOG_PRINTER
										    (irqT, 0,
										     _LOG_DBG);
										LOG_ERR
										    ("[rtbc]error:dma(%d),PA(0x%x),over enque_2",
										     rt_dma,
										     pstRTBuf->
										     ring_buf
										     [rt_dma].
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01) || (camsv_reg_cal[0] & 0x01)
				...
				if (MTRUE == _bFlag) {
					...
					/**/	if (m_T_STAMP.fps > SlowMotion) {
							/*      patch here is because of that uncertain should happen only in missing SOF. And because of FBC, image still can be deque.
...
static MINT32 ISP_SOF_Buf_Get(eISPIrq irqT, CQ_RTBC_FBC *pFbc, MUINT32 *pCurr_pa,
			      unsigned long long sec, unsigned long usec, MBOOL bDrop)
{
#if	defined(_rtbc_use_cq0c_)
...
/* above behavior will make	FBC	write-buffer-patch fail	at p1_done */
/* curr_pa also	have this prob.	too	*/
static MINT32 ISP_CAMSV_SOF_Buf_Get(unsigned int dma, CQ_RTBC_FBC camsv_fbc, MUINT32 curr_pa,
				    unsigned long long sec, unsigned long usec, MBOOL bDrop)
{
...
/* above behavior will make	FBC	write-buffer-patch fail	at p1_done */
/* curr_pa also	have this prob.	too */
static MINT32 ISP_CAMSV_DONE_Buf_Time(unsigned int dma, CQ_RTBC_FBC fbc, unsigned long long sec,
				      unsigned long usec)
{
	unsigned int curr;
	eISPIrq irqT;
...
/* mark	the	behavior of	reading	FBC	at local. to prevent hw	interruptting duing	sw isr flow. */
/* above behavior will make	FBC	write-buffer-patch fail	at p1_done */
static MINT32 ISP_DONE_Buf_Time(eISPIrq irqT, CQ_RTBC_FBC *pFbc, unsigned long long sec,
				unsigned long usec)
{
	int i, k, m;
	int i_dma;
...
static MINT32 ISP_ED_BufQue_CTRL_FUNC(ISP_ED_BUFQUE_STRUCT param)
{
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_DEQUE:
		...
		if (idx == -1) {
			...
			ISP_GetEDBufQueWaitDequeState(idx), ISP_UsToJiffies(5000000));	/* 5s */
			if (restTime == 0) {
	...
	case ISP_ED_BUFQUE_CTRL_DEQUE_FAIL:
		...
		wake_up_interruptible_all((wait_queue_head_t *)&WaitQueueHead_EDBuf_WaitFrame);
		wake_up_interruptible_all((wait_queue_head_t *)&WaitQueueHead_EDBuf_WaitDeque);
		break;
	...
	case ISP_ED_BUFQUE_CTRL_WAKE_WAITFRAME:
		/* wake all     sleeped users   to check buffer is dequeued     or not */
		wake_up_interruptible_all((wait_queue_head_t *)&WaitQueueHead_EDBuf_WaitFrame);
		break;
...
/* do_gettimeofday(&time_ready2return);*/
	...
	spin_unlock_irqrestore(&(IspInfo.SpinLockIrq[eIrq]), flags);
	LOG_VRB
	    (" [ISP_GET_MARKtoQEURY_TIME] user/type/idx(%d/%d/%d),mark sec/usec (%d/%d),	irq	sec/usec (%d/%d),query sec/usec(%d/%d),sig(%d)\n",
...
static MINT32 ISP_FLUSH_IRQ_V3(ISP_WAIT_IRQ_STRUCT irqinfo)
{
	eISPIrq eIrq = _IRQ;
	unsigned long flags; /* old: MUINT32 flags;*//* FIX to avoid build warning */
...
/*	int cnt = 0;*/
	...
	if (Timeout == 0) {
		spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
		LOG_ERR
		    ("v1	ERR	WaitIrq	Timeout	Clear(%d), Type(%d), IrqStatus(0x%08X),	WaitStatus(0x%08X),	Timeout(%d),user(%d)",
...
/*	int cnt = 0;*/
	...
	if (WaitIrq->Clear == ISP_IRQ_CLEAR_STATUS) {
		spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
		/* LOG_DBG("WARNING: Clear(%d), Type(%d): IrqStatus(0x%08X)     has     been cleared",WaitIrq->Clear,WaitIrq->UserInfo.Type,IspInfo.IrqInfo.Status[WaitIrq->UserInfo.Type]); */
		...
		if (WaitIrq->UserInfo.Status & IspInfo.IrqInfo.
			...
			if (WaitIrq->Clear == ISP_IRQ_CLEAR_WAIT) {
				...
				if (IspInfo.IrqInfo.
					...
					IspInfo.IrqInfo.Status[WaitIrq->UserInfo.UserKey][WaitIrq->
											  UserInfo.
											  Type] &=
					    (~WaitIrq->UserInfo.Status);
				}
...
/* LOG_DBG("- E.");*/
	...
	volatile MUINT32 IrqStatus[ISP_IRQ_TYPE_AMOUNT];
...
#endif
			lost_pass1_done_cnt++;
			if (2 == lost_pass1_done_cnt) {
				/*check any buffer is ready? */
				for (k = 0; k < pstRTBuf->ring_buf[rt_dma].total_count; k++) {
					if (ISP_RTBC_BUF_FILLED == pstRTBuf->ring_buf[rt_dma].data[k].bFilled) {
...
/*		ktime_t time;*/
		...
		if (sof_pass1done[0] == 1)
			ISP_SOF_Buf_Get(_IRQ, p1_fbc, (MUINT32 *)curr_pa, sec, usec, MTRUE);
		else
			ISP_SOF_Buf_Get(_IRQ, p1_fbc, (MUINT32 *)curr_pa, sec, usec, MFALSE);
	}
	spin_unlock(&(IspInfo.SpinLockIrq[_IRQ]));
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT_P1_ST_D] & ISP_IRQ_P1_STATUS_D_SOF1_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			CQ_RTBC_FBC _fbc_chk[2];	/* can chk fbc status compare to p1_fbc. */
...
/*--  get time at sof beginning for shrink log --*/
/*		unsigned long long sec;*/
/*		unsigned long usec;*/
/*		ktime_t time;*/
		/*      */
		...
		if (sof_pass1done[1] == 1)
			ISP_SOF_Buf_Get(_IRQ_D, p1_fbc, (MUINT32 *)curr_pa, sec, usec, MTRUE);
		else
			ISP_SOF_Buf_Get(_IRQ_D, p1_fbc, (MUINT32 *)curr_pa, sec, usec, MFALSE);
	}
	/*      */
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	case ISP_GET_INT_ERR:
		if (copy_to_user((void *)Param, (void *)g_ISPIntErr, sizeof(MUINT32) * _IRQ_MAX) != 0)
	...
	case ISP_REGISTER_IRQ_USER_KEY:
		if (copy_from_user(&RegUserKey, (void *)Param, sizeof(ISP_REGISTER_USERKEY_STRUCT))
		    == 0) {
			userKey = ISP_REGISTER_IRQ_USERKEY(RegUserKey.userName);
			RegUserKey.userKey = userKey;
			if (copy_to_user
			    ((void *)Param, &RegUserKey, sizeof(ISP_REGISTER_USERKEY_STRUCT)) != 0)
				LOG_ERR("copy_to_user failed\n");
				       ...
				       ISP_WAIT_IRQ_STRUCT __user *data)
{
	...
	err |=
	    copy_to_user((void *)&data->UserInfo, &isp_irq_user_tmp, sizeof(ISP_IRQ_USER_STRUCT));
...
/*	compat_uptr_t uptr;*/
	...
	err |=
	    copy_to_user((void *)&data32->UserInfo, &isp_irq_user_tmp, sizeof(ISP_IRQ_USER_STRUCT));
...
static int compat_get_isp_register_userkey_struct_data(compat_ISP_REGISTER_USERKEY_STRUCT __user *
						       data32,
...
static int compat_put_isp_register_userkey_struct_data(compat_ISP_REGISTER_USERKEY_STRUCT __user *
						       data32,
						       ISP_REGISTER_USERKEY_STRUCT __user *data)
{
	compat_uint_t tmp;
...
static MINT32 mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
	...
	if (length > ISP_RTBUF_REG_RANGE) {
		LOG_ERR("mmap range	error! : length(0x%lx),ISP_RTBUF_REG_RANGE(0x%x)!", length,
			ISP_RTBUF_REG_RANGE);
		return -EAGAIN;
	}
	ret =
...
static MINT32 ISP_probe(struct platform_device *pDev)
{
	...
	isp_clk.CG_IMAGE_CAM_SMI = devm_clk_get(&pDev->dev, "CG_IMAGE_CAM_SMI");
	isp_clk.CG_IMAGE_CAM_CAM = devm_clk_get(&pDev->dev, "CG_IMAGE_CAM_CAM");
	isp_clk.CG_IMAGE_SEN_TG = devm_clk_get(&pDev->dev, "CG_IMAGE_SEN_TG");
	isp_clk.CG_IMAGE_SEN_CAM = devm_clk_get(&pDev->dev, "CG_IMAGE_SEN_CAM");
	isp_clk.CG_IMAGE_CAM_SV = devm_clk_get(&pDev->dev, "CG_IMAGE_CAM_SV");
	isp_clk.CG_IMAGE_LARB2_SMI = devm_clk_get(&pDev->dev, "CG_IMAGE_LARB2_SMI");
	...
	if (IS_ERR(isp_clk.CG_IMAGE_CAM_SMI)) {
		LOG_ERR("cannot get CG_IMAGE_CAM_SMI clock\n");
		return PTR_ERR(isp_clk.CG_IMAGE_CAM_SMI);
	}
	if (IS_ERR(isp_clk.CG_IMAGE_CAM_CAM)) {
		LOG_ERR("cannot get CG_IMAGE_CAM_CAM clock\n");
		return PTR_ERR(isp_clk.CG_IMAGE_CAM_CAM);
	}
	if (IS_ERR(isp_clk.CG_IMAGE_SEN_TG)) {
		LOG_ERR("cannot get CG_IMAGE_SEN_TG clock\n");
		return PTR_ERR(isp_clk.CG_IMAGE_SEN_TG);
	}
	if (IS_ERR(isp_clk.CG_IMAGE_SEN_CAM)) {
		LOG_ERR("cannot get CG_IMAGE_SEN_CAM clock\n");
		return PTR_ERR(isp_clk.CG_IMAGE_SEN_CAM);
	}
	if (IS_ERR(isp_clk.CG_IMAGE_CAM_SV)) {
		LOG_ERR("cannot get CG_IMAGE_CAM_SV clock\n");
...
#endif
	/*      */
	init_waitqueue_head((wait_queue_head_t *)&IspInfo.WaitQueueHead);
	...
	init_waitqueue_head((wait_queue_head_t *)&WaitQueueHead_ImemDbgDump);
	init_waitqueue_head((wait_queue_head_t *)&WaitQueueHead_ImemDbgDumpDone);
	spin_lock_init((spinlock_t *)&(SpinLockImemDump));
	spin_lock_init((spinlock_t *)&(SpinLockEDBufQueList));
...
#else
	mt_irq_set_sens(gISPSYS_Irq[0]+32, MT_LEVEL_SENSITIVE);
	mt_irq_set_polarity(gISPSYS_Irq[0]+32, MT_POLARITY_LOW);
#endif
	return 0;
...
static const struct file_operations imem_debugtrigger_fops = {
	.read  = imem_debug_trigger,
};


void DBGfs_Init(void)
{
	/* for AEE */
	cisp_dbgTrigfs = debugfs_create_file("cisp_dbgtrig",
	S_IFREG|S_IRUGO, NULL, (void *)0, &imem_debugtrigger_fops);
}

void DBGfs_UnInit(void)
{
...
/* FIX-ME: linux-3.10 procfs API changed */
	...
	cmdqCoreRegisterCB(CMDQ_GROUP_ISP, ISP_MDPClockOnCallback, ISP_MDPDumpCallback,	ISP_MDPResetCallback, ISP_MDPClockOffCallback);
...
static void	__exit ISP_Exit(void)
{
	...
	/* unreserve the pages */
	for (i = 0; i <	RT_BUF_TBL_NPAGES * PAGE_SIZE; i += PAGE_SIZE)
		ClearPageReserved(virt_to_page(((unsigned	long)pTbl_RTBuf) + i));
