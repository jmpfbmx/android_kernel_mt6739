...
static int mt_fdvt_clk_ctrl(int en)
{
	...
	if (en) {
		...
		if (err)
			LOG_ERR("clk_prepare_enable(clk_SCP_SYS_DIS) FAIL! ERR CODE: %x\n", err);
		err = clk_prepare_enable(fdvt_devs->clk_SMI_COMMON);
		if (err)
			LOG_ERR("clk_prepare_enable(clk_SMI_COMMON) FAIL! ERR CODE: %x\n", err);
		err = clk_prepare_enable(fdvt_devs->clk_SCP_SYS_ISP);
		if (err)
			LOG_ERR("clk_prepare_enable(clk_SCP_SYS_ISP) FAIL! ERR CODE: %x\n", err);
		err = clk_prepare_enable(fdvt_devs->clk_FD);
		if (err)
			LOG_ERR("clk_prepare_enable(clk_FD) FAIL! ERR CODE: %x\n", err);
	} else {
		clk_disable_unprepare(fdvt_devs->clk_FD);
		clk_disable_unprepare(fdvt_devs->clk_SCP_SYS_ISP);
		clk_disable_unprepare(fdvt_devs->clk_SMI_COMMON);
		clk_disable_unprepare(fdvt_devs->clk_SCP_SYS_DIS);
	}
#endif
	return 0;
}
