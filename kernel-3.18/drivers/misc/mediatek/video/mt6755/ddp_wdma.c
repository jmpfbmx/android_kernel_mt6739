...
static inline unsigned long wdma_to_cmdq_engine(DISP_MODULE_ENUM module)
{
	switch (module) {
	case DISP_MODULE_WDMA0:
		return CMDQ_ENG_DISP_WDMA0;
	case DISP_MODULE_WDMA1:
		return CMDQ_ENG_DISP_WDMA1;
	default:
...
static inline unsigned long wdma_to_cmdq_event_nonsec_end(DISP_MODULE_ENUM module)
{
	switch (module) {
	case DISP_MODULE_WDMA0:
		return CMDQ_SYNC_DISP_WDMA0_2NONSEC_END;
	case DISP_MODULE_WDMA1:
		return CMDQ_SYNC_DISP_WDMA1_2NONSEC_END;
	default:
		DISPERR("invalid rdma module=%d\n", module);
		BUG();
	}
...
void wdma_dump_analysis(DISP_MODULE_ENUM module)
{
	...
	DISPDMP("wdma%d:en=%d,w=%d,h=%d,clip=(%d,%d,%d,%d),pitch=(W=%d,UV=%d),addr=(0x%x,0x%x,0x%x),fmt=%s\n",
	...
	DISPDMP("wdma%d:status=%s,in_req=%d(prev sent data),in_ack=%d(ask data to prev),exec=%d,in_pix=(L:%d,P:%d)\n",
		index,
...
void wdma_dump_reg(DISP_MODULE_ENUM module)
{
	...
	DISPDMP("== START: DISP WDMA%d REGS ==\n", idx);
	...
	DISPDMP("WDMA:0x090=0x%08x,0x094=0x%08x,0x098=0x%08x\n",
	...
	DISPDMP("0x10203018 = 0x%08x\n", *(unsigned int *)(emi_va+0x18));
	iounmap(emi_va);
#endif
	DISPDMP("-- END: DISP WDMA%d REGS --\n", idx);
...
static int wdma_check_input_param(WDMA_CONFIG_STRUCT *config)
{
	if (!is_unified_color_fmt_supported(config->outputFormat)) {
		DISPERR("wdma parameter invalidate outfmt %s:0x%x\n",
...
static inline int wdma_switch_to_sec(DISP_MODULE_ENUM module, void *handle)
{
	...
	cmdq_engine = wdma_to_cmdq_engine(module);
...
static inline int wdma_switch_to_nonsec(DISP_MODULE_ENUM module, void *handle)
{
	unsigned int wdma_idx = wdma_index(module);
	/*int *wdma_is_sec = svp_pgc->module_sec.wdma_sec;*/
	...
	cmdq_engine = wdma_to_cmdq_engine(module);
	cmdq_event  = wdma_idx == 0 ? CMDQ_EVENT_DISP_WDMA0_EOF : CMDQ_EVENT_DISP_WDMA1_EOF;
	cmdq_event_nonsec_end  = wdma_to_cmdq_event_nonsec_end(module);
...
static int setup_wdma_sec(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	int ret;
	int is_engine_sec = 0;
	...
	if (ret)
		DISPAEE("[SVP]fail to setup_ovl_sec: %s ret=%d\n",
			__func__, ret);
