...
void dump_dbgregs(int cpuid)
{
	...
	pr_debug("[MTK WP]dump_dbgregs: cpu %d, DBGLSR 0x%x, DBGOSLSR 0x%x\n", cpuid, dbglsr,
		  oslsr);
	...
	for (i = 1; i < 1 + (wp_context->bp_nr); i++) {
		dbgregs[cpuid].regs[i] =
		    cs_cpu_read(wp_context->debug_regs[cpuid], (DBGBVR + ((i - 1) << 4)));
	}
	...
	for (i = 13; i < 13 + (wp_context->wp_nr); i++) {
		...
		pr_debug("[MTK WP]dump_dbgregs:DBGWVR &0x%p=0x%x\n",
...
void print_dbgregs(int cpuid)
{
	...
	pr_debug("[MTK WP] cpu %d, DBGDSCR 0x%lx\n", cpuid, dbgregs[cpuid].DBGDSCRext);
