...
void wp_test1(void)
{
	init_wp_event(&wp_event, (unsigned int)&my_watch_data2, (unsigned int)&my_watch_data2,
...
int enable_hw_watchpoint(void)
{
	...
	for (i = 0; i < num_possible_cpus(); i++) {
		if (cpu_online(i)) {
			...
			pr_debug("[MTK WP] cpu %d, DBGDSCR 0x%x\n", i, args);
			if (args & HDBGEN) {
				pr_debug
			...
			if (args & MDBGEN) {
				/* already enabled */
				pr_debug("[MTK WP] already enabled, DBGDSCR = 0x%x\n", args);
			}
			...
			pr_debug("[MTK WP] cpu %d, DBGLSR 0x%x, DBGOSLSR 0x%x\n", i, dbglsr,
				  oslsr);
			pr_debug("[MTK WP] cpu %d, DBGDSCR 0x%x. (after set dbgdscr)\n", i, args);
		} else {
...
int add_hw_watchpoint(struct wp_event *wp_event)
{
	...
	for (j = 0; j < num_possible_cpus(); j++) {
		if (cpu_online(j)) {
			cs_cpu_write(wp_tracer.debug_regs[j], DBGWVR + (i << 4),
				     wp_tracer.wp_events[i].virt);
			cs_cpu_write(wp_tracer.debug_regs[j], DBGWCR + (i << 4), ctl);

			pr_debug("[MTK WP] cpu %d, DBGWVR%d, &0x%p=0x%x\n", j, i,
...
int watchpoint_handler(unsigned long addr, unsigned int fsr, struct pt_regs *regs)
{
	...
	|| defined(CONFIG_ARCH_MT6757) || defined(CONFIG_ARCH_MT6757M) || defined(CONFIG_ARCH_MT6753)
	    /* Notes
...
int wp_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < wp_tracer.nr_dbg; i++) {
		...
		if (wp_tracer.debug_regs[i] == NULL) {
			...
			pr_debug("[MTK WP] debug_interface %d @ vm:0x%p pm:0x%x\n", i,
				  wp_tracer.debug_regs[i],
				  IO_VIRT_TO_PHYS((unsigned int)wp_tracer.debug_regs[i]));
		}
	}
...
static const struct of_device_id dbg_of_ids[] = {
	{.compatible = "mediatek,mt6757-dbg_debug",},
	{}
};
