...
enum _HDMI_CEC_RX_MODE {
	CEC_NORMAL_MODE = 0,
	CEC_CTS_MODE,
	CEC_SLT_MODE
};
#define HDMI_CEC_RX_MODE enum _HDMI_CEC_RX_MODE
...
#define CEC_TX_FAIL  enum _CEC_TX_FAIL
...
#define CEC_SW_STATE enum _CEC_SW_STATE

enum _CEC_ERR_STATUS {
...
#define CEC_ERR_STATUS enum _CEC_ERR_STATUS
...
#define CEC_OPCODE enum _CEC_OPCODE
...
struct _CEC_HEADER_BLOCK {
	unsigned char destination:4;
	unsigned char initiator:4;
};
#define CEC_HEADER_BLOCK struct _CEC_HEADER_BLOCK

struct _CEC_FRAME_BLOCK {
	CEC_HEADER_BLOCK header;
	unsigned char opcode;
	unsigned char operand[15];
};
#define CEC_FRAME_BLOCK struct _CEC_FRAME_BLOCK

struct _CEC_FRAME_DESCRIPTION {
	unsigned char size;
...
#define CEC_FRAME_DESCRIPTION struct _CEC_FRAME_DESCRIPTION

struct _CEC_ADDRESS {
	unsigned char ui1_la;
	unsigned short ui2_pa;
};
#define CEC_ADDRESS struct _CEC_ADDRESS

struct _CEC_LA_ADDRESS {
	unsigned char ui1_num;
	unsigned char aui1_la[3];
	unsigned short ui2_pa;
};
#define CEC_LA_ADDRESS struct _CEC_LA_ADDRESS

struct _CEC_SLT_DATA {
	unsigned char u1Size;
	unsigned char au1Data[14];
};
#define CEC_SLT_DATA struct _CEC_SLT_DATA

enum _CEC_DRV_LOG_ADDR_T {
	CEC_LOG_ADDR_TV = 0,
...
#define CEC_DRV_LOG_ADDR_T enum _CEC_DRV_LOG_ADDR_T
...
#define CEC_CMD_STATE enum _CEC_CMD_STATE

struct _CEC_ADDR_CONFIG {
	unsigned char ui1_la_num;
	unsigned char e_la[CEC_MAX_DEV_LA_NUM];
	unsigned short ui2_pa;
	unsigned short h_cecm_svc;
};
#define CEC_DRV_ADDR_CFG_T struct _CEC_ADDR_CONFIG

struct _CEC_FRAME_INFO_T {
...
#define CEC_FRAME_INFO_T struct _CEC_FRAME_INFO_T

struct _CEC_SEND_MSG_T {
	CEC_FRAME_INFO_T t_frame_info;
	unsigned char b_enqueue_ok;
};
#define CEC_SEND_MSG_T struct _CEC_SEND_MSG_T

extern void mt8193_cec_init(void);
extern unsigned char mt8193_cec_isrprocess(unsigned char u1rxmode);
extern void mt8193_cec_mainloop(unsigned char u1rxmode);
extern void mt8193_CECMWSetLA(CEC_DRV_ADDR_CFG_T *prAddr);
extern void mt8193_u4CecSendSLTData(unsigned char *pu1Data);
...
extern void mt8193_CECMWSetEnableCEC(unsigned char u1EnCec);
extern void mt8193_NotifyApiCECAddress(CEC_ADDRESS *cecaddr);
extern void mt8193_SetPhysicCECAddress(unsigned short u2pa, unsigned char u1la);
#endif
#endif
