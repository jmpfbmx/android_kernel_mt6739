...
/*----------------------------------------------------------------------------*/
unsigned int mt8193_cec_read(unsigned short u2Reg)
{
	unsigned int u4Data;
...
void mt8193_cec_write(unsigned short u2Reg, unsigned int u4Data)
{
...
unsigned int mt8193_cec_2n(unsigned int u4Data)
{
...
unsigned int mt8193_cec_maskvalue(unsigned int u4Width, unsigned int u4Startbit)
{
...
static unsigned char _fgUICECEnable;
...
#define	RegReadFldAlign(reg16, fld) \
	((mt8193_cec_read(reg16)>>(Fld_shft(fld)))&(mt8193_cec_2n(Fld_wid(fld))))
#define	vRegWriteFldAlign(reg16, val, fld) (mt8193_cec_write(reg16,\
	(((mt8193_cec_read(reg16))&(mt8193_cec_maskvalue(Fld_wid(fld), Fld_shft(fld))))|(val<<(Fld_shft(fld))))))
...
void mt8193_u4CecSendSLTData(unsigned char *pu1Data)
{
	unsigned char i;
...
void mt8193_cec_mainloop(unsigned char u1rxmode)
{
	/* MT8193_CEC_FUNC(); */
	_CEC_TX_Queue_Loop();
...
unsigned char mt8193_cec_isrprocess(unsigned char u1rxmode)
{
...
void mt8193_CECMWSend(CEC_SEND_MSG_T *msg)
{
	...
	if (i4Ret == 0x01) {
		...
		MT8193_CEC_LOG("MW Set cmd success\n");
		; return;
	}

}

void mt8193_CECMWSetEnableCEC(unsigned char u1EnCec)
{
	MT8193_CEC_FUNC();
...
void mt8193_SetPhysicCECAddress(unsigned short u2pa, unsigned char u1la)
{
	MT8193_CEC_FUNC();
