...
unsigned short syncShutter_ov8858;
...
static imgsensor_info_struct imgsensor_info = {
	...
	.i2c_addr_table = {0x6C, 0x42, 0x20, 0xff},
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{3296, 2480, 12, 12, 3272, 2456, 1636, 1228, 0, 0, 1632, 1224, 0, 0, 1632, 1224},	/* hight speed video */
	{3296, 2480, 12, 12, 3272, 2456, 1636, 1228, 0, 0, 1632, 1224, 0, 0, 1632, 1224}
};				/* slim video */
...
kal_uint8 *pOTP_Data = NULL;
bool isOTPLenC_CheckSumPASS = FALSE;
kal_uint16 gOTP_AWB_Data[3] = { 0, };

static void OV8858_read_otp_lenc(void)
{
	kal_uint16 loop = 0;
	kal_uint16 flag;
	kal_uint16 start_addr, end_addr;
	kal_uint16 otp_start_addr = 0x7010, otp_end_addr = 0x720E;
	kal_uint16 temp;
	kal_uint32 checksum, ReadCheckSum;

	LOG_INF("OV8858_read_otp_lenc Start\n");

	if (pOTP_Data != NULL) {
		LOG_INF("[OV8858_read_otp_lenc] OTP LenC Data already read\n");
		return;
	}

	pOTP_Data = kzalloc(sizeof(kal_uint8) * 240, GFP_KERNEL);
	/*if (pOTP_Data==NULL) {
		LOG_INF("[OV8858_read_otp_lenc] Memory allocation fail\n");
		return;
	}*/
	/* Disable OTP_DPC , set 0x5002[3] to "0" */
	temp = read_cmos_sensor(0x5002);
	write_cmos_sensor(0x5002, (temp & (~0x08)));

	write_cmos_sensor(0x3d84, 0xc0);

	write_cmos_sensor(0x3d88, (otp_start_addr >> 8) & 0xff);
	write_cmos_sensor(0x3d89, otp_start_addr & 0xff);
	write_cmos_sensor(0x3d8a, (otp_end_addr >> 8) & 0xff);
	write_cmos_sensor(0x3d8b, otp_end_addr & 0xff);
	...
	if (flag == 1) {
		...
		LOG_INF("[OV8858_read_otp_lenc] flag_lenc error : %d\n", flag);
		if (pOTP_Data != NULL) {
			kfree(pOTP_Data);
			pOTP_Data = NULL;
		}
	...
	for (loop = 0; loop < 240; loop++) {
		pOTP_Data[loop] = (read_cmos_sensor(start_addr + loop) & 0xFF);
		checksum += pOTP_Data[loop];
	}

	/* Read LenC Checksum */
	ReadCheckSum =
	    ((read_cmos_sensor(end_addr + 1) & 0xFF) |
	     ((read_cmos_sensor(end_addr + 2) & 0xFF) << 8));
	LOG_INF("[OV8858_read_otp_lenc] checksum = 0x%x, ReadCheckSum = 0x%x\n", checksum,
	       ReadCheckSum);

	if (checksum != ReadCheckSum) {
		LOG_INF("[OV8858_read_otp_lenc] checksum error\n");
		if (pOTP_Data != NULL) {
			kfree(pOTP_Data);
			pOTP_Data = NULL;
		}
		isOTPLenC_CheckSumPASS = FALSE;
		return;
	}
	isOTPLenC_CheckSumPASS = TRUE;
	LOG_INF("OV8858_read_otp_lenc End\n");

}

static void OV8858_write_otp_lenc(void)
{
	kal_uint16 loop = 0, temp = 0;

	LOG_INF("OV8858_write_otp_lenc Start\n");

	/* Write LenC Data */
	for (loop = 0; loop < 240; loop++)
		write_cmos_sensor(0x5800 + loop, pOTP_Data[loop]);

	/* apply the LenC , set 0x5000[7] to "1" */
	temp = read_cmos_sensor(0x5000);
	write_cmos_sensor(0x5000, (temp | 0x80));

	LOG_INF("OV8858_write_otp_lenc End\n");

}

static void OV8858_read_otp_awb(void)
{
	typedef enum {
		LGIT = 0,
		IMTECH,
	...
	kal_uint16 otp_start_addr = 0x7010, otp_end_addr = 0x720E;
	kal_uint16 temp = 0;
	kal_uint32 checksum = 0, ReadCheckSum = 0;
	kal_uint16 module_vendor_id = 0, module_rev = 0;
	MODULE_VENDOR module_vendor = 0;

	LOG_INF("OV8858_read_otp_awb Start\n");

	memset(gOTP_AWB_Data, 0x00, sizeof(gOTP_AWB_Data));
	...
	if (flag == 1) {
		...
		LOG_INF("[OV8858_read_otp_awb] flag error : %d\n", flag);
		return;
	}

	/* Check Module Vendor and Rev */
	if (flag == 1) {
		module_vendor_id = read_cmos_sensor(0x7015);
		module_rev = read_cmos_sensor(0x7016);
	} else if (flag == 3) {
		module_vendor_id = read_cmos_sensor(0x7112);
		module_rev = read_cmos_sensor(0x7113);
	} else {
		LOG_INF("[OV8858_read_otp_awb] flag error : %d\n", flag);
		return;
	}
	switch (module_vendor_id) {
	case 0x01:
	case 0x02:
	case 0x05:
	case 0x06:
	...
	*/
	default:
	module_vendor = MODUE_VENDOR_MAX;
	LOG_INF("[OV8858_read_otp_awb] module vendor id error [0x%x]\n", module_vendor_id);
	break;
	}
	...
	for (loop = 0; loop < 3; loop++) {
		...
		temp1 = (read_cmos_sensor(start_addr + (loop * 2)) & 0xFF);
		temp2 = (read_cmos_sensor(start_addr + (loop * 2) + 1) & 0xFF);
		checksum += (temp1 + temp2);
		if ((module_vendor == IMTECH) && (module_rev == 0x00))
			gOTP_AWB_Data[loop] = (temp1 << 8) | temp2;
		else
			gOTP_AWB_Data[loop] = temp1 | (temp2 << 8);

		LOG_INF("[OV8858_read_otp_awb] gOTP_AWB_Data[%d]=%d\n", loop, gOTP_AWB_Data[loop]);
	}
	...
	ReadCheckSum =
	    ((read_cmos_sensor(end_addr + 1) & 0xFF) |
	     ((read_cmos_sensor(end_addr + 2) & 0xFF) << 8));
	LOG_INF("[OV8858_read_otp_awb] checksum = 0x%x, ReadCheckSum = 0x%x\n", checksum,
	       ReadCheckSum);

	if (checksum != ReadCheckSum) {
		LOG_INF("[OV8858_read_otp_awb] checksum error\n");
		gOTP_AWB_Data[0] = 0;
		gOTP_AWB_Data[1] = 0;
		gOTP_AWB_Data[2] = 0;
		return;
	...
	LOG_INF("OV8858_read_otp_awb End\n");
}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	shutter =
	    (shutter >
	     (imgsensor_info.max_frame_length -
	      imgsensor_info.margin)) ? (imgsensor_info.max_frame_length -
					 imgsensor_info.margin) : shutter;
	write_cmos_sensor(0x3208, 0x00);	/* Group hold start recommended by OVT korea */
	if (imgsensor.autoflicker_en) {
		realtime_fps = imgsensor.pclk / imgsensor.line_length * 10 / imgsensor.frame_length;
		if (realtime_fps >= 297 && realtime_fps <= 305)
			set_max_framerate(296, 0);
	...
	write_cmos_sensor(0x3501, (shutter >> 4) & 0xFF);
	write_cmos_sensor(0x3500, (shutter >> 12) & 0x0F);
	if ((imgsensor_info.ae_shut_delay_frame != 0)
	    || (imgsensor_info.ae_sensor_gain_delay_frame != 0)) {
		write_cmos_sensor(0x3208, 0x10);	/* Group hold end recommended by OVT korea */
		write_cmos_sensor(0x3208, 0xA0);	/* Group launch  recommended by OVT korea */
	}
	LOG_INF("Exit! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length);
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				if ((read_cmos_sensor(0x302A)) == 0XB2) {
					...
					LOG_INF
					    ("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = %d(0=r2a,1=r1a)\n",
...
static kal_uint32 open(void)
{
	...
	{
		int retry_otp_read = 0;

		while (retry_otp_read < 3) {
			if (isOTPLenC_CheckSumPASS == TRUE)
				break;
			OV8858_read_otp_lenc();
			retry_otp_read++;
		}

		if (isOTPLenC_CheckSumPASS == FALSE)
			LOG_INF("[OV8858_OTP_Error] please check the ov8858 module\n");

		if (isOTPLenC_CheckSumPASS == TRUE)
			OV8858_write_otp_lenc();
	}
	OV8858_read_otp_awb();
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_ESHUTTER:
		if ((imgsensor_info.ae_shut_delay_frame == 0)
		    && (imgsensor_info.ae_sensor_gain_delay_frame == 0)) {
			syncShutter_ov8858 = (unsigned short)*feature_data;
	...
	case SENSOR_FEATURE_SET_GAIN:
		if ((imgsensor_info.ae_shut_delay_frame == 0)
		    && (imgsensor_info.ae_sensor_gain_delay_frame == 0)) {
			set_shutter(syncShutter_ov8858);
		}
