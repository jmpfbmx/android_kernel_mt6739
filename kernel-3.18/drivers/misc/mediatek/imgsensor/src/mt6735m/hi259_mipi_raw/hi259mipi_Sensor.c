...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI259MIPI_SENSOR_ID, //HI259MIPI_SENSOR_ID,  /*sensor_id = 0x2680*/ //record sensor id defined in Kd_imgsensor.h
 ...
 //Gionee: malp <2016-05-24> modify for CR01703897 bigin
	...
	.i2c_addr_table = {0x60, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
/*according toHI259 datasheet p53 image cropping*/
...
static void set_dummy(void)
{
	...
	/*  Add dummy pixels: */
    /* 0x380c [0:4], 0x380d defines the PCLKs in one line of HI259  */
    /* Add dummy lines:*/
    /* 0x380e [0:1], 0x380f defines total lines in one frame of HI259 */
//rampart
...
{
	//LOG_INF("1111111read_cmos_sensor(r0x04); = %d", read_cmos_sensor(0x04));
   // return ((read_cmos_sensor(0x04) << 8) | read_cmos_sensor(0x04));
//   kal_uint32 tmp = 0;
   #if 0
   while(1)
	{
   		mdelay(100);
		tmp = read_cmos_sensor(0x04);
		if(tmp == 0)
		{
		}
		else
...
//////////////////
 ...
 //add 20150424
 write_cmos_sensor(0x39, 0x52);  // lvds_bias_ctl
...
static kal_uint32 open(void)
{
    ...
    printk("open camera sensor hi259\n");
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
    ...
    printk("open camera sensor hi259 done\n");
	return ERROR_NONE;
}	/*	open  */
...
/*UINT32 HI259_MIPI_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)*/
