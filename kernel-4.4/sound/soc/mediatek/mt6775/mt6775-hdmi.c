...
#include "mt6775-hdmi.h"
#include "mtk-soc-hdmi-type.h"

void mtk_Hdmi_Configuration_Init(void *hdmi_stream_format)
{
	memset_io(hdmi_stream_format, 0, sizeof(struct audio_hdmi_format));
}

void mtk_Hdmi_Clock_Set(void *hdmi_stream_format)
{
	struct audio_hdmi_format *hdmi_stream = (struct audio_hdmi_format *) hdmi_stream_format;
		...
		int channels, snd_pcm_format_t format, int displaytype)
{
	...
	pr_debug("[%s] format = %d, displaytype = %d\n", __func__, format, displaytype);
...
void SetHDMIDumpReg(void)
{
	...
	kal_uint32 u4tmpClkdiv0;
	kal_uint32 u4tmpClkdiv1;
	kal_uint32 u4tmpClkdiv2;
	kal_uint32 u4tmpHDMode;
	kal_uint32 u4tmpHDAlign;
	kal_uint32 u4RegDAC;
	...
	pr_debug("%s, IRQ = 0x%x, HDMI_CON0 = 0x%x, TDM_CON1 = 0x%x, TDM_CON2 = 0x%x\n",
		__func__, u4RegValue, u4tmpValue, u4tmpValue1, u4tmpValue2);

	pr_debug("%s, DIV0 = 0x%x, DIV1 = 0x%x, DIV2 = 0x%x, Mode = 0x%x, Align = 0x%x, DAC_CON0 = 0x%x\n",
