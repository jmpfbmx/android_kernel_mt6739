					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (usb_prepare_done[SNDRV_PCM_STREAM_CAPTURE] &&
		...
		pr_warn("%s(), ul source change to usb mic!\n", __func__);
...
static const struct snd_kcontrol_new speech_usb_controls[] = {
	...
	SOC_ENUM_EXT("USB_Voice_UL_Select",
		     voice_usb_ul_enum[0],
		     Audio_USB_MD_UL_Select_Ctl_Get,
		     Audio_USB_MD_UL_Select_Ctl_Set),
	SOC_ENUM_EXT("USB_Voice_Loopback_Switch",
		     voice_usb_lpbk_enum[0],
		     Audio_USB_Lpbk_Get,
		     Audio_USB_Lpbk_Set),
				     ...
				     struct snd_pcm_hw_params *hw_params)
{
	...
	pr_warn("%s(), substream %p, stream %d, dma_bytes = %zu, dma_area = %p, dma_addr = 0x%lx, use_dram %d\n",
		...
		(long)runtime->dma_addr, usb_use_dram[stream]);
...
static snd_pcm_uframes_t mtk_voice_usb_pointer_play
			 (struct snd_pcm_substream *substream)
{
	...
	pr_usbp("%s(), Afe_Block->u4DMAReadIdx = 0x%x\n",
	...
	if (GetMemoryPathEnable(usb_mem_blk[stream]) == true) {
		...
		if (Afe_Block->u4DataRemained < 0) {
			pr_warn("%s(), u4DataRemained 0x%x < 0, HW_Cur_ReadIdx 0x%x, HW_memory_index 0x%x, Afe_consumed_bytes 0x%x\n",
		...
		pr_usbp("%s(), ReadIdx 0x%x, Remained 0x%x, Cur_ReadIdx 0x%x, memory_index 0x%x, consumed 0x%x\n",
			__func__, Afe_Block->u4DMAReadIdx,
...
static snd_pcm_uframes_t mtk_voice_usb_pointer_cap
			 (struct snd_pcm_substream *substream)
{
	int stream = substream->stream;
	kal_int32 HW_memory_index = 0;
	...
	if (GetMemoryPathEnable(usb_mem_blk[stream]) == true) {
		...
		Hw_Get_bytes = (HW_Cur_ReadIdx - Awb_Block->pucPhysBufAddr) - Awb_Block->u4WriteIdx;
		...
		Awb_Block->u4DataRemained += Hw_Get_bytes;
		 ...
		 pr_usbc("%s ReadIdx=0x%x WriteIdx = 0x%x Remained = 0x%x BufferSize= 0x%x, Get_bytes= 0x%x\n",
			__func__, Awb_Block->u4DMAReadIdx, Awb_Block->u4WriteIdx, Awb_Block->u4DataRemained,
		...
		if (Awb_Block->u4DataRemained > Awb_Block->u4BufferSize) {
			pr_warn("%s buffer overflow u4DMAReadIdx:%x, u4WriteIdx:%x, DataRemained:%x, BufferSize:%x\n",
			       __func__, Awb_Block->u4DMAReadIdx, Awb_Block->u4WriteIdx,
			       Awb_Block->u4DataRemained, Awb_Block->u4BufferSize);
		}

		pr_usbc("%s(), HW_Cur_ReadIdx 0x%x, HW_memory_index 0x%x\n",
...
static snd_pcm_uframes_t mtk_voice_usb_pointer
			 (struct snd_pcm_substream *substream)
{
				  ...
				  void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_usbc("%s finish0, read_count:0x%x, read_size:0x%x, Remained:0x%x, ReadIdx:0x%x, WriteIdx:0x%x\n",
	...
	if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize) {
		...
		pr_usbc("%s finish1, size:0x%x, ReadIdx:0x%x, WriteIdx:0x%x, Remained:0x%x\n",
		...
		if (copy_to_user((void __user *)Read_Data_Ptr,
			...
			pr_err("%s Fail 2 copy to user Ptr:%p, Addr:%p, ReadIdx:0x%x, Ptr:0x%x, read_size:0x%x",
			       __func__, Read_Data_Ptr, Awb_Block->pucVirtBufAddr,
			       Awb_Block->u4DMAReadIdx, DMA_Read_Ptr,
			       read_size);
			return 0;
		...
		pr_usbc("%s finish2, copy size_1:0x%x, ReadIdx:0x%x, WriteIdx:0x%x, Remained:0x%x\n",
			__func__, size_1, Awb_Block->u4DMAReadIdx,
			Awb_Block->u4WriteIdx, Awb_Block->u4DataRemained);

		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			pr_warn("%s 3, size2:%x, Remained:%x, Read_Ptr:0x%x, ReadIdx:%x\n",
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_warn("%s Fail 3 copy to userPtr:%p, Addr:%p, ReadIdx:0x%x , Ptr:0x%x, size:0x%x",
		...
		pr_usbc("%s(), finish3, size_2:0x%x, ReadIdx:0x%x, WriteIdx:0x%x Remained:0x%x\n",
				   ...
				   void __user *dst, snd_pcm_uframes_t count)
{
	...
	pr_usbp("%s(), WriteIdx = 0x%x, ReadIdx = 0x%x, DataRemained = 0x%x\n",
	...
	if (copy_size != 0) {
		...
		if (Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize) { /* copy once */
			if (!access_ok(VERIFY_READ, data_w_ptr, copy_size)) {
				...
				pr_usbp("memcpy Idx= %p data_w_ptr = %p copy_size = 0x%x\n",
			...
			pr_usbp("\tfinish1, copy_size:%x, WriteIdx:%x, ReadIdx=%x, Remained:%x, count=%x\n",
			...
			pr_usbp("size_1=0x%x, size_2=0x%x\n", size_1, size_2);
			if (!access_ok(VERIFY_READ, data_w_ptr, size_1)) {
				pr_warn(" 1ptr invalid data_w_ptr=%p, size_1=%d", data_w_ptr, size_1);
				pr_warn(" u4BufferSize=%d, u4DataRemained=%d", Afe_Block->u4BufferSize,
				       Afe_Block->u4DataRemained);
			} else {
				pr_usbp("mcmcpy Idx= %p data_w_ptr = %p size_1 = %x\n",
			...
			if (!access_ok(VERIFY_READ, data_w_ptr + size_1, size_2)) {
				pr_warn("2ptr invalid data_w_ptr=%p, size_1=%d, size_2=%d",
					data_w_ptr, size_1, size_2);
				pr_warn("u4BufferSize=%d, u4DataRemained=%d",
					Afe_Block->u4BufferSize, Afe_Block->u4DataRemained);
			} else {
				pr_usbp("mcmcpy Idx= %p data_w_ptr+size_1 = %p size_2 = %x\n",
			...
			pr_usbp("\tfinish2, copy size:%x, WriteIdx:%x,ReadIdx=%x DataRemained:%x\n",
...
#else
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		return mtk_voice_usb_copy_play(substream, channel, pos,
					       dst, count);
	else
		return mtk_voice_usb_copy_cap(substream, channel, pos,
					      dst, count);
#endif
}
