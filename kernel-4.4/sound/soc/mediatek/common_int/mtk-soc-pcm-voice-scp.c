...
static struct snd_dma_buffer scp_voice_DramBuffer; /* scp pre allcoate dram buffer*/

static struct snd_dma_buffer scp_voice_mddl_dma_buf;
static struct snd_dma_buffer scp_voice_runtime_feedback_dma_buf; /* real time for IV feedback buffer*/
...
static void stop_scp_voice_i2s2adc2_hardware(struct snd_pcm_substream *substream);
static void start_scp_voice_i2s2adc2_hardware(struct snd_pcm_substream *substream);
...
static void scp_voice_int_wakelock(bool enable)
{
	...
	if (enable == true)
		wake_lock(&scp_voice_suspend_lock);
	else
		wake_unlock(&scp_voice_suspend_lock);
...
static const struct soc_enum audio_scp_voice_Enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(scp_voice_hd_output), scp_voice_hd_output),
};
...
{
	pr_debug("%s()\n", __func__);
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(scp_voice_hd_output)) {
...
static int audio_scp_voice_Irqcnt_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	AudDrv_Clk_On();
...
static int audio_scp_voice_Irqcnt_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s(), irq_user_id = %p, irq1_cnt = %d, value = %ld\n",
		__func__,
		scp_voice_irq_user_id,
		scp_voice_irq_cnt,
	...
	if (scp_voice_irq_user_id && scp_voice_irq_cnt)
		...
		pr_notice("warn, cannot update irq counter, user_id = %p, irq1_cnt = %d\n",
...
static const struct snd_kcontrol_new Audio_Scp_voice_controls[] = {
	SOC_SINGLE_EXT("Audio_Scp_Voice_Debug_Info", SND_SOC_NOPM, 0, 0x80000, 0,
	Audio_Scp_Debug_Get, Audio_ScpSpk_Debug_Set),
	...
	audio_scp_voice_hdoutput_get, audio_scp_voice_hdoutput_set),
...
static int mtk_pcm_scp_voice_stop(struct snd_pcm_substream *substream)
{
	...
	if (mscp_voice_iv_meminterface_type ==  Soc_Aud_Digital_Block_MEM_AWB2)
		SetIntfConnection(Soc_Aud_InterCon_DisConnect,
...
static snd_pcm_uframes_t mtk_pcm_scp_voice_pointer(struct snd_pcm_substream *substream)
{
...
static int scp_voice_get_scpdram_buffer(void)
{
	...
	scp_voiceReserveBuffer.size = scp_get_reserve_mem_size(scp_voiceReserveBuffer.num);
	scp_voice_DramBuffer.addr = scp_voiceReserveBuffer.start_phys;
	scp_voice_DramBuffer.area = (kal_uint8 *)scp_voiceReserveBuffer.start_virt;
	scp_voice_DramBuffer.bytes =  scp_voiceReserveBuffer.size;
	pr_debug("%s scp_voice_DramBuffer.addr = %llx scp_voice_DramBuffer.area = %p bytes = %zu",
		__func__, scp_voice_DramBuffer.addr, scp_voice_DramBuffer.area, scp_voice_DramBuffer.bytes);
	return 0;
}

/* dm data with AWB*/
static int scp_voice_allocate_mddl_buffer(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *hw_params)
{
	int ret = 0;
	...
	if (AllocateAudioSram(&scp_voice_mddl_dma_buf.addr,
		...
		scp_voice_mddl_dma_buf.area = (unsigned char *)scp_voice_DramBuffer.area;
		scp_voice_mddl_dma_buf.bytes = buffer_size;
		SetHighAddr(Soc_Aud_Digital_Block_MEM_AWB, true, scp_voice_mddl_dma_buf.addr);
	...
	pr_debug("%s addr = %llx area = %p bytes  = %zu mspkPlaybackFeedbackDramState = %u\n",
		__func__, scp_voice_mddl_dma_buf.addr, scp_voice_mddl_dma_buf.area,
		scp_voice_mddl_dma_buf.bytes, mscp_voice_mdbackDramState);
	return ret;
...
static int scp_voice_allocate_feedback_buffer(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *hw_params)
{
	int ret = 0;
	unsigned int buffer_size = 0;
	...
	if (AllocateAudioSram(&scp_voice_runtime_feedback_dma_buf.addr,
		...
		scp_voice_runtime_feedback_dma_buf.area = (unsigned char *)scp_voice_DramBuffer.area + buffer_size;
		scp_voice_runtime_feedback_dma_buf.bytes = buffer_size;
		SetHighAddr(mscp_voice_iv_meminterface_type, true, scp_voice_runtime_feedback_dma_buf.addr);
	...
	set_memif_addr(mscp_voice_iv_meminterface_type,
		...
		scp_voice_runtime_feedback_dma_buf.bytes);
	pr_debug("%s addr = %llx area = %p bytes  = %zu mspkPlaybackFeedbackDramState = %u\n",
		__func__, scp_voice_runtime_feedback_dma_buf.addr, scp_voice_runtime_feedback_dma_buf.area,
		scp_voice_runtime_feedback_dma_buf.bytes, mscp_voice_FeedbackDramState);
	return ret;

}

/* DL data can use Sram or Dram*/
static int scp_voice_allocate_platformdl_buffer(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *hw_params)
{
	...
	if (buffer_size <= GetPLaybackSramFullSize() &&
		...
		scp_voice_DL1Buffer.addr = scp_voice_DramBuffer.addr + scpvoiceDL1BufferOffset * 2;
		scp_voice_DL1Buffer.area = (unsigned char *)scp_voice_DramBuffer.area + buffer_size;
		scp_voice_DL1Buffer.bytes = buffer_size;
		SetHighAddr(Soc_Aud_Digital_Block_MEM_DL1, true, scp_voice_DL1Buffer.addr);
	...
	pr_debug("%s scp_voice_DL1Buffer.addr = %llx scp_voice_DL1Buffer.area = %p bytes  = %zu\n",
		__func__, scp_voice_DL1Buffer.addr, scp_voice_DL1Buffer.area, scp_voice_DL1Buffer.bytes);
	return ret;
}

/* return for payload length*/
static unsigned int scp_voice_packIpi_payload(uint16_t msg_id, uint32_t param1, uint32_t param2,
		struct snd_dma_buffer *bmd_buffer, struct snd_pcm_substream *substream)
{
	unsigned int ret = 0;
	/* clean payload data */
	memset_io((void *)scp_voice_ipi_payload_buf, 0, sizeof(uint32_t)*DEFAULT_VOICE_PAYLOAD_SIZE);
	switch (msg_id) {
	...
	case SPK_PROTECT_SPEECH_PREPARE:
		if (substream->runtime->format == SNDRV_PCM_FORMAT_S32_LE ||
			...
			scp_voice_ipi_payload_buf[0] = Soc_Aud_I2S_WLEN_WLEN_32BITS;
		else
			scp_voice_ipi_payload_buf[0] = Soc_Aud_I2S_WLEN_WLEN_16BITS;

		scp_voice_ipi_payload_buf[1] = (kal_uint32) substream->runtime->rate;
		scp_voice_ipi_payload_buf[2] = (kal_uint32) substream->runtime->channels;
		scp_voice_ipi_payload_buf[3] = (kal_uint32) substream->runtime->period_size;
		ret = sizeof(unsigned int) * 4;
		break;
	case SPK_PROTECT_SPEECH_IVMEMPARAM:
		...
		scp_voice_ipi_payload_buf[4] = (mscp_voice_iv_meminterface_type);
...
static int mtk_pcm_scp_voice_open(struct snd_pcm_substream *substream)
{
	...
	AudDrv_Clk_On();
	mscp_voice_iv_meminterface_type = get_usage_digital_block(AUDIO_USAGE_SCP_SPK_IV_DATA);
	if (mscp_voice_iv_meminterface_type < 0) {
		pr_warn("%s get_pcm_mem_id err using VUL_Data2 as default\n", __func__);
		mscp_voice_iv_meminterface_type = Soc_Aud_Digital_Block_MEM_VUL_DATA2;
	}
...
static int mtk_pcm_voice_scp_close(struct snd_pcm_substream *substream)
{
	...
	if (mscp_voice_PrepareDone == true) {
		...
		set_memif_pbuf_size(Soc_Aud_Digital_Block_MEM_DL1, MEMIF_PBUF_SIZE_256_BYTES);
	...
	if (mscp_voice_PrepareDone == false) {
		...
		if (runtime->format == SNDRV_PCM_FORMAT_S32_LE || runtime->format == SNDRV_PCM_FORMAT_U32_LE) {
			...
			SetMemIfFetchFormatPerSample(Soc_Aud_Digital_Block_MEM_AWB,
				AFE_WLEN_32_BIT_ALIGN_8BIT_0_24BIT_DATA);
			SetMemIfFetchFormatPerSample(Soc_Aud_Digital_Block_MEM_AWB2,
				AFE_WLEN_32_BIT_ALIGN_8BIT_0_24BIT_DATA);

			SetConnectionFormat(OUTPUT_DATA_FORMAT_24BIT, mscp_voice_iv_meminterface_type);
			...
			SetMemIfFetchFormatPerSample(mscp_voice_iv_meminterface_type,
				AFE_WLEN_32_BIT_ALIGN_8BIT_0_24BIT_DATA);
			...
			SetMemIfFetchFormatPerSample(Soc_Aud_Digital_Block_MEM_AWB2, AFE_WLEN_16_BIT);
			SetConnectionFormat(OUTPUT_DATA_FORMAT_16BIT, mscp_voice_iv_meminterface_type);
			...
			SetMemIfFetchFormatPerSample(mscp_voice_iv_meminterface_type, AFE_WLEN_16_BIT);
		...
		set_memif_pbuf_size(Soc_Aud_Digital_Block_MEM_DL1, MEMIF_PBUF_SIZE_32_BYTES);
...
static void start_scp_voice_i2s2adc2_hardware(struct snd_pcm_substream *substream)
{
	...
	SetSampleRate(mscp_voice_iv_meminterface_type, substream->runtime->rate);
	SetChannels(mscp_voice_iv_meminterface_type, substream->runtime->channels);
...
static int mtk_pcm_scp_voice_start(struct snd_pcm_substream *substream)
{
	...
	irq_add_user(substream,
		...
		scp_voice_irq_cnt ? scp_voice_irq_cnt : substream->runtime->period_size);
...
static int mtk_pcm_scp_voice_trigger(struct snd_pcm_substream *substream, int cmd)
{
	pr_debug("%s cmd = %d\n", __func__, cmd);
...
static int mtk_afe_voice_scp_probe(struct snd_soc_platform *platform)
{
	pr_debug("mtk_afe_voice_scp_probe\n");
	snd_soc_add_platform_controls(platform, Audio_Scp_voice_controls,
				ARRAY_SIZE(Audio_Scp_voice_controls));

#ifdef use_wake_lock
	wake_lock_init(&scp_voice_suspend_lock, WAKE_LOCK_SUSPEND, "scpvoice lock");
#endif
