...
struct fuse_proc_info *fuse_iolog;
struct fuse_proc_log *fuse_ringbuf;
int fuse_ringbuf_index;
...
/* kernel log thread */
struct task_struct *fuse_iolog_thread;

/* debugfs dentries */
struct dentry *fuseio_droot;
struct dentry *fuseio_dklog;
struct dentry *fuseio_dlog;

/* kernel log enable */
unsigned int fuseio_klog_enable;
...
static const char *fuse_iolog_type[FUSE_IOLOG_TYPE_MAX] = {
	...
	"readdirplus",	/*44*/
	"fuse_rename2",	/* 45 */
	"canonical_path",	/* FUSE_CANONICAL_PATH, 2016 */
	"cuse_init"	/* CUSE_INIT=4096 */
};
...
static const char *fuse_iolog_type2str(int type)
{
	...
	else if (type == FUSE_CANONICAL_PATH)
		type = FUSE_IOLOG_TYPE_MAX-2;
	else
		type = 0;
out:
