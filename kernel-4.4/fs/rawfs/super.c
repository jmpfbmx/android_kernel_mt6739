...
unsigned int rawfs_parm;
module_param(rawfs_parm, int, 0);
MODULE_PARM_DESC(rawfs_parm, "");

/* Debug message level */
u32 rawfs_debug_msg_mask = RAWFS_DEBUG_MSG_DEFAULT;
static struct dentry *rawfs_droot;
static struct dentry *rawfs_dlog;
...
int rawfs_ci_hash(const struct dentry *dentry, struct qstr *q)
{
		...
		hash = init_name_hash();
		for (i = 0; i < q->len; i++)
			hash = partial_name_hash(nls_tolower(codepage, q->name[i]), hash);
...
static int __init init_rawfs_fs(void)
{
	...
	if (IS_ERR_OR_NULL(rawfs_droot))
		rawfs_droot = debugfs_create_dir("rawfs", NULL);

	if (IS_ERR(rawfs_droot)) {
		pr_info("rawfs: fail to create debugfs root\n");
		goto out;
	}
