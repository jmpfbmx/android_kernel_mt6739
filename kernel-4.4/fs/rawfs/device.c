...
int rawfs_block_level_analysis(struct super_block *sb)
{
	...
	if (rawfs_sb->data_block < 0) {
		...
		if (rawfs_sb->empty_block < 0) { /* Case B: Empty block was not exist */
			...
			/* This should not happen, since data blocks already had been found. */
			WARN_ON(gc_src_block_index < 0);
			...
			WARN_ON(rawfs_sb->empty_block == rawfs_sb->data_block);
		} else {
			/* Case B.2:  Both data block and empty block are valid => Normal Case */
			RAWFS_PRINT(RAWFS_DBG_MOUNT,
	...
	WARN_ON(rawfs_sb->empty_block < 0);
	WARN_ON(rawfs_sb->data_block < 0);
	...
	for (i = 0; i < 2; i++) {   /* Check files on the same folder */
		list_for_each_entry(ptr, lists[i], list) {
			if (ptr->file_info.i_parent_folder_id != folder_id)
				continue;
			if (strncasecmp(ptr->file_info.i_name, name,
...
void rawfs_file_list_remove(struct super_block *sb, struct rawfs_file_info *fi)
{
	...
	list_for_each_entry_safe(ptr, tmp, list_p, list) {
		...
		if (ptr->file_info.i_parent_folder_id != fi->i_parent_folder_id)
			continue;
		if (strncasecmp(ptr->file_info.i_name, fi->i_name,
			RAWFS_MAX_FILENAME_LEN+4) == 0) {
			entry = ptr;
			break;
		}
	...
	for (i = 1; i < RAWFS_NAND_PAGES(rawfs_sb); i++)	{
		...
		case RAWFS_PAGE_STAT_VALID:
			...
			if (fi_ptr->i_chunk_index == 1) {   /* First chunk of file */
				...
				/* No head chunk were found preceding this middle chunk, it is invalid. */
				if (head_pos < 0)
					break;

				/* Expected index of current chunk is (i-head_pos+1+uncorrectable_pages) */
		...
		case RAWFS_PAGE_STAT_DELETED:
			...
			/* Search list for redundant entry (by name), delete entry */
			free_page_index = -1;
		...
		case RAWFS_PAGE_STAT_EMPTY:
			...
			/* Set last free page, verify all pages are free, till end. */
...
int rawfs_dev_free_space(struct super_block *sb)
{
	...
	/* -2: Except block header page, last page is reserved for deletion & rename */
