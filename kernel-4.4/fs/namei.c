...
//Task5354494 added file lock patch by chuanbo.qiu begin
    ...
    if (!get_lockhide_flag()) {
        //if((nd->inode->i_mode & MAY_WIL_LOCK) == MAY_WIL_LOCK)
        if((nd->inode->i_flags & S_LOCK) == S_LOCK)    
        {
            printk(KERN_ERR "  hello may_lookup path is lock,  name is %s\n", nd->path.dentry->d_name.name);
            return -EACCES;
        }
    }
#endif	
...
#ifdef CONFIG_WIL_LOCK_FILE
	if (!get_lockhide_flag()) {
    	//if((victim->d_inode->i_mode & MAY_WIL_LOCK) == MAY_WIL_LOCK)
        if((victim->d_inode->i_flags& S_LOCK) == S_LOCK)               
    	{
    		printk(KERN_ERR "  hello file %s is not allowed delete\n", victim->d_name.name);				
    		return -EACCES;
    	}
    }
#endif
...
#ifdef CONFIG_WIL_LOCK_FILE
	if (!get_lockhide_flag()) {
    	//if((inode->i_mode & MAY_WIL_LOCK) == MAY_WIL_LOCK)
        if((inode->i_flags & S_LOCK) == S_LOCK)      
    	{
            printk(KERN_ERR "  hello may_open nd->inode->i_mode is %x, nd->inode->i_flags:%x\n", inode->i_mode, inode->i_flags);
    		return -EACCES;
    	}
    }
#endif
