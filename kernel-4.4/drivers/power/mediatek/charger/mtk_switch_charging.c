...
static void swchg_select_cv(struct charger_manager *info)
{
	...
	/*Begin added by rongxiao.deng for task 5451358 on 2017/12/14*/
	if (info->battery_temperature > 46)
		constant_voltage = 4100000;
	/*End added by rongxiao.deng for task 5451358 on 2017/12/14*/
	//Begin added by rongxioa.deng for task 5495859 on 2017/11/28
	#ifdef CONFIG_SUPPORT_IEEE1725_BATTERY_CV
	constant_voltage = 4100000;
	#endif
	//End added by rongxioa.deng for task 5495859 on 2017/11/28 
...
/*Begin modified by rongxiao.deng for task 5452302 on 2017/11/28*/
#ifdef TARGET_BUILD_MMITEST
	if(battery_get_bat_uisoc() > 65 && (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT
		|| get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT))
	{
		swchgalg->state = CHR_HOLD;
	}
#endif
/*End modified by rongxiao.deng for task 5452302 on 2017/11/28*/
...
/*Begin modified by rongxiao.deng for task 5452302 on 2017/11/28*/
int mtk_switch_chr_hold(struct charger_manager *info)
{
	struct switch_charging_alg_data *swchgalg = info->algorithm_data;

	swchgalg->total_charging_time = 0;
	if(battery_get_bat_uisoc() <= 60 && (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT
		|| get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT))
	{
		swchgalg->state = CHR_CC;
