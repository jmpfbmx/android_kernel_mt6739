...
#define LOW_TEMP_TO_LEAVE_PE40 TEMP_T2_THRESHOLD
#endif
	...
	const struct tcp_dpm_event_cb_data *cb_data)
{
	...
	ret = tcpm_set_apdo_charging_policy(pinfo->tcpc, DPM_CHARGING_POLICY_PPS,
		adapter_mv, adapter_ma, NULL);
...
void mtk_pe40_init_cap(struct charger_manager *info)
{
	...
	while (1) {
		ret = tcpm_inquire_pd_source_apdo(info->tcpc, TCPM_POWER_CAP_APDO_TYPE_PPS, &cap_i, &cap);
...
int mtk_pe40_get_setting_by_watt(struct charger_manager *pinfo, int *voltage, int *adapter_ibus,
	int *actual_current, int watt, int *ibus_current_setting)
{
	int i;
	struct mtk_pe40 *pe40;
	struct pe40_power_cap *pe40_cap;
...
int mtk_pe40_get_init_watt(struct charger_manager *pinfo)
{
	...
	ret = mtk_pe40_pd_request(pinfo, voltage, adapter_ibus, input_current, NULL);
	if (ret != 0) {
...
int mtk_pe40_init_state(struct charger_manager *pinfo)
{
	...
	if (pe40->can_query == true) {
		...
		mtk_pe40_get_setting_by_watt(pinfo, &voltage, &adapter_ibus,
			&actual_current, 5000000, &input_current);
		ret = mtk_pe40_pd_request(pinfo, voltage, actual_current, actual_current, NULL);
		if (ret != 0) {
		...
		for (i = 0; i < 4; i++) {
			...
			charger_dev_get_ibus(pinfo->chg1_dev, &ibus1);
			ibus1 = ibus1 / 1000;
			ret = pe40_tcpm_dpm_pd_get_pps_status(pinfo->tcpc, NULL, &cap1);
			if (ret != 0) {
		...
		for (i = 0; i < 4; i++) {
			...
			charger_dev_get_ibus(pinfo->chg1_dev, &ibus2);
			ibus2 = ibus2 / 1000;
			ret = pe40_tcpm_dpm_pd_get_pps_status(pinfo->tcpc, NULL, &cap2);
			if (ret != 0)
				goto err;
		...
		pe40->r_sw = abs((vbus2 - vbus1) - (vbat2 - vbat1)) * 1000 / abs(cap2.output_ma - cap1.output_ma);
		pe40->r_cable = abs((cap2.output_mv - cap1.output_mv)-(vbus2 - vbus1))
			* 1000 / abs(cap2.output_ma - cap1.output_ma);
		pe40->r_cable_2 = abs(cap2.output_mv - pe40->vbus_cali - vbus2) * 1000 / abs(cap2.output_ma);
		pe40->r_cable_1 = abs(cap1.output_mv - pe40->vbus_cali - vbus1) * 1000 / abs(cap1.output_ma);
	...
	voltage = 0;
	mtk_pe40_get_setting_by_watt(pinfo, &voltage, &adapter_ibus, &actual_current, watt,
		&input_current);
	pe40->avbus = voltage / 10 * 10;
	ret = mtk_pe40_pd_request(pinfo, pe40->avbus, adapter_ibus, input_current, NULL);
	if (ret != 0) {
...
int mtk_pe40_cc_state(struct charger_manager *pinfo)
{
	...
	vbus = battery_get_vbus();
	ibus = ibus / 1000;
	icl = pinfo->chg1_data.input_current_limit / 1000 * (100 - IBUS_ERR) / 100;
	...
	if ((ibus >= (max_icl - icl_threshold)) || (ibus <= (max_icl - icl_threshold * 2))) {

		oldavbus = pe40->avbus;

		if (ibus >= (max_icl - icl_threshold)) {
		...
		ret = mtk_pe40_get_setting_by_watt(pinfo, &pe40->avbus, &adapter_ibus, &actual_current, new_watt,
			&input_current);
