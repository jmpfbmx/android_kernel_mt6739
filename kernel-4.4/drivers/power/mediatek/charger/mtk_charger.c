...
//Begin added by shiqiang.chen.hz on 20180104, add charging_hold_state .
bool charging_hold_state = false;
bool charging_hold_debug = false;
//End added by shiqiang.chen.hz on 20180104, add charging_hold_state .
...
{
	struct charger_manager *info = consumer->cm;
	return 0;/*Added by rongxiao.deng for task 5860212 on 2018.01.08*/
	if (info != NULL) {
		struct charger_data *pdata;
...
//Begin added by shiqiang.chen.hz for FR5597245 on 20171130,for charging factory test
static ssize_t show_Charging_Voltage(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	return sprintf(buf, "%d\n", battery_get_vbus());
}

static DEVICE_ATTR(Charging_Voltage, 0664, show_Charging_Voltage, NULL);

static ssize_t show_Charging_Current(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	struct charger_manager *pinfo = dev->driver_data;
	struct charger_data chg_data = pinfo->chg1_data;
	
	//if((pinfo->chr_type != CHARGER_UNKNOWN) && chg_data && chg_data.charging_current_limit)
	if(pinfo->chr_type != CHARGER_UNKNOWN)
		return sprintf(buf, "%d\n", chg_data.charging_current_limit/1000);
	else
		chr_err("[show_Charging_Current]: no get Charging_Current.\n");
	
	return 0;
}
static DEVICE_ATTR(Charging_Current, 0664, show_Charging_Current, NULL);
//End added by shiqiang.chen.hz for FR5597245 on 20171130,for charging factory test

//Begin added by shiqiang.chen.hz on 20180104, add charging_hold_state file node .
static ssize_t show_charging_hold(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	return sprintf(buf, "charging_hold_debug:%d, charging_hold_state:%d.\n", charging_hold_state, charging_hold_debug);
}

static ssize_t store_charging_hold(struct device *dev, struct device_attribute *attr,
					 const char *buf, size_t size)
{
	unsigned long val = 0;
	int ret;
	
	chr_err("[store_charging_hold]\n");

	if (buf != NULL && size != 0) {
		ret = kstrtoul(buf, 10, &val);
		if (val < 0) {
			chr_err("[store_charging_hold] val is %d ??\n", (int)val);
			val = 0;
		}
			//charging_hold_state = (bool)val;
	}
	
	switch (val) {
	case 0:
		charging_hold_debug = false;
		break;
	case 1:
		charging_hold_debug = true;
		break;
	case 3:
		charging_hold_state = false;
		break;
	case 4:
		charging_hold_state = true;
		break;
	default:
		chr_err("[store_charging_hold]: invaild pamram. \n");
		break;
	}
...
static DEVICE_ATTR(charging_hold, 0664, show_charging_hold, store_charging_hold);
//End added by shiqiang.chen.hz on 20180104, add charging_hold_state file node .
...
//Begin added by hailong.chen for task 5495858 third party battery cannot charge on 2017/12/04
extern char battery_module_name[256];
//End added by hailong.chen for task 5495858 third party battery cannot charge on 2017/12/04
...
static void charger_check_status(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita == true) {
		...
		//Begin added by hailong.chen for task 5818123 mini must charge on 2017/12/28
		#ifndef TARGET_BUILD_MMITEST
		//Begin modified by hailong.chen for task 5495858 third party battery cannot charge on 2017/12/04
		if (strcmp(battery_module_name, "BAT:NA:NA") == 0)
		{
			charging = false;
			goto stop_charging;
		}
		//End modified by hailong.chen for task 5495858 third party battery cannot charge on 2017/12/04
		#endif
		//End added by hailong.chen for task 5818123 mini must charge on 2017/12/28
	}
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	//Begin added by shiqiang.chen.hz for FR5597245 on 20171130,for charging factory test
	ret = device_create_file(&(pdev->dev), &dev_attr_Charging_Voltage);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_Charging_Current);
	if (ret)
		goto _out;
	//End added by shiqiang.chen.hz for FR5597245 on 20171130,for charging factory test

	//Begin added by shiqiang.chen.hz on 20180104, add charging_hold_state file node .
	ret = device_create_file(&(pdev->dev), &dev_attr_charging_hold);
	if (ret)
		goto _out;
	//End added by shiqiang.chen.hz on 20180104, add charging_hold_state file node .
...
//Begin added by hailong.chen for task 5662647 adding charge ic deviceinfo on 2017/11/28
char charger_module_name[256]="PMIC:NA:NA";
...
static ssize_t charger_info_store(struct device* dev,
                                  struct device_attribute *attr, const char *buf, size_t count)
{
        return count;
}

DEVICE_ATTR(charger, S_IWUSR | S_IRUGO, charger_info_show, charger_info_store);
...
static void Create_charger_node_ForMMI(void)
{
    ...
    charger=get_deviceinfo_dev();
    if (device_create_file(charger, &dev_attr_charger) < 0)
        printk("Failed to create device file(%s)!\n", dev_attr_charger.attr.name);
    return;
}
//End added by hailong.chen for task 5662647 adding charge ic deviceinfo on 2017/11/28
