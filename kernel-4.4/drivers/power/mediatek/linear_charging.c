			       ...
			       int *max_charging_current, int *max_charging_current_i)
{
	...
	do {
		...
		current_vchr = battery_meter_get_charger_voltage();
		if ((abs(current_vchr - ta_current_level) > 300) && (ta_cable_out_occur == KAL_FALSE)) {
...
static void mtk_ta_BJT_check(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		    "mtk_ta_BJT_check() vchr=%d, vbat=%d, current=%d, Watt=%d, ta_current_level=%d cnt=%d\n",
...
static void battery_pump_express_algorithm_start(void)
{
	...
	if (is_ta_connect == KAL_TRUE) {
		...
		/* --for normal charging */
		if ((is_ta_connect == KAL_TRUE) && (curr_vbat > TA_START_VCHR_TUNUNG_VOLTAGE) &&
			(ta_check_ta_control == KAL_TRUE)) {
