...
// Begin modified by bin.song.hz for open battery log
static int Enable_BATDRV_LOG = 7;	/* Todo: charging.h use it, should removed */
// End modified by bin.song.hz for open battery log
static int loglevel_count;
static int id_volt=0;//Added by hailong.chen for task 5662647 adding batt_id on 2017/11/28
...
static enum power_supply_property battery_props[] = {
	...
	POWER_SUPPLY_PROP_ISenseCurrent,// Added by rongxiao.deng for task 5424620 on 2017/11/27
	POWER_SUPPLY_PROP_batt_id,//Added by hailong.chen for task 5662647 adding batt_id on 2017/11/28
...
// Added by rongxiao.deng for task 5424735 on 2017/10/21 begin
signed int battery_meter_get_ISense(void)
{
	...
	is_charging = gauge_get_current(&fg_current_temp);
	printk("%s\n", __func__);
	return fg_current_temp/10;// Added by rongxiao.deng for task 5424620 on 2017/11/27
#endif
}
// Added by rongxiao.deng for task 5424735 on 2017/10/21 end

//Begin added by hailong.chen for task 5762541 adding batt_id on 2017/12/14
int battery_get_batt_id(void);
//End added by hailong.chen for task 5762541 adding batt_id on 2017/12/14
...
void battery_update_psd(struct battery_data *bat_data)
{
	...
	bat_data->BAT_BatteryAverageCurrent = pmic_get_charging_current();// Modified by rongxiao.deng for task 5424620 on 2017/11/27
	bat_data->BAT_ISenseVoltage = battery_meter_get_VSense();
	bat_data->BAT_ISenseCurrent = battery_meter_get_ISense();// Added by rongxiao.deng for task 5424735 on 2017/10/21
	bat_data->BAT_batt_id= battery_get_batt_id();//Modified by hailong.chen for task 5762541 modify batt_id >>>battery_get_batt_id() on 2017/12/14
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_batt_vol:
		//Begin modified by shiqiang.chen.hz for MiniSw on 20171224, for the voltage of battery.
		//val->intval = data->BAT_batt_vol * 1000;
		val->intval = battery_get_bat_voltage() * 1000;
		//End modified by shiqiang.chen.hz for MiniSw on 20171224, for the voltage of battery.
		break;
	...
	case POWER_SUPPLY_PROP_ISenseVoltage:
		...
		break;
	case POWER_SUPPLY_PROP_ISenseCurrent:// Added by rongxiao.deng for task 5424735 on 2017/10/21
		val->intval = data->BAT_ISenseCurrent;
		break;
	case POWER_SUPPLY_PROP_batt_id://Added by hailong.chen for task 5662647 adding batt_id on 2017/11/28
		val->intval = data->BAT_batt_id;
		break;
...
//Begin added by hailong.chen for defect 5774243 set full status on 2017/12/22
	if ((FG_status.ui_soc== 100) && (upmu_get_rgs_chrdet() != 0)
	    && (battery_main.BAT_STATUS != POWER_SUPPLY_STATUS_DISCHARGING))
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_FULL;
//End added by hailong.chen for defect 5774243 set full status on 2017/12/22
...
/* ============================================================ */
int g_fg_battery_id;
// Begin added by hailong.chen for XR5598906 adding deviceinfo on 2017/11/24
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);

char battery_module_name[256]="BAT:NA:NA";
...
static ssize_t battery_info_store(struct device* dev,
                                  struct device_attribute *attr, const char *buf, size_t count)
{
        return count;
}

DEVICE_ATTR(battery_info, S_IWUSR | S_IRUGO, battery_info_show, battery_info_store);	//modified by shiqiang.chen.hz on 2018/01/25, modified battery deviceinfo filenode name.
...
static void Create_battery_node_ForMMI(void)
{
    ...
    battery=get_deviceinfo_dev();
//Begin modified by shiqiang.chen.hz on 2018/01/25, modified battery deviceinfo filenode name.
    //if (device_create_file(battery, &dev_attr_battery) < 0)
    if (device_create_file(battery, &dev_attr_battery_info) < 0)
//End modified by shiqiang.chen.hz on 2018/01/25, modified battery deviceinfo filenode name.
        printk("Failed to create device file(%s)!\n", dev_attr_battery_info.attr.name);
    return;
}
// End added by hailong.chen for XR5598906 adding deviceinfo on 2017/11/24 
...
int battery_get_batt_id(void)
{
	...
	for (i = 0; i < 3; i++)
	{
	   ret = IMM_GetOneChannelValue(BATTERY_ID_CHANNEL_NUM,data,&rawdata);
	   if (ret != 0)
	   {
		bm_debug("[battery_get_batt_id]id_volt read fail\n");
		return 0;
	   }
	   else
	   {
		bm_debug("[battery_get_batt_id]id_volt = %d\n", rawdata);
	   }
	   msleep(5);
	   id_volt += (rawdata * 1500 / 4096);
	   printk("adc: channel%d[%d]vol =%d\n",BATTERY_ID_CHANNEL_NUM,i,rawdata * 1500 / 4096);
	}
...
//Begin modified by hailong.chen for XR5598906 get id on 2017/11/24
#if 0
...
void fgauge_get_profile_id(void)
{
	//int id_volt = 0;//Deleted by hailong.chen for task 5662647 enable global id_volt on 2017/11/28
	int ret = 0;
	int data[4] = {0,0,0,0};
	int i;
	int rawdata;
	...
	int bat_id = 0;
	static int batid_max, batid_min;
	const char *battery_name = NULL;
	int Q_max = 0;
	id_volt = 0;//Added by hailong.chen for task 5662647 id_volt = 0 on 2017/11/28
	...
	np = of_find_compatible_node(NULL, NULL, (const char *)"mediatek,bat_gm30");
	if (NULL == np)
	{
		printk("get battery node failed\n");
		goto ERR;
	}

	for (child = of_get_next_child(np, NULL); child != NULL; child = of_get_next_child(np,child))
	{
		ret = of_property_read_u32(child, "maxid", &batid_max);
		ret = of_property_read_u32(child, "minid", &batid_min);
		printk("maxid:%d, minid:%d, id_volt:%d\n", batid_max, batid_min, id_volt);
		if(id_volt > batid_min && id_volt < batid_max)
		{
			of_property_read_string(child, "battery_name", &battery_name);
			of_property_read_u32(child, "battery_id", &bat_id);
			of_property_read_u32(child, "Q_max", &Q_max);
			printk("battery_name battery_id:%d battery_name:%s Q_max:%d \n",bat_id, battery_name, Q_max);
			sprintf(battery_module_name,"BAT:%s:%d", battery_name, Q_max);
			break;
		}
	}
	
	//battery node not found,g_fg_battery_id assignment byd id,battery assignment default byd node
	if (NULL == child)
	{
		bm_err("failed to find compatible battery ID, will user default parameter\n");
		child = of_get_next_child(np, NULL);
		of_property_read_u32(child, "battery_id", &bat_id);	
	}
	battery = child;
	g_fg_battery_id = bat_id;
	return;

ERR:
	/*	bat_gm30 node not found
	*	g_fg_battery_id assignment 0
	*	battery assignment NULL
	*/
	g_fg_battery_id = 0;
	battery = NULL;
}
//End modified by hailong.chen for XR5598906 get id on 2017/11/24
...
//Begin modified by hailong.chen for XR5598906 on 2017/11/24
#if 0
		switch (bat_id) {
		case 0:
...
#endif
//if battery is null,then do nothing.
//if not null,then from dts get battery parameter to overwrite the mtk_battery_table.h parameter.
	if (battery != NULL)
	{
			ret0 = !of_property_read_u32(battery, "battery_profile_t0_num", &val_0);
			ret1 = !of_property_read_u32(battery, "battery_profile_t1_num", &val_1);
			ret2 = !of_property_read_u32(battery, "battery_profile_t2_num", &val_2);
			ret3 = !of_property_read_u32(battery, "battery_profile_t3_num", &val_3);
#ifdef CONFIG_MTK_ADDITIONAL_BATTERY_TABLE
			ret4 = !of_property_read_u32(battery, "battery_profile_t4_num", &val_4);
#endif
	if (ret0) {
...
#endif
			fg_custom_parse_table(battery, "battery_profile_t0",
				fg_table_cust_data.fg_profile_t0);
			fg_custom_parse_table(battery, "battery_profile_t1",
				fg_table_cust_data.fg_profile_t1);
			fg_custom_parse_table(battery, "battery_profile_t2",
				fg_table_cust_data.fg_profile_t2);
			fg_custom_parse_table(battery, "battery_profile_t3",
				fg_table_cust_data.fg_profile_t3);
#ifdef CONFIG_MTK_ADDITIONAL_BATTERY_TABLE
			fg_custom_parse_table(battery, "battery_profile_t4",
				fg_table_cust_data.fg_profile_t4);
#endif
	}			/*if (battery != NULL) end*/
//End modified by hailong.chen for XR5598906 on 2017/11/24
...
{
/*Modified by rongxiao.deng for 5424735 on 2017/10/21 begin*/
#ifndef DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY
	...
	return 25;
#endif
/*Modified by rongxiao.deng for 5424735 on 2017/10/21 end*/
}
...
//Begin modified by hailong.chen for task 5818123 temperature behaviour on 2017/12/28
#ifndef TARGET_BUILD_MMITEST
#ifndef DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY
/*Begin added by rongxiao.deng for task on 2017/12/14*/
struct timespec last_tempreture_error_time;
static void mt_battery_thermal_check(void)
{
	struct timespec now_tempreture_error_time;
    unsigned int delta_tempreture_error_time = 0;
    static unsigned int total_tempreture_error_time = 0;
    static int is_first_time_error_temp =0 ;
	if((BMT_status.temperature >= 60 )||(BMT_status.temperature <= -20))
	{
		is_first_time_error_temp++;
		getrawmonotonic(&now_tempreture_error_time);
		if(is_first_time_error_temp==1)
			last_tempreture_error_time.tv_sec=now_tempreture_error_time.tv_sec;

		delta_tempreture_error_time = now_tempreture_error_time.tv_sec - last_tempreture_error_time.tv_sec;
		total_tempreture_error_time=total_tempreture_error_time+delta_tempreture_error_time;
		last_tempreture_error_time.tv_sec= now_tempreture_error_time.tv_sec;
		printk("delta_time:%d, total_time:%d \n",delta_tempreture_error_time, total_tempreture_error_time);
		if(total_tempreture_error_time>=30)
		{
...
/*End added by rongxiao.deng for task on 2017/12/14*/
#endif
#endif
//End modified by hailong.chen for task 5818123 temperature behaviour on 2017/12/28
...
//Begin added by rongxiao.deng for task 5424620 on 2017/11/27 
int chg_enable = true;
extern void charging_suspend_enable(void);
extern void charging_suspend_disable(void);
static ssize_t show_chg_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
       bm_err("show_chg_enable\n");
       return sprintf(buf, "%u\n", chg_enable);
}

static ssize_t store_chg_enable(struct device *dev, struct device_attribute *attr,
                                 const char *buf, size_t size)
{

       bm_err("store_chg_enable\n");
       if (kstrtouint(buf, 10, &chg_enable) == 0) {
               bm_err("chg_enable = %d\n", chg_enable);
...
static DEVICE_ATTR(chg_enable, 0664, show_chg_enable, store_chg_enable);
//End added by rongxiao.deng for task 5424620 on 2017/11/27
...
/*****************************/
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_chg_enable); //added by rongxiao.deng for task 5424620 on 2017/11/27
	...
	sw_iavg_init();
//Begin added by hailong.chen for XR5598906 create battery deviceinfo on 2017/11/24 
	Create_battery_node_ForMMI();
//End added by hailong.chen for XR5598906 create battery deviceinfo on 2017/11/24 
	return 0;
}
