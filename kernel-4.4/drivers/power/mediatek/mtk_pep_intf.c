...
static bool pep_is_cable_out_occur; /* Plug out happened while detecting PE+ */
...
static int pep_check_leave_status(void)
{
	...
	/* Check SOC & Ichg */
	if (batt_cust_data.ta_stop_battery_soc < BMT_status.SOC &&
...
int mtk_pep_check_charger(void)
{
	...
	if (!pep_to_check_chr_type ||
	    ...
	    batt_cust_data.ta_start_battery_soc > BMT_status.SOC ||
	    batt_cust_data.ta_stop_battery_soc <= BMT_status.SOC)
		goto _err;
