...
/*Begin modified by rongxiao.deng for task 5495865 minisw on 2017/11/27 */
#ifndef TARGET_BUILD_MMITEST
#if ((!defined CONFIG_FCC_PTCRB_SUPPORT) && (!defined TARGET_BUILD_CERTIFICATION))
...
#endif
#else
		kpd_info("MINI Enable Powerkey + Homekey in normal mode LPRST\n");
		pmic_set_register_value(PMIC_RG_PWRKEY_RST_EN, 0x01);
		pmic_set_register_value(PMIC_RG_HOMEKEY_RST_EN, 0x01);
#endif
	} else {
...
#ifndef TARGET_BUILD_MMITEST
#if ((!defined CONFIG_FCC_PTCRB_SUPPORT) && (!defined TARGET_BUILD_CERTIFICATION))//Modified by rongxiao.deng for task 5440157 on 2017/10/31
		...
		pmic_set_register_value(PMIC_RG_HOMEKEY_RST_EN, 0x01);
#endif
/*End modified by rongxiao.deng for task 5495865 minisw on 2017/11/27*/
	}
#endif
}
