...
{
#if defined CONFIG_MTK_PMIC_NEW_ARCH/*for pmic not ready*/
...
void mt_eint_register(void)
{
	...
	g_num = of_get_named_gpio(node, "deb-gpios", 0);
	ret = of_property_read_u32(node, "debounce", &kpd_deb);
	if (ret)
		kpd_print("fail to get dts deb\n");
	else
		gpio_set_debounce(g_num, kpd_deb);
