...
static irqreturn_t ff_ctl_device_irq(int irq, void *dev_id)
{
    ...
    wake_lock_timeout(&g_context->wakelock, msecs_to_jiffies(5000));
...
static int ff_ctl_init_driver(void)
{
    ...
    do {
        ...
        FF_LOGI("ff_ctl_init_driver------2");
        /* Register spidev device. For REE-Emulation solution only. */
        //if (g_config && g_config->enable_spidev) {
        //    err = ff_spi_register_device(g_config->spidev_bus, g_config->spidev_c_s);
        //    if (err) {
        //        FF_LOGE("ff_spi_register_device(..) = %d.", err);
        //        break;
        //    }
        //}

        enable_irq_wake(g_context->irq_num);
        FF_LOGI("ff_ctl_init_driver------3");
    } while (0);
...
static int __init ff_ctl_driver_init(void)
{
    ...
    wake_lock_init(&ff_ctl_context.wakelock, WAKE_LOCK_SUSPEND, "focaltech_wakelock");
...
static void __exit ff_ctl_driver_exit(void)
{
    ...
    wake_lock_destroy(&g_context->wakelock);
