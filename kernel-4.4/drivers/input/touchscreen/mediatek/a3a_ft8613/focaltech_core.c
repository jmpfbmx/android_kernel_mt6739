...
static int fts_report_touch(struct ts_event *data)
{
    ...
    for (i = 0; i < data->touch_point; i++)
    {
        ...
        if (data->au8_touch_event[i]== 0 || data->au8_touch_event[i] == 2)
        {
            ...
            //FTS_DEBUG("[B]P%d(%4d,%4d)[p:%d,tm:%d] DOWN!", data->au8_finger_id[i], data->au16_x[i], data->au16_y[i],
...
extern int tp_firmware_version;//Added by qicai.gu.hz for Task 5000517 on 2017/07/06
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    int retval = 0;
    ...
    FTS_DEBUG("[TPD] Touch Panel read the Firmware version = 0x%x\n", uc_tp_fm_ver);
    tp_firmware_version=uc_tp_fm_ver;
    ...
    //Modify by longfang.liu.hz for Task 5710878 on 2017/12/06 begin
    snprintf(ctp_module_name, PAGE_SIZE, "FT8613:%s:0x%x", "CSOT",  tp_firmware_version); 
    //Modify by longfang.liu.hz for Task 5710878 on 2017/12/06  end
    return 0;
}
