...
#define DEVICE_DRIVER_RELEASE_VERSION   ("7.0.4.0")
...
#define CONFIG_USE_IRQ_INTERRUPT_FOR_MTK_PLATFORM //notice!!!
...
#define ILITEK_ILI21XX_FIRMWARE_SIZE (160)  // 160KB. The xxx.hex file size of ILI21XX shall be smaller than 160KB.
#define ILITEK_UPDATE_FIRMWARE_PAGE_LENGTH (128) // 32byte/64byte/128byte/256byte
#define ILITEK_MAX_UPDATE_FIRMWARE_BUFFER_SIZE (256) // 256KB. The xxx.ili file size shall be large enough for stored any kind firmware size of ILI21XX(256KB).
...
typedef struct
{
	...
	u16 nKeyAreaXLength;
	u16 nKeyAreaYLength;
	struct IliTekKeyInfo_t tKeyInfo[10];
...
//------------------------------------------------------------------------------//
extern s32 InWrite(u32 nAddr);
extern s32 OutWrite(u32 nAddr, u32 nData, s32 nSize);
extern s32 InWriteOneByte(u32 nAddr);
extern s32 vfIceRegRead(s32 nInAddr);
extern s32 ExitIceMode(void);

extern s32 DrvIliTekReadTouchPanelInfo(IliTekTpInfo_t *pTpInfo);
extern s32 DrvIliTekConvertFirmwareData(u8 *pBuf, u32 nSize);
extern s32 DrvIliTekUpdateFirmware(u8 *pszFwData, bool bIsUpdateFirmwareBySwId);
//------------------------------------------------------------------------------//
