...
#include "ilitek_drv_hotknot.h"
#include "ilitek_drv_hotknot_queue.h"
...
#include "mp_test/MpCommon.h"

/*** begin: added by liuhe.zhong for auto upgrade when normal boot on 2017.12.19 ***/
#include <mt-plat/mtk_boot_common.h>
/*** end:   added by liuhe.zhong for auto upgrade when normal boot on 2017.12.19 ***/

/*=============================================================*/
// VARIABLE DECLARATION
/*=============================================================*/
/*** begin: modified by liuhe.zhong for rawdata test on 2017.12.08 ***/
unsigned char tp_info[128] = {0};
EXPORT_SYMBOL(tp_info);
/*** end:   modified by liuhe.zhong for rawdata test on 2017.12.08 ***/

/*** begin: added by liuhe.zhong for getting lcm color on 2018.03.08 **/
extern unsigned char lcm_color;
/*** end: added by liuhe.zhong for getting lcm color on 2018.03.08 **/

extern struct i2c_client *g_I2cClient;
#define I2C_DMA_FLAG  (0x2000) //add by liuyang3.wt.
static unsigned char proc_operate_mode =0;
...
u8 g_DemoModePacket[ILI21XX_DEMO_MODE_PACKET_LENGTH] = {0}; // for MSG22xx : DEMO_MODE_PACKET_LENGTH = SELF_DEMO_MODE_PACKET_LENGTH, for MSG28xx/MSG58xxA : DEMO_MODE_PACKET_LENGTH = MUTUAL_DEMO_MODE_PACKET_LENGTH, for ILI21xx : DEMO_MODE_PACKET_LENGTH = ILI21XX_DEMO_MODE_PACKET_LENGTH

MutualFirmwareInfo_t g_MutualFirmwareInfo;
SelfFirmwareInfo_t g_SelfFirmwareInfo;
IliTekTpInfo_t g_IliTekTpInfo;

u8 g_IliTekFwData[ILITEK_MAX_UPDATE_FIRMWARE_BUFFER_SIZE*1024];
u8 g_IliTekFwDataBuf[ILITEK_ILI21XX_FIRMWARE_SIZE*1024] = {0}; // for update firmware(ILI21xx) from SD card
...
/* Create ILITEK Node */
extern int DrvCreateDeviceNode(void);
...
#ifdef CONFIG_ENABLE_HOTKNOT
static s32 _DrvHotknotRegistry(void);
#endif //CONFIG_ENABLE_HOTKNOT
...
//    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
    ...
    szOutBuf[0] = 0x25;
    szOutBuf[1] = (char)((nAddr & 0x000000FF) >> 0);
    szOutBuf[2] = (char)((nAddr & 0x0000FF00) >> 8);
    szOutBuf[3] = (char)((nAddr & 0x00FF0000) >> 16);
    
    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, szOutBuf, 4);
//    udelay(10);
    rc = IicReadData(SLAVE_I2C_ID_DWI2C, szOutBuf, 4);
    
    nData = (szOutBuf[0] + szOutBuf[1] * 256 + szOutBuf[2] * 256 * 256 + szOutBuf[3] * 256 * 256 * 256);
//    DBG(&g_I2cClient->dev, "nData=0x%x, szOutBuf[0]=%x, szOutBuf[1]=%x, szOutBuf[2]=%x, szOutBuf[3]=%x\n", nData, szOutBuf[0], szOutBuf[1], szOutBuf[2], szOutBuf[3]);

    return nData;
}

s32 OutWrite(u32 nAddr, u32 nData, s32 nSize)
{
    s32 rc = 0, i = 0;
    char szOutBuf[64] = {0};
    
//    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
//    DBG(&g_I2cClient->dev, "nAddr=0x%X, nData=0x%X, nSize=%d\n", nAddr, nData, nSize); // TODO : add for debug
    ...
    for (i = 0; i < nSize; i ++)
    {
        szOutBuf[i + 4] = (char)(nData >> (8 * i));
    }
	
    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, szOutBuf, nSize + 4);
	
    return rc;
}

s32 InWriteOneByte(u32 nAddr)
{
    s32 rc = 0, nData = 0;
    char szOutBuf[64] = {0};
...
//    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
    ...
    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, szOutBuf, 4);
//    udelay(10);
    rc = IicReadData(SLAVE_I2C_ID_DWI2C, szOutBuf, 1);
    nData = (szOutBuf[0]);
	
    return nData;
}

s32 vfIceRegRead(s32 nInAddr)
{
    s32 i = 0;
    s32 nInTimeCount = 100;
    u8 szBuf[4] = {0};

//    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);

    OutWrite(0x41000, 0x3B | (nInAddr << 8), 4);
    OutWrite(0x041004, 0x66AA5500, 4);
    // Check Flag
    // Check Busy Flag
    for (i = 0; i < nInTimeCount; i ++)
    {
        szBuf[0] = InWriteOneByte(0x41011);
    ...
    return InWriteOneByte(0x41012);
}

s32 ExitIceMode(void)
{
    s32 rc = 0;
...
//    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);  
	...
	{
        DBG(&g_I2cClient->dev, "ICE mode reset\n");  
		
        rc = OutWrite(0x04004C, 0x2120, 2);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "OutWrite(0x04004C, 0x2120, 2) error, rc = %d\n", rc);  
        }
        mdelay(10);
		
        rc = OutWrite(0x04004E, 0x01, 1);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "OutWrite(0x04004E, 0x01, 1) error, rc = %d\n", rc);  
        }
    }
    ...
    if (g_I2cClient != NULL)
    {
        ...
        else
        {
            ...
            if (nSize > 8 && NULL != I2CDMABuf_va)
            {
                ...
                g_I2cClient->addr = g_I2cClient->addr | I2C_DMA_FLAG;
...
/*** begin: modified by liuhe.zhong for report point performance on 2017.12.14 ***/
//  input_report_abs(g_InputDevice, ABS_MT_PRESSURE, nPressure);
/*** end:   modified by liuhe.zhong for report point performance on 2017.12.14 ***/
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
...
//        FIRMWARE_MODE_UNKNOWN_MODE = MSG22XX_FIRMWARE_MODE_UNKNOWN_MODE;
    ...
    else if (g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
        ...
        DEBUG_MODE_PACKET_LENGTH = MUTUAL_DEBUG_MODE_PACKET_LENGTH;
        MAX_TOUCH_NUM = MUTUAL_MAX_TOUCH_NUM; /* g_IliTekTpInfo.nMaxTouchNum */
    }	
}	
...
static u16 _DrvChangeFirmwareMode(u16 nMode)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)  
    {
        u8 szTxData[2] = {0};
        u32 i = 0;
        s32 rc;
        ...
        if (g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
        {
            szTxData[0] = 0xD1; 
        }
        else
...
static void _DrvMutualGetFirmwareInfo(MutualFirmwareInfo_t *pInfo) // for MSG28xx/MSG58xxA/ILI21xx
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)   
    {
        ...
        if ((szRxData[1] == FIRMWARE_MODE_DEBUG_MODE && szRxData[2] == 0xA7 && szRxData[5] == PACKET_TYPE_TOOTH_PATTERN) || (szRxData[1] == FIRMWARE_MODE_DEMO_MODE && szRxData[2] == 0x5A))
        {
...
void DrvRestoreFirmwareModeToLogDataMode(void)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
        if (g_IsSwitchModeByAPK == 1)
        {
...
static void _DrvGetTouchPacketAddress(u16* pDataAddress, u16* pFlagAddress)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121 || (g_ChipType == CHIP_TYPE_MSG22XX && IS_SELF_FREQ_SCAN_ENABLED))   
    {
        s32 rc = 0;
        u32 i = 0;
        u8 szTxData[1] = {0};
...
s32 DrvIliTekReadTouchPanelInfo(IliTekTpInfo_t *pTpInfo)
{
	u32 i = 0;
	u8 szWriteBuf[2] = {0};
	u8 szReadBuf[64] = {0};
    ...
    if (g_ChipType == CHIP_TYPE_ILI2120)
    {
        for (i = 0; i < 20; i ++)
        {
            szWriteBuf[0] = ILITEK_TP_CMD_READ_DATA;
            rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 1);
            if (rc < 0)
            {
                DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA : IicWriteData() failed, rc = %d\n", rc);
            }
        
            mdelay(10);

            rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szReadBuf[0], 3);
            if (rc < 0)
            {
                DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA : IicReadData() failed, rc = %d\n", rc);
            }
        
            DBG(&g_I2cClient->dev, "Check FW status : szReadBuf[0][1][2] = 0x%X, 0x%X, 0x%X\n", szReadBuf[0], szReadBuf[1], szReadBuf[2]);

            if (szReadBuf[1] >= 0x80)
            {
                DBG(&g_I2cClient->dev, "FW is ready\n");
                break;
            }
            else
            {
        ...
        szWriteBuf[0] = ILITEK_TP_CMD_READ_DATA_CONTROL;
        szWriteBuf[1] = ILITEK_TP_ILI2120_CMD_GET_FIRMWARE_VERSION;
        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 2);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA_CONTROL & ILITEK_TP_ILI2120_CMD_GET_FIRMWARE_VERSION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }
    
        mdelay(10);

        szWriteBuf[0] = ILITEK_TP_ILI2120_CMD_GET_FIRMWARE_VERSION;
        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 1);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_FIRMWARE_VERSION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }
        ...
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_FIRMWARE_VERSION : IicReadData() failed, rc = %d\n", rc);
            return -1;
        }

        pTpInfo->szFwVersion[0] = 0;
        
        for (i = 1; i < 4; i ++)
        {
            pTpInfo->szFwVersion[i] = szReadBuf[i-1];
        }

        DBG(&g_I2cClient->dev, "Firmware Version = %d.%d.%d\n", szReadBuf[0], szReadBuf[1], szReadBuf[2]);
    
        // read protocol version
        szWriteBuf[0] = ILITEK_TP_CMD_READ_DATA_CONTROL;
        szWriteBuf[1] = ILITEK_TP_ILI2120_CMD_GET_PROTOCOL_VERSION;
        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 2);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA_CONTROL & ILITEK_TP_ILI2120_CMD_GET_PROTOCOL_VERSION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }    

        mdelay(10);

        szWriteBuf[0] = ILITEK_TP_ILI2120_CMD_GET_PROTOCOL_VERSION;
        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 1);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_PROTOCOL_VERSION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }
    
        mdelay(10);

        rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szReadBuf[0], 2);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_PROTOCOL_VERSION : IicReadData() failed, rc = %d\n", rc);
            return -1;
        }

        DBG(&g_I2cClient->dev, "Protocol Version = %d.%d\n", szReadBuf[0], szReadBuf[1]);

		if (szReadBuf[0] == 0)
		{
			pTpInfo->nProtocolVersion = ((szReadBuf[1]) << 8) + szReadBuf[0];
		}
		else
		{
			pTpInfo->nProtocolVersion = ((szReadBuf[0]) << 8) + szReadBuf[1];
		}

        // read touch resolution
        szWriteBuf[0] = ILITEK_TP_CMD_READ_DATA_CONTROL;
        szWriteBuf[1] = ILITEK_TP_CMD_GET_RESOLUTION;
        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 2);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA_CONTROL & ILITEK_TP_CMD_GET_RESOLUTION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }    
        ...
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_GET_RESOLUTION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }
    
        mdelay(10);

        rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szReadBuf[0], 10);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_GET_RESOLUTION : IicReadData() failed, rc = %d\n", rc);
            return -1;
        }
    
        // calculate the resolution for x and y direction
        pTpInfo->nMaxX = szReadBuf[2];
        pTpInfo->nMaxX += (szReadBuf[3]*256);
        pTpInfo->nMaxY = szReadBuf[4];
        pTpInfo->nMaxY += (szReadBuf[5]*256);
        pTpInfo->nMinX = szReadBuf[0];
        pTpInfo->nMinY = szReadBuf[1];
        pTpInfo->nXChannelNum = szReadBuf[6];
        pTpInfo->nYChannelNum = szReadBuf[7];
        pTpInfo->nMaxTouchNum = szReadBuf[8];
        pTpInfo->nKeyCount = szReadBuf[9];
    
        DBG(&g_I2cClient->dev, "nMinX=%d, nMaxX=%d, nMinY=%d, nMaxY=%d, nXChannelNum=%d, nYChannelNum=%d, nMaxTouchNum=%d, nKeyCount=%d\n", pTpInfo->nMinX, pTpInfo->nMaxX, pTpInfo->nMinY, pTpInfo->nMaxY, pTpInfo->nXChannelNum, pTpInfo->nYChannelNum, pTpInfo->nMaxTouchNum, pTpInfo->nKeyCount);

        // read key information
        if (pTpInfo->nKeyCount)
        {
            szWriteBuf[0] = ILITEK_TP_CMD_READ_DATA_CONTROL;
            szWriteBuf[1] = ILITEK_TP_ILI2120_CMD_GET_KEY_INFORMATION;
            rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 2);
            if (rc < 0)
            {
                DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA_CONTROL & ILITEK_TP_ILI2120_CMD_GET_KEY_INFORMATION : IicWriteData() failed, rc = %d\n", rc);
                return -1;
            }    

            mdelay(10);

            szWriteBuf[0] = ILITEK_TP_ILI2120_CMD_GET_KEY_INFORMATION;
            rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 1);
            if (rc < 0)
            {
                DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_KEY_INFORMATION : IicWriteData() failed, rc = %d\n", rc);
                return -1;
            }

            mdelay(10);

            rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szReadBuf[0], 29);
            if (rc < 0)
            {
                DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_KEY_INFORMATION : IicReadData() failed, rc = %d\n", rc);
                return -1;
            }

            if (pTpInfo->nKeyCount > 5)
            {
                rc = IicReadData(SLAVE_I2C_ID_DWI2C, (szReadBuf+29), 25);
                if (rc < 0)
                {
                    DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2120_CMD_GET_KEY_INFORMATION : IicReadData() failed, rc = %d\n", rc);
                    return -1;
            ...
            pTpInfo->nKeyAreaXLength = (szReadBuf[0] << 8) + szReadBuf[1];
            pTpInfo->nKeyAreaYLength = (szReadBuf[2] << 8) + szReadBuf[3];

            DBG(&g_I2cClient->dev, "nKeyAreaXLength=%d, nKeyAreaYLength=%d\n", pTpInfo->nKeyAreaXLength, pTpInfo->nKeyAreaYLength);

            for (i = 0; i < pTpInfo->nKeyCount; i ++)
            {
                pTpInfo->tKeyInfo[i].nId = szReadBuf[i*5+4];	
                pTpInfo->tKeyInfo[i].nX = (szReadBuf[i*5+5] << 8) + szReadBuf[i*5+6];
                pTpInfo->tKeyInfo[i].nY = (szReadBuf[i*5+7] << 8) + szReadBuf[i*5+8];
                pTpInfo->tKeyInfo[i].nStatus = 0;
            
                DBG(&g_I2cClient->dev, "KeyInfo[%d].nId=%d, KeyInfo[%d].nX=%d, KeyInfo[%d].nY=%d, KeyInfo[%d].nStatus=%d\n", i, pTpInfo->tKeyInfo[i].nId, i, pTpInfo->tKeyInfo[i].nX, i, pTpInfo->tKeyInfo[i].nY, i, pTpInfo->tKeyInfo[i].nStatus);
            }
        }
    }
    else if (g_ChipType == CHIP_TYPE_ILI2121)
    {
        // read firmware version
        ...
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2121_CMD_GET_FIRMWARE_VERSION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }
    
        mdelay(10);

        rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szReadBuf[0], 4);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2121_CMD_GET_FIRMWARE_VERSION : IicReadData() failed, rc = %d\n", rc);
            return -1;
        }
    
        for (i = 0; i < 4; i ++)
        {
            pTpInfo->szFwVersion[i] = szReadBuf[i];
        }

        DBG(&g_I2cClient->dev, "Firmware Version = %d.%d.%d.%d\n", szReadBuf[0], szReadBuf[1], szReadBuf[2], szReadBuf[3]);
        ...
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2121_CMD_GET_PROTOCOL_VERSION : IicWriteData() failed, rc = %d\n", rc);
            return -1;
        }
        ...
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_ILI2121_CMD_GET_PROTOCOL_VERSION : IicReadData() failed, rc = %d\n", rc);
            return -1;
        }
        ...
        // calculate the resolution for x and y direction
        pTpInfo->nMaxX = szReadBuf[0];
        pTpInfo->nMaxX += (szReadBuf[1]*256);
        pTpInfo->nMaxY = szReadBuf[2];
        pTpInfo->nMaxY += (szReadBuf[3]*256);
        pTpInfo->nMinX = 0;
        pTpInfo->nMinY = 0;
        pTpInfo->nXChannelNum = szReadBuf[4];
        pTpInfo->nYChannelNum = szReadBuf[5];
        pTpInfo->nMaxTouchNum = szReadBuf[6];
        pTpInfo->nMaxKeyButtonNum = szReadBuf[7];
        pTpInfo->nKeyCount = szReadBuf[8];
    
        DBG(&g_I2cClient->dev, "nMaxX=%d, nMaxY=%d, nXChannelNum=%d, nYChannelNum=%d, nMaxTouchNum=%d, nMaxKeyButtonNum=%d, nKeyCount=%d\n", pTpInfo->nMaxX, pTpInfo->nMaxY, pTpInfo->nXChannelNum, pTpInfo->nYChannelNum, pTpInfo->nMaxTouchNum, pTpInfo->nMaxKeyButtonNum, pTpInfo->nKeyCount);

        // read key information
        if ((pTpInfo->nProtocolVersion & 0x300) == 0x300)
        {
            if (pTpInfo->nKeyCount)
            {
                ...
                if (rc < 0)
                {
                    DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_GET_KEY_INFORMATION : IicWriteData() failed, rc = %d\n", rc);
                    return -1;
                }    
                ...
                if (rc < 0)
                {
                    DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_GET_KEY_INFORMATION : IicReadData() failed, rc = %d\n", rc);
                    return -1;
                }
...
static s32 _DrvIliTekReadAndParseFingerTouchData(u8 *pPacket, u16 nReportPacketLength, MutualTouchInfo_t *pInfo)
{
    u32 i = 0, j = 0;
    s32 rc;
    u16 nX = 0;
...
//    {	
        if (g_FirmwareMode == FIRMWARE_MODE_DEMO_MODE)
        {
            ...
            if (g_ChipType == CHIP_TYPE_ILI2121 || (g_ChipType == CHIP_TYPE_ILI2120 && g_IliTekTpInfo.nProtocolVersion > 0x100))
            {
            }
            else
            {
            ...
            DBG(&g_I2cClient->dev, "pPacket[0]=%x pPacket[1]=%x pPacket[2]=%x\n", pPacket[0], pPacket[1], pPacket[2]);

#ifdef DEBUG_NETLINK
            if (debug_flag) 
            {				
                udp_reply(pid, seq, pPacket, 53);	
            }	
            ...
            if (g_GestureWakeupFlag == 1)
            {
                ...
                {
                    case 0x60:
                        DBG(&g_I2cClient->dev, "gesture wake up this is c\n");
                        ...
                        break;
                    case 0x62:
                        DBG(&g_I2cClient->dev, "gesture wake up this is e\n");
                        ...
                        break;
                    case 0x64:
                        DBG(&g_I2cClient->dev, "gesture wake up this is m\n");
                        ...
                        break;		
                    case 0x66:
                        DBG(&g_I2cClient->dev, "gesture wake up this is w\n");
                        ...
                        break;		
                    case 0x68:
                        DBG(&g_I2cClient->dev, "gesture wake up this is o\n");
                        ...
                        break;
                    case 0x84:
                        DBG(&g_I2cClient->dev, "gesture wake up this is slide right\n");
                        ...
                        break;
                    case 0x8c:
                        DBG(&g_I2cClient->dev, "gesture wake up this is slide left\n");
                        ...
                        break;
                    case 0x88:
                        DBG(&g_I2cClient->dev, "gesture wake up this is slide down\n");
                        ...
                        break;
                    case 0x80:
                        DBG(&g_I2cClient->dev, "gesture wake up this is slide up\n");
                        ...
                        break;
                    case 0x22:
                        DBG(&g_I2cClient->dev, "gesture wake up this is duble click\n");
                        ...
                        break;
                    default:
                        DBG(&g_I2cClient->dev, "no support this gesture!\n");
                        break;		
                }
            ...
            for (i = 0; i < g_IliTekTpInfo.nMaxTouchNum/* MAX_TOUCH_NUM */; i ++)
            {
                if (g_ChipType == CHIP_TYPE_ILI2121 || (g_ChipType == CHIP_TYPE_ILI2120 && g_IliTekTpInfo.nProtocolVersion > 0x100))
                {
                    nTouchStatus = (pPacket[i*5+3] >> 7);	
                    nX = (((pPacket[i*5+3] & 0x3F) << 8) + pPacket[i*5+4]);
                    nY = (((pPacket[i*5+5] & 0x3F) << 8) + pPacket[i*5+6]);
                }
                else
                {
                    if (i >= nLength)
                    ...
                    szWriteBuf[0] = ILITEK_TP_CMD_READ_SUB_DATA;
                    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szWriteBuf[0], 1);
                    if (rc < 0)
                    {
                        DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_SUB_DATA : IicWriteData() failed, rc = %d\n", rc);
                        return -1;
                    }

                    rc = IicReadData(SLAVE_I2C_ID_DWI2C, &pPacket[0], 7);
                    if (rc < 0)
                    {
                        DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_SUB_DATA : IicReadData() failed, rc = %d\n", rc);
                        return -1;
                    }
                    
                    if ((pPacket[0] & 0xC0) == 0x80)
                    {
                        nTouchStatus = 1;
                    }
                    else
                    ...
                    nX = ((pPacket[2] << 8) + pPacket[1]);
                    nY = ((pPacket[4] << 8) + pPacket[3]);
                    nTouchId = (pPacket[0] & 0x3F) - 1;
                }
#ifdef CONFIG_SWAP_X_Y
                nTemp = nX;
                nX = nY;
                ...
                nX = g_IliTekTpInfo.nMaxX - nX;
#endif //CONFIG_REVERSE_X
				
#ifdef CONFIG_REVERSE_Y
                nY = g_IliTekTpInfo.nMaxY - nY;
#endif //CONFIG_REVERSE_X

                if (nTouchStatus)
                {
                    DBG(&g_I2cClient->dev, "TOUCH_POINT : i=%d, nX=%d, nY=%d\n", i, nX, nY);

//#ifdef VIRTUAL_KEY_PAD
                    if (0 == g_IliTekTpInfo.nHandleKeyFlag) // can handle key
                    {
                        for (j = 0; j < g_IliTekTpInfo.nKeyCount; j ++)
                        {
                            if ((nX >= g_IliTekTpInfo.tKeyInfo[j].nX && nX <= g_IliTekTpInfo.tKeyInfo[j].nX + g_IliTekTpInfo.nKeyAreaXLength) && (nY >= g_IliTekTpInfo.tKeyInfo[j].nY && nY <= g_IliTekTpInfo.tKeyInfo[j].nY + g_IliTekTpInfo.nKeyAreaYLength))
                            {
                                input_report_key(g_InputDevice, BTN_TOUCH, 1);
                                input_report_key(g_InputDevice, g_IliTekTpInfo.tKeyInfo[j].nId, 1);
                                input_sync(g_InputDevice);
                                
                                g_IliTekTpInfo.tKeyInfo[j].nStatus = 1;
                                _gIsFirstTouchKeyPressed[i] = 1;
            
                                DBG(&g_I2cClient->dev, "KEY : Key Down => nId=%d, nX=%d, nY=%d, nStatus=%d, nHandleKeyFlag=%d\n", g_IliTekTpInfo.tKeyInfo[j].nId, nX, nY, g_IliTekTpInfo.tKeyInfo[j].nStatus, g_IliTekTpInfo.nHandleKeyFlag);
                                break;
                            }
                        }
                    }

                    if (0 == _gIsFirstTouchKeyPressed[i])
//#endif //VIRTUAL_KEY_PAD
                    {
                        pInfo->tPoint[pInfo->nCount].nX = nX;
                        pInfo->tPoint[pInfo->nCount].nY = nY;
#ifdef CONFIG_ENABLE_REPORT_PRESSURE
                        pInfo->tPoint[pInfo->nCount].nP = pPacket[i*5+7];
#else
                        pInfo->tPoint[pInfo->nCount].nP = 10;
                        ...
                        {
                            pInfo->tPoint[pInfo->nCount].nId = nTouchId;
#ifdef CONFIG_ENABLE_TYPE_B_PROTOCOL
                            _gCurrentTouch[nTouchId] = 1;
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL
                        }
                        pInfo->nCount ++;

                        g_IliTekTpInfo.nHandleKeyFlag = 1; // can not handle key
                    }

//#ifdef VIRTUAL_KEY_PAD
                    if (1 == _gIsFirstTouchKeyPressed[i])
                    {
                        for (j = 0; j < g_IliTekTpInfo.nKeyCount; j ++)
                        {
                            if ((1 == g_IliTekTpInfo.tKeyInfo[j].nStatus) && (nX < g_IliTekTpInfo.tKeyInfo[j].nX || nX > g_IliTekTpInfo.tKeyInfo[j].nX + g_IliTekTpInfo.nKeyAreaXLength || nY < g_IliTekTpInfo.tKeyInfo[j].nY || nY > g_IliTekTpInfo.tKeyInfo[j].nY + g_IliTekTpInfo.nKeyAreaYLength))
                            {
                                input_report_key(g_InputDevice, BTN_TOUCH, 0);
                                input_report_key(g_InputDevice, g_IliTekTpInfo.tKeyInfo[j].nId, 0);
                                input_sync(g_InputDevice);

                                g_IliTekTpInfo.tKeyInfo[j].nStatus = 0;
                                _gIsFirstTouchKeyPressed[i] = 0;

                                DBG(&g_I2cClient->dev, "KEY : Key Up => nId=%d, nX=%d, nY=%d, nStatus=%d, nHandleKeyFlag=%d\n", g_IliTekTpInfo.tKeyInfo[j].nId, nX, nY, g_IliTekTpInfo.tKeyInfo[j].nStatus, g_IliTekTpInfo.nHandleKeyFlag);
                                break;
                            }
                        }
                    }
//#endif //VIRTUAL_KEY_PAD
                ...
                {
                    DBG(&g_I2cClient->dev, "RELEASE_POINT : i=%d\n", i);
                    ...
                    if (g_ChipType == CHIP_TYPE_ILI2121 || (g_ChipType == CHIP_TYPE_ILI2120 && g_IliTekTpInfo.nProtocolVersion > 0x100))
                    {
                        if (1 == _gIsFirstTouchKeyPressed[i])
                        {
                            for (j = 0; j < g_IliTekTpInfo.nKeyCount; j ++)
                            {
                                if ((1 == g_IliTekTpInfo.tKeyInfo[j].nStatus) && 
                                    (nX < g_IliTekTpInfo.tKeyInfo[j].nX || nX > g_IliTekTpInfo.tKeyInfo[j].nX + g_IliTekTpInfo.nKeyAreaXLength || 
                                     nY < g_IliTekTpInfo.tKeyInfo[j].nY || nY > g_IliTekTpInfo.tKeyInfo[j].nY + g_IliTekTpInfo.nKeyAreaYLength))
                                {
                                    input_report_key(g_InputDevice, BTN_TOUCH, 0);
                                    input_report_key(g_InputDevice, g_IliTekTpInfo.tKeyInfo[j].nId, 0);
                                    input_sync(g_InputDevice);
            ...
            if (0 == nLength)
            {
                g_IliTekTpInfo.nHandleKeyFlag = 0; // can handle key
            }
        }
        else if (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE)
        {
            ...
            if (pPacket[0] != 0xA7 && pPacket[3] != PACKET_TYPE_TOOTH_PATTERN)
            {
            ...
            if (g_TouchKObj != NULL)
            {
                ...
                memcpy(g_LogModePacket, g_DebugModePacket, g_MutualFirmwareInfo.nLogModePacketLength); // Copy g_DebugModePacket to g_LogModePacket for avoiding the debug mode data which is received by MTPTool APK may be modified. 
...
static void _DrvIliTekHandleFingerTouch(void) // for ILI21xx
{
    MutualTouchInfo_t tInfo;
    u32 i = 0;
#ifdef CONFIG_TP_HAVE_KEY
...
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
    if (0 != _DrvIliTekReadAndParseFingerTouchData(&pPacket[0], nReportPacketLength, &tInfo))
    {
         goto TouchHandleEnd;		
    }
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
...
static ssize_t DrvMainProcfsMpTestCustomisedWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)  
{
    ...
    if(pBuffer != NULL)
    {
        ...
        if(pValid)
        {
            ...
            if(g_IsInMpTest == 0)
            {
                res = startMPTest(g_ChipType, szFilePath);
                if(res < 0)
                    goto out;
                else
                {
                    save_test_data(g_ChipType);                    
                }
            }
            else
            {
        ...
        {
            DBG(&g_I2cClient->dev, "*** %s(): The format of file is invaild \n", __func__);
        }
    }
    else
    {
        DBG(&g_I2cClient->dev, "*** %s(): The path is invaild \n", __func__);
    }

out:
    EndMpTest(g_ChipType);
    return nCount;
processing:
    return nCount;
}
...
static ssize_t DrvMainProcfsMpTestCustomisedRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    status = get_mp_test_status();
    nLength = sprintf(pBuffer, "%d", status);

    res = copy_to_user((int *)pBuffer, &status, nLength);
    if (res < 0)
    {
...
static ssize_t _DrvProcfsChipTypeRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    {
        DBG(&g_I2cClient->dev, "nMaxX=%d, nMaxY=%d, nXChannelNum=%d, nYChannelNum=%d, nMaxTouchNum=%d, nMaxKeyButtonNum=%d, nKeyCount=%d\n", g_IliTekTpInfo.nMaxX, g_IliTekTpInfo.nMaxY, g_IliTekTpInfo.nXChannelNum, g_IliTekTpInfo.nYChannelNum, g_IliTekTpInfo.nMaxTouchNum, g_IliTekTpInfo.nMaxKeyButtonNum, g_IliTekTpInfo.nKeyCount); // TODO : add for debug
        DBG(&g_I2cClient->dev, "Firmware Version = %d.%d.%d.%d\n", g_IliTekTpInfo.szFwVersion[0], g_IliTekTpInfo.szFwVersion[1], g_IliTekTpInfo.szFwVersion[2], g_IliTekTpInfo.szFwVersion[3]); // TODO : add for debug
        DBG(&g_I2cClient->dev, "Protocol Version = %d\n", g_IliTekTpInfo.nProtocolVersion); // TODO : add for debug
    }
...
static ssize_t _DrvProcfsFirmwareDataWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)  
{
    ...
    else if (g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
        if (nNum > 0) // nCount >= 1024
        {
            for (i = 0; i < nNum; i ++)
            {
                memcpy(g_IliTekFwDataBuf+(g_FwDataCount), pBuffer+(i*1024), 1024);

                g_FwDataCount = g_FwDataCount + 1024;
            }
            ...
            if (nRemainder > 0) 
            {
                ...
                memcpy(g_IliTekFwDataBuf+(g_FwDataCount), pBuffer+(i*1024), nRemainder);

                g_FwDataCount = g_FwDataCount + nRemainder;
            }
        }
        else // nCount < 1024
        {
            if (nCount > 0)
            {
                memcpy(g_IliTekFwDataBuf+(g_FwDataCount), pBuffer, nCount);

                g_FwDataCount = g_FwDataCount + nCount;
            }
        }
    }
    else
...
{    
    ...
    {
        nRetVal = DrvIliTekConvertFirmwareData(g_IliTekFwDataBuf, g_FwDataCount);

        if (0 == nRetVal)
        {
            nRetVal = DrvIliTekUpdateFirmware(g_IliTekFwData, false);    

            DrvIliTekReadTouchPanelInfo(&g_IliTekTpInfo); // Update TP panel info for ILI21xx
        }
    }
    else
    {
...
static ssize_t _DrvProcfsSeLinuxLimitFirmwareUpdateRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    {
        DBG(&g_I2cClient->dev, "ILITEK_FIRMWARE_FILE_PATH_ON_SD_CARD = %s\n", ILITEK_FIRMWARE_FILE_PATH_ON_SD_CARD);
        nRetVal = DrvIliTekUpdateFirmwareBySdCard(ILITEK_FIRMWARE_FILE_PATH_ON_SD_CARD);
    }
    else
    {
...
static ssize_t _DrvProcfsMpTestRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	//Begin modified by chenjindong for tp test  20171202
    u32 nLength = 0;
    ...
    nLength = sprintf(buf, "%d\n", DrvMpTestGetTestResult());

    *pPos += nLength;
 if (copy_to_user(pBuffer, buf, nLength))
    {
        DBG(&g_I2cClient->dev,"[APK]: copy to user error!!");
        return -EFAULT;
    }
    return nLength;
}
...
static ssize_t _DrvProcfsMpTestWrite(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	...
	int buflen = nCount;
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
	if (copy_from_user(&writebuf, pBuffer, buflen))
    {
        DBG(&g_I2cClient->dev,"[APK]: copy from user error!!");
        return -EFAULT;
    }
	proc_operate_mode = writebuf[0]-48;
	DBG(&g_I2cClient->dev, "Mp Test Mode  write buffer = %d\n", writebuf[0]);
    switch (proc_operate_mode)
    {
        case ITO_TEST_MODE_OPEN_TEST:
            _gItoTestMode = ITO_TEST_MODE_OPEN_TEST; //open test
            DrvMpTestScheduleMpTestWork(ITO_TEST_MODE_OPEN_TEST);
			break;
		 case ITO_TEST_MODE_SHORT_TEST:
            _gItoTestMode = ITO_TEST_MODE_SHORT_TEST;  //short test
            DrvMpTestScheduleMpTestWork(ITO_TEST_MODE_SHORT_TEST);
			break;
		 case ITO_TEST_MODE_WATERPROOF_TEST:
           _gItoTestMode = ITO_TEST_MODE_WATERPROOF_TEST; //waterproof test
	...
	return nCount;
	//End modified by chenjindong for tp test  20171202
}
...
static ssize_t _DrvProcfsMpTestLogRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    DrvMpTestGetTestDataLog(_gItoTestMode, pBuffer, &nLength);
...
static ssize_t _DrvProcfsMpTestLogAllRead(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A)
    {
        DrvMpTestGetTestLogAll(pBuffer, &nLength);
    }
    ...
    if (strcmp(pAttr->attr.name, "packet") == 0)
    {
        ...
        else //g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE || g_FirmwareMode == FIRMWARE_MODE_RAW_DATA_MODE
        {
            if (g_LogModePacket != NULL)
            {
                ...
                if ((g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2118A || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121) && (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE) && (g_LogModePacket[0] == 0xA7))
                {
...
#ifdef CONFIG_ENABLE_HOTKNOT
    _DrvHotknotRegistry();    //register hotknot here...         
#endif //CONFIG_ENABLE_HOTKNOT
...
//add for debug,20111103
    ...
    if (g_ChipType != 0) // To make sure TP is attached on cell phone.
    {
        ...
        {
            memset(&g_IliTekTpInfo, 0x0, sizeof(IliTekTpInfo_t));
        }
...
//#ifdef VIRTUAL_KEY_PAD
    if (g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121) 
    {
        nRetVal = DrvIliTekReadTouchPanelInfo(&g_IliTekTpInfo);

        for (i = 0; i < g_IliTekTpInfo.nKeyCount; i ++)
        {
            if (g_IliTekTpInfo.tKeyInfo[i].nId <= 0)
            {
                continue;
            }
		
            set_bit(g_IliTekTpInfo.tKeyInfo[i].nId & KEY_MAX, g_InputDevice->keybit);
        }
    }
//#endif //VIRTUAL_KEY_PAD
...
s32 DrvTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
    ...
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__); 
    /*begin modify by zhihua.fang for enter meta error 20180716 */
    _gInterruptFlag = 1;
    /*end modify by zhihua.fang for enter meta error 20180716 */
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
...
void DrvOpenGestureWakeup(u32 *pMode)
{
    ...
    else if (g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121)
    {
        ...
        if (rc > 0)
        {
            DBG(&g_I2cClient->dev, "Enable gesture wakeup success(Cmd : 0x%x 0x%x)\n", szTxData[0], szTxData[1]);
        }
        else
        {
            DBG(&g_I2cClient->dev, "Enable gesture wakeup failed(Cmd : 0x%x 0x%x), rc = %d\n", szTxData[0], szTxData[1], rc);
        }
...
{
	return sprintf(buf, "ilitek FW version:%d.%d.%d\n", g_IliTekTpInfo.szFwVersion[1], g_IliTekTpInfo.szFwVersion[2], g_IliTekTpInfo.szFwVersion[3]);
}
...
static s32 _DrvCreateProcfsDirEntry(void)
{
	...
	nRetVal = class_create_file(touchscreen_class, &class_attr_ts_info);
	if (nRetVal < 0) {
		DBG(&g_I2cClient->dev, "%s class_create_file failed!\n", __func__);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
    ...
    /* end modify by shenwenbin for enter meta error 20180716 */
    DBG(&g_I2cClient->dev, "***  int_status = %d ***\n", gpio_get_value(MS_TS_MSG_IC_GPIO_INT));
...
/*** begin: added by liuhe.zhong for getting touchpanel hardware information on 2017.12.13 ***/
	sprintf(tp_info, "MSG22XX:HOLITECH:V%s\n", _gFwVersion);
        DBG(&g_I2cClient->dev, "*** tp_info = %s ***\n", tp_info);
/*** end:   added by liuhe.zhong for getting touchpanel hardware information on 2017.12.13 ***/
/*** begin: added by liuhe.zhong for getting lcm color on 2018.03.08 **/
	if('1' == _gFwVersion[4]) {
		lcm_color = 1;
	}
	else if('2' == _gFwVersion[4]) {
		lcm_color = 2;
        }
        DBG(&g_I2cClient->dev, "*** lcm_color = %d ***\n", lcm_color);
/*** end:   added by liuhe.zhong for getting lcm color on 2018.03.08 **/
#ifdef CONFIG_UPDATE_FIRMWARE_BY_SW_ID
/*** begin: added by liuhe.zhong for auto upgrade when normal boot on 2017.12.19 ***/
