...
s32 Msg28xxSwitchFwMode(u16 *nFMode, u16 *deep_standby)
{
    ...
    if (*deep_standby == 1)
    {
    	DBG(0, "*** Msg28xx MP Test# enter deep standby! ***\n");
    	...
    	{
    		*deep_standby = 0;
    		printk("*** Msg28xx MP Test# enter deep standby FAILED! ***\n");
    		return -1;
    	}
    }
    DBG(0, "nFMode = %x\n", *nFMode);
    ...
    if (*nFMode == MUTUAL_SINE) {
    	...
    	if (abs(freq - ptMutualMpTest->Open_fixed_carrier) >= 2) {
    		printk("Fixed carrier failed, current frequency = %d khz, need fixed frequency = %d khz", freq, ptMutualMpTest->Open_fixed_carrier);
    		return -1;
    	}
    }
