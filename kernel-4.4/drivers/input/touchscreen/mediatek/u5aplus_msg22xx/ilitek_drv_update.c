...
extern IliTekTpInfo_t g_IliTekTpInfo;
extern u8 g_IliTekFwData[ILITEK_MAX_UPDATE_FIRMWARE_BUFFER_SIZE*1024];
extern u8 g_IliTekFwDataBuf[ILITEK_ILI21XX_FIRMWARE_SIZE*1024]; // for update firmware(ILI21xx) from SD card
...
int DrvIliTekGetChecksum(u32 nStartAddr, u32 nEndAddr)
{
	...
	{	
		...
		OutWrite(0x41009, nEndAddr, 2);
		OutWrite(0x41000, 0x3B | (nStartAddr << 8), 4);
		...
		return InWrite(0x41018);
	}
	
	return -2;
}

int DrvIliTekEntryIceMode(void)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
int DrvIliTekIceModeInitial(void)
{
    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);

	if ((g_ChipType == CHIP_TYPE_ILI2120) || (g_ChipType == CHIP_TYPE_ILI2121) || g_ChipType == 0)
	{
		// close watch dog
		if (OutWrite(0x5200C, 0x0000, 2) < 0)
			return -2;
		if (OutWrite(0x52020, 0x01, 1) < 0)
			return -2;
		if (OutWrite(0x52020, 0x00, 1) < 0)
			return -2;
		if (OutWrite(0x42000, 0x0F154900, 4) < 0)
			return -2;
		if (OutWrite(0x42014, 0x02, 1) < 0)
			return -2;
		if (OutWrite(0x42000, 0x00000000, 4) < 0)
			return -2;
		//---------------------------------
...
u16 DrvGetChipType(void)
{
    ...
    if (rc < 0)
    {
        ...
        if (0 == nChipType)
        {
            SLAVE_I2C_ID_DWI2C = (0x82>>1); //0x41
			
            DrvIliTekEntryIceMode();
            DrvIliTekIceModeInitial();
            //get PID (0xXX000000 is ILI212X)
            nTemp = InWrite(0x4009C);
            DBG(&g_I2cClient->dev, "nTemp=0x%x\n", nTemp);
			
            if ((nTemp & 0xFFFFFF00) == 0)
            {
                nChipType = (vfIceRegRead(0xF001) << (8 * 1)) + (vfIceRegRead(0xF000));
                DBG(&g_I2cClient->dev, "nChipType=0x%x\n", nChipType);
                if (0xFFFF == nChipType)
                {
            ...
            if ((nChipType != CHIP_TYPE_ILI2120) && (nChipType != CHIP_TYPE_ILI2121))
            {
                nChipType = 0;
            }
        ...
        if (0 == nChipType) // If can not get chip type successfully, reset SLAVE_I2C_ID_DWI2C to 0x26 as defalut setting.
        {
            SLAVE_I2C_ID_DWI2C = (0x4C>>1); //0x26 
        }
...
void DrvCheckFirmwareUpdateBySwId(void)
{
    ...
    {
#ifdef CONFIG_ENABLE_CHIP_TYPE_ILI21XX
        _DrvIliTekCheckFirmwareUpdateBySwId();
#endif //CONFIG_ENABLE_CHIP_TYPE_ILI21XX
    }
    else
    {
...
static void _DrvIliTekCheckFirmwareUpdateBySwId(void) 
{
    ...
    _gUpdateRetryCount = 1;
    g_IsUpdateFirmware = 0x01; // Set flag to 0x01 for indicating update firmware is processing

    queue_work(_gUpdateFirmwareBySwIdWorkQueue, &_gUpdateFirmwareBySwIdWork);

    // Note : Since DrvTouchDeviceHwReset() and DrvEnableFingerTouchReport() shall be called at _DrvUpdateFirmwareBySwIdDoWork() later, can not call the two functions here. 
}
#endif //CONFIG_ENABLE_CHIP_TYPE_ILI21XX    
...
static void _DrvUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    {
#ifdef CONFIG_ENABLE_CHIP_TYPE_ILI21XX
        nRetVal = DrvIliTekUpdateFirmware(CTPM_FW, true);
#endif //CONFIG_ENABLE_CHIP_TYPE_ILI21XX
    }
    else
    {
...
//                DrvIliTekReadTouchPanelInfo(&g_IliTekTpInfo); // TODO : add for debug

/*
                if (nRetVal == 1)
                {
                    DBG(&g_I2cClient->dev, "%s() : File error\n", __func__);
                }
*/
                if (nRetVal == 2)
                {
                    DBG(&g_I2cClient->dev, "%s() : I2C transfer error\n", __func__);
                }
                else if (nRetVal == 3)
                {
                    DBG(&g_I2cClient->dev, "%s() : Upgrade FW failed\n", __func__);
                }
                else if (nRetVal == 4)
                {
                    DBG(&g_I2cClient->dev, "%s() : No need to upgrade FW\n", __func__);
                }
/*
                else if (nRetVal == 5)
                {
                    DBG(&g_I2cClient->dev, "%s() : System is suspend\n", __func__);
                }        
*/
            }            
        }
...
static u32 HexToDec(char *pHex, s32 nLength)
{
...
s32 DrvIliTekConvertFirmwareData(u8 *pBuf, u32 nSize) // for ILI21xx
{
    u32 i = 0, j = 0, k = 0;
    u32 nApStartAddr = 0xFFFF, nDfStartAddr = 0xFFFF, nStartAddr = 0xFFFF, nExAddr = 0;
    u32 nApEndAddr = 0x0, nDfEndAddr = 0x0, nEndAddr = 0x0;
    u32 nApChecksum = 0x0, nDfChecksum = 0x0, nChecksum = 0x0, nLength = 0, nAddr = 0, nType = 0;

    DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);

    _gnIliTekApStartAddr = 0; // Reset _gnIliTekApStartAddr and _gnIliTekApEndAddr and _gnIliTekApChecksum to 0 
    _gnIliTekApEndAddr = 0;
    _gnIliTekApChecksum = 0;

    if (0 == nSize)
    {
        DBG(&g_I2cClient->dev, "nSize is 0. Can not convert firmware data.\n");
        return -1;        
    }
        
    memset(g_IliTekFwData, 0xFF, ILITEK_MAX_UPDATE_FIRMWARE_BUFFER_SIZE*1024);
...
//        DBG(&g_I2cClient->dev, "nLength = %d(0x%02X), nAddr = %d(0x%04X), nType = %d\n", nLength, nLength, nAddr, nAddr, nType);
...
//                    DBG(&g_I2cClient->dev, "nAddr = 0x%04X, nApChecksum = 0x%06X, Data = 0x%02X\n", nAddr + (j - 8)/2, nApChecksum, HexToDec(&pBuf[i + 1 + j], 2));
                }
                else
                {
                    nDfChecksum = nDfChecksum + HexToDec(&pBuf[i + 1 + j], 2);
//                    DBG(&g_I2cClient->dev, "nAddr = 0x%04X, nDfChecksum = 0x%06X, Data = 0x%02X\n", nAddr + (j - 8)/2, nDfChecksum, HexToDec(&pBuf[i + 1 + j], 2));
                }		
            }
        }
        ...
        if (nType == 0x00)
        {
            ...
            {
                DBG(&g_I2cClient->dev, "Invalid hex format!\n");
            ...
            {
                g_IliTekFwData[nAddr + k] = HexToDec(&pBuf[i + 9 + j], 2);
            }
        }
    ...
    _gnIliTekApStartAddr = nApStartAddr; // Assign _gnIliTekApStartAddr and _gnIliTekApEndAddr and _gnIliTekApChecksum here for upgrade firmware by MTPTool APK later 
    _gnIliTekApEndAddr = nApEndAddr;
    _gnIliTekApChecksum = nApChecksum;

    DBG(&g_I2cClient->dev, "nStartAddr = 0x%06X, nEndAddr = 0x%06X, nChecksum = 0x%06X\n", nStartAddr, nEndAddr, nChecksum);
    DBG(&g_I2cClient->dev, "nApStartAddr = 0x%06X, nApEndAddr = 0x%06X, nApChecksum = 0x%06X\n", nApStartAddr, nApEndAddr, nApChecksum);
    DBG(&g_I2cClient->dev, "nDfStartAddr = 0x%06X, nDfEndAddr = 0x%06X, nDfChecksum = 0x%06X\n", nDfStartAddr, nDfEndAddr, nDfChecksum);
    
    return 0;
}

s32 DrvIliTekUpdateFirmware(u8 *pszFwData, bool bIsUpdateFirmwareBySwId) // for ILI21xx
{
    s32 rc = 0, nUpdateRetryCount = 0, nUpgradeStatus = 0, nUpdateLength = 0, nCheckFwFlag = 0, nChecksum = 0, i = 0, j = 0, k = 0;
    u8 szFwVersion[4] = {0};
    u8 szBuf[512] = {0};
	u8 szCmd[2] = {0};
    u32 nApStartAddr = 0, nDfStartAddr = 0, nApEndAddr = 0, nDfEndAddr = 0, nApChecksum = 0, nDfChecksum = 0, nTemp = 0, nIcChecksum = 0;
    ...
    {    
        nApStartAddr = (pszFwData[0] << 16) + (pszFwData[1] << 8) + pszFwData[2];
        nApEndAddr = (pszFwData[3] << 16) + (pszFwData[4] << 8) + pszFwData[5];
        nApChecksum = (pszFwData[6] << 16) + (pszFwData[7] << 8) + pszFwData[8];
        nDfStartAddr = (pszFwData[9] << 16) + (pszFwData[10] << 8) + pszFwData[11];
        nDfEndAddr = (pszFwData[12] << 16) + (pszFwData[13] << 8) + pszFwData[14];
        nDfChecksum = (pszFwData[15] << 16) + (pszFwData[16] << 8) + pszFwData[17];
        szFwVersion[0] = pszFwData[18];
        szFwVersion[1] = pszFwData[19];
        szFwVersion[2] = pszFwData[20];
        szFwVersion[3] = pszFwData[21];
        nUpdateLength = ((pszFwData[26] << 16) + (pszFwData[27] << 8) + pszFwData[28]) + ((pszFwData[29] << 16) + (pszFwData[30] << 8) + pszFwData[31]);
	
        DBG(&g_I2cClient->dev, "nApStartAddr=0x%06X, nApEndAddr=0x%06X, nApChecksum=0x%06X\n", nApStartAddr, nApEndAddr, nApChecksum);
        DBG(&g_I2cClient->dev, "nDfStartAddr=0x%06X, nDfEndAddr=0x%06X, nDfChecksum=0x%06X\n", nDfStartAddr, nDfEndAddr, nDfChecksum);    
        DBG(&g_I2cClient->dev, "nUpdateLength=%d\n", nUpdateLength);    

        // Check if FW is ready
        szCmd[0] = ILITEK_TP_CMD_READ_DATA;
        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szCmd[0], 1);
        if (rc < 0)
        {
        ...
        rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szBuf[0], 3);
        if (rc < 0)
        {
            DBG(&g_I2cClient->dev, "ILITEK_TP_CMD_READ_DATA : IicReadData() failed, rc = %d\n", rc);
        }

        DBG(&g_I2cClient->dev, "szBuf[0][1][2] = 0x%X, 0x%X, 0x%X\n", szBuf[0], szBuf[1], szBuf[2]);
        if (szBuf[1] >= 0x80)
        {
            DBG(&g_I2cClient->dev, "FW is ready. Continue to check FW version.\n");

            for (i = 0; i < 4; i ++)
            {
                DBG(&g_I2cClient->dev, "g_IliTekTpInfo.szFwVersion[%d] = %d, pszFwData[%d] = %d\n", i, g_IliTekTpInfo.szFwVersion[i], i, pszFwData[i + 18]);
	
                if (g_IliTekTpInfo.szFwVersion[i] == 0xFF)
                {
                    DBG(&g_I2cClient->dev, "FW version is 0xFF\n");
                    break;
                }
                else if (g_IliTekTpInfo.szFwVersion[i] != pszFwData[i + 18])
                {
                    break;
                }
                else if ((i == 3) && (g_IliTekTpInfo.szFwVersion[3] == pszFwData[3 + 18]))
                {
                    nCheckFwFlag = 1;
                }
            }
        ...
        {
            DBG(&g_I2cClient->dev, "FW is not ready. Start to upgrade FW directly.\n");
        }
    }
    else
    {
        nApStartAddr = _gnIliTekApStartAddr;
        nApEndAddr = _gnIliTekApEndAddr;
        nUpdateLength = _gnIliTekApEndAddr;
		nApChecksum = _gnIliTekApChecksum;

        DBG(&g_I2cClient->dev, "_gnIliTekApStartAddr=0x%06X, _gnIliTekApEndAddr=0x%06X, _gnIliTekApChecksum = 0x%06X\n", _gnIliTekApStartAddr, _gnIliTekApEndAddr, _gnIliTekApChecksum);
    }
...
UPDATE_RETRY:
    ...
    if (true == bIsUpdateFirmwareBySwId)
    {    
		nIcChecksum = DrvIliTekGetChecksum(nApStartAddr, nApEndAddr);
        ...
        {
            DBG(&g_I2cClient->dev, "DrvIliTekUpdateFirmware() do not need update\n"); 
            if (ExitIceMode() < 0)
            {
                return 2;
            }
...
//        DBG(&g_I2cClient->dev, "%s, i = %X\n", __func__, i);
        ...
        nTemp = (i << 8) + 0x20;
        if (OutWrite(0x041000, nTemp, 4) < 0)
            return 2;
        mdelay(3);
        ...
        for (j = 0; j < 50; j ++)
        {
            ...
            szBuf[0] = InWrite(0x041013);
//            DBG(&g_I2cClient->dev, "%s, szBuf[0] = %X\n", __func__, szBuf[0]);
            if (szBuf[0] == 0)
            {
                break;
            }
	...
	DBG(&g_I2cClient->dev,"ILITEK_UPDATE_FIRMWARE_PAGE_LENGTH:%d\n", ILITEK_UPDATE_FIRMWARE_PAGE_LENGTH);
...
//        DBG(&g_I2cClient->dev, "%s, i = %X\n", __func__, i);
        ...
        if (OutWrite(0x041000, nTemp, 4) < 0)
            return 2;
        if (OutWrite(0x041004, 0x66aa5500 + ILITEK_UPDATE_FIRMWARE_PAGE_LENGTH - 1, 4) < 0)
            return 2;
        ...
        if (IicWriteData(SLAVE_I2C_ID_DWI2C, szBuf, ILITEK_UPDATE_FIRMWARE_PAGE_LENGTH + 4) < 0) {
            DBG(&g_I2cClient->dev, "IicWriteData() error, address = 0x%X, start_addr = 0x%X, end_addr = 0x%X\n", (int)i, (int)nApStartAddr, (int)nApEndAddr);
            return 2;
        }

        nUpgradeStatus = (i * 100) / nUpdateLength;
        DBG(&g_I2cClient->dev, "%cupgrade firmware(ap code), %02d%c\n", 0x0D, nUpgradeStatus, '%');
        mdelay(3);
    }

	nIcChecksum = 0;
    nIcChecksum = DrvIliTekGetChecksum(nApStartAddr, nApEndAddr);
    DBG(&g_I2cClient->dev, "nIcChecksum = 0x%X, nApChecksum = 0x%X\n", nIcChecksum, nApChecksum);
	...
	if (nIcChecksum != nApChecksum)
	{
		...
		{
			ExitIceMode();
			DBG(&g_I2cClient->dev, "Upgrade FW Failed\n");
			return 3;
		}
	}
    ...
    DBG(&g_I2cClient->dev, "Upgrade ILITEK_IOCTL_I2C_RESET end\n");
    ...
    if (szBuf[1] >= 0x80)
    {
        DBG(&g_I2cClient->dev, "Upgrade FW Success\n");
    }
    else
    {
        if (nUpdateRetryCount < 2)
...
s32 DrvIliTekUpdateFirmwareBySdCard(const char *pFilePath) // for ILI21xx
{
    s32 nRetVal = 0;
    struct file *pfile = NULL;
    struct inode *inode;
    ...
    if ((g_ChipType == CHIP_TYPE_ILI2120 || g_ChipType == CHIP_TYPE_ILI2121) && (fsize < (ILITEK_ILI21XX_FIRMWARE_SIZE * 1024)))    
    {
        nRetVal = DrvIliTekConvertFirmwareData(g_IliTekFwDataBuf, fsize); // convert _gIliTekFwDataBuf to g_IliTekFwData
        ...
        {
            DBG(&g_I2cClient->dev, "Invalid hex file for upgrade firmware!\n");
        }
    }
    else
    {
