...
#if 0
		pr_alert("cpuxgpt_regs=0x%x, irq0=%d, irq1=%d, irq2=%d, irq3=%d\n"
			cpuxgpt_regs, cpuxgpt_irq[0], cpuxgpt_irq[1],
			cpuxgpt_irq[2], cpuxgpt_irq[3]);

		pr_alert("irq4=%d, irq5=%d, irq6=%d, irq7=%d\n",
			cpuxgpt_irq[4], cpuxgpt_irq[5], cpuxgpt_irq[6], cpuxgpt_irq[7]);
#endif
...
int cpu_xgpt_register_timer(unsigned int id, irqreturn_t (*func)(int irq, void *dev_id))
{
	...
	if (!cpu_present(id)) {
		pr_err("%s: err idnumber id=%d not present\n", __func__, id);
		return -1;
	}
