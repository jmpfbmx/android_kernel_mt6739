...
static int mtktspa_get_hw_temp(void)
{
	...
	for (i = 0; i < size; i++) {
		mtktspa_dprintk("PA temperature: name:%s, value:%d, invalid_value=%d\n",
...
static int tspa_sysrst_get_max_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
	return 0;
}

static int tspa_sysrst_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
...
static int __init mtktspa_init(void)
{
	...
	mtkTTimer_register("mtktspa", mtkts_pa_start_thermal_timer, mtkts_pa_cancel_thermal_timer);
