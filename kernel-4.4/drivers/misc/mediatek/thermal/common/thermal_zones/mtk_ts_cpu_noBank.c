...
static int tscpu_thermal_probe(struct platform_device *dev)
{
...
#ifdef CONFIG_OF
	err =
	    request_irq(thermal_irq_number, tscpu_thermal_all_tc_interrupt_handler,
			IRQF_TRIGGER_LOW, THERMAL_NAME, NULL);
	if (err)
		tscpu_warn("tscpu_init IRQ register fail\n");
#else
	err =
	    request_irq(THERM_CTRL_IRQ_BIT_ID, tscpu_thermal_all_tc_interrupt_handler,
			IRQF_TRIGGER_LOW, THERMAL_NAME, NULL);
	if (err)
		tscpu_warn("tscpu_init IRQ register fail\n");
#endif
	...
	mtkTTimer_register("mtktscpu", tscpu_start_thermal_timer, tscpu_cancel_thermal_timer);
