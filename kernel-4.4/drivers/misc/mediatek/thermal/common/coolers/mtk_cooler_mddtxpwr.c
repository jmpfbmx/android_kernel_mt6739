...
static void mtk_cl_mddtxpwr_set_mddtxpwr_limit(void)
{
	...
	if (min_limit != cl_mddtxpwr_cur_limit) {
		...
		if (cl_mddtxpwr_cur_limit >= 65535) {
			int ret = eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_DTX_REQ, 8);

			mtk_cooler_mddtxpwr_dprintk_always
			    ("mtk_cl_mddtxpwr_set_mddtxpwr_limit() ret %d limit=30\n", ret);
		} else {
			int ret =
			    eemcs_notify_md_by_sys_msg(MD_SYS5, EXT_MD_DTX_REQ,
						       cl_mddtxpwr_cur_limit);
			mtk_cooler_mddtxpwr_dprintk_always
			    ("mtk_cl_mddtxpwr_set_mddtxpwr_limit() ret %d limit=%d\n",
			     cl_mddtxpwr_cur_limit);
		}
#endif
	}
...
static int mtk_cl_mddtxpwr_get_max_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
...
static int mtk_cl_mddtxpwr_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
...
static int mtk_cl_mddtxpwr_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
...
static int mtk_cooler_mddtxpwr_register_ltf(void)
{
	...
	for (i = MAX_NUM_INSTANCE_MTK_COOLER_MDDTXPWR; i-- > 0;) {
		...
		cl_mddtxpwr_dev[i] = mtk_thermal_cooling_device_register(temp, (void *)&cl_mddtxpwr_state[i],
									 &mtk_cl_mddtxpwr_ops);
	}
...
static int _mtk_cl_mddtxpwr_proc_read(char *buf, char **start, off_t off, int count, int *eof,
				      void *data)
{
	int len = 0;
	char *p = buf;
...
static ssize_t _mtk_cl_mddtxpwr_proc_write(struct file *file, const char *buffer,
...
static int __init mtk_cooler_mddtxpwr_init(void)
{
	...
	/* create a proc file */
	{
		...
		entry =
		    create_proc_entry("driver/mtk-cl-mddtxpwr", S_IRUGO | S_IWUSR | S_IWGRP, NULL);
