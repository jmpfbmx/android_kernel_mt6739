...
struct thermal_controller tscpu_g_tc[THERMAL_CONTROLLER_NUM] = {
	[0] = {
		.ts = {TS_MCU1, TS_MCU2, TS_MCU7, TS_ABB},
		.ts_number = 4,
		.tc_offset = 0x0,
		.tc_speed = {
			0x0000000C,
	...
	[1] = {
		.ts = {TS_MCU3, TS_MCU4, TS_MCU5, TS_MCU6},
		.ts_number = 4,
		.tc_offset = 0x100,
		.tc_speed = {
			0x0000000C,
...
void get_thermal_slope_intercept(struct TS_PTPOD *ts_info, enum thermal_bank_name ts_bank)
{
	...
	/*
	* Bank0: MCU_mp0(L)	(TS_MCU2)
	* Bank1: MCU_mp2(B)	(TS_MCU1 + TS_MCU7)
	* Bank2: MFG(GPU)	(TS_MCU3)
	* Bank3: MCU_mp(CCI)	(TS_MCU1 + TS_MCU2 + TS_MCU7)
	* Bank4: VPU(IPU)		(TS_MCU4 + TS_MCU5)
	* Bank5: TOP		(TS_MCU1 + TS_MCU2 + TS_MCU4 + TS_MCU5)
	*/
	/*
	* TC0:(TS_MCU1, TS_MCU2, TS_MCU7, TS_ABB)
	* TC1:(TS_MCU3, TS_MCU4, TS_MCU5, TS_MCU6)
	*/
...
void tscpu_thermal_cal_prepare(void)
{
	...
	temp3 = get_devinfo_with_index(ADDRESS_INDEX_3);
...
static __s32 temperature_to_raw_room(__u32 ret, enum thermal_sensor ts_name)
{
	/* Ycurr = [(Tcurr - DEGC_cali/2)*(1534+O_slope*10)/10*(18/15)*(1/10000)+X_roomtabb]*Gain*4096 + OE */
...
static __s32 raw_to_temperature_roomt(__u32 ret, enum thermal_sensor ts_name)
{
	...
	if (g_o_slope_sign == 0)
		format_4 = ((format_3 * 1000) / (1534 + g_o_slope * 10));	/* uint = 0.1 deg */
	else
		format_4 = ((format_3 * 1000) / (1534 - g_o_slope * 10));	/* uint = 0.1 deg */
...
int get_immediate_cpuB_wrap(void)
{
	...
	curr_temp = MAX(tscpu_ts_temp[TS_MCU1], tscpu_ts_temp[TS_MCU7]);
...
int get_immediate_mcucci_wrap(void)
{
	...
	curr_temp = MAX(curr_temp, tscpu_ts_temp[TS_MCU7]);
...
int get_immediate_top_wrap(void)
{
	...
	curr_temp = MAX(tscpu_ts_temp[TS_MCU1], tscpu_ts_temp[TS_MCU2]);
	curr_temp = MAX(curr_temp, tscpu_ts_temp[TS_MCU4]);
	curr_temp = MAX(curr_temp, tscpu_ts_temp[TS_MCU5]);
...
int tscpu_get_curr_temp(void)
{
	...
	*/
	...
	tscpu_curr_cpu_temp = MAX(tscpu_curr_cpu_temp, tscpu_ts_temp[TS_MCU7]);
...
int get_io_reg_base(void)
{
	...
	if (!node) {
		tscpu_printk("[CLK_INFRACFG_AO] find node failed\n");
		return 0;
	}

	therm_clk_infracfg_ao_base = of_iomap(node, 0);
	if (!therm_clk_infracfg_ao_base) {
		tscpu_printk("[CLK_INFRACFG_AO] base failed\n");
		return 0;
	}
#endif
	return 1;
