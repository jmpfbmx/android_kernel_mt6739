...
static void mtktspmic_read_efuse(void)
{
	...
	mtktspmic_info("[pmic_debug] 6356_efuse: g_o_vts        = %d\n", g_o_vts);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_o_vts_2      = %d\n", g_o_vts_2);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_o_vts_3      = %d\n", g_o_vts_3);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_degc_cali    = %d\n", g_degc_cali);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_adc_cali_en  = %d\n", g_adc_cali_en);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_o_slope      = %d\n", g_o_slope);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_o_slope_sign = %d\n", g_o_slope_sign);
	mtktspmic_info("[pmic_debug] 6356_efuse: g_id		   = %d\n", g_id);
...
int mtktspmic_get_hw_temp(void)
{
	...
	temp = pmic_get_auxadc_value(AUXADC_LIST_MT6356_CHIP_TEMP);
...
int mt6356tsbuck1_get_hw_temp(void)
{
	int temp = 0, temp1 = 0;

	mutex_lock(&TSPMIC_lock);

	temp = pmic_get_auxadc_value(AUXADC_LIST_MT6356_BUCK1_TEMP);
...
int mt6356tsbuck2_get_hw_temp(void)
{
	int temp = 0, temp1 = 0;

	mutex_lock(&TSPMIC_lock);

	temp = pmic_get_auxadc_value(AUXADC_LIST_MT6356_BUCK2_TEMP);
