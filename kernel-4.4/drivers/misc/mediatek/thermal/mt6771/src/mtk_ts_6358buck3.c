...
static long int mt6358tsbuck3_cur_temp;
/*=============================================================*/
...
static int mt6358tsbuck3_bind(struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	int table_val = 0;

	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
		mtktspmic_dprintk("[mt6358tsbuck3_bind] %s\n", cdev->type);
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		mtktspmic_info("[mt6358tsbuck3_bind] error binding cooling dev\n");
		return -EINVAL;
	}

	mtktspmic_dprintk("[mt6358tsbuck3_bind] binding OK, %d\n", table_val);
	return 0;
}
			    ...
			    struct thermal_cooling_device *cdev)
{
	...
	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
		mtktspmic_dprintk("[mt6358tsbuck3_unbind] %s\n", cdev->type);
	} else if (!strcmp(cdev->type, g_bind1)) {
		table_val = 1;
		mtktspmic_dprintk("[mt6358tsbuck3_unbind] %s\n", cdev->type);
	} else if (!strcmp(cdev->type, g_bind2)) {
	...
	if (thermal_zone_unbind_cooling_device(thermal, table_val, cdev)) {
		mtktspmic_info("[mt6358tsbuck3_unbind] error unbinding cooling dev\n");
		return -EINVAL;
	}

	mtktspmic_dprintk("[mt6358tsbuck3_unbind] unbinding OK\n");
	return 0;
}

static int mt6358tsbuck3_get_mode(struct thermal_zone_device *thermal, enum thermal_device_mode *mode)
{
	*mode = (kernelmode) ? THERMAL_DEVICE_ENABLED : THERMAL_DEVICE_DISABLED;
	return 0;
}

static int mt6358tsbuck3_set_mode(struct thermal_zone_device *thermal, enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
}

static int mt6358tsbuck3_get_trip_type(struct thermal_zone_device *thermal, int trip,
...
static int mt6358tsbuck3_get_trip_temp(struct thermal_zone_device *thermal, int trip,
				   int *temp)
{
	*temp = trip_temp[trip];
	return 0;
...
static int mt6358tsbuck3_get_crit_temp(struct thermal_zone_device *thermal, int *temperature)
{
...
static struct thermal_zone_device_ops mt6358tsbuck3_dev_ops = {
	...
	.get_mode = mt6358tsbuck3_get_mode,
	.set_mode = mt6358tsbuck3_set_mode,
...
static int mt6358tsbuck3_sysrst_get_max_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = 1;
	return 0;
}

static int mt6358tsbuck3_sysrst_get_cur_state(struct thermal_cooling_device *cdev, unsigned long *state)
{
	*state = cl_dev_sysrst_state;
	return 0;
}

static int mt6358tsbuck3_sysrst_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
...
{
	seq_printf(m, "[mt6358tsbuck3_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,\n",
...
static ssize_t mt6358tsbuck3_write(struct file *file, const char __user *buffer, size_t count,
			       loff_t *data)
{
	int len = 0;
	int i;
	...
	ptr_mt6358tsbuck3_data = kmalloc(sizeof(*ptr_mt6358tsbuck3_data), GFP_KERNEL);

	if (ptr_mt6358tsbuck3_data == NULL)
		return -ENOMEM;

	len = (count < (sizeof(ptr_mt6358tsbuck3_data->desc) - 1)) ? count : (sizeof(ptr_mt6358tsbuck3_data->desc) - 1);
	...
	if (sscanf
		...
		&num_trip,
		&ptr_mt6358tsbuck3_data->trip[0], &ptr_mt6358tsbuck3_data->t_type[0], ptr_mt6358tsbuck3_data->bind0,
		&ptr_mt6358tsbuck3_data->trip[1], &ptr_mt6358tsbuck3_data->t_type[1], ptr_mt6358tsbuck3_data->bind1,
		&ptr_mt6358tsbuck3_data->trip[2], &ptr_mt6358tsbuck3_data->t_type[2], ptr_mt6358tsbuck3_data->bind2,
		&ptr_mt6358tsbuck3_data->trip[3], &ptr_mt6358tsbuck3_data->t_type[3], ptr_mt6358tsbuck3_data->bind3,
		&ptr_mt6358tsbuck3_data->trip[4], &ptr_mt6358tsbuck3_data->t_type[4], ptr_mt6358tsbuck3_data->bind4,
		&ptr_mt6358tsbuck3_data->trip[5], &ptr_mt6358tsbuck3_data->t_type[5], ptr_mt6358tsbuck3_data->bind5,
		&ptr_mt6358tsbuck3_data->trip[6], &ptr_mt6358tsbuck3_data->t_type[6], ptr_mt6358tsbuck3_data->bind6,
		&ptr_mt6358tsbuck3_data->trip[7], &ptr_mt6358tsbuck3_data->t_type[7], ptr_mt6358tsbuck3_data->bind7,
		&ptr_mt6358tsbuck3_data->trip[8], &ptr_mt6358tsbuck3_data->t_type[8], ptr_mt6358tsbuck3_data->bind8,
		&ptr_mt6358tsbuck3_data->trip[9], &ptr_mt6358tsbuck3_data->t_type[9], ptr_mt6358tsbuck3_data->bind9,
		&ptr_mt6358tsbuck3_data->time_msec) == 32) {

		down(&sem_mutex);
		mtktspmic_dprintk("[mt6358tsbuck3_write] mt6358tsbuck3_unregister_thermal\n");
		mt6358tsbuck3_unregister_thermal();

		if (num_trip < 0 || num_trip > 10) {
			#ifdef CONFIG_MTK_AEE_FEATURE
			aee_kernel_warning_api(__FILE__, __LINE__, DB_OPT_DEFAULT, "mt6358tsbuck3_write",
					"Bad argument");
			#endif
			mtktspmic_dprintk("[mt6358tsbuck3_write] bad argument\n");
			kfree(ptr_mt6358tsbuck3_data);
			up(&sem_mutex);
			return -EINVAL;
		}
		...
		mtktspmic_dprintk("[mt6358tsbuck3_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,",
		...
		mtktspmic_dprintk("[mt6358tsbuck3_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,",
			g_bind0, g_bind1, g_bind2, g_bind3, g_bind4);
		...
		mtktspmic_dprintk("[mt6358tsbuck3_write] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,",
			trip_temp[0], trip_temp[1], trip_temp[2], trip_temp[3]);
		...
		mtktspmic_dprintk("[mt6358tsbuck3_write] mt6358tsbuck3_register_thermal\n");
...
{
	cl_dev_sysrst = mtk_thermal_cooling_device_register("sysrst.6358buck3", NULL,
...
static int mt6358tsbuck3_register_thermal(void)
{
	mtktspmic_dprintk("[mt6358tsbuck3_register_thermal]\n");

	/* trips : trip 0~2 */
	thz_dev = mtk_thermal_zone_device_register("mt6358tsbuck3", num_trip, NULL,
...
static void mt6358tsbuck3_unregister_thermal(void)
{
	mtktspmic_dprintk("[mt6358tsbuck3_unregister_thermal]\n");
...
static const struct file_operations mt6358tsbuck3_fops = {
	.owner = THIS_MODULE,
	.open = mt6358tsbuck3_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.write = mt6358tsbuck3_write,
	.release = single_release,
};

static int __init mt6358tsbuck3_init(void)
{
	int err = 0;
	...
	mtktspmic_info("[mt6358tsbuck3_init]\n");
	...
	if (!mt6358tsbuck3_dir) {
		...
		entry =
		    proc_create("tz6358buck3", S_IRUGO | S_IWUSR | S_IWGRP, mt6358tsbuck3_dir,
				&mt6358tsbuck3_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
	}

	mtkTTimer_register("mt6358tsbuck3", mt6358tsbuck3_start_thermal_timer, mt6358tsbuck3_cancel_thermal_timer);
...
static void __exit mt6358tsbuck3_exit(void)
{
	mtktspmic_info("[mt6358tsbuck3_exit]\n");
	mt6358tsbuck3_unregister_thermal();
	mt6358tsbuck3_unregister_cooler();
