...
static __s32 tsbuck3_raw_to_temp(__u32 ret)
{
	...
	t_current = g_tsbuck3_intercept + ((g_tsbuck3_slope1 * y_curr) / (g_tsbuck3_slope2));
...
void mtktspmic_cali_prepare2(void)
{
	...
	g_tsbuck3_slope1 = (100 * 1000 * 10);	/* 1000 is for 0.001 degree */
	...
	if (g_o_slope_sign == 0)
		g_tsbuck3_intercept = (vbe_t * 1000) / (-(factor + g_o_slope * 10));	/*0.001 degree */
	else
		g_tsbuck3_intercept = (vbe_t * 1000) / (-(factor - g_o_slope * 10));	/*0.001 degree */
