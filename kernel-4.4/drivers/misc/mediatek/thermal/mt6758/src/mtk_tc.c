...
void get_thermal_slope_intercept(struct TS_PTPOD *ts_info, enum thermal_bank_name ts_bank)
{
	...
	/*
	*TC0:(TS_MCU5, TS_MCU4)
	*TC1:(TS_MCU1, TS_MCU2, TS_MCU3, TS_ABB)
	*/
	/* chip dependent */
	...
	*/
	...
	case THERMAL_BANK3:
		...
		break;
	case THERMAL_BANK4:
		x_roomt = g_x_roomt[4];/*TSMCU4 almost same as TSMCU5*/
		break;
...
void tscpu_thermal_cal_prepare(void)
{
	...
	pr_err("[calibration] temp0=0x%x, temp1=0x%x, temp2=0x%x\n", temp0, temp1, temp2);
