...
void m4u_get_main_tlb(int m4u_id, int m4u_slave_id, int idx, struct mmu_tlb_t *pTlb)
{
	pTlb->tag = m4u_get_main_tag(m4u_id, m4u_slave_id, idx);
	pTlb->desc = m4u_get_main_descriptor(m4u_id, m4u_slave_id, idx);
}

unsigned int m4u_get_pfh_tlb(int m4u_id, int set, int page, int way, struct mmu_tlb_t *pTlb)
{
	unsigned int regValue = 0;
	unsigned long m4u_base = gM4UBaseAddr[m4u_id];
...
char *smi_clk_name[] = {
	"m4u_smi_common", "m4u_smi_larb0", "m4u_smi_larb1", "m4u_smi_larb4", "m4u_smi_larb4_2",
	"m4u_smi_larb5", "m4u_smi_larb6", "m4u_smi_larb7"
};
...
{
	int m4u_id = m4u_port_2_m4u_id(port);
	unsigned long m4u_base;

	if (m4u_id == -1) {
		M4UMSG("%s: invalid port(%d)\n", __func__, port);
		return -1;
	}
	m4u_base = gM4UBaseAddr[m4u_id];
	spin_lock(&gM4u_reg_lock);
...
int m4u_config_prog_dist(M4U_PORT_ID port, int dir, int dist, int en, int mm_id, int sel)
{
	...
	if (m4u_index == -1 || larb == -1 || larb == 8 || larb_port == -1) {
		M4UMSG("%s: invalid port(%d)\n", __func__, port);
		return -1;
	}
...
int m4u_insert_seq_range(M4U_PORT_ID port, unsigned int MVAStart, unsigned int MVAEnd)
{
	...
	if (m4u_index == -1 || m4u_slave_id == -1 || larb_id == -1) {
		M4UMSG("%s: invalid port(%d)\n", __func__, port);
		return -1;
	}
		...
		if (pSeq[i].Enabled == 1) {
			if (MVAEnd < pSeq[i].MVAStart || MVAStart > pSeq[i].MVAEnd)     /* no overlap */
				...
				M4ULOG_HIGH("insert range overlap!: larb=%d,module=%s\n",
					    larb_id, m4u_get_port_name(port));
				M4ULOG_HIGH
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
	...
	if (m4u_index == 0) {
		...
		/* debug use */
		mmu_en = m4uHw_get_field_by_mask(larb_base, SMI_LARB_NON_SEC_CONx(larb_port), F_SMI_MMU_EN);
		if (!!(mmu_en) != virt) {
			M4ULOG_HIGH("%s error: larb_base=%lu port=%s, Virtuality=%d, mmu_en=%x (%x, %x)\n",
			__func__, larb_base,
			m4u_get_port_name(port), virt, mmu_en,
			M4U_ReadReg32(larb_base, SMI_LARB_NON_SEC_CONx(larb_port)), F_SMI_MMU_EN);
		}
	} else {
		larb_port = m4u_port_2_larb_port(port);
		if (larb_port == -1) {
...
int m4u_config_port_array(struct m4u_port_array *port_array)
{
	...
	for (port = 0; port < M4U_PORT_NR; port++) {
		if (port_array->ports[port] && M4U_PORT_ATTR_EN != 0) {
			...
			larb = m4u_port_2_larb_id(port);
			larb_port = m4u_port_2_larb_port(port);
			if (larb == -1 || larb == 8 || larb_port == -1 || larb >= SMI_LARB_NR || larb_port >= 32)
				return -1;
		...
		if (m4u_index == 0) {
			...
			orig_value =
				...
				if (orig_value != regNew[larb][larb_port]) {
					spin_lock(&gM4u_reg_lock);
					m4uHw_set_field_by_mask(gLarbBaseAddr[larb], SMI_LARB_NON_SEC_CONx(larb_port),
								F_SMI_MMU_EN,
...
irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
...

	{
		...
		if (IntrSrc & F_INT_TRANSLATION_FAULT(m4u_slave_id)) {
			...
			if (m4u_port != -1 && gM4uPort[m4u_port].enable_tf == 1 && bypass_DISP_TF == 0) {
...
int m4u_reg_init(struct m4u_domain *m4u_domain, unsigned long ProtectPA, int m4u_id)
{
...

	{
		...
		/*DE's request*/
		M4U_WriteReg32(gM4UBaseAddr[m4u_id], REG_MMU_MMU_COHERENCE_EN, 0x0);
