...
static int m4u_debug_set(void *data, u64 val)
{
	...
	{                   /* map4kpageonly */
		...
		m4u_map_sgtable(domain,
		M4U_TEST_DOMAIN, mva, sg_table, page_num * PAGE_SIZE, M4U_PROT_WRITE | M4U_PROT_READ);
		m4u_dump_pgtable(domain, NULL);
		m4u_unmap(domain, M4U_TEST_DOMAIN, mva, page_num * PAGE_SIZE);
		m4u_dump_pgtable(domain, NULL);
	...
	{                   /* mapmiscpages */
		...
		m4u_map_sgtable(domain, M4U_TEST_DOMAIN, mva, sg_table, size, M4U_PROT_WRITE | M4U_PROT_READ);
		m4u_dump_pgtable(domain, NULL);
		m4u_unmap(domain, M4U_TEST_DOMAIN, mva, size);
		m4u_dump_pgtable(domain, NULL);
		sg_free_table(sg_table);
	}
	break;
	...
	case 10:
	{
		...
		mva = m4u_do_mva_alloc_fix(M4U_TEST_DOMAIN, 0, 0x90000000, 0x10000000, NULL);
		M4UINFO("mva alloc fix done:mva=0x%x\n", mva);
		mva = m4u_do_mva_alloc_fix(M4U_TEST_DOMAIN, 0, 0xb0000000, 0x10000000, NULL);
		M4UINFO("mva alloc fix done:mva=0x%x\n", mva);
		mva = m4u_do_mva_alloc_fix(M4U_TEST_DOMAIN, 0, 0xa0000000, 0x10000000, NULL);
		M4UINFO("mva alloc fix done:mva=0x%x\n", mva);
		mva = m4u_do_mva_alloc_fix(M4U_TEST_DOMAIN, 0, 0xa4000000, 0x10000000, NULL);
		M4UINFO("mva alloc fix done:mva=0x%x\n", mva);
		m4u_mvaGraph_dump(M4U_TEST_DOMAIN);
	...
	case 24:
	{
		...
		new_mva = 0x7000000;
		M4UMSG("(0) mva_start from:0x%x\n", new_mva);
		m4u_alloc_mva(client, M4U_PORT_DISP_OVL0,
			(unsigned long)pSrc, NULL, 128, 0, M4U_FLAGS_START_FROM, &new_mva);
		M4UMSG("(0) mva:0x%x\n", new_mva);
	...
	case 30:
	{
		...
		unsigned int mva_rd;
		unsigned int mva_wr;
		unsigned int allocated_size = 4096;
		unsigned int i;
		struct m4u_port_array port_array;

		m4u_client_t *client = m4u_create_client();

		mau_start_monitor(0, 0, 0, 1, 1, 0, 0, 0x0, 0xffffffff, 0xffffffff, 0xffffffff);
		mau_start_monitor(0, 0, 1, 0, 1, 0, 0, 0x0, 0xffffffff, 0xffffffff, 0xffffffff);
		mau_start_monitor(0, 0, 2, 1, 0, 1, 0, 0x0, 0xffffffff, 0xffffffff, 0xffffffff);
		mau_start_monitor(0, 0, 3, 0, 0, 1, 0, 0x0, 0xffffffff, 0xffffffff, 0xffffffff);

		pSrc = kmalloc(allocated_size, GFP_KERNEL | __GFP_ZERO);
		memset(pSrc, 0xFF, allocated_size);
		M4UMSG("(0) vmalloc pSrc:0x%p\n", pSrc);
		M4UMSG("(0) pSrc check 0x%x 0x%x 0x%x 0x%x  0x%x\n",
			*pSrc, *(pSrc+1), *(pSrc+126), *(pSrc+127), *(pSrc+128));
		pDst =  kmalloc(allocated_size, GFP_KERNEL | __GFP_ZERO);
		memset(pDst, 0x0F, allocated_size);
		M4UMSG("(0) vmalloc pDst:0x%p\n", pDst);
		M4UMSG("(1) pDst check 0x%x 0x%x 0x%x 0x%x  0x%x\n",
			*pDst, *(pDst+1), *(pDst+126), *(pDst+127), *(pDst+128));
		...
		m4u_alloc_mva(client, M4U_PORT_DISP_FAKE0,
			(unsigned long)pSrc, NULL, allocated_size, 0, 0, &mva_rd);

		mva_wr = 0x50000000;

		m4u_alloc_mva(client, M4U_PORT_DISP_FAKE0,
			(unsigned long)pDst, NULL, allocated_size, 0, M4U_FLAGS_FIX_MVA, &mva_wr);
		...
		port.ePortID = M4U_PORT_HW_VDEC_VLD_EXT;
		port.Virtuality = 0;
		m4u_config_port(&port);

		m4u_port_array_init(&port_array);
		m4u_port_array_add(&port_array, M4U_PORT_DISP_RDMA0, 0, 0);
		m4u_port_array_add(&port_array, M4U_PORT_MDP_WDMA0, 0, 0);
		m4u_port_array_add(&port_array, M4U_PORT_DISP_2L_OVL1, 1, 0);
		m4u_port_array_add(&port_array, M4U_PORT_DISP_FAKE1, 0, 0);
		m4u_config_port_array(&port_array);

		m4u_insert_seq_range(M4U_PORT_DISP_FAKE0, 0, 0);

		m4u_display_fake_engine_test(mva_rd, mva_wr);

		M4UMSG("(2)mva_rd:0x%x mva_wr:0x%x\n", mva_rd, mva_wr);

		m4u_dump_pfh_tlb(0);

		m4u_dealloc_mva(client, M4U_PORT_DISP_FAKE0, mva_rd);
		m4u_dealloc_mva(client, M4U_PORT_DISP_FAKE0, mva_wr);
		...
		m4u_print_port_status(NULL, 0);

		m4u_dump_main_tlb(0, 0);
		m4u_dump_main_tlb(0, 1);

		m4u_dump_victim_tlb(0);

		m4u_dump_pfh_tlb(0);

		m4u_invalid_seq_range_by_id(M4U_PORT_DISP_FAKE0, 0);
	...
	*val = 0;
	{
		...
		M4UMSG("(0) pSrc check 0x%x 0x%x 0x%x 0x%x  0x%x\n",
			*pSrc, *(pSrc+1), *(pSrc+126), *(pSrc+127), *(pSrc+128));
		pDst =  kmalloc(allocated_size, GFP_KERNEL | __GFP_ZERO);
		memset(pDst, 0x22, allocated_size);
		M4UMSG("(0) vmalloc pDst:0x%p\n", pDst);
		M4UMSG("(1) pDst check 0x%x 0x%x 0x%x 0x%x  0x%x\n",
			*pDst, *(pDst+1), *(pDst+126), *(pDst+127), *(pDst+128));
...
static int m4u_test_set(void *data, u64 val)
{
	...
	case 3:
		...
		m4u_invalid_tlb_by_range(domain, gM4U_seed_mva, gM4U_seed_mva + 0x1000000, 0);
		m4u_confirm_range_invalidated(0, gM4U_seed_mva, gM4U_seed_mva + 0x1000000, 0);
		m4u_dump_valid_main_tlb(0, 0);
...
static int m4u_debug_freemva_set(void *data, u64 val)
{
	...
	pMvaInfo = mva_get_priv(M4U_TEST_DOMAIN, mva);
	if (pMvaInfo) {
		m4u_unmap(domain, M4U_TEST_DOMAIN, mva, pMvaInfo->size);
