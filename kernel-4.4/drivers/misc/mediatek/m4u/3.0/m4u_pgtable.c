...
void *__m4u_print_pte(m4u_pte_info_t *info, void *data)
{
	if (info->valid) {
		if (info->size == SZ_4K) {
			M4U_PRINT_LOG_OR_SEQ(data, "mva(0x%lx)-->pgd(0x%x)-->pte(0x%x)-->pa(0x%lx) small\n",
					info->mva, imu_pgd_val(*info->pgd), imu_pte_val(*info->pte), info->pa);
		} else if (info->size == SZ_64K) {
			M4U_PRINT_LOG_OR_SEQ(data, "mva(0x%lx)-->pgd(0x%x)-->pte(0x%x)-->pa(0x%lx) large\n",
					info->mva, imu_pgd_val(*info->pgd), imu_pte_val(*info->pte), info->pa);
		} else if (info->size == SZ_1M) {
			M4U_PRINT_LOG_OR_SEQ(data, "mva(0x%lx)-->pgd(0x%x)-->pa(0x%lx) section\n",
					     info->mva, imu_pgd_val(*info->pgd), info->pa);
		} else if (info->size == SZ_16M) {
			M4U_PRINT_LOG_OR_SEQ(data, "mva(0x%lx)-->pgd(0x%x)-->pa(0x%lx) super\n",
					     info->mva, imu_pgd_val(*info->pgd), info->pa);
		}
	} else {
		M4U_PRINT_LOG_OR_SEQ(data, "va(0x%lx)", info->mva);
...
void *m4u_for_each_pte(struct m4u_domain_t *domain, m4u_pte_fn_t *fn, void *data)
{
	...
	while (1) {
		...
		if ((mva + pte_info.size) >= ((unsigned long)M4U_PGD_SIZE >> 2) * MMU_SECTION_SIZE)
			break;
		mva += pte_info.size;
	}
...
int m4u_map_16M(struct m4u_domain_t *m4u_domain, unsigned long mva, unsigned long pa, unsigned int prot)
{
	...
	if ((mva & (~F_PGD_PA_SUPERSECTION_MSK)) != ((unsigned int)pa & (~F_PGD_PA_SUPERSECTION_MSK))) {
		m4u_aee_print("error to mk_pte: mva=0x%lx, pa=0x%lx, type=%s\n", mva, pa, "supersection");
		return -EINVAL;
	}
	...
	M4ULOG_LOW("%s: mva: 0x%lx, pgd: 0x%lx (0x%lx + 0x%lx), pa: 0x%lx, value: 0x%x\n",
	...
	for (i = 0; i < 16; i++) {
		if (unlikely(imu_pgd_val(*pgd))) {
			m4u_aee_print("%s: mva=0x%lx, pgd=0x%x, i=%d\n", __func__, mva, imu_pgd_val(*pgd), i);
			goto err_out;
		}
...
int m4u_map_1M(struct m4u_domain_t *m4u_domain, unsigned long mva, unsigned long pa, unsigned int prot)
{
	...
	if (unlikely(imu_pgd_val(*pgd))) {
		write_unlock_domain(m4u_domain);
		m4u_aee_print("%s: mva=0x%lx, pgd=0x%x\n", __func__, mva, imu_pgd_val(*pgd));
		return -1;
	}
...
int m4u_map_64K(struct m4u_domain_t *m4u_domain, unsigned long mva, unsigned long pa, unsigned int prot)
{
	...
	if (!imu_pgd_val(*pgd)) {
		...
		if (unlikely((imu_pgd_val(*pgd) & (~F_PGD_PA_PAGETABLE_MSK)) != pgprot)) {
			write_unlock_domain(m4u_domain);
			m4u_aee_print("%s: mva=0x%lx, pgd=0x%x, pgprot=0x%x\n",
	...
	M4ULOG_LOW("%s: mva: 0x%lx, pte: 0x%p (0x%lx + 0x%lx), pa: 0x%lx, value: 0x%x\n",
...
{
	int ret;
	unsigned long mva = (unsigned long) iova + ((unsigned long)domain_idx << 32);

	if (size == SZ_16M)
		ret = m4u_map_16M(m4u_domain, mva, paddr, prot);
	else if (size == SZ_1M)
		ret = m4u_map_1M(m4u_domain, mva, paddr, prot);
	else if (size == SZ_64K)
		ret = m4u_map_64K(m4u_domain, mva, paddr, prot);
	else if (size == SZ_4K)
		ret = m4u_map_4K(m4u_domain, mva, paddr, prot);
	else {
		       ...
		       unsigned long paddr, unsigned int size, unsigned int prot)
{
	...
	while (size) {
		...
		ret = m4u_map_phys_align(m4u_domain, domain_idx, iova, paddr, pgsize, prot);
		if (ret)
			break;
	...
	if (ret)
		m4u_unmap(m4u_domain, domain_idx, iova, size);
	return ret;
}
		    ...
		    struct sg_table *sg_table, unsigned int size, unsigned int prot)
{
	...
	for_each_sg(sg_table->sgl, sg, sg_table->nents, i) {
		...
		if (len == SZ_4K) {	/* for most cases */
			...
			ret = m4u_map_phys_range(m4u_domain, domain_idx, map_mva, pa, len, prot);
		}
	...
	m4u_clean_pte(m4u_domain, (unsigned long)mva | (unsigned long)domain_idx << 32, size);

	m4u_invalid_tlb_by_range(m4u_domain, mva, mva + size - 1, domain_idx);
...
err_out:
	...
	m4u_unmap(m4u_domain, domain_idx, mva, size);
	return -EINVAL;
}
...
int m4u_unmap(struct m4u_domain_t *domain, unsigned int domain_idx, unsigned int mva, unsigned int size)
{
	...
	while (mva < end_plus_1) {
		pgd = imu_pgd_offset(domain, (unsigned long)mva | ((unsigned long)domain_idx << 32));
		...
		if (F_PGD_TYPE_IS_PAGE(*pgd)) {
			...
			if (ret == 1) {	/* pte is not freed, need to flush pte */
				m4u_clean_pte(domain, (unsigned long)mva | ((unsigned long)domain_idx << 32),
	...
	m4u_invalid_tlb_by_range(domain, start, end_plus_1 - 1, domain_idx);
