...
int m4u_dump_buf_info(struct seq_file *seq)
{
	...
	for (i = 0; i < MVA_DOMAIN_NR; i++) {
		M4U_PRINT_LOG_OR_SEQ(seq, "Domain index: %d\n", i);
		mva_foreach_priv((void *) m4u_buf_show, seq, i);
	}
...
/***********************************************************/
static struct m4u_buf_info_t *m4u_client_find_buf(m4u_client_t *client, unsigned int mva, int del, unsigned int domain)
{
		  ...
		  unsigned int size, unsigned int prot, unsigned int flags, unsigned int *pMva)
{
	...
	if (flags & M4U_FLAGS_FIX_MVA)
		mva = m4u_do_mva_alloc_fix(domain_idx, va, *pMva, size, pMvaInfo);
	else if (flags & M4U_FLAGS_START_FROM)
		mva = m4u_do_mva_alloc_start_from(domain_idx, va, *pMva, size, pMvaInfo);
	else
	...
	ret = m4u_map_sgtable(m4u_get_domain_by_port(port), domain_idx, mva_align, sg_table,
...
int m4u_dealloc_mva(m4u_client_t *client, M4U_PORT_ID port, unsigned int mva)
{
	...
	ret = m4u_unmap(m4u_get_domain_by_port(port), domain_idx, pMvaInfo->mva_align, pMvaInfo->size_align);
	if (ret) {
		is_err = 1;
...
{
	struct m4u_buf_info_t *pMvaInfo;
	unsigned int domain_idx = 0;  /* only support query domain 0*/
...
static int m4u_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == 0) {
...
#ifdef M4U_TEE_SERVICE_ENABLE
		{
			...
			mva = m4u_do_mva_alloc(M4U_SEC_MVA_DOMAIN, 0, M4U_NONSEC_MVA_START - 0x100000, pMvaInfo);
...

		{
			...
			mva = m4u_do_mva_alloc(M4U_IPU_DOMAIN, 0, 0x50000000 - 0x100000, pMvaInfo);
			M4UINFO("reserve un-used ipu domain: 0x%x\n", mva);
		}
