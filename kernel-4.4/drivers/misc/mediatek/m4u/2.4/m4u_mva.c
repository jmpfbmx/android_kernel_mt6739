...
void m4u_mvaGraph_init(void *priv_reserve)
{
	...
	mvaGraph[vpu_reset_block_end + 1] =
		GET_RANGE_SIZE((vpu_reset_block_end + 1), (vpu_fix_block_start - 1));
...
void m4u_mvaGraph_dump(void)
{
	...
	M4ULOG_HIGH("[M4U_2.4] free region fragments in 2^x blocks unit:===============\n");
	...
	M4ULOG_HIGH("[M4U_2.4] mva alloc dump done=========================<\n");
}
...
int m4u_do_mva_free(unsigned int mva, unsigned int size)
{
	...
	if (is_in_ccu_region_flag) {
		 ...
		 */
		nr_tmp = MVA_GET_NR(endIdx + 1);
		if (is_in_vpu_region(endIdx + 1, nr_tmp) && !MVA_IS_BUSY(endIdx + 1)) {
			nr += nr_tmp;
			mvaGraph[endIdx] = 0;
			mvaGraph[endIdx + 1] = 0;
			MVA_SET_RESERVED(endIdx);
		 ...
		 */
		nr_tmp = MVA_GET_NR(startIdx - 1);
		if (is_in_vpu_region(GET_START_INDEX((startIdx - 1), nr_tmp), nr_tmp)
			&& (!MVA_IS_BUSY(startIdx - 1))) {
			int pre_nr = nr_tmp;
