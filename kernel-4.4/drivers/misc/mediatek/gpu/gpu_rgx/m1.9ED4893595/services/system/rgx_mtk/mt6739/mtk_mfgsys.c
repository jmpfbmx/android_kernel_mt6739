...
#define MTKCLK_set_parent(clkC, clkP) \
	do { \
		if (clkC && clkP) { \
			...
			pr_debug("PVR_K: clk_set_parent failed when enabling " #clkC, #clkP); } \
	} while (0)
#else
#define MTKCLK_prepare_enable(clk)
...
static void MTKEnableMfgMtcmos0(void)
{
	...
	if (isPowerOn == false) {
		if (gpu_debug_enable)
			PVR_DPF((PVR_DBG_MESSAGE, "MTKEnableMfgMtcmos0 mfg0 (0x%p)", mtcmos_mfg0));
...
static void MTKEnableMfgClock(IMG_BOOL bForce)
{
	...
	MTKCLK_set_parent(mfg_clk_top, mfg_clk_on);
	...
	if (gpu_debug_enable)
		PVR_DPF((PVR_DBG_ERROR, "MTKEnableMfgClock mfg1 (0x%p)", mtcmos_mfg1));
...
static void MTKDisableMfgClock(IMG_BOOL bForce)
{
	...
	/* MFGPLL enable */
	/* MTKCLK_prepare_enable(mfg_clk_top); */
	MTKCLK_set_parent(mfg_clk_top, mfg_clk_off);
	MTKCLK_disable_unprepare(mfg_clk_top);
...
static void MTKCommitFreqIdx(unsigned long ui32NewFreqID, GED_DVFS_COMMIT_TYPE eCommitType, int *pbCommited)
{
	...
	if (psDevNode) {
		...
		if (gpu_debug_enable)
			...
			* letting GPU power down with high possibility while DVFS committing
			*/
			if (pbCommited)
				*pbCommited = IMG_TRUE;
			return;
...
static void MTKCalGpuLoading(unsigned int *pui32Loading, unsigned int *pui32Block, unsigned int *pui32Idle)
{
	...
	if (psDevInfo && psDevInfo->pfnGetGpuUtilStats) {
		...
		*pui32Loading =
			(100*(sGpuUtilStats.ui64GpuStatActiveHigh + sGpuUtilStats.ui64GpuStatActiveLow))
			/ sGpuUtilStats.ui64GpuStatCumulative;
		*pui32Block =
			(100*(sGpuUtilStats.ui64GpuStatBlocked)) / sGpuUtilStats.ui64GpuStatCumulative;
		*pui32Idle =
			(100*(sGpuUtilStats.ui64GpuStatIdle)) / sGpuUtilStats.ui64GpuStatCumulative;
#else
		*pui32Loading =	(unsigned long)(100*(sGpuUtilStats.ui64GpuStatActiveHigh +
			sGpuUtilStats.ui64GpuStatActiveLow)) / (unsigned long)sGpuUtilStats.ui64GpuStatCumulative;
	...
	IMG_BOOL bForced)
{
	if (eNewPowerState == PVRSRV_DEV_POWER_STATE_OFF &&
		...
		MTKDisableMfgClock(bForced);
			...
			bForced == true) {
		if (gpu_debug_enable)
			PVR_DPF((PVR_DBG_MESSAGE, "MTKDevPrePowerState MTKDisableMfgMtcmos0"));
		MTKDisableMfgMtcmos0();
	}
#endif
	return PVRSRV_OK;
	...
	if (s_ui32GPUFreq != ui32GPUFreq) {
		if (psDevNode) {
			...
			if ((eResult == PVRSRV_OK) || (eResult == PVRSRV_ERROR_RETRY)) {
				...
				MTKWriteBackFreqToRGX(psDevNode, ui32GPUFreq);
				/*PVR_DPF((PVR_DBG_ERROR, "mtk_fdvfs_update_cur_freq %d, %d", ui32NewFreqID, param2));*/
...
PVRSRV_ERROR MTKMFGSystemInit(void)
{
	...
	g_hDVFSTimer = OSAddTimer(MTKDVFSTimerFuncCB, (void *)NULL, MTK_DVFS_SWITCH_INTERVAL_MS);
...
int MTKRGXDeviceInit(PVRSRV_DEVICE_CONFIG *psDevConfig)
{
	...
	_mtk_ged_log = ged_log_buf_alloc(64, 64 * 32, GED_LOG_BUF_TYPE_RINGBUFFER, "PowerLog", "ppL");
	_track_ged_log = ged_log_buf_alloc(32, 32 * 32, GED_LOG_BUF_TYPE_RINGBUFFER, "RegStack", "RTrace");
	...
	if (psDevConfig) {
		...
		mfg_clk_top = devm_clk_get(pdev, "CLK_TOP_MFG");

		if (IS_ERR(mfg_clk_top))
			PVR_DPF((PVR_DBG_ERROR, "FAILED to get CLK_TOP_MFG (%ld)\n", PTR_ERR(mfg_clk_top)));

		mfg_clk_off = devm_clk_get(pdev, "CLK_TOP_MFG_OFF");

		if (IS_ERR(mfg_clk_off))
			PVR_DPF((PVR_DBG_ERROR, "FAILED to get CLK_TOP_MFG_OFF (%ld)\n", PTR_ERR(mfg_clk_off)));

		mfg_clk_on = devm_clk_get(pdev, "CLK_TOP_MFG_ON");

		if (IS_ERR(mfg_clk_on))
			PVR_DPF((PVR_DBG_ERROR, "FAILED to get CLK_TOP_MFG_ON (%ld)\n", PTR_ERR(mfg_clk_on)));

		mtcmos_mfg0 = devm_clk_get(pdev, "CG_SCP_SYS_MFG0");

		if (IS_ERR(mtcmos_mfg0))
			PVR_DPF((PVR_DBG_ERROR, "FAILED to get CG_SCP_SYS_MFG0 (%ld)\n", PTR_ERR(mtcmos_mfg0)));

		mtcmos_mfg1 = devm_clk_get(pdev, "CG_SCP_SYS_MFG1");

		if (IS_ERR(mtcmos_mfg1))
			PVR_DPF((PVR_DBG_ERROR, "FAILED to get CG_SCP_SYS_MFG1 (%ld)\n", PTR_ERR(mtcmos_mfg1)));
	}
#endif
