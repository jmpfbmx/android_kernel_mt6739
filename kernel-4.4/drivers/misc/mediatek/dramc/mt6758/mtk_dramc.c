...
void __iomem *SLEEP_SRAM_BASE_ADDR;
...
#if 0
	if (time_cnt == 0)
		pr_info("[DRAMC] read mode reg time out 2\n");
#endif
...
pi_new[shu_index][rank][byte]);
	...
	if (time_cnt == 0) {
		pr_info("[DRAMC] write DDRPHY time out\n");
...
void dump_tx_log(unsigned int res)
{
	...
	case TX_TIMEOUT_MRR_ENABLE:
		pr_info("[DRAMC] TX MRR enable timeout\n");
		break;
	case TX_TIMEOUT_MRR_DISABLE:
		pr_info("[DRAMC] TX MRR disable timeout\n");
		break;
	case TX_TIMEOUT_DQSOSC:
		pr_info("[DRAMC] TX DQS OSC timeout\n");
		break;
	case TX_TIMEOUT_DDRPHY:
		pr_info("[DRAMC] TX DDRPHY update timeout\n");
		break;
	case TX_FAIL_DATA_RATE:
		pr_info("[DRAMC] TX read data rate fail\n");
		break;
	case TX_FAIL_VARIATION:
		pr_info("[DRAMC] TX variation is too large\n");
		break;
	default:
		pr_info("[DRAMC] TX unknown error\n");
		break;
	}
}
#endif
			   ...
			   unsigned char segment_rank1)
{
	...
	if (acquire_dram_ctrl() != 0) {
		pr_info("[DRAMC0] can NOT get SPM HW SEMAPHORE!\n");
...
#if defined(SW_ZQCS) || defined(SW_TX_TRACKING)
	spm_request_dvfs_opp(0, OPP_UNREQ);
	mod_timer(&zqcs_timer, jiffies + msecs_to_jiffies(280));
#endif
...
static int dram_probe(struct platform_device *pdev)
{
	...
	if (node) {
		...
		SLEEP_SRAM_BASE_ADDR = of_iomap(node, 1);
		pr_info("[DRAMC]get SLEEP_BASE_ADDR @ %p, SLEEP_SRAM_BASE_ADDR @ %p\n",
		SLEEP_BASE_ADDR, SLEEP_SRAM_BASE_ADDR);
	} else {
	...
	if (!CH_NUM) {
		pr_info("[DRAMC Driver] channel number error !!\n");
		return -1;
	}
#else
	CH_NUM = 2;
