...
static unsigned int dramc_tx_tracking(int channel)
{
	...
	unsigned int dqm_orig[4][2][2];
	unsigned int dqm_new[4][2][2];
	...
	tx_freq_ratio[3] = dram_steps_freq(3) * 8 / dram_steps_freq(shu_level);
...
static int dram_probe(struct platform_device *pdev)
{
	...
	if (node) {
		...
		pr_err("[DRAMC]can't find SYS_TIMER_BASE_ADDR compatible node\n");
		return -1;
	}
