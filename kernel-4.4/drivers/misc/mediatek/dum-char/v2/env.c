...
#else
long long data_free_size_th;
#endif

/* /dev/misc is not exist until Android is started, */
/* postpone read Nand to first read/write function */
...
/* limit variable count to avoid overflow */
	if (is_first_read_write < 2)
		is_first_read_write++;
			      ...
			      loff_t *ppos)
{
	...
	if (!strcmp(buffer, DATA_FREE_SIZE_TH_NAME)) {
		...
		ret = kstrtoll(value_buf, 0, &value_new);
		value_new *= mum;
