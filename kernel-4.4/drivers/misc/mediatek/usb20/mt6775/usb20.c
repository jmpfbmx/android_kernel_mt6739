...
#if 1
static void usb_6775_dpidle_request(int mode)
{
...
#if !defined(FPGA_PLATFORM) && defined(CONFIG_MTK_BOOT)
		s64 diff_time;
		ktime_t ktime_now;
...
#define	VUSB33_VOL_MAX 3070000
	...
	if (!IS_ERR(reg_va12)) {
		...
		if (ret < 0) {
			pr_info("regulator_enable va12 failed: %d\n", ret);
			regulator_put(reg_va12);
		}
	} else
		pr_info("regulator_get va12 failed\n");
...
static int mt_usb_probe(struct platform_device *pdev)
{
	...
	if (!glue) {
		/* pr_info("failed to allocate glue context\n"); */
		goto err0;
	}

	musb = platform_device_alloc("musb-hdrc", PLATFORM_DEVID_NONE);
	if (!musb) {
		pr_info("failed to allocate musb device\n");
		goto err1;
	}
	...
	if (!pdata) {
		pr_info("failed to allocate musb platform data\n");
		goto err2;
	}

	config = devm_kzalloc(&pdev->dev, sizeof(*config), GFP_KERNEL);
	if (!config) {
		/* pr_info(&pdev->dev, "failed to allocate musb hdrc config\n"); */
		goto err2;
	}
#ifdef CONFIG_USB_MTK_OTG
	pdata->mode = MUSB_OTG;
	...
	if (ap_gpio_node == NULL) {
		pr_info("USB get ap_gpio_node failed\n");
		if (ap_gpio_base)
	...
	register_usb_hal_dpidle_request(usb_6775_dpidle_request);
