...
static int __init oem_uart_enabled_setup(char *str)
{
    unsigned long enabled;
    if (!kstrtoul(str, 0, &enabled))
        oem_enable_uart = enabled ? true : false;
    DBG(0,"%s oem_enable_uart is %d\n",__func__,oem_enable_uart);
    return 1;
}
__setup("oem_uart=", oem_uart_enabled_setup);
