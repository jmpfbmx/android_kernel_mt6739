...
int32_t cmdq_mdp_reset_with_mmsys(const uint64_t engineToResetAgain)
{
	...
	int engineResetBit1[32] = {
		...
		CMDQ_ENG_MDP_CCORR0,   /* bit  12 : MDP_CCORR */
		CMDQ_ENG_MDP_AAL0,   /* bit  13 : MDP_AAL */
		[14 ... 31] = -1
	};
...
void testcase_clkmgr_mdp(void)
{
	...
	testcase_clkmgr_impl(CMDQ_ENG_MDP_CCORR0,
			     "CMDQ_TEST_MDP_CCORR",
			     MDP_CCORR_BASE + 0x30, 0x1FFF1FFF, MDP_CCORR_BASE + 0x30, true);
	/* AAL clk test with debug port */
	testcase_clkmgr_impl(CMDQ_ENG_MDP_AAL0,
			     "CMDQ_TEST_MDP_AAL",
			     MDP_AAL_BASE + 0x30, 0x1FFF1FFF, MDP_AAL_BASE + 0x30, true);
#endif
}
