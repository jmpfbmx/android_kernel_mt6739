...
static atomic_t gSMILarb1Usage;
...
struct CmdqMdpModuleClock {
	...
	struct clk *clk_IPU_MDP_TX;
	struct clk *clk_IPU_MDP_RX;
...
IMP_ENABLE_MDP_HW_CLOCK(IPU_MDP_TX, IPU_MDP_TX);
IMP_ENABLE_MDP_HW_CLOCK(IPU_MDP_RX, IPU_MDP_RX);
...
IMP_MDP_HW_CLOCK_IS_ENABLE(IPU_MDP_TX, IPU_MDP_TX);
IMP_MDP_HW_CLOCK_IS_ENABLE(IPU_MDP_RX, IPU_MDP_RX);
...
int32_t cmdq_mdp_reset_with_mmsys(const uint64_t engineToResetAgain)
{
	...
	int engineResetBit[32] = {
		...
		CMDQ_ENG_MDP_IPUIN,	/* bit  4 : IPU_MDP */
...
bool cmdq_mdp_clock_is_on(enum CMDQ_ENG_ENUM engine)
{
	...
	case CMDQ_ENG_MDP_IPUIN:
		return cmdq_mdp_clock_is_enable_IPU_MDP_TX();
...
void cmdq_mdp_enable_larb1_clock(bool enable)
{
	if (enable) {
		CMDQ_VERBOSE("[CLOCK] Enable SMI Larb %d\n", atomic_read(&gSMILarb1Usage));
		if (atomic_inc_return(&gSMILarb1Usage) == 1)
			smi_bus_enable(SMI_LARB_MMSYS1, "CMDQ");
	} else {
		CMDQ_VERBOSE("[CLOCK] Disable SMI Larb %d\n", atomic_read(&gSMILarb1Usage));
		if (atomic_dec_return(&gSMILarb1Usage) == 0)
...
void cmdq_mdp_enable_clock(bool enable, enum CMDQ_ENG_ENUM engine)
{
	...
	case CMDQ_ENG_MDP_RDMA1:
		if (true == enable) {
			cmdq_mdp_enable_larb1_clock(enable);
		...
		break;
	case CMDQ_ENG_MDP_IPUIN:
		cmdq_mdp_enable_clock_IPU_MDP_TX(enable);
		cmdq_mdp_enable_clock_IPU_MDP_RX(enable);
		break;
	case CMDQ_ENG_MDP_RSZ0:
...
void cmdq_mdp_init_module_clk(void)
{
	...
	cmdq_dev_get_module_clock_by_name("mmsys_config", "IPU_MDP_TX",
					  &gCmdqMdpModuleClock.clk_IPU_MDP_TX);
	cmdq_dev_get_module_clock_by_name("mmsys_config", "IPU_MDP_RX",
					  &gCmdqMdpModuleClock.clk_IPU_MDP_RX);
...
void cmdqMdpInitialSetting(void)
{
	...
	mtk_iommu_register_fault_callback(M4U_PORT_MDP_RDMA1, cmdq_TranslationFault_callback, (void *)data);
	mtk_iommu_register_fault_callback(M4U_PORT_MDP_WROT0, cmdq_TranslationFault_callback, (void *)data);
	mtk_iommu_register_fault_callback(M4U_PORT_MDP_WROT1, cmdq_TranslationFault_callback, (void *)data);
#elif defined(CONFIG_MTK_M4U)
	char *data = kzalloc(MDP_DISPATCH_KEY_STR_LEN, GFP_KERNEL);
	...
	m4u_register_fault_callback(M4U_PORT_MDP_RDMA1, cmdq_TranslationFault_callback, (void *)data);
	m4u_register_fault_callback(M4U_PORT_MDP_WROT0, cmdq_TranslationFault_callback, (void *)data);
	m4u_register_fault_callback(M4U_PORT_MDP_WROT1, cmdq_TranslationFault_callback, (void *)data);
#endif
	atomic_set(&gSMILarb1Usage, 0);
}
