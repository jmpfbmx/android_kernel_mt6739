...
static struct task_struct *Ripi_eem_task;
static struct task_struct *Ripi_met_task;
...
static struct task_struct *Sipi_eem_task;
...
static const int pin_size[] = { 3, 4, 3, 4, 4, 9, 2, 8, 5, 7, 4 };
static const int pin_mode[] = { 1, 1, 0, 1, 0, 1, 1, 1, 1, 0, 0 };
...
int Sipi_eem_thread(void *data)
{
	int i, j, ret;
	uint32_t retdata, retchk;
	uint32_t ipi_buf[4];

	pr_debug("Info: IPI EEM thread...\n");
	do {
		if (atomic_read(&myipi_err) == 0) {
			i = IPI_ID_EEM;
			for (j = 0; j < pin_size[i]; j++)
...
int ipi_test_thread(void *data)
{
	...
	if (atomic_read(&myipi) == 1) {
		...
		Sipi_eem_task = kthread_run(Sipi_eem_thread, data, "Sipi_eem_thread");
		Sipi_met_task = kthread_run(Sipi_met_thread, data, "Sipi_met_thread");
		...
		kthread_stop(Sipi_eem_task);
		kthread_stop(Sipi_met_task);
...
static struct ipi_action plf_act, cpudvfs_act, gpudvfs_act, eem_act, met_act;
static uint32_t plf_buf[3], cpudvfs_buf[4], gpudvfs_buf[3], eem_buf[4], met_buf[4];
...
int Ripi_eem_thread(void *data)
{
	int i, ret;
	uint32_t adata;

	pr_debug("EEM received thread\n");

	eem_act.data = (void *)eem_buf;
	ret = sspm_ipi_recv_registration(IPI_ID_EEM, &eem_act);
	if (ret != 0) {
		pr_debug("Error: ipi_recv_registration EEM error: %d\n", ret);
		do {
			msleep(1000);
		} while (!kthread_should_stop());
		return (-1);
	...
	do {
		i++;
		sspm_ipi_recv_wait(IPI_ID_EEM);
		pr_debug("Info: EEM thread received ID=%d, i=%d\n", eem_act.id, i);
		chkdata("EEM", &eem_act, &adata);
		pr_debug("Info: EEM Task send back ack data=%08X\n", adata);
		ret = sspm_ipi_send_ack(IPI_ID_EEM, &adata);
		if (ret != 0)
			pr_debug("Error: ipi_send_ack EEM error: %d\n", ret);

	} while (!kthread_should_stop());
	return 0;
}

int Ripi_met_thread(void *data)
{
	int i, ret;
	uint32_t adata;
...
static int __init ipi_test_init_module(void)
{
	...
	Ripi_eem_task = kthread_run(Ripi_eem_thread, NULL, "ipi_eem_rtask");
	Ripi_met_task = kthread_run(Ripi_met_thread, NULL, "ipi_met_rtask");
	return 0;
}
...
static void __exit ipi_test_cleanup_module(void)
{
	...
	if (Ripi_eem_task) {
		kthread_stop(Ripi_eem_task);
		Ripi_eem_task = NULL;
	}
	if (Ripi_met_task) {
		kthread_stop(Ripi_met_task);
		Ripi_met_task = NULL;
	}
