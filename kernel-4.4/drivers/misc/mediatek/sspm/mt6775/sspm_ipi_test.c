...
static struct task_struct *Ripi_qos_task;
static struct task_struct *Ripi_tst1_task;

#if 1
static struct task_struct *Sipi_plf_task;
static struct task_struct *Sipi_cpu_dvfs_task;
static struct task_struct *Sipi_qos_task;
static struct task_struct *Sipi_tst1_task;
...
static struct task_struct *Sipi_upwr_task;
#endif

static const int pin_size[] = { 3, 4, 4, 4, 9, 2, 8, 5, 7, 4, 4 };
static const int pin_mode[] = { 1, 1, 1, 0, 1, 1, 1, 1, 1, 0, 1 };
...
int Sipi_qos_thread(void *data)
{
	int i, j, ret;
	uint32_t retdata, retchk;
	uint32_t ipi_buf[4];

	pr_debug("Info: IPI QOS thread...\n");
	do {
		if (atomic_read(&myipi_err) == 0) {
			i = IPI_ID_QOS;
			for (j = 0; j < pin_size[i]; j++)
...
int Sipi_tst1_thread(void *data)
{
	int i, j, ret;
	uint32_t retdata, retchk;
	uint32_t ipi_buf[4];
...
int Sipi_thml_thread(void *data)
{
	int i, j, ret;
	uint32_t ipi_buf[7];

	pr_debug("Info: IPI Thermal thread...\n");
	do {
		if (atomic_read(&myipi_err) == 0) {
			i = IPI_ID_THERMAL;
			for (j = 0; j < pin_size[i]; j++)
...
int Sipi_upwr_thread(void *data)
{
	int i, j, ret;
	uint32_t ipi_buf[7];

	pr_debug("Info: IPI UPower thread...\n");
	do {
		if (atomic_read(&myipi_err) == 0) {
			i = IPI_ID_UPOWER;
			for (j = 0; j < pin_size[i]; j++)
...
int ipi_test_thread(void *data)
{
	...
	if (atomic_read(&myipi) == 1) {
		...
		Sipi_qos_task = kthread_run(Sipi_qos_thread, data, "Sipi_qos_thread");
		Sipi_tst1_task = kthread_run(Sipi_tst1_thread, data, "Sipi_tst1_thread");
		...
		Sipi_thml_task = kthread_run(Sipi_thml_thread, data, "Sipi_thml_thread");
		Sipi_upwr_task = kthread_run(Sipi_upwr_thread, data, "Sipi_upwr_thread");
		...
		kthread_stop(Sipi_qos_task);
		kthread_stop(Sipi_tst1_task);
		...
		kthread_stop(Sipi_thml_task);
		kthread_stop(Sipi_upwr_task);
	}
...
static struct ipi_action plf_act, cpudvfs_act, qos_act, tst1_act;
static uint32_t plf_buf[3], cpudvfs_buf[4], qos_buf[4], tst1_buf[4];
...
int Ripi_qos_test_thread(void *data)
{
	int i, ret;
	uint32_t adata;

	pr_debug("QOS received thread\n");

	qos_act.data = (void *)qos_buf;
	ret = sspm_ipi_recv_registration(IPI_ID_QOS, &qos_act);
	if (ret != 0) {
		pr_debug("Error: ipi_recv_registration QOS error: %d\n", ret);
		do {
			msleep(1000);
		} while (!kthread_should_stop());
		return (-1);
	...
	do {
		...
		pr_debug("Info: QOS thread received ID=%d, i=%d\n", qos_act.id, i);
		chkdata("QOS", &qos_act, &adata);
		pr_debug("Info: QOS Task send back ack data=%08X\n", adata);
		ret = sspm_ipi_send_ack(IPI_ID_QOS, &adata);
		if (ret != 0)
			pr_debug("Error: ipi_send_ack QOS error: %d\n", ret);

	} while (!kthread_should_stop());
	return 0;
}

int Ripi_tst1_test_thread(void *data)
{
	int i, ret;
	uint32_t adata;
	...
	tst1_act.data = (void *)tst1_buf;
	ret = sspm_ipi_recv_registration(IPI_ID_TST1, &tst1_act);
	if (ret != 0) {
...
static int __init ipi_test_init_module(void)
{
	...
	Ripi_qos_task = kthread_run(Ripi_qos_test_thread, NULL, "ipi_qos_rtask");
	Ripi_tst1_task = kthread_run(Ripi_tst1_test_thread, NULL, "ipi_tst1_rtask");
	return 0;
}
...
static void __exit ipi_test_cleanup_module(void)
{
	...
	if (Ripi_qos_task) {
		kthread_stop(Ripi_qos_task);
		Ripi_qos_task = NULL;
	}
	if (Ripi_tst1_task) {
		kthread_stop(Ripi_tst1_task);
		Ripi_tst1_task = NULL;
	}
}
