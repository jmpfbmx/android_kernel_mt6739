...
#define VALIDATE_PLLID(id) WARN_ON((id >= FH_PLL_NUM)  || (g_reg_pll_con0[id] == REG_PLL_NOT_SUPPORT))
...
static int fhctl_to_sspm_command(unsigned int cmd, struct fhctl_ipi_data *ipi_data)
{
	...
	case FH_DCTL_CMD_DVFS:
		ipi_data->cmd = cmd;
		ret = sspm_ipi_send_sync(IPI_ID_FHCTL, IPI_OPT_POLLING, ipi_data, FHCTL_D_LEN, &ack_data, 1);
		if (ret != 0)
...
static int __freqhopping_ctrl(struct freqhopping_ioctl *fh_ctl, bool enable)
{
	...
	if (g_reg_pll_con0[fh_ctl->pll_id] == REG_PLL_NOT_SUPPORT) {
		FH_MSG("(ERROR) %s [pll_id]:%d frequency hopping is not supported ", __func__, fh_ctl->pll_id);
		return -1;
	}
...
static int mt_fh_hal_general_pll_dfs(enum FH_PLL_ID pll_id, unsigned int target_dds)
{
	...
	/* [MT6759 Only] All new platform should confirm again!!! */
	switch (pll_id) {
	case FH_MEM_PLLID: /* MEMPLL Confirmed with DRAM SA that MEMPLL hopping will only control by clk DIV*/
		FH_MSG("ERROR! The [PLL_ID]:%d was forbidden hopping by MT6759 FHCTL.", pll_id);
		WARN_ON(1);
		break;
	default:
		break;
...
/* [For MT6759] GPU CLK hopping, MFGPLL */
...
static int fh_dvfs_proc_write(struct file *file, const char *buffer, unsigned long count,
			      void *data)
{
	FH_MSG("[ERROR] %s is not surpported in this chip !!!", __func__);
...
int mt_pause_armpll(unsigned int pll, unsigned int pause)
{
	...
	if (g_reg_pll_con0[pll] == REG_PLL_NOT_SUPPORT) {
		FH_MSG("(ERROR) %s [pll_id]:%d frequency hopping is not supported ", __func__, pll);
		return -1;
	}
