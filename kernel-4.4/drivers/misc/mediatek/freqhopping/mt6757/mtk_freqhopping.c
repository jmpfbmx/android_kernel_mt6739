...
static const unsigned int g_ssc_setting_size[] = {
	ARRAY_SIZE(ssc_armpll1_setting),
	ARRAY_SIZE(ssc_armpll2_setting),
	ARRAY_SIZE(ssc_armpll3_setting),
	ARRAY_SIZE(ssc_ccipll_setting),
	ARRAY_SIZE(ssc_gpupll_setting),
...
#define VALIDATE_PLLID(id)	WARN_ON((id >= FH_PLL_NUM) || (g_reg_pll_con1[id] == REG_PLL_NOT_SUPPORT))
...
static int mt_fh_hal_general_pll_dfs(enum FH_PLL_ID pll_id, unsigned int target_dds)
{
	...
	if (g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC) {
		const struct freqhopping_ssc *p_setting = &g_ssc_setting[pll_id][2];
