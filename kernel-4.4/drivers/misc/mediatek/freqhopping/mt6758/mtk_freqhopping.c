...
static int mt_fh_hal_hopping(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	if (g_reg_pll_con0[pll_id] == REG_PLL_NOT_SUPPORT) {
		FH_MSG("(ERROR) %s [pll_id]:%d frequency hopping is not supported ", __func__, pll_id);
		return -1;
	}
...
static int mt_fh_hal_dfs_armpll(unsigned int coreid, unsigned int dds)
{
	...
	if (g_fh_pll[pll].fh_status == FH_FH_ENABLE_SSC) {
		...
		if (g_fh_pll[pll].user_defined == true)
			...
			p_setting = &g_pll_ssc_setting_tbl[pll][PLL_SETTING_IDX__DEF];
...
/*  GPU CLK hopping, MFGPLL */
static int mt_fh_hal_dfs_mmpll(unsigned int target_dds)
{
	/* unsigned long flags = 0; */
	/* MFGPLL, ToDo: Need to confirmed with GPU clk owner */
	...
	if (g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC) {
		...
		if (g_fh_pll[pll_id].user_defined == true)
			...
			p_setting = &g_pll_ssc_setting_tbl[pll_id][PLL_SETTING_IDX__DEF];
...
static void mt_fh_hal_popod_save(void)
{
	...
	if (g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC) {
		...
		fh_write32(g_reg_pll_con1[pll_id],
			   (fh_read32(g_reg_dds[pll_id]) & MASK22b) |
			   ((fh_read32(REG_FH_PLL2_CON1) & 0xFFC00000)) | (BIT32));
	...
	case 4370:		/* TODO: and what this case for? Nobody know! */
		{
			...
			fh_write32(g_reg_updnlmt[p2],
				   (PERCENT_TO_DDSLMT((fh_read32(reg_cfg) & MASK22b), p5) << 16));
