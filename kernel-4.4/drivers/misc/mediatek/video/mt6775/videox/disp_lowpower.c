...
int primary_display_dsi_vfp_change(int state)
{
	...
	if (state == 1) {
		...
		dpmgr_path_ioctl(primary_get_dpmgr_handle(), handle, DDP_DSI_PORCH_CHANGE,
...
void primary_disp_set_sodi_mode(int forbid_pdmode)
{
	if (forbid_pdmode)
		rdma_sodi_mode(DISP_MODULE_RDMA0, 1);
	else
		rdma_sodi_mode(DISP_MODULE_RDMA0, 0);
}
...
int primary_display_request_dvfs_perf(int scenario, int req)
{
	...
	if (atomic_read(&dvfs_ovl_req_status) != req) {
		...
		case HRT_LEVEL_LEVEL1:
			step = MMDVFS_FINE_STEP_OPP5;
			break;
		case HRT_LEVEL_LEVEL0:
			step = MMDVFS_FINE_STEP_OPP6;
			break;
