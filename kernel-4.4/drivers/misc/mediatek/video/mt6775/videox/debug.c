...
{
	/*after config fake engine, command mode enter idle will power off MMSYS, so we should disable this*/
	enable_idlemgr(0);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
		...
		gen_hrt_pattern();
		DISPMSG("hrt_debug\n");
	} else if (strncmp(opt, "rpo_en:", 7) == 0) {
		int en = 0;

		ret = sscanf(opt, "rpo_en:%d\n", &en);
		if (ret != 1) {
			DISPERR("%d error to parse cmd %s\n", __LINE__, opt);
			return;
		}
		disp_rsz_sram_set_en(!!en);
		DISPMSG("rpo_en:%d\n", !!en);
	} else if (strncmp(opt, "rpo_cache", 9) == 0) {
		primary_display_use_input_cache();
		DISPMSG("rpo cache\n");
		...
		if (strncmp(opt + 9, "de", 2) == 0) {
			disp_fake_engine_config(fb_pa, fb_pa+4, 1, 2047, 11, 0, 0, 0, 1);
		} else {
	...
	if (strncmp(opt, "primary_basic_show:", 19) == 0) {
		int mode = 0;

		ret = sscanf(opt, "primary_basic_show:%d\n", &mode);
		if (ret != 1) {
			DISPERR("error to parse cmd %s, ret=%d\n", opt, ret);
			return;
		}

		primary_display_basic_test(3, 0xff, 300, 300, DISP_FORMAT_RGBA8888, 20,
			1, 100, 100, 20, 170, 190, 125, mode, 0);
	}
	/* it may have BG flash in the frist image, because it just set dst_drity = 1
	* and do not config ovl when DC->DL in vdo mode.
	*/
	if (strncmp(opt, "ovl_bg:", 7) == 0) {
		int en = 0;

		ret = sscanf(opt, "ovl_bg:%d\n", &en);
		if (ret != 1) {
			DISPERR("error to parse cmd %s, ret=%d\n", opt, ret);
			return;
		}
