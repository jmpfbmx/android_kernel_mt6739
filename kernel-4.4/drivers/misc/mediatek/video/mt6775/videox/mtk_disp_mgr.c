...
/* must below  #include "disp_drv_platform.h" */
#ifdef SUPPORT_EXT_DISP
...
#ifndef SUPPORT_EXT_DISP
static int external_display_switch_mode(enum DISP_MODE mode,
...
static int ext_disp_get_max_layer(void)
{
	return 0;
}

static int external_display_get_info(void *info, unsigned int session)
{
	return 0;
}
#endif

static int external_display_wait_for_vsync(void *config, unsigned int session)
{
	return 0;
}

static void external_display_control_init(void)
{
}
#endif
...
int _ioctl_get_display_caps(unsigned long arg)
{
	...
	if (disp_helper_get_option(DISP_OPT_RSZ) ||
		...
		caps_info.rsz_in_max[0] = primary_display_get_width();
		caps_info.rsz_in_max[1] = primary_display_get_height();
	}
...
int _ioctl_query_valid_layer(unsigned long arg)
{
	...
	for (i = 0; i < 2; i++) {
		if (!access_ok(VERIFY_READ, disp_info_user.input_config[i], sizeof(struct layer_config))) {
...
{
	DISPMSG("mtk_disp_mgr_init start\n");
