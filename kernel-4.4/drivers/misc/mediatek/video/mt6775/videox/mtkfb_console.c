...
static void _MFC_DrawRow(struct MFC_CONTEXT *ctxt, BYTE *dest, BYTE raw_color)
{
	UINT32 pixel_row, i;
	uint16_t color;
	int cols;
...
static void _MFC_DrawChar(struct MFC_CONTEXT *ctxt, UINT32 x, UINT32 y, char c)
{
	...
	if (x > (MFC_WIDTH - MFC_FONT_WIDTH)) {
		DISPERR("draw width too large,x=%d\n", x);
		return;
	}
	if (y > (MFC_HEIGHT - MFC_FONT_HEIGHT)) {
		DISPERR("draw hight too large,y=%d\n", y);
		return;
	}
	...
	case 4:
		...
		break;
	default:
		DISPERR("draw char fail,MFC_BPP=%d\n", MFC_BPP);
	}
}
