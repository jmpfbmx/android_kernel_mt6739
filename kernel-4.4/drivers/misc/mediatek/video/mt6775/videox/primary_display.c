				...
				unsigned int x_pos, unsigned int block_sz)
{
	...
	if (ret) {
		DISPERR("%s error to query mva = 0x%x\n", __func__, mva);
		return -1;
	}
	ret = m4u_mva_map_kernel(real_mva, real_size, &map_va, &map_size);
	if (ret) {
		DISPERR("%s error to map mva = 0x%x\n", __func__, real_mva);
		return -1;
	}
...
static int _DL_switch_to_DC_fast(int block)
{
	...
	if (disp_helper_get_option(DISP_OPT_RPO)) {
		memcpy(&data_config_dc->rsz_config, &data_config_dl->rsz_config,
		...
		data_config_dc->hrt_scale_h = data_config_dl->hrt_scale_h;
		data_config_dc->hrt_scale_w = data_config_dl->hrt_scale_w;
	}
...
static int _DC_switch_to_DL_fast(int block)
{
	...
	if (disp_helper_get_option(DISP_OPT_RPO)) {
		memcpy(&data_config_dl->rsz_config, &data_config_dc->rsz_config,
		       sizeof(data_config_dl->rsz_config));
		data_config_dl->rsz_enable = data_config_dc->rsz_enable;
		data_config_dl->hrt_path = data_config_dc->hrt_path;
		data_config_dl->hrt_scale_h = data_config_dc->hrt_scale_h;
		data_config_dl->hrt_scale_w = data_config_dc->hrt_scale_w;
	}
...
static int _convert_disp_input_to_ovl(struct OVL_CONFIG_STRUCT *dst, struct disp_input_config *src)
{
	...
	if (src->src_width != 0 && src->src_width != src->tgt_width)
		dst->dst_x = src->tgt_offset_x * src->src_width / src->tgt_width;
	else
		dst->dst_x = src->tgt_offset_x;

	if (src->src_height != 0 && src->src_height != src->tgt_height)
		dst->dst_y = src->tgt_offset_y * src->src_height / src->tgt_height;
	else
...
static int _DL_switch_to_rdma_witch_cache(int rsz_disable)
{
	int ret = 0;
	enum DDP_SCENARIO_ENUM old_scenario, new_scenario;
	struct RDMA_CONFIG_STRUCT rdma_config = decouple_rdma_config;
	...
	DISPMSG("primary display will switch from %s to RDMA_MODE with cache\n",
	...
	if (rsz_disable)
		dpmgr_path_ioctl(pgc->dpmgr_handle, pgc->cmdq_handle_config, DDP_RSZ_BYPASS_MODE, NULL);
...
int primary_display_use_input_cache(void)
{
	int ret = 0;
	struct cmdqRecStruct *cmdq_handle = NULL;
	struct disp_ddp_path_config *pconfig = NULL;

	if (!disp_helper_get_option(DISP_OPT_RPO_INPUT_CACHE))
		return ret;

	_primary_path_lock(__func__);

	if (primary_display_is_decouple_mode()) {
		pconfig = dpmgr_path_get_last_config_notclear(pgc->ovl2mem_path_handle);
		if (!pconfig->rsz_enable)
			goto out;
		ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle);
		if (ret) {
			DISPERR("%s create cmdq handle\n", __func__);
			ret = -1;
			goto out;
		}
		cmdqRecReset(cmdq_handle);
		cmdqRecWaitNoClear(cmdq_handle, CMDQ_EVENT_DISP_WDMA0_EOF);
		dpmgr_path_ioctl(pgc->ovl2mem_path_handle, cmdq_handle, DDP_RSZ_BYPASS_MODE, NULL);
		cmdqRecFlush(cmdq_handle);
		cmdqRecDestroy(cmdq_handle);
	} else if (primary_display_is_directlink_mode()) {
		pconfig = dpmgr_path_get_last_config_notclear(pgc->dpmgr_handle);
		if (pconfig->rsz_enable)
			_DL_switch_to_rdma_witch_cache(1);
	}
out:
	_primary_path_unlock(__func__);
...
int primary_display_release_fence_fake(void)
{
	...
	for (i = 0; i < PRIMARY_SESSION_INPUT_LAYER_COUNT; i++) {
		if (i == primary_display_get_option("ASSERT_LAYER") && is_DAL_Enabled()) {
			...
			if (fence_idx == -1) {
				DISPERR("find fence for layer %d,addr 0x%08x fail, unregistered\n", i, addr);
			} else if (fence_idx < 0) {
				DISPERR("find fence idx for layer %d,addr 0x%08x fail,unknown\n", i, addr);
			} else {
				if (layer_en)
	...
	if (pgc->force_fps_keep_count && pgc->force_fps_skip_count) {
		...
		if (g_keep == pgc->force_fps_keep_count) {
			...
			while (g_skip != pgc->force_fps_skip_count) {
				...
				if (ret == -2) {
					DISPERR("vsync for primary display path not enabled yet\n");
					return -1;
				} else if (ret == 0) {
...
static int trigger_decouple_mirror(void)
{
	if (pgc->need_trigger_dcMirror_out == 0) {
		/* DISPERR("There is no output config when decouple mirror!!\n"); */
	} else {
...
static int primary_display_trigger_nolock(int blocking, void *callback, int need_merge)
{
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE ||
		...
		if (pgc->need_trigger_ovl1to2 == 0) {
			DISPERR("There is no output config when directlink mirror!!\n");
		} else {
...
void primary_display_set_round_corner_layers(struct disp_ddp_path_config *cfg, int enable)
{
	int ret = 0;
	...
	int offset = disp_helper_get_option(DISP_OPT_ROUND_CORNER_OFFSET) ? 100 : 0;
	/* by chip */
	int phy_layer_idx = disp_helper_get_option(DISP_OPT_RPO) ? 3 : 1;  /* the layer index in its ovl module */
	int phy_layer_id = PRIMARY_SESSION_INPUT_LAYER_COUNT;
	int ext_layer_id = phy_layer_id + 1;
	enum DISP_MODULE_ENUM module = disp_helper_get_option(DISP_OPT_RPO) ?
...
static int _convert_input_to_rsz(struct disp_ddp_path_config *pconfig,
			struct disp_input_config *rsz_layer_cfg)
{
	struct RSZ_CONFIG_STRUCT *rsz_config = &pconfig->rsz_config;

	enum RSZ_COLOR_FORMAT rsz_cfmt = UNKNOWN_RSZ_CFMT;

	pconfig->hrt_scale_h = rsz_layer_cfg->tgt_height * 1000 / rsz_layer_cfg->src_height;
	pconfig->hrt_scale_w = rsz_layer_cfg->tgt_width * 1000 / rsz_layer_cfg->src_width;
	/* update frame in/out and ratio */
	rsz_config->frm_in_h = pconfig->dst_h * rsz_layer_cfg->src_height / rsz_layer_cfg->tgt_height;
	rsz_config->frm_in_w = pconfig->dst_w * rsz_layer_cfg->src_width / rsz_layer_cfg->tgt_width;
	...
	DISPINFO("rsz layer info : dwh(%d,%d),swh(%d,%d), rsz: dwh(%d,%d),swh(%d,%d)\n",
		rsz_layer_cfg->tgt_width, rsz_layer_cfg->tgt_height,
		rsz_layer_cfg->src_width, rsz_layer_cfg->src_height,
...
static int primary_display_config_rsz(disp_path_handle disp_handle,
	struct disp_ddp_path_config *pconfig, struct disp_frame_cfg_t *cfg, struct cmdqRecStruct *cmdq_handle)
{
	int i = 0;
	int rsz_layer_id = -1;
	unsigned int hrt_path_type = 0;
	bool rsz_enable = false;

	if (!cfg->overlap_layer_num) {
		DISPMSG("No have hrt info!\n");
		return -1;
	}
	rsz_layer_id = HRT_GET_SCALE_SCENARIO(cfg->overlap_layer_num);
	hrt_path_type = HRT_GET_PATH_SCENARIO(cfg->overlap_layer_num);
	pconfig->hrt_path = HRT_GET_PATH_RSZ_TYPE(hrt_path_type);
	rsz_enable = pconfig->hrt_path == HRT_PATH_RSZ_PARTIAL ? true : false;
	pconfig->rsz_enable = rsz_enable;

	DISPINFO("ovlerlap layer num: 0x%x, rsz_en:%d\n", cfg->overlap_layer_num, rsz_enable);
#if 0
	if (pconfig->rsz_enable != rsz_enable) {
		pconfig->rsz_enable = rsz_enable;
		dpmgr_path_switch_rpo(disp_handle, cmdq_handle != NULL, rsz_enable);
	}
#endif
	if (!rsz_enable)
		return 0;
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		struct disp_input_config *in_cfg = &cfg->input_cfg[i];

		/* cover rsz info */
		if (rsz_layer_id == in_cfg->layer_id) {
			_convert_input_to_rsz(pconfig, in_cfg);
			break;
		}
	}
	return 0;
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	if (disp_helper_get_option(DISP_OPT_RPO)) {
		primary_display_config_rsz(disp_handle, data_config, cfg, cmdq_handle);

		if (disp_partial_is_support() && data_config->rsz_enable)
			assign_full_lcm_roi(&total_dirty_roi);
	}

#ifdef CONFIG_MTK_ROUND_CORNER_SUPPORT
	if (disp_helper_get_option(DISP_OPT_ROUND_CORNER))
		primary_display_set_round_corner_layers(data_config, 1);
	else
		primary_display_set_round_corner_layers(data_config, 0);
#endif
