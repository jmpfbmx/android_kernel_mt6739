		      ...
		      struct golden_setting_context *p_golden_setting)
{
	...
	unsigned int fifo_valid_size = 256;
	...
	temp = (long long)rdma_golden_setting->rdma_width * rdma_golden_setting->rdma_height * Bytes_per_sec;
	do_div(temp, 16);
	temp -= 1;
	...
	sodi_threshold_low = DIV_ROUND_UP(sodi_threshold_low, 10000/* 1000 * 10 */);
...
void rdma_sodi_mode(enum DISP_MODULE_ENUM module, int forbid_pdmode)
{
	if (module == DISP_MODULE_RDMA0) {
		if (forbid_pdmode) {
			DISP_REG_SET_FIELD(NULL, RDMA0_PD_MODE_MASK,
				DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 1);
			DISP_REG_SET_FIELD(NULL, RDMA0_PD_MODE_VALUE,
				DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 1);
		...
		if (forbid_pdmode) {
			DISP_REG_SET_FIELD(NULL, RDMA1_PD_MODE_MASK,
				DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 1);
			DISP_REG_SET_FIELD(NULL, RDMA1_PD_MODE_VALUE,
				DISP_REG_CONFIG_MMSYS_SODI_REQ_MASK, 1);
