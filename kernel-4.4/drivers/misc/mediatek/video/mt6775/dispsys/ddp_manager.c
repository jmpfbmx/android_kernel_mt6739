...
static bool ddp_valid_engine[DISP_MODULE_NUM] = {
	...
	0, /* DISP_MODULE_RSZ */
	0, /* DISP_MODULE_RSZ_VIRTUAL0 */
	0, /* DISP_MODULE_RSZ_VIRTUAL1 */
...
{
	ddp_scenario_support_rpo(use_rsz);
	return 0;
}
int dpmgr_path_switch_rpo(disp_path_handle dp_handle, int encmdq, int use_rsz)
{
	enum DISP_MODULE_ENUM rsz_m = DISP_MODULE_RSZ;
	struct ddp_path_handle *handle;
	struct cmdqRecStruct *cmdq_handle;

	ASSERT(dp_handle != NULL);
	handle = (struct ddp_path_handle *)dp_handle;
	cmdq_handle = encmdq ? handle->cmdqhandle : NULL;

	if ((ddp_get_module_driver(rsz_m) != 0)) {
		if (use_rsz && (ddp_get_module_driver(rsz_m)->start != 0))
			ddp_get_module_driver(rsz_m)->start(rsz_m, cmdq_handle);

		/* turn off clk in the cmdq callback */
		if (!use_rsz && ddp_get_module_driver(rsz_m)->stop != 0)
			ddp_get_module_driver(rsz_m)->stop(rsz_m, cmdq_handle);
	}

	dpmgr_path_disconnect(dp_handle, cmdq_handle != NULL);
	ddp_scenario_support_rpo(use_rsz);
	dpmgr_path_connect(dp_handle, cmdq_handle != NULL);
	return 0;
}
