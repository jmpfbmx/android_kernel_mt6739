...
enum DSI_STATUS DSI_DumpRegisters(enum DISP_MODULE_ENUM module, int level)
{
	...
	u32 k = 0;
	unsigned long mipi_base_addr = 0;
#endif
	unsigned long module_base = module;

	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		module_base = ddp_get_module_va(module);
...
#ifndef CONFIG_FPGA_EARLY_PORTING
		mipi_base_addr = (unsigned long)DSI_PHY_REG[i];
...
static enum DSI_STATUS DSI_Reset(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		/* For reset DPHY FIFO read/write pointer purpose */
		/* DSI_OUTREGBIT(cmdq, struct DSI_COM_CTRL_REG, DSI_REG[i]->DSI_COM_CTRL, DPHY_RESET, 1); */
		/* DSI_OUTREGBIT(cmdq, struct DSI_COM_CTRL_REG, DSI_REG[i]->DSI_COM_CTRL, DPHY_RESET, 0); */
	}
			    ...
			    LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_MEM_CONTI, DSI_WMEM_CONTI);
		/* enable dsi DSI receives the redundant input when underrun */
		DSI_OUTREG32(cmdq, DISPSYS_DSI0_BASE + i * 0x1000 + DISP_REG_DSI_INPUT_SETTING, 1);
	}
	return DSI_STATUS_OK;
}
...
static void _DSI_PHY_clk_setting(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	unsigned int pdelta1 = 0;
	enum MIPITX_PAD_VALUE pad_mapping[MIPITX_PHY_LANE_NUM] = {PAD_D0P_V, PAD_D1P_V,
...
int ddp_dsi_trigger(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	DISP_REG_SET_FIELD(cmdq, MISC_FLD_RDMA_VDE_IGNORE_SPLIT_EMPTY,
...
int ddp_dsi_power_on(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	/* To avoid write pointer in DPHY FIFO write over read pointer. */
	/* MIPITX_OUTREGBIT(DSI_PHY_REG[0] + MIPITX_SW_CTRL_CON4, REG_FLD(1, 8), 1); */
...
void dsi_analysis(enum DISP_MODULE_ENUM module)
{
	...
	unsigned long dsi_base_addr = 0;

	DDPDUMP("== DISP DSI ANALYSIS ==\n");
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		DSI_REG[0] = (struct DSI_REGS *)DISPSYS_DSI0_BASE;
		dsi_base_addr = (unsigned long)DSI_REG[i];
#ifndef CONFIG_FPGA_EARLY_PORTING
