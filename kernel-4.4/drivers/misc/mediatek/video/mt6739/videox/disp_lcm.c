...
//Add by llf for device_info node begain
...
//Add by llf for device_info node end
...
//Add by llf for device_info node begain
//Modify by liulongfang for task5198699 begain
static ssize_t LCM_info_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	char lcm_name[60]="u5aplus_ili9806e_fwvga_dsi_vdo_holitech_sm5109_drv";
	char retstr[60]="NA:NA:NA";
	char *ret=NULL;
	...
	if(explcm!=NULL){
		...
		printk("[LLF_LCM] the plcm is NULL!!!!!\n");
	}

	ret=strcpy(lcm_name,plcm->drv->name);

	//sprintf( lcm_name,"%s",plcm->drv->name);
	printk("[LLF]ictype :-------lcm_name:%s ----------\n",lcm_name);
	if (!strcmp(lcm_name, "u5aplus_ili9806e_fwvga_dsi_vdo_holitech_sm5109_drv")) {
		ret=strcpy(retstr,"ILI9806E:HOLITECH:480x960");			
                                                  //u5aplus_jd9161_fwvga_dsi_vdo_skyworth_lcm_drv
	}else if (!strcmp(lcm_name, "u5aplus_jd9161_fwvga_dsi_vdo_skyworth_drv")) {
		ret=strcpy(retstr,"JD9161:SKYWORTH:480x960");
          //    sprintf( lcm_name,"%s",plcm->drv->name);
	}else{
...
//Modify by liulongfang for task5198699 end
//Add by llf for device_info node end
...

	{
		...
		if ((lcm_id == LCM_INTERFACE_NOTDEFINED) || lcm_id == plcm->lcm_if_id) {
			...
			if(deviceinfo_lcm){
				...
				printk("[LLF] create the lcm device info node!\n");
			}
			//Add by llf for device_info node end
			return plcm;
		}
