...
enum DPI_STATUS ddp_dpi_ConfigPclk(struct cmdqRecStruct *cmdq, unsigned int clk_req, enum DPI_POLARITY polarity)
{
	...
	case DPI_CLK_480p_3D:
		{
			pr_debug("DISP/DPI DPI_CLK_480p_3D\n");
			clksrc = TVDPLL_D8;
			con1 = 0xB0213B13;	/*54M*/
			break;
		}
...
int ddp_dpi_power_on(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	pr_debug("DISP/DPI,ddp_dpi_power_on, s_isDpiPowerOn %d\n", s_isDpiPowerOn);
	if (!s_isDpiPowerOn) {
...
#endif
		if (ret > 0)
			pr_debug("DISP/DPI,power manager API return FALSE\n");
...
int ddp_dpi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	pr_debug("DISP/DPI,ddp_dpi_power_off, s_isDpiPowerOn %d\n", s_isDpiPowerOn);
	if (s_isDpiPowerOn) {
#ifndef DISABLE_CLOCK_API
		/*_BackupDPIRegisters();*/
#if defined(CONFIG_MTK_LEGACY)
...
int ddp_dpi_config(enum DISP_MODULE_ENUM module, struct disp_ddp_path_config *config, void *cmdq_handle)
{
	if (s_isDpiConfig == FALSE) {
		...
		pr_debug("DISP/DPI,ddp_dpi_config DPI status:%x, cmdq:%p\n", INREG32(&DPI_REG->STATUS), cmdq_handle);
		...
		s_isDpiConfig = TRUE;
		pr_debug("DISP/DPI,ddp_dpi_config done\n");
	}
	...
	if (s_isDpiConfig == TRUE) {
		...
		/*pr_debug("DISP/DPI,now_bg_parameters: 0x%x, cache_bg_parameter: 0x%x\n",*/
...
int ddp_dpi_reset(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	pr_debug("DISP/DPI,ddp_dpi_reset\n");
...
int ddp_dpi_trigger(enum DISP_MODULE_ENUM module, void *cmdq)
{
	if (s_isDpiStart == FALSE) {
		pr_debug("DISP/DPI,ddp_dpi_start\n");
...
{
	pr_debug("DISP/DPI,ddp_dpi_stop\n");
...
int ddp_dpi_init(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	pr_debug("DISP/DPI,ddp_dpi_init- %p\n", cmdq);
	/*_BackupDPIRegisters();*/
	...
	_Enable_Interrupt();
	pr_debug("DISP/DPI,ddp_dpi_init done %p\n", cmdq);
...
{
	pr_debug("DISP/DPI,ddp_dpi_deinit- %p\n", cmdq_handle);
