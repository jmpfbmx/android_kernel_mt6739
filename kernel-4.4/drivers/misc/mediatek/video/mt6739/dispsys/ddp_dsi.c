...
//Modify by llf for no this LDO pin
...
#else
//Modify by llf for no this LDO pin
//	utils->set_gpio_lcd_vci_bias = lcd_vci_bias_setting;
//	utils->set_gpio_lcd_vdd_bias = lcd_vdd_bias_setting;
...
int ddp_dsi_build_cmdq(enum DISP_MODULE_ENUM module, void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < 3; i++) {
			...
			/* Modify lcd ESD check by llf for task 5395134 on 20171011 begain*/
			printk("[LLF_DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			     read_data0.byte0, read_data0.byte1, read_data0.byte2,
			     read_data0.byte3);
			printk("[LLF_DSI]enter cmp check_table cmd=0x%x,count=0x%x,para_list[0]=0x%x,para_list[1]=0x%x\n",
			...
			/* Modify lcd ESD check by llf for task 5395134 on 20171011 end*/
