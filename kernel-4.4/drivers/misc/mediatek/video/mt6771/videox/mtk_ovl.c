...
static struct wake_lock mem_wk_lock;
...
static int _convert_disp_input_to_ovl(struct OVL_CONFIG_STRUCT *dst, struct disp_input_config *src)
{
	...
	if (!src || !dst) {
		DISPPR_ERROR("%s src(0x%p) or dst(0x%p) is null\n", __func__, src, dst);
		return -1;
	}
...
int ovl2mem_init(unsigned int session)
{
	...
	if (ret == 0) {
		...
		DISPPR_ERROR("config M4U Port %s to %s FAIL(ret=%d)\n",
			  ddp_get_module_name(DISP_MODULE_OVL1_2L),
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	cmdqRecFlushAsyncCallback(pgcl->cmdq_handle_config, (CmdqAsyncFlushCB)ovl2mem_callback,
...
void ovl2mem_wait_done(void)
{
	...
	while ((atomic_read(&g_trigger_ticket) - atomic_read(&g_release_ticket)) > 1) {
		dpmgr_wait_event_timeout(pgcl->dpmgr_handle, DISP_PATH_EVENT_FRAME_COMPLETE,
					 HZ / 30);
...
int ovl2mem_deinit(void)
{
	...
	if (loop_cnt >= 10)
		DISPMSG("ovl2mem_deinit loop_cnt >= 10, g_trigger_ticket=%d, g_release_ticket=%d\n",
					atomic_read(&g_trigger_ticket), atomic_read(&g_release_ticket));

	/*[SVP]switch ddp mosule to nonsec when deinit the extension path*/
	switch_module_to_nonsec(pgcl->dpmgr_handle, NULL, DISP_MODULE_NUM, __func__);
