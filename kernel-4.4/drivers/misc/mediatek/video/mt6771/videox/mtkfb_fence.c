...
#define MTKFB_FENCE_PR_ERR(fmt, arg...)	pr_err("DISP/fence error(%d):"fmt, __LINE__, ##arg)
...
unsigned int mtkfb_query_buf_mva(unsigned int session_id, unsigned int layer_id, unsigned int idx)
{
	...
	if (mva != 0x0) {
		...
		DISPPR_ERROR("can't find session(0x%x) buf, layer=%d, idx=%d, fence_idx=%d, timeline=%d, cur_idx=%d\n",
				     ...
				     unsigned int idx, unsigned int ticket)
{
	...
	if (layer_id >= DISP_SESSION_TIMELINE_COUNT) {
		DISPPR_ERROR("mtkfb_update_buf_state return MVA=0x0 mtkfb_query_buf_mva layer_id %d !!!!!!(Warning)\n",
			layer_id);
		return mva;
	}
	...
	if (layer_id != layer_info->layer_id) {
		DISPPR_ERROR("wrong layer id %d(rt), %d(in)!\n", layer_info->layer_id, layer_id);
		return 0;
	}
...
bool mtkfb_update_buf_info_new(unsigned int session_id, unsigned int mva_offset, struct disp_input_config *buf_info)
{
	...
	if (buf_info->layer_id >= DISP_SESSION_TIMELINE_COUNT) {
		DISPPR_ERROR("layer_id %d !!!!!!!!(Warning)\n", buf_info->layer_id);
		return mva;
	}
...
struct mtkfb_fence_buf_info *disp_sync_prepare_buf(struct disp_buffer_info *buf)
{
	...
	DISPPR_FENCE("P+/%s%d/L%d/id%d/fd%d/mva0x%08lx/size0x%08x/ion_fd:%d\n",
		     ...
		     buf_info->idx, buf_info->fence, buf_info->mva, buf_info->size, buf->ion_fd);
				     ...
				     unsigned long phy_addr)
{
	...
	if (layer_info->fence_idx == 0)
		/* DISPPR_ERROR("layer_info->fence_idx == 0!\n"); */
		return -2;
