...
err_out:
	DDPPR_ERR("%s failed: ovl_idx=%d, cur_phy=%d, cur_ext=%d\n", __func__, ovl_idx, cur_phy_num, cur_ext_num);
...
static int alloc_buffer_from_ion(size_t size, struct test_buf_info *buf_info)
{
	...
	if (IS_ERR(buf_info->handle)) {
		DDPPR_ERR("Fatal Error, ion_alloc for size %zu failed\n", size);
	...
	if (buf_info->buf_va == NULL) {
		DDPPR_ERR("ion_map_kernrl failed\n");
	...
	if (ion_kernel_ioctl(client, ION_CMD_MULTIMEDIA, (unsigned long)&mm_data) < 0) {
		DDPPR_ERR("ion_test_drv: Config buffer failed.\n");
		ion_free(client, handle);
		ion_client_destroy(client);
		return -1;
	}
	...
	if (buf_info->buf_mva == 0) {
		DDPPR_ERR("Fatal Error, get mva failed\n");
		ion_free(client, handle);
		ion_client_destroy(client);
		return -1;
	}
...
static int alloc_buffer_from_dma(size_t size, struct test_buf_info *buf_info)
{
	...
	if (ion_display_client == NULL) {
		DDPPR_ERR("primary capture:Fail to create ion\n");
		ret = -1;
		goto out;
	}

	ion_display_handle = disp_ion_alloc(ion_display_client,
			ION_HEAP_MULTIMEDIA_PA2MVA_MASK, buf_info->buf_pa, size_align);
	if (ret != 0) {
		DDPPR_ERR("primary capture:Fail to allocate buffer\n");
		ret = -1;
		goto out;
	}
	disp_ion_get_mva(ion_display_client, ion_display_handle, (unsigned int *)&mva, DISP_M4U_PORT_DISP_WDMA0);
...
static int test_alloc_buffer(size_t size, struct test_buf_info *buf_info)
{
	...
	if (ret)
		DDPPR_ERR("error to alloc buffer size = %lu\n", (unsigned long)size);
	return ret;
}
...
static int __maybe_unused compare_dsi_checksum(unsigned long unused)
{
	...
	if (ret) {
		DDPPR_ERR("Fail to read cksum from cmdq slot\n");
		return -1;
	}

	if (cksum_golden != cksum)
		DDPPR_ERR("%s fail, cksum=0x%08x, golden=0x%08x\n", __func__, cksum, cksum_golden);
...
static int __maybe_unused check_dsi_checksum(void)
{
	...
	if (ret) {
		DDPPR_ERR("Fail to create cmdq handle\n");
		return -1;
	}
	if (!cksum_slot) {
		ret = cmdqBackupAllocateSlot(&cksum_slot, 1);
		if (ret) {
			DDPPR_ERR("Fail to alloc cmd slot\n");
					...
					int mode, unsigned int cksum)
{
	...
	for (frame = 0; frame < frame_num; frame++) {
		...
		if (unlikely(basic_test_cancel)) {
			DDPPR_ERR("%s stop because fatal signal\n", __func__);
			break;
		}
	}
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
		...
		if (maxlayer)
			...
			DDPPR_ERR("can't set max layer to 0\n");
		...
		if (ret != 14 && ret != 1) {
			DDPPR_ERR("error to parse cmd %s, ret=%d\n", opt, ret);
			return;
		}
