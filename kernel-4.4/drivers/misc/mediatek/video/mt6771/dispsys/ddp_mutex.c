...
static struct module_map_t module_mutex_map[DISP_MODULE_NUM] = {
	...
	{DISP_MODULE_RSZ0, 22, 0},
	{DISP_MODULE_DPI, 20, 0},
	{DISP_MODULE_DBI, 25, 0},
	{DISP_MODULE_DPI_VIRTUAL, -1, 0},
			     ...
			     unsigned int *SOF_src, unsigned int *EOF_src)
{
	...
	if (dest_module == DISP_MODULE_WDMA0) {
		if (ddp_mode == DDP_VIDEO_MODE)
			DDPPR_WARN("%s: dst_mode=%s, but is video mode !!\n", __func__,
...
static int ddp_mutex_add_module(int mutex_id, enum DISP_MODULE_ENUM module, void *handle)
{
	int value = 0;
...
static int ddp_mutex_set_l(int mutex_id, int *module_list, enum DDP_MODE ddp_mode, void *handle)
{
	...
	if (mutex_id < DISP_MUTEX_DDP_FIRST || mutex_id > DISP_MUTEX_DDP_LAST) {
		DDPPR_ERR("exceed mutex max (0 ~ %d)\n", DISP_MUTEX_DDP_LAST);
		return -1;
	}
...
void ddp_mutex_interrupt_disable(int mutex_id, void *handle)
{
	...
	DISP_REG_MASK(handle, DISP_REG_CONFIG_MUTEX_INTEN, 0, 0x1 << mutex_id); /* sof irq */
	DISP_REG_MASK(handle, DISP_REG_CONFIG_MUTEX_INTEN, 0, 0x1 << (mutex_id + DISP_MUTEX_TOTAL));

	DISP_REG_MASK(handle, DISP_REG_CONFIG_MUTEX_INTEN_1, 0, 0x1 << mutex_id); /* regs update irq */
	DISP_REG_MASK(handle, DISP_REG_CONFIG_MUTEX_INTEN_1, 0, 0x1 << (mutex_id + DISP_MUTEX_TOTAL));
}
...
int ddp_is_moudule_in_mutex(int mutex_id, enum DISP_MODULE_ENUM module)
{
	...
	if (module == DISP_MODULE_DSIDUAL) {
		if (1 == ((real_value >> module_mutex_map[DISP_MODULE_DSI0].bit) & 0x01)
			&& 1 == ((real_value >> module_mutex_map[DISP_MODULE_DSI1].bit) & 0x01))
			ret = 1;
	} else {
...
int ddp_mutex_set(int mutex_id, enum DDP_SCENARIO_ENUM scenario, enum DDP_MODE mode, void *handle)
{
	...
	DDPPR_ERR("Invalid scenario %d when setting mutex\n", scenario);
	return -1;
}
