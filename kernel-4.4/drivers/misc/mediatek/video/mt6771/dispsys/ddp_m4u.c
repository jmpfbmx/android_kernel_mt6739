...
int module_to_m4u_port(enum DISP_MODULE_ENUM module)
{
	...
	DDPPR_ERR("module_to_m4u_port, get m4u port fail(module=%d)\n", module);
	return M4U_PORT_NR;
}
...
int module_to_m4u_larb(enum DISP_MODULE_ENUM module)
{
	...
	DDPPR_ERR("module_to_m4u_port, get m4u larb fail(module=%d)\n", module);
	return M4U_PORT_NR;
}
...
enum DISP_MODULE_ENUM m4u_port_to_module(int port)
{
	...
	DDPPR_ERR("m4u_port_to_module, unknown port=%d\n", port);
...
void disp_m4u_init(void)
{
	...
	if (disp_helper_get_option(DISP_OPT_USE_M4U)) {
		...
		for (i = 0; i < ARRAY_SIZE(module_to_m4u_port_mapping); i++) {
#ifdef CONFIG_MTK_IOMMU
			mtk_iommu_register_fault_callback(module_to_m4u_port_mapping[i].port,
				(mtk_iommu_fault_callback_t *)disp_m4u_callback, 0);
...
enum m4u_callback_ret disp_m4u_callback(int port, unsigned long mva, void *data)
{
...
int disp_mva_map_kernel(enum DISP_MODULE_ENUM module, unsigned int mva, unsigned int size,
...
int disp_mva_unmap_kernel(unsigned int mva, unsigned int size, unsigned long map_va)
{
#ifdef CONFIG_MTK_IOMMU
			...
			unsigned int heap_id_mask, size_t align, unsigned int size)
{
	...
	if (IS_ERR(disp_handle)) {
		DDPPR_ERR("disp_ion_alloc 1error %p\n", disp_handle);
		DDPPR_ERR("disp_ion_alloc 2error %p\n", disp_handle);
		DDPPR_ERR("disp_ion_alloc 3error %p\n", disp_handle);
		DDPPR_ERR("disp_ion_alloc 4error %p\n", disp_handle);
		return NULL;
	}
				      ...
				      unsigned int *mva, int port)
{
	...
	if (ion_kernel_ioctl(client, ION_CMD_MULTIMEDIA, (unsigned long)&mm_data) < 0) {
		DDPPR_ERR("disp_ion_get_mva: config buffer failed.%p -%p\n", client, handle);
