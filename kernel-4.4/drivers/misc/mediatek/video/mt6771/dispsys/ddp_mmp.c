...
void init_ddp_mmp_events(void)
{
	if (DDP_MMP_Events.DDP == 0) {
		...
		DDP_MMP_Events.hrt = mmprofile_register_event(DDP_MMP_Events.DDP, "hrt");
		DDP_MMP_Events.dvfs =
		...
		DDP_MMP_Events.primary_switch_fps =
			    mmprofile_register_event(DDP_MMP_Events.primary_Parent, "switch_fps");
		DDP_MMP_Events.primary_mode[DISP_SESSION_DIRECT_LINK_MODE] =
			mmprofile_register_event(DDP_MMP_Events.primary_switch_mode, "directlink");
		DDP_MMP_Events.primary_mode[DISP_SESSION_DECOUPLE_MODE] =
			mmprofile_register_event(DDP_MMP_Events.primary_switch_mode, "decouple");
		DDP_MMP_Events.primary_mode[DISP_SESSION_DIRECT_LINK_MIRROR_MODE] =
			mmprofile_register_event(DDP_MMP_Events.primary_switch_mode, "DL_mirror");
		...
		DDP_MMP_Events.primary_seq_info =
		    mmprofile_register_event(DDP_MMP_Events.primary_Parent, "seq_info");
		DDP_MMP_Events.primary_seq_insert =
		    mmprofile_register_event(DDP_MMP_Events.primary_seq_info, "seq_insert");
		DDP_MMP_Events.primary_seq_config =
		    mmprofile_register_event(DDP_MMP_Events.primary_seq_info, "seq_config");
		DDP_MMP_Events.primary_seq_trigger =
		    mmprofile_register_event(DDP_MMP_Events.primary_seq_info, "seq_trigger");
		DDP_MMP_Events.primary_seq_rdma_irq =
		    mmprofile_register_event(DDP_MMP_Events.primary_seq_info, "seq_rdma_irq");
		DDP_MMP_Events.primary_seq_release =
		    mmprofile_register_event(DDP_MMP_Events.primary_seq_info, "seq_release");
		...
		DDP_MMP_Events.svp_module[DISP_MODULE_OVL0] =
			mmprofile_register_event(DDP_MMP_Events.sec, "ovl0_sec");
		DDP_MMP_Events.svp_module[DISP_MODULE_OVL1_2L] =
			mmprofile_register_event(DDP_MMP_Events.sec, "ovl1_2L_sec");
		DDP_MMP_Events.svp_module[DISP_MODULE_RDMA0] =
			mmprofile_register_event(DDP_MMP_Events.sec, "rdma0_sec");
		DDP_MMP_Events.svp_module[DISP_MODULE_WDMA0] =
			mmprofile_register_event(DDP_MMP_Events.sec, "wdma0_sec");
		...
		DDP_MMP_Events.primary_pm_qos =
		    mmprofile_register_event(DDP_MMP_Events.primary_Parent, "primary_pm_qos");
		       ...
		       unsigned int session /*1:primary, 2:external, 3:memory */)
{
	...
	if (pLayer->layer_en) {
		...
		case UFMT_YVYU:
			...
			break;
		default:
			DDPPR_ERR("ddp_mmp_ovl_layer(), unknown fmt=0x%x,dump raw\n", pLayer->fmt);
			raw = 1;
			break;
		}
		...
		if (!raw) {
			...
			if (yuv == 0) {
				if (disp_mva_map_kernel(module, pLayer->addr,
							...
							&Bitmap.data_size) == 0) {
					if (session == 1)
						mmprofile_log_meta_bitmap(DDP_MMP_Events.layer_dump[pLayer->layer],
					...
					DDPPR_ERR("ddp_mmp_ovl_layer(),fail to dump rgb(0x%x)\n", pLayer->fmt);
				}
			} else {
				...
				if (m4u_mva_map_kernel(pLayer->addr, Bitmap.data_size,
						       ...
						       &Bitmap.data_size) == 0) {
					if (session == 1)
						mmprofile_log_meta_yuv_bitmap(DDP_MMP_Events.layer_dump[pLayer->layer],
									      MMPROFILE_FLAG_PULSE, &Bitmap);
					else if (session == 2 || session == 3)
					...
					DDPPR_ERR("ddp_mmp_ovl_layer(),fail to dump yuv(0x%x)\n", pLayer->fmt);
				}
#endif
			}
		} else {
			...
			if (disp_mva_map_kernel(module, pLayer->addr, meta.size,
				...
				DDPPR_ERR("ddp_mmp_ovl_layer(),fail to dump raw(0x%x)\n", pLayer->fmt);
			}
		}
	}
			...
			unsigned int down_sample_x, unsigned int down_sample_y)
{
	...
	case UFMT_YVYU:
		...
		break;
	default:
		DDPPR_ERR("dprec_mmp_dump_wdma_layer(), unknown fmt=%d, dump raw\n",
	...
	if (!raw) {
		...
		Bitmap.down_sample_x = down_sample_x;
		Bitmap.down_sample_y = down_sample_y;
		if (disp_mva_map_kernel(DISP_MODULE_WDMA0, wdma_layer->dstAddress,
					Bitmap.data_size, (unsigned long *)&Bitmap.p_data,
					&Bitmap.data_size) == 0) {
			mmprofile_log_meta_bitmap(DDP_MMP_Events.wdma_dump[wdma_num],
						  MMPROFILE_FLAG_PULSE, &Bitmap);
			disp_mva_unmap_kernel(wdma_layer->dstAddress, Bitmap.data_size,
					     (unsigned long)Bitmap.p_data);
		} else {
			DDPPR_ERR("dprec_mmp_dump_wdma_layer(),fail to dump rgb(0x%x)\n",
			       wdma_layer->outputFormat);
		}
	} else {
		meta.data_type = MMPROFILE_META_RAW;
		...
		if (disp_mva_map_kernel(DISP_MODULE_WDMA0, wdma_layer->dstAddress,
					...
					&meta.size) == 0) {
			mmprofile_log_meta(DDP_MMP_Events.wdma_dump[wdma_num],
					   MMPROFILE_FLAG_PULSE, &meta);
			disp_mva_unmap_kernel(wdma_layer->dstAddress, Bitmap.data_size,
					     (unsigned long)Bitmap.p_data);
		} else {
			DDPPR_ERR("dprec_mmp_dump_wdma_layer(),fail to dump raw(0x%x)\n",
			       wdma_layer->outputFormat);
		}
	}
}
			...
			unsigned int down_sample_x, unsigned int down_sample_y)
{
	...
	if (rdma_num > 1) {
		DDPPR_ERR("dump_rdma_layer is error %d\n", rdma_num);
		return;
	}
	...
	case UFMT_YVYU:
		...
		break;
	default:
		DDPPR_ERR("dump_rdma_layer(), unknown fmt=%d, dump raw\n", rdma_layer->inputFormat);
		raw = 1;
		break;
	}
	...
	if (!raw) {
		...
		Bitmap.down_sample_x = down_sample_x;
		Bitmap.down_sample_y = down_sample_y;
		if (disp_mva_map_kernel(module, rdma_layer->address, Bitmap.data_size,
				       (unsigned long *)&Bitmap.p_data, &Bitmap.data_size) == 0) {
			mmprofile_log_meta_bitmap(DDP_MMP_Events.rdma_dump[rdma_num],
						  MMPROFILE_FLAG_PULSE, &Bitmap);
			disp_mva_unmap_kernel(rdma_layer->address, Bitmap.data_size,
					     (unsigned long)Bitmap.p_data);
		} else {
			DDPPR_ERR("dump_rdma_layer(),fail to dump rgb(0x%x)\n",
			       rdma_layer->inputFormat);
		}
	} else {
		meta.data_type = MMPROFILE_META_RAW;
		...
		if (disp_mva_map_kernel(module, rdma_layer->address, meta.size,
				       (unsigned long *)(&meta.p_data), &meta.size) == 0) {
			mmprofile_log_meta(DDP_MMP_Events.rdma_dump[rdma_num],
					   MMPROFILE_FLAG_PULSE, &meta);
			disp_mva_unmap_kernel(rdma_layer->address, meta.size,
					     (unsigned long)meta.p_data);
		} else {
			DDPPR_ERR("dprec_mmp_dump_rdma_layer(),fail to dump raw(0x%x)\n",
			       rdma_layer->inputFormat);
		}
	}
}
