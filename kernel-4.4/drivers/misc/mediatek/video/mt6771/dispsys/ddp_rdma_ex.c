...
static inline unsigned long rdma_to_cmdq_engine(enum DISP_MODULE_ENUM module)
{
	...
	default:
		DDPPR_ERR("invalid rdma module=%d,rdma to cmdq engine fail\n", module);
		ASSERT(0);
...
int rdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON) & 0x700) == 0x100) {
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPPR_ERR("rdma%d_reset timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON) & 0x700) != 0x100) {
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPPR_ERR("rdma%d_reset timeout, stage 2! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			       idx, DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON));
			break;
		}
	}
		      ...
		      struct golden_setting_context *p_golden_setting)
{
	...
	if (idx == 0) {
		...
		consume_rate = consume_rate * rdma_golden_setting->ext_dst_height
				* frame_rate * Bytes_per_sec;
	...
	issue_req_threshold = min(fifo_valid_size - preultra_high, (unsigned int)255);
	...
	/* SODI threshold */
	sodi_threshold_low = (ultra_low_us * 10 + fifo_off_spm) * consume_rate_div;
...
/* set ultra registers */
void rdma_set_ultra_l(unsigned int idx, unsigned int bpp, void *handle,
		      golden_setting_context *p_golden_setting)
{
	unsigned int regval;
...
static int rdma_config(enum DISP_MODULE_ENUM module, enum RDMA_MODE mode,
		       ...
		       unsigned ufoe_enable, enum DISP_BUFFER_TYPE sec,
		       ...
		       struct golden_setting_context *p_golden_setting, unsigned int bpp)
{
	...
	if (sec != DISP_SECURE_BUFFER) {
		...
		m4u_port = idx == 0 ? DISP_M4U_PORT_DISP_RDMA0 : DISP_M4U_PORT_DISP_RDMA1;
		/*
...
void rdma_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("(0x10c)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT1 + DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP("(0x110)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT2 + DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP("(0x114)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT3 + DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP("(0x118)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT4 + DISP_RDMA_INDEX_OFFSET * idx));
	DDPDUMP("(0x11c)0x%x\n",
		DISP_REG_GET(DISP_REG_RDMA_DBG_OUT5 + DISP_RDMA_INDEX_OFFSET * idx));
...
static int do_rdma_config_l(enum DISP_MODULE_ENUM module, struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (mode == RDMA_MODE_DIRECT_LINK && r_config->security != DISP_NORMAL_BUFFER)
		DDPPR_ERR("%s: rdma directlink BUT is sec ??!!\n", __func__);
...
static inline int rdma_switch_to_sec(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	if (rdma_is_sec[rdma_idx] == 0) {
		...
		/* mmprofile_log_ex(ddp_mmp_get_events()->svp_module[module], MMPROFILE_FLAG_PULSE, rdma_idx, 1); */
	}
	rdma_is_sec[rdma_idx] = 1;
...
int rdma_switch_to_nonsec(enum DISP_MODULE_ENUM module, struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (rdma_is_sec[rdma_idx] == 1) {
		...
		cmdqRecSetSecure(nonsec_switch_handle, 1);
		/*
		 * ugly workaround by kzhang !! will remove when cmdq delete disable scenario.
		 * To avoid translation fault like ovl (see notes in ovl.c)
		...
		/* mmprofile_log_ex(ddp_mmp_get_events()->svp_module[module], MMPROFILE_FLAG_PULSE, rdma_idx, 0); */
	}
...
void rdma_set_color_matrix(enum DISP_MODULE_ENUM module, struct rdma_color_matrix *matrix,
