...
void init_tplay_handle(struct device *dev)
{
	...
	if (va != NULL)
		...
		DDPPR_ERR("[SVP] failed to allocate handle_pa\n");
...
static enum mc_result late_open_mobicore_device(void)
{
	...
	if (opened_device == 0) {
		...
		if (mcRet == MC_DRV_ERR_INVALID_OPERATION) {
			...
			DDPPR_ERR("mc_open_device failed: %d @%s line %d\n",
	...
	do {
		...
		mcRet =
		    mc_malloc_wsm(mc_deviceId, 0, sizeof(tplay_tciMessage_t),
				  (uint8_t **) &pTplayTci, 0);
		if (mcRet != MC_DRV_OK) {
			DDPPR_ERR("mc_malloc_wsm failed: %d @%s line %d\n", mcRet, __func__, __LINE__);
			return -1;
		}
		...
		if (mcRet != MC_DRV_OK) {
			DDPPR_ERR("mc_open_session failed: %d @%s line %d\n", mcRet, __func__, __LINE__);
...
static int close_tplay_driver_connection(void)
{
	...
	if (tplaySessionHandle.session_id != 0)	{
		...
		if (mcRet != MC_DRV_OK) {
			DDPPR_ERR("mc_close_session failed: %d @%s line %d\n",
			       mcRet, __func__, __LINE__);
	...
	if (mcRet != MC_DRV_OK) {
		DDPPR_ERR("mc_free_wsm failed: %d @%s line %d\n", mcRet, __func__, __LINE__);
		return -1;
	}
...
static int set_tplay_handle_addr_request(void)
{
	...
	if (tplaySessionHandle.session_id == 0) {
		DDPPR_ERR("[SVP] invalid tplay session\n");
		return -1;
	}
	...
	if (mcRet != MC_DRV_OK) {
		DDPPR_ERR("[SVP] mc_notify failed: %d @%s line %d\n", mcRet, __func__, __LINE__);
		ret = -1;
	...
	if (mcRet != MC_DRV_OK) {
		DDPPR_ERR("[SVP] mc_wait_notification failed: %d @%s line %d\n", mcRet, __func__,
		       __LINE__);
		ret = -1;
...
static long disp_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == DISP_IOCTL_SET_TPLAY_HANDLE) {
		...
		if (copy_from_user(&value, (void *)arg, sizeof(unsigned int))) {
			DDPPR_ERR("DISP_IOCTL_SET_TPLAY_HANDLE, copy_from_user failed\n");
			return -EFAULT;
		}
