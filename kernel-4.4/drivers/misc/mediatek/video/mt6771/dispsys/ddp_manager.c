...
static int module_power_off(enum DISP_MODULE_ENUM module)
{
	...
	if (ddp_get_module_driver(module) != 0) {
		if (ddp_get_module_driver(module)->power_off != 0)
			ddp_get_module_driver(module)->power_off(module, NULL);	/* now just 0; */
	}
	return 0;
}
...
static int module_power_on(enum DISP_MODULE_ENUM module)
{
	...
	if (ddp_get_module_driver(module) != 0) {
		if (ddp_get_module_driver(module)->power_on != 0)
			ddp_get_module_driver(module)->power_on(module, NULL);	/* now just 0; */
	}
	return 0;
}
...
disp_path_handle dpmgr_create_path(enum DDP_SCENARIO_ENUM scenario, struct cmdqRecStruct *cmdq_handle)
{
	...
	if (path_handle != NULL) {
		...
		DDPDBG(" scenario %s include module: ", ddp_get_scenario_name(scenario));
		...
		DDPPR_ERR("Fail to create handle on scenario %s\n",
			   ddp_get_scenario_name(scenario));
	}
	return path_handle;
}
...
static int _dpmgr_path_connect(enum DDP_SCENARIO_ENUM scenario, void *handle)
{
	...
	for (i = 0; i < module_num; i++) {
		module = modules[i];
		if (ddp_get_module_driver(module) && ddp_get_module_driver(module)->connect) {
			int prev = i == 0 ? DISP_MODULE_UNKNOWN : modules[i - 1];
			int next = i == module_num - 1 ? DISP_MODULE_UNKNOWN : modules[i + 1];

			ddp_get_module_driver(module)->connect(module, prev, next, 1, handle);
		}
	}
...
int dpmgr_path_add_memout(disp_path_handle dp_handle, enum DISP_MODULE_ENUM engine, void *cmdq_handle)
{
	...
	if (ddp_is_module_in_scenario(handle->scenario, wdma) == 1) {
		DDPPR_ERR("dpmgr_path_add_memout error, wdma is already in scenario=%s\n",
		       ddp_get_scenario_name(handle->scenario));
		return -1;
	}
	/* update contxt */
...
int dpmgr_path_remove_memout(disp_path_handle dp_handle, void *cmdq_handle)
{
	...
	if (ddp_is_module_in_scenario(handle->scenario, wdma) == 0) {
		DDPPR_ERR("dpmgr_path_remove_memout error, wdma is not in scenario=%s\n",
		       ddp_get_scenario_name(handle->scenario));
		return -1;
	}
	/* update contxt */
...
int dpmgr_path_deinit(disp_path_handle dp_handle, int encmdq)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->deinit != 0)
				ddp_get_module_driver(module_name)->deinit(module_name, cmdqHandle);

			if (ddp_get_module_driver(module_name)->set_listener != 0)
				ddp_get_module_driver(module_name)->set_listener(module_name, NULL);
		}
	}
...
int dpmgr_path_stop(disp_path_handle dp_handle, int encmdq)
{
	...
	for (i = module_num - 1; i >= 0; i--) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->stop != 0)
				ddp_get_module_driver(module_name)->stop(module_name, cmdqHandle);
		}
	}
	return 0;
}
	...
	for (i = module_num - 1; i >= 0; i--) {
		...
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->ioctl != 0) {
				ret += ddp_get_module_driver(module_name)->ioctl(module_name, cmdq_handle,
									      ioctl_cmd, params);
			}
		}
	}
...
int dpmgr_path_enable_irq(disp_path_handle dp_handle, void *cmdq_handle, enum DDP_IRQ_LEVEL irq_level)
{
	...
	for (i = module_num - 1; i >= 0; i--) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) && ddp_get_module_driver(module_name)->enable_irq) {
			DDPDBG("scenario %s, module %s enable irq level %d\n",
			       ddp_get_scenario_name(handle->scenario),
			       ddp_get_module_name(module_name), irq_level);
			ret += ddp_get_module_driver(module_name)->enable_irq(module_name, cmdq_handle,
									      irq_level);
		}
	}
	return ret;
...
int dpmgr_path_reset(disp_path_handle dp_handle, int encmdq)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->reset != 0) {
				ret = ddp_get_module_driver(module_name)->reset(module_name, cmdqHandle);
				if (ret != 0)
					error++;
			}
		}
...
int dpmgr_path_config(disp_path_handle dp_handle, struct disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->config != 0) {
				ddp_get_module_driver(module_name)->config(module_name, config,
									cmdq_handle);
			}

			if (disp_helper_get_option(DISP_OPT_BYPASS_PQ)
			    && dpmgr_is_PQ(module_name) == 1) {
				if (ddp_get_module_driver(module_name)->bypass != NULL)
					ddp_get_module_driver(module_name)->bypass(module_name, 1);
			}
		}
	}
	return 0;
...
void dpmgr_get_input_address(disp_path_handle dp_handle, unsigned long *addr)
{
	...
	if (modules[0] == DISP_MODULE_OVL0 || modules[0] == DISP_MODULE_OVL1_2L)
			  ...
			  enum CMDQ_STATE state, int reverse)
{
	...
	if (reverse) {
		for (i = module_num - 1; i >= 0; i--) {
			module_name = modules[i];
			if (ddp_get_module_driver(module_name) &&
			    ddp_get_module_driver(module_name)->build_cmdq) {
				ret = ddp_get_module_driver(module_name)->build_cmdq(module_name,
										  trigger_loop_handle,
										  state);
			}
		}
...
int dpmgr_path_flush(disp_path_handle dp_handle, int encmdq)
{
	...
	return ddp_mutex_enable(handle->hwmutexid, handle->scenario, handle->mode, cmdqHandle);
}
...
int dpmgr_path_user_cmd(disp_path_handle dp_handle, int msg, unsigned long arg, void *cmdqhandle)
{
	...
	ASSERT(dp_handle != NULL);
	handle = (struct ddp_path_handle *)dp_handle;
	/* DDPPR_WARN("dpmgr_path_user_cmd msg 0x%08x\n",msg); */
	switch (msg) {
	...
	case DISP_IOCTL_AAL_SET_PARAM:
		...
		if (is_module_in_path(DISP_MODULE_AAL0, handle)) {
			if (ddp_get_module_driver(DISP_MODULE_AAL0)->cmd != NULL)
				ret =
				    ddp_get_module_driver(DISP_MODULE_AAL0)->cmd(DISP_MODULE_AAL0, msg,
									     arg, cmdqhandle);
	...
	case DISP_IOCTL_SET_GAMMALUT:
		if (ddp_get_module_driver(DISP_MODULE_GAMMA0)->cmd != NULL)
			ret =
			    ddp_get_module_driver(DISP_MODULE_GAMMA0)->cmd(DISP_MODULE_GAMMA0, msg, arg,
								       cmdqhandle);
		break;
	...
	case DISP_IOCTL_READ_SW_REG:
		...
		break;
	default:
		DDPPR_WARN("dpmgr_path_user_cmd io not supported\n");
		break;
	}
	return ret;
}
...
int dpmgr_path_is_busy(disp_path_handle dp_handle)
{
	...
	for (i = module_num - 1; i >= 0; i--) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) != 0) {
			if (ddp_get_module_driver(module_name)->is_busy != 0) {
				if (ddp_get_module_driver(module_name)->is_busy(module_name)) {
...
int dpmgr_set_lcm_utils(disp_path_handle dp_handle, void *lcm_drv)
{
	...
	for (i = 0; i < module_num; i++) {
		module_name = modules[i];
		if (ddp_get_module_driver(module_name) != 0) {
			if ((ddp_get_module_driver(module_name)->set_lcm_utils != 0) && lcm_drv) {
				DDPDBG("%s set lcm utils\n", ddp_get_module_name(module_name));
				ddp_get_module_driver(module_name)->set_lcm_utils(module_name, lcm_drv);
			}
		}
	}
	return 0;
...
int dpmgr_map_event_to_irq(disp_path_handle dp_handle, enum DISP_PATH_EVENT event, enum DDP_IRQ_BIT irq_bit)
{
	...
	DDPPR_ERR("fail to map event %s to irq 0x%x on scenario %s\n", path_event_name(event),
		   irq_bit, ddp_get_scenario_name(handle->scenario));
	return -1;
}
...
int dpmgr_wait_event_timeout(disp_path_handle dp_handle, enum DISP_PATH_EVENT event, int timeout)
{
	...
	if (wq_handle->init) {
		...
		ret = wait_event_interruptible_timeout(wq_handle->wq, cur_time < wq_handle->data,
						       timeout);
		if (ret == 0) {
			DDPPR_ERR("wait %s timeout on scenario %s\n", path_event_name(event),
				   ddp_get_scenario_name(handle->scenario));
			/* dpmgr_check_status(dp_handle); */
		} else if (ret < 0) {
			DDPPR_ERR("wait %s interrupt by other timeleft %d on scenario %s\n",
	...
	DDPPR_ERR("wait event %s not initialized on scenario %s\n", path_event_name(event),
		   ddp_get_scenario_name(handle->scenario));
	return ret;
}
...
int _dpmgr_wait_event(disp_path_handle dp_handle, enum DISP_PATH_EVENT event, unsigned long long *event_ts)
{
	...
	if (ret < 0) {
		DDPPR_ERR("wait %s interrupt by other ret %d on scenario %s\n",
			   path_event_name(event), ret,
			   ddp_get_scenario_name(handle->scenario));
	}
	if (event_ts)
...
int dpmgr_path_dsi_power_off(disp_path_handle dp_handle, void *cmdqhandle)
{
	...
	if (ddp_get_module_driver(dst_module) != 0) {
		if (ddp_get_module_driver(dst_module)->power_off != 0) {
			DDPDBG(" %s power off\n", ddp_get_module_name(dst_module));
			ddp_get_module_driver(dst_module)->power_off(dst_module, cmdqhandle);
		}
	}
	return 0;
}
...
int dpmgr_path_dsi_power_on(disp_path_handle dp_handle, void *cmdqhandle)
{
	...
	module_name = dst_module;
	if (ddp_get_module_driver(dst_module) != 0) {
		if (ddp_get_module_driver(dst_module)->power_on != 0) {
			DDPDBG(" %s power on\n", ddp_get_module_name(dst_module));
			ddp_get_module_driver(dst_module)->power_on(dst_module, cmdqhandle);
		}
	}
	return 0;
}
