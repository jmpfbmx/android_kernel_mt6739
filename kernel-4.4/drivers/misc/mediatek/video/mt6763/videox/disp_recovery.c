...
int primary_display_esd_check(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_rdlcm, MMPROFILE_FLAG_START, 0,
		       primary_display_cmdq_enabled());

	if (primary_display_cmdq_enabled() == 0) {
		DISPERR("[ESD]not support cpu read do esd check\n");
...
int primary_display_esd_recovery(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_recovery_t, MMPROFILE_FLAG_PULSE, 0, 0xff);
...
int external_display_switch_esd_mode(int mode)
{
	...
	if (node == NULL) {
		DISPERR("[EXTD-ESD][%s] can't find DSI_TE eint compatible node\n", __func__);
		return ret;
	}
...
static int external_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		...
		if (extd_esd_check_enable) {
			...
			if (ret == 1) {
				DISPERR("[EXTD-ESD]after esd recovery %d times, still fail, disable esd check\n",
					esd_try_cnt);
