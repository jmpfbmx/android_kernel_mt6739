...
/*** begin: added by liuhe.zhong for close backlight when lcd is not connected on 2018.01.29 ***/
extern unsigned char openbacklight;
/*** end:   added by liuhe.zhong for close backlight when lcd is not connected on 2018.01.29 ***/
	...
	if (old_pwm != level_1024 || atomic_cmpxchg(&g_pwm_is_change_state[index], 1, 0) == 1) {
		...
		if (old_pwm == 0 || level_1024 == 0 || abs_diff > 64) {
			...
			if (old_pwm != level_1024) {
				...
				PWM_NOTICE("[LLF_backlight]disp_pwm_set_backlight_cmdq(id = 0x%x, level_1024 = %d), old = %d",
...
/*** begin: modified by liuhe.zhong for close backlight when lcd is not connected on 2018.01.29 ***/
		if ((level_1024 > 0) && openbacklight)
/*** end:   modified by liuhe.zhong for close backlight when lcd is not connected on 2018.01.29 ***/
	...
	/* Print information if backlight is changed */
	//Modify by longfang.liu for task 5658038
	//printk("[LLF_backlight]disp_pwm_set_backlight_cmdq(id = 0x%x, level_1024 = %d)",id, level_1024);
	return 0;
}
...
static int ddp_pwm_power_on(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	//printk("[LLF_backlight] ddp_pwm_power_on (id = 0x%x",id);
	...
	//Begin deleted by shiqiang.chen.hz for FR5597245 on 20171210, for the abnromal on&off of disp_pwm
	//Add by liulongfang for backlight control for task 5424727 on 2017.10.25
	//ret =disp_pwm_set_backlight_cmdq(id, 408, NULL);
	//End deleted by shiqiang.chen.hz for FR5597245 on 20171210, for the abnromal on&off of disp_pwm
...
static int ddp_pwm_power_off(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	//Begin modified by shiqiang.chen.hz for FR5597245 on 20171210 , for the abnromal on&off of disp_pwm
	//disp_pwm_backlight_status(id, 0);
	//printk("[LLF_backlight] ddp_pwm_power_off (id = 0x%x",id);
	disp_pwm_backlight_status(id, 1);
	//End modified by shiqiang.chen.hz for FR5597245 on 20171210, for the abnromal on&off of disp_pwm
	...
	//Begin deleted by shiqiang.chen.hz for FR5597245 on 20171210 , for the abnromal on&off of disp_pwm
	//Add by liulongfang for backlight control for task 5424727 on 2017.10.25
	//ret =disp_pwm_set_backlight_cmdq(id, 0, NULL);
	//End deleted by shiqiang.chen.hz for FR5597245 on 20171210 , for the abnromal on&off of disp_pwm
