...
{
	unsigned long ovl_addr;

	ovl_addr = ovl_base_addr(module);
	DDPDBG("%s clock_on\n", ddp_get_module_name(module));
#ifdef ENABLE_CLK_MGR
	ddp_clk_prepare_enable(ddp_get_module_clk_id(module));
#endif
	/* this register is used to prevent pixel overflow*/
	DISP_REG_SET(NULL, DISP_REG_OVL_DUMMY_REG + ovl_addr, 0x80000000);
	return 0;
}
