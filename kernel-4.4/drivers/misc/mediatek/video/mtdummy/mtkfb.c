...
static int mtkfb_check_var(struct fb_var_screeninfo *var, struct fb_info *fbi)
{
	...
	pr_warn("mtkfb_check_var, xres=%u, yres=%u, xres_virtual=%u, yres_virtual=%u, xoffset=%u, yoffset=%u, bits_per_pixel=%u)\n",
	...
	if (bpp == 16) {
		...
		if (!(var->green.offset == 8)) {
			pr_err("mtkfb_check_var: green.offset=%d\n", var->green.offset);
			return -1;
		}
		if (!(var->red.offset + var->blue.offset == 16)) {
			pr_err("mtkfb_check_var: red.offset=%d, blue.offset=%d\n",
			       var->green.offset, var->blue.offset);
			return -1;
		}
		if (!(var->red.offset == 16 || var->red.offset == 0)) {
			pr_err("mtkfb_check_var: red.offset=%d\n",
			       var->green.offset);
			return -1;
		}
	} else if (bpp == 32) {
		...
		if (!(var->green.offset == 8 && var->transp.offset == 24)) {
			pr_err("mtkfb_check_var: green.offset=%d, transp.offset=%d\n",
			       var->green.offset, var->transp.offset);
			return -1;
		}
		if (!(var->red.offset + var->blue.offset == 16)) {
			pr_err("mtkfb_check_var: red.offset=%d, blue.offset=%d\n",
