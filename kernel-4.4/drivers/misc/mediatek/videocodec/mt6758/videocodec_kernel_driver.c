...
static struct clk *clk_MT_SCP_SYS_MM0;          /* SCP_SYS_MM0 */
...
{
	/* MODULE_MFV_PR_ERR ("@@ timeOld(%d, %d), timeNew(%d, %d)", */
...
void SendDvfsRequest(int level)
{
	...
	if (ret != 0) {
		...
		MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] OOPS: mmdvfs_set_fine_step error!");
	}
}
...
{
	MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] VdecDVFS monitor %dms, decoded %d frames\n",
		 MONITOR_DURATION_MS,
		 gMMDFVFSMonitorCounts);
	MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] total time %d, max duration %d, target lv %d\n",
		 gHWLockInterval,
		 gHWLockMaxDuration,
		 level);
...
void VdecDvfsAdjustment(void)
{
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE && gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
		...
		if (_monitor_duration < MONITOR_DURATION_MS) {
			...
			MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] DROP_PERCENTAGE = %d, RAISE_PERCENTAGE = %d\n",
				 DROP_PERCENTAGE, RAISE_PERCENTAGE);
			MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] reset monitor duration (%d ms), percent: %d\n",
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE) {
		...
		if (gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
			if (gFirstDvfsLock == VAL_TRUE) {
				...
				/* MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] LOCK 1 start monitor instance = 0x%p\n", */
...
void vdec_power_on(void)
{
	...
	ret = clk_prepare_enable(clk_MT_SCP_SYS_MM0);
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] clk_MT_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_SMI_COMMON is not enabled, ret = %d\n",
		ret);
		}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_GALS_VDEC2MM is not enabled, ret = %d\n",
		ret);
		}
#endif
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_VDEC is not enabled, ret = %d\n", ret);
	}
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][venc_power_on] clk_MT_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][venc_power_on] clk_MT_CG_SMI_COMMON is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][venc_power_on] clk_MT_CG_GALS_VENC2MM is not enabled, ret = %d\n",
		ret);
	}
#endif
...
static long vcodec_alloc_non_cache_buffer(unsigned long arg)
{
	...
	MODULE_MFV_PR_ERR("VCODEC_ALLOC_NON_CACHE_BUFFER + tid = %d\n", current->pid);
...
static long vcodec_free_non_cache_buffer(unsigned long arg)
{
	...
	MODULE_MFV_PR_ERR("VCODEC_FREE_NON_CACHE_BUFFER + tid = %d\n", current->pid);
	...
	if (FirstUseEncHW != 1) {
		...
		if (grVcodecEncHWLock.pvHandle == 0) {
			...
			if (gLockTimeOutCount > 30) {
				MODULE_MFV_PR_ERR("[ERROR] VCODEC_LOCKHW - ID %d fail\n", current->pid);
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (DecHWLockEvent.u4TimeoutMs == 1) {
				MODULE_MFV_PR_ERR("VCODEC_LOCKHW, First Use Dec HW!!\n");
				FirstUseDecHW = 1;
			} else {
				FirstUseDecHW = 0;
			}
			...
			if (grVcodecDecHWLock.pvHandle ==
				...
				MODULE_MFV_PR_ERR("may cause lock HW timeout!! instance = 0x%lx, CurrentTID = %d\n",
			...
			if (grVcodecDecHWLock.pvHandle == 0) { /* No one holds dec hw lock now */
#if USE_WAKELOCK == 1
				MODULE_MFV_LOGD("wake_lock(&vcodec_wake_lock) +");
				...
				while (is_entering_suspend == 1) {
					...
					if (suspend_block_cnt > 100000) {
						...
						MODULE_MFV_LOGD("VCODEC_LOCKHW blocked by suspend flow for long time");
						suspend_block_cnt = 0;
					}
					msleep(1);
				}
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (EncHWLockEvent.u4TimeoutMs == 1) {
				MODULE_MFV_PR_ERR("VCODEC_LOCKHW, First Use Enc HW %d!!\n", rHWLock.eDriverType);
				FirstUseEncHW = 1;
			} else {
				FirstUseEncHW = 0;
			}
			...
			if (grVcodecEncHWLock.pvHandle ==
				...
				MODULE_MFV_PR_ERR("may cause lock HW timeout!! instance=0x%lx, CurrentTID=%d, type:%d\n",
			...
			if (grVcodecEncHWLock.pvHandle == 0) { /* No process use HW, so current process can use HW */
				if (rHWLock.eDriverType == VAL_DRIVER_TYPE_H264_ENC ||
					...
					rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC) {
#if USE_WAKELOCK == 1
					MODULE_MFV_LOGD("wake_lock(&vcodec_wake_lock2) +");
				...
				if (gLockTimeOutCount > 30) {
					MODULE_MFV_PR_ERR("[ERROR] VCODEC_LOCKHW %d fail,someone locked HW over 30 times\n",
						 current->pid);
					MODULE_MFV_PR_ERR("without timeout 0x%lx,%lx,0x%lx,type:%d\n",
						 (VAL_ULONG_T)grVcodecEncHWLock.pvHandle,
						 pmem_user_v2p_video((VAL_ULONG_T)rHWLock.pvHandle),
						 (VAL_ULONG_T)rHWLock.pvHandle,
						 rHWLock.eDriverType);
...
static long vcodec_unlockhw(unsigned long arg)
{
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		if (grVcodecDecHWLock.pvHandle == (VAL_VOID_T *)pmem_user_v2p_video((VAL_ULONG_T)rHWLock.pvHandle)) {
			...
			MODULE_MFV_PR_ERR("[ERROR] VCODEC_UNLOCKHW\n");
...
#if USE_WAKELOCK == 1
		MODULE_MFV_LOGD("wake_unlock(&vcodec_wake_lock) +");
...
#if USE_WAKELOCK == 1
		MODULE_MFV_LOGD("wake_unlock(&vcodec_wake_lock2) +");
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{
		/* MODULE_MFV_PR_ERR("VCODEC_SET_THREAD_ID [EMPTY] + tid = %d\n", current->pid); */
		/* MODULE_MFV_PR_ERR("VCODEC_SET_THREAD_ID [EMPTY] - tid = %d\n", current->pid); */
	}
	break;
...
#ifdef ENABLE_MMDVFS_VDEC
		/* MM DVFS related */
		/* MODULE_MFV_PR_ERR("[VCODEC][MMDVFS_VDEC] INC_DEC_EMI MM DVFS init\n"); */
		/* raise voltage */
	...
	case VCODEC_INC_ENC_EMI_USER:
	{
		...
		gu4EncEMICounter++;
		MODULE_MFV_PR_ERR("[VCODEC] ENC_EMI_USER = %d\n", gu4EncEMICounter);
		user_data_addr = (VAL_UINT8_T *)arg;
	...
	case VCODEC_INC_PWR_USER:
	{
		...
		if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			if (gu4L2CCounter == 1) {
				...
				MODULE_MFV_PR_ERR("[VCODEC] Switch L2C size to 512K successful\n");
	...
	case VCODEC_DEC_PWR_USER:
	{
		...
		if (rPowerParam.eDriverType == VAL_DRIVER_TYPE_H264_ENC) {
			...
			if (gu4L2CCounter == 0) {
				...
				MODULE_MFV_PR_ERR("[VCODEC] Switch L2C size to 0K successful\n");
			}
		}
#endif
		mutex_unlock(&L2CLock);
	...
	{
		MODULE_MFV_PR_ERR("VCODEC_INITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
		MODULE_MFV_PR_ERR("VCODEC_INITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
	}
	break;
	...
	{
		MODULE_MFV_PR_ERR("VCODEC_DEINITHWLOCK [EMPTY] + - tid = %d\n", current->pid);
		MODULE_MFV_PR_ERR("VCODEC_DEINITHWLOCK [EMPTY] - - tid = %d\n", current->pid);
	}
	break;
	...
	{
		MODULE_MFV_PR_ERR("VCODEC_SET_CPU_OPP_LIMIT [EMPTY] + - tid = %d\n", current->pid);
		user_data_addr = (VAL_UINT8_T *)arg;
		...
		MODULE_MFV_PR_ERR("+VCODEC_SET_CPU_OPP_LIMIT (%d, %d, %d), tid = %d\n",
		 ...
		 * }
		 */
		MODULE_MFV_PR_ERR("-VCODEC_SET_CPU_OPP_LIMIT tid = %d, ret = %lu\n", current->pid, ret);
		MODULE_MFV_PR_ERR("VCODEC_SET_CPU_OPP_LIMIT [EMPTY] - - tid = %d\n", current->pid);
	}
	break;
...
static long vcodec_unlocked_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_UNLOCKHW:
	{
		struct OMPAT_VAL_HW_LOCK_T __user *data32;
...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	mutex_lock(&DriverOpenCountLock);
	MODULE_MFV_PR_ERR("vcodec_release pid = %d, Driver_Open_Count %d\n", current->pid, Driver_Open_Count);
	Driver_Open_Count--;
	...
	if (Driver_Open_Count == 0) {
		...
		mutex_lock(&L2CLock);
		if (gu4L2CCounter != 0) {
			MODULE_MFV_PR_ERR("vcodec_flush pid = %d, L2 user = %d, force restore L2 settings\n",
...
static int vcodec_suspend(struct platform_device *pDev, pm_message_t state)
{
	if (grVcodecDecHWLock.pvHandle != 0 || grVcodecEncHWLock.pvHandle != 0) {
		MODULE_MFV_LOGD("vcodec_suspend fail due to videocodec active");
		return -EBUSY;
	}
	MODULE_MFV_LOGD("vcodec_suspend ok");
	return 0;
}

static int vcodec_resume(struct platform_device *pDev)
{
	MODULE_MFV_LOGD("vcodec_resume ok");
	return 0;
}
...
static int vcodec_suspend_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	MODULE_MFV_LOGD("vcodec_suspend_notifier ok action = %ld", action);
	switch (action) {
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (IS_ERR(clk_MT_CG_SMI_COMMON)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_SMI_COMMON\n");
	...
	if (IS_ERR(clk_MT_CG_GALS_VDEC2MM)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_GALS_VDEC2MM\n");
	...
	if (IS_ERR(clk_MT_CG_GALS_VENC2MM)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_GALS_VENC2MM\n");
	...
	if (IS_ERR(clk_MT_CG_VDEC)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VDEC\n");
	...
	clk_MT_SCP_SYS_MM0 = devm_clk_get(&dev->dev, "MT_SCP_SYS_MM0");
	if (IS_ERR(clk_MT_SCP_SYS_MM0)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_SCP_SYS_MM0\n");
		return PTR_ERR(clk_MT_SCP_SYS_MM0);
	}
#if USE_WAKELOCK == 0
...
static int __init vcodec_driver_init(void)
{
	...
	MODULE_MFV_LOGD("wake_lock_init(&vcodec_wake_lock, WAKE_LOCK_SUSPEND, \"vcodec_wake_lock\")");
	wake_lock_init(&vcodec_wake_lock2, WAKE_LOCK_SUSPEND, "vcodec_wake_lock2");
	MODULE_MFV_LOGD("wake_lock_init(&vcodec_wake_lock2, WAKE_LOCK_SUSPEND, \"vcodec_wake_lock2\")");
#endif
	mutex_unlock(&DriverOpenCountLock);
...
static void __exit vcodec_driver_exit(void)
{
	...
	MODULE_MFV_LOGD("wake_lock_destroy(&vcodec_wake_lock)");
	wake_lock_destroy(&vcodec_wake_lock2);
	MODULE_MFV_LOGD("wake_lock_destroy(&vcodec_wake_lock2)");
	mutex_unlock(&DriverOpenCountLock);
#endif
