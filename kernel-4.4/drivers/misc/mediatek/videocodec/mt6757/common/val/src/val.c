...
VAL_RESULT_T eVideoWaitEvent(VAL_EVENT_T *a_prParam, VAL_UINT32_T a_u4ParamSize)
{
	...
	i4Ret = wait_event_interruptible_timeout(*pWaitQueue,
						 *((VAL_UINT8_T *) a_prParam->
						 ...
						 timeout_jiff);
	if (i4Ret == 0) {
		MODULE_MFV_PR_ERR("[VCODEC] eVideoWaitEvent timeout: %d ms", a_prParam->u4TimeoutMs);
...
VAL_RESULT_T eVideoWaitEventNoTimeout(VAL_EVENT_T *a_prParam, VAL_UINT32_T a_u4ParamSize, VAL_BOOL_T a_bKeepCond)
{
	...
	pWaitQueue = (wait_queue_head_t *) a_prParam->pvWaitQueue;
	/* MODULE_MFV_PR_DEBUG("[MFV]eVideoWaitEventNoTimeOut,a_prParam->u4TimeoutMs=%d, timeout = %ld\n",
	...
	i4Ret = wait_event_interruptible(*pWaitQueue,
					*((VAL_UINT8_T *) a_prParam->
					pvReserved) /*g_mflexvideo_interrupt_handler */);
	if (i4Ret == 0) {
		/* MODULE_MFV_PR_DEBUG("[VCODEC] eVideoWaitEvent timeout: %d ms", a_prParam->u4TimeoutMs); */
