...
{
	/* MODULE_MFV_PR_DEBUG ("@@ timeOld(%d, %d), timeNew(%d, %d)", */
...
void SendDvfsRequest(int level)
{
	...
	if (level == MMDVFS_VOLTAGE_LOW) {
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] SendDvfsRequest(MMDVFS_FINE_STEP_OPP3)");
#ifdef CONFIG_MTK_SMI_EXT
		ret = mmdvfs_set_fine_step(SMI_BWC_SCEN_VP, MMDVFS_FINE_STEP_OPP3);
#endif
	} else if (level == MMDVFS_VOLTAGE_HIGH) {
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] SendDvfsRequest(MMDVFS_FINE_STEP_OPP0)");
#ifdef CONFIG_MTK_SMI_EXT
		ret = mmdvfs_set_fine_step(SMI_BWC_SCEN_VP, MMDVFS_FINE_STEP_OPP0);
#endif
	} else if (level == DVFS_UNREQUEST) {
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] SendDvfsRequest(MMDVFS_FINE_STEP_UNREQUEST)");
#ifdef CONFIG_MTK_SMI_EXT
...
void VdecDvfsBegin(void)
{
	...
	MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] VdecDvfsBegin");
...
{
	MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] VdecDVFS monitor %dms, decoded %d frames\n",
		 MONITOR_DURATION_MS,
		 gMMDFVFSMonitorCounts);
	MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] total time %d, max duration %d, target lv %d\n",
		 gHWLockInterval,
		 gHWLockMaxDuration,
		 level);
...
void VdecDvfsAdjustment(void)
{
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE && gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
		...
		if (_monitor_duration < MONITOR_DURATION_MS) {
			_diff = VdecDvfsStep();
			MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] lock time(%d ms, %d ms), cnt=%d, _monitor_duration=%d\n",
				 _diff, gHWLockInterval, gMMDFVFSMonitorCounts, _monitor_duration);
		} else {
			VdecDvfsStep();
			_perc = (VAL_UINT32_T)(100 * gHWLockInterval / _monitor_duration);
			MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] DROP_PERCENTAGE = %d, RAISE_PERCENTAGE = %d\n",
				 DROP_PERCENTAGE, RAISE_PERCENTAGE);
			MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] reset monitor duration (%d ms), percent: %d\n",
...
void VdecDvfsMonitorStart(void)
{
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE) {
		...
		/* MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] Pause handle prev_diff = %dms, diff = %dms\n", */
		/*		gHWLockPrevInterval, _diff); */
		if (_diff > PAUSE_DETECTION_GAP && _diff > gHWLockPrevInterval * PAUSE_DETECTION_RATIO) {
			/* MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] Pause detected, reset\n"); */
	...
	if (gMMDFVFSMonitorStarts == VAL_TRUE) {
		MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] LOCK 1\n");
		if (gMMDFVFSMonitorCounts > MONITOR_START_MINUS_1) {
			if (gFirstDvfsLock == VAL_TRUE) {
				gFirstDvfsLock = VAL_FALSE;
				/* MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] LOCK 1 start monitor*/
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		MODULE_MFV_PR_ERR("[VCODEC][ERROR][venc_power_on] clk_MT_CG_VENC is not enabled, ret = %d\n", ret);
	}
#endif
#endif
...
{
	MODULE_MFV_PR_ERR("[ERROR]@1 VCODEC_LOCKHW, Dec HWLockEvent TimeOut, CurrentTID = %d\n", current->pid);
	if (FirstUseDecHW != 1) {
		mutex_lock(&HWLock);
		if (grVcodecHWLock.pvHandle == 0) {
			/* Add one line comment for avoid kernel coding style, WARNING:BRACES: */
...
static long vcodec_lockhw(unsigned long arg)
{
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (bLockedHW == VAL_TRUE) {
				MODULE_MFV_PR_DEBUG("VCODEC_LOCKHW, Enc Lock ok grVcodecHWLock.pvHandle = 0x%lx, va:%lx, type:%d\n",
					 (VAL_ULONG_T)grVcodecHWLock.pvHandle,
					 (VAL_ULONG_T)rHWLock.pvHandle,
					 rHWLock.eDriverType);
				gLockTimeOutCount = 0;
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		u4VcodecSel = 0x0;
		MODULE_MFV_PR_ERR("[WARNING] Unknown driver type\n");
	}
...
#ifdef ENABLE_MMDVFS_VDEC
		/* MM DVFS related */
		/* MODULE_MFV_PR_DEBUG("[VCODEC][MMDVFS_VDEC] INC_DEC_EMI MM DVFS init\n"); */
		/* raise voltage */
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (IS_ERR(clk_MT_CG_VENC)) {
		MODULE_MFV_PR_ERR("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VENC\n");
...
MODULE_DESCRIPTION("Zion Vcodec Driver");
MODULE_LICENSE("GPL");
