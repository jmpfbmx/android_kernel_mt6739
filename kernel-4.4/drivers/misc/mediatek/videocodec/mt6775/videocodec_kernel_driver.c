...
static struct clk *clk_MT_CG_GALS_COMM0;   /* CLK_MM_GALS_COMM0 */
static struct clk *clk_MT_CG_GALS_COMM1;   /* CLK_MM_GALS_COMM1 */
...
static struct clk *clk_MT_CG_VENC;         /* VENC_VENC */
...
static VAL_UINT32_T gVDECFreq[2] = {499, 320};
static VAL_UINT32_T gVDECFrmTRAVC[4] = {4, 6, 10, 3}; /* /3 for real ratio */
static VAL_UINT32_T gVDECFrmTRHEVC[4] = {3, 5, 13, 3};
static VAL_UINT32_T gVDECFrmTRVP9[5] = {4, 13, 13, 20, 3}; /* 3rd element for VP mode */
...
void vdec_power_on(void)
{
	...
	if (ret) {
		...
		pr_debug("[VCODEC][ERROR][vdec_power_on] clk_MT_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}

#if defined(CONFIG_MTK_SMI_EXT)
	smi_bus_enable(SMI_LARB_VDECSYS, "VDEC");
#else
	ret = clk_prepare_enable(clk_MT_CG_GALS_COMM0);
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_GALS_COMM0 is not enabled, ret = %d\n",
		ret);
	}

	ret = clk_prepare_enable(clk_MT_CG_GALS_COMM1);
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_GALS_COMM1 is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_SMI_COMMON is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][vdec_power_on] clk_MT_CG_GALS_VDEC2MM is not enabled, ret = %d\n",
		ret);
	}
#endif
...
void vdec_power_off(void)
{
	...
	if (gu4VdecPWRCounter == 0) {
		...
		clk_disable_unprepare(clk_MT_CG_GALS_COMM1);
		clk_disable_unprepare(clk_MT_CG_GALS_COMM0);
#endif
...
void venc_power_on(void)
{
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][venc_power_on] clk_MT_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][venc_power_on] clk_MT_CG_GALS_COMM0 is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][venc_power_on] clk_MT_CG_GALS_COMM1 is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][venc_power_on] clk_MT_CG_SMI_COMMON is not enabled, ret = %d\n",
		ret);
	}
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][venc_power_on] clk_MT_CG_GALS_VENC2MM is not enabled, ret = %d\n",
		ret);
	}
#endif
	...
	if (ret) {
		/* print error log & error handling */
		pr_debug("[VCODEC][ERROR][venc_power_on] clk_MT_CG_VENC is not enabled, ret = %d\n", ret);
	}
	...
	if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
		...
		while (bLockedHW == VAL_FALSE) {
			...
			if (grVcodecDecHWLock.pvHandle == 0) { /* No one holds dec hw lock now */
				...
				QOS_DEBUG("cur_job freq %llu", target_freq_64);
				mutex_unlock(&VdecDVFSLock);
#endif
#ifdef CONFIG_PM
...
static long vcodec_set_frame_info(unsigned long arg)
{
	...
	if (rFrameInfo.driver_type == VAL_DRIVER_TYPE_H264_DEC ||
		...
		rFrameInfo.driver_type == VAL_DRIVER_TYPE_VP9_DEC) {
		mutex_lock(&DecPMQoSLock);
		...
		if (gVDECBWRequested == 0) {
			...
			case VAL_DRIVER_TYPE_HEVC_DEC:
				...
				break;
			case VAL_DRIVER_TYPE_VP9_DEC:
				emi_bw = emi_bw * gVDECFrmTRVP9[frame_type] /
					(2 * 3 * gVDECFreq[b_freq_idx]);
				QOS_DEBUG("[PMQoS Kernel] VP9 frame_type %d, ratio %d, VDec freq %d emi_bw %ld",
				frame_type, gVDECFrmTRVP9[frame_type], gVDECFreq[gVDECLevel], emi_bw);
				break;
		...
		rFrameInfo.driver_type == VAL_DRIVER_TYPE_HEVC_ENC) {
		mutex_lock(&EncPMQoSLock);
		{
...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{
		pr_debug("[WARNING] VCODEC_ALLOC_NON_CACHE_BUFFER empty!\n");
	}
	break;

	case VCODEC_FREE_NON_CACHE_BUFFER:
	{
		pr_debug("[WARNING] VCODEC_FREE_NON_CACHE_BUFFER empty\n");
	}
	break;
	...
	{
		pr_debug("[WARNING] VCODEC_INC_PWR_USER empty\n");
	}
	break;

	case VCODEC_DEC_PWR_USER:
	{
		pr_debug("[WARNING] VCODEC_DEC_PWR_USER empty\n");
	}
	break;
	...
	{
		pr_debug("[WARNING] VCODEC_INITHWLOCK empty\n");
	}
	break;

	case VCODEC_DEINITHWLOCK:
	{
		pr_debug("[WARNING] VCODEC_DEINITHWLOCK empty\n");
	}
	break;

	case VCODEC_GET_CPU_LOADING_INFO:
	{
		pr_debug("[WARNING] VCODEC_GET_CPU_LOADING_INFO empty\n");
	}
	break;

	case VCODEC_GET_CORE_LOADING:
	{
		pr_debug("[WARNING] VCODEC_GET_CORE_LOADING empty\n");
	}
	break;
	...
	{
		pr_debug("[WARNING] VCODEC_SET_CPU_OPP_LIMIT empty\n");
	}
	break;
...
static int vcodec_probe(struct platform_device *dev)
{
	...
	if (IS_ERR(clk_MT_SCP_SYS_MM0)) {
		pr_debug("[VCODEC][ERROR] Unable to devm_clk_get MT_SCP_SYS_MM0\n");
		return PTR_ERR(clk_MT_SCP_SYS_MM0);
	}

#ifndef CONFIG_MTK_SMI_EXT
	clk_MT_CG_GALS_COMM0 = devm_clk_get(&dev->dev, "MT_CG_GALS_COMM0");
	if (IS_ERR(clk_MT_CG_GALS_COMM0)) {
		pr_debug("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_GALS_COMM0\n");
		return PTR_ERR(clk_MT_CG_GALS_COMM0);
	}

	clk_MT_CG_GALS_COMM1 = devm_clk_get(&dev->dev, "MT_CG_GALS_COMM1");
	if (IS_ERR(clk_MT_CG_GALS_COMM1)) {
		pr_debug("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_GALS_COMM1\n");
		return PTR_ERR(clk_MT_CG_GALS_COMM1);
	}
	...
	if (IS_ERR(clk_MT_CG_VENC)) {
		pr_debug("[VCODEC][ERROR] Unable to devm_clk_get MT_CG_VENC\n");
...
MODULE_DESCRIPTION("Canon Vcodec Driver");
MODULE_LICENSE("GPL");
