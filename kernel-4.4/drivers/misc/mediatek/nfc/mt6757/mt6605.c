...
static int mt_nfc_gpio_init(void)
{
	...
	if (node) {
		mt6605_platform_data.ven_gpio = of_get_named_gpio(node, "gpio-ven", 0);
		mt6605_platform_data.sysrstb_gpio = of_get_named_gpio(node, "gpio-rst", 0);
		mt6605_platform_data.eint_gpio = of_get_named_gpio(node, "gpio-eint", 0);
		mt6605_platform_data.irq_gpio = of_get_named_gpio(node, "gpio-irq", 0);
	} else {
