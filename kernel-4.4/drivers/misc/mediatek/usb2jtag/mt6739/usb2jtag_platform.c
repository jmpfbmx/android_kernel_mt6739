...
static int mtk_usb2jtag_hw_init(void)
{
	...
	if (!node) {
		pr_debug("[USB2JTAG] map node @ mediatek,infracfg_ao failed\n");
		return -1;
	}

	INFRACFG_AO_BASE = of_iomap(node, 0);
	if (!INFRACFG_AO_BASE) {
		pr_debug("[USB2JTAG] iomap INFRACFG_AO_BASE failed\n");
		return -1;
	}

	temp = readl(INFRACFG_AO_BASE + 0xF00) | (0x1 << 14);
	writel(temp, INFRACFG_AO_BASE + 0xF00);

	/* Init USB config */
	if (usb2jtag_usb_init() != 0) {
		pr_debug("[USB2JTAG] USB init failed\n");
		return -1;
	}
