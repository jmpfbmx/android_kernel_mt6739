...
static int __init extmem_scan_memory(unsigned long node, const char *uname, int depth, void *data)
{
	...
	if (mem_desc && mem_desc->size) {
		...
		extmem_printk("extmem_scan_memory extmem_phys_base: 0x%zx, extmem_mspace_size: 0x%zx\n",
						(size_t)extmem_phys_base, extmem_mspace_size);
	}
...
static int extmem_get_lca_reserved_mem(void)
{
	...
	/* avoid double init */
	if (extmem_phys_base) {
		pr_err("extmem_phys_base exist: 0x%zx\n", (size_t)extmem_phys_base);
		return -1;
	}
...
void __init extmem_init(void)
{
	if (extmem_mspace == NULL) {
		...
		extmem_printk("extmem_phys_base:0x%zx,extmem_mspace_size: 0x%zx,extmem_mspace:0x%p\n",
					(size_t)extmem_phys_base, extmem_mspace_size, extmem_mspace);
...
{
#ifdef CONFIG_OF
	extmem_printk("%s va: 0x%lx extmem_phys_base:0x%zx extmem_mspace_base:0x%p\n",
					__func__, va, (size_t)extmem_phys_base, extmem_mspace_base);
...
{
#ifdef CONFIG_OF
	extmem_printk("%s pa:0x%lx extmem_phys_base:0x%zx extmem_mspace_base:0x%p\n",
					__func__, pa, (size_t)extmem_phys_base, extmem_mspace_base);
