...
#ifdef PCADR
#define IT66121_plus 0x02	/* Define it66121's I2c slave Address of all pages by the status of PCADR pin. */
#else
#define IT66121_plus 0x00	/* Define it66121's I2c Address of all pages by the status of PCADR pin. */
#endif
...
static int hdmi_ite_probe(struct i2c_client *client, const struct i2c_device_id *id);
...
static struct i2c_board_info it66121_i2c_hdmi __initdata = {
	I2C_BOARD_INFO(DEVICE_NAME, (HDMI_TX_I2C_SLAVE_ADDR >> 1) + IT66121_plus)};
...
/*Declare and definition for a hdmi kthread, this thread is used to check the HDMI Status */
...
static int it66121_video_config(enum HDMI_VIDEO_RESOLUTION vformat, enum HDMI_VIDEO_INPUT_FORMAT vin,
				int vout)
{
...
static int hdmi_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mt_eint_registration(CUST_EINT_EINT_HDMI_HPD_NUM, EINTF_TRIGGER_LOW, &_it66121_irq_handler,
			     0);
	mt_eint_mask(CUST_EINT_EINT_HDMI_HPD_NUM);
...
static int hdmi_ite_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (match_id(&hdmi_ite_id[0], client)) {
		...
		dev_info(&client->adapter->dev, "attached hmdi_ite_id[0] %s into i2c adapter successfully\n",
			 id->name);
		...
		IT66121_LOG("invalid i2c adapter: can not found dev_id matched\n");
		return -EIO;
	}
...
const struct HDMI_DRIVER *HDMI_GetDriver(void)
{
	static const struct HDMI_DRIVER HDMI_DRV = {
		...
		.video_config = it66121_video_config,	/* it66121_video_config,HDMITX_SetOutput */
		.audio_config = it66121_audio_config,	/* it66121_audio_config,HDMITX_SetAudioOutput */
		.video_enable = it66121_video_enable,	/* HDMITX_EnableVideoOutput */
		.audio_enable = it66121_audio_enable,	/* HDMITX_SetAudioOutput */
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "res:", 4) == 0) {
		...
		IT66121_LOG("hdmi %d\n", res);
		it66121_video_config((enum HDMI_VIDEO_RESOLUTION)res, HDMI_VIN_FORMAT_RGB888, HDMI_VOUT_FORMAT_RGB888);
	}
...
{
	ite66121_dbgfs = debugfs_create_file("hdmi_test", S_IFREG | S_IRUGO, NULL, (void *)0, &debug_fops);
}
