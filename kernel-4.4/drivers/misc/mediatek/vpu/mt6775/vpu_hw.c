...
#ifndef MTK_VPU_FPGA_PORTING
/* regulator */
static struct regulator *buck_vimvo;
static unsigned int is_regulator_getted;
...
static struct clk *clk_ipu_core2_jtag_cg;
static struct clk *clk_ipu_core2_axi_m_cg;
static struct clk *clk_ipu_core2_ipu_cg;
static struct clk *clk_ipu_vcore_ahb_cg;
static struct clk *clk_ipu_vcore_axi_cg;
static struct clk *clk_ipu_vcore_adl_cg;
...
static struct clk *mtcmos_vpu_vcore;
static struct clk *mtcmos_vpu_vcnn;
...
static struct mutex regulator_mutex;
...
#define PREPARE_VPU_MTCMOS(clk) \
	...
	PREPARE_VPU_MTCMOS(mtcmos_vpu_vcore);
	PREPARE_VPU_MTCMOS(mtcmos_vpu_vcnn);
...
#define PREPARE_VPU_CLK(clk) \
	...
	PREPARE_VPU_CLK(clk_ipu_vcore_ahb_cg);
	PREPARE_VPU_CLK(clk_ipu_vcore_axi_cg);
	PREPARE_VPU_CLK(clk_ipu_vcore_adl_cg);
	...
	PREPARE_VPU_CLK(clk_ipu_core2_jtag_cg);
	PREPARE_VPU_CLK(clk_ipu_core2_axi_m_cg);
	PREPARE_VPU_CLK(clk_ipu_core2_ipu_cg);
	PREPARE_VPU_CLK(clk_top_dsp_sel);
	PREPARE_VPU_CLK(clk_top_dsp1_sel);
	PREPARE_VPU_CLK(clk_top_dsp2_sel);
	PREPARE_VPU_CLK(clk_top_dsp3_sel);
...
static int vpu_enable_regulator_and_clock(int core)
{
	...
	if (is_regulator_getted == 0) {
		buck_vimvo = regulator_get(vpu_dev->dev[0], "ext_buck_vpu");
		ret = IS_ERR(buck_vimvo);
		CHECK_RET("can not find regulator: vimvo\n");
	};
	is_regulator_getted++;
	mutex_unlock(&regulator_mutex);

	LOG_DBG("[vpu_%d] regulator getted	%d+\n", core, is_regulator_getted);

	ret = (buck_vimvo == NULL) ? -1 : 0;
	CHECK_RET("regulator not existed: vimvo\n");

	LOG_DBG("before setting regulator_get_voltagereg_id= %d uV\n", regulator_get_voltage(buck_vimvo));

	vpu_trace_begin("vimvo:set_voltage");
	ret = regulator_set_voltage(buck_vimvo, 800000, 1300000);
	vpu_trace_end();

	vpu_trace_begin("vimvo:enable");
	ret = regulator_enable(buck_vimvo);
	vpu_trace_end();

	LOG_INF("regulator_get_voltagereg_id= %d uV\n", regulator_get_voltage(buck_vimvo));
...
#define ENABLE_VPU_CLK(clk) \
	...
	ENABLE_VPU_CLK(clk_top_dsp3_sel);
	vpu_trace_end();
	...
	ENABLE_VPU_MTCMOS(mtcmos_vpu_vcore);
	ENABLE_VPU_MTCMOS(mtcmos_vpu_vcnn);

	ENABLE_VPU_CLK(clk_ipu_vcore_ahb_cg);
	ENABLE_VPU_CLK(clk_ipu_vcore_axi_cg);
	ENABLE_VPU_CLK(clk_ipu_vcore_adl_cg);
	...
	case 2:
		...
		ENABLE_VPU_CLK(clk_ipu_core2_jtag_cg);
		ENABLE_VPU_CLK(clk_ipu_core2_axi_m_cg);
		ENABLE_VPU_CLK(clk_ipu_core2_ipu_cg);
		break;
	}
...
#undef ENABLE_VPU_CLK
	...
	ret = vpu_set_clock_source(clk_top_dsp3_sel, opps.dspcore[1].index);
		CHECK_RET("[vpu_%d]fail to set dsp1 freq, step=%d, ret=%d\n", core, opps.dspcore[1].index, ret);
...
#define DISABLE_VPU_MTCMOS(clk) \
	...
	case 1:
		...
		break;
	case 2:
		DISABLE_VPU_CLK(clk_ipu_core2_jtag_cg);
		DISABLE_VPU_CLK(clk_ipu_core2_axi_m_cg);
		DISABLE_VPU_CLK(clk_ipu_core2_ipu_cg);
	...
	DISABLE_VPU_CLK(clk_ipu_vcore_ahb_cg);
	DISABLE_VPU_CLK(clk_ipu_vcore_axi_cg);
	DISABLE_VPU_CLK(clk_ipu_vcore_adl_cg);
	DISABLE_VPU_MTCMOS(mtcmos_vpu_vcore);
	DISABLE_VPU_MTCMOS(mtcmos_vpu_vcnn);
	...
	DISABLE_VPU_CLK(clk_top_dsp3_sel);
...
#undef DISABLE_VPU_CLK
	...
	LOG_INF("[vpu_%d] disable result vcore=%d, ddr=%d\n", core, vcorefs_get_curr_vcore(), vcorefs_get_curr_ddr());
out:
	is_power_on[core] = false;
	if (!buck_vimvo)
		LOG_INF("regulator_get reg_id failed\n");
	...
	if (is_regulator_getted == 0) {
		...
		vpu_trace_begin("vimvo:set_voltage");
		ret = regulator_set_voltage(buck_vimvo, 550000, 1300000);
		vpu_trace_end();

		vpu_trace_begin("vimvo:enable");
		ret = regulator_enable(buck_vimvo);
		vpu_trace_end();
		LOG_DBG("regulator_get_voltagereg_id= %d uV\n", regulator_get_voltage(buck_vimvo));
	}
	mutex_unlock(&regulator_mutex);
...
#define UNPREPARE_VPU_CLK(clk) \
	...
	UNPREPARE_VPU_CLK(clk_ipu_core2_jtag_cg);
	UNPREPARE_VPU_CLK(clk_ipu_core2_axi_m_cg);
	UNPREPARE_VPU_CLK(clk_ipu_core2_ipu_cg);
	UNPREPARE_VPU_CLK(clk_ipu_vcore_ahb_cg);
	UNPREPARE_VPU_CLK(clk_ipu_vcore_axi_cg);
	UNPREPARE_VPU_CLK(clk_ipu_vcore_adl_cg);
...
{
	LOG_DBG("vpu 2 received a interrupt\n");
...
static int vpu_map_mva_of_bin(int core, uint64_t bin_pa)
{
	...
	case 2:
		...
		binpa_reset_vector = bin_pa + (VPU_DDR_SHIFT_RESET_VECTOR << 1);
		binpa_main_program = binpa_reset_vector + VPU_OFFSET_MAIN_PROGRAM;
		binpa_iram_data = bin_pa + VPU_OFFSET_MAIN_PROGRAM_IMEM + (VPU_DDR_SHIFT_IRAM_DATA << 1);
		break;
	}
...
int vpu_hw_enque_request(int core, struct vpu_request *request)
{
	...
	if (ret) {
		...
		LOG_ERR("[vpu_%d] hw_enque_request timeout , status(%d/%d), ret(%d)\n", core,
			vpu_read_field(core, FLD_XTENSA_INFO00), vpu_service_cores[core].is_cmd_done, ret);
		vpu_dump_buffer_mva(request);
		vpu_dump_mesg(NULL);
		vpu_dump_register(NULL);
...
int vpu_hw_processing_request(int core, struct vpu_request *request)
{
	...
	if (need_reload) {
		...
		LOG_INF("[vpu_%d] algo_%d ptr/length (0x%lx/0x%x), bf(%d)\n", core, algo->id[core],
				(unsigned long)algo->bin_ptr, algo->bin_length, vpu_service_cores[core].is_cmd_done);
...
void vpu_dump_code_segment(int core)
{
	...
	case 1:
		...
		bin_offset = VPU_DDR_SHIFT_RESET_VECTOR + VPU_OFFSET_MAIN_PROGRAM;
		break;
	}
