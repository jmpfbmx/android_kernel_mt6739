...
/* define device tree */
#ifndef LED2601_DTNAME
#define LED2601_DTNAME "mediatek,flashlights_led2601_rear"
#endif
...
#define LED2601_NAME "flashlights-led2601-rear"
...
#define LED2601_PINCTRL_PINSTATE_LOW 0
#define LED2601_PINCTRL_PINSTATE_HIGH 1

#define LED2601_PINCTRL_STATE_FLASHLIGHT_HIGH 		"rear_flashlight_high" 
#define LED2601_PINCTRL_STATE_FLASHLIGHT_LOW  		"rear_flashlight_low"
...
static int led2601_pinctrl_init(struct platform_device *pdev)
{
	...
	led2601_flashlight_high = pinctrl_lookup_state(led2601_pinctrl, LED2601_PINCTRL_STATE_FLASHLIGHT_HIGH);
	if (IS_ERR(led2601_flashlight_high)) {
		pr_err("Failed to init (%s)\n", LED2601_PINCTRL_STATE_FLASHLIGHT_HIGH);
		ret = PTR_ERR(led2601_flashlight_high);
	}
	led2601_flashlight_low = pinctrl_lookup_state(led2601_pinctrl, LED2601_PINCTRL_STATE_FLASHLIGHT_LOW);
	if (IS_ERR(led2601_flashlight_low)) {
		pr_err("Failed to init (%s)\n", LED2601_PINCTRL_STATE_FLASHLIGHT_LOW);
...
static int led2601_pinctrl_set(int state)
{
	...
	if (state == LED2601_PINCTRL_PINSTATE_LOW && !IS_ERR(led2601_flashlight_low))
		ret = pinctrl_select_state(led2601_pinctrl, led2601_flashlight_low);
	else if (state == LED2601_PINCTRL_PINSTATE_HIGH && !IS_ERR(led2601_flashlight_high))
...
static int led2601_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	//pr_info("cmd is %d,  _IOC_NR(cmd) is %d", _IOC_NR(cmd), cmd);
		...
		struct led2601_platform_data *pdata)
{
	...
	for_each_child_of_node(np, cnp) {
		...
		if (of_property_read_u32(cnp, "part", &pdata->dev_id[i].part))
			goto err_node_put;
		snprintf(pdata->dev_id[i].name, FLASHLIGHT_NAME_SIZE, LED2601_NAME);
...
static int led2601_probe(struct platform_device *pdev)
{
	...
	if (pdata->channel_num) {
		...
		if (flashlight_dev_register(LED2601_NAME, &led2601_ops)) {
			err = -EFAULT;
			goto err;
		}
	}
...
static int led2601_remove(struct platform_device *pdev)
{
	...
	if (pdata && pdata->channel_num)
		...
		flashlight_dev_unregister(LED2601_NAME);
