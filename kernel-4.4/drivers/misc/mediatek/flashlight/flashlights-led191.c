...
#define PK_ERR(fmt, arg...)         pr_debug(TAG_NAME "%s: " fmt, __func__, ##arg)


/*#define DEBUG_LEDS_STROBE*/
#ifdef DEBUG_LEDS_STROBE
#define PK_LOG(fmt, arg...)       pr_info(TAG_NAME "%s is called.\n", __func__)
...
#ifndef LED191_DTNAME_I2C
#define LED191_DTNAME_I2C "mediatek,flashlights_led191_i2c"
#endif
...
#define LED191_PINCTRL_PIN_XXX1 1
#define LED191_PINCTRL_PINSTATE_LOW 0
#define LED191_PINCTRL_PINSTATE_HIGH 1
#define LED191_PINCTRL_STATE_HWEN_HIGH1 "hwen_high1"
#define LED191_PINCTRL_STATE_HWEN_LOW1  "hwen_low1"

static struct pinctrl *led191_pinctrl;
static struct pinctrl_state *led191_hwen_high1;
static struct pinctrl_state *led191_hwen_low1;
...
static int led191_pinctrl_init(struct platform_device *pdev)
{
	...
	led191_hwen_high1 = pinctrl_lookup_state(led191_pinctrl, LED191_PINCTRL_STATE_HWEN_HIGH1);
	if (IS_ERR(led191_hwen_high1)) {
		pr_err("Failed to init (%s)\n", LED191_PINCTRL_STATE_HWEN_HIGH1);
		ret = PTR_ERR(led191_hwen_high1);
	}
	led191_hwen_low1 = pinctrl_lookup_state(led191_pinctrl, LED191_PINCTRL_STATE_HWEN_LOW1);
	if (IS_ERR(led191_hwen_low1)) {
		pr_err("Failed to init (%s)\n", LED191_PINCTRL_STATE_HWEN_LOW1);
		ret = PTR_ERR(led191_hwen_low1);
	}
...
static int led191_pinctrl_set(int pin, int state)
{
	...
	//End modified by zhanyong.yin for XR5424731 on 2017/10/19
		...
		break;
	//Begin added by zhanyong.yin for XR5424731 on 2017/10/19
	case LED191_PINCTRL_PIN_XXX1:
		if (state == LED191_PINCTRL_PINSTATE_LOW && !IS_ERR(led191_hwen_low1))
			ret = pinctrl_select_state(led191_pinctrl, led191_hwen_low1);
		else if (state == LED191_PINCTRL_PINSTATE_HIGH && !IS_ERR(led191_hwen_high1))
			ret = pinctrl_select_state(led191_pinctrl, led191_hwen_high1);
		else
...
static int led191_enable(void)
{
	...
	if (g_flash_duty < 1) {
		...
		led191_pinctrl_set(pin1, 0);
	}
	else{
		led191_pinctrl_set(pin, 1);
		led191_pinctrl_set(pin1, 1);
	}
	//End added by zhanyong.yin for XR5424731 on 2017/10/19
	return 0;
}
