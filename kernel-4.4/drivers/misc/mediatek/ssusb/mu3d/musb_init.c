...
static int low_power_enter(struct ssusb_mtk *ssusb)
{
	 ...
	 */
	if (ssusb->str_mode != SSUSB_STR_DEEP)
...
static struct musb_hdrc_platform_data *mu3d_parse_dts(struct device_node *dn)
{
	...
	if (ret) {
		...
		goto prp_err;
	} else if (pdata->drv_mode == SSUSB_MODE_DEVICE) {
	...
	if (ret) {
		...
		goto prp_err;
	} else if (pdata->p0_vbus_mode == SSUSB_VBUS_GPIO) {
	...
	if (pdata->drv_mode == SSUSB_MODE_HOST) {
	...
	} else if (pdata->otg_mode == SSUSB_OTG_MANUAL) {
...
static int get_vbus_gpio(struct vbus_ctrl_info *vbus, const char *label)
{
	...
	if (vbus->vbus_mode != SSUSB_VBUS_GPIO)
		return 0;
...
static int get_ssusb_rscs(struct platform_device *pdev, struct ssusb_mtk *ssusb)
{
	...
	if ((pdata->drv_mode == SSUSB_MODE_DEVICE)
	    || (pdata->otg_mode == SSUSB_OTG_IDPIN)
	    || ((pdata->otg_mode == SSUSB_OTG_MANUAL) && !pdata->otg_init_as_host)) {
		/*
	...
	ssusb->is_power_saving_mode = !!(ssusb->str_mode == SSUSB_STR_NONE);
	mu3d_dbg(K_INFO, "is_power_saving_mode: %d\n", ssusb->is_power_saving_mode);

	return 0;

p1_vbus_err:
	if (p0_vbus->vbus_mode == SSUSB_VBUS_GPIO)
...
static int put_ssusb_rscs(struct ssusb_mtk *ssusb)
{
	...
	vbus = &ssusb->p1_vbus;
	if (ssusb->p1_exist && vbus->vbus_mode == SSUSB_VBUS_GPIO)
		gpio_free(vbus->vbus_gpio_num);
