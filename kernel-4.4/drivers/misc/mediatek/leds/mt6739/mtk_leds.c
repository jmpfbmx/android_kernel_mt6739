...
/*Modified by rongxiao.deng for defect 5424735 on 2017/10/21 start*/
#define PMIC_BREATH_ARRAY_NUM 16
int pmic_breath_time_array[] = {0,123,338,523,707,1107,1291,1507,1691,1876,2091,2276,2460,2676,2860,3075};// breathe mode rising falling time ms
static int find_breath_time_index_pmic(int time_ms)
{
    int i;

    for (i = 0; i < PMIC_BREATH_ARRAY_NUM; i++) {
        if (time_ms <= pmic_breath_time_array[i])
            return i;
    }
    return PMIC_BREATH_ARRAY_NUM - 1;
}
/*Modified by rongxiao.deng for defect 5424735 on 2017/10/21 end*/
...
/*Modified by rongxiao.deng for defect 5424735 on 2017/10/21 end*/
	...
	case MT65XX_LED_PMIC_NLED_ISINK1:
/*Modified by rongxiao.deng for defect 5424735 on 2017/10/21 start*/
		time_tr2 = find_breath_time_index_pmic((led->blink_on_time)/2);
		time_tf2 = time_tr2;
		time_toff = find_breath_time_index_pmic((led->blink_off_time)/2);
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN, 0);
		pmic_set_register_value(PMIC_RG_DRV_ISINK_CK_CKSEL, 0);//Added by rongxiao.deng for task 5451365 on 2017/11/27
		pmic_set_register_value(PMIC_ISINK_CH1_MODE,ISINK_BREATH_MODE);
		pmic_set_register_value(PMIC_ISINK_CH1_STEP,ISINK_3);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TR1_SEL,time_tr1);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TR2_SEL,time_tr2);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF1_SEL,time_tf1);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF2_SEL,time_tf2);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TON_SEL,time_ton);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TOFF_SEL,time_toff);
		pmic_set_register_value(PMIC_ISINK_DIM1_FSEL,11);
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK0) {
		...
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_PDN, 0);
		pmic_set_register_value(PMIC_ISINK_CH1_MODE, ISINK_PWM_MODE);
		pmic_set_register_value(PMIC_ISINK_CH1_STEP, ISINK_1);	/* Modifed by bin.song.hz for 5157125 reduce charge led current */
	...
	if (disp_aal_is_support() == true) {
		if (led_data->level != level) {
			...
			if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
				...
				mt_mt65xx_led_work(&led_data->work);//modified by hailong.chen for task 6619615 schedule_work>>>>mt_mt65xx_led_work on 2018/07/18
			} else {
				if (level != 0
