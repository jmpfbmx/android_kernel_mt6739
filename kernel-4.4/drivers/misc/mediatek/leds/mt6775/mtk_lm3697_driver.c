...
#define CPD_FUN(f)               pr_info("[ChargePump] %s\n", __func__)
#define CPD_PR_ERR(fmt, args...)    pr_err("[ChargePump] %s %d : "fmt, __func__, __LINE__, ##args)
...
unsigned int lm3697_set_brightness_level(unsigned int level_a)
{
	...
	if (ret_code != 0) {
		CPD_PR_ERR("[Cust_SetBacklight] CTL_A_LSB fail: %d\n", ret_code);
		return ret_code;
	}
	...
	if (ret_code != 0) {
		CPD_PR_ERR("[Cust_SetBacklight] CTL_A_MSB fail: %d\n", ret_code);
		return ret_code;
	}
	...
	if (ret_code != 0) {
		CPD_PR_ERR("[Cust_SetBacklight] CTL_B_LSB fail: %d\n", ret_code);
		return ret_code;
	}
	...
	if (ret_code != 0) {
		CPD_PR_ERR("[Cust_SetBacklight] CTL_B_MSB fail: %d\n", ret_code);
		return ret_code;
	}
