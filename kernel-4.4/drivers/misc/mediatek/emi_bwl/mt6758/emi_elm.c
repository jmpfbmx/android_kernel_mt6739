...
static unsigned int emi_elm_spm_dbg[2];
...
static void lt_init(void)
{
	...
	if (get_dram_type() == TYPE_LPDDR3) {
		...
		writel(0x04e209c4, IOMEM(EMI_LTCT1_2ND));
		writel(0x00000754, IOMEM(EMI_LTCT2_2ND));
		mt_reg_sync_writel(0x00714a94, EMI_LTCT3_2ND);
	}
...
static int lt_reg_save(void)
{
	...
	emi_elm_spm_dbg[0] = readl(IOMEM(mt_emi_elm_dbg_base_get()));
	emi_elm_spm_dbg[1] = readl(IOMEM(mt_emi_elm_dbg_base_get()+0x4));
...
void elm_dump(char *buf, unsigned int leng)
{
	...
	spin_lock_irqsave(&elm_lock, flags);
	snprintf(buf, leng,
		"[EMI ELM] spm_dbg_0: 0x%08x\n spm_dbg_1: 0x%08x\n"
		"ddr data rate: %d, valid: 0x%08x\n"
		"%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n"
		"%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n"
		"%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n"
		"%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n"
		...
		"%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n%s: 0x%08x\n"
		"%s: 0x%08x\n",
		emi_elm_spm_dbg[0], emi_elm_spm_dbg[1],
		dram_data_rate, elm_valid,
		"EMI_CGMA_ST0", emi_cgma_st0,
		"EMI_CGMA_ST1", emi_cgma_st1,
		"EMI_CGMA_ST2", emi_cgma_st2,
