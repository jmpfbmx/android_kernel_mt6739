...
{ 5760,  4312,  0,  0,  5760,  4312,  2880,  2156,  0000,  0000,  2880,  2156,  0,  0,  2880,  2156},/*Preview*/
{ 5760,  4312,  0,  0,  5760,  4312,  5760,  4312,  0000,  0000,  5760,  4312,  0,  0,  5760,  4312},/*capture*/
{ 5760,  4312,  0,  0,  5760,  4312,  2880,  2156,  0000,  0000,  2880,  2156,  0,  0,  2880,  2156}, /* video */
{ 5760,  4312,  0,  0,  5760,  4312,  2880,  2156,  0000,  0000,  2880,  2156,  0,  0,  2880,  2156}, /*hs video*/
{ 5760,  4312,  0,  0,  5760,  4312,  2880,  2156,  0000,  0000,  2880,  2156,  0,  0,  2880,  2156}, /* slim video*/
};
...
static void preview_setting(void)
{
	...
	LOG_INF("AEHIST_LOG 0xB73A:0x%x 0x37EC:0x%x 0x37ED:0x%x 0x37EE:0x%x 0x37EF:0x%x\n",
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0016) << 8) | read_cmos_sensor_8(0x0017));
			LOG_INF("read_0x0016=0x%x, 0x0017=0x%x,0x0016_0017=0x%x\n",
				read_cmos_sensor_8(0x0016), read_cmos_sensor_8(0x0017), read_cmos_sensor(0x0016));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				imgsensor_info.sensor_ver = read_sensor_version();
...
static void hdr_write_tri_gain(kal_uint16 lg, kal_uint16 mg, kal_uint16 sg)
{
	...
	LOG_INF("lg:0x%x, reg_lg:0x%x, sg:0x%x, reg_mg:0x%x, mg:0x%x, reg_sg:0x%x\n",
			lg, reg_lg, mg, reg_mg, sg, reg_sg);

}

static void imx576_set_lsc_reg_setting(kal_uint8 index, kal_uint16 *regDa, MUINT32 regNum)
{
	int i;
	int startAddr[4] = {0x9D88, 0x9CB0, 0x9BD8, 0x9B00}; /*0:B,1:Gb,2:Gr,3:R*/
...
static void set_imx576_ATR(kal_uint16 LimitGain, kal_uint16 LtcRate, kal_uint16 PostGain)
{
...
static kal_uint32 imx576_awb_gain(SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	UINT32 rgain_32, grgain_32, gbgain_32, bgain_32;

	LOG_INF("imx576_awb_gain\n");
	...
	case SENSOR_FEATURE_SET_LSC_TBL:
		{
			...
			imx576_set_lsc_reg_setting(index, feature_data_16, (*feature_para_len)/sizeof(UINT16));
		}
		break;
