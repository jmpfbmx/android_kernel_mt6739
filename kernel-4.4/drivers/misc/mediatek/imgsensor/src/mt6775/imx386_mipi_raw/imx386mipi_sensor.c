...
u8 *bayer_primax_otp_buf;
#define	MAX_READ_WRITE_SIZE	8
#define	OTP_DATA_SIZE	4047
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	*/
	...
	.hs_video = {
		...
		.linelength  = 4296,				/* record different mode's linelength */
		.framelength = 1144,			/* record different mode's framelength */
		.startx = 0,					/* record different mode's startx of grabwindow */
		.starty = 0,					/* record different mode's starty of grabwindow */
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[8] = {/* Preview */
...
{ 4032, 3016,	0,  380, 4032, 2256, 4032, 2256,   0,	0, 4032, 2256,	 0, 0, 4032, 2256}, /* custom3 */
};
...
{
	memcpy((void *)data, (void *)(bayer_primax_otp_buf + PDAF_CAL_DATA_OFFSET), PDAF_CAL_DATA_SIZE);
}
...
static void load_imx386_spc_data(void)
{
	...
	for (i = 0; i < SPC_DATA_SIZE; i++)
		write_cmos_sensor(0x7D4C+i, bayer_primax_otp_buf[SPC1_START_ADDR + i]);
#if 0
	for (i = 0; i < SPC_DATA_SIZE; i++)
		write_cmos_sensor(0x7D80+i, bayer_primax_otp_buf[SPC2_START_ADDR + i]);
#endif
}
...
otp_read:
	 ...
	 */
	bayer_primax_otp_buf = kzalloc(OTP_DATA_SIZE, GFP_KERNEL);

	/* read lsc calibration from E2PROM */
	imx386_read_otp_burst(OTP_START_ADDR, bayer_primax_otp_buf);

/* for (i = 0; i < OTP_DATA_SIZE; i++) { */
/* LOG_INF("========imx386_otp idx:%d val:0x%x======\n", i, *(bayer_primax_otp_buf + i)); */
/* } */
