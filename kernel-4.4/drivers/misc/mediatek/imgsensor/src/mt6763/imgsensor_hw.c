...
#if 0
enum IMGSENSOR_RETURN imgsensor_hw_power_add_sequence(struct IMGSENSOR_HW *phw, struct IMGSENSOR_HW_POWER_SEQ *ppwr_seq)
{
	...
	if((psensor_pwr - phw->sensor_pwr) > IMGSENSOR_HW_SENSOR_MAX_NUM * sizeof(struct IMGSENSOR_HW_SENSOR_POWER))
		return IMGSENSOR_RETURN_ERROR;
