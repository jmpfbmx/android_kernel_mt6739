...
**************************************************************************************************/
int s5k4h8_MAC_MSB,s5k4h8_INF_MSB,s5k4h8_MAC_LSB,s5k4h8_INF_LSB;
int s5k4h8_RG_MSB,s5k4h8_BG_MSB,s5k4h8_RG_LSB,s5k4h8_BG_LSB;
int s5k4h8_gRG_MSB,s5k4h8_gBG_MSB,s5k4h8_gRG_LSB,s5k4h8_gBG_LSB;
int S5K4H8_OTP_FLAG;
int macro=0,inf=0;
...
void Darling_S5K4H8_read_OTP(void)
{
		...
		//int addr=0;
		int checksum=0,checksum1=0,checksum_awb=0,checksum_af=0;
		int module_id=0,lens_id=0,vcm_id=0,vcm_ic_id=0,year=0,month=0,date=0;
		int rg_ratio=0;
		int bg_ratio=0;
		int g_rg_ratio=0;
		int g_bg_ratio=0;
		//int af_flag=0,af_flag1=0;
		//int checksumaf=0,checksumaf1=0;
		//int macro=0,inf=0;
	...
	{
		flag_otp = 0x01;
		checksum_awb =S5K4H8_byteread_cmos_sensor(0x0A05);
		module_id           =S5K4H8_byteread_cmos_sensor(0x0A06);
		year                      =S5K4H8_byteread_cmos_sensor(0x0A07);
		month                  =S5K4H8_byteread_cmos_sensor(0x0A08);
		date                      =S5K4H8_byteread_cmos_sensor(0x0A09);
		lens_id                 =S5K4H8_byteread_cmos_sensor(0x0A0A);
		vcm_id                 =S5K4H8_byteread_cmos_sensor(0x0A0B);
		vcm_ic_id            =S5K4H8_byteread_cmos_sensor(0x0A0C);
		rg_ratio =(S5K4H8_byteread_cmos_sensor(0x0A0E)<<8)|S5K4H8_byteread_cmos_sensor(0x0A0F);
		bg_ratio =(S5K4H8_byteread_cmos_sensor(0x0A10)<<8)|S5K4H8_byteread_cmos_sensor(0x0A11);
		s5k4h8_RG_MSB=S5K4H8_byteread_cmos_sensor(0x0A0E);
		s5k4h8_RG_LSB=S5K4H8_byteread_cmos_sensor(0x0A0F);
		s5k4h8_BG_MSB=S5K4H8_byteread_cmos_sensor(0x0A10);
		s5k4h8_BG_LSB=S5K4H8_byteread_cmos_sensor(0x0A11);
		...
		for(i=0;i<13;i++){
			checksum+=S5K4H8_byteread_cmos_sensor(0x0A06+i);
		}
	...
	{
		flag_otp = 0x01;
		checksum_awb =S5K4H8_byteread_cmos_sensor(0x0A25);
		module_id           =S5K4H8_byteread_cmos_sensor(0x0A26);
		year                      =S5K4H8_byteread_cmos_sensor(0x0A27);
		month                  =S5K4H8_byteread_cmos_sensor(0x0A28);
		date                      =S5K4H8_byteread_cmos_sensor(0x0A29);
		lens_id                 =S5K4H8_byteread_cmos_sensor(0x0A2A);
		vcm_id                 =S5K4H8_byteread_cmos_sensor(0x0A2B);
		vcm_ic_id            =S5K4H8_byteread_cmos_sensor(0x0A2C);
		rg_ratio =(S5K4H8_byteread_cmos_sensor(0x0A2E)<<8)|S5K4H8_byteread_cmos_sensor(0x0A2F);
		bg_ratio =(S5K4H8_byteread_cmos_sensor(0x0A30)<<8)|S5K4H8_byteread_cmos_sensor(0x0A31);
		s5k4h8_RG_MSB=S5K4H8_byteread_cmos_sensor(0x0A2E);
		s5k4h8_RG_LSB=S5K4H8_byteread_cmos_sensor(0x0A2F);
		s5k4h8_BG_MSB=S5K4H8_byteread_cmos_sensor(0x0A30);
		s5k4h8_BG_LSB=S5K4H8_byteread_cmos_sensor(0x0A31);
		...
		for(i=0;i<13;i++){
			checksum+=S5K4H8_byteread_cmos_sensor(0x0A26+i);
		}
		checksum=(checksum)%255+1;
		 
		}
	}
	printk("TCL s5k4h8 OTP module_id is 0x%x,lens_id is 0x%x,vcm_id is 0x%x,vcm_ic_id is 0x%x\n",module_id,lens_id,vcm_id,vcm_ic_id);
	printk("TCL s5k4h8 OTP date is %d-%d-%d\n",year,month,date);
	printk("TCL s5k4h8 OTP rg_ratio:0x%x,bg_ratio:0x%x\n",rg_ratio,bg_ratio);
	printk("TCL s5k4h8 OTP g_rg_ratio:0x%x,g_bg_ratio:0x%x\n",g_rg_ratio,g_bg_ratio);
	printk("TCL s5k4h8 OTP checksum is %d,checksum_awb is %d\n",checksum,checksum_awb);
	...
	if(val == 0x01)
	{
		printk("TCL s5k4h8 AF OTP group1 is valid\n");
		checksum_af=S5K4H8_byteread_cmos_sensor(0x0A15);
		macro=(S5K4H8_byteread_cmos_sensor(0x0A16)<<8)|(S5K4H8_byteread_cmos_sensor(0x0A17)&0xff);
		inf=(S5K4H8_byteread_cmos_sensor(0x0A18)<<8)|(S5K4H8_byteread_cmos_sensor(0x0A19)&0xff);
		s5k4h8_MAC_MSB=S5K4H8_byteread_cmos_sensor(0x0A16);
		s5k4h8_MAC_LSB=S5K4H8_byteread_cmos_sensor(0x0A17);
		s5k4h8_INF_MSB=S5K4H8_byteread_cmos_sensor(0x0A18);
		s5k4h8_INF_LSB=S5K4H8_byteread_cmos_sensor(0x0A19);
		for(i=0;i<4;i++){
			checksum1+=S5K4H8_byteread_cmos_sensor(0x0A16+i);
		}
		checksum1=(checksum1)%255+1;
		printk("TCL s5k4h8 OTP checksumaf is %d,checksum1 is %d\n",checksum_af,checksum1);
		printk("TCL s5k4h8 OTP macro is %d,inf is %d\n",macro,inf);	
	}
	else if(val == 0x00)
	{
		flag_otp += 0x00;
	...
	if(val == 0x01)
	{
		printk("TCL s5k4h8 AF OTP group2 is valid\n");
		checksum_af=S5K4H8_byteread_cmos_sensor(0x0A35);
		macro=(S5K4H8_byteread_cmos_sensor(0x0A36)<<8)|(S5K4H8_byteread_cmos_sensor(0x0A37)&0xff);
		inf=(S5K4H8_byteread_cmos_sensor(0x0A38)<<8)|(S5K4H8_byteread_cmos_sensor(0x0A39)&0xff);
		s5k4h8_MAC_MSB=S5K4H8_byteread_cmos_sensor(0x0A36);
		s5k4h8_MAC_LSB=S5K4H8_byteread_cmos_sensor(0x0A37);
		s5k4h8_INF_MSB=S5K4H8_byteread_cmos_sensor(0x0A38);
		s5k4h8_INF_LSB=S5K4H8_byteread_cmos_sensor(0x0A39);
		for(i=0;i<4;i++){
			checksum1+=S5K4H8_byteread_cmos_sensor(0x0A36+i);
		}
		checksum1=(checksum1)%255+1;
		printk("TCL s5k4h8 OTP checksumaf is %d,checksum1 is %d\n",checksum_af,checksum1);
		printk("TCL s5k4h8 OTP macro is %d,inf is %d\n",macro,inf);
	...
	if((module_id==0xc)&&(lens_id==0xb1)&&(vcm_id==0xc1)&&(vcm_ic_id==0xd)&&(checksum==checksum_awb)&&(checksum_af==checksum1)){
		S5K4H8_OTP_FLAG=1;
	}
	else{
		S5K4H8_OTP_FLAG=0;
	}
	printk("TCL s5k4h8 OTP S5K4H8_OTP_FLAG is %d\n",S5K4H8_OTP_FLAG);
...
void Darling_S5K4H8_apply_OTP(void){
	...
	S5K4H8_wordwrite_cmos_sensor(0x602A,0x0B00);
	S5K4H8_bytewrite_cmos_sensor(0x6F12,0x01);
...
//	write_cmos_sensor(0x6F12,0x0900);
	mdelay(2);
	S5K4H8_wordwrite_cmos_sensor(0x6028,0x4000);
	S5K4H8_wordwrite_cmos_sensor(0x602A,0x0100);
	S5K4H8_bytewrite_cmos_sensor(0x6F12,0x01);	
#endif
}
...
else if(offset==0x0A10)
{
	*data=s5k4h8_BG_MSB;
	data++;
	*data=s5k4h8_BG_LSB;
}
   else if(offset==0x0A12)
    {
	*data=s5k4h8_gRG_MSB;
	data++;
	*data=s5k4h8_gRG_LSB;
    }
else if(offset==0x0A14)
{
	*data=s5k4h8_gBG_MSB;
	data++;
	*data=s5k4h8_gBG_LSB;
}
else if(offset==0x0A1C)
{
	*data=s5k4h8_MAC_MSB;
	data++;
	*data=s5k4h8_MAC_LSB;
}
else if(offset==0x0A1E)
{
	*data=s5k4h8_INF_MSB;
	data++;
	*data=s5k4h8_INF_LSB;
	...
	*data=0xff;	
    ...
    SENSORDB_S5K4H8_OTP("TCL: Read CMD s5k4h8_RG_MSB=%x, s5k4h8_RG_LSB=%x\n",s5k4h8_RG_MSB,s5k4h8_RG_LSB);
     SENSORDB_S5K4H8_OTP("TCL: Read CMD s5k4h8_BG_MSB=%x, s5k4h8_BG_LSB=%x\n",s5k4h8_BG_MSB,s5k4h8_BG_LSB);
	SENSORDB_S5K4H8_OTP("TCL: Read CMD s5k4h8_gRG_MSB=%x, s5k4h8_gRG_LSB=%x\n",s5k4h8_gRG_MSB,s5k4h8_gRG_LSB);
     SENSORDB_S5K4H8_OTP("TCL: Read CMD s5k4h8_gBG_MSB=%x, s5k4h8_gBG_LSB=%x\n",s5k4h8_gBG_MSB,s5k4h8_gBG_LSB);
	SENSORDB_S5K4H8_OTP("TCL: Read CMD s5k4h8_INF_MSB=%x, s5k4h8_INF_LSB=%x\n",s5k4h8_INF_MSB,s5k4h8_INF_LSB);
     SENSORDB_S5K4H8_OTP("TCL: Read CMD s5k4h8_MAC_MSB=%x, s5k4h8_MAC_LSB=%x\n",s5k4h8_MAC_MSB,s5k4h8_MAC_LSB);
	return 0;
...
static int __init CAM_CAL_i2C_init(void)
{
	...
	SENSORDB_S5K4H8_OTP("CAM_CAL_i2C_init\n");
	i4RetValue = RegisterCAM_CALCharDrv();
	if (i4RetValue) {
		SENSORDB_S5K4H8_OTP(" register char device failed!\n");
		return i4RetValue;
	}
	SENSORDB_S5K4H8_OTP(" Attached!!\n");
