...
//#include "../../../../../cam_cal/src/common/hi846otp/hi846otp.h"
...
//	write_cmos_sensor(0x0A04, 0x014A);
	...
	write_cmos_sensor(0x0958, 0x9980); 
...
static kal_uint32 get_module_id(void)
{
	...
	group_flag = OTP_read_cmos_sensor(0x0201);
	...
	g_module_id = OTP_read_cmos_sensor(addr);
...
LOG_INF("HI846 addr = 0x%x \n", addr);
	if (addr == 0) {
		...
		HI846_otp.vcm_id = OTP_read_cmos_sensor(addr + 2);
		HI846_otp.Driver_ic_id = OTP_read_cmos_sensor(addr + 3);
		HI846_otp.prodyction_year = OTP_read_cmos_sensor(addr + 4);
		HI846_otp.production_month = OTP_read_cmos_sensor(addr + 5);
		HI846_otp.production_day = OTP_read_cmos_sensor(addr + 6);
		HI846_otp.infocheck = OTP_read_cmos_sensor(addr + 16);
	}

	checksum = (HI846_otp.module_integrator_id + HI846_otp.prodyction_year + HI846_otp.production_month + HI846_otp.production_day + HI846_otp.lens_id + HI846_otp.vcm_id + HI846_otp.Driver_ic_id) % 0xFF + 1;
	...
	LOG_INF("HI846 module_integrator_id = 0x%x, AF_FLAG = 0x%x, prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id = 0x%x, infocheck = 0x%x \n",\
		HI846_otp.module_integrator_id,HI846_otp.AF_FLAG, HI846_otp.prodyction_year, HI846_otp.production_month, HI846_otp.production_day, HI846_otp.sensor_id, HI846_otp.lens_id, HI846_otp.infocheck);
/* end, it's for basic info on 2017-12-01 */


/* begin, it's for AWB info on 2017-12-01 */
	HI846_otp.WB_FLAG = OTP_read_cmos_sensor(0x18B3);
	...
	if (wb_start_addr != 0) {
		...
		for (i = 0; i < 9; i++) {
			HI846_otp.wb_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI846 wb_data[%d] = 0x%x  ", i, HI846_otp.wb_data[i]);
		}
	}
	else{
...
/* begin, it's for AF info on 2017-12-01 */
	HI846_otp.AF_FLAG = OTP_read_cmos_sensor(0x18CF);
	LOG_INF("HI846 AF_FLAG = 0x%x\n",HI846_otp.AF_FLAG);
	if (HI846_otp.AF_FLAG == 0x01)
		...
		LOG_INF("HI846 read af data invalid \n");
		return 0 ;
	}
	
	if (af_start_addr != 0) {
		HI846_OTP_write_cmos_sensor(0x70a, (af_start_addr >> 8) & 0xff);	//start addr H
		HI846_OTP_write_cmos_sensor(0x70b, af_start_addr & 0xff);	//start addr L
		HI846_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
		for (i = 0; i < 5; i++) {
			HI846_otp.af_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI846 af_data[%d] = 0x%x  ", i, HI846_otp.af_data[i]);
		}
	}
...
int hi846_read_lsc(unsigned short *buffer,int size)
{
	...
	lsc_flag = OTP_read_cmos_sensor(0x0235);
	LOG_INF("HI846 LSC_FLAG = 0x%x\n",lsc_flag);
	if (lsc_flag == 0x01)
		...
		LOG_INF("HI846 read lsc data invalid \n");
		return 0;
	}
	//spin_lock(&imgsensor_drv_lock);
	if (lsc_start_addr != 0) {
		...
		for (i = 0; i < size; i++) {
			*buffer = read_cmos_sensor(0x708);	//otp data read
			//pr_info("846_lsc_data 0x%x , ", *buffer);
			buffer++;
		}
	}
	//spin_unlock(&imgsensor_drv_lock);
...
EXPORT_SYMBOL(hi846_read_lsc);

static int HI846_otp_check(void)
{
	int checksum = 0, i=0;
	LOG_INF("HI846 HI846_otp_check \n");
	...
	HI846_otp.wb_checksum = HI846_otp.wb_data[8];
	

	if (checksum == HI846_otp.wb_checksum)
	{
		LOG_INF("zhijun.fu HI846_otp.wb_checksum is %d, checksum is %d\n ", HI846_otp.wb_checksum,checksum);
		LOG_INF("zhijun.fu HI846_Sensor: WB checksum PASS\n ");
	}
	else{
		LOG_INF("zhijun.fu HI846_Sensor: WB checksum Fail\n ");
		LOG_INF("zhijun.fu HI846_otp.wb_checksum is %d, checksum is %d\n ", HI846_otp.wb_checksum,checksum);
		//return 0;
	}
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {				
					...
					sprintf(Back_Camera_Name,"HI846:Dmegc:8M_AF");
