...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0, 0, 1632, 1224, 0, 0, 1632, 1224}
};/* slim video */
...
#endif
static kal_uint16 ov8858_table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300) {
		ov8858_table_write_cmos_sensor(addr_data_pair_capture_30fps_ov8858,
					       sizeof(addr_data_pair_capture_30fps_ov8858) /
					       sizeof(kal_uint16));
	}
	if (currefps == 150) {
		ov8858_table_write_cmos_sensor(addr_data_pair_capture_15fps_ov8858,
					       sizeof(addr_data_pair_capture_15fps_ov8858) /
					       sizeof(kal_uint16));
	}
	if (currefps == 240) {
		ov8858_table_write_cmos_sensor(addr_data_pair_capture_24fps_ov8858,
					       sizeof(addr_data_pair_capture_24fps_ov8858) /
					       sizeof(kal_uint16));
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",
			     imgsensor.current_fps / 10, imgsensor_info.cap.max_framerate);
