...
struct gc8034_otp gc8034_otp_info = {0};
EXPORT_SYMBOL(gc8034_otp_info);
		...
		for (i = 0; i < total_number; i++) {
			if ((ddtempbuff[4 * i + 3] & 0x80) == 0x80) {
				if ((ddtempbuff[4 * i + 3] & 0x03) == 0x03) {
					gc8034_otp_info.dd_param[cnt].x = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
					gc8034_otp_info.dd_param[cnt].y = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc8034_otp_info.dd_param[cnt++].t = 2;
					gc8034_otp_info.dd_param[cnt].x = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
					gc8034_otp_info.dd_param[cnt].y = ((kal_uint16)ddtempbuff[4 * i + 2] << 4) + ((ddtempbuff[4 * i + 1] & 0xf0) >> 4) + 1;
					gc8034_otp_info.dd_param[cnt++].t = 2;
				}
				else {
					gc8034_otp_info.dd_param[cnt].x = (((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) + ddtempbuff[4 * i];
...
int gc8034_read_lsc(kal_uint8 *buffer,int size)
{
	
	int lsc_start_addr = 0, lsc_flag = 0;
	lsc_flag = gc8034_read_otp(3,0x83);//read 0x218
	
	LOG_INF("gc8034 LSC_FLAG = 0x%x\n",lsc_flag);
	
	if ( (lsc_flag|0xc) == 0x01)
		lsc_start_addr = 0x0288;
	else if ( (lsc_flag|0x3) == 0x01)
		lsc_start_addr = 0x220;  //zy
	else{
		LOG_INF("gc8034 read lsc data invalid \n");
		return 0;
	}
	//spin_lock(&imgsensor_drv_lock);
	LOG_INF("gc8034 lsc_start_addr = 0x%x\n",lsc_start_addr);
	
	if (lsc_start_addr == 0x0288) 
	{
	//static void gc8034_read_otp_group(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, int size)
		gc8034_read_otp_group(6, 0x51, buffer, 60);
		gc8034_read_otp_group(7, 0x00, buffer, 128);
		gc8034_read_otp_group(8, 0x00, buffer, 128);
		gc8034_read_otp_group(9, 0x00, buffer, 81);
	}
	else if(lsc_start_addr == 0x0220)
	{
		gc8034_read_otp_group(3, 0x44, buffer, 60);
		gc8034_read_otp_group(4, 0x00, buffer, 128);
		gc8034_read_otp_group(5, 0x00, buffer, 128);
		gc8034_read_otp_group(6, 0x00, buffer, 81);
	}
	else
		LOG_INF("gc8034 gc8034_read_lsc fail\n");
	/*{
		for (i = 0; i < size; i++) {
			if( i<60 )
				*buffer = gc8034_read_otp(6,lsc_start_addr+i);	//otp data read
			else if( i < 188)
				*buffer = gc8034_read_otp(7,i-60);	//otp data read
			else if( i < 316)
				*buffer = gc8034_read_otp(8,i-188);	//otp data read
			else if( i < 398)
				*buffer = gc8034_read_otp(9,i-316);	//otp data read
			else 
				LOG_INF("gc8034 gc8034_read_lsc i = %d over-size\n",i);
			buffer++;
		}
	}
	else if (lsc_start_addr == 0x0220) {
		for (i = 0; i < size; i++) {
			if( i<60 )
				*buffer = gc8034_read_otp(3,lsc_start_addr+i);	//otp data read
			else if( i < 188)
				*buffer = gc8034_read_otp(4,i-60);	//otp data read
			else if( i < 316)
				*buffer = gc8034_read_otp(5,i-188);	//otp data read
			else if( i < 398)
				*buffer = gc8034_read_otp(6,i-316);	//otp data read
			else 
				LOG_INF("gc8034 gc8034_read_lsc i = %d over-size\n",i);
			buffer++;
		}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x15, 0x12);//10
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0x94, 0xb0 + GC8034_BinStartX);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				sprintf(Back_Camera_Name,"GC8034:C&T:8M_AF");
...
UINT32 GC8034_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
