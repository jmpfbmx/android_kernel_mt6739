...
#include "hi846_brodsands_mipi_raw_Sensor.h"

#define PFX "HI847_sensor"
#define LOG_INF(format, args...)    pr_info(PFX "[%s] " format, __FUNCTION__, ##args)
#define Hi846_MaxGain 16

#define Hi847_OTP_FUNCTION

void HI847_OTPSetting(void);
static kal_uint16 Hi847_OTP_read_cmos_sensor(kal_uint16 otp_addr);
static kal_uint32 return_sensor_id(void);
//begin, add for read sensor_id first 2018/02/02
static kal_uint32 sensor_id_zhijun = 0x00;
//end, add for read sensor_id first 2018/02/02

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = HI846_BRODSANDS_SENSOR_ID,
...
static kal_uint16 read_cmos_sensor_16(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, temp_data, 2, imgsensor.i2c_write_id);
	return get_word;
}
#endif
...
//	write_cmos_sensor(0x0A04, 0x014A);
	...
	write_cmos_sensor(0x0958, 0x8980); 
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0x005C, 0x4202); 
	//write_cmos_sensor(0x0006, 0x034A);  // 90fps
	write_cmos_sensor(0x0006, 0x09d8);  // 30fps
	write_cmos_sensor(0x0008, 0x0ED8); 
	write_cmos_sensor(0x000C, 0x0122); 
	write_cmos_sensor(0x0A22, 0x0100); 
	write_cmos_sensor(0x0A24, 0x0000); 
...
struct HI846_otp_struct HI847_otp;

EXPORT_SYMBOL(HI847_otp);

static void HI847_OTP_write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
...
void HI847_OTPSetting(void)
{	
	LOG_INF("HI847OTPSetting begin:\n ");
	HI847_OTP_write_cmos_sensor(0x0A02, 0x01); //fast sleep On
	HI847_OTP_write_cmos_sensor(0x0A00, 0x00); //sleep On
	mDELAY(10);
	HI847_OTP_write_cmos_sensor(0x0F02, 0x00); //pll disable
	HI847_OTP_write_cmos_sensor(0x071A, 0x01); //CP TRI_H
	HI847_OTP_write_cmos_sensor(0x071B, 0x09); //IPGM TRIM_H
	HI847_OTP_write_cmos_sensor(0x0D04, 0x01); //Fsync Output enable
	HI847_OTP_write_cmos_sensor(0x0D00, 0x07); //Fsync Output Drivability
	HI847_OTP_write_cmos_sensor(0x003E, 0x10); //OTP R/W
	HI847_OTP_write_cmos_sensor(0x0a00, 0x01); //sleep off
	LOG_INF("HI847OTPSetting exit :\n ");
}
...
static kal_uint16 Hi847_OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
	...
	sensor_init();
	HI847_OTPSetting();
    HI847_OTP_write_cmos_sensor(0x70a, (otp_addr & 0xFF00) >> 8 ); //start address H        
    HI847_OTP_write_cmos_sensor(0x70b, otp_addr & 0xFF); //start address L
    HI847_OTP_write_cmos_sensor(0x702, 0x01); //single read
    	//mDELAY(10);
    data = read_cmos_sensor(0x708); //OTP data read
	return data;
}

static kal_uint32 get_module_id(void)
{
	int addr = 0,group_flag = 0, g_module_id = 0;
	group_flag = Hi847_OTP_read_cmos_sensor(0x0201);
	LOG_INF("zhijun.fu group_flag = %d\n", group_flag);
	if (group_flag == 0x01)	//Base Info Group1 valid
		addr = 0x0202;
	else if (group_flag == 0x13)	//Base Info Group2 valid
		addr = 0x0213;
	else if (group_flag == 0x37)	//Base Info Group3 valid
		addr = 0x0224;
	else

	{
		LOG_INF("zhijun.fu g_module_id = %d\n", g_module_id);
		return 0;
	}
	g_module_id = Hi847_OTP_read_cmos_sensor(addr);
	LOG_INF("zhijun.fu module_id = %d\n", g_module_id);
	return g_module_id;
}

static kal_uint32 return_sensor_id(void)
{
	static kal_uint32 sensor_id = 0, module_id =0;
	//HI847_OTPSetting();
	
	sensor_id = (read_cmos_sensor(0x0F17) << 8) | read_cmos_sensor(0x0F16);
	module_id = get_module_id();
	LOG_INF("zhijun.fu module_id = %d, sensor_id = %d\n", module_id, sensor_id);
	if( (module_id != 0x13) && (sensor_id == 0x0846))// add for division dmegc&bolixin
	{
		LOG_INF("zhijun.fu module_id = %d\n", module_id);
		sensor_id = 0x0847;
	}
	LOG_INF("zhijun.fu sensor_id = %d\n", sensor_id);
	return sensor_id;
}

static int HI847_otp_read(void)
{
	int i = 0, addr = 0, wb_start_addr = 0,af_start_addr = 00;
	int  checksum = 0; //wb_data[28];
	LOG_INF("HI847 HI847_otp_read \n");


/* begin, it's for basic info on 2017-12-01 */
	HI847_otp.Base_Info_Flag = Hi847_OTP_read_cmos_sensor(0x0201);
	if (HI847_otp.Base_Info_Flag == 0x01)	//Base Info Group1 valid
		addr = 0x0202;
	else if (HI847_otp.Base_Info_Flag == 0x13)	//Base Info Group2 valid
		addr = 0x0213;
	else if (HI847_otp.Base_Info_Flag == 0x37)	//Base Info Group3 valid
		addr = 0x0224;
	else
		return 0;
LOG_INF("HI847 addr = 0x%x \n", addr);
	if (addr == 0) {
		HI847_otp.module_integrator_id = 0;
		HI847_otp.prodyction_year = 0;
		HI847_otp.production_month = 0;
		HI847_otp.production_day = 0;
		HI847_otp.lens_id = 0;
		HI847_otp.vcm_id = 0;
		HI847_otp.Driver_ic_id = 0;
		HI847_otp.infocheck = 0;
	} else {
		HI847_otp.module_integrator_id = Hi847_OTP_read_cmos_sensor(addr);
		HI847_otp.lens_id = Hi847_OTP_read_cmos_sensor(addr + 1);
		HI847_otp.vcm_id = Hi847_OTP_read_cmos_sensor(addr + 2);
		HI847_otp.Driver_ic_id = Hi847_OTP_read_cmos_sensor(addr + 3);
		HI847_otp.prodyction_year = Hi847_OTP_read_cmos_sensor(addr + 4);
		HI847_otp.production_month = Hi847_OTP_read_cmos_sensor(addr + 5);
		HI847_otp.production_day = Hi847_OTP_read_cmos_sensor(addr + 6);
		HI847_otp.infocheck = Hi847_OTP_read_cmos_sensor(addr + 16);
	}

	checksum = (HI847_otp.module_integrator_id + HI847_otp.prodyction_year + HI847_otp.production_month + HI847_otp.production_day + HI847_otp.lens_id + HI847_otp.vcm_id + HI847_otp.Driver_ic_id) % 0xFF + 1;

	if (checksum == HI847_otp.infocheck)
		LOG_INF("HI847_Sensor: Module information checksum PASS\n ");
	else{
		LOG_INF("HI847_Sensor: Module information checksum Fail\n ");
		return 0;
	}
	
	LOG_INF("HI847 module_integrator_id = 0x%x, AF_FLAG = 0x%x, prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id = 0x%x, infocheck = 0x%x \n",\
		HI847_otp.module_integrator_id,HI847_otp.AF_FLAG, HI847_otp.prodyction_year, HI847_otp.production_month, HI847_otp.production_day, HI847_otp.sensor_id, HI847_otp.lens_id, HI847_otp.infocheck);
/* end, it's for basic info on 2017-12-01 */


/* begin, it's for AWB info on 2017-12-01 */
	HI847_otp.WB_FLAG = Hi847_OTP_read_cmos_sensor(0x18B3);
	if (HI847_otp.WB_FLAG == 0x01)
		wb_start_addr = 0x18B4;	// Group 1
	else if (HI847_otp.WB_FLAG == 0x13)
		wb_start_addr = 0x18BD;	// Group 2
	else if (HI847_otp.WB_FLAG == 0x37)
		wb_start_addr = 0x18C6;
	else{
		LOG_INF("HI847 WB data invalid \n");
		return 0 ;
	}

	LOG_INF("HI847 WB_FLAG = 0x%x \n", HI847_otp.WB_FLAG);
	LOG_INF("HI847 wb_start_addr = 0x%x \n", wb_start_addr);

	if (wb_start_addr != 0) {
		HI847_OTP_write_cmos_sensor(0x70a, (wb_start_addr >> 8) & 0xff);	//start addr H
		HI847_OTP_write_cmos_sensor(0x70b, wb_start_addr & 0xff);	//start addr L
		HI847_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
		for (i = 0; i < 9; i++) {
			HI847_otp.wb_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI847 wb_data[%d] = 0x%x  ", i, HI847_otp.wb_data[i]);
		}
	}
	else{
		LOG_INF("zhijun.fu wb_start_addr is invalid");
		return 0 ;
	}
/* end, it's for AWB info on 2017-12-01 */	


/* begin, it's for AF info on 2017-12-01 */
	HI847_otp.AF_FLAG = Hi847_OTP_read_cmos_sensor(0x18CF);
	LOG_INF("HI847 AF_FLAG = 0x%x\n",HI847_otp.AF_FLAG);
	if (HI847_otp.AF_FLAG == 0x01)
		af_start_addr = 0x18D0;
	else if (HI847_otp.AF_FLAG == 0x13)
		af_start_addr = 0x18D8;  //zy
	else if (HI847_otp.AF_FLAG == 0x37)
		af_start_addr = 0x18E0;  //zy
	else{
		LOG_INF("HI847 read af data invalid \n");
		return 0 ;
	}
	
	if (af_start_addr != 0) {
		HI847_OTP_write_cmos_sensor(0x70a, (af_start_addr >> 8) & 0xff);	//start addr H
		HI847_OTP_write_cmos_sensor(0x70b, af_start_addr & 0xff);	//start addr L
		HI847_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
		for (i = 0; i < 5; i++) {
			HI847_otp.af_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI847 af_data[%d] = 0x%x  ", i, HI847_otp.af_data[i]);
		}
	}
...
int hi847_read_lsc(unsigned short *buffer,int size)
{
	...
	int lsc_start_addr = 0, i = 0, lsc_flag = 0;
	sensor_init();
	HI847_OTPSetting();
	
	lsc_flag = Hi847_OTP_read_cmos_sensor(0x0235);
	LOG_INF("HI847 LSC_FLAG = 0x%x\n",lsc_flag);
	if (lsc_flag == 0x01)
		lsc_start_addr = 0x0236;
	else if (lsc_flag == 0x13)
		lsc_start_addr = 0x09B5;  //zy
	else if (lsc_flag == 0x37)
		lsc_start_addr = 0x1134;  //zy
	else{
		LOG_INF("HI847 read lsc data invalid \n");
		return 0;
	}
	//spin_lock(&imgsensor_drv_lock);
	LOG_INF("HI847 lsc_start_addr = 0x%x\n",lsc_start_addr);
	if (lsc_start_addr != 0) {
		HI847_OTP_write_cmos_sensor(0x70a, (lsc_start_addr >> 8) & 0xff);	//start addr H
		HI847_OTP_write_cmos_sensor(0x70b, lsc_start_addr & 0xff);	//start addr L
		HI847_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
		for (i = 0; i < size; i++) {
			*buffer = read_cmos_sensor(0x708);	//otp data read
			//pr_info("847_lsc_data 0x%x , ", *buffer);
			buffer++;
		}
	}
	//spin_unlock(&imgsensor_drv_lock);
...
EXPORT_SYMBOL(hi847_read_lsc);

static int HI847_otp_check(void)
{
	int checksum = 0 ,i =0;//, j = 0;

	LOG_INF("HI847 HI847_otp_check \n");

	for(i=0;i< 8 ;i++)
		checksum += HI847_otp.wb_data[i];
	checksum = checksum % 0xFF + 1;
	HI847_otp.wb_checksum = HI847_otp.wb_data[8];
	

	if (checksum == HI847_otp.wb_checksum)
	{
		LOG_INF("zhijun.fu HI847_otp.wb_checksum is %d, checksum is %d\n ", HI847_otp.wb_checksum,checksum);
		LOG_INF("zhijun.fu HI847_Sensor: WB checksum PASS\n ");
	}
	else{
		LOG_INF("zhijun.fu HI847_Sensor: WB checksum Fail\n ");
		LOG_INF("zhijun.fu HI847_otp.wb_checksum is %d, checksum is %d\n ", HI847_otp.wb_checksum,checksum);
	}

	HI847_OTP_write_cmos_sensor(0x0a00, 0x00); //sleep On
	mdelay(10);
    	HI847_OTP_write_cmos_sensor(0x003e, 0x00); //OTP mode off
    	HI847_OTP_write_cmos_sensor(0x0a00, 0x01); //sleep Off


	return HI847_otp.WB_FLAG;
}
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					LOG_INF("Brodsands,i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					sprintf(Back_Camera_Name,"HI846:Brodsands:8M_AF");
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) { 
		...
		write_cmos_sensor(0x0a04, 0x0142);                // ADPC Enable
...
UINT32 HI846_BRODSANDS_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
