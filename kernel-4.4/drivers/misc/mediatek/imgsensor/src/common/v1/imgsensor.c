...
int front_camera_flag=0;
int back_camera_flag=0;
...
//Begin added by zhanyong.yin for XR5301378 on 2017/10/19
				 	...
				 	{
						if(0 == back_camera_flag){
							back_camera_flag=1;
							err = alloc_chrdev_region(&midia1_devno, 0, 1, "media2");
							if (err < 0) {
								pr_err("%s:alloc_chrdev_region failed  error code=%d\n", __func__, err);
								return err;
							}
							device_create(probustClass, NULL, midia1_devno, NULL, "media2");
						}
					}
					else if(psensor->inst.sensor_idx==IMGSENSOR_SENSOR_IDX_SUB)
					{
						if(0 == front_camera_flag){
							front_camera_flag=1;
							err = alloc_chrdev_region(&midia2_devno, 0, 1, "media1");
							if (err < 0) {
								pr_err("%s:alloc_chrdev_region failed  error code=%d\n", __func__, err);
								return err;
							}
							device_create(probustClass, NULL, midia2_devno, NULL, "media1");	
						}
					}
//End added by zhanyong.yin for XR5301378 on 2017/10/19
		err = ERROR_NONE;
...
#define STRINGIZE(stringizedName) TOSTRING(stringizedName)
	...
	*get_search_list,
	*orderedSearchList[0], orderedSearchList[1], orderedSearchList[2], orderedSearchList[3],
	*orderedSearchList[4], orderedSearchList[5], orderedSearchList[6], orderedSearchList[7],
	*orderedSearchList[8], orderedSearchList[9], orderedSearchList[10], orderedSearchList[11],
	*orderedSearchList[12], orderedSearchList[13], orderedSearchList[14], orderedSearchList[15]);
	*/
...
static void cam_temperature_report_wq_routine(struct work_struct *data)
{
	...
	if (ret != ERROR_NONE)
		PK_PR_ERR("Get Sub2 cam temperature error(%d)!\n", ret);
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	psensorResolution = kzalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	if (pSensorCtrl == NULL) {
		PK_PR_ERR("[adopt_CAMERA_HW_Control] NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorCtrl->InvokeCamera);
	if (psensor == NULL) {
		PK_PR_ERR("[adopt_CAMERA_HW_Control] NULL psensor.\n");
		return -EFAULT;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	if (psensor == NULL) {
		PK_PR_ERR("[adopt_CAMERA_HW_FeatureControl] NULL psensor.\n");
		return -EFAULT;
	}
...
#endif
//Begin added by zhanyong.yin for XR5301378 on 2017/10/19
char Front_Camera_Name[256]="NA:NA:NA";

static ssize_t Front_Camera_Name_Show(struct device *dev, 
...
static ssize_t Front_Camera_Name_store(struct device* dev, 
...
char Back_Camera_Name[256]="NA:NA:NA";

static ssize_t  Back_Camera_Name_Show(struct device *dev, 
                                  struct device_attribute *attr, char *buf)
{	
     printk("Dump  %s \n",Back_Camera_Name);
...
static ssize_t  Back_Camera_Name_store(struct device* dev, 
                                   struct device_attribute *attr, const char *buf, size_t count)
{
...
DEVICE_ATTR(CamNameF,     S_IWUSR | S_IRUGO, Front_Camera_Name_Show, Front_Camera_Name_store);
DEVICE_ATTR(CamNameB,     S_IWUSR | S_IRUGO, Back_Camera_Name_Show, Back_Camera_Name_store);
char Front_Camera_OTP[256]="NA:NA";

static ssize_t Front_Camera_OTP_Show(struct device *dev, 
                                  struct device_attribute *attr, char *buf)
{	
   printk("Dump  %s \n",Front_Camera_OTP);
...
static ssize_t Front_Camera_OTP_store(struct device* dev, 
                                   struct device_attribute *attr, const char *buf, size_t count)
{
...
char Back_Camera_OTP[256]="NA:NA";

static ssize_t  Back_Camera_OTP_Show(struct device *dev, 
                                  struct device_attribute *attr, char *buf)
{	
     printk("Dump  %s \n",Back_Camera_OTP);
...
static ssize_t  Back_Camera_OTP_store(struct device* dev, 
                                   struct device_attribute *attr, const char *buf, size_t count)
{
...
DEVICE_ATTR(CamOTPF,     S_IWUSR | S_IRUGO, Front_Camera_OTP_Show, Front_Camera_OTP_store);
DEVICE_ATTR(CamOTPB,     S_IWUSR | S_IRUGO, Back_Camera_OTP_Show, Back_Camera_OTP_store);
extern struct device* get_deviceinfo_dev(void);
static int Create_Camera_node_ForMMI(void)
{
	struct device * cam;

#if 1
	cam=get_deviceinfo_dev();
	if (device_create_file(cam, &dev_attr_CamNameB) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_CamNameB.attr.name);

	if (device_create_file(cam, &dev_attr_CamNameF) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_CamNameF.attr.name);
	
	if (device_create_file(cam, &dev_attr_CamOTPB) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_CamNameB.attr.name);

	if (device_create_file(cam, &dev_attr_CamOTPF) < 0)
		printk("Failed to create device file(%s)!\n", dev_attr_CamNameF.attr.name);
