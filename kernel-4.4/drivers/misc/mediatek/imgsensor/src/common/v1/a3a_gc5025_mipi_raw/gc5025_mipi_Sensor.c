...
static kal_uint8 gc5025_read_otp_zhijun(kal_uint16 addr)
{
	...
	write_cmos_sensor(0xfa,0xb0);          //OTP clk enable
	
	Address_High = (addr >> 8) & 0x03;
	Address_Low = addr & 0x00ff;
	
	write_cmos_sensor(0xd4,0x84+Address_High); //OTP enable[7]  OTP page Select[2] AddressHigh[1:0]
	write_cmos_sensor(0xd5,Address_Low);  //Address_Low byte
	
	mdelay(1);
	write_cmos_sensor(0xf3,0x20);   //OTP Read
...
static void gc5025_gcore_read_otp_info(void)
{
	...
	kal_uint8 info[8];
	kal_uint8 wb[8];
	kal_uint8  group1_valid = 0, group2_valid = 0;
	kal_uint16 start_addr = 0;
#endif	
...
//INFO&WB
	...
	group1_valid = gc5025_read_otp_zhijun(0x00);
	group2_valid = gc5025_read_otp_zhijun(0x40);
	if( group2_valid == 0x55 )
		start_addr = 0x48;
	else if( group1_valid == 0x55 )
		start_addr = 0x08;
	else
	{
		start_addr = 0x00; 
		LOG_INF("zhijun.fu, GC5025_OTP read basic failed, return !!!");
		return;
	}
	LOG_INF("GC5025_OTP : info start_addr = 0x%x\n",start_addr);
	check = 0;
	for(i=0; i< 8; i++)
	{
		info[i] = gc5025_read_otp_zhijun(start_addr + i*8);
		LOG_INF(" info[%d] = %d",i, info[i]);
	}
	...
	if ((check % 255 + 1) == info[INFO_WIDTH-1])
	{
		...
		gc5025_otp_info.vcm_id = info[2];
		gc5025_otp_info.year = info[3];
		gc5025_otp_info.month = info[4];
		gc5025_otp_info.day = info[5];
	}
	else
	{
		LOG_INF("GC5025_OTP_INFO Check sum Error, check = %d,  info[7] = %d!!\n",check,info[INFO_WIDTH-1]);
	}


	group1_valid = gc5025_read_otp_zhijun(0x80);
	group2_valid = gc5025_read_otp_zhijun(0xd0);
	if( group2_valid == 0x55 )
		start_addr = 0xd8;
	else if( group1_valid == 0x55 )
		start_addr = 0x88;
	...
	{
		start_addr = 0x00; 
		LOG_INF("zhijun.fu, GC5025_OTP read AWB info failed");
		return;
	}
	
	LOG_INF("GC5025_OTP : AWB start_addr = 0x%x\n",start_addr);

	if( start_addr != 0x00 )
	{
		check = 0;
		LOG_INF("GC5025_OTP : AWB start_addr = 0x%x ",start_addr);
		for(i=0; i< 8; i++)
		{
			wb[i] = gc5025_read_otp_zhijun(start_addr + i*8);
			check += wb[i];
			LOG_INF(" wb[%d] = %d",i, wb[i]);
		}
		gc5025_otp_info.wbcheck = gc5025_read_otp_zhijun(start_addr + 8*8);
	}
	else
	{
		LOG_INF("zhijun.fu, GC5025_OTP read AWB data failed");
		//return;
	}

	if ( gc5025_otp_info.wbcheck == (check%0xff + 1) )
	{
		for(i=0; i< 8; i++)
			LOG_INF("zhijun.fu, GC5025_otp wb[%d] = %d\n",i,wb[i]);
		
		gc5025_otp_info.rg_gain = (((wb[1]<<8)&0xff00)|(wb[0]&0x00ff)) > 0 ? (((wb[1]<<8)&0xff00)|(wb[0]&0x00ff)) : 0x400;
		gc5025_otp_info.bg_gain = (((wb[3]<<8)&0xff00)|(wb[2]&0x00ff)) > 0 ? (((wb[3]<<8)&0xff00)|(wb[2]&0x00ff)) : 0x400;
		gc5025_otp_info.wb_flag = gc5025_otp_info.wb_flag|0x01;

		gc5025_otp_info.golden_rg = (((wb[5]<<8)&0xff00)|(wb[4]&0x00ff))  > 0 ? (((wb[5]<<8)&0xff00)|(wb[4]&0x00ff)) : RG_TYPICAL;
		gc5025_otp_info.golden_bg = (((wb[7]<<8)&0xff00)|(wb[6]&0x00ff)) > 0 ? (((wb[7]<<8)&0xff00)|(wb[6]&0x00ff)) : BG_TYPICAL;
		gc5025_otp_info.golden_flag = gc5025_otp_info.golden_flag|0x01;

	}
	else
	{
		LOG_INF("zhijun.fu GC5025_OTP_WB gc5025_otp_info.wbcheck =  %d, check = %d", gc5025_otp_info.wbcheck, check);
		//return;
	}
...
{	
    kal_uint8 flag_GC5025 = 0;
	
	gc5025_gcore_initial_otp();
	gc5025_gcore_enable_otp(otp_open);
	
	gc5025_select_page_otp(otp_page1);
    flag_GC5025 = gc5025_read_otp(0x27);
	
	gc5025_gcore_enable_otp(otp_close);
	
	if((flag_GC5025 & 0x01) == 0x01) {
		LOG_INF("GC5025 sensor! \n");
		return 0;
    }
	else {
		LOG_INF("error! \n");
...
static kal_uint32 return_sensor_id(void)
{
	...
	module_id = gc5025_read_otp_zhijun(0x08);
	if(module_id != 0x16)
		module_id = gc5025_read_otp_zhijun(0x48);
...
static void set_shutter(kal_uint16 shutter)
{
	...
	LOG_INF("Exit! shutter =%d, framelength =%d\n", shutter, imgsensor.frame_length*2);
...
#define ANALOG_GAIN_4 181 /* 1.83x*/
...
{	
	...
	else
	{
		...
		write_cmos_sensor(0xb1, temp>>6);
		write_cmos_sensor(0xb2, (temp<<2)&0xfc);
		LOG_INF("GC5025MIPI analogic gain 2.8x, GC5025MIPI add pregain = %d\n", temp);
	}
	return gain;
...
{
   LOG_INF("E");
   LOG_INF("Ackman update 20180302");
	/*SYS*/
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    	...
    	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			sprintf(Front_Camera_Name,"GC5025:Brodsands:5M_FF"); 
