...
//#define S5K4H8_OTP_USE
extern void Darling_S5K4H8_read_OTP(void);
extern bool otp_daling_update_wb(void);
...
/*Modify by yuda.chen for defect 4619342 at 20170421 begin*/
	.cap_delay_frame = 1, 
	.pre_delay_frame = 1,  
	.video_delay_frame = 1,
    ...
    /*Modify by yuda.chen for defect 4619342 at 20170421 end*/
...
static void write_cmos_sensor_8(kal_uint16 addr, kal_uint8 para)
{
 ...
 void S5K4H8_wordwrite_cmos_sensor(u16 addr, kal_uint16 para)
{
	char puSendCmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) ,  (char)(para >> 8),	(char)(para & 0xFF) };
	iWriteRegI2C(puSendCmd , 4,imgsensor.i2c_write_id);
}
 void S5K4H8_bytewrite_cmos_sensor(u16 addr, kal_uint16 para)
{
	char puSendCmd[4] = {(char)(addr >> 8), (char)(addr & 0xFF), (char)(para & 0xFF)};
	iWriteRegI2C(puSendCmd , 3,imgsensor.i2c_write_id);
}
kal_uint16 S5K4H8_byteread_cmos_sensor(u16 addr)
{
...
{
	// LOG_INF("dummyline = %s, dummypixels = %s ", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void s5k4h8_read_otp(void)
{
	int i=0;
	int addr=0;
	int awb_flag=0,awb_flag1=0,checksum=0,checksum1=0;
	...
	write_cmos_sensor(0x0306,0x0073);
	...
	{
		int temp=0;
		temp=read_cmos_sensor_8(0x0A04+i);
		printk("TCL s5k4h8 OTP addr:0A04+%d,value:0x%x,i2c ID:0x%x\n",i,temp,imgsensor.i2c_write_id);
	}
//awb otp
	awb_flag=read_cmos_sensor_8(0x0A04);
	awb_flag1=read_cmos_sensor_8(0x0A24);
	if(awb_flag==0x1)
	{
		printk("TCL s5k4h8 AWB OTP group1 is valid\n");
		addr=0x0A04;
		checksum=read_cmos_sensor_8(addr+1);
		module_id=read_cmos_sensor_8(addr+2);
		year=read_cmos_sensor_8(addr+3);
		month=read_cmos_sensor_8(addr+4);
		date=read_cmos_sensor_8(addr+5);
		lens_id=read_cmos_sensor_8(addr+6);
		vcm_id=read_cmos_sensor_8(addr+7);
		vcm_ic_id=read_cmos_sensor_8(addr+8);
		rg_ratio=(read_cmos_sensor_8(addr+10)<<8)|(read_cmos_sensor_8(addr+11)&0xff);
		bg_ratio=(read_cmos_sensor_8(addr+12)<<8)|(read_cmos_sensor_8(addr+13)&0xff);
		g_rg_ratio=(read_cmos_sensor_8(addr+14)<<8)|(read_cmos_sensor_8(addr+15)&0xff);
		g_bg_ratio=(read_cmos_sensor_8(addr+16)<<8)|(read_cmos_sensor_8(addr+17)&0xff);
		s5k4h8_RG_MSB=read_cmos_sensor_8(addr+10);
		s5k4h8_RG_LSB=read_cmos_sensor_8(addr+11);
		s5k4h8_BG_MSB=read_cmos_sensor_8(addr+12);
		s5k4h8_BG_LSB=read_cmos_sensor_8(addr+13);

		s5k4h8_gRG_MSB=read_cmos_sensor_8(addr+14);
		s5k4h8_gRG_LSB=read_cmos_sensor_8(addr+15);
		s5k4h8_gBG_MSB=read_cmos_sensor_8(addr+16);
		s5k4h8_gBG_LSB=read_cmos_sensor_8(addr+17);
		for(i=0;i<20;i++){
			checksum1+=read_cmos_sensor_8(0x0A06+i);
		}
		...
		printk("TCL s5k4h8 OTP checksum is %d,checksum1 is %d\n",checksum,checksum1);
	}
	else if(awb_flag1==0x1){
		printk("TCL s5k4h8 AWB OTP group2 is valid\n");
		addr=0x0A24;
		checksum=read_cmos_sensor_8(addr+1);
		module_id=read_cmos_sensor_8(addr+2);
		year=read_cmos_sensor_8(addr+3);
		...
		s5k4h8_gBG_LSB=read_cmos_sensor_8(addr+17);
		for(i=0;i<16;i++){
			checksum1+=read_cmos_sensor_8(0x0A24+i);
		}
		checksum1=(checksum1)%255+1;
		printk("TCL s5k4h8 OTP module_id is 0x%x,lens_id is 0x%x,vcm_id is 0x%x,vcm_ic_id is 0x%x\n",module_id,lens_id,vcm_id,vcm_ic_id);
		printk("TCL s5k4h8 OTP date is %d-%d-%d\n",year,month,date);
		...
		printk("TCL s5k4h8 AWB OTP is invalid\n");
	}
	//af otp
	af_flag=read_cmos_sensor_8(0x0A14);
	af_flag1=read_cmos_sensor_8(0x0A34);
	if(af_flag==0x1){
		printk("TCL s5k4h8 AF OTP group1 is valid\n");
		checksumaf=read_cmos_sensor_8(0x0A15);
		macro=(read_cmos_sensor_8(0x0A16)<<8)|(read_cmos_sensor_8(0x0A17)&0xff);
		inf=(read_cmos_sensor_8(0x0A18)<<8)|(read_cmos_sensor_8(0x0A19)&0xff);
		s5k4h8_MAC_MSB=read_cmos_sensor_8(0x0A16);
		s5k4h8_MAC_LSB=read_cmos_sensor_8(0x0A17);
		s5k4h8_INF_MSB=read_cmos_sensor_8(0x0A18);
		s5k4h8_INF_LSB=read_cmos_sensor_8(0x0A19);
		for(i=0;i<4;i++){
			checksumaf1+=read_cmos_sensor_8(0x0A16+i);
		}
		checksumaf1=(checksumaf1)%255+1;
		printk("TCL s5k4h8 OTP checksumaf is %d,checksumaf1 is %d\n",checksumaf,checksumaf1);
		printk("TCL s5k4h8 OTP macro is %d,inf is %d\n",macro,inf);
	}
	else if(af_flag1==0x1){
		printk("TCL s5k4h8 AF OTP group2 is valid\n");
		checksumaf=read_cmos_sensor_8(0x0A35);
		macro=(read_cmos_sensor_8(0x0A36)<<8)|(read_cmos_sensor_8(0x0A37)&0xff);
		inf=(read_cmos_sensor_8(0x0A38)<<8)|(read_cmos_sensor_8(0x0A39)&0xff);
		s5k4h8_MAC_MSB=read_cmos_sensor_8(0x0A36);
		s5k4h8_MAC_LSB=read_cmos_sensor_8(0x0A37);
		s5k4h8_INF_MSB=read_cmos_sensor_8(0x0A38);
		s5k4h8_INF_LSB=read_cmos_sensor_8(0x0A39);
		for(i=0;i<4;i++){
			checksumaf1+=read_cmos_sensor_8(0x0A36+i);
		}
		checksumaf1=(checksumaf1)%255+1;
		printk("TCL s5k4h8 OTP checksumaf is %d,checksumaf1 is %d\n",checksumaf,checksumaf1);
		printk("TCL s5k4h8 OTP macro is %d,inf is %d\n",macro,inf);
		...
		printk("TCL s5k4h8 AF OTP is invalid\n");
	}
	
	if(((af_flag==0x1)||(af_flag1==0x1))&&(module_id==0x1)&&(lens_id==0x63)&&(vcm_id==0x13)&&(vcm_ic_id==0x12)&&(checksum==checksum1)&&(checksumaf==checksumaf1)){
		S5K4H8_OTP_FLAG=1;
	}
	else{
		S5K4H8_OTP_FLAG=0;
	}
	printk("TCL s5k4h8 OTP S5K4H8_OTP_FLAG is %d",S5K4H8_OTP_FLAG);
	//write_cmos_sensor_8(0x0A00,0x04);
	write_cmos_sensor_8(0x0A00,0x00);
}
void s5k4h8_apply_lsc_otp(void)
{
	write_cmos_sensor(0x6028,0x4000);
...
write_cmos_sensor(0x0100, 0x0000);
  ...
  write_cmos_sensor(0x0B00, 0x0180);//0x0080    open otp lsc
  write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x0C40);
  write_cmos_sensor(0x6F12, 0x0140);
  write_cmos_sensor(0xFCFC, 0x4000);
 ...
 #ifdef S5K4H8_OTP_USE
otp_daling_update_wb();
#endif
}
...
static void slim_video_setting(void)
{
  ...
  write_cmos_sensor(0x0B00, 0x0180);//0x0080    open otp lsc 0x0180
	write_cmos_sensor(0x6028, 0x2000);
  write_cmos_sensor(0x602A, 0x0C40);
  write_cmos_sensor(0x6F12, 0x0140);
  write_cmos_sensor(0xFCFC, 0x4000);
...
extern int S5K4H8_OTP_FLAG;

void S5K4H8_OTP_Check(void)
{
	write_cmos_sensor(0x0100, 0x0100);
	check_output_stream_on();
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
	   	 ...
	   	 #ifdef S5K4H8_OTP_USE
		 sprintf(Back_Camera_Name,"S5K4H8 : SUNSRISE : SUP1: 8M_AF"); 
		S5K4H8_OTP_Check();
			#ifdef TARGET_BUILD_MMITEST
				if(S5K4H8_OTP_FLAG==1)
				{
					LOG_INF("OTP_OK!!!!\n");
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
        ...
        write_cmos_sensor(0x0600, 0x0304);
	} else {
		// 0x5E00[8]: 1 enable,  0 disable
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
