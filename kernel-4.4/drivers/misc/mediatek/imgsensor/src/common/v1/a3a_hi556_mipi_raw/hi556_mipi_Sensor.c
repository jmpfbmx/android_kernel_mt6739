 ...
 ****************************************************************************/
/************************************************* 
History: 1.0 Modified by zhanyong.yin for XR 5301378 on 2017/09/21
       2.0 Modified by zhanyong.yin for XR 5301378 on 2017/10/09
*************************************************/
...
static int RGr_unit=0,BGr_unit=0;
static int RGr_golden=0,BGr_golden=0;
#endif
...
write_cmos_sensor(0x0c00, 0x1111); 
...
static kal_uint16 OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
   	...
   	write_cmos_sensor_8(0x010a, (otp_addr & 0xFF00) >> 8);
	write_cmos_sensor_8(0x010b, otp_addr & 0xFF);
	write_cmos_sensor_8(0x0102,0x01);
	data=read_cmos_sensor(0x0108);
	return data;
}

void read_hi556_otp(void)
{
	int i = 0,  awb_start_addr=0, start_addr=0 ;
	...
	HI556_otp.Base_Info_Flag = OTP_read_cmos_sensor(0x0401);
	
	HI556_otp.Base_Info_Flag = read_cmos_sensor(0x0108);
	
	if(HI556_otp.Base_Info_Flag ==0x01)
		start_addr=0x0402;
	else if (HI556_otp.Base_Info_Flag ==0x13)
		start_addr=0x0413;
	else if(HI556_otp.Base_Info_Flag ==0x37)
		start_addr=0x0424;
	else
		LOG_INF("no OTP data");

/*	begin, read basic info */	
	if (start_addr == 0) {
		HI556_otp.module_integrator_id = 0;
		...
		HI556_otp.module_integrator_id = OTP_read_cmos_sensor(start_addr);
		//HI556_otp.lens_id = OTP_read_cmos_sensor(start_addr + 1);
		//HI556_otp.vcm_id = OTP_read_cmos_sensor(start_addr + 2);
		//HI556_otp.Driver_ic_id = OTP_read_cmos_sensor(start_addr + 3);
		HI556_otp.prodyction_year = OTP_read_cmos_sensor(start_addr + 2);
		HI556_otp.production_month = OTP_read_cmos_sensor(start_addr + 3);
		HI556_otp.production_day = OTP_read_cmos_sensor(start_addr + 4);
		HI556_otp.infocheck = OTP_read_cmos_sensor(start_addr + 16);
	}
/*	end, read basic info */
	
	//checksum = (HI556_otp.module_integrator_id + HI556_otp.prodyction_year + HI556_otp.production_month + HI556_otp.production_day + HI556_otp.lens_id + HI556_otp.vcm_id + HI556_otp.Driver_ic_id) % 0xFF + 1;
...
LOG_INF("HI556 module_integrator_id = 0x%x,  date =  = 0x%d - %d - %d, sensor_id = 0x%x \n",\
		HI556_otp.module_integrator_id, HI556_otp.prodyction_year, HI556_otp.production_month, HI556_otp.production_day, HI556_otp.sensor_id);	



	HI556_otp.WB_FLAG = OTP_read_cmos_sensor(0x0435);
	...
	if(HI556_otp.WB_FLAG !=0)
	{
		...
		RGr_unit=((data_awb[1]<<8)&0xff00)|(data_awb[0]&0xff);
		BGr_unit=((data_awb[3]<<8)&0xff00)|(data_awb[2]&0xff);
		RGr_golden=((data_awb[5]<<8)&0xff00)|(data_awb[4]&0xff);
		BGr_golden=((data_awb[7]<<8)&0xff00)|(data_awb[6]&0xff);
		//gBGb=((data_awb[8]<<8)&0xff00)|(data_awb[9]&0xff);
		//gGbGr=((data_awb[10]<<8)&0xff00)|(data_awb[11]&0xff);
		check_sum_awb=data_awb[29];
		//check_sum_awb_cal=(check_sum_awb_cal%255)+1;
	}
	
	LOG_INF("awb_flag=0x%x\n",HI556_otp.WB_FLAG);
	LOG_INF("RGr_unit=0x%x,BGr_unit=0x%x\n",RGr_unit,BGr_unit);
	LOG_INF("RGr_golden=0x%x,BGr_golden=0x%x\n",RGr_golden,BGr_golden);
...
void apply_hi556_otp(void)
{
	int R_gain=256,G_gain=256,B_gain=256;
	int data_gain[8]={0};
	int i=0;
	LOG_INF("RGr_unit=0x%x,BGr_unit=0x%x\n",RGr_unit,BGr_unit);
	LOG_INF("RGr_golden=0x%x,BGr_golden=0x%x\n",RGr_golden,BGr_golden);
	R_gain=RGr_golden*256/RGr_unit;
	B_gain=BGr_golden*256/BGr_unit;
	if(R_gain<B_gain)
	{
		if(R_gain<256)
		{
			B_gain=B_gain*256/R_gain;
			G_gain=G_gain*256/R_gain;
			R_gain=256;
		}
	}
	else
	{
		if(B_gain<256)
		{
			R_gain=R_gain*256/B_gain;
			G_gain=G_gain*256/B_gain;
			B_gain=256;
		}
	}
	...
	for(i=0;i<=7;i++)
	{
		write_cmos_sensor_8((0x0078+i),data_gain[i]);
	}
	for(i=0;i<=7;i++)
	{
		LOG_INF("data_gain_read[%d]=0x%x\n",i,read_cmos_sensor(0x0078+i)); 
	}
...
{    
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	LOG_INF("[chen][hi556_get_imgsensor_id begin]\n ");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
			...
			do {
				*sensor_id =return_sensor_id();
				...
				sprintf(Front_Camera_Name,"HI556:Dmegc:5M_FF"); 
