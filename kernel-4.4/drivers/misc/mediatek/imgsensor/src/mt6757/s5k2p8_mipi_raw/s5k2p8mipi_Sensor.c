...
static SET_PD_BLOCK_INFO_T imgsensor_pd_info = {
	...
	.i4SubBlkH = 16,
	.i4PosL = {
		{8, 35}, {60, 35}, {24, 39}, {44, 39}, {12, 55}, {56, 55}, {28, 59}, {40, 59},
		{28, 67}, {40, 67}, {12, 71}, {56, 71}, {24, 87}, {44, 87}, {8, 91}, {60, 91}
	},
	.i4PosR = {
		{8, 39}, {60, 39}, {24, 43}, {44, 43}, {12, 51}, {56, 51}, {28, 55}, {40, 55},
		{28, 71}, {40, 71}, {12, 75}, {56, 75}, {24, 83}, {44, 83}, {8, 87}, {60, 87}
	},
	.i4BlockNumX = 83,
	.i4BlockNumY = 46,
};
...
static void sensor_init(void)
{
	...
	if (imgsensor_info.sensor_output_dataformat == SENSOR_OUTPUT_FORMAT_RAW_RWB_Wr)
		S5K2P8MIPI_table_write_cmos_sensor(addr_data_pair_init_RWB,
						   sizeof(addr_data_pair_init_RWB) /
						   sizeof(kal_uint16));
	else
		S5K2P8MIPI_table_write_cmos_sensor(addr_data_pair_init,
						   sizeof(addr_data_pair_init) /
						   sizeof(kal_uint16));
#else
...
#if  MULTI_WRITE
	S5K2P8MIPI_table_write_cmos_sensor(addr_data_pair_preview,
...
static void normal_capture_setting(void)
{
	...
	write_cmos_sensor(0x0100, 0x0000);
#if MULTI_WRITE
	S5K2P8MIPI_table_write_cmos_sensor(addr_data_pair_capture,
...
static void pip_capture_setting(void)
{
	...
	write_cmos_sensor(0x0100, 0x0000);
#if MULTI_WRITE
	S5K2P8MIPI_table_write_cmos_sensor(addr_data_pair_capture_pip,
					   sizeof(addr_data_pair_capture_pip) / sizeof(kal_uint16));
#else
#ifdef FIX_VIEW_ANGLE
	write_cmos_sensor(0x0344, 0x0018);	/* smiaRegs_rw_frame_timing_x_addr_start */
	write_cmos_sensor(0x0346, 0x0014);	/* smiaRegs_rw_frame_timing_y_addr_start */
...
static void pip_capture_15fps_setting(void)
{
	...
	write_cmos_sensor(0x0100, 0x0000);
#if MULTI_WRITE
	S5K2P8MIPI_table_write_cmos_sensor(addr_data_pair_capture_pip_15,
					   sizeof(addr_data_pair_capture_pip_15) /
					   sizeof(kal_uint16));
#else
#ifdef FIX_VIEW_ANGLE
...
static kal_uint32 open(void)
{
	...
	if ((imgsensor_info.sensor_id != sensor_id) && (sensor_id != 0x2102))
		return ERROR_SENSOR_CONNECT_FAIL;
						...
						MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps == 300) {
			...
			imgsensor.min_frame_length = imgsensor.frame_length;
			spin_unlock(&imgsensor_drv_lock);
		} else if (imgsensor.current_fps == 150) {
			/* 317Mhz, PIP capture: 15fps for less than 13M, 20fps for 16M,15fps for 20M */
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:	/* For PDAF EEPROM Read Information */
		 ...
		 */
		read_eeprom(0x763, (char *)(uintptr_t) (*(feature_data + 1)), 2048);
		break;
