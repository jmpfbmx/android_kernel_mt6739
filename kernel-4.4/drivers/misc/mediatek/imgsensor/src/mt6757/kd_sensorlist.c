...
#endif
static int g_seninfWakeLockCnt;
...
#if !defined(CONFIG_MTK_LEGACY)
/*PMIC*/
struct regulator *regVCAMA;
struct regulator *regVCAMD;
struct regulator *regVCAMIO;
struct regulator *regVCAMAF;
struct regulator *regSubVCAMA;
struct regulator *regSubVCAMD;
struct regulator *regSubVCAMIO;
struct regulator *regMain2VCAMA;
struct regulator *regMain2VCAMD;
struct regulator *regMain2VCAMIO;
#endif
...
#define PK_INF(fmt, args...)     pr_debug("[%s] " fmt, __func__, ##args)
...
static inline void KD_IMGSENSOR_PROFILE_INIT_I2C(void)
{
}

static inline void KD_IMGSENSOR_PROFILE_I2C(char *tag, int trans_num)
{
}
#endif
...
#endif
#ifdef MTK_SUB2_IMGSENSOR
static struct i2c_client *g_pstI2Cclient4;
#endif
...
#endif
#ifdef MTK_SUB2_IMGSENSOR
static const struct i2c_device_id CAMERA_HW_i2c_id4[] = {{CAMERA_HW_DRVNAME4, 0}, {} };
#endif
...
#ifdef MTK_SUB2_IMGSENSOR
	else if (gI2CBusNum == SUPPORT_I2C_BUS_NUM4) {
		spin_lock(&kdsensor_drv_lock);
		g_pstI2Cclient4->addr = (i2cId >> 1);
		spin_unlock(&kdsensor_drv_lock);

		i4RetValue = i2c_master_send(g_pstI2Cclient4, puReadCmd, 2);
		if (i4RetValue != 2) {
			PK_ERR("[CAMERA SENSOR] I2C send failed, addr = 0x%x, data = 0x%x !!\n", a_u2Addr, *a_puBuff);
			return -1;
		}

		i4RetValue = i2c_master_recv(g_pstI2Cclient4, (char *)a_puBuff, number);
		if (i4RetValue != 1) {
			PK_ERR("[CAMERA SENSOR] I2C read failed!!\n");
			return -1;
		}
...
int iReadReg(u16 a_u2Addr, u8 *a_puBuff, u16 i2cId)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		/* Remove i2c ack error log during search sensor */
		if (g_IsSearchSensor == 1)
			g_pstI2Cclient4->ext_flag = (g_pstI2Cclient4->ext_flag) | I2C_A_FILTER_MSG;
		else
			g_pstI2Cclient4->ext_flag = (g_pstI2Cclient4->ext_flag)&(~I2C_A_FILTER_MSG);
#endif
		spin_unlock(&kdsensor_drv_lock);
		...
		i4RetValue = i2c_master_recv(g_pstI2Cclient4, (char *)a_puBuff, 1);
		if (i4RetValue != 1) {
			PK_ERR("[CAMERA SENSOR] I2C read failed!!\n");
			return -1;
		}
...
#endif
			spin_unlock(&kdsensor_drv_lock);
			i4RetValue = i2c_master_send(g_pstI2Cclient4, a_pSendData, a_sizeSendData);
			if (i4RetValue != a_sizeSendData) {
				PK_ERR("[CAMERA SENSOR] I2C send failed!!, Addr = 0x%x\n", a_pSendData[0]);
				return -1;
			}

			i4RetValue = i2c_master_recv(g_pstI2Cclient4, (char *)a_pRecvData, a_sizeRecvData);
			if (i4RetValue != a_sizeRecvData) {
				PK_ERR("[CAMERA SENSOR] I2C read failed!!\n");
				return -1;
			}
...
int iWriteReg(u16 a_u2Addr, u32 a_u4Data, u32 a_u4Bytes, u16 i2cId)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		g_pstI2Cclient4->addr = (i2cId >> 1);
#ifdef CONFIG_MTK_I2C_EXTENSION
		g_pstI2Cclient4->ext_flag = (g_pstI2Cclient4->ext_flag)&(~I2C_DMA_FLAG);
#endif
	}
#endif
	else {
	...
	do {
		if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1)
			...
			i4RetValue = i2c_master_send(g_pstI2Cclient4, puSendCmd, (a_u4Bytes + 2));
#endif
		else
...
int iWriteRegI2C(u8 *a_pSendData, u16 a_sizeSendData, u16 i2cId)
{
	...
	if (g_IsSearchSensor != 1) {
		...
		if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1)
			pClient = g_pstI2Cclient;
#ifdef MTK_SUB2_IMGSENSOR
		else if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1)
			pClient = g_pstI2Cclient4;
#endif
		else
			pClient = g_pstI2Cclient2;
...
int kdSetI2CBusNum(u32 i2cBusNum)
{
	...
	if ((i2cBusNum != SUPPORT_I2C_BUS_NUM3) && (i2cBusNum != SUPPORT_I2C_BUS_NUM2)
...
int iBurstWriteReg_multi(u8 *pData, u32 bytes, u16 i2cId, u16 transfer_length, u16 timing)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		buf = dma_alloc_coherent(&(pCamerahw_platform_device->dev), bytes,
		...
		PK_DBG("transfer_length %d\n",
		       ...
		       : (((bytes / transfer_length) << 16) | transfer_length));
...
int iMultiWriteReg(u8 *pData, u16 lens, u16 i2cId)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		...
		g_pstI2Cclient4->ext_flag = (g_pstI2Cclient4->ext_flag)|(I2C_DMA_FLAG);
#endif
		ret = i2c_master_send(g_pstI2Cclient4, pData, lens);
	}
#endif
	else {
		g_pstI2Cclient2->addr = (i2cId >> 1);
...
void set_seninf_wake_lock(int enable)
{
	if (enable == 1) {		/* Enable	  wakelock */
		if (g_seninfWakeLockCnt == 0) {
#ifdef CONFIG_PM_WAKELOCKS
			__pm_stay_awake(&seninf_wake_lock);
#else
			wake_lock(&seninf_wake_lock);
#endif
			PK_INF("seninf_wake_lock enable!!\n");
		}
		g_seninfWakeLockCnt += 1;

	} else {		/* Disable wakelock */
			g_seninfWakeLockCnt -= 1;
		if (g_seninfWakeLockCnt == 0) {
#ifdef CONFIG_PM_WAKELOCKS
			__pm_relax(&seninf_wake_lock);
#else
			wake_unlock(&seninf_wake_lock);
#endif
			PK_INF("seninf_wake_lock disable!!\n");

		}
	}
	PK_INF("g_seninfWakeLockCnt %d\n", g_seninfWakeLockCnt);
...
#else
				if (g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_2_SENSOR) {
					spin_lock(&kdsensor_drv_lock);
...
#ifdef MTK_SUB2_IMGSENSOR
				else if (g_invokeSocketIdx[i] == DUAL_CAMERA_SUB_2_SENSOR) {
					spin_lock(&kdsensor_drv_lock);
					gI2CBusNum = SUPPORT_I2C_BUS_NUM4;
					spin_unlock(&kdsensor_drv_lock);
					PK_XLOG_INFO("kd_MultiSensorOpen: switch I2C BUS%d\n", gI2CBusNum);
				}
#endif
...
#endif
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_SENSOR) {
				ret = g_pInvokeSensorFunc[i]->SensorGetInfo((MSDK_SCENARIO_ID_ENUM) (*pScenarioId[0]),
									    pSensorInfo[0], pSensorConfigData[0]);
			} else if ((g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_2_SENSOR)
				   || (g_invokeSocketIdx[i] == DUAL_CAMERA_SUB_SENSOR)) {
				ret = g_pInvokeSensorFunc[i]->SensorGetInfo((MSDK_SCENARIO_ID_ENUM) (*pScenarioId[1]),
									    pSensorInfo[1], pSensorConfigData[1]);
#ifdef MTK_SUB2_IMGSENSOR
			} else if (g_invokeSocketIdx[i] == DUAL_CAMERA_SUB_2_SENSOR) {
				ret = g_pInvokeSensorFunc[i]->SensorGetInfo((MSDK_SCENARIO_ID_ENUM)(*pScenarioId[2]),
						pSensorInfo[2], pSensorConfigData[2]);
#endif
			}
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			...
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				...
				/* KD_SET_I2C_SLAVE_ID(i,g_invokeSocketIdx[i],IMGSENSOR_SET_I2C_ID_STATE); */
				/*  */
				ret = g_pInvokeSensorFunc[i]->SensorFeatureControl(FeatureId,
										   pFeaturePara,
										   pFeatureParaLen);
				if (ret != ERROR_NONE) {
					PK_ERR("[%s]\n", __func__);
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		if (g_bEnableDriver[i] && g_pInvokeSensorFunc[i]) {
			if (InvokeCamera == g_invokeSocketIdx[i]) {
				...
				ret = g_pInvokeSensorFunc[i]->SensorControl(ScenarioId, pImageWindow,
									    pSensorConfigData);
				if (ret != ERROR_NONE) {
...
int kdSetDriver(unsigned int *pDrvIndex)
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		...
		g_invokeSocketIdx[i] = (CAMERA_DUAL_CAMERA_SENSOR_ENUM)
			((pDrvIndex[i] & KDIMGSENSOR_DUAL_MASK_MSB) >> KDIMGSENSOR_DUAL_SHIFT);
		spin_unlock(&kdsensor_drv_lock);
...
static inline int adopt_CAMERA_HW_Open(void)
{
	...
	/* turn on power */
	/* kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM*) g_invokeSocketIdx, g_invokeSensorNameStr,true,
	 * CAMERA_HW_DRVNAME);
	 */
	/* wait for power stable */
	/* mDELAY(10); */
...
static inline int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	if (g_pSensorFunc) {
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (g_invokeSocketIdx[i] != DUAL_CAMERA_NONE_SENSOR) {
				err = g_pSensorFunc->SensorFeatureControl(g_invokeSocketIdx[i],
...
#ifdef MTK_SUB2_IMGSENSOR
	pRes[2] = (ACDK_SENSOR_RESOLUTION_INFO_STRUCT *)
		kmalloc(sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT), GFP_KERNEL);
	if (pRes[2] == NULL) {
		kfree(pRes[0]);
		kfree(pRes[1]);
...
#ifdef MTK_SUB2_IMGSENSOR
		if (copy_to_user((void __user *) (pBufResolution->pResolution[2]), (void *)pRes[2],
				 sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT))) {
			PK_ERR("copy to user failed\n");
		}
#endif
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if ((pSensorGetInfo->pInfo[0] == NULL) || (pSensorGetInfo->pInfo[1] == NULL) ||
	    (pSensorGetInfo->pInfo[2] == NULL) || (pSensorGetInfo->pConfig[0] == NULL) ||
	    (pSensorGetInfo->pConfig[0] == NULL) || (pSensorGetInfo->pConfig[2] == NULL)) {
#else
	if ((pSensorGetInfo->pInfo[0] == NULL) || (pSensorGetInfo->pInfo[1] == NULL) ||
	...
	for (i = 0; i < 2; i++) {
#endif
		/* SenorInfo */
		if (copy_to_user((void __user *)(pSensorGetInfo->pInfo[i]), (void *)pInfo[i],
		...
		/* SensorConfig */
		if (copy_to_user((void __user *)(pSensorGetInfo->pConfig[i]), (void *)pConfig[i],
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (pConfig[0] == NULL || pConfig[1] == NULL || pConfig1[0] == NULL || pConfig1[1] == NULL
	    || pConfig2[0] == NULL || pConfig2[1] == NULL || pConfig3[0] == NULL
	    || pConfig3[1] == NULL || pConfig4[0] == NULL || pConfig4[1] == NULL
	    || pSensorInfo == NULL || psensorResolution[0] == NULL
	    || psensorResolution[1] == NULL)
#endif
	{
		PK_ERR(" ioctl allocate mem failed\n");
#ifdef MTK_SUB2_IMGSENSOR
		...
		for (i = 0; i < 2; i++) {
			...
			if (pConfig4[i] != NULL)
				kfree(pConfig4[i]);
			if (psensorResolution[i] != NULL)
				kfree(psensorResolution[i]);
			pConfig[i] = NULL;
			pConfig1[i] = NULL;
			pConfig2[i] = NULL;
			pConfig3[i] = NULL;
	...
	if (pSensorGetInfo->SensorId == DUAL_CAMERA_MAIN_SENSOR)
		IDNum = 0;
#ifdef MTK_SUB2_IMGSENSOR
	else if (pSensorGetInfo->SensorId == DUAL_CAMERA_SUB_2_SENSOR)
		IDNum = 2;
#endif
	else
		IDNum = 1;
...
g_pSensorFunc->SensorGetInfo(pScenarioId, pInfo2, pConfig2);
	...
	if (copy_to_user((void __user *)(pSensorGetInfo->pInfo), (void *)(pSensorInfo),
	...
	PK_DBG("[CAMERA_HW][Pre]w=0x%x, h = 0x%x\n", psensorResolution[0]->SensorPreviewWidth,
	       psensorResolution[0]->SensorPreviewHeight);
	PK_DBG("[CAMERA_HW][Full]w=0x%x, h = 0x%x\n", psensorResolution[0]->SensorFullWidth,
	       psensorResolution[0]->SensorFullHeight);
	PK_DBG("[CAMERA_HW][VD]w=0x%x, h = 0x%x\n", psensorResolution[0]->SensorVideoWidth,
	       psensorResolution[0]->SensorVideoHeight);
	...
	if (pSensorGetInfo->SensorId == DUAL_CAMERA_MAIN_SENSOR) {
		...
		/* Resolution */
		PK_DBG("[adopt_CAMERA_HW_GetInfo2]Resolution\n");
		if (copy_to_user((void __user *) (pSensorGetInfo->pSensorResolution), (void *)psensorResolution[2],
						sizeof(MSDK_SENSOR_RESOLUTION_INFO_STRUCT))) {
			PK_DBG("[CAMERA_HW][Resolution] ioctl copy to user failed\n");
			for (i = 0; i < 3; i++) {
				if (pConfig[i] != NULL)
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	if (copy_from_user((void *)&imageWindow, (void *)pSensorCtrl->pImageWindow,
			   sizeof(ACDK_SENSOR_EXPOSURE_WINDOW_STRUCT))) {
		PK_DBG("[CAMERA_HW][pFeatureData32] ioctl copy from user failed\n");
		return -EFAULT;
	}

	if (copy_from_user((void *)&sensorConfigData, (void *)pSensorCtrl->pSensorConfigData,
			   sizeof(ACDK_SENSOR_CONFIG_STRUCT))) {
		PK_DBG("[CAMERA_HW][pFeatureData32] ioctl copy from user failed\n");
		return -EFAULT;
	}
	...
	/*  */
	if (g_pSensorFunc) {
		ret = g_pSensorFunc->SensorControl(pSensorCtrl->InvokeCamera, pSensorCtrl->ScenarioId,
	...
	/*  */
	if (copy_to_user((void __user *)pSensorCtrl->pImageWindow, (void *)&imageWindow,
	...
	/*  */
	if (copy_to_user((void __user *)pSensorCtrl->pSensorConfigData, (void *)&sensorConfigData,
			 sizeof(MSDK_SENSOR_CONFIG_STRUCT))) {
		PK_DBG("[CAMERA_HW][imageWindow] ioctl copy to user failed\n");
		return -EFAULT;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_SET_DUAL_GAIN:
		...
		spin_unlock(&kdsensor_drv_lock);
		/* fall through */
	case SENSOR_FEATURE_SET_WAKE_LOCK:
	...
	case SENSOR_FEATURE_GET_SENSOR_HDR_CAPACITY:
		{
			...
			*(pFeaturePara_64 + 1) = (uintptr_t) pValue;
			...
			if (g_pSensorFunc) {
				ret = g_pSensorFunc->SensorFeatureControl(pFeatureCtrl->InvokeCamera,
	...
	case SENSOR_FEATURE_GET_EV_AWB_REF:
		{
			...
			*(pFeaturePara_64) = (uintptr_t) pAeAwbRef;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pAeAwbRef,
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		{
			...
			*(pFeaturePara_64 + 1) = (uintptr_t) pCrop;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pCrop,
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		{
			...
			*(pFeaturePara_64 + 1) = (uintptr_t) pVcInfo;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pVcInfo,
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		{
			...
			*(pFeaturePara_64 + 1) = (uintptr_t) pPdInfo;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pPdInfo,
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		{
			...
			if (copy_to_user((void __user *)usr_ptr_Reg, (void *)pReg,
	...
	case SENSOR_FEATURE_GET_EXIF_INFO:
		{
			...
			*(pFeaturePara_64) = (uintptr_t) pExif;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pExif,
	...
	case SENSOR_FEATURE_GET_SHUTTER_GAIN_AWB_GAIN:
		{
			...
			*(pFeaturePara_64) = (uintptr_t) pCurAEAWB;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pCurAEAWB,
	...
	case SENSOR_FEATURE_GET_DELAY_INFO:
		{
			...
			*(pFeaturePara_64) = (uintptr_t) pDelayInfo;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pDelayInfo,
	...
	case SENSOR_FEATURE_GET_AE_FLASHLIGHT_INFO:
		{
			...
			*(pFeaturePara_64) = (uintptr_t) pFlashInfo;
			...
			if (copy_to_user((void __user *)usr_ptr, (void *)pFlashInfo,
	...
	case SENSOR_FEATURE_SET_I2C_BUF_MODE_EN:
		...
		break;
	case SENSOR_FEATURE_SET_WAKE_LOCK:
		set_seninf_wake_lock((*(unsigned long long *)pFeaturePara));
		break;
	case SENSOR_FEATURE_SET_ESHUTTER:
	case SENSOR_FEATURE_SET_GAIN:
	case SENSOR_FEATURE_SET_DUAL_GAIN:
	...
	case SENSOR_FEATURE_SET_PDFOCUS_AREA:
		/*  */
		if (copy_to_user((void __user *)pFeatureCtrl->pFeaturePara, (void *)pFeaturePara,
				 FeatureParaLen)) {
			kfree(pFeaturePara);
	...
	case SENSOR_FEATURE_GET_GROUP_INFO:
		/* copy 32 bytes */
		if (copy_to_user((void __user *)pUserGroupNamePtr, (void *)kernelGroupNamePtr,
				 sizeof(char) * 32)) {
			kfree(pFeaturePara);
	...
	kfree(pFeaturePara);
	if (copy_to_user((void __user *)pFeatureCtrl->pFeatureParaLen, (void *)&FeatureParaLen,
			 sizeof(unsigned int))) {
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	case KDIMGSENSORIOC_X_GET_ISP_CLK:
		...
		if (mmdvfs_get_stable_isp_clk() == MMSYS_CLK_HIGH)	/*1(High):400, 2(Medium):320 */
			*(unsigned int *)pBuff = 400;
		else
	...
	case KDIMGSENSORIOC_X_GET_CSI_CLK:
		...
		PK_DBG("abist_meter=%d %d %d %d %d\n", abist_meter(54), abist_meter(55),
		       abist_meter(56), abist_meter(57), abist_meter(58));
...
int mmsys_clk_change_cb(int ori_clk_mode, int new_clk_mode)
{
	PK_DBG("mmsys_clk_change_cb ori: %d, new: %d, current_mmsys_clk %d\n", ori_clk_mode,
	       new_clk_mode, current_mmsys_clk);
...
#endif
	PK_INF("kd_sensorlist release g_seninfWakeLockCnt %d\n", g_seninfWakeLockCnt);
	if (atomic_read(&g_CamDrvOpenCnt) == 0) {
		if (g_seninfWakeLockCnt > 0) {
#ifdef CONFIG_PM_WAKELOCKS
			__pm_relax(&seninf_wake_lock);
...
#ifdef MTK_SUB2_IMGSENSOR
static int CAMERA_HW_i2c_probe4(struct i2c_client *client, const struct i2c_device_id *id)
{
	/* int i4RetValue = 0; */
	PK_DBG("[CAMERA_HW] Attach I2C for HW trriger g_pstI2Cclient4 %p\n", client);

	spin_lock(&kdsensor_drv_lock);

	/* get sensor i2c client */
	g_pstI2Cclient4 = client;

	/* set I2C clock rate */
	#ifdef CONFIG_MTK_I2C_EXTENSION
	g_pstI2Cclient4->timing = 100;/* 100k */
	g_pstI2Cclient4->ext_flag &= ~I2C_POLLING_FLAG; /* No I2C polling busy waiting */
	#endif
	spin_unlock(&kdsensor_drv_lock);
#if 0
	/* Register char driver */
	i4RetValue = RegisterCAMERA_HWCharDrv4();
...
static int CAMERA_HW_i2c_remove4(struct i2c_client *client)
{
	return 0;
}
#endif
...
#ifdef MTK_SUB2_IMGSENSOR
#ifdef CONFIG_OF
static const struct of_device_id CAMERA_HW4_i2c_driver_of_ids[] = {
	{ .compatible = "mediatek,camera_main_two", },
	{}
};
#endif

struct i2c_driver CAMERA_HW_i2c_driver4 = {
	.probe = CAMERA_HW_i2c_probe4,
	.remove = CAMERA_HW_i2c_remove4,
	.driver = {
	.name = CAMERA_HW_DRVNAME4,
	.owner = THIS_MODULE,
#ifdef CONFIG_OF
	.of_match_table = CAMERA_HW4_i2c_driver_of_ids,
#endif
	},
	.id_table = CAMERA_HW_i2c_id4,
};
#endif
...
#ifdef CONFIG_PM_WAKELOCKS
	wakeup_source_init(&seninf_wake_lock, "seninf_lock_wakelock");
#else
	wake_lock_init(&seninf_wake_lock, WAKE_LOCK_SUSPEND, "seninf_lock_wakelock");
#endif
...
#endif
#ifdef MTK_SUB2_IMGSENSOR
	i2c_add_driver(&CAMERA_HW_i2c_driver4);
#endif
...
#endif
#ifdef	MTK_SUB2_IMGSENSOR
	i2c_del_driver(&CAMERA_HW_i2c_driver4);
#endif
...
static ssize_t proc_set_pdaf_type_write(struct file *file, const char *buffer, size_t count,
					loff_t *data)
{
	char regBuf[64] = { '\0' };
	u32 u4CopyBufSize = (count < (sizeof(regBuf) - 1)) ? (count) : (sizeof(regBuf) - 1);
