...
static bool selective_read_byte(u32 addr, u8 *data)
{
	...
	if (iReadRegI2C(&offset, 1, (u8 *) data, 1, IMX300_EEPROM_READ_ID + (page << 1)) < 0) {
		LOG_INF("jeff fail selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x",
			addr, *data, page, offset);
		return false;
	}
	LOG_INF("jeff selective_read_byte addr =0x%x data = 0x%x,page %d, offset 0x%x", addr, *data,
		page, offset);
	return true;
}
...
void imx300_DCC_conversion(u16 addr, u8 *data, u32 size)
{
	...
	for (i = 0; i < IMX300_DCC_DATA_SIZE; i++) {
		dataCon = (IMX300_DCC_data_3point5[i] << 8) * (IMX300_DCC_CONVERT_COEFFICIENT) / act_sensitivity;
		LOG_INF("dataCon  (%lld)\n", dataCon);
