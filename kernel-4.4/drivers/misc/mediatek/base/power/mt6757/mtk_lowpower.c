...
{
	clk_writel(ARMPLL_LL_PWR_CON0_1, (clk_readl(ARMPLL_LL_PWR_CON0_1) | 0x01));
	udelay(1);
...
{
	clk_writel(ARMPLL_LL_CON0_1, (clk_readl(ARMPLL_LL_CON0_1) & 0xfffffffe));
	clk_writel(ARMPLL_LL_PWR_CON0_1,
...
{
	clk_writel(ARMPLL_L_PWR_CON0_1, (clk_readl(ARMPLL_L_PWR_CON0_1) | 0x01));
	udelay(1);
...
void switch_armpll_l_hwmode(int enable)
{
	/* ARM CA15*/
	if (enable == 1) {
		clk_writel(AP_PLL_CON3_1, (clk_readl(AP_PLL_CON3_1) & 0xFFFEEEEF));
		clk_writel(AP_PLL_CON4_1, (clk_readl(AP_PLL_CON4_1) & 0xFFFFFFFE));

	} else{
		clk_writel(AP_PLL_CON3_1, (clk_readl(AP_PLL_CON3_1) | 0x00011110));
		clk_writel(AP_PLL_CON4_1, (clk_readl(AP_PLL_CON4_1) | 0x00000001));
	}
}
...
void switch_armpll_ll_hwmode(int enable)
{
	/* ARM CA7*/
	if (enable == 1) {
		clk_writel(AP_PLL_CON3_1, (clk_readl(AP_PLL_CON3_1) & 0xFF87FFFF));
		clk_writel(AP_PLL_CON4_1, (clk_readl(AP_PLL_CON4_1) & 0xFFFFDFFF));
	} else {
		clk_writel(AP_PLL_CON3_1, (clk_readl(AP_PLL_CON3_1) | 0x00780000));
		clk_writel(AP_PLL_CON4_1, (clk_readl(AP_PLL_CON4_1) | 0x00002000));
	}
}
					 ...
					 unsigned long action, void *hcpu)
{
	...
	case CPU_UP_PREPARE_FROZEN:
		if (cpu < cpumask_weight(cpu_cluster0_mask)) {
			first_cpu = cpumask_first_and(cpu_online_mask, cpu_cluster0_mask);
			...
			first_cpu = cpumask_first_and(cpu_online_mask, cpu_cluster1_mask);
			if (first_cpu == CONFIG_NR_CPUS) {
				#ifdef CONFIG_ARMPLL_CTRL
				/* turn on arm pll */
				enable_armpll_l();
