...
static void get_all_clock_state(u32 clks[NR_GRPS])
{
	...
	if (sys_is_on(SYS_DIS) && (idle_readl(APMIXEDSYS(pll_grp[VENC_PLL].reg))&0x1)) {
...
bool cg_check_idle_can_enter(
	unsigned int *condition_mask, unsigned int *block_mask, enum mtk_idle_mode mode)
{
	int i;
...
{
	return pll_grp[id].name;
}

int is_pll_on(int id)
{
	return idle_readl(APMIXEDSYS(pll_grp[id].reg)) & 0x1;
}
