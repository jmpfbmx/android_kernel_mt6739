...
static ssize_t show_pwr_ctrl(const struct pwr_ctrl *pwrctrl, char *buf)
{
	...
	p += sprintf(p, "md_srcclkena_1_2d_dvfs_req_mask_b = %u\n",
		pwrctrl->md_srcclkena_1_2d_dvfs_req_mask_b);
	p += sprintf(p, "dvfs_up_2d_dvfs_req_mask_b = %u\n",
		pwrctrl->dvfs_up_2d_dvfs_req_mask_b);
	p += sprintf(p, "disable_off_load_lpm = %u\n",
...
static ssize_t store_pwr_ctrl(struct pwr_ctrl *pwrctrl, const char *buf, size_t count)
{
	...
	/* SPM_SW_RSV_6 */
		...
		pwrctrl->md_srcclkena_0_2d_dvfs_req_mask_b = val;
	else if (!strcmp(cmd, "md_srcclkena_1_2d_dvfs_req_mask_b"))
		pwrctrl->md_srcclkena_1_2d_dvfs_req_mask_b = val;
	else if (!strcmp(cmd, "dvfs_up_2d_dvfs_req_mask_b"))
		pwrctrl->dvfs_up_2d_dvfs_req_mask_b = val;
	else if (!strcmp(cmd, "disable_off_load_lpm"))
	...
	/* SPM_SW_RSV_6 version control */
		...
		pwrctrl->rsv6_legacy_version = val;
					...
					char *buf)
{
	...
	for (i = 0; i < 10; i++) {
		...
		if (__spm_suspend.wakestatus[i].event_reg != 0x90100000)
