...
#endif
/*0:disable, 1: enable : mtk_idle.c , mtk_spm_mcdi.c and mtk_cpuidle.c mtk_cpuidle.c */
#define MCDI_DVT_IPI 0
/*0:disable, 1: enable : GPT need to modify mtk_idle.c and mtk_spm_mcdi.c mtk_cpuidle.c */
#define MCDI_DVT_CPUxGPT 0
...
static bool mtk_idle_chk_golden;
static bool mtk_dpidle_chk_golden;

#ifdef CONFIG_HYBRID_CPU_DVFS
static bool mtk_dvfsp_paused_by_idle;
#endif
...
bool soidle3_can_enter(int cpu)
{
	...
	if (soidle3_by_pass_cg == 0) {
		...
		if (!cg_check_idle_can_enter(soidle3_condition_mask, soidle3_block_mask, MTK_SOIDLE)) {
			reason = BY_CLK;
			goto out;
		}
	}
	...
	if (soidle3_by_pass_i2c_appm_cg == 0) {
		...
		if (!cg_i2c_appm_check_idle_can_enter(soidle3_block_mask)) {
			...
			mtk_dvfsp_paused_by_idle = false;
#endif
...
out:
	...
	return mtk_idle_state_pick(IDLE_TYPE_SO3, cpu, reason);
}
...
bool soidle_can_enter(int cpu)
{
	...
	if (soidle_by_pass_cg == 0) {
		...
		if (!cg_check_idle_can_enter(soidle_condition_mask, soidle_block_mask, MTK_SOIDLE)) {
			reason = BY_CLK;
			goto out;
		}
	}
...
out:
	...
	return mtk_idle_state_pick(IDLE_TYPE_SO, cpu, reason);
}
...
#ifdef USING_STD_TIMER_OPS
#ifdef CONFIG_CPU_ISOLATION
	if (per_cpu(cpu_isolation_state, cpu) && !cpu_isolation_disable_mcdi == CPU_STATE_NEED_MCDI)
		goto mcidle_out;
	else {
		reason = BY_ISO;
		goto mcidle_out;
...
static bool dpidle_can_enter(int cpu)
{
	...
	if (dpidle_by_pass_cg == 0) {
		if (!mtk_cpufreq_earlysuspend_status_get()) {
			reason = BY_VTG;
			goto out;
		}
	}
	...
	if (dpidle_by_pass_cg == 0) {
		...
		if (!cg_check_idle_can_enter(dpidle_condition_mask, dpidle_block_mask, MTK_DPIDLE)) {
			reason = BY_CLK;
			goto out;
		}
	}
...
out:
	...
	return mtk_idle_state_pick(IDLE_TYPE_DP, cpu, reason);
}

void spm_dpidle_before_wfi(int cpu)
{
	if (mtk_dpidle_chk_golden) {
		/* FIXME: */
#if 0
		mtk_power_gs_dump_dpidle();
#endif
	}
	bus_dcm_enable();
	faudintbus_pll2sq();
...
static bool slidle_can_enter(void)
{
	...
	if (!cg_check_idle_can_enter(slidle_condition_mask, slidle_block_mask, MTK_SLIDLE)) {
		reason = BY_CLK;
		goto out;
	}
...
static inline void soidle3_update_flags(void)
{
	...
	if (univpll_is_off() && is_auxadc_released())
		...
		slp_spm_SODI3_flags |= SPM_FLAG_DIS_SRCCLKEN_LOW;
#elif defined(SODI3_AUXADC_CHECK)
			       ...
			       char __user *userbuf, size_t count, loff_t *f_pos)
{
	...
	p += snprintf(p, DBG_BUF_LEN - strlen(dbg_buf), "twam_handler:%s (clk:%s)\n",
			(mtk_idle_get_twam()->running)?"on":"off", (mtk_idle_get_twam()->speed_mode)?"speed":"normal");
...
{
	mtk_idle_twam_init();
	mtk_idle_block_setting(IDLE_TYPE_DP, dpidle_cnt, dpidle_block_cnt, dpidle_block_mask);
	mtk_idle_block_setting(IDLE_TYPE_SO3, soidle3_cnt, soidle3_block_cnt, soidle3_block_mask);
	mtk_idle_block_setting(IDLE_TYPE_SO, soidle_cnt, soidle_block_cnt, soidle_block_mask);
	mtk_idle_block_setting(IDLE_TYPE_SL, slidle_cnt, slidle_block_cnt, slidle_block_mask);
