...
char *spm_vcorefs_dump_dvfs_regs(char *p)
{
	...
	if (p) {
		...
		p += sprintf(p, "DVFSRC_EMI_MD2SPM0/1   : 0x%x / 0x%x\n",
							spm_read(DVFSRC_EMI_MD2SPM0), spm_read(DVFSRC_EMI_MD2SPM1));
...
static int spm_trigger_dvfs(int kicker, int opp, bool fix)
{
	...
	if (fix) {
		if (opp < 0) {
			spm_write(DVFSRC_BASIC_CONTROL, spm_read(DVFSRC_BASIC_CONTROL) & ~(1 << 15 | 1 << 14 | 1 << 8));
	...
	if (fix) {
		if (!(opp < 0)) {
			spm_write(DVFSRC_BASIC_CONTROL, spm_read(DVFSRC_BASIC_CONTROL) | (1 << 14));
			spm_write(DVFSRC_FORCE, 0);
		}
	}
