...
#define ARMPLL_LL_CON1		(apmixed_base + 0x314)	/* ARMPLL1 */
...
static unsigned int mt6355_vproc1_transfer2pmicval(unsigned int volt)
{
	return ((volt - 40625) + 625 - 1) / 625;
}

static unsigned int mt6355_vproc1_transfer2volt(unsigned int val)
{
	return val * 625 + 40625;
}

static unsigned int mt6355_vproc1_settletime(unsigned int old_volt, unsigned int new_volt)
{
...
static unsigned int mt6355_vsram1_transfer2pmicval(unsigned int volt)
{
	return ((volt - 51875) + 625 - 1) / 625;
}

static unsigned int mt6355_vsram1_transfer2volt(unsigned int val)
{
	return val * 625 + 51875;
}

static unsigned int mt6355_vsram1_settletime(unsigned int old_volt, unsigned int new_volt)
{
	if (new_volt > old_volt)
		return ((new_volt - old_volt) + 1250 - 1) / 1250 + PMIC_CMD_DELAY_TIME;
	else
...
static struct buck_ctrl_ops buck_ops_mt6355_vproc1 = {
	.get_cur_volt		= get_cur_volt_proc1_cpu,
	.set_cur_volt		= set_cur_volt_proc1_cpu,
	.transfer2pmicval	= mt6355_vproc1_transfer2pmicval,
	.transfer2volt		= mt6355_vproc1_transfer2volt,
	.settletime		= mt6355_vproc1_settletime,
};

static struct buck_ctrl_ops buck_ops_mt6355_vsram1 = {
	.get_cur_volt		= get_cur_volt_sram1_cpu,
	.set_cur_volt		= set_cur_volt_sram1_cpu,
	.transfer2pmicval	= mt6355_vsram1_transfer2pmicval,
	.transfer2volt		= mt6355_vsram1_transfer2volt,
	.settletime		= mt6355_vsram1_settletime,
};
...
struct buck_ctrl_t buck_ctrl[NR_MT_BUCK] = {
	[CPU_DVFS_VPROC1] = {
		...
		.buck_ops	= &buck_ops_mt6355_vproc1,
	},

	[CPU_DVFS_VSRAM1] = {
		.name		= __stringify(LDO_MT6355_VSRAM_PROC),
		.buck_id	= CPU_DVFS_VSRAM1,
		.buck_ops	= &buck_ops_mt6355_vsram1,
	},
};
...
void mt_cpufreq_turbo_action(unsigned long action, unsigned int *cpus, enum mt_cpu_dvfs_id cluster_id)
{
#ifdef CONFIG_HYBRID_CPU_DVFS
	if (cluster_id == MT_CPU_DVFS_L && cpus[MT_CPU_DVFS_L] == 1 && cpu_online(L_PLUS_CORE_ID)) {
...
int mt_cpufreq_turbo_config(enum mt_cpu_dvfs_id id, unsigned int turbo_f, unsigned int turbo_v)
{
	if (id != MT_CPU_DVFS_L)
		return 0;	/* try next cluster */
...
#ifdef CONFIG_HYBRID_CPU_DVFS
	turbo_flag = _GET_BITS_VAL_(3:3, get_devinfo_with_index(54));	/* 0x590[3] */
#endif

	tag_pr_info("level = %u, tb = %d\n", lv, turbo_flag);
