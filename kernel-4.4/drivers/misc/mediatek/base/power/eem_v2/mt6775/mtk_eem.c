...
static int get_devinfo(void)
{
	...
	val[10] = get_devinfo_with_index(DEVINFO_IDX_13);
	val[11] = get_devinfo_with_index(DEVINFO_IDX_14);
	val[12] = get_devinfo_with_index(DEVINFO_IDX_15);
	...
	aee_rr_rec_ptp_e11((unsigned int)val[12]);
	...
	eem_debug("M_HW_RES14 = 0x%08X\n", val[11]);
	eem_debug("M_HW_RES15 = 0x%08X\n", val[12]);

	FUNC_ENTER(FUNC_LV_HELP);
	eem_efuse_type = get_devinfo_with_index(DEVINFO_IDX_39) & 0x3;
...
#if ENABLE_LOO
	if ((detid == EEM_DET_L) || (detid == EEM_DET_B)) {
		...
		((detid == EEM_DET_B_HI) && (det != &eem_detector_cci)) /* For share cci bank */
		) {
		eem_write(EEM_FREQPCT30,
...
#if ENABLE_LOO
				if ((det->ctrl_id == EEM_CTRL_L) || (det->ctrl_id == EEM_CTRL_B)
...
static void eem_init_det(struct eem_det *det, struct eem_devinfo *devinfo)
{
	...
	case EEM_DET_L:
		...
		if (eem_checkEfuse == 0)
			...
			det->DVTFIXED = CPU_L_DVTFIXED_M_VAL;
		break;
	...
	case EEM_DET_B:
		...
		det->DVTFIXED = CPU_B_DVTFIXED_M_VAL;
		if (eem_checkEfuse == 0) {
			det->volt_offset = MARGIN_SAFE_EFUSE_ADD_OFF;
			det->features = FEA_INIT01 | FEA_INIT02;
			det->VMAX = CPU_B_TYPT_B_VMAX_VAL;
			det->DVTFIXED = 3;
		} else if (eem_efuse_type == 3)
			det->VMAX = CPU_B_TYPT_B_VMAX_VAL;
		break;
#else
	case EEM_DET_L:
	...
	case EEM_DET_CCI:
		...
		if (det == &eem_detector_cci) {
			...
			/* for DVT B_HI input values are the same as CCI */
...
static void eem_set_eem_volt(struct eem_det *det)
{
#if SET_PMIC_VOLT
	unsigned i;
	int low_temp_offset = 0, new_clamp_off = 0;
	...
	FUNC_ENTER(FUNC_LV_HELP);
#if ENABLE_LOO
	/* remap to L/B bank for update dvfs table, also copy high opp volt table */
	/* Band HIGHL will update its volt table (opp0~7) to bank L */
	if (det->ctrl_id == EEM_CTRL_L_HI) {
		det = id_to_eem_det(EEM_DET_L);
		memcpy(det->volt_tbl, org_det->volt_tbl, sizeof(det->volt_tbl)/2);
		ctrl = id_to_eem_ctrl(det->ctrl_id);
	} else if ((det->ctrl_id == EEM_CTRL_B_HI) && (det != &eem_detector_cci)) {
		/* For share cci bank */
		det = id_to_eem_det(EEM_DET_B);
		memcpy(det->volt_tbl, org_det->volt_tbl, sizeof(det->volt_tbl)/2);
		ctrl = id_to_eem_ctrl(det->ctrl_id);
	}
#endif
	...
	for (i = 0; i < det->num_freq_tbl; i++) {
		...
		case EEM_CTRL_B:
			if ((eem_checkEfuse == 0) || (eem_efuse_type == 3))
				new_clamp_off = MARGIN_ADD_CLAMP;
			...
			(unsigned int)(clamp(
				...
				det->ops->eem_2_pmic(det, det->VMIN),
				det->ops->eem_2_pmic(det, det->VMAX))),
				det->volt_tbl_orig[i] + new_clamp_off);
		...
		default:
			eem_error("[eem_set_eem_volt] incorrect det :%s!!", det->name);
			break;
		}
#if 0
		eem_error("[%s].volt_tbl[%d] = 0x%X ----- Ori[0x%x] volt_tbl_pmic[%d] = 0x%X (%d)\n",
			det->name,
	...
	if (det->set_volt_to_upower == 0) {
		if (((det->ctrl_id == EEM_CTRL_L) &&
			...
			((final_init02_flag & EEM_B_INIT02_FLAG) == EEM_B_INIT02_FLAG)) ||
...
static void read_volt_from_VOP(struct eem_det *det)
{
	...
	if ((detid == EEM_DET_L) || (detid == EEM_DET_B)) {
		...
		((detid == EEM_DET_B_HI) && (det != &eem_detector_cci))
		) {
		temp = eem_read(EEM_VOP30);
		/* eem_debug("(%s_VOP30) = 0x%X\n", ((char *)(det->name) + 8), temp); */
		/* EEM_VOP30=>pmic value */
...
static inline void handle_init02_isr(struct eem_det *det)
{
	...
	if ((detid == EEM_DET_L_HI) ||
		...
		backupdet = (detid == EEM_DET_L_HI) ? id_to_eem_det(EEM_DET_L) : id_to_eem_det(EEM_DET_B);
		memcpy(backupdet->volt_tbl_init2, det->volt_tbl, sizeof(det->volt_tbl_init2)/2);
		final_init02_flag |= BIT(detid);
	} else if ((detid == EEM_DET_L) || (detid == EEM_DET_B))
		final_init02_flag |= BIT(detid);
#endif
	...
	for (i = 0; i < NR_FREQ; i++) {
		...
		/* case EEM_CTRL_CCI, for cci share bank */
			if ((det->ctrl_id == EEM_CTRL_B_HI) && (det != &eem_detector_cci)) {
				if (i < 8) {
...
#if ENABLE_LOO
	/* For share cci bank */
	if ((det == &eem_detector_cci) && (cci_init02_done == 0)) {
		cci_init02_done = 1;
		/* eem_error("First CCI init2 isr received\n"); */
		return;
	}
#endif
...
static inline void handle_mon_mode_isr(struct eem_det *det)
{
...
#ifdef CONFIG_THERMAL
	eem_debug("THERMAL_BANK0_temp=%d, THERMAL_BANK1temp=%d, THERMAL_BANK2_temp=%d, THERMAL_BANK3_temp=%d\n",
	...
	*/
	...
	if ((detid == EEM_DET_L_HI) &&
		...
		((final_init02_flag & EEM_B_INIT02_FLAG) == EEM_B_INIT02_FLAG)) {
		backupdet = id_to_eem_det(EEM_DET_B);
		if (backupdet->isTempInv == 1) {
			det->ops->switch_bank(backupdet, NR_EEM_PHASE);
			read_volt_from_VOP(backupdet);
			det->ops->switch_bank(det, NR_EEM_PHASE);
...
void eem_init02(const char *str)
{
	...
	/* run init02 except cci*/
	for_each_det_ctrl(det, ctrl) {
		if ((det->ctrl_id != EEM_CTRL_CCI) && HAS_FEATURE(det, FEA_INIT02)) {
			unsigned long flag;
	...
	det = id_to_eem_det(EEM_CTRL_B_HI);
	if (HAS_FEATURE(det, FEA_INIT02)) {
		unsigned long flag;
...
static int eem_buck_get(struct platform_device *pdev)
{
	...
	eem_regulator_vproc3 = regulator_get(&pdev->dev, "ext_buck_cpul");
	if (!eem_regulator_vproc3) {
		eem_error("eem_regulator_vproc3 error\n");
		return -EINVAL;
	}
...
static void eem_buck_set_mode(unsigned int mode)
{
	...
	if (mode) {
		...
		regulator_set_mode(eem_regulator_vproc3, REGULATOR_MODE_FAST);
	} else {
...
void eem_init01(void)
{
	...
	*/
	...
	while (1) {
		...
		/* For share cci bank */
		det = &eem_detector_cci;
		if (((out & BIT(det->ctrl_id)) == 0) && (det->eem_eemEn[EEM_PHASE_INIT01] == (1 | SEC_MOD_SEL)))
			out |= BIT(det->ctrl_id);
#endif
	...
	eem_detectors[EEM_DET_L_HI].DCVOFFSETIN = eem_detectors[EEM_DET_L].DCVOFFSETIN;
	eem_detectors[EEM_DET_L_HI].AGEVOFFSETIN = eem_detectors[EEM_DET_L].AGEVOFFSETIN;
	eem_detectors[EEM_DET_B_HI].DCVOFFSETIN = eem_detectors[EEM_DET_B].DCVOFFSETIN;
	eem_detectors[EEM_DET_B_HI].AGEVOFFSETIN = eem_detectors[EEM_DET_B].AGEVOFFSETIN;
#endif
	eem_init02(__func__);
	FUNC_EXIT(FUNC_LV_LOCAL);
}
