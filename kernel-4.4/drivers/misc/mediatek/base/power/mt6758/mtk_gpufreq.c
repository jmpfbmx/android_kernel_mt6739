...
#define GPU_DVFS_FREQ1	 (917000)	/* KHz */
#define GPU_DVFS_FREQ2	 (901000)	/* KHz */
#define GPU_DVFS_FREQ3	 (868000)	/* KHz */
#define GPU_DVFS_FREQ4	 (835000)	/* KHz */
#define GPU_DVFS_FREQ5	 (819000)	/* KHz */
#define GPU_DVFS_FREQ6	 (786000)	/* KHz */
#define GPU_DVFS_FREQ7	 (770000)	/* KHz */
#define GPU_DVFS_FREQ8	 (722000)	/* KHz */
#define GPU_DVFS_FREQ9	 (674000)	/* KHz */
#define GPU_DVFS_FREQ10	  (650000)	 /* KHz */
#define GPU_DVFS_FREQ11	  (602000)	 /* KHz */
#define GPU_DVFS_FREQ12	  (554000)	 /* KHz */
#define GPU_DVFS_FREQ13	  (502000)	 /* KHz */
#define GPU_DVFS_FREQ14	  (432000)	 /* KHz */
...
#define GPU_DVFS_VOLT4	 (83750)	/* mV x 100 */
...
#define GPU_DVFS_VSRAM4	  (93750)	/* mV x 100 */
...
#define GPU_DVFS_VSRAM11   (85000)	/* mV x 100 */
#define GPU_DVFS_VSRAM12   (85000)	/* mV x 100 */
#define GPU_DVFS_VSRAM13   (85000)	/* mV x 100 */
#define GPU_DVFS_VSRAM14   (85000)	/* mV x 100 */
#define GPU_DVFS_VSRAM15   (85000)	/* mV x 100 */
...
#define PMIC_MAX_VOLT_DIFF 26250
#define PMIC_MIN_VOLT_DIFF 10000

/* mt6759 us * 100 BEGIN */
#define DELAY_FACTOR_VSRAM  1000
#define DELAY_FACTOR_VGPU_R 1250
#define DELAY_FACTOR_VGPU_F 312
#define DELAY_FACTOR 625
#define HALF_DELAY_FACTOR 312
#define BUCK_INIT_US 750
#define VGPU_ENABLE_TIME_US 240 /* spec is 220, but more time means more safety */
...
static unsigned int g_vgpu_sfchg_rrate = 3; /* just a default value */
static unsigned int g_vgpu_sfchg_frate = 3; /* just a default value */
...
#endif
#define GPU_OFF_SETTLE_TIME_MS		(240)
...
{
#define GPU_ACT_REF_POWER		1283	/* mW  */
...
#else
	return (regulator_is_enabled(mt_gpufreq_pmic->reg_vgpu) << 1 |
		regulator_is_enabled(mt_gpufreq_pmic->reg_vsram));
#endif
}
EXPORT_SYMBOL(mt_gpufreq_query_volt_enable_state);

void mt_gpufreq_voltage_low_power_mode_set(unsigned int enable)
{
#ifdef MTK_SSPM
	mt_gpufreq_ap2sspm(IPI_GPU_DVFS_SET_PMIC_LOW_POWER_MODE, enable, 0);
#else
	if (enable && !mt_gpufreq_ptpod_disable) {
		pmic_config_interface_nolock(PMIC_RG_BUCK_VGPU_LP_ADDR,
			0x1, PMIC_RG_BUCK_VGPU_LP_MASK, PMIC_RG_BUCK_VGPU_LP_SHIFT);
		pmic_config_interface_nolock(PMIC_RG_LDO_VSRAM_GPU_LP_ADDR,
			0x1, PMIC_RG_LDO_VSRAM_GPU_LP_MASK, PMIC_RG_LDO_VSRAM_GPU_LP_SHIFT);
	} else {
		pmic_config_interface_nolock(PMIC_RG_LDO_VSRAM_GPU_LP_ADDR,
			0x0, PMIC_RG_LDO_VSRAM_GPU_LP_MASK, PMIC_RG_LDO_VSRAM_GPU_LP_SHIFT);
		pmic_config_interface_nolock(PMIC_RG_BUCK_VGPU_LP_ADDR,
			0x0, PMIC_RG_BUCK_VGPU_LP_MASK, PMIC_RG_BUCK_VGPU_LP_SHIFT);
	}
#endif
}
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
		...
		if (enable == 1) {
			...
			if (ret > 0)
				...
				if (ret > 0) {
					...
					gpufreq_err("FATAL: VSRAM_GPU regulator error !!!\n");
			}
		}
	...
	if (enable == BUCK_ON) {
		/* Enable VSRAM_VGPU */
		/*gpufreq_warn("start enable VSRAM_VGPU...\n");*/
		...
		/* Enable VGPU */
		/*gpufreq_warn("start enable VGPU...\n");*/
		...
		udelay(VGPU_ENABLE_TIME_US);
		 ...
		 /* Try Disable VGPU */
		do {
			/*gpufreq_warn("start disable VGPU...\n");*/
			...
			/*gpufreq_warn("start disable VSRAM_VGPU...\n");*/
	...
	if (!ret) {
		...
		if (g_pFreqSampler)
			g_pFreqSampler((mt_gpufreq_volt_enable_state == 0) ? 0 : g_cur_gpu_freq);
	}
#ifndef MTK_SSPM
SET_EXIT:
	mutex_unlock(&mt_gpufreq_lock);
...
unsigned int mtk_notify_sspm_fdvfs_gpu_pow_status(unsigned int enable)
{
	int ret = 0;
#ifdef MTK_SSPM
	ret = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_POW_STATUS_UPDATE, enable, 0);
#endif
	return ret;
}
EXPORT_SYMBOL(mtk_notify_sspm_fdvfs_gpu_pow_status);
...
unsigned int mt_gpufreq_update_volt(unsigned int pmic_volt[], unsigned int array_size)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		gpufreq_dbg("@%s: mt_gpufreqs[%d].gpufreq_volt = %d\n", __func__, i,
				mt_gpufreqs[i].gpufreq_volt);
		mt_gpufreqs[i].gpufreq_vsram = pmic_volt[i] + 10000;
		if (mt_gpufreqs[i].gpufreq_vsram < GPU_DVFS_VSRAM15)
			mt_gpufreqs[i].gpufreq_vsram = GPU_DVFS_VSRAM15;
	}
...
EXPORT_SYMBOL(mt_gpufreq_get_idx_by_target_freq);
...
static void mt_gpufreq_clock_switch_transient(unsigned int freq_new,  enum post_div_order_enum post_div_order)
{
	...
	*/
	...
	/* udelay(20); */
	DRV_WriteReg32(GPUPLL_CON1, ((DRV_Reg32(GPUPLL_CON1) & ~POST_DIV_MASK) | (post_div_order << POST_DIV_SHIFT)));
...
#ifndef MTK_SSPM
static unsigned int _calculate_vgpu_sfchg_rate(bool isRising)
{
...
static void mt_gpufreq_volt_switch(unsigned int volt_old, unsigned int volt_new)
{
	...
	/* unsigned int sfchg_rate_vgpu; */
	/* unsigned int sfchg_rate_vsram; */
	unsigned int old_vgpu;
	unsigned int old_vsram;
	unsigned int sram_volt_bound;
	unsigned int vgpu_volt_bound;
	unsigned int new_vsram;
	unsigned int new_vgpu;
	int i;
	...
	old_vsram = mt_gpufreqs[i].gpufreq_vsram;

	old_vgpu = volt_old;
	new_vgpu = volt_new;
#ifdef VGPU_SET_BY_PMIC
	if (new_vgpu > old_vgpu) {
		do {
			/* new_vsram = PMIC_MAX_VOLT_DIFF + old_vgpu; */
			sram_volt_bound = PMIC_MAX_VOLT_DIFF + old_vgpu;
			/* next_vsram = MIN( PMIC_MAX_VOLT_DIFF + volt_old), target_vsram); */

			new_vsram = (sram_volt_bound < target_vsram) ? sram_volt_bound : target_vsram;
			if (new_vsram > GPU_DVFS_VSRAM0)
				new_vsram = GPU_DVFS_VSRAM0;

			/* VSRAM */
			regulator_set_voltage(mt_gpufreq_pmic->reg_vsram,
								new_vsram*10, (PMIC_MAX_VSRAM_VGPU*10) + 125);
			max_diff = new_vsram - old_vsram;
			delay_unit_us = (max_diff / DELAY_FACTOR_VSRAM) + 1;
			gpufreq_dbg("1. udelay us(%d):\n", delay_unit_us);
			udelay(delay_unit_us);
			/* udelay(20); */
			old_vsram = new_vsram;

			/* VGPU */
			new_vgpu  = new_vsram - PMIC_MIN_VOLT_DIFF;
			regulator_set_voltage(mt_gpufreq_pmic->reg_vgpu,
								new_vgpu*10, (PMIC_MAX_VGPU*10) + 125);
			max_diff = new_vgpu - old_vgpu;
			delay_unit_us = (max_diff / DELAY_FACTOR_VGPU_R) + 1;
			gpufreq_dbg("2. udelay us(%d):\n", delay_unit_us+5);
			udelay(delay_unit_us+5);
			/* udelay(20); */
			old_vgpu = new_vgpu;
		} while (target_vsram > new_vsram);
	}
#if 0
	if (volt_new > volt_old) {
		...
		udelay(delay_unit_us+5);
			...
			target_vgpu = new_vgpu;
		do {
			vgpu_volt_bound = old_vsram - PMIC_MAX_VOLT_DIFF;
			new_vgpu = (vgpu_volt_bound > target_vgpu) ? vgpu_volt_bound : target_vgpu;

			/* VGPU */
			regulator_set_voltage(mt_gpufreq_pmic->reg_vgpu,
								new_vgpu*10, (PMIC_MAX_VGPU*10) + 125);
			max_diff = old_vgpu - new_vgpu;
			delay_unit_us = (max_diff / DELAY_FACTOR_VGPU_F) + 1;
			gpufreq_dbg("3. udelay us(%d):\n",	delay_unit_us+5);
			udelay(delay_unit_us+5);
			/* udelay(20); */
			old_vgpu = new_vgpu;

			new_vsram = new_vgpu + PMIC_MIN_VOLT_DIFF;
			new_vsram = (new_vsram > GPU_DVFS_VSRAM0) ? GPU_DVFS_VSRAM0 : new_vsram;
			new_vsram = (new_vsram < GPU_DVFS_VSRAM15) ? GPU_DVFS_VSRAM15 : new_vsram;

			/* VSRAM */
			regulator_set_voltage(mt_gpufreq_pmic->reg_vsram,
								new_vsram*10, (PMIC_MAX_VSRAM_VGPU*10) + 125);
			max_diff = old_vsram - new_vsram;
			delay_unit_us = (max_diff / DELAY_FACTOR_VSRAM) + 1;
			gpufreq_dbg("4. udelay us(%d):\n", delay_unit_us);
			udelay(delay_unit_us);
			/* udelay(20); */
			old_vsram = new_vsram;

		} while (target_vgpu < new_vgpu);
...
static unsigned int _mt_gpufreq_get_cur_freq(void)
{
	...
	freq_khz = (((dds * TO_MHz_TAIL + ROUNDING_VALUE)*GPUPLL_FIN) >> DDS_SHIFT)
		...
		if (freq_khz < DIV8_MIN_FREQ || freq_khz > DIV4_MAX_FREQ)
...
#define MT_GPUFREQ_OC_LIMIT_FREQ_1	 GPU_DVFS_FREQ9	/* no need to throttle when OC */
...
#define MT_GPUFREQ_LOW_BATT_VOLT_LIMIT_FREQ_2	 GPU_DVFS_FREQ14
...
#ifdef MTK_SSPM
			mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OC_CB, 1, 0);
#endif
...
#ifdef MTK_SSPM
			mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OC_CB, 0, 0);
#endif
...
#ifdef MTK_SSPM
			mt_gpufreq_ap2sspm(IPI_GPU_DVFS_LOW_BATT_VOLT_CB, 2, 0);
#endif
		}
	} else {		/* unlimit gpu */
		if (mt_gpufreq_low_batt_volt_limited_index !=
...
#ifdef MTK_SSPM
			mt_gpufreq_ap2sspm(IPI_GPU_DVFS_LOW_BATT_VOLT_CB, 0, 0);
#endif
		}
	}
}
...
{
#ifdef MTK_SSPM
	return mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, 0, QUERY_POWER);
#else
...
{
#ifdef MTK_SSPM
	return mt_gpufreq_ap2sspm(IPI_GPU_DVFS_OPPIDX_INFO, (mt_gpufreqs_num - 1), QUERY_POWER);
#else
	if (!mt_gpufreqs_power)
		return 0;
	else
...
static const struct of_device_id mt_gpufreq_of_match[] = {
	{.compatible = "mediatek,mt6758-gpufreq",},
	{ /* sentinel */ },
};
#endif
...
static int k_gpu_dvfs_host_recv_thread(void *arg)
{
	while (1) {
		...
		case IPI_GPU_DVFS_SSPM_KICK_PBM:
			...
			break;
#endif
		case IPI_GPU_DVFS_CUR_FREQ_UPDATE:
			/* mtk_fdvfs_update_cur_freq(gdvfs_rev_buf[1], gdvfs_rev_buf[2]); */
			/* ged_log_trace_counter("predict_frequence_sspm", gdvfs_rev_buf[1]); */
			/* mt_set_vfreq_target(gdvfs_rev_buf[1]); */
			/* ged_log_trace_counter("sum_gpu_sspm", gdvfs_rev_buf[2]); */
			break;
		}
...
static int mt_gpufreq_pdrv_probe(struct platform_device *pdev)
{
	...
	g_vgpu_sfchg_rrate = _calculate_vgpu_sfchg_rate(true); /* rising rate */
	g_vgpu_sfchg_frate = _calculate_vgpu_sfchg_rate(false); /* falling rate */
	...
	gpufreq_info("VGPU Enabled (%d) %d mV\n",
		...
		regulator_get_voltage(mt_gpufreq_pmic->reg_vgpu)/1000);
	...
	apmixed_node = of_find_compatible_node(NULL, NULL, "mediatek,mt6758-apmixedsys");
...
static int mt_gpufreq_opp_dump_proc_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < mt_gpufreqs_num; i++) {
		...
		seq_printf(m, "vgpu = %d, ", gpufreq_volt);
		seq_printf(m, "idx = %d\n", i);
	}
#else
	for (i = 0; i < mt_gpufreqs_num; i++) {
		...
		seq_printf(m, "vgpu = %d, ", mt_gpufreqs[i].gpufreq_volt);
		seq_printf(m, "vsram = %d, ", mt_gpufreqs[i].gpufreq_vsram);
	...
	if (kstrtoint(desc, 0, &fixed_freq) == 0) {
		if (fixed_freq == 0) {
			...
			if (mt_gpufreq_ap2sspm(IPI_GPU_DVFS_SET_FREQ, fixed_freq, SUB_CMD_FIX_FREQ) == 0) {
				...
				/* mt_set_vfreq_target(mt_gpufreq_keep_opp_index); */
			}
		}
	} else
#else
...
static int mt_gpufreq_var_dump_proc_show(struct seq_file *m, void *v)
{
	...
	/* This print real volt from PMIC */
	 seq_printf(m, "_mt_gpufreq_get_cur_volt = %d, mt_get_ckgen_freq = %d KHz\n",
		 mt_gpufreq_ap2sspm(IPI_GPU_DVFS_STATUS_OP, QUERY_CUR_VOLT, 0), mt_get_ckgen_freq(10));
	...
	*/
	...
	seq_printf(m, "_mt_gpufreq_get_cur_freq = %d, mt_get_ckgen_freq = %d KHz\n",
		_mt_gpufreq_get_cur_freq(), mt_get_ckgen_freq(10));
...
/*#ifdef BRING_UP*/
...
#ifdef MTK_SSPM
	gpu_fdvfs_phy_addr  = sspm_reserve_mem_get_phys(GPU_DVFS_MEM_ID);
	gpu_fdvfs_virt_addr = sspm_reserve_mem_get_virt(GPU_DVFS_MEM_ID);
	gpu_dvfs_mem_size   = sspm_reserve_mem_get_size(GPU_DVFS_MEM_ID);
	...
	if (mt_gpufreq_dvfs_table_type == 0)
		mt_setup_gpufreqs_table(mt_gpufreq_opp_tbl_e1_0,
					ARRAY_SIZE(mt_gpufreq_opp_tbl_e1_0));
	gpufreq_info("Init opptab on SSPM\n");
	mt_gpufreqs_num = ARRAY_SIZE(mt_gpufreq_opp_tbl_e1_0);

	for (i = 0; i < mt_gpufreqs_num; i++) {
		ui32combin[0] = ((mt_gpufreqs[i].gpufreq_khz / 1000) << 20) | mt_gpufreqs[i].gpufreq_vsram;
		ui32combin[1] = mt_gpufreqs[i].gpufreq_volt;
		mt_gpufreq_ap2sspm(IPI_GPU_DVFS_INIT_OPPTAB, ui32combin[0], ui32combin[1]);
	}
	...
	mt_gpufreqs_num = mt_gpufreq_ap2sspm(IPI_GPU_DVFS_INIT, mt_gpufreq_dvfs_table_type, gpu_fdvfs_phy_addr);
	if (mt_gpufreqs_num <= 0) {
