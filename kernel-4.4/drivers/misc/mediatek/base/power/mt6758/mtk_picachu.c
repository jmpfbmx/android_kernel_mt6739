...
#define PICACHU_PR_ERR(fmt, args...) pr_err(TAG"[ERROR]"fmt, ##args)
#define PICACHU_PR_INFO(fmt, args...) pr_info(TAG""fmt, ##args)
...
static unsigned int eem_ctrl_id_map[] = {
...
static int __init picachu_init(void)
{
	...
	for (i = 0; i < PICACHU_SUPPORT_CLUSTERS; i++) {
		...
		eem_set_pi_efuse(eem_ctrl_id_map[i],
