...
static int freq_idx_mapping_tbl_sb[COBRA_OPP_NUM] = {0, 2, 4, 7, 9, 11, 13, 15};
static int freq_idx_mapping_tbl_sb_r[DVFS_OPP_NUM] = {0, 1, 1, 2, 2, 3, 3, 3, 4, 4, 5, 5, 6, 6, 7, 7};
...
#if PPM_COBRA_NEED_OPP_MAPPING
		unsigned int prev_opp = freq_idx_mapping_tbl[opp+1];
#else
...
void ppm_cobra_update_limit(enum ppm_power_state new_state, void *user_req)
{
	...
	for_each_ppm_clusters(i) {
		...
		opp[i] = (cl_status[i].freq_idx >= 0)
			? freq_idx_mapping_tbl_r[cl_status[i].freq_idx] : -1;
#else
