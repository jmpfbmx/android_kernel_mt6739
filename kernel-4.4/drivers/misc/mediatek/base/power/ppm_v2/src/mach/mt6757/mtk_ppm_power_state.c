...
int __attribute__((weak)) sched_get_nr_overutil_avg(int cluster_id, int *l_avg, int *h_avg)
{
	return 0;
}
#endif
...
#if PPM_HICA_VARIANT_SUPPORT
#define TRANS_DATA(state, mask, rule, delta, hold, bond, f_hold, tlp, oul, ouh) {	\
	...
	.freq_hold_cnt = 0,		\
	.tlp_bond = tlp,		\
	.overutil_l_hold_time = oul,	\
	.overutil_l_hold_cnt = 0,	\
	.overutil_h_hold_time = ouh,	\
	.overutil_h_hold_cnt = 0,	\
}
#else
	...
	struct ppm_hica_algo_data data, struct ppm_state_transfer *settings)
{
	...
	if (cur_hica_variant == 1) {
		...
		if (overutil_h > hica_overutil) {
			...
			if (settings->overutil_h_hold_cnt >= settings->overutil_h_hold_time)
				return true;
		} else {
	...
	struct ppm_hica_algo_data data, struct ppm_state_transfer *settings)
{
	...
	if (cur_hica_variant == 1) {
		...
		if (overutil_l <= hica_overutil) {
			...
			if (settings->overutil_l_hold_cnt >= settings->overutil_l_hold_time)
				return true;
		} else {
