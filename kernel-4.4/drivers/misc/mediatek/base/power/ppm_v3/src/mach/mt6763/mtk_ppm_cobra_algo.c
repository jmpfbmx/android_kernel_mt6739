...
static int get_delta_pwr_LxLL(unsigned int L_core, unsigned int LL_core, unsigned int opp, int prefer)
{
	...
	if (opp == COBRA_OPP_NUM - 1) {
		if (prefer == PPM_CLUSTER_LL) {
			prev_pwr = (LL_core) ? ((LL_core > 1)
				...
				((L_core) ? cobra_tbl.basic_pwr_tbl[idx_L+L_core-1][cur_opp].power_idx : 0))
				: ((L_core) ? cobra_tbl.basic_pwr_tbl[idx_L+L_core-1][cur_opp].power_idx : 0))
				: ((L_core > 1) ? cobra_tbl.basic_pwr_tbl[idx_L+L_core-2][cur_opp].power_idx : 0);
		} else {
...
void ppm_cobra_update_limit(void *user_req)
{
	...
	if (delta_power >= 0) {
		while (1) {
			...
			if (opp[LxLL] > 0) {
				target_delta_pwr =
					get_delta_pwr_LxLL(ACT_CORE(L), ACT_CORE(LL), opp[LxLL]-1, LxLL);
			...
			while (CORE_LIMIT(LL) < get_cluster_max_cpu_core(PPM_CLUSTER_LL)) {
				target_delta_pwr = get_delta_pwr_LxLL(ACT_CORE(L),
						CORE_LIMIT(LL)+1, COBRA_OPP_NUM-1, LxLL);
				if (delta_power < target_delta_pwr)
					break;
			...
			while (CORE_LIMIT(L) < get_cluster_max_cpu_core(PPM_CLUSTER_L)) {
				target_delta_pwr = get_delta_pwr_LxLL(CORE_LIMIT(L)+1,
						ACT_CORE(LL), COBRA_OPP_NUM-1, LxLL);
				if (delta_power < target_delta_pwr)
					break;
...
prepare_next_round:
		...
		while (delta_power < 0) {
			...
			ppm_dbg(COBRA_ALGO, "exclusive_core_flag = %d,%d(LxLL=%d)\n",
				ex_core_flag[PPM_CLUSTER_LL], ex_core_flag[PPM_CLUSTER_L], LxLL);

			/* LxLL */
			if (opp[LxLL] >= 0 && opp[LxLL] < PPM_COBRA_MAX_FREQ_IDX && !ex_core_flag[LxLL]) {
				ChoosenCl = 1;
				ChoosenPwr = get_delta_pwr_LxLL(ACT_CORE(L), ACT_CORE(LL), opp[LxLL], LxLL);

				if (ACT_CORE(L) > 0 && opp[PPM_CLUSTER_L] < PPM_COBRA_MAX_FREQ_IDX)
					opp[PPM_CLUSTER_L] += 1;
				if (ACT_CORE(LL) > 0 && opp[PPM_CLUSTER_LL] < PPM_COBRA_MAX_FREQ_IDX)
					opp[PPM_CLUSTER_LL] += 1;
			}
			...
			if (opp[LxLL] == PPM_COBRA_MAX_FREQ_IDX) {
				if (!ignore_ex_core && max_throttle_core_num[LxLL] <= 0) {
					...
					opp[PPM_CLUSTER_L] = (ACT_CORE(L) > 0)
						? opp[PPM_CLUSTER_L] - 1 : opp[PPM_CLUSTER_L];
					opp[PPM_CLUSTER_LL] = (ACT_CORE(LL) > 0)
						? opp[PPM_CLUSTER_LL] - 1 : opp[PPM_CLUSTER_LL];
					/* check LL next round if L has no more core to throttle */
					LxLL = (LxLL == PPM_CLUSTER_L) ? PPM_CLUSTER_LL : LxLL;
					goto check_exclusive_core_flag;
				}

				if ((ignore_ex_core || !ex_core_flag[PPM_CLUSTER_L])
					&& (ACT_CORE(L) > 1 || (ACT_CORE(LL) > 0 && ACT_CORE(L) > 0))) {
					req->limit[PPM_CLUSTER_L].max_cpu_core = --ACT_CORE(L);
					max_throttle_core_num[PPM_CLUSTER_L]--;
					if (!ignore_ex_core && max_throttle_core_num[PPM_CLUSTER_L] <= 0) {
						ex_core_flag[PPM_CLUSTER_L] = 1;
						LxLL = PPM_CLUSTER_LL;
					}
				} else if ((ignore_ex_core || !ex_core_flag[PPM_CLUSTER_LL]) && ACT_CORE(LL) > 1) {
...
check_exclusive_core_flag:
			...
			if (ex_core_flag[PPM_CLUSTER_LL] && ex_core_flag[PPM_CLUSTER_L]) {
				ignore_ex_core = 1;
				ex_core_flag[PPM_CLUSTER_LL] = 0;
				ex_core_flag[PPM_CLUSTER_L] = 0;
				if (ACT_CORE(L) > 0)
					LxLL = PPM_CLUSTER_L; /* re-scan from L */
			}
	...
	if (ppm_debug > 0) {
		...
		for (i = 0; i <= get_cluster_max_cpu_core(PPM_CLUSTER_L); i++) {
			for (j = 0; j <= get_cluster_max_cpu_core(PPM_CLUSTER_LL); j++) {
				for (k = 0; k < COBRA_OPP_NUM; k++) {
					seq_printf(m, "[%d][%d][%d] = (%d)\n", i, j, k,
						get_delta_pwr_LxLL(i, j, k, PPM_CLUSTER_L));
				}
			}
		}
	}
