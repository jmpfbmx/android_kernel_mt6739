...
static short get_delta_pwr(enum ppm_cluster cluster, unsigned int core, unsigned int opp)
{
	unsigned int idx;
...
static short get_delta_perf(enum ppm_cluster cluster, unsigned int core, unsigned int opp)
{
...
void ppm_cobra_update_limit(void *user_req)
{
	...
	if (cl_status[PPM_CLUSTER_L].core_num == 0 && cl_status[PPM_CLUSTER_B].core_num == 0) {
		if (Core_limit[PPM_CLUSTER_L] > 0) {
			...
			cl_status[PPM_CLUSTER_B].freq_idx = get_cluster_max_cpufreq_idx(PPM_CLUSTER_B);
		}
	}
	...
	if (delta_power >= 0) {
		while (1) {
			...
			if (opp[PPM_CLUSTER_L] == 0 && ACT_CORE(B) == 0) {
				target_delta_pwr = get_delta_pwr(PPM_CLUSTER_B, 1, COBRA_OPP_NUM-1);
			...
			ppm_dbg(COBRA_ALGO, "[+]ChoosenCl=-1! delta=%d, (opp/c_lmt)=(%d,%d/%d%d)\n",
					delta_power, opp[PPM_CLUSTER_L], opp[PPM_CLUSTER_B],
					CORE_LIMIT(L), CORE_LIMIT(B));
		...
		while (delta_power < 0) {
			...
			if (ACT_CORE(L) > 0 && opp[PPM_CLUSTER_L] < PPM_COBRA_MAX_FREQ_IDX) {
				...
				if (MinEff > target_delta_eff) {
					...
					ChoosenCl = PPM_CLUSTER_L;
					ChoosenPwr = get_delta_pwr(PPM_CLUSTER_L, ACT_CORE(L), opp[PPM_CLUSTER_L]);
				}
			}
			...
			if (ChoosenCl == -1) {
				ppm_err("No lower OPP!(bgt/delta/cur)=(%d/%d/%d),(opp/act)=(%d,%d/%d%d)\n",
					power_budget, delta_power, curr_power,
					opp[PPM_CLUSTER_L], opp[PPM_CLUSTER_B], ACT_CORE(L), ACT_CORE(B));
				break;
			}
			...
			} else if (opp[PPM_CLUSTER_L] == PPM_COBRA_MAX_FREQ_IDX) {
				if (ACT_CORE(L) > 1)
...
static unsigned int get_limit_opp_and_budget(void)
{
	...
	for (i = 0; i <= get_cluster_min_cpufreq_idx(PPM_CLUSTER_L); i++) {
		...
		for (j = 0; j <= get_cluster_min_cpufreq_idx(PPM_CLUSTER_B); j++) {
