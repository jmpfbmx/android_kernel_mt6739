...
unsigned int all_dcm_type = (ARMCORE_DCM_TYPE | MCUSYS_DCM_TYPE | MCSI_DCM_TYPE
				    | STALL_DCM_TYPE | BIG_CORE_DCM_TYPE | RGU_DCM_TYPE
...
unsigned long dcm_mcu_misc1cfg_base;
...
unsigned long dcm_mp2_ca15m_base;

#define INFRACFG_AO_NODE "mediatek,infracfg_ao"
#define MP0_CPUCFG_NODE "mediatek,mp0_cpucfg"
#define MP1_CPUCFG_NODE "mediatek,mp1_cpucfg"
#define MCU_MISCCFG_NODE "mediatek,mcu_misccfg"
#define MCU_MISC1CFG_NODE "mediatek,mcu_misc1cfg"
...
int mt_dcm_dts_map(void)
{
	...
	/* mp0 cpucfg */
	node = of_find_compatible_node(NULL, NULL, MP0_CPUCFG_NODE);
	if (!node) {
		dcm_pr_err("error: cannot find node %s\n", MP0_CPUCFG_NODE);
		return -1;
	}

	dcm_mp0_cpucfg_base = (unsigned long)of_iomap(node, 0);
	if (!dcm_mp0_cpucfg_base) {
		dcm_pr_err("error: cannot iomap %s\n", MP0_CPUCFG_NODE);
		return -1;
	}

	/* mp1 cpucfg */
	node = of_find_compatible_node(NULL, NULL, MP1_CPUCFG_NODE);
	if (!node) {
		dcm_pr_err("error: cannot find node %s\n", MP1_CPUCFG_NODE);
		return -1;
	}

	dcm_mp1_cpucfg_base = (unsigned long)of_iomap(node, 0);
	if (!dcm_mp1_cpucfg_base) {
		dcm_pr_err("error: cannot iomap %s\n", MP1_CPUCFG_NODE);
		return -1;
	}

	/* mcu misccfg */
	node = of_find_compatible_node(NULL, NULL, MCU_MISCCFG_NODE);
	if (!node) {
		dcm_pr_err("error: cannot find node %s\n", MCU_MISCCFG_NODE);
		return -1;
	}

	dcm_mcu_misccfg_base = (unsigned long)of_iomap(node, 0);
	if (!dcm_mcu_misccfg_base) {
		dcm_pr_err("error: cannot iomap %s\n", MCU_MISCCFG_NODE);
		return -1;
	}

	/* mcu misc1cfg */
	node = of_find_compatible_node(NULL, NULL, MCU_MISC1CFG_NODE);
	if (!node) {
		dcm_pr_err("error: cannot find node %s\n", MCU_MISC1CFG_NODE);
		return -1;
	}

	dcm_mcu_misc1cfg_base = (unsigned long)of_iomap(node, 0);
	if (!dcm_mcu_misc1cfg_base) {
		dcm_pr_err("error: cannot iomap %s\n", MCU_MISC1CFG_NODE);
		return -1;
	}
	...
	dcm_mp2_ca15m_base = (unsigned long)of_iomap(node, 0);
...
#ifdef __KERNEL__
	dcm_pr_dbg("%s: MCUCFG_SYNC_DCM_MP0_REG=0x%08x, mp0_div_sel=%u, mp0 =%u\n",
#else
	dcm_pr_dbg("%s: MCUCFG_SYNC_DCM_MP0_REG=0x%X, mp0_div_sel=%u, mp0 =%u\n",
#endif
...
#else
	dcm_pr_dbg("%s: MCUCFG_SYNC_DCM_MP2_REG=0x%X, mp2_div_sel=%u,%u\n",
#endif
...
int dcm_armcore(int mode)
{
	...
	dcm_mcu_misccfg_mp2_arm_pll_divider_dcm(mode);
...
int dcm_big_core(int on)
{
	...
	if (dcm_cpu_cluster_stat & DCM_CPU_CLUSTER_B)
		dcm_mp2_ca15m_config_sync_dcm_cfg(on);
#endif
	return 0;
}
