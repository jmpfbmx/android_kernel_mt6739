...
static void clk_buf_ctrl_internal(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_UFS:
		...
		break;
	default:
		pr_err("[Power/clkbuf] %s: id=%d isn't supported\n", __func__, id);
		break;
	}
...
bool clk_buf_ctrl(enum clk_buf_id id, bool onoff)
{
	...
	case CLK_BUF_CONN:
		...
		if (!(pwrap_dcxo_en_flag & DCXO_CONN_ENABLE)) {
			ret = -1;
			pr_err("[Power/clkbuf] %s: id=%d skip due to non co-clock for CONN\n", __func__, id);
			pmic_clk_buf_ctrl_wcn(0);
			pmic_clk_buf_swctrl[XO_WCN] = 0;
			break;
		}
				 ...
				 char *buf)
{
	...
	srcclkena_o1 = !!(spm_read(PCM_REG13_DATA) & R13_MD_SRCCLKENA_0);
...
int clk_buf_fs_init(void)
{
	...
	if (r)
		pr_err("[Power/clkbuf] FAILED TO CREATE /sys/power/clk_buf (%d)\n", r);
...
int clk_buf_dts_map(void)
{
	...
	if (node) {
		...
		pr_err("[Power/clkbuf] %s can't find compatible node for pmic_clock_buffer\n", __func__);
		return -1;
	}
	...
	if (node)
		...
		pr_err("[Power/clkbuf] %s can't find compatible node for pwrap\n",
		       __func__);
		return -1;
	}
