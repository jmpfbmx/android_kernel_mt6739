	...
	if (data[0] == '0') {
		...
		while (infinity) {
			...
			for (i = 0; i < CONFIG_NR_CPUS; i++)
				random_cpuhp_stress[i] = random_cpuhp_stress[i] % CONFIG_NR_CPUS;
...
static int proc_show(struct seq_file *m, void *v)
{
	...
	seq_puts(m, "Usage:\n");
	seq_puts(m, "   CPU0 hotplug secondary cores: echo 0 > /proc/cpuhp_ut\n");
	seq_puts(m, "   Random CPU hotplug:           echo 1 > /proc/cpuhp_ut\n");
	seq_puts(m, "   Stop all CPU hotplug test:    echo X > /proc/cpuhp_ut\n");
...
{
	proc_create("cpuhp_ut", 0, NULL, &proc_fops);
	return 0;
}
