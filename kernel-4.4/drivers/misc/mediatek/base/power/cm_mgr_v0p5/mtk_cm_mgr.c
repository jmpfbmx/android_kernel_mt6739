...
static void cm_mgr_update_met(void)
{
	...
	if (cm_mgr_power_dbg_handler)
		cm_mgr_power_dbg_handler(CM_MGR_ARRAY(met_data.cm_mgr_power), met_data.cm_mgr_power);
	if (cm_mgr_count_dbg_handler)
		cm_mgr_count_dbg_handler(CM_MGR_ARRAY(met_data.cm_mgr_count), met_data.cm_mgr_count);
	if (cm_mgr_opp_dbg_handler)
		cm_mgr_opp_dbg_handler(CM_MGR_ARRAY(met_data.cm_mgr_opp), met_data.cm_mgr_opp);
	if (cm_mgr_loading_dbg_handler)
		cm_mgr_loading_dbg_handler(CM_MGR_ARRAY(met_data.cm_mgr_loading), met_data.cm_mgr_loading);
	if (cm_mgr_ratio_dbg_handler)
		cm_mgr_ratio_dbg_handler(CM_MGR_ARRAY(met_data.cm_mgr_ratio), met_data.cm_mgr_ratio);
	if (cm_mgr_bw_dbg_handler)
		cm_mgr_bw_dbg_handler(1, &met_data.cm_mgr_bw);
	if (cm_mgr_valid_dbg_handler)
		cm_mgr_valid_dbg_handler(CM_MGR_ARRAY(met_data.cm_mgr_valid), met_data.cm_mgr_valid);
#endif
}
...
#endif
{
	...
	ret = mt_secure_call(id, x1, x2, x3);
...
void check_cm_mgr_status(unsigned int freq_idx, unsigned int cluster)
{
	...
	if (spin_trylock(&cm_mgr_lock)) {
		...
		if (((cm_mgr_abs_load < light_load_cps) && (vcore_dram_opp_cur == CM_MGR_EMI_OPP)) || is_game) {
			if (is_game) {
				/* unreq */
				vcore_dram_opp = CM_MGR_EMI_OPP;
				set_dram_opp(vcore_dram_opp);
		...
		for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
			...
			/* calc scaled ratio */
			ratio_scale[i] = (max_load[i] > 0) ? (ratio[i] * 100 / max_load[i]) : ratio[i];
		...
		if (vcore_dram_opp_cur != 0) {
			...
			ret = smc_call(MTK_SIP_KERNEL_PMU_EVA, 1, idx, cpu_opp_cur[0] || (cpu_opp_cur[1] << 16));
			...
			for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
				cpu_power_up_array[i] = cpu_power_gain_opp(
					total_bw, IS_UP, cpu_opp_cur[i], local_ratio_idx[i], idx + i);
		...
		if (vcore_dram_opp_cur != CM_MGR_EMI_OPP) {
...
#ifdef ATF_SECURE_SMC_PER_CPU_STALL_RATIO
			ret = smc_call(MTK_SIP_KERNEL_PMU_EVA, 2, idx, cpu_opp_cur[0] || (cpu_opp_cur[1] << 16));
			...
			for (i = 0; i < CM_MGR_CPU_CLUSTER; i++) {
				cpu_power_down_array[i] = cpu_power_gain_opp(
					total_bw, IS_DOWN, cpu_opp_cur[i], local_ratio_idx[i], idx + i);
		...
		case 2:
			...
			vcore_power_down = 0;
			if ((vcore_power_up * vcore_power_ratio_up[idx]) <
				((cpu_power_up[0] + cpu_power_up[1]) * cpu_power_ratio_up[idx])) {
				debounce_times_down = 0;
				if (++debounce_times_up >= debounce_times_up_adb[idx]) {
					if (debounce_times_reset_adb)
						debounce_times_up = 0;
		...
		case 1:
			...
			idx = level;
			vcore_power_up = vcore_power_gain(vcore_power_gain, total_bw, idx);
			vcore_power_down = vcore_power_gain(vcore_power_gain, total_bw, idx - 1);
			if ((vcore_power_up * vcore_power_ratio_up[idx]) <
				((cpu_power_up[0] + cpu_power_up[1]) * cpu_power_ratio_up[idx])) {
				debounce_times_down = 0;
				if (++debounce_times_up >= debounce_times_up_adb[idx]) {
			...
			} else if ((vcore_power_down * vcore_power_ratio_down[idx - 1]) >
				((cpu_power_down[0] + cpu_power_down[1]) * cpu_power_ratio_down[idx - 1])) {
				debounce_times_up = 0;
				if (++debounce_times_down >= debounce_times_down_adb[idx - 1]) {
					if (debounce_times_reset_adb)
						debounce_times_down = 0;
					vcore_dram_opp = vcore_dram_opp_cur + 1;
					set_dram_opp(vcore_dram_opp);
		...
		case 0:
			...
			vcore_power_up = 0;
			vcore_power_down = vcore_power_gain(vcore_power_gain, total_bw, idx - 1);
			if ((vcore_power_down * vcore_power_ratio_down[idx - 1]) >
				((cpu_power_down[0] + cpu_power_down[1]) * cpu_power_ratio_down[idx - 1])) {
				debounce_times_up = 0;
				if (++debounce_times_down >= debounce_times_down_adb[idx - 1]) {
					if (debounce_times_reset_adb)
...
#else
	seq_printf(m, "diff/cnt/max = %llu/%llu/%d\n", test_diff, cnt, test_max);
#endif
	seq_printf(m, "< 50us    = %d\n", time_cnt_data[0]);
...
static int create_cm_mgr_debug_fs(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(entries); i++) {
		if (!proc_create_data
			...
			tag_pr_notice("%s(), create /proc/cm_mgr/%s failed\n", __func__,
				    entries[i].name);
	}
