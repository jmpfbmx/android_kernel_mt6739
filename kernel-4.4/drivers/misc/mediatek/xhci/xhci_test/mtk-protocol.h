...
int dev_config_ep0(int maxp, struct usb_device *dev);
...
int dev_loopback(char bdp, int length, int gpd_buf_size, int bd_buf_size,
	char dram_offset, char extension, struct usb_device *dev);
...
int dev_random_stop(int length, int gpd_buf_size, int bd_buf_size, char dev_dir_1,
	char dev_dir_2, int stop_count_1, int stop_count_2);
...
int dev_lpm_config_host(char lpm_mode, char wakeup, char beslck, char beslck_u3,
	char beslckd, char cond, char cond_en);
