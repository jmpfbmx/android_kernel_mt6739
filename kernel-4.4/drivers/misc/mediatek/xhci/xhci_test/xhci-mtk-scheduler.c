...
static int count_ss_bw(int is_in, int ep_type, int maxp, int interval, int burst, int mult, int offset, int repeat
	, int td_size)
{
	int i, j, k;
	int bw_required[3];
...
static int count_hs_bw(int ep_type, int maxp, int interval, int offset, int td_size)
{
	int i;
	int bw_required;
	struct sch_ep *cur_sch_ep;
...
static int count_tt_isoc_bw(int is_in, int maxp, int interval, int offset, int td_size)
{
	char is_cs;
...
static int count_tt_intr_bw(int interval, int frame_offset)
{
	...
	, int mult, unsigned int *ep, unsigned int *ep_ctx, struct sch_ep *sch_ep)
{
	...
	best_bw_repeat = 0;
	pr_err("add_ep parameters, dev_speed : %d\n"
		"is_in   : %d\n"
		"isTT    : %d\n"
		"ep_type : %d\n"
		"maxp    : %d\n"
	...
	if (isTT && ep_type == USB_EP_INT
		...
		for (frame_idx = 0; frame_idx < frame_interval; frame_idx++) {
			pr_err("check tt_intr_bw interval %d, frame_idx %d\n",
		...
		bw_cost = 188;
		bRepeat = 0;
		if (add_sch_ep(dev_speed, is_in, isTT, ep_type, maxp, interval, burst, mult, bOffset,
		...
		pr_err("final best idx %d, best repeat %d\n", best_bw_idx, best_bw_repeat);
		if (best_bw_idx == -1)
			return SCH_FAIL;
	...
	if (ret == SCH_SUCCESS) {
		...
		pr_err("[DBG] BPKTS: %x, BCSCOUNT: %x, BBM: %x\n", (unsigned int)bPkts,
			 (unsigned int)bCsCount, (unsigned int)bBm);
		pr_err("[DBG] BOFFSET: %x, BREPEAT: %x\n", (unsigned int)bOffset,
