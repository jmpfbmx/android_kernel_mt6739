...
void print_speed(int speed)
{
	if (speed == USB_SPEED_SUPER)
		log_debug("SUPER_SPEED device\n");

	if (speed == USB_SPEED_HIGH)
		log_debug("HIGH_SPEED device\n");

	if (speed == USB_SPEED_FULL)
		log_debug("FULL_SPEED device\n");

	if (speed == USB_SPEED_LOW)
		log_debug("LOW_SPEED device\n");

}

int mtk_xhci_handshake(struct xhci_hcd *xhci, void __iomem *ptr, u32 mask, u32 done, int msec)
{
	u32 result;
	...
	do {
		...
		if (result == ~(u32) 0) {	/* card removed */
			log_notice("[XHCI] mtk_xhci_handshake card removed, result is 0x%x\n", result);
			return RET_FAIL;
		}
		result &= mask;
		if (result == done) {
			log_notice("[XHCI] mtk_xhci_handshake done, result is 0x%x\n", result);
			return RET_SUCCESS;
		}
		mdelay(1);
		msec--;
	} while (msec > 0);
	log_notice("[XHCI] mtk_xhci_handshake timeout !, result is 0x%x\n", result);
	return RET_FAIL;
}
...
int get_port_index(int port_id)
{
	...
	for (i = 0; i < RH_PORT_NUM; i++) {
		port = rh_port[i];
		log_notice("port[%d]: 0x%x\n", i, port);
		log_notice("   port_id=%d\n", port->port_id);
		log_notice("   port_speed=%d\n", port->port_speed);
		log_notice("   port_status=%d\n", port->port_status);
	}
#endif
	for (i = 0; i < RH_PORT_NUM; i++) {
		port = rh_port[i];
...
int xhci_usbif_resource_get(void)
{
	...
	if (node == NULL) {
		node = of_find_compatible_node(NULL, NULL, "mediatek,usb3_xhci_ldvt");
		if (node == NULL) {
			log_err("xhci_test get node failed\n");
			return -1;
		}
	}
	...
	log_err("irq =%d , mac =%p , sif = %p\n", xhci_usbif_nirq, xhci_usbif_base,
		xhci_usbif_sif_base);
	return 0;
}
...
int f_test_lib_cleanup(void)
{
	...
	if (my_hcd == NULL) {
		log_err("driver already cleared\n");
		return RET_SUCCESS;
	}
	/* USBIF, reset this */
	g_otg_test = false;
				   ...
				   int power_required)
{
	...
	xhci_err(xhci,
		 "start random access regs thread, port_id: %d, port_rev %d, power required %d\n",
	...
	kthread_run(access_regs_thread, (void *)rreg_data, "rrgt");
	return 0;
}
...
/*else*/
	...
	port = rh_port[index];
	msleep(100);
	return wait_event_on_timeout((int *)&(port->port_reenabled), 2, ATTACH_TIMEOUT);
/*endif*/
	return 0;
}
...
int f_enable_slot(struct usb_device *dev)
{
	...
	if (dev == NULL) {
		...
		log_notice("[OTG_H] rhdev = my_hcd->self.root_hub = 0x%p", rhdev);

		/*udev = rhdev->children[g_port_id-1]; */

		rhdev->maxchild = 4;
		if (rhdev->children == NULL)
			rhdev->children = kcalloc(rhdev->maxchild, sizeof(struct usb_device *), GFP_KERNEL);

		udev = mtk_usb_alloc_dev(rhdev, rhdev->bus, g_port_id);
		log_notice("[OTG_H] f_enable_slot => mtk_usb_alloc_dev udev = 0x%p", udev);
		udev->speed = port->port_speed;
		udev->level = rhdev->level + 1;
		rhdev->children[g_port_id - 1] = udev;
		log_notice("[OTG_H] g_port_id %d, rhdev->children[g_port_id-1] = 0x%p", g_port_id,
		       rhdev->children[g_port_id - 1]);
	} else {
		udev = dev;
	}
	/* enable slot */
	...
	mtktest_xhci_ring_cmd_db(xhci);
	/* wait_event_on_timeout((int *)&g_cmd_status, CMD_DONE, CMD_TIMEOUT); */
	wait_event_on_timeout_esc_running((int *)&g_cmd_status, CMD_TIMEOUT);
...
int f_disable_slot(void)
{
	...
	mtktest_xhci_ring_cmd_db(xhci);
	/* wait_event_on_timeout((int *)&g_cmd_status, CMD_DONE, CMD_TIMEOUT); */
	wait_event_on_timeout_esc_running((int *)&g_cmd_status, CMD_TIMEOUT / 4);
	if (g_cmd_status == CMD_RUNNING) {
...
int f_evaluate_context(int max_exit_latency, int maxp0, int preping_mode, int preping, int besl,
		       int besld)
{
	struct xhci_hcd *xhci;
	struct usb_device *udev, *rhdev;
...
struct urb *alloc_ctrl_urb(struct usb_ctrlrequest *dr, char *buffer, struct usb_device *udev)
{
	...
	if (buffer) {
		...
		dma_sync_single_for_device(dev, mapping, urb->transfer_buffer_length,
					   DMA_BIDIRECTIONAL);
#endif
...
int f_ctrlrequest_nowait(struct urb *urb, struct usb_device *udev)
{
	...
	xhci_dbg(xhci, "setup packet: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
		 *(urb->setup_packet), *(urb->setup_packet + 1), *(urb->setup_packet + 2),
		 *(urb->setup_packet + 3)
		 , *(urb->setup_packet + 4), *(urb->setup_packet + 5), *(urb->setup_packet + 6),
		 *(urb->setup_packet + 7));
	spin_lock_irqsave(&xhci->lock, flags);
	...
	if (urb->status == 0) {
		if (urb->transfer_buffer_length > 0) {
			dma_sync_single_for_cpu(dev, urb->transfer_dma, urb->transfer_buffer_length,
						DMA_BIDIRECTIONAL);
		}
...
int f_config_ep(char ep_num, int ep_dir, int transfer_type, int maxp, int bInterval, int burst,
		int mult, struct usb_device *udev, int config_xhci)
{
	int ret;
	struct xhci_hcd *xhci;
...
int f_loopback_config_ep(char ep_out, char ep_in, int transfer_type, int maxp, int bInterval,
...
int f_queue_urb(struct urb *urb, int wait, struct usb_device *dev)
{
	...
	if (!urb->hcpriv) {
		if (usb_endpoint_xfer_isoc(&ep->desc))
			...
			size = 1;
		urb_priv =
		    kmalloc(sizeof(struct urb_priv) + size * sizeof(struct xhci_td *), GFP_KERNEL);
		if (!urb_priv) {
			xhci_err(xhci, "[ERROR] allocate urb_priv failed\n");
			return RET_FAIL;
		}
	...
	if (usb_endpoint_xfer_bulk(&ep->desc)) {
		...
		spin_lock_irqsave(&xhci->lock, flags);
		ret =
		    mtktest_xhci_queue_isoc_tx_prepare(xhci, GFP_KERNEL, urb, udev->slot_id,
						       ep_index);
		spin_unlock_irqrestore(&xhci->lock, flags);
	}
	...
	if (wait) {
		...
		if (urb->status != 0) {
			...
			if (urb->status == -EINPROGRESS) {
				xhci_err(xhci,
					 "[ERROR] Timeout, stop endpoint and set tr dequeue pointer\n");
...
int f_fill_urb_with_buffer(struct urb *urb, int ep_num, int data_length, void *buffer,
			   int start_add, int dir, int iso_num_packets, int psize,
			   dma_addr_t dma_mapping, struct usb_device *usbdev)
{
	...
	if (usb_endpoint_xfer_bulk(&ep->desc) && usb_endpoint_dir_out(&ep->desc)) {
		usb_fill_bulk_urb(urb, udev, usb_sndbulkpipe(udev, usb_endpoint_num(&ep->desc)),
				  buffer, data_length, NULL, NULL);
	} else if (usb_endpoint_xfer_bulk(&ep->desc) && usb_endpoint_dir_in(&ep->desc)) {
		usb_fill_bulk_urb(urb, udev, usb_rcvbulkpipe(udev, usb_endpoint_num(&ep->desc)),
				  buffer, data_length, NULL, NULL);
	} else if (usb_endpoint_xfer_int(&ep->desc) && usb_endpoint_dir_out(&ep->desc)) {
		usb_fill_int_urb(urb, udev, usb_sndintpipe(udev, usb_endpoint_num(&ep->desc)),
				 buffer, data_length, NULL, NULL, ep->desc.bInterval);
	} else if (usb_endpoint_xfer_int(&ep->desc) && usb_endpoint_dir_in(&ep->desc)) {
		usb_fill_int_urb(urb, udev, usb_rcvintpipe(udev, usb_endpoint_num(&ep->desc)),
				 buffer, data_length, NULL, NULL, ep->desc.bInterval);
...
int f_fill_urb(struct urb *urb, int ep_num, int data_length, int start_add, int dir,
	       int iso_num_packets, int psize, struct usb_device *usbdev)
{
	struct usb_device *udev, *rhdev;
	struct device *dev;
	/*struct xhci_virt_device *virt_dev;*/
	...
	xhci_err(xhci, "dma buffer address 0x%p\n", buffer);
	xhci_err(xhci, "mapping buffer address 0x%p\n", (void *)(unsigned long)mapping);
	if (usb_endpoint_xfer_bulk(&ep->desc) && usb_endpoint_dir_out(&ep->desc)) {
		usb_fill_bulk_urb(urb, udev, usb_sndbulkpipe(udev, usb_endpoint_num(&ep->desc)),
				  buffer, data_length + start_add, NULL, NULL);
	} else if (usb_endpoint_xfer_bulk(&ep->desc) && usb_endpoint_dir_in(&ep->desc)) {
		usb_fill_bulk_urb(urb, udev, usb_rcvbulkpipe(udev, usb_endpoint_num(&ep->desc)),
				  buffer, data_length + start_add, NULL, NULL);
	} else if (usb_endpoint_xfer_int(&ep->desc) && usb_endpoint_dir_out(&ep->desc)) {
		usb_fill_int_urb(urb, udev, usb_sndintpipe(udev, usb_endpoint_num(&ep->desc)),
				 buffer, data_length + start_add, NULL, NULL, ep->desc.bInterval);
	} else if (usb_endpoint_xfer_int(&ep->desc) && usb_endpoint_dir_in(&ep->desc)) {
		usb_fill_int_urb(urb, udev, usb_rcvintpipe(udev, usb_endpoint_num(&ep->desc)),
				 buffer, data_length + start_add, NULL, NULL, ep->desc.bInterval);
	} else if (usb_endpoint_xfer_isoc(&ep->desc) && usb_endpoint_dir_out(&ep->desc)) {
...
int f_fill_urb_sg(struct urb *urb, int dir, int ep_num, int data_length, int sg_len,
						...
						struct usb_host_endpoint *ep)
{
	...
	if (usb_endpoint_xfer_control(&ep->desc) || usb_endpoint_xfer_bulk(&ep->desc))
		return 0;
...
int f_loopback_loop(int ep_out, int ep_in, int data_length, int start_add,
		    struct usb_device *usbdev)
{
	struct device *dev;
	struct usb_device *udev, *rhdev;
...
/* log_err("[Debug] max_esit_payload: %d\n", max_esit_payload); */
	...
	ret =
	    f_fill_urb(urb_tx, ep_out, data_length, start_add, URB_DIR_OUT, iso_num_packets,
		       max_esit_payload, udev);
	if (ret) {
	...
	if (ret) {
		xhci_err(xhci, "[ERROR]tx urb transfer failed ret =%d !!\n", ret);
		return RET_FAIL;
	}
	if (usb_endpoint_xfer_isoc(&ep_tx->desc))
		msleep(1000);
	...
	ret =
	    f_fill_urb(urb_rx, ep_in, data_length, start_add, URB_DIR_IN, iso_num_packets,
		       max_esit_payload, udev);
	if (ret) {
	...
	dma_sync_single_for_cpu(dev, urb_tx->transfer_dma - start_add, data_length + start_add,
				DMA_BIDIRECTIONAL);
	dma_sync_single_for_cpu(dev, urb_rx->transfer_dma - start_add, data_length + start_add,
				DMA_BIDIRECTIONAL);
	/* Compare */
...
int f_loopback_sg_loop(int ep_out, int ep_in, int data_length, int start_add, int sg_len,
		       struct usb_device *usbdev)
{
	...
	ret =
	    f_fill_urb(urb_tx, ep_out, data_length, start_add, URB_DIR_OUT, 0, max_esit_payload,
		       udev);
	if (ret) {
		xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
		return RET_FAIL;
	...
	ret =
	    f_fill_urb(urb_rx, ep_in, data_length, start_add, URB_DIR_IN, 0, max_esit_payload,
		       udev);
	if (ret) {
		xhci_err(xhci, "[ERROR]fill rx urb Error!!\n");
		return RET_FAIL;
...
int f_loopback_loop_gpd(int ep_out, int ep_in, int data_length, int start_add, int gpd_length,
			struct usb_device *usbdev)
{
	struct xhci_hcd *xhci;
	struct device *dev;
	...
	for (i = 0; i < num_urbs; i++) {
		...
		xhci_dbg(xhci, "[LOOPBACK]Tx round %d, urb_length %d, buffer 0x%p, mapping 0x%p\n",
			 i, tmp_urb_len, buffer_tx + total_running,
			 (void *)(unsigned long)(mapping_tx + total_running));
		urb_tx = usb_alloc_urb(iso_num_packets, GFP_KERNEL);
		f_fill_urb_with_buffer(urb_tx, ep_out, tmp_urb_len, buffer_tx + total_running,
				       start_add, URB_DIR_OUT, iso_num_packets, max_esit_payload,
				       mapping_tx + total_running, udev);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
			return RET_FAIL;
		}
/* log_err("urb_tx buffer 0x%x transfer_dma 0x%x\n", urb_tx->transfer_buffer, urb_tx->transfer_dma); */
		ret = f_queue_urb(urb_tx, 1, NULL);
		if (ret) {
			xhci_err(xhci, "[ERROR]queue tx urb Error!!\n");
			return RET_FAIL;
		}
		urb_tx->transfer_buffer = (void *)NULL;
		urb_tx->transfer_dma = (dma_addr_t)NULL;
		usb_free_urb(urb_tx);
...
/* log_err("[Debug] max_esit_payload: %d\n", max_esit_payload); */
		...
		xhci_dbg(xhci, "[LOOPBACK]Rx round %d, urb_length %d, buffer 0x%p, mapping 0x%p\n",
			 i, tmp_urb_len, buffer_rx + total_running,
			 (void *)(unsigned long)(mapping_rx + total_running));
		f_fill_urb_with_buffer(urb_rx, ep_in, tmp_urb_len, buffer_rx + total_running,
				       start_add, URB_DIR_IN, iso_num_packets, max_esit_payload,
				       mapping_rx + total_running, udev);
		xhci_dbg(xhci, "urb_rx buffer 0x%p transfer_dma 0x%p\n", urb_rx->transfer_buffer,
			 (void *)(unsigned long)urb_rx->transfer_dma);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
			return RET_FAIL;
		}
		...
		urb_rx->transfer_buffer = (void *)NULL;
		urb_rx->transfer_dma = (dma_addr_t)NULL;
		usb_free_urb(urb_rx);
		total_running += tmp_urb_len;
	}
	...
	for (i = 0; i < data_length; i++) {
		...
		if ((*tmp1) != (*tmp2)) {
			xhci_err(xhci,
				 "[ERROR] buffer %d not match, tx buf 0x%p, rx buf 0x%p, tx 0x%x, rx 0x%x\n",
...
int f_loopback_sg_loop_gpd(int ep_out, int ep_in, int data_length, int start_add, int sg_len,
...
/* log_err("[Debug] max_esit_payload: %d\n", max_esit_payload); */
		...
		ret =
		    f_fill_urb_with_buffer(urb_tx, ep_out, tmp_urb_len, buffer_tx + total_running,
					   start_add, URB_DIR_OUT, iso_num_packets,
					   max_esit_payload, mapping_tx + total_running, udev);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
			return RET_FAIL;
		}
		ret = f_fill_urb_sg(urb_tx, URB_DIR_OUT, ep_out, tmp_urb_len, sg_len, udev);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill tx urb sg Error!!\n");
			return RET_FAIL;
		}
...
/* log_err("[Debug] max_esit_payload: %d\n", max_esit_payload); */
		...
		ret = f_fill_urb_sg(urb_rx, URB_DIR_IN, ep_in, tmp_urb_len, sg_len, udev);
		if (ret) {
...
static int loopback_thread(void *data)
{
	...
	if (lb_data->is_ctrl) {
		...
		do {
			...
			xhci_err(xhci,
				 "[LOOPBACK] dev %d, ep_out %d, ep_in %d, length %d, start_add %d, sg_length %d\n",
				 lb_data->dev_num, lb_data->out_ep_num, lb_data->in_ep_num, length,
				 start_add, sg_length);
			mutex_lock(lb_data->lock);
			...
			if ((length > sg_length) && sg_length != 0 && (length / sg_length < 60)) {
				ret =
				    f_loopback_sg_loop(lb_data->out_ep_num, lb_data->in_ep_num,
						       length, start_add, sg_length, udev);
			} else {
				ret =
				    f_loopback_loop(lb_data->out_ep_num, lb_data->in_ep_num, length,
						    start_add, udev);
			}
			if (ret) {
...
int f_add_loopback_thread(struct xhci_hcd *xhci, int dev_num, struct usb_device *udev,
			  int out_ep_num, int in_ep_num, struct mutex *lock, char is_ctrl)
{

	struct loopback_data *lb_data;

	xhci_err(xhci,
		 "[LOOPBACK]Start loopback thread, devnum %d, ep_out %d, ep_in %d, isctrl %d\n",
...
/* isWarmReset = false; */
	...
	if (c_slot_id == 0)
		c_slot_id = g_slot_id;
	if (c_slot_id == 0)
		log_err("[ERROR] slot ID not valid\n");
...
/* g_port_reset = false; */
	...
	wait_event_on_timeout((int *)&(port->port_status), ENABLED, ATTACH_TIMEOUT);
	if (port->port_status != ENABLED) {
	...
	if ((ep_ctx->ep_info & EP_STATE_MASK) != EP_STATE_STOPPED) {
		xhci_err(xhci, "[FAIL] EP0 state is not in stopped after reset device command\n");
		ret = RET_FAIL;
	}
	for (i = 1; i < 31; ++i) {
		...
		if ((ep_ctx->ep_info & EP_STATE_MASK) != EP_STATE_DISABLED) {
			xhci_err(xhci,
				 "[FAIL] EP%d state is not in disabled after reset device command\n",
				 i);
			ret = RET_FAIL;
		}
	}
...
int f_power_remotewakeup(void)
{
	...
	wait_event_on_timeout((int *)&g_port_resume, 1, 5000);
	if (g_port_resume == 0) {
...
int f_power_config_lpm(u32 slot_id, u32 hirdm, u32 L1_timeout, u32 rwe, u32 besl, u32 besld,
		       u32 hle, u32 int_nak_active, u32 bulk_nyet_active)
{
	...
	log_notice("[OTG_H] read SSUSB_XHCI_HSCH_CFG2 address is 0x%p , value is 0x%x\n", addr, temp);
	/* pdn */
	...
	log_notice("[OTG_H] read PORTPMSC address is 0x%p", addr);
	...
	log_notice("[OTG_H] read port_lpm_ctrl_base address is 0x%p , value is 0x%x\n", addr, temp);
...
int f_ring_enlarge(int ep_dir, int ep_num, int dev_num)
{
	...
	xhci_dbg(xhci, "Linking segment 0x%llx to segment 0x%llx (DMA)\n",
		 (unsigned long long)prev->dma, (unsigned long long)next->dma);
	/* adjust cycle bit */
...
int f_ring_set_tr_dequeue_pointer(int slot_id, int ep_index, struct urb *urb)
{
	...
	mtktest_xhci_find_new_dequeue_state(xhci, slot_id, ep_index, 0, cur_td, &deq_state);

	spin_lock_irqsave(&xhci->lock, flags);
	mtktest_xhci_queue_new_dequeue_state(xhci, slot_id, ep_index, 0, &deq_state);
	spin_unlock_irqrestore(&xhci->lock, flags);
...
int f_ring_stop_cmd(void)
{
	...
	if (my_hcd == NULL) {
		log_err("[ERROR]host controller driver not initiated\n");
		return RET_FAIL;
	}
	xhci = hcd_to_xhci(my_hcd);
...
int f_hub_setremoteWakeup(int hdev_num, int wValue)
{
	int ret;
	struct usb_device *udev, *rhdev;
	struct xhci_virt_device *virt_dev;
...
/* urb = usb_alloc_urb(0, GFP_NOIO); */
	dr = kmalloc(sizeof(struct usb_ctrlrequest), GFP_NOIO);
	dr->bRequestType = USB_RECIP_DEVICE;
	dr->bRequest = USB_REQ_SET_FEATURE;
	dr->wValue = cpu_to_le16(wValue);	/* USB_DEVICE_REMOTE_WAKEUP */
...
int f_hub_config_subhub(int parent_hub_num, int hub_num, int port_num)
{
	...
	if (f_hub_clearportfeature(parent_hub_num, HUB_FEATURE_C_PORT_RESET, port_num) !=
	    RET_SUCCESS) {
		xhci_err(xhci, "[ERROR] Set port reset failed\n");
		return RET_FAIL;
	}
	...
	for (i = 1; i <= 4; i++) {
		if (f_hub_clearportfeature(hub_num, HUB_FEATURE_C_PORT_CONNECTION, i) !=
		    RET_SUCCESS) {
...
int f_add_ixia_thread(struct xhci_hcd *xhci, int dev_num, struct ixia_dev *ix_dev)
{
	...
	for (i = 0; i < TOTAL_RX_URB; i++) {
		urb_rx = usb_alloc_urb(0, GFP_NOIO);
		ret =
		    f_fill_urb(urb_rx, ix_data->ep_in, 1514, 0, URB_DIR_IN, 0, max_esit_payload,
			       udev);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill rx urb Error!!\n");
			return RET_FAIL;
	...
	for (i = 0; i < TOTAL_RX_URB; i++) {
		...
		ret =
		    f_fill_urb(urb_tx, ix_data->ep_out, 1514, 0, URB_DIR_OUT, 0, max_esit_payload,
			       udev);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
			return RET_FAIL;
...
void SetETHEPConfig(int dev_num, char *buf, struct usb_device *udev)
{
	...
	pConfDes = (struct MUSB_ConfigurationDescriptor *)buf;
	pBuf = (char *)buf;
	log_err("pBuf 0x%p\n", pBuf);
	pBuf += 9;		/* sizeof(struct MUSB_ConfigurationDescriptor); */
	log_err("pBuf 0x%p\n", pBuf);
	log_err("bNumInterfaces %d\n", pConfDes->bNumInterfaces);
...
/* Read GPIOs */
	...
	if (ret) {
		log_err("[DEV]Get Device descriptor ctrl request failed!!\n");
		kfree(dr);
		kfree(ptr);
		usb_free_urb(urb);
		return RET_FAIL;
	...
	while (ethenumeration_step[nEnumStep].pDesciptor != NULL) {
		...
		usb_free_urb(urb);
		if (ret) {
			log_err("[DEV]config ep ctrl request failed!!\n");
			return RET_FAIL;
		}
		nEnumStep++;
	}
...
int f_hub_configuredevice(int hub_num, int port_num, int dev_num, int transfer_type, int maxp,
			  int bInterval, char is_config_ep, char is_stress, int stress_config)
{
	u32 status;
	int ret;
	struct usb_device *udev, *hdev;
...
#if 0
	if (speed == USB_SPEED_SUPER) {
		if (f_hub_clearportfeature(hub_num, HUB_FEATURE_C_PORT_CONNECTION, port_num) !=
		    RET_SUCCESS) {
	...
	if (is_config_ep) {
		...
		ret =
		    dev_config_ep(1, USB_RX, transfer_type, maxp, bInterval, dev_slot, burst, mult,
				  udev);
		if (ret != RET_SUCCESS)
			return RET_FAIL;

		ret =
		    dev_config_ep(1, USB_TX, transfer_type, maxp, bInterval, dev_slot, burst, mult,
				  udev);
		if (ret != RET_SUCCESS)
			return RET_FAIL;
		...
		ret =
		    f_config_ep(1, EPADD_OUT, transfer_type, maxp, bInterval, burst, mult, udev, 0);
		if (ret != RET_SUCCESS)
			return RET_FAIL;

		ret =
		    f_config_ep(1, EPADD_IN, transfer_type, maxp, bInterval, burst, mult, udev, 1);
		if (ret != RET_SUCCESS)
			return RET_FAIL;
		...
		if (speed == USB_SPEED_SUPER) {
			...
			/* return f_loopback_config_ep(1,2,transfer_type, maxp, bInterval, udev); */
			ret =
			    dev_config_ep(1, USB_RX, EPATT_BULK, 512, 0, dev_slot, burst, mult,
					  udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;

			ret =
			    dev_config_ep(1, USB_TX, EPATT_BULK, 512, 0, dev_slot, burst, mult,
					  udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;
...
#if 1
			ret =
			    dev_config_ep(2, USB_RX, EPATT_INT, 1024, 1, dev_slot, burst, mult,
					  udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;

			ret =
			    dev_config_ep(2, USB_TX, EPATT_INT, 1024, 1, dev_slot, burst, mult,
					  udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;
			...
			ret =
			    dev_config_ep(1, USB_RX, EPATT_BULK, 64, 0, dev_slot, burst, mult,
					  udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;

			ret =
			    dev_config_ep(1, USB_TX, EPATT_BULK, 64, 0, dev_slot, burst, mult,
					  udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;
...
#if 1
			ret =
			    dev_config_ep(2, USB_RX, EPATT_INT, 64, 1, dev_slot, burst, mult, udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;

			ret =
			    dev_config_ep(2, USB_TX, EPATT_INT, 64, 1, dev_slot, burst, mult, udev);
			if (ret != RET_SUCCESS)
				return RET_FAIL;
		     ...
		     int dir, int interval, struct usb_host_endpoint *ep, int dev_index)
{
	...
	if (transfer_type == EPATT_BULK && dir == URB_DIR_OUT) {
		usb_fill_bulk_urb(urb, udev, usb_sndbulkpipe(udev, ep_num), buffer, max_data_length,
				  NULL, NULL);
	} else if (transfer_type == EPATT_INT && dir == URB_DIR_OUT) {
		usb_fill_int_urb(urb, udev, usb_sndintpipe(udev, ep_num), buffer, max_data_length,
				 NULL, NULL, interval);
	} else if (transfer_type == EPATT_BULK && dir == URB_DIR_IN) {
		usb_fill_bulk_urb(urb, udev, usb_rcvbulkpipe(udev, ep_num), buffer, max_data_length,
				  NULL, NULL);
	} else if (transfer_type == EPATT_INT && dir == URB_DIR_IN) {
		usb_fill_int_urb(urb, udev, usb_rcvintpipe(udev, ep_num), buffer, max_data_length,
				 NULL, NULL, interval);
	}
	urb->status = -EINPROGRESS;
	urb->actual_length = 0;
...
static int transfer_thread(void *data)
{
	...
	do {
		...
		if (ret) {
			...
			xhci_err(xhci,
				 "Transfer of ep %d, dir %x  t_data->cur_stop_count :%d   stop_count :%d\n",
				 t_data->ep_num, t_data->dir, t_data->cur_stop_count,
				 t_data->stop_count);
			...
			if (t_data->cur_stop_count > t_data->stop_count) {
				/* error */
				xhci_err(xhci,
					 "Transfer of ep %d, dir %d fail occurred more than set %d times\n",
...
int f_random_stop(int ep_1_num, int ep_2_num, int stop_count_1, int stop_count_2, int urb_dir_1,
...
static int stress_ep0_thread(void *data)
{
	...
	do {
		...
		if (count >= count_boundary) {
			dma_sync_single_for_device(dev, urb2->transfer_dma, 2048,
						   DMA_BIDIRECTIONAL);
			for (i = 0; i < 2048; i++) {
				if ((*(ptr1 + i)) != (*(ptr2 + i))) {
					xhci_err(xhci,
						 "[ERROR] buffer %d not match, tx 0x%x, rx 0x%x\n",
						 i, *(ptr1 + i), *(ptr2 + i));
					return RET_FAIL;
				}
			}
...
static int stress_rx_done_thread(void *data)
{
	int i;
	struct urb *urb_tx, *urb_rx;
	struct stress_data *str_data = data;
	...
	do {
		urb_rx = str_data->urb_rx_list[tx_index];
		rx_status = urb_rx->status;
		/*xhci_dbg(xhci, "[STRESS] check rx urb %d 0xld, dev %d, ep %d\n" */
		/*	, tx_index, (unsigned long)urb_rx, str_data->dev_num, str_data->ep_num);*/
		while (rx_status != 0) {
			if (rx_status != 0 && rx_status != -EINPROGRESS) {
...
static int stress_rdn_len_rx_done_thread(void *data)
{
	int i/*, ret*/;
	struct urb *urb_tx, *urb_rx;
	struct stress_data *str_data = data;
...
int f_add_rdn_len_str_threads(int dev_num, int ep_num, int maxp, char isCompare,
...
/* xhci_err(xhci, "iso_packet_num %d\n", iso_packet_num); */
	for (i = 0; i < TOTAL_URB; i++) {
		...
		ret =
		    f_fill_urb(str_data->urb_tx_list[i], ep_num, data_len, 0, EPADD_OUT,
			       iso_packet_num, maxp, udev);
		if (ret) {
			xhci_err(xhci, "[ERROR]fill tx urb Error!!\n");
			return RET_FAIL;
		}
		memcpy(str_data->urb_tx_list[i]->transfer_buffer, str_data->buffer[i], data_len);
		dma_sync_single_for_device(dev, str_data->urb_tx_list[i]->transfer_dma, data_len,
					   DMA_BIDIRECTIONAL);
...
/* xhci_err(xhci, "[STRESS] URB_TX %d -- 0x%x\n", i, str_data->urb_tx_list[i]); */
	...
	for (i = 0; i < TOTAL_URB; i++) {
		...
		ret = f_fill_urb_with_buffer(str_data->urb_rx_list[i]
					     , ep_num, data_len,
					     str_data->urb_tx_list[i]->transfer_buffer, 0, EPADD_IN,
					     iso_packet_num, maxp,
					     str_data->urb_tx_list[i]->transfer_dma, udev);
...
int f_add_str_threads(int dev_num, int ep_num, int maxp, char isCompare, struct usb_device *usbdev,
		      char isEP0)
{
	int ret, i, j;
	struct device *dev;
...
int otg_dev_A_host_thread(void *data)
{
	...
	log_err("[OTG_H] enter u3h_dev_A_host thread\n");
	if (f_test_lib_init() != RET_SUCCESS) {
		log_err("[OTG_H][FAIL] controller driver init failed\n");
		return RET_FAIL;
	}
	...
	if (f_enable_port(0) != RET_SUCCESS) {
		log_err("[OTG_H][FAIL] Device attach timeout\n");
		return RET_FAIL;
	}
	...
	if (f_address_slot(false, NULL) != RET_SUCCESS) {
		log_err("[OTG_H][FAIL] Address device failed\n");
		return RET_FAIL;
	}
	log_err("[OTG_H] exit u3h_dev_A_host thread\n");
	return 0;
}

int otg_dev_B_hnp(void *data)
{
	log_err("[OTG_H] enter u3h_otg_dev_B_hnp thread\n");
	...
	/* wait to become host */
	wait_event_on_timeout((int *)&g_otg_hnp_become_host, true, TRANS_TIMEOUT);
	if (!g_otg_hnp_become_host) {
		log_err("[OTG_H][FAIL] doesn't become host\n");
		return RET_FAIL;
	}
	log_err("[OTG_H] HNP become host\n");
	if (f_enable_port(0) != RET_SUCCESS) {
		log_err("[OTG_H][FAIL] Device attach timeout\n");
		return RET_FAIL;
	}
	...
	log_err("[OTG_H] exit u3h_otg_dev_B_hnp\n");
	return 0;
}
...
int otg_dev_A_hnp(void *data)
{
	...
	log_err("[OTG_H] enter u3h_otg_dev_A_hnp\n");
	...
	log_err("[OTG_H] device A become device\n");
	ret = f_disable_slot();
	if (ret) {
		log_err("[OTG_H][FAIL] disable slot failed!!!!!!!!!!\n");
		return ret;
	}
	/* alert device */
	writel(SSUSB_CHG_B_ROLE_B, SSUSB_OTG_STS);
	log_err("[OTG_H] exit u3h_otg_dev_A_hnp\n");
	return 0;
}
...
int otg_dev_A_hnp_back(void *data)
{
	...
	log_err("[OTG_H] enter u3h_otg_dev_A_hnp_back\n");
	wait_event_on_timeout((int *)&g_otg_hnp_become_host, true, TRANS_TIMEOUT);
	log_err("[OTG_H] device_A become host\n");
	if (f_enable_port(0) != RET_SUCCESS) {
		log_err("[OTG_H][FAIL] Device attach timeout\n");
		return RET_FAIL;
	}
	...
	log_err("[OTG_H] exit u3h_otg_dev_A_hnp_back\n");
	return 0;
}
...
int otg_dev_B_hnp_back(void *data)
{
	...
	log_err("[OTG_H] enter u3h_otg_dev_B_hnp_back\n");
	/* suspend port */
	port_id = 1;
	xhci = hcd_to_xhci(my_hcd);
	addr = &xhci->op_regs->port_status_base + NUM_PORT_REGS * ((port_id - 1) & 0xff);
	...
	if (!g_otg_hnp_become_dev) {
		log_err("[OTG_H][FAIL] doesn't get B_ROLE_A interrupt\n");
		return RET_FAIL;
	}
#if 0
	temp = readl(SSUSB_OTG_STS);
...
#endif
	log_err("[OTG_H] device B become device\n");
	ret = f_disable_slot();
	if (ret) {
		log_err("[OTG_H][FAIL] disable slot failed!!!!!!!!!!\n");
		return ret;
	...
	/* set B_role_B */
	writel(SSUSB_CHG_B_ROLE_B, SSUSB_OTG_STS);
	log_err("[OTG_H] exit u3h_otg_dev_B_hnp_back\n");
	return 0;
}
...
int otg_dev_A_srp(void *data)
{
	...
	log_err("[OTG_H] enter u3h_otg_dev_A_srp thread\n");
	xhci = hcd_to_xhci(my_hcd);
	if (!g_port_reset) {
		log_err("[OTG_H][FAIL] device not reset\n");
		return RET_FAIL;
	}
	if (g_slot_id == 0) {
		log_err("[OTG_H][FAIL] slot not enabled\n");
		return RET_FAIL;
	}
	...
	ret = f_enable_port(0);
	if (ret) {
		log_err("[OTG_H][FAIL]  port reenable failed\n");
		return RET_FAIL;
	}
	...
	ret = f_address_slot(false, NULL);
	if (ret) {
		log_err("[OTG_H][FAIL] address slot failed!!!!!!!!!!\n");
		return ret;
	}
	/* reset SW scheduler algorithm */
	mtktest_mtk_xhci_scheduler_init();

	log_err("[OTG_H] exit u3h_otg_dev_A_srp thread\n");
	return 0;
}
...
#define USB_CLASS_ID_HUB			9
	...
	if ((le16_to_cpu(desc->idVendor) == 0x1a0a && le16_to_cpu(desc->idProduct) == 0xbadd))
		return 0;

	/* PET */
	if ((le16_to_cpu(desc->idVendor) == 0x1A0A && le16_to_cpu(desc->idProduct) == 0x0200))
		return 1;

	/* PET : unsupport test */
	if ((le16_to_cpu(desc->idVendor) == 0x1A0A && le16_to_cpu(desc->idProduct) == 0x0201))
		return 0;

	/* PET : unsupport test */
	if ((le16_to_cpu(desc->idVendor) == 0x1A0A && le16_to_cpu(desc->idProduct) == 0x0202))
		return 0;
	...
	if (le16_to_cpu(desc->bDeviceClass) == USB_CLASS_ID_HID &&
	    le16_to_cpu(desc->bDeviceSubClass) == 0 && le16_to_cpu(desc->bDeviceProtocol) == 0) {
		return 1;
	}
	/* STORAGE */
...
int f_getdescriptor(void)
{
	...
	ret = f_ctrlrequest(urb, udev);
	if (ret == RET_SUCCESS) {
		log_notice("[OTG_H] -----------------------desc->idProduct: 0x%x------------------\n",
		       desc->idProduct);
		log_notice("[OTG_H] -----------------------desc->idVendor: 0x%x-------------------\n",
		       desc->idVendor);
		log_notice("[OTG_H] -----------------------desc->bDeviceClass: 0x%x---------------\n",
		       desc->bDeviceClass);
		log_notice("[OTG_H] -----------------------desc->bDeviceSubClass: 0x%x------------\n",
		       desc->bDeviceSubClass);
		log_notice("[OTG_H] -----------------------desc->bDeviceProtocol: 0x%x------------\n",
		       desc->bDeviceProtocol);
		log_notice("[OTG_H] -----------------------desc->bcdDevice: 0x%x------------------\n",
		       desc->bcdDevice);
		log_notice("[OTG_H] -----------------------desc->bNumConfigurations: 0x%x---------\n",
		       desc->bNumConfigurations);

		if (desc->idProduct == 0x1234)
			log_notice("[OTG_H] ===================Device not support OTG=================\n");

		if (desc->idProduct == 0x0201) {
			log_notice("[OTG_H] ==============Unsupported device=====================\n");
			g_otg_unsupported_dev = true;
		}

		if (is_targeted(desc) == 0) {
			log_notice("[OTG_H] ============== not support device=====================\n");
			usbif_u3h_test_send_event(USBIF_OTG_EVENT_DEV_NOT_SUPPORTED);
		}
	} else {
		log_err("[OTG_H] [ERROR] f_getdescriptor failed\n");
	}
	...
	usb_free_urb(urb);
/* kfree(urb); */
	log_err("[OTG_H] <=== f_getdescriptor, ret = %d\n", ret);
	return ret;
}

int f_getqualifierdescriptor(void)
{
	int ret;
	struct usb_device *udev, *rhdev;
	/*struct xhci_virt_device *virt_dev;*/
	...
	log_err("[OTG_H] ===> f_getqualifierdescriptor\n");
...
/* kfree(urb); */
	...
	log_err("[OTG_H] <=== f_getqualifierdescriptor, ret = %d\n", ret);

	return ret;
}

int f_getconfiguration_desc(void)
{
	int ret;
	int config_len;
	struct usb_device *udev, *rhdev;
	...
	log_err("[OTG_H] ===> f_getconfiguration_desc\n");
...
/* kfree(urb); */
	...
	if (ret == RET_SUCCESS) {
		log_notice("[OTG_H] ------------desc->bDescriptorType: 0x%x-------------\n",
		       desc->bDescriptorType);
		log_notice("[OTG_H] ------------desc->wTotalLength: 0x%x----------------\n",
		       desc->wTotalLength);
		log_notice("[OTG_H] ------------desc->bNumInterfaces: 0x%x--------------\n",
		       desc->bNumInterfaces);
		log_notice("[OTG_H] ------------desc->bmAttributes: 0x%x----------------\n",
		       desc->bmAttributes);
		log_notice("[OTG_H] ------------desc->bMaxPower: 0x%x-------------------\n",
		       desc->bMaxPower);
	} else {
		log_err("[OTG_H] [ERROR] f_getconfiguration_desc failed\n");
	}
	...
	log_err("[OTG_H] <=== f_getconfiguration_desc, ret = %dr\n", ret);
	return ret;
}


int f_interface_desc(void)
{
	int ret;
	/*int config_len;*/
	struct usb_device *udev, *rhdev;
...
int f_get_string_desc(void)
{
	...
	log_err("[OTG_H] ===> f_get_string_desc\n");
	...
	log_err("[OTG_H] <=== f_get_string_desc 0 , ret = %d\n", ret);
	...
	log_err("[OTG_H] <=== f_get_string_desc 2 , ret = %d\n", ret);
	...
	kfree(desc);
	usb_free_urb(urb);
	log_err("[OTG_H] <=== f_get_string_desc 1 , ret = %d\n", ret);
...
int f_set_a_hnp(void)
{
	...
	ret = f_ctrlrequest(urb, udev);
	if (ret == RET_SUCCESS)
		log_err("[OTG_H] f_set_a_hnp OK\n");
	else
		log_err("[OTG_H] [ERROR] f_set_a_hnp failed\n");
...
int f_set_configuration(void)
{
	...
	ret = f_ctrlrequest(urb, udev);
	if (ret == RET_SUCCESS)
		log_err("[OTG_H] f_ctrlrequest OK\n");
	else
		log_err("[OTG_H] [ERROR] f_ctrlrequest failed\n");
...
int f_get_otg_status(void)
{
	...
	*desc = 0;
	/* desc = 0; */
/* log_notice("[OTG_H] before get OTG status, addr 0x%x content=%d\n", desc, *desc); */
	urb = alloc_ctrl_urb(dr, (char *)desc, udev);
	ret = f_ctrlrequest(urb, udev);
/* log_notice("[OTG_H] get OTG status, addr 0x%x content=%d\n", desc, *desc); */
	ret = *desc;
	kfree(dr);
	kfree(desc);
	usb_free_urb(urb);
...
u32 f_load_to_fifo(u32 ep_num, u32 length, u8 *buf, u32 maxp)
{
	...
	if (ep_num == 0) {
		if (count == 0) {
			os_log_notice(K_DEBUG, "USB_EP0_DATAEND %8X+\n", os_readl(U3D_EP0CSR));
			...
			if (count < maxp) {
				...
				os_log_notice(K_DEBUG, "EP0_TXPKTRDY\n");
			}
#else
			os_writel(U3D_EP0CSR, os_readl(U3D_EP0CSR) | EP0_TXPKTRDY);
#endif
		...
		if (count == 0) {
			...
			if (count < maxp) {
				USB_WriteCsr32(U3D_TX1CSR0, ep_num,
					       USB_ReadCsr32(U3D_TX1CSR0, ep_num) | TX_TXPKTRDY);
				os_log_notice(K_DEBUG, "short packet\n");
			}
#else
			USB_WriteCsr32(U3D_TX1CSR0, ep_num,
				       USB_ReadCsr32(U3D_TX1CSR0, ep_num) | TX_TXPKTRDY);
...
#endif
#if 0
void musb_host_load_testpacket(void)
{
...
void otg_dump_xhci_register(void)
{
	...
	log_notice("cap_regs: hc_capbase ==================>\n");
	addr = &xhci->cap_regs->hc_capbase;
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	...
	log_notice("op_regs:command_base ==================>\n");
	...
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	addr = (u32 __iomem *)&xhci->op_regs->cmd_ring;
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	addr = (u32 __iomem *)&xhci->op_regs->dcbaa_ptr;
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	addr = &xhci->op_regs->config_reg;
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	log_notice("op_regs: port_status_base <==================\n");
	...
	log_notice("run_reg: run_reg_base ==================>\n");
	...
	log_notice("intr_reg: xhci_intr_reg_base ==================>\n");
	...
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	addr = (u32 __iomem *)&xhci->ir_set->erst_base;
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
	addr = (u32 __iomem *)&xhci->ir_set->erst_dequeue;
	temp = xhci_readl(xhci, addr);
	log_notice("[0x%p] 0x%x\n", addr, temp);
...
int otg_drv_vbus_on(bool on)
{
	...
	log_err("[OTG_H] Set dma to host done\n");
	temp = readl(SSUSB_U2_CTRL(0));
	temp = temp | SSUSB_U2_PORT_HOST_SEL;
	writel(temp, SSUSB_U2_CTRL(0));
	log_err("[OTG_H] enter u3h_pet_A_host thread\n");
	/* if(f_test_lib_init() != RET_SUCCESS){ */
	/* log_err("[OTG_H][FAIL] controller driver init failed\n"); */
	/* } */
	...
	log_err("[OTG_H] manual set SSUSB_ATTACH_A_ROLE\n");
	...
	/* temp = readl(SSUSB_U3_XHCI_BASE + 0x47c8) ; */
	/* log_err("[OTG_H] read  SSUSB_U3_XHCI_BASE + 0x47c8 is 0x%x\n", temp); */
	/* port_id = 1; */
	/* xhci = hcd_to_xhci(my_hcd); */
	...
	if (!on) {
		/* turn off vbus */
		log_notice("[OTG_H] turn off vbus\n");
		...
		if (g_hs_block_reset) {
			...
			writel((readl(SSUSB_U2_SYS_BASE + 0xc) & (~0x100)),
			       SSUSB_U2_SYS_BASE + 0xc);
		}
		/* set port_power=0 */
...
#endif
		log_notice("[OTG_H]turn off vbus done, OTG_STS: 0x%x\n", readl(SSUSB_OTG_STS));

	} else {		/* HOST mode */
		/* turn on vbus */
		log_notice("[OTG_H]turn on vbus\n");
		...
		log_notice("[OTG_H] turn on vbus done, OTG_STS: 0x%x\n", readl(SSUSB_OTG_STS));
		/* mu3d_hal_dump_register() ; */
	}


	log_err("[OTG_H] Exit otg_drv_vbus_on\n");
...
int f_host_test_mode(int case_num)
{
	...
	writel(temp, SSUSB_U2_CTRL(0));
	temp = readl(SSUSB_U2_CTRL(0));
	log_notice("SSUSB_U2_CTRL is 0x%x\n", temp);
	...
	log_notice("USB2_PORT_SC(0x%p is 0x%x\n", addr, temp);
	...
	usb_phy_recover(0);
#endif
	log_notice("U3D_POWER_MANAGEMENT is 0x%x\n", readl(POWER_MANAGEMENT));
	writel(readl(POWER_MANAGEMENT) | USB_HS_ENABLE, POWER_MANAGEMENT);


	/* msleep(2000) ; */

	log_notice("SSUSB_OTG_STS is 0x%x\n", readl(SSUSB_OTG_STS));

	switch (case_num) {
	case HOST_CMD_TEST_SE0_NAK:
		log_notice("TEST_SE0_NAK\n");
		...
		log_notice("USB2_PORT_PMSC(0x%p) is 0x%x\n", addr, temp);
	...
	case HOST_CMD_SUSPEND_RESUME:	/* HS_HOST_PORT_SUSPEND_RESUME */
		log_notice("HS_HOST_PORT_SUSPEND_RESUME\n");
		msleep(5000);	/* the host must continue sending SOFs for 15s */
		log_notice("please begin to trigger suspend!\n");
		msleep(10000);

		/* suspend bus */
		addr = &xhci->op_regs->port_status_base + NUM_PORT_REGS * ((port_id - 1) & 0xff);
		temp = xhci_readl(xhci, addr);
		log_notice("[OTG_H]before reset port %d = 0x%x\n", (port_id - 1), temp);
		temp = mtktest_xhci_port_state_to_neutral(temp);
		temp = (temp & ~(0xf << 5));
		...
		msleep(5000);
		log_notice("please begin to trigger resume!\n");
		msleep(10000);
	...
	case HOST_CMD_GET_DESCRIPTOR:	/* SINGLE_STEP_GET_DEVICE_DESCRIPTOR setup */
		log_notice("HOST_CMD_GET_DESCRIPTOR\n");
		/* the host issues SOFs for 15s allowing the test engineer to */
	...
	case HOST_CMD_SET_FEATURE:	/* SINGLE_STEP_GET_DEVICE_DESCRIPTOR execute */
		log_notice("HOST_CMD_SET_FEATURE\n");
		/* get device descriptor */
		f_getdescriptor();
		/* musb_h_setup(&setup_packet); */
		msleep(15000);
...
int f_host_test_mode_stop(void)
{
	u32 temp;
	struct xhci_hcd *xhci;
	/*u32 halted;*/
	...
	log_notice("f_host_test_mode_stop ===>\n");
...
int otg_opt_uut_a(void *data)
{
	...
	writel(temp, SSUSB_U2_CTRL(0));
#endif
	log_err("[OTG_H] enter u3h_uut_A_host thread\n");
	if (f_test_lib_init() != RET_SUCCESS) {
		log_err("[OTG_H][FAIL] controller driver init failed\n");
		goto FAIL_RETURN;
	}
	...
	/* set hnp */
	f_set_hnp();
	log_err("[OTG_H]Set HNP done\n");
	/* set configuration */
	f_set_configuration();
	log_err("[OTG_H]Set configuration done\n");
	g_port_resume = 0;
	g_port_connect = 1;
TD_4_6:
	g_otg_hnp_become_dev = false;
	...
	log_err("[OTG_H]Device wakeup or disconnect\n");
	if (g_port_resume == 1) {
		/* do remote wakeup */
		log_err("[OTG_H]Device do remote wakeup\n");
		msleep(20);
		temp = xhci_readl(xhci, addr);
		temp = mtktest_xhci_port_state_to_neutral(temp);
		temp = (temp & ~(0xf << 5));
		...
		if (PORT_PLS_VALUE(temp) != 0) {	/* (temp & PORT_PLS(15)) != PORT_PLS(0)){ */
			...
			log_err("[OTG_H]exit u3h_uut_A_host thread\n");
		} else {
			log_err("[OTG_H]exit u3h_uut_A_host thread\n");
		}
	}
	if (g_port_connect == 0) {
		log_err("[OTG_H]Disconnect, looks device do hnp\n");
		if (!g_otg_hnp_become_dev) {
			log_err("[OTG_H][FAILE]Doesn't become device, exit\n");
			goto FAIL_RETURN;
		}
		ret = f_disable_slot();
		if (ret) {
	...
	log_err("[OTG_H]Wait to become host again\n");
...
int otg_opt_uut_b(void *data)
{
	...
	temp = temp | SSUSB_ATTACH_A_ROLE_INT_EN | SSUSB_CHG_A_ROLE_A_INT_EN
	    | SSUSB_CHG_B_ROLE_A_INT_EN | SSUSB_SRP_REQ_INTR_EN;
	writel(temp, SSUSB_OTG_INT_EN);
	log_err("[OTG_H] enter u3h_uut_B_host thread\n");
...
#endif
	if (f_test_lib_init() != RET_SUCCESS) {
		log_err("[OTG_H] controller driver init failed\n");
		goto FAIL_RETURN;
	}
	...
	log_err("[OTG_H] Wait to become host\n");
	while (!g_otg_hnp_become_host) {
		msleep(20);
		if (g_stopped) {
			log_err("[OTG_H]force stop\n");
...
int otg_pet_uut_a(void *data)
{
	...
	while (g_stopped == false) {
		...
		if ((g_otg_pet_status == OTG_DISCONNECTED) && (vbus_on_timeout == 0) && vbus_on) {
			...
			log_notice("[OTG_H] vbus on and no connect event timeout, turn off vbus\n");
#if 0
		...
		if (g_otg_iddig != cur_iddig) {
			if (g_otg_iddig == 1) {	/* DEVICE mode */
				...
				writel(temp, SSUSB_U2_CTRL(0));
				msleep(20);
				temp = readl(DEVICE_CONTROL);
				temp = temp | (DEVICE_CONTROL_otg2_en);
				writel(temp, DEVICE_CONTROL);
...
#if 0
				addr =
				    &xhci->op_regs->port_status_base + NUM_PORT_REGS * (0 & 0xff);
				temp = xhci_readl(xhci, addr);
				temp = mtktest_xhci_port_state_to_neutral(temp);
				temp &= ~PORT_POWER;
				xhci_writel(xhci, temp, addr);
				...
				mtk_set_host_mode_out();
#endif
				log_notice("[OTG_H] iddig up, turn off vbus done, OTG_STS: 0x%x\n",
				       readl(SSUSB_OTG_STS));
				cur_iddig = 1;
				...
				log_notice("Device ct2 =%x\n", readl(DEVICE_CONTROL));
				...
				log_notice("[OTG_H] iddig down, turn on vbus done, OTG_STS: 0x%x\n",
				       readl(SSUSB_OTG_STS));
				cur_iddig = 0;
		...
		if (g_otg_csc && g_otg_pet_status != OTG_HNP_DEV) {
			...
			if (g_port_connect) {
				...
				if (!g_port_connect) {
					...
					ret = f_disable_slot();
					if (ret) {
						log_notice("[OTG_H][FAIL] disable slot failed!!!!!!!!!!\n");
						/* goto FAIL_RETURN ; */
					}
					g_otg_pet_status = OTG_DISCONNECTED;
					g_otg_slot_enabled = false;
				...
				ret = f_getdescriptor();
				if (ret == RET_FAIL) {
					log_notice("[OTG_H] ============Device No Response=================\n");
					usbif_u3h_test_send_event(USBIF_OTG_EVENT_DEV_CONN_TMOUT);
				...
				log_err("[OTG_H] g_otg_dev_conf_len is %d\n",
				       g_otg_dev_conf_len);
				f_get_string_desc();
				...
				if (g_otg_dev_conf_len == 21) {
					/* otg13 */
					/* set host hnp */
					log_err("[OTG_H] OTG13 set A_HNP_SUPPORT\n");
					f_set_a_hnp();
				}
				f_set_configuration();
				log_err("[OTG_H]Set configuration done\n");
				...
				if (g_otg_pet_status == OTG_HNP_SUSPEND) {
					...
					/* alert device */
					log_err("[OTG_H] set CHG_B_ROLE_B\n");
					writel(SSUSB_CHG_B_ROLE_B, SSUSB_OTG_STS);
					log_err("[OTG_H]Port become device\n");
					/* continue; */
				} else {
					g_otg_pet_status = OTG_DISCONNECTED;
				}
		...
		if (g_otg_pet_status == OTG_POLLING_STATUS) {
			...
			if (ret == 1) {
				log_err("[OTG_H]HNP enabled got\n");
				g_otg_pet_status = OTG_SET_NHP;
			}
			/* log_err("[OTG_H]get otg status = %d\n", ret); */
			continue;
		}
		if (g_otg_pet_status == OTG_SET_NHP) {
			...
			if (ret == RET_FAIL) {
				log_notice("[OTG_H]================Device No Response===============");
				log_err("[OTG_H] suspending port\n");
				/* usbif_u3h_test_send_event(USBIF_OTG_EVENT_NO_RESP_FOR_HNP_ENABLE); */
				/* suspend bus */
				port_id = 1;
				xhci = hcd_to_xhci(my_hcd);
				addr =
				    &xhci->op_regs->port_status_base + NUM_PORT_REGS * ((port_id - 1) & 0xff);
				temp = xhci_readl(xhci, addr);
		...
		*/
		if ((g_otg_pet_status == OTG_HNP_DEV) && g_otg_hnp_become_host) {
			...
			while ((!g_port_connect || !g_port_reset) && (vbus_on_timeout > 0)) {
				if (g_otg_iddig == 1) {
					log_notice("[OTG_H] IDDIG is up, just goto disconnect state\n");
					vbus_on_timeout = 0;
					break;
				}
				vbus_on_timeout--;
			...
			if (vbus_on_timeout == 0) {
				log_notice("[OTG_H] Not connected after role change back to host, back to DISCONNECT state\n");
				g_otg_csc = false;
				g_port_reset = false;
				g_otg_pet_status = OTG_DISCONNECTED;
				continue;
			...
			if (!g_port_connect) {
				log_notice("[OTG_H] Device disconnected, go to DISCONNECTED\n");
				g_otg_csc = false;
				g_port_reset = false;
				g_otg_pet_status = OTG_DISCONNECTED;
				mb(); /* */
...
int otg_pet_uut_b(void *data)
{
	...
	/* USBIF , HCRST should not be software reset for U2 MAC port0 */
	/*  HCRST_U2_MAC_EN_0P RW PUBLIC , host HCRST does not reset MAC2 */
	/* writel((readl(0xf00409b4) & ~(0x10000)), 0xf00409b4);*/
	/* writel((readl(SSUSB_U3_XHCI_BASE + 0x9b4) & ~(0x10000)), SSUSB_U3_XHCI_BASE + 0x9b4); */
	port_id = 1;
	xhci = hcd_to_xhci(my_hcd);
		...
		if (g_otg_pet_status == OTG_DEV) {
			...
			if (g_otg_hnp_become_host) {	/* receive SSUSB_CHG_A_ROLE_A from U3D test driver */
				...
				if (!g_otg_wait_con && !g_port_connect) {
					log_err("[OTG_H]Disconnected, back to device\n");
#if 0
					port_id = 1;
					xhci = hcd_to_xhci(my_hcd);
					addr =
					...
					while (readl(SSUSB_OTG_STS) & SSUSB_XHCI_MAS_DMA_REQ) {
						msleep(20);
						log_err("[OTG_H] DMA in used\n");
					}
					writel(SSUSB_CHG_B_ROLE_B, SSUSB_OTG_STS);
					log_err("[OTG_H]alert device done\n");
#endif
					continue;
				}
				if (g_port_connect && g_port_reset) {
					log_err("[OTG_H] Device attached\n");
					/* avoid doing the follow action with the fast disconnection case */
					msleep(200);
					if (g_otg_hnp_become_dev) {
						log_err("[OTG_H] early disconnectd, skip enumeration\n");
					...
					log_err("[OTG_H] start enumeration device\n");
					...
					log_err("[OTG_H] enable slot done\n");
					if (!g_port_connect) {
