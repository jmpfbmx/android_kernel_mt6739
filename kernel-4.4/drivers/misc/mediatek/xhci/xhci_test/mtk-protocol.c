...
/* ret=hcd_ctrl_request(dr,ptr); */
	...
	if (ret != RET_SUCCESS) {
		pr_err("[DEV]query status ctrl request failed!!\n");
		ret = STATUS_FAIL;
	} else {
		ret = query->result;
	}
...
int dev_polling_result(void)
{
	...
	count = POLLING_COUNT;
	delay_msecs = POLLING_STOP_DELAY_MSECS;

	for (i = 0; i < count; i++) {
		value = dev_query_result();
		pr_notice("polling device status: %d !!\n", value);
		msleep(delay_msecs);
...
int dev_polling_status(struct usb_device *dev)
{
	...
	count = POLLING_COUNT;
	delay_msecs = POLLING_DELAY_MSECS;
...
/* kfree(urb); */
	...
	if (ret) {
		pr_err("[DEV]reset device ctrl request failed!!\n");
		return RET_FAIL;
	}
...
int dev_config_ep0(int maxp, struct usb_device *usbdev)
{
	struct usb_device *udev, *rhdev;
	struct urb *urb;
	struct usb_ctrlrequest *dr;
...
/* kfree(urb); */
	...
	if (ret) {
		pr_err("[DEV]config ep ctrl request failed!!\n");
		return RET_FAIL;
	}
...
int dev_config_ep(char ep_num, char dir, char type, int maxp, char bInterval, char slot,
			char burst, char mult, struct usb_device *dev)
{
	struct usb_device *udev, *rhdev;
	struct urb *urb;
	struct usb_ctrlrequest *dr;
...
int dev_ctrl_loopback(int length, struct usb_device *dev)
{
	...
	for (i = 0; i < length; i++) {
		if ((*(ptr1 + i)) != (*(ptr2 + i))) {
			pr_err("[ERROR] buffer %d not match, tx 0x%x, rx 0x%x\n", i,
						 *(ptr1 + i), *(ptr2 + i));
			break;
		}
	}
...
int dev_loopback(char bdp, int length, int gpd_buf_size, int bd_buf_size, char dram_offset,
		 char extension, struct usb_device *dev)
{
	struct usb_device *udev, *rhdev;
	struct usb_ctrlrequest *dr;
	char *ptr;
...
#if 0
int dev_remotewakeup(char bdp, int length, int gpd_buf_size, int bd_buf_size,
...
#endif
int dev_stress(char bdp, int length, int gpd_buf_size, int bd_buf_size, char num,
...
int dev_random_stop(int length, int gpd_buf_size, int bd_buf_size, char dev_dir_1, char dev_dir_2,
				int stop_count_1, int stop_count_2)
{
	struct usb_device *udev, *rhdev;
	struct usb_ctrlrequest *dr;
	char *ptr;
...
int dev_power(int test_mode, char u1_value, char u2_value, char en_u1, char en_u2,
				struct usb_device *usbdev)
{
	struct usb_device *udev, *rhdev;
	struct usb_ctrlrequest *dr;
