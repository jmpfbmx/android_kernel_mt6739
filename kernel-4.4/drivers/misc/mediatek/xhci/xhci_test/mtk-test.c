...
static int t_u3auto_hw_lpm1(int argc, char **argv);
...
static long xhci_mtk_test_unlock_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case IOCTL_READ:
		if (copy_to_user((char *) arg, r_buf_host, len))
			return -EFAULT;
		mtk_test_dbg("IOCTL_READ: %s\r\n", r_buf_host);
		break;
	case IOCTL_WRITE:
		if (copy_from_user(w_buf_host, (char *) arg, len))
			return -EFAULT;
...
void framecount_disabe(void)
{
	writel(0x01, (void __iomem *)0xf1270944);
	mtk_test_dbg("disable frame counter\n");
}

void framecount_enable(void)
{
	writel(0x00, (void __iomem *)0xf1270944);
	mtk_test_dbg("enable frame counter\n");
}
int u3auto_hcd_reset(void)
{
	if (xhci_usbif_resource_get())
		return RET_FAIL;
...
static int t_u3auto_ctrl_loopback(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &num))
			return RET_FAIL;
	}
	if (argc > 2) {
		/* length = (int)simple_strtol(argv[2], &argv[2], 10); */
		if (kstrtoint(argv[2], 0, &length))
			return RET_FAIL;
	}
	if (argc > 3) {
		if (!strcmp(argv[3], "ss")) {
...
#define SG_HS_ISOC_INTERVAL_SIZE  1
#define SG_HS_ISOC_MAXP_SIZE    1
#define SG_HS_ISOC_OUT_EP_SIZE    1
#define SG_HS_ISOC_IN_EP_SIZE   1
#define SG_HS_ISOC_SG_LEN_SIZE    3
...
#define SG_FS_ISOC_INTERVAL_SIZE  2
#define SG_FS_ISOC_MAXP_SIZE    2
#define SG_FS_ISOC_OUT_EP_SIZE    1
#define SG_FS_ISOC_IN_EP_SIZE   1
#define SG_FS_ISOC_SG_LEN_SIZE    3
...
static int t_u3auto_loopback_scan_sg(int argc, char **argv)
{
	...
	int speed;
	int /*start_add_index,*/ ep_out_index, ep_in_index, maxp_index;
	int interval_index, sg_len_index, mult_index, burst_index;
	int interval_arr_size, maxp_arr_size, out_ep_arr_size;
	int in_ep_arr_size, sg_len_arr_size, mult_arr_size, burst_arr_size;
	...
	int sg_hs_isoc_interval[SG_HS_ISOC_INTERVAL_SIZE] = {1};
	int sg_hs_isoc_maxp[SG_HS_ISOC_MAXP_SIZE] = {1024}; /* 32, 512, 1024 */
	int sg_hs_isoc_out_ep[SG_HS_ISOC_OUT_EP_SIZE] = {2};
	int sg_hs_isoc_in_ep[SG_HS_ISOC_IN_EP_SIZE] = {2};
	int sg_hs_isoc_sg_len[SG_HS_ISOC_SG_LEN_SIZE] = {64, 512, 1024};
	...
	int sg_fs_isoc_interval[SG_FS_ISOC_INTERVAL_SIZE] = {1, 2};
	int sg_fs_isoc_maxp[SG_FS_ISOC_MAXP_SIZE] = {128, 1023};
	int sg_fs_isoc_out_ep[SG_FS_ISOC_OUT_EP_SIZE] = {1};
	int sg_fs_isoc_in_ep[SG_FS_ISOC_IN_EP_SIZE] = {1};
	int sg_fs_isoc_sg_len[SG_FS_ISOC_SG_LEN_SIZE] = {64, 512, 1024};
	...
	if (speed == DEV_SPEED_HIGH && transfer_type == EPATT_ISO) {
		arr_maxp = sg_hs_isoc_maxp;
		maxp_arr_size = SG_HS_ISOC_MAXP_SIZE;
		arr_interval = sg_hs_isoc_interval;
		interval_arr_size = SG_HS_ISOC_INTERVAL_SIZE;
		/* length 1~2048 */
		sec = init_num_sec(64-10, 64+10);
		add_num_sec(512-10, 512+10, sec);
		add_num_sec(10240-10, 10240+10, sec);
		/* start_add 0~63 */
		min_start_add = 0;
		max_start_add = 0;
		/* ep_out_num 1,8,15 */
		arr_ep_out = sg_hs_isoc_out_ep;
		out_ep_arr_size = SG_HS_ISOC_OUT_EP_SIZE;
		/* ep_in_num 1,8,15 */
		arr_ep_in = sg_hs_isoc_in_ep;
		in_ep_arr_size = SG_HS_ISOC_IN_EP_SIZE;
		/* sg_len */
		arr_sg_len = sg_hs_isoc_sg_len;
		sg_len_arr_size = SG_HS_ISOC_SG_LEN_SIZE;
	...
	if (speed == DEV_SPEED_FULL && transfer_type == EPATT_ISO) {
		arr_maxp = sg_fs_isoc_maxp;
		maxp_arr_size = SG_FS_ISOC_MAXP_SIZE;
		arr_interval = sg_fs_isoc_interval;
		interval_arr_size = SG_FS_ISOC_INTERVAL_SIZE;
		/* length 1~2048*/
		sec = init_num_sec(512-10, 512+10);
		add_num_sec(10240-10, 10240+10, sec);
		/* start_add 0~63 */
		min_start_add = 0;
		max_start_add = 0;
		/* ep_out_num 1,8,15 */
		arr_ep_out = sg_fs_isoc_out_ep;
		out_ep_arr_size = SG_FS_ISOC_OUT_EP_SIZE;
		/* ep_in_num 1,8,15 */
		arr_ep_in = sg_fs_isoc_in_ep;
		in_ep_arr_size = SG_FS_ISOC_IN_EP_SIZE;
		/* sg_len */
		arr_sg_len = sg_fs_isoc_sg_len;
		sg_len_arr_size = SG_FS_ISOC_SG_LEN_SIZE;
...
#define HS_INTR_OUT_EP_SIZE   2
#define HS_INTR_IN_EP_SIZE    2
...
static int t_u3auto_loopback_scan(int argc, char **argv)
{
	...
	int ss_bulk_out_ep[SS_BULK_OUT_EP_SIZE] = {1};
	int ss_bulk_in_ep[SS_BULK_IN_EP_SIZE] = {1};
	int ss_bulk_burst[SS_BULK_BURST_SIZE] = {0, 15};
	int ss_bulk_mult[SS_BULK_MULT_SIZE] = {0};

	int ss_intr_interval[SS_INTR_INTERVAL_SIZE] = {1, 2};
	int ss_intr_maxp[SS_INTR_MAXP_SIZE] = {8, 1024};
	...
	int ss_isoc_interval[SS_ISOC_INTERVAL_SIZE] = {1, 2};
	int ss_isoc_maxp[SS_ISOC_MAXP_SIZE] = {512, 1024};
	...
	int ss_hb_isoc_burst[SS_HB_ISOC_BURST_SIZE] = {1, 2};
	int ss_hb_isoc_mult[SS_HB_ISOC_MULT_SIZE] = {0, 1};
	...
	int hs_intr_interval[HS_INTR_INTERVAL_SIZE] = {1, 2}; /* 0,8,15 */
	int hs_intr_maxp[HS_INTR_MAXP_SIZE] = {32, 128, 512, 1024}; /* 8,16,128,512, 1024, 2048, 3072 */
	int hs_intr_out_ep[HS_INTR_OUT_EP_SIZE] = {1, 2};
	int hs_intr_in_ep[HS_INTR_IN_EP_SIZE] = {1, 2};

	int hs_hb_intr_interval[HS_HB_INTR_INTERVAL_SIZE] = {1, 2}; /* 0,8,15 */
	...
	int hs_isoc_maxp[HS_ISOC_MAXP_SIZE] = {128, 1024};
	...
	for (ep_out_index = 0; ep_out_index < out_ep_arr_size; ep_out_index++) {
		...
		for (maxp_index = 0; maxp_index < maxp_arr_size; maxp_index++) {
			...
			for (interval_index = 0; interval_index < interval_arr_size; interval_index++) {
				...
				if (speed == DEV_SPEED_SUPER && transfer_type == EPATT_ISO) {
					dma_burst_sz = 0;
					dma_tmp = readl((void __iomem *)SSUSB_XHCI_HDMA_CFG);
				...
				if (speed == DEV_SPEED_SUPER && transfer_type == EPATT_ISO) {
					...
					ret = f_loopback_loop_gpd(ep_out_num, ep_in_num, length,
						start_add, gpd_buf_size, NULL);
					if (ret) {
...
static int t_u3auto_hw_lpm1(int argc, char **argv)
{
	...
	if (argc > 1) {
		if (!strcmp(argv[1], "ss")) {
			pr_err("Test super speed\n");
			speed = DEV_SPEED_SUPER; /* TODO: superspeed */
		} else if (!strcmp(argv[1], "hs")) {
			pr_err("Test high speed\n");
			speed = DEV_SPEED_HIGH;
		} else if (!strcmp(argv[1], "fs")) {
			pr_err("Test full speed\n");
			speed = DEV_SPEED_FULL;
		}
	}
	if (argc > 2) {
		if (!strcmp(argv[2], "bulk")) {
			pr_err("Test bulk transfer\n");
			transfer_type = EPATT_BULK;
		} else if (!strcmp(argv[2], "intr")) {
			pr_err("Test intr transfer\n");
			transfer_type = EPATT_INT;
		} else if (!strcmp(argv[2], "isoc")) {
			pr_err("Test isoc transfer\n");
			transfer_type = EPATT_ISO;
		} else if (!strcmp(argv[2], "ctrl")) {
			pr_err("Test ctrl transfer\n");
		...
		for (besl = 0; besl < 8; besl++) {
			for (besld = besl + 6; besld < 10; besld++) {
				...
				for (l1_timeout = l1_timeout_min; l1_timeout <= l1_timeout_max; l1_timeout++) {
					...
					ret = f_disable_slot();
					if (ret) {
						pr_err("disable slot failed!!!!!!!!!!\n");
						return ret;
					}

					/* if XHCI_DEBUG = 1, the debug log is so much that it affects the timing :
					*  when we read the port status,
					*  it is in connected already and the disconnected status is ignored
					*  so, if XHCI_DEBUG = 1, we only wait the device connect to the host.
					*/
#if XHCI_DEBUG
					ret = f_enable_port(0);
					if (ret != RET_SUCCESS) {
						pr_err("device enable failed!!!!!!!!!!\n");
						return ret;
					}
#else
					ret = f_reenable_port(0);
					if (ret != RET_SUCCESS) {
						pr_err("device reenable failed!!!!!!!!!!\n");
						return ret;
					}
#endif
					...
					ret = f_enable_slot(NULL);
					if (ret) {
						pr_err("enable slot failed!!!!!!!!!!\n");
						return ret;
					}

					ret = f_address_slot(false, NULL);
					if (ret) {
						pr_err("address slot failed!!!!!!!!!!\n");
						return ret;
					}
					...
					ret = f_evaluate_context(max_exit_latency, maxp0, preping_mode,
						preping, besl_preping, besld_preping);

					if (ret != RET_SUCCESS) {
						pr_err("evaluate slot context fail!!\n");
						return RET_FAIL;
					}

						/* ==phase 1 : config EP==*/
					if (transfer_type != EPATT_CTRL) {
						ret = dev_config_ep(ep_out_num, USB_RX, transfer_type, maxp, bInterval,
							dev_mult, burst, mult, NULL);
					if (ret) {
						pr_err("config dev EP fail!!\n");
						return ret;
					}

						ret = dev_config_ep(ep_in_num, USB_TX, transfer_type, maxp, bInterval,
							dev_mult, burst, mult, NULL);
					if (ret) {
						pr_err("config dev EP fail!!\n");
						return ret;
						...
						ret = f_config_ep(ep_out_num, EPADD_OUT, transfer_type, maxp, bInterval,
							burst, mult, NULL, 0);
					if (ret) {
						pr_err("config EP fail!!\n");
						return ret;
					}

						ret = f_config_ep(ep_in_num, EPADD_IN, transfer_type, maxp, bInterval,
							burst, mult, NULL, 1);
					if (ret) {
						pr_err("config EP fail!!\n");
						return ret;
					...
					pr_err("U3D LPM config:\n");
					pr_err("    lpm_mode = %d\n", lpm_mode);
					pr_err("    wakeup = %d\n", wakeup);
					pr_err("    beslck = %d, beslck_u3 = %d, beslckd = %d\n",
						beslck, beslck_u3, beslckd);
					pr_err("    cond = %d, cond_en = %d\n", cond, cond_en);

					dev_lpm_config_host(lpm_mode, wakeup, beslck,
						beslck_u3, beslckd, cond, cond_en);


					hle = 1;

					pr_err("U3H LPM config:\n");
					pr_err("    hle = %d\n", hle);
					pr_err("    rwe = %d\n", rwe);
					pr_err("    hirdm = %d\n", hirdm);
					pr_err("    besl = %d, besld = %d\n", besl, besld);
					pr_err("    besl_preping = %d, besld_preping = %d\n",
						besl_preping, besld_preping);
					pr_err("    bInterval = %d, l1_timeout = %d\n", bInterval, l1_timeout);

					f_power_config_lpm(g_slot_id, hirdm, l1_timeout, rwe,
						besl, besld, hle, 0, 0);
#endif
					/* ==phase 3 : loopback==*/
					if (transfer_type != EPATT_CTRL) {
			...
			for (besld = besl + 6; besld < 10; besld++) {
				...
				for (l1_timeout = l1_timeout_min; l1_timeout <= l1_timeout_max; l1_timeout++) {
					...
					*/
					...
					/*evaluate the slot context to update besl_preping/besld_preping*/
					ret = f_evaluate_context(max_exit_latency, maxp0, preping_mode, preping,
						besl_preping, besld_preping);
...
static int t_u3auto_random_suspend(int argc, char **argv)
{
	...
	int mult, burst, dev_mult;
	int dram_offset, extension;
	int suspend_count, suspend_boundary;
	char isSuspend;
	...
	if (argc > 2) {
		/* suspend_boundary = (int)simple_strtol(argv[2], &argv[2], 10); */
		if (kstrtoint(argv[2], 0, &suspend_boundary))
			return RET_FAIL;
		mtk_test_dbg("suspend_boundary set to %d\n", suspend_boundary);
	}
	...
	suspend_count = 0;
	while (suspend_count < suspend_boundary) {
...
#if 0
	f_config_ep(ep_out_num, EPADD_OUT, transfer_type, maxp, bInterval, burst, mult, NULL, 0);
#endif
	return ret;
}
#if 0
static int t_u3auto_reconfig(int argc, char **argv)
{
	...
	if (argc > 2) {
		...
		if (kstrtoint(argv[2], 0, &round))
			return RET_FAIL;
	}
	if (argc > 3) {
		/* num_ep = (int)simple_strtol(argv[3], &argv[3], 10); */
		if (kstrtoint(argv[3], 0, &num_ep))
			return RET_FAIL;
	}
	for (i = 0; i < round; i++) {
		/* random new endpoint */
...
static int t_u3auto_stress(int argc, char **argv)
{
	...
	/* ==phase 0 : device reset==*/
	if (dev_num == 0) {
		/*doesn't reset now, should reset to tested speed before execute this test item*/
#if 1
		start_port_reenabled(0, speed);
		ret = dev_reset(speed, NULL);
		if (ret) {
...
static int t_u3auto_randomstop_dev(int argc, char **argv)
{
	...
	if (argc > 4) {
		...
		if (kstrtoint(argv[4], 0, &ep_1_num))
			return RET_FAIL;
	}
	if (argc > 5) {
		/* ep_2_num= (int)simple_strtol(argv[5], &argv[5], 10); */
		if (kstrtoint(argv[5], 0, &ep_2_num))
			return RET_FAIL;
	}
	if (argc > 6) {
...
static int t_power_random_access_regs(int argc, char **argv)
{
	...
	if (kstrtoint(argv[1], 0, &port_id))
		return RET_FAIL;
	/* port_rev = (int)simple_strtol(argv[2], &argv[2], 10); */
	if (kstrtoint(argv[2], 0, &port_rev))
		return RET_FAIL;
	if (!strcmp(argv[3], "true"))
		power_required = 1;
	else
...
static int ep_instop, ep_outstop;
...
static int t_loopback_deconfigep(int argc, char **argv)
{
	...
	if (argc > 3) {
		...
		if (kstrtoint(argv[3], 0, &is_config))
			return RET_FAIL;
		mtk_test_dbg("is_config set to %d\n", is_config);
	}
...
static int t_power_u1u2(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &u_num))
			return RET_FAIL;
	}
	if (argc > 2) {
		/* value1 = (int)simple_strtol(argv[2], &argv[2], 10); */
		if (kstrtoint(argv[2], 0, &value1))
			return RET_FAIL;
	}
	if (argc > 3) {
		/* value2 = (int)simple_strtol(argv[3], &argv[3], 10); */
		if (kstrtoint(argv[3], 0, &value2))
			return RET_FAIL;
	}
	if (u_num < 4)
...
static int t_power_u2_swlpm(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &expected_L1S))
			return RET_FAIL;
	}
	if (expected_L1S == 5) {
		expected_L1S = 1;
	...
	if (is_wakeup) {
		wait_event_on_timeout((int *)&g_port_plc, 1, 5000);
		if (g_port_plc == 0) {
...
static int t_power_fla(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[2], 0, &fla_value))
			return RET_FAIL;
...
#endif
	addr = (void __iomem *)0xf0008098;
	temp = readl((void __iomem *)addr);
	temp |= (1<<16);
	writel(temp, addr);
	msleep(20);
	if (g_port_occ == true)
...
static int t_ring_intr_moderation(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &intr_mod_value))
			return RET_FAIL;
...
static int t_ring_idt(int argc, char **argv)
{
	...
	if (argc > 1) {
		if (!strcmp(argv[1], "ss")) {
			pr_err("[DEV]Reset device to super speed\n");
			speed = DEV_SPEED_SUPER;
			maxp = 1024;
			burst = 8;
		} else if (!strcmp(argv[1], "hs")) {
			pr_err("[DEV]Reset device to high speed\n");
			speed = DEV_SPEED_HIGH;
			maxp = 512;
			burst = 0;
		} else if (!strcmp(argv[1], "fs")) {
			pr_err("[DEV]Reset device to full speed\n");
			speed = DEV_SPEED_FULL;
			maxp = 64;
			burst = 0;
		}
...
static int t_ring_noop_transfer(int argc, char **argv)
{
	...
	urb_tx1->transfer_buffer = (void *)NULL;
	urb_tx2->transfer_buffer = (void *)NULL;
	urb_tx1->transfer_dma = (dma_addr_t)NULL;
	urb_tx2->transfer_dma = (dma_addr_t)NULL;
	...
	urb_rx1->transfer_buffer = (void *)NULL;
	urb_rx1->transfer_dma = (dma_addr_t)NULL;
	urb_rx2->transfer_buffer = (void *)NULL;
	urb_rx2->transfer_dma = (dma_addr_t)NULL;
...
static int t_hub_configurehub(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &port_index))
			return RET_FAIL;
	...
	f_hub_setremoteWakeup((port_index+1), 1);
...
static int t_hub_ixia_stress(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &dev_count))
			return RET_FAIL;
...
static int t_hub_loop_stress(int argc, char **argv)
{
	...
	if (argc > 2) {
		...
		if (kstrtoint(argv[2], 0, &ep_count))
			return RET_FAIL;
...
static int t_hub_loop(int argc, char **argv)
{
	...
	if (round == 1) {
		...
		if (sg_len == 0)
			...
			ret = f_loopback_sg_loop(ep_out_num, ep_in_num, length, start_add, sg_len, udev);
...
static int t_hub_set_u1u2(int argc, char **argv)
{
	...
	if (argc > 4) {
		...
		if (kstrtoint(argv[4], 0, &port_num))
			return RET_FAIL;
		xhci_dbg(xhci, "port_num set to %d\n", port_num);
	}
...
static int t_dev_notification(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &type))
			return RET_FAIL;
	}
	if (argc > 2) {
		/* value = (int)simple_strtol(argv[2], &argv[2], 10); */
		if (kstrtoint(argv[2], 0, (int *)&value))
			return RET_FAIL;
	}
	f_enable_dev_note();
	...
	ret = wait_event_on_timeout((int *)&g_dev_notification, type, CMD_TIMEOUT);
	if (ret) {
		mtk_test_dbg("device notification event not received!!\n");
		return ret;
	}
	if (g_dev_not_value != value) {
		mtk_test_dbg("device notification value error, expected %ld, recived %ld!!\n", value, g_dev_not_value);
		return RET_FAIL;
	}
	return ret;
}
...
#if 0
static int t_cmd_debug(int argc, char **argv)
{
	if (u3auto_hcd_reset() != RET_SUCCESS)
		return RET_FAIL;
}

static int t_event_debug(int argc, char **argv)
{
	if (u3auto_hcd_reset() != RET_SUCCESS)
		return RET_FAIL;
}
...
static int t_dev_remotewakeup(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		if (kstrtoint(argv[1], 0, &delay_us))
			return RET_FAIL;
...
static int t_u3auto_concurrent_remotewakeup(int argc, char **argv)
{
		...
		if (argc > 2) {
			...
			if (kstrtoint(argv[2], 0, &count))
				return RET_FAIL;
		}
		if (argc > 3) {
			/* delay_time = (int)simple_strtol(argv[3], &argv[3], 10); */
			if (kstrtouint(argv[3], 0, &delay_time))
				return RET_FAIL;
		}
		...
		for (i = 0; i < count; i++) {
			...
			host_delay_ms = get_random_bytes(&rand_num, sizeof(rand_num));
			/* suspend 30~60 ms to make sure the host side check timing saving */
			dev_delay_ms = 30 + rand_num % 30;
			#else
			dev_delay_us = 0;
			#endif
...
static int t_start_host_test_mode(int argc, char **argv)
{
	...
	pr_err("[OTG_H] call t_start_host_test_mode\n");

	td_no = 0;  /*  td_no = 6 ~ 12 (0x6 ~ 0xc) */
	if (argc > 1) {
		/*td_no = (int)simple_strtol(argv[1], &argv[1], 10);*/
		if (kstrtoint(argv[1], 0, &td_no))
			return RET_FAIL;
	}
...
#endif
	if (f_test_lib_init() != RET_SUCCESS) {
		pr_err("[OTG_H] controller driver init failed\n");
		return RET_FAIL;
	}
	return RET_SUCCESS;
}
							...
							int type, gfp_t flags)
{
	...
	WARN_ON((type != XHCI_CTX_TYPE_DEVICE) && (type != XHCI_CTX_TYPE_INPUT));
	ctx->type = type;
...
static int u2ct_disconnect_detect(void)
{
	...
	wait_event_on_timeout((int *)&g_port_connect, true, TRANS_TIMEOUT);
	if (!g_port_connect) {
	...
	wait_event_on_timeout((int *)&g_port_connect, false, TRANS_TIMEOUT);
	if (g_port_connect) {
...
static int dbg_sch_algorithm(int argc, char **argv)
{
	...
	int /*is_continue,*/ ep_boundary, ep_num;
	...
	if (argc > 1) {
		...
		/* st_speed = (int)simple_strtol(argv[1], &argv[1], 10); */
		if (kstrtoint(argv[1], 0, &test_speed))
			return RET_FAIL;
		mtk_test_dbg("Test speed %d\n", test_speed);
	}
	if (argc > 2) {
		/* _boundary = (int)simple_strtol(argv[2], &argv[2], 10); */
		if (kstrtoint(argv[2], 0, &ep_boundary))
			return RET_FAIL;
...
static int dbg_u3_calibration(int argc, char **argv)
{
		...
		mtk_test_dbg("num_u3_port = %d u2port = %d\n", num_u3_port, num_u2_port);
		...
		if (argc > 1) {
			/* phase_val = (int)simple_strtol(argv[1], &argv[1], 10);*/
			if (kstrtoint(argv[1], 0, &phase_val))
				return RET_FAIL;
			phase_start = phase_val;
			phase_end = phase_val+1;
		}
		...
		for (u1RxSel = 0; u1RxSel < 2; u1RxSel++) {
			u1PhyDrv = 2;
			for (u1TimeDelay = phase_start; u1TimeDelay < phase_end; u1TimeDelay++) {
				...
				writel(u4LatchValue, (void __iomem *)latch_sel_addr);
				/*set hcrst*/
				writel(2, (void __iomem *)u3_cmd_addr);
				/*polling CNR*/
				for (i = 0; i < 500; i++) {
					u4USBStsVal = readl((void __iomem *)u3_sts_addr);
				...
				temp = readl((void __iomem *)u3_p1_sc_addr);
				temp = mtktest_xhci_port_state_to_neutral(temp);
				temp |= PORT_POWER;
				writel(temp, (void __iomem *)u3_p1_sc_addr);
				/*Wait 100 ms*/
				msleep(100);
				/*polling PLS*/
				u4CorrectCnt = 0;
				 ...
				 * if polling status in U0 more than 3 times, check recovery & link error count,
				 * report PASS
	...
	/*      for (i = 0;  i < 200; i++) {*/
				for (i = 0; i < 50; i++) {
					u4P1ScVal = readl((void __iomem *)u3_p1_sc_addr);
					u1P1PlsVal = (u4P1ScVal>>U3_PLS_OFFSET) & 0xf;
					u1Macltssm = readl((void __iomem *)u3_p1_ltssm_addr);
					...
					if (u1P1PlsVal == 10) {
						mtk_test_dbg("rx_detect = %d\n", readl((void __iomem *)U3_LTSSM_INFO));
						msleep(500);
						mtk_test_dbg("rx_detect = %d\n", readl((void __iomem *)U3_LTSSM_INFO));
					...
					if (u4CorrectCnt == 3) {
						...
						u4ErrorCnt = readl((void __iomem *)u3_link_err_count_addr);
						u4RecoveryCnt = readl((void __iomem *)u3_recovery_count_addr);
						u4CorrectCnt = 4;
					}
					msleep(20);
				}
				if (u1IsPass == 0) {
					mtk_test_dbg("[PASS] PLS=0, Error=%d, Recovery=%d, TxSel[0x%02x] RxSel[0x%02x]\n",
						u4ErrorCnt, u4RecoveryCnt, u1TxSel, u1RxSel);
					mtk_test_dbg("PhyDrv[0x%02x] Reg[0x%02x] TimeDelay[0x%02x] Reg[0x%02x]\n",
						u1PhyDrv, u1PhyDrvValue, u1TimeDelay, u1TimingValue);
				} else
		...
		for (u1TimeDelay = 0; u1TimeDelay < 32; u1TimeDelay++) {
			...
			*/
			...
			if (u1IsPass == 0) {
				mtk_test_dbg("[PASS] PLS=0, Error=%d, Recovery=%d, TxSel[0x%02x] RxSel[0x%02x] "
					, u4ErrorCnt, u4RecoveryCnt, u1TxSel, u1RxSel);
				mtk_test_dbg("PhyDrv[0x%02x] Reg[0x%02x] TimingDelay[0x%02x] Reg[0x%02x]\n"
					, u1PhyDrv, u1PhyDrvValue, u1TimeDelay, u1TimingValue);
			} else
				mtk_test_dbg("[FAIL] PLS=%d\n", u1P1PlsVal);
...
static int dbg_u3w(int argc, char **argv)
{
	...
	if (kstrtoint(argv[1], 16, &u4TimingAddress))
		return RET_FAIL;
	if (kstrtou8(argv[2], 16, (u8 *)&u4TimingValue))
		return RET_FAIL;
...
{
#if 1
	if (kstrtou8(argv[1], 0, &_rEye1.bX_tl))
		return RET_FAIL;
	if (kstrtou8(argv[2], 0, &_rEye1.bY_tl))
		return RET_FAIL;
	if (kstrtou8(argv[3], 0, &_rEye1.bX_br))
		return RET_FAIL;
	if (kstrtou8(argv[4], 0, &_rEye1.bY_br))
		return RET_FAIL;
	if (kstrtou8(argv[5], 0, &_rEye1.bDeltaX))
		return RET_FAIL;
	if (kstrtou8(argv[6], 0, &_rEye1.bDeltaY))
		return RET_FAIL;
	if (kstrtou8(argv[7], 0, (u8 *)&_rTestCycle.wEyeCnt))
		return RET_FAIL;
	if (kstrtou8(argv[8], 0, &_rTestCycle.bNumOfEyeCnt))
		return RET_FAIL;
	if (kstrtou8(argv[9], 0, &_rTestCycle.bNumOfIgnoreCnt))
		return RET_FAIL;
	_rEye2.bX_tl = _rEye1.bX_tl;
	_rEye2.bY_tl = _rEye1.bY_tl;
	_rEye2.bX_br = _rEye1.bX_br;
	_rEye2.bY_br = _rEye1.bY_br;
	_rEye2.bDeltaX = _rEye1.bDeltaX;
	_rEye2.bDeltaY = _rEye1.bDeltaY;
#else
...
static int dbg_u2_testmode(int argc, char **argv)
{
	...
	if (kstrtoint(argv[1], 0, &test_value))
		return RET_FAIL;
...
static int dbg_memorywrite(int argc, char **argv)
{
	...
	if (kstrtoint(argv[1], 16, &addr))
		return RET_FAIL;
	/*value = (int)simple_strtol(argv[2], &argv[2], 16);*/
	if (kstrtoint(argv[2], 16, &value))
		return RET_FAIL;
...
static int dbg_printslotcontext(int argc, char **argv)
{
	...
	if (argc > 1) {
		...
		ret = kstrtoint(argv[1], 0, &slot_id);
		if (ret)
			return ret;
...
static int dbg_read_xhci(int argc, char **argv)
{
	...
	ret = kstrtouint(argv[1], 16, &u4SrcAddr);
	if (ret)
		return ret;
	u4SrcAddr = u4SrcAddr & 0xfffffffc;

	ret = kstrtoint(argv[2], 16, &u4Len);
	if (ret)
		return ret;
	...
	for (u4Idx = 0; u4Idx < u4Len; u4Idx += 16) {
		temp1 = xhci_readl(xhci, (u32 *)(unsigned long)(u4SrcAddr + u4Idx + 0));
		temp2 = xhci_readl(xhci, (u32 *)(unsigned long)(u4SrcAddr + u4Idx + 4));
		temp3 = xhci_readl(xhci, (u32 *)(unsigned long)(u4SrcAddr + u4Idx + 8));
		temp4 = xhci_readl(xhci, (u32 *)(unsigned long)(u4SrcAddr + u4Idx + 12));
...
int usbif_u3h_test_send_event(char *event)
{
	...
	pr_notice("usbif_u3h_test_send_event - sending event - %s in %s\n",
	...
	if (ret < 0)
		pr_notice("usbif_u3h_test_send_event sending failed with ret = %d\n", ret);
...
static int __init mtk_test_init(void)
{
	...
	if (!misc_register(&mu3h_test_uevent_device))
		pr_notice("create the mu3h_test_uevent_device uevent device OK!\n");
	else
		pr_notice("[ERROR] create the mu3h_test_uevent_device uevent device fail\n");
	return 0;
fail:
	return retval;
}
