...
struct usb_device *mtk_usb_alloc_dev(struct usb_device *parent, struct usb_bus *bus, unsigned port1)
{
	...
	if (unlikely(!parent)) {
		...
		if (parent->devpath[0] == '0') {
			...
			dev->route = 0;
			pr_debug("device attached on roothub\n");
		} else {
			...
			if (port1 < 15)
				...
				dev->route = parent->route +
					(15 << ((parent->level - 1)*4));
			pr_debug("device route string %d\n", dev->route);
			pr_debug("parent level %d\n", parent->level);
			pr_debug("parent route string %d\n", parent->route);
		}
		...
		unsigned int irqnum, unsigned long irqflags)
{
	...
	rhdev = mtk_usb_alloc_rhdev(NULL, &hcd->self, 0);
	if (rhdev == NULL) {
