				  ...
				  u64 event_time)
{
	...
	if (status == STATUS_MAX - 1) {
		...
		record[GOT_IPI].sum_kernel_time += record[GOT_IPI].check_time - event_time;
		...
		if (record[status].count == LIMIT) {
			for (i = 0; i < STATUS_MAX; i++) {
				pr_debug("Sensor[%d] ====> last event stage[%d] check time:%lld\n",
					sensor_type, i, record[i].check_time);
				pr_debug("sensor[%d] ====> stage[%d] average delta time:%lld on %d events\n",
