...
#define SE_PR_ERR(fmt, args...)		pr_err(SENSOR_EVENT_TAG fmt, ##args)
	...
	for (;;) {
		...
		if (client->head == client->tail) {
			...
			if (file->f_flags & O_NONBLOCK)
				return -EAGAIN;
			SE_LOG("sensor_event_read handle: %d wait_event_interruptible\n", handle);
			if (wait_event_interruptible(client->wait, client->head != client->tail))
				return -ERESTARTSYS;
			SE_LOG("sensor_event_read handle: %d wait_up\n", handle);
#endif
...
#if 0
		if (!(file->f_flags & O_NONBLOCK)) {
			SE_PR_ERR("sensor_event_read handle: %d open BLOCK\n", handle);
			error = wait_event_interruptible(client->wait,
					client->head != client->tail);
			SE_PR_ERR("sensor_event_read handle: error %d open BLOCK\n", error);
			if (error)
				return error;
		}
#endif
