...
char msensor_module_name[256]="NA:NA:NA:0";
static ssize_t  msensorinfo_show(struct device *dev,
                                  struct device_attribute *attr, char *buf)
{
     printk("Dump  %s \n",msensor_module_name);
...
static ssize_t msensorinfo_store(struct device* dev,
                                   struct device_attribute *attr, const char *buf, size_t count)
{
        return count;
}

DEVICE_ATTR(msensor,     S_IWUSR | S_IRUGO, msensorinfo_show, msensorinfo_store);
extern struct device* get_deviceinfo_dev(void);

static void Create_msensorinfo_node_ForMMI(void)
{
       struct device * msensor;
       msensor=get_deviceinfo_dev();
       if (device_create_file(msensor, &dev_attr_msensor) < 0)
...
extern int magnetometer_init_flag;
...
static int mag_probe(void)
{
	...
	//Begin added by lanying.he for XR5495833 on 2017/1031 robust
	magnetometer_init_flag = 1;
	//End added by lanying.he for XR5495833 on 2017/1031 robust
	//Begin added by lanying.he for XR5424729 on 2017/10/20 deviceinfo
    Create_msensorinfo_node_ForMMI();
    //End added by lanying.he for XR5424729 on 2017/10/20 deviceinfo
