...
#define DEBUG 0
#define AKM09911_DEV_NAME	"akm09918"
...
static const struct dev_pm_ops akm09911_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(akm09911_suspend, akm09911_resume)
};
#endif
...
static int AKECS_CheckDevice(void)
{
	...
	if ((akm_device == 0x05) || (akm_device == 0x04)) {/* ak09911 & ak09912 */
		ret = AKECS_ReadFuse();
		if (ret < 0) {
			MAGN_PR_ERR("AKM09911 akm09911_probe: read fuse fail\n");
			return -ENXIO;
		}
// Add-Begin by zhikui.li@tcl.com for porting akmd09918  driver 
	//} else if (akm_device == 0x10) {/* ak09915 & ak09916c & ak09916d & ak09918 */
	} else {/* ak09915 & ak09916c & ak09916d & ak09918 */
// Add-end by zhikui.li@tcl.com for porting akmd09918  driver 
		akm_fuse[0] = 0x80;
		akm_fuse[1] = 0x80;
		akm_fuse[2] = 0x80;
	}
...
#endif
	if (ret < 0) {
		MAGN_PR_ERR("AKM8975 akm8975_work_func: I2C failed\n");
		return -1;
	}
	rbuf[0] = temp;
#ifdef AKM_Device_AK8963
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj == NULL) {
		MAGN_PR_ERR("akm09911_i2c_data is null!!\n");
		return 0;
	}
...
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(akm09911_attr_list));
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,akmd09918");
    if(node != NULL)
    {
    ...
    {
        MAGN_PR_ERR("get mag dts node fail\n");
        err = -EFAULT;
		goto exit;
    }
	...
	data->hw->direction--;
	MAGN_PR_ERR("hw->direction = %d,data->hw->direction=%d\n",hw->direction,data->hw->direction);
	...
	if (err < 0) {
		MAGN_PR_ERR("AKM09911 akm09911_probe: check device connect error\n");
...
static int akm09911_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		MAGN_PR_ERR("akm09911_delete_attr fail: %d\n", err);
...
extern char msensor_module_name[256]; //added by lanying.he for deviceinfo
static int akm09911_local_init(void)
{
	akm09911_power(hw, 1);
	...
	if (-1 == akm09911_init_flag)
		return -1;
	sprintf(msensor_module_name,"AKM09918:AKM:NA");  //added by lanying.he for deviceinfo
	return 0;
}
