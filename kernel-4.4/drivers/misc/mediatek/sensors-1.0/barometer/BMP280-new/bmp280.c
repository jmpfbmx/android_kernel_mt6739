...
static int bmp_get_calibration_data(struct i2c_client *client)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {
		...
		status = bmp_i2c_read_block(client,
					    BMP280_CALIBRATION_DATA_START,
					    a_data_u8r, BMP280_CALIBRATION_DATA_LENGTH);
		if (status < 0)
			return status;

		obj->bmp280_cali.dig_T1 = (BMP280_U16_t) ((((BMP280_U16_t)
							    ((unsigned char)a_data_u8r[1])) <<
							   SHIFT_LEFT_8_POSITION) | a_data_u8r[0]);
		obj->bmp280_cali.dig_T2 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[3])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[2]);
		obj->bmp280_cali.dig_T3 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[5])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[4]);
		obj->bmp280_cali.dig_P1 =
		    (BMP280_U16_t) ((((BMP280_U16_t) ((unsigned char)a_data_u8r[7])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[6]);
		obj->bmp280_cali.dig_P2 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[9])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[8]);
		obj->bmp280_cali.dig_P3 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[11])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[10]);
		obj->bmp280_cali.dig_P4 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[13])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[12]);
		obj->bmp280_cali.dig_P5 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[15])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[14]);
		obj->bmp280_cali.dig_P6 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[17])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[16]);
		obj->bmp280_cali.dig_P7 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[19])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[18]);
		obj->bmp280_cali.dig_P8 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[21])) <<
				     SHIFT_LEFT_8_POSITION) | a_data_u8r[20]);
		obj->bmp280_cali.dig_P9 =
		    (BMP280_S16_t) ((((BMP280_S16_t) ((signed char)a_data_u8r[23])) <<
...
static int bmp_read_raw_temperature(struct i2c_client *client, s32 *temperature)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		...
		*temperature = (BMP280_S32_t) ((((BMP280_U32_t) (a_data_u8r[0])) <<
						...
						SHIFT_LEFT_4_POSITION) | ((BMP280_U32_t)
									  a_data_u8r[2] >>
...
static int bmp_get_pressure(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (obj->sensor_type == BMP280_TYPE) {	/* BMP280 */
		...
		v_x2_u32r = v_x2_u32r + ((v_x1_u32r *
					  (BMP280_S64_t) obj->bmp280_cali.dig_P5) << 17);
		...
		v_x1_u32r = (((((BMP280_S64_t) 1) << 47) + v_x1_u32r)) *
		    ((BMP280_S64_t) obj->bmp280_cali.dig_P1) >> 33;
		if (v_x1_u32r == 0)
...
static long compat_bmp_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
#if IS_ENABLED(CONFIG_COMPAT)
	.compat_ioctl = compat_bmp_unlocked_ioctl,
#endif
};
...
static int bmp_resume(struct device *dev)
{
	...
	err = bmp_set_powermode(obj->client, BMP_NORMAL_MODE);
	if (err) {
		BAR_ERR("bmp set normal mode failed, err = %d\n", err);
		return err;
	}
#ifdef CONFIG_BMP_LOWPASS
...
static int bmp_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
