...
char gsensor_module_name[256]="NA:NA:NA:0";
static ssize_t  gsensorinfo_show(struct device *dev,
                                  struct device_attribute *attr, char *buf)
{
     printk("Dump  %s \n",gsensor_module_name);
...
static ssize_t gsensorinfo_store(struct device* dev,
                                   struct device_attribute *attr, const char *buf, size_t count)
{
        return count;
}

DEVICE_ATTR(gsensor,     S_IWUSR | S_IRUGO, gsensorinfo_show, gsensorinfo_store);
extern struct device* get_deviceinfo_dev(void);

static void Create_gsensorinfo_node_ForMMI(void)
{
       struct device * gsensor;
       gsensor=get_deviceinfo_dev();
       if (device_create_file(gsensor, &dev_attr_gsensor) < 0)
               printk("Failed to create device file(%s)!\n", dev_attr_gsensor.attr.name);
        return;
}
//End added by lanying.he for XR5424729 on 2017/10/20 deviceinfor
...
static int acc_probe(void)
{
	...
	//End added by lanying.he for XR5495833 on 2017/1031 robust
	//Begin added by lanying.he for XR5424729 on 2017/10/20  deviceinfo
    Create_gsensorinfo_node_ForMMI(); 
