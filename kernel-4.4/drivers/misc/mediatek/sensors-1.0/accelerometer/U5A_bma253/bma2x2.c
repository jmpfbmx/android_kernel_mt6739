 ...
 * history: Added by lanying.he for Task 4581347 on 2017/4/17
 * history02: Modified by lanying.he for Task 4737716 on 2017/4/17
 * history03: Modified by lanying.he for Task 4869315 on 2017/4/17
  *history04  Modified by lanying.he for Task 4933042 on 2017/6/15
 *history05  Modified by lanying.he for XR5592217 on 2017/11/27 for gsensor calibration
 */
...
//#include <linux/earlysuspend.h>//Modified by yuduan.xie for Task on 2016-08-03 begin/end
#include <linux/platform_device.h>
#include <asm/atomic.h>

//Modifyed by yuduan.xie for Task on 2016-08-03 begin
//#include <mach/mt_typedefs.h>
//#include <mach/mt_gpio.h>
//#include <mach/mt_pm_ldo.h>
//Modifyed by yuduan.xie for Task on 2016-08-03 end
...
#define BMA2XX_DEV_NAME        "BMA253"

/***************************SIGNIFICANT***************************/
#define SIGNIFICANT
#ifdef SIGNIFICANT
int bma2xx_set_acc_interrupter(bool flag);
int bma2xx_acc_init_flag = -1;
...
//#define COMPATIABLE_NAME "mediatek,bma2xx_new"//Modify by yuduan.xie for Task on 2016-08-03 begin/end
...
static int bma2xx_resume(struct device *client);
#endif
...
/*----------------------------------------------------------------------------*/
// Begin added by yan.gong
#ifdef CONFIG_PM_SLEEP
static const struct dev_pm_ops bma2xx_acc_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(bma2xx_suspend, bma2xx_resume)
};
#endif
// End added by yan.gong
...
#endif
//Modified by yuduan.xie for Task on 2016-08-03 begin/end
...
#endif 
/*----------------------------------------------------------------------------*/
//Modified by yuduan.xie for Task on 2016-08-03 begin
/*
...
*/
//Modified by yuduan.xie for Task on 2016-08-03 end
...
static int BMA2XX_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	GSE_ERR("BMA2XX_SetPowerMode+enable = %d,sensor_power = %d\n",enable,sensor_power);
	...
	//GSE_ERR("set power mode value = 0x%x!\n",databuf[0]);
...
static int BMA2XX_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	{
		GSE_ERR("bma2xx read Dataformat failt \n");
		return BMA2XX_ERR_I2C;
	}
	mdelay(1);
	...
	//GSE_ERR("BMA2XX_SetDataFormat OK! \n");
	mdelay(1);
...
static int BMA2XX_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	{
		GSE_ERR("bma2xx read rate failt \n");
		return BMA2XX_ERR_I2C;
	}
	mdelay(1);
...
//Added by yuduan.xie for significant on 2017.2.27 begin
static int bma2xx_SetMotionDetectEnable(struct i2c_client *client, u8 enable)
{
        int res = 0;
	u8 databuf[2];
	...
	{
		GSE_ERR("read motion detect  ctl register err!\n");
		return N2DM_ERR_I2C;
	}
	if(enable)
	{
		databuf[0] |= KXTJ2_1009_MOTION_DETECT;
	}
	else
	{
		databuf[0] &= ~KXTJ2_1009_MOTION_DETECT;
	...
	*/
	...
	if(enable)
	{
		GSE_ERR("bma2xx_SetMotionDetectEnable--1\n");
		databuf[1] = 0x04;
	}
	else
	{
		GSE_ERR("bma2xx_SetMotionDetectEnable--2\n");
		databuf[1] = 0x00;
	}
	...
	GSE_LOG("bma2xx_SetMotionDetectEnable  %d!\n",enable);
	mdelay(5);
	return BMA2XX_SUCCESS;    
}
//Added by yuduan.xie for significant on 2017.2.27 end
...
static int BMA2XX_SetIntEnable(struct i2c_client *client, u8 intenable)
{
	//u8 databuf[10];    
	int res = 0;
	GSE_ERR("BMA2XX_SetIntEnable intenable = %d\n",intenable);
	//Modified by yuduan.xie for significant on 2017.2.27 begin
	//res = hwmsen_write_byte(client, BMA2XX_INT_REG_1, 0x00);
	res = hwmsen_write_byte(client, BMA2XX_INT_REG_1, intenable);
	...
	//res = hwmsen_write_byte(client, BMA2XX_INT_REG_2, intenable);
	if(res != BMA2XX_SUCCESS) 
	{
		return res;
	}
	//GSE_ERR("BMA2XX_SetIntEnable\n");
...
#if 1
//Added by yuduan.xie for significant on 2017.2.27 begin
static int BMA2XX_setMotionDetectFunc(struct i2c_client *client)
{
         u8 databuf[10];
	int res;
	 BMA2XX_SetPowerMode(client, false);
...
static int bma2xx_init_client(struct i2c_client *client, int reset_cali)
{
	...
	//GSE_ERR("BMA2XX_CheckDeviceID ok \n");
...
#endif//#ifdef CUSTOM_KERNEL_SENSORHUB
#if 1
         //deleted by yan.gong for significant on 2017.11.9 begin
	//Added by yuduan.xie for significant on 2017.2.27 begin
       res = BMA2XX_setMotionDetectFunc(client);
	if(res != BMA2XX_SUCCESS)
	{
		return res;
	}
	//Added by yuduan.xie for significant on 2017.2.27 end
	//deleted by yan.gong for significant on 2017.11.9 end
#endif
	...
	//GSE_ERR("BMA2XX disable interrupt function!\n");
	...
	//GSE_ERR("BMA2XX_SetPowerMode OK!\n");
...
static int BMA2XX_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	//Modifyed by lanying.he for Task 4869315 on 2017/5/26 begin
	if(atomic_read(&obj->suspend))
	{
		//GSE_ERR("sensor in suspend read not data!\n");
		return 0;
	}
    ...
    //Modifyed by lanying.he for Task 4869315 on 2017/5/26 end 
 ...
 ******************************************************************************/
//Begin modified by lanying.he for gsensor calibration
...
//End modified by lanying.he for gsensor calibration
#ifdef CONFIG_COMPAT
...
{	
	...
	if(0 != (err = BMA2XX_SetPowerMode(obj->client, pre_suspend)))
#else
...
*/
	ACC_LOG("gsensor_batch (%d), chip only use 1024HZ\n", value);
	return 0;
}

static int bma2xx_flush(void)
{
	return acc_flush_report();
}
//Begin deleted by lanying.he for gsensor calibration
#if 0
static int bma2xx_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;
	...
	err = gsensor_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static int bma2xx_factory_get_data(int32_t data[3], int *status)
{
	return gsensor_get_data(&data[0], &data[1], &data[2], status);

}
static int bma2xx_factory_get_raw_data(int32_t data[3])
{
	char strbuf[BMA2XX_BUFSIZE] = { 0 };

	BMA2XX_ReadRawData(bma2xx_i2c_client, strbuf);
	ACC_LOG("support bma2xx_factory_get_raw_data!\n");
	return 0;
}
static int bma2xx_factory_enable_calibration(void)
{
	return 0;
}
static int bma2xx_factory_clear_cali(void)
{
	int err = 0;

	err = BMA2XX_ResetCalibration(bma2xx_i2c_client);
	if (err) {
		ACC_PR_ERR("bma2xx_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int bma2xx_factory_set_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };

	/* obj */
	obj_i2c_data->cali_sw[BMA2XX_AXIS_X] += data[0];
	obj_i2c_data->cali_sw[BMA2XX_AXIS_Y] += data[1];
	obj_i2c_data->cali_sw[BMA2XX_AXIS_Z] += data[2];

	cali[BMA2XX_AXIS_X] = data[0] * gsensor_gain.x / GRAVITY_EARTH_1000;
	cali[BMA2XX_AXIS_Y] = data[1] * gsensor_gain.y / GRAVITY_EARTH_1000;
	cali[BMA2XX_AXIS_Z] = data[2] * gsensor_gain.z / GRAVITY_EARTH_1000;
	err = BMA2XX_WriteCalibration(bma2xx_i2c_client, cali);
	if (err) {
		ACC_PR_ERR("bma2xx_WriteCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int bma2xx_factory_get_cali(int32_t data[3])
{
	data[0] = obj_i2c_data->cali_sw[BMA2XX_AXIS_X];
	data[1] = obj_i2c_data->cali_sw[BMA2XX_AXIS_Y];
	data[2] = obj_i2c_data->cali_sw[BMA2XX_AXIS_Z];
	return 0;
}
static int bma2xx_factory_do_self_test(void)
{
	return 0;
}

static struct accel_factory_fops bma2xx_factory_fops = {
	.enable_sensor = bma2xx_factory_enable_sensor,
	.get_data = bma2xx_factory_get_data,
	.get_raw_data = bma2xx_factory_get_raw_data,
	.enable_calibration = bma2xx_factory_enable_calibration,
	.clear_cali = bma2xx_factory_clear_cali,
	.set_cali = bma2xx_factory_set_cali,
	.get_cali = bma2xx_factory_get_cali,
	.do_self_test = bma2xx_factory_do_self_test,
};

static struct accel_factory_public bma2xx_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &bma2xx_factory_fops,
};
#endif 
//End deleted by lanying.he for gsensor calibration
/*----------------------------------------------------------------------------*/
static int bma2xx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    
	struct i2c_client *new_client;
	struct bma2xx_i2c_data *obj = NULL;
...
//Begin deleted by lanying.he for gsensor calibration
#if 0
	err = accel_factory_device_register(&bma2xx_factory_device);
	if (err) {
...
exit_kfree:
	kfree(obj);
exit:
//Begin added by lanying.he 
    err = -1;
//End added by lanying.he 
	GSE_ERR("%s: err = %d\n", __func__, err);
	gsensor_init_flag = -1;
	bma2xx_acc_init_flag = -1;
	return err;
...
static int bma2xx_i2c_remove(struct i2c_client *client)
{
	...
	i2c_unregister_device(client);
        //accel_factory_device_deregister(&bma2xx_factory_device);
...
{
    struct bma2xx_i2c_data *priv = obj_i2c_data;
...
static void eintWork(struct work_struct *work)
{
   u8 databuf[2] = {0}; 
	struct bma2xx_i2c_data *obj = obj_i2c_data;
	GSE_ERR("BMA2XX_EintWork--1\n");
	if(obj == NULL)
	{
		GSE_ERR("obj_i2c_data is null pointer!!\n");
		goto bma2xx_eint_work_exit;
	}	
	
	if(hwmsen_read_byte(obj->client, 0x1E, databuf))   
	{
		GSE_ERR("%s read bma2xx_FUNC_CK_GATE register err!\n", __func__);
		goto bma2xx_eint_work_exit;
	}
       GSE_ERR("BMA2XX_EintWork--2+databuf[0] = %d\n",databuf[0]);
	//if(0x2 & databuf[0])
	//{
...
int bma2xx_setup_eint(void)
{
    ...
    if (obj_i2c_data->irq_node)
    {
        ...
        ret = request_irq(obj_i2c_data->irq, eint_func,  IRQF_TRIGGER_FALLING, "mediatek, GSE_1-eint", NULL);
        if (ret != 0)
        {
...
int bma2xx_set_acc_interrupter(bool flag)
{
	...
	if(flag)
	{
		GSE_ERR("bma2xx_enable_significant flag = %d--1\n",flag);
		
	       significant_enable_status = true;
		   
	        BMA2XX_SetPowerMode(obj->client, false); 
		
		res = bma2xx_SetMotionDetectEnable(obj->client,true);
		if(res !=0)
		{
		    GSE_ERR("bma2xx_SetMotionDetectEnable faild\n");
		}
		
		res = BMA2XX_SetIntEnable(obj->client,0x07);
                if(res !=0)
		{
	...
	{
		GSE_ERR("bma2xx_enable_significant flag = %d--2\n",flag);
		
		significant_enable_status = false;
		
		BMA2XX_SetPowerMode(obj->client, false); 
		
		res = bma2xx_SetMotionDetectEnable(obj->client,false);
		if(res !=0)
		{
		    GSE_ERR("bma2xx_SetMotionDetectEnable faild\n");
		}

		res = BMA2XX_SetIntEnable(obj->client,0x00);
                if(res !=0)
		{
		    GSE_ERR("bma2xx_SetIntEnable faild\n");
		}
...
#endif
//End added by yan.gong for significant
/*----------------------------------------------------------------------------*/
...
static int  gsensor_local_init(void)
{
	...
	BMA2XX_power(hw, 1);
	GSE_ERR("BMA2XX_power  wingtech add driver before\n");
	if(i2c_add_driver(&bma2xx_i2c_driver))
	{
		GSE_ERR("add driver error\n");
		return -1;
	}
	GSE_ERR("i2c_add_driver(&bma2xx_i2c_driver) zhi.chen wingtech add driver after\n");
	if(-1 == gsensor_init_flag)
	{
		return -1;
	}
	sprintf(gsensor_module_name,"BMA253:BOSCh:NA");//add by lanying.he for gsensor deviceinfo
	GSE_ERR("fwq loccal init---\n");
	return 0;
...
static int __init bma2xx_init(void)
{
  int err = -1;
   struct device_node *node = NULL;
	GSE_FUN();
	node = of_find_compatible_node(NULL, NULL, "mediatek,bma2xx_new");
	 if(node!=NULL)
    {
		...
		ACC_PR_ERR("bma2xx_init wingtech get cust_baro dts success\n");
    }
	ACC_PR_ERR("bma2xx_init wingtech get cust_baro dts success\n");
	acc_driver_add(&bma2xx_init_info);
    ACC_PR_ERR("acc_driver_add(&bma2xx_init_info) wingtech get cust_baro dts \n");
