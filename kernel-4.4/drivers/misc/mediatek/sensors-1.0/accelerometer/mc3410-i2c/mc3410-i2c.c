...
static int mcube_write_log_data(struct i2c_client *client, u8 data[0x3f])
{
	...
	if (_pszBuffer == NULL) {
		GSE_PR_ERR("fail to allocate memory for buffer\n");
		return -1;
	}
	...
	if (!s_nIsRBM_Enabled) {
		if (s_bResolution == MC3XXX_RESOLUTION_LOW) {
			...
			MC3XXX_LOW_REOLUTION_DATA_SIZE)) {
				GSE_PR_ERR("ERR: fail to read data via I2C!\n");
...
static int MC3XXX_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (MC3XXX_i2c_read_block(client, addr, databuf, 1)) {
		GSE_PR_ERR("read power ctl register err!\n");
		return MC3XXX_RETCODE_ERROR_I2C;
	}
...
static void MC3XXX_SetResolution(void)
{
	...
	default:
		GSE_PR_ERR("ERR: no resolution assigned!\n");
	}
...
static int MC3XXX_SetSampleRate(struct i2c_client *pt_i2c_client)
{
	...
	if (IS_MCFM12() || IS_MCFM3X()) {
		...
		case 0xC0:
			...
			break;
		default:
			GSE_PR_ERR("[%s] no chance to get here... check code!\n", __func__);
			break;
		}
	} else
		_baDataBuf[0] = 0x00;
...
static void MC3XXX_ConfigRegRange(struct i2c_client *pt_i2c_client)
{
	...
	if (res < 0)
		GSE_PR_ERR("MC3XXX_ConfigRegRange fail\n");
...
static int MC3XXX_ReadSensorData(struct i2c_client *pt_i2c_client, char *pbBuf, int nBufSize)
{
	...
	if (false == mc3xxx_sensor_power) {
		if (MC3XXX_SetPowerMode(pt_i2c_client, true) != MC3XXX_RETCODE_SUCCESS) {
			GSE_PR_ERR("ERR: fail to set power mode!\n");
			return MC3XXX_RETCODE_ERROR_I2C;
		}
	}
	...
	if (MC3XXX_ReadData(pt_i2c_client, _pt_i2c_obj->data) != MC3XXX_RETCODE_SUCCESS) {
		GSE_PR_ERR("ERR: fail to read data!\n");
...
static int MC3XXX_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if (mc3xxx_sensor_power == false) {
		...
		if (res) {
			GSE_PR_ERR("Power on mc3xxx error %d!\n", res);
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "rst", 3)) {
		...
		mc3xxx_mutex_unlock();
		if (err)
			GSE_PR_ERR("write calibration err = %d\n", err);
	} else
		GSE_PR_ERR("invalid format\n");
...
static ssize_t store_firlen_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret != 0)
		GSE_PR_ERR("invallid format\n");
	else if (firlen > C_MAX_FIR_LENGTH)
		GSE_PR_ERR("exceeds maximum filter length\n");
	else {
...
static ssize_t store_chip_orientation(struct device_driver *ptDevDrv, const char *pbBuf, size_t tCount)
{
	...
	if (ret == 0) {
		if (hwmsen_get_convert(_nDirection, &_pt_i2c_obj->cvt))
			GSE_PR_ERR("ERR: fail to set direction\n");
	}
...
static ssize_t store_accuracy_status(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret != 0) {
		GSE_PR_ERR("incorrect argument\n");
		return count;
	}

	if (_nAccuracyStatus > SENSOR_STATUS_ACCURACY_HIGH) {
		GSE_PR_ERR("illegal accuracy status\n");
		return count;
	}
...
static int mc3xxx_suspend(struct device *dev)
{
	...
	/* GSE_LOG("mc3xxx_suspend\n"); */
		...
		mc3xxx_mutex_unlock();
		if (err) {
			GSE_PR_ERR("write power control fail!!\n");
			return err;
		}
	return err;
}
...
static int mc3410_factory_clear_cali(void)
{
	...
	if (err) {
		GSE_PR_ERR("mc3410_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
static int mc3410_factory_set_cali(int32_t data[3])
{
	...
	if (err) {
		GSE_PR_ERR("mc3410_WriteCalibration failed!\n");
		return -1;
	}
	return 0;
}
...
static int mc3xxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		GSE_PR_ERR("get cust_baro dts info fail\n");
		goto exit_kfree;
	}
	...
	if (err) {
		GSE_PR_ERR("acc_register_control_path(%d)\n", err);
		goto exit_kfree;
	}
	...
	if (err) {
		GSE_PR_ERR("acc_register_data_path(%d)\n", err);
		goto exit_kfree;
	}
	GSE_LOG("%s: OK\n", __func__);
...
static int mc3xxx_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		GSE_PR_ERR("mc3xxx_delete_attr fail: %d\n", err);
