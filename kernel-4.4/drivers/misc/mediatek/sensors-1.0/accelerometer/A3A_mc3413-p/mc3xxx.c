...
#define SIGNIFICANT
#ifdef SIGNIFICANT
int mc3413_set_acc_interrupter(bool flag);
int mc3413_acc_init_flag = -1;
static void eintWork(struct work_struct *work);
static bool enable_status = false;
extern bool significant_enable_status;
#endif

static bool pre_suspend;   //added by lanying.he for suspend
/***************************SIGNIFICANT***************************/
//End added by lanying.he for significant
...
static int mcube_write_log_data(struct i2c_client *client, u8 data[0x3f])
{
	...
	if (_pszBuffer == NULL) {
		ACC_PR_ERR("fail to allocate memory for buffer\n");
		return -1;
	}
...
static int MC3XXX_SetPowerMode(struct i2c_client *client, bool enable)
{
    ...
    ACC_LOG("set power=%x,enable= %x,mc3xxx_sensor_power=%x\n", databuf[0],enable,mc3xxx_sensor_power);
	return MC3XXX_RETCODE_SUCCESS;
}
...
#ifdef SIGNIFICANT
static int ConfigureThd(struct i2c_client *client)
{
    ...
    databuf[0] = MC3XXX_REG_TAP_DWELL_REJECT;
    databuf[1] = 0x35;
    ...
    //0xb = 0x20
    memset(databuf, 0, sizeof(u8)*2);
    databuf[0] = MC3XXX_REG_DROP_CONTROL;
    databuf[1] = 0x37;
    res = i2c_master_send(client, databuf, 0x2);
    if(res < 0)
    {
    ...
    memset(databuf, 0, sizeof(u8)*2);
    databuf[0] = MC3XXX_REG_SHAKE_DEBOUNCE;
    databuf[1] = 0x20;
    res = i2c_master_send(client, databuf, 0x2);
    if(res < 0)
    {
...
static int setMotionEnable(struct i2c_client *client, u8 enable)
{

    int res = 0;
    u8 databuf[2];
    u8 addr = MC3XXX_REG_TAP_DETECTION_ENABLE;
    ...
    ACC_PR_ERR("Motiondetect  %d!\n",enable);

    msleep(200);
    return 0;
}

static int setIntEnable(struct i2c_client *client, u8 enable)
{
    u8 databuf[2];
    u8 addr = MC3XXX_REG_INTERRUPT_ENABLE;
    int res = 0;
...
static int MC3XXX_ReadSensorData(struct i2c_client *pt_i2c_client, char *pbBuf, int nBufSize)
{
	...
	if ((pt_i2c_client == NULL) || (pbBuf == NULL)) {
	 ...
	 //Begin modify by lanying.he for resume later i2c_timeout
    #if 0
    ...
    #endif
    if(1==atomic_read(&_pt_i2c_obj->suspend))
    {
        ACC_LOG("[%s] power is suspend \n",__func__);
        return 0;
    }
    //End modify by lanying.he for for resume later i2c_timeout
...
static int MC3XXX_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if (mc3xxx_sensor_power == false) {
		...
		if (res)
			ACC_PR_ERR("Power on mc3xxx error %d!\n", res);
	}
	...
	if (res) {
		ACC_PR_ERR("I2C error: ret value=%d", res);
		return -EIO;
	}
...
static ssize_t store_firlen_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret != 0)
		ACC_PR_ERR("invallid format\n");
	else if (firlen > C_MAX_FIR_LENGTH)
		ACC_PR_ERR("exceeds maximum filter length\n");
	else {
...
static ssize_t store_accuracy_status(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret != 0) {
		ACC_PR_ERR("incorrect argument\n");
		return count;
	}

	if (_nAccuracyStatus > SENSOR_STATUS_ACCURACY_HIGH) {
		ACC_PR_ERR("illegal accuracy status\n");
		return count;
	}
...
static ssize_t show_reg_val(struct device_driver *ddri, char *buf)
{
    ...
    for(i=3;i<=0x12;i++)
    {
        ret = MC3XXX_i2c_read_block(obj->client, i, &rbuf[i], 0x01);
        if(ret < 0)
        {
            ACC_PR_ERR("error: %d\n", ret);
            return -EFAULT;
        ...
        if(i==6 || i ==9)
        {
            ACC_PR_ERR("lanying.he rbuf[%d] = %d\n",i, rbuf[i]);
        }
    }
    #endif

    for(cnt=3;cnt<=0x12;cnt++)
    {
        ACC_PR_ERR("reg[0x%x]=0x%x\n", cnt, rbuf[cnt]);
...
static ssize_t store_reg_val(struct device_driver *ddri, const char *buf, size_t count)
{
    ...
    if (res == 2)
    {
          MC3XXX_SetPowerMode(obj->client,false);

          //if(MC3XXX_i2c_write_block(obj->client,reg,(unsigned char)&values,0x01)<0)
          if(hwmsen_write_byte(obj->client,reg,values))
          {
                ACC_PR_ERR("reg =%d write values = %dfailed\n",reg,values);
         //N2DM_SetPowerMode(obj->client,true);
        return -EFAULT;
          }
          ACC_PR_ERR("reg =%d write values = %d success\n",reg,values);
          MC3XXX_SetPowerMode(obj->client,true);
      }
       else
       {
        ACC_PR_ERR("invalid content: %s , res = %d\n", buf,res);
    }
     return count;
}
//End added by lanying.he for significant
...
static DRIVER_ATTR(regval,               S_IWUSR | S_IRUGO, show_reg_val,        store_reg_val);  //added by lanying.he for significant
...
static long mc3xxx_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_INIT:
		ACC_LOG("fwq GSENSOR_IOCTL_INIT\n");
		mc3xxx_mutex_lock();
	...
	case GSENSOR_IOCTL_READ_CHIPINFO:
		ACC_LOG("fwq GSENSOR_IOCTL_READ_CHIPINFO\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_IOCTL_READ_GAIN:
		ACC_LOG("fwq GSENSOR_IOCTL_READ_GAIN\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_IOCTL_READ_OFFSET:
		ACC_LOG("fwq GSENSOR_IOCTL_READ_OFFSET\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_IOCTL_READ_RAW_DATA:
		ACC_LOG("fwq GSENSOR_IOCTL_READ_RAW_DATA\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_IOCTL_SET_CALI:
		ACC_LOG("fwq GSENSOR_IOCTL_SET_CALI!!\n");
		data = (void __user *)arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_IOCTL_CLR_CALI:
		ACC_LOG("fwq GSENSOR_IOCTL_CLR_CALI!!\n");
		mc3xxx_mutex_lock();
		err = MC3XXX_ResetCalibration(client);
		mc3xxx_mutex_unlock();
		break;
	...
	case GSENSOR_IOCTL_GET_CALI:
		ACC_LOG("fwq mc3xxx GSENSOR_IOCTL_GET_CALI\n");
		data = (void __user *)arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_MCUBE_IOCTL_SET_CALI:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_SET_CALI!!\n");
		data = (void __user *)arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_IOCTL_SET_CALI_MODE:
		ACC_LOG("fwq mc3xxx GSENSOR_IOCTL_SET_CALI_MODE\n");
		break;

	case GSENSOR_MCUBE_IOCTL_READ_RBM_DATA:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_READ_RBM_DATA\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_MCUBE_IOCTL_SET_RBM_MODE:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_SET_RBM_MODE\n");
		mc3xxx_mutex_lock();
		MC3XXX_rbm(client, 1);
		mc3xxx_mutex_unlock();
		break;
	...
	case GSENSOR_MCUBE_IOCTL_CLEAR_RBM_MODE:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_CLEAR_RBM_MODE\n");
		mc3xxx_mutex_lock();
		MC3XXX_rbm(client, 0);
		mc3xxx_mutex_unlock();
		break;
	...
	case GSENSOR_MCUBE_IOCTL_REGISTER_MAP:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_REGISTER_MAP\n");
	...
	case GSENSOR_MCUBE_IOCTL_READ_PRODUCT_ID:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_READ_PRODUCT_ID\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
		...
		if (MC3XXX_RETCODE_SUCCESS != prod)
			ACC_LOG("Not mCube accelerometers!\n");
	...
	case GSENSOR_MCUBE_IOCTL_READ_FILEPATH:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_READ_FILEPATH\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_MCUBE_IOCTL_VIRTUAL_Z:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_VIRTUAL_Z\n");
		data = (void __user *) arg;
		tempZ = VIRTUAL_Z;
		if (data == NULL) {
			err = -EINVAL;
	...
	case GSENSOR_MCUBE_IOCTL_READ_PCODE:
		ACC_LOG("fwq GSENSOR_MCUBE_IOCTL_READ_PCODE\n");
		data = (void __user *) arg;
		if (data == NULL) {
			err = -EINVAL;
			break;
	...
	case GSENSOR_MCUBE_IOCTL_GET_OFLAG:
		...
		ACC_LOG("[%s] GSENSOR_MCUBE_IOCTL_GET_OFLAG\n", __func__);
	   ...
	   unsigned long arg)
{
	...
	case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
		...
		if (err) {
			ACC_PR_ERR("GSENSOR_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
			return err;
		}
		break;
	...
	case COMPAT_GSENSOR_IOCTL_SET_CALI:
		...
		if (err) {
			ACC_PR_ERR("GSENSOR_IOCTL_SET_CALI unlocked_ioctl failed.");
			return err;
		}
		break;
	...
	case COMPAT_GSENSOR_IOCTL_GET_CALI:
		...
		if (err) {
			ACC_PR_ERR("GSENSOR_IOCTL_GET_CALI unlocked_ioctl failed.");
			return err;
		}
	break;
	...
	case COMPAT_GSENSOR_IOCTL_CLR_CALI:
		...
		if (err) {
			ACC_PR_ERR("GSENSOR_IOCTL_CLR_CALI unlocked_ioctl failed.");
			return err;
		}
		break;
...
static int mc3xxx_suspend(struct device *dev)
{
	...
	mc3xxx_mutex_lock();
	pre_suspend = mc3xxx_sensor_power;    //added by lanying.he for suspend 
...
static int mc3xxx_resume(struct device *dev)
{
	...
	mc3xxx_mutex_lock();
	//err = MC3XXX_Init(client, 0);   //delete by lanying.he
	err = MC3XXX_SetPowerMode(client,pre_suspend); //modified by lanying.he for suspend 
	if (err) {
		mc3xxx_mutex_unlock();
...
#ifdef SIGNIFICANT   
    enable_status = power;
    if(!significant_enable_status)
    {
        for (retry = 0; retry < 3; retry++)
        {
            res = MC3XXX_SetPowerMode(mc3xxx_obj_i2c_data->client, enable_status);
            if (res == 0) {
...
static int mc3xxx_flush(void)
{
	return acc_flush_report();
}

//Begin delete by lanying.he for calibration
#if 0
static int mc3xxx_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	...
	err = mc3xxx_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static int mc3xxx_factory_get_data(int32_t data[3], int *status)
{
	return mc3xxx_get_data(&data[0], &data[1], &data[2], status);

}
static int mc3xxx_factory_get_raw_data(int32_t data[3])
{
	char strbuf[MC3XXX_BUF_SIZE] = { 0 };
...
static int mc3xxx_factory_clear_cali(void)
{
	...
	if (err) {
		ACC_PR_ERR("mc3xxx_ResetCalibration failed!\n");
		return -1;
	}
	return 0;
}
...
static struct accel_factory_fops mc3xxx_factory_fops = {
	.enable_sensor = mc3xxx_factory_enable_sensor,
	.get_data = mc3xxx_factory_get_data,
	.get_raw_data = mc3xxx_factory_get_raw_data,
	.enable_calibration = mc3xxx_factory_enable_calibration,
	.clear_cali = mc3xxx_factory_clear_cali,
	.set_cali = mc3xxx_factory_set_cali,
	.get_cali = mc3xxx_factory_get_cali,
	.do_self_test = mc3xxx_factory_do_self_test,
};

static struct accel_factory_public mc3xxx_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &mc3xxx_factory_fops,
};
#endif
...
static int mc3xxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mc3413-p");
    if(node!=NULL)
    {
    ...
    {
        ACC_PR_ERR("get accel dts node fail\n");
		goto exit;
    }
	...
	obj->hw = hw;
	printk("hw->direction = %d\n",hw->direction); err = hwmsen_get_convert(obj->hw->direction, &obj->cvt);
	if (err) {
   ...
   //Begin added by lanying.he for gsensor calibration
	...
	if (err) {
		ACC_PR_ERR("mc3xxx_device register failed\n");
	...
	//Begin delete by lanying.he for calibration
	#if 0    
	err = accel_factory_device_register(&mc3xxx_factory_device);
	if (err) {
	...
	if (err) {
		ACC_PR_ERR("register acc control path err\n");
		goto exit_kfree;
	}
...
//End added by lanying.he for gsensor calibration
	...
	//Begin delete by lanying.he for calibration
	#if 0
	accel_factory_device_deregister(&mc3xxx_factory_device);
	#endif
	//End delete by lanying.he for calibration
...
//  ACC_LOG("GSE into  interrupt liupeng !\n");
    disable_irq_nosync(mc3xxx_obj_i2c_data->irq); //liupeng
...
static void eintWork(struct work_struct *work)
{
    ...
    if(obj == NULL)
    {
        ACC_PR_ERR("mc3xxx_obj_i2c_data is null pointer!!\n");
        goto eint_work_exit;
    }
    ...
    msleep(10);
    memset(databuf,0,sizeof(u8)*2);
    if(MC3XXX_i2c_read_block(obj->client, MC3XXX_REG_TILT_STATUS, databuf, 0x01)<0)
    {
        ACC_PR_ERR("%s read kxtj2_1009_FUNC_CK_GATE register err!\n", __func__);
        goto eint_work_exit;
    }
    ...
    enable_irq(mc3xxx_obj_i2c_data->irq);  //liupeng
}

int mc3413_setup_eint(void)
{
    int ret;
    u32 ints[2] = { 0, 0 };
    mc3xxx_obj_i2c_data->irq_node = of_find_compatible_node(NULL,NULL,"mediatek, GSE_1-eint");
    if (mc3xxx_obj_i2c_data->irq_node)
    {
        of_property_read_u32_array(mc3xxx_obj_i2c_data->irq_node, "debounce", ints, ARRAY_SIZE(ints));
        //of_property_read_u32_array(node, "interrupts", ints1, ARRAY_SIZE(ints1));

        gpio_set_debounce(ints[0], ints[1]);
        mc3xxx_obj_i2c_data->irq = irq_of_parse_and_map(mc3xxx_obj_i2c_data->irq_node, 0);
        if (!mc3xxx_obj_i2c_data->irq)
        {
            ACC_PR_ERR("irq_of_parse_and_map fail!!\n");
            return -EINVAL;
        }
        ret = request_irq(mc3xxx_obj_i2c_data->irq, eint_func,  IRQF_TRIGGER_LOW, "mediatek, GSE_1-eint", NULL);
        if (ret != 0)
        {
        ...
        {
            printk("%s set EINT finished, mc3xxx_obj_i2c_data->irq=%d, acceldebounce=??\n",
                     __func__,mc3xxx_obj_i2c_data->irq);
        }
    }
    else
    {
    ...
    enable_irq_wake(mc3xxx_obj_i2c_data->irq);
    return 0;
}
...
int mc3413_set_acc_interrupter(bool flag)
{
    ...
    printk("set_acc_interrupter en =%d\n",flag);
    if(flag)
    {
        significant_enable_status = true;
        MC3XXX_SetPowerMode(obj->client, false);     
        res = setMotionEnable(obj->client,true);  //set 0x1b enable wake up function
        if(res !=0)
        {
            ACC_PR_ERR("SetMotionEnable faild\n");
        }
        
        res = setIntEnable(obj->client,true);   //0x20  physical interrupt pin (7) is enabled
        if(res !=0)
        {
            ACC_PR_ERR("SetIntEnable faild\n");
        }
        MC3XXX_SetPowerMode(obj->client, true); 
        msleep(500);
    }
    else
    {
        ...
        MC3XXX_SetPowerMode(obj->client, false);  //set register power must to  false
        res = setIntEnable(obj->client,false); 
        res = setMotionEnable(obj->client,false);
        if(res !=0)
        {
            ACC_PR_ERR("setMotionEnable faild\n");
        }
...
static int  mc3xxx_local_init(void)
{
	...
	if (s_nInitFlag == MC3XXX_INIT_FAIL)
		return -1;
    sprintf(gsensor_module_name,"MC3413-P:mcube:NA");//add by lanying.he for gsensor deviceinfo
	return 0;
}
