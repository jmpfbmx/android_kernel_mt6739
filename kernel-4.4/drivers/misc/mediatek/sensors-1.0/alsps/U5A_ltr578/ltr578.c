...
int extern_ltr578_dynamic_calibrate(void);
extern int ps_backlight_on;
/******************************************************************************
 * configuration
*******************************************************************************/
/*----------------------------------------------------------------------------*/
#define LTR578_DEV_NAME			"ltr578"
//Begin add for lcm information for light persion 16/03/2018
extern unsigned char lcm_color;
//End add for lcm information for light persion 16/03/2018
/*----------------------------------------------------------------------------*/
...
static int ltr578_ps_enable(struct i2c_client *client, int enable)
{
	...
	APS_LOG("ltr578_ps_enable() ...start! enable=%d, ps_enabled=%d\n",enable, ps_enabled);
...
static int ltr578_dynamic_calibrate(void)
{
	...
	if (noise < 100) {
		...
		ps_thd_val_high = noise + 105;
		ps_thd_val_low  = noise + 75;
	}
	else if (noise < 600) {
		...
		ps_thd_val_high = noise + 130;
		ps_thd_val_low  = noise + 100;
	}
	else if (noise < 800) {
		ps_thd_val_high = noise + 135;
		ps_thd_val_low  = noise + 105;
	}
	else if (noise < 1000) {
		ps_thd_val_high = noise + 140;
		ps_thd_val_low  = noise + 115;
	}
	else if( noise < 1250){
		ps_thd_val_high = noise + 155;
		ps_thd_val_low  = noise + 130;
	}
	else if( noise < 1400){
		ps_thd_val_high = noise + 180;
		ps_thd_val_low  = noise + 150;
	}
	else if (noise < 1600) {
		ps_thd_val_high = noise + 190;
		ps_thd_val_low  = noise + 160;
	}
	else {
		ps_thd_val_high = 1850;
		ps_thd_val_low  = 1750;		
	}
...
#ifdef GN_MTK_BSP_PS_DYNAMIC_CALI
static int ltr578_powerkey_dynamic_calibrate(void)
{
	int i = 0;
	int data;
	int data_total = 0;
...
static int ltr578_als_read(struct i2c_client *client, u16* data)
{
	...
	ret = ltr578_master_recv(client, LTR578_MAIN_CTRL, &regdata, 0x01);
	if ( ((regdata&0x02) != 0x02) && (ps_enabled == 0)) {
		//APS_LOG("als: enable als 20180427");
		regdata |= 0x02;
		ret = ltr578_master_send(client, LTR578_MAIN_CTRL, (char *)&regdata, 1);
	}
	...
	} else if (clearval == 0x1FFFF)
    {
     ...
     goto out;
      ...
      switch(lcm_color){
         ...
         case 2:
            if (ratio < 25) 
		luxdata_int = alsval * 41 * ALS_WIN_FACTOR / 20;	  //CWF is ok
	   else
                luxdata_int = alsval * 17 * ALS_WIN_FACTOR / 10;	  // D65 A 
               break;
		...
		luxdata_int = alsval * 41 * ALS_WIN_FACTOR / 20;	  //CWF is ok
      	    ...
      	    if(alsval < 25){
	        luxdata_int = alsval * 13 * 52 * ALS_WIN_FACTOR / 500;
           }else if( alsval < 200){
                luxdata_int = alsval * 17 * 91 * ALS_WIN_FACTOR / 1000;
           }else
               luxdata_int = alsval * 17 * ALS_WIN_FACTOR / 10;	  // D65 A 
           }
           break;
...
static int LTR578_ReadChipInfo(struct i2c_client *client,
	 char *buf, int bufsize)
{
	...
	if (NULL == client) {
		*buf = 0;
		...
		return snprintf(buf, LTR578_BUFSIZE, "LTR578 Chip");
}
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	struct i2c_client *client = ltr578_i2c_client;
	char strbuf[LTR578_BUFSIZE];
	if (NULL == client) {
		return 0;
	}
	LTR578_ReadChipInfo(client, strbuf, LTR578_BUFSIZE);
...
#ifndef DELAYED_PS_CALI
#if 0
			if(obj->ps > 20 && obj->ps < (dynamic_calibrate - 60))
			{
	        		if(obj->ps < 100){
	        			...
	        			atomic_set(&obj->ps_thd_val_high,  1850);
	        			atomic_set(&obj->ps_thd_val_low, 1750);
	        		}

        			dynamic_calibrate = obj->ps;
				ltr578_ps_set_thres(); // add by liuyang3.wt.
        		}
#endif
#ifdef UPDATE_PS_THRESHOLD
		databuf[0] = LTR578_PS_THRES_LOW_0;	
...
//	printk(KERN_ERR "als_get_data *value = %d \n" , *value);
...
{
	int res = 0;
	APS_LOG("test ltr578_obj ps enable value = %d\n", en);
...
int extern_ltr578_dynamic_calibrate(void){
	int value = 0;
	value = ltr578_ps_read(ltr578_obj->client, &ltr578_obj->ps);
	APS_ERR("ALS ltr578 ps_enabled =%d, intr_flag_value =%d,value =%d\n",ps_enabled,intr_flag_value,value);
	if ( 1 == ps_enabled && intr_flag_value == 1 && value > 300 && value < 1800 && ps_backlight_on == 0)
	{
			ltr578_powerkey_dynamic_calibrate();
			ltr578_ps_set_thres();
...
static int ltr578_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,ltr578");
      if(node != NULL)
      {	  
