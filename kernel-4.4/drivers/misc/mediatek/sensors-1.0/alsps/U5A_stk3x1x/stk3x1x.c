...
#define DRIVER_VERSION          "3.9.1 20170622"
...
/*Begin ersen.shang for robust 201610*/
...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	60 //185 //150//200
	#define STK_LT_N_CT	   20 //80   //100
	#define STK_HT_N_CT	   50 //180 //150
#endif /* #ifdef STK_TUNE0 */

#define STK_H_PS		500 //1500 //500  
#define STK_H_HT		150 //400
#define STK_H_LT		60 //350
...
static int STK_IRC_ALS_A_CORRECT=860;   //748
static int STK_IRC_ALS_D65_CORRECT=1200;  //860
static int STK_IRC_ALS_CWF_CORRECT=1575;
/*----------------------------------------------------------------------------*/
...
static struct alsps_hw alsps_cust_1;
static struct alsps_hw *hw = &alsps_cust_1;
...
void stk_als_ir_get_corr(struct stk3x1x_priv *obj, u32 als_data)
{
    ...
    {
		if(als_data!=0)
			als_comperator = obj->ir_code/als_data;
		else
			als_comperator = 0;
        obj->als_correct_factor = STK_IRC_ALS_CWF_CORRECT;
		// 20000 & 25 &50
        if (als_data < STK_IRC_MAX_ALS_CODE && als_data > STK_IRC_MIN_ALS_CODE &&obj->ir_code > STK_IRC_MIN_IR_CODE)
        {
              if(obj->ir_code == 65535)
                  obj->als_correct_factor  =  STK_IRC_ALS_A_CORRECT;
              else if(als_comperator >= 32){
				  if(als_data < 30){
					  STK_IRC_ALS_A_CORRECT = 245;
				  }else if(als_data < 100){
					  STK_IRC_ALS_A_CORRECT = 600;
				  }else{
					  STK_IRC_ALS_A_CORRECT = 860;
				  }
                  obj->als_correct_factor  =  STK_IRC_ALS_A_CORRECT;
			  }
			  else if(als_comperator > 5 ){
				  if(als_data < 30){
					  STK_IRC_ALS_D65_CORRECT = 715; 
				  }else{
					  STK_IRC_ALS_A_CORRECT = 1200;
				  }
			  obj->als_correct_factor  =  STK_IRC_ALS_D65_CORRECT;
			  }
			  else{
				  if(als_data < 30){
					  STK_IRC_ALS_CWF_CORRECT = 2350;				 
				  }else{
					  STK_IRC_ALS_CWF_CORRECT = 1575;
				  }
				  obj->als_correct_factor  =  STK_IRC_ALS_CWF_CORRECT;
			  }
        }
        printk("stk als test : als=%d, ir=%d, als_comperator=%d, als_correct_factor=%d \n",  als_data, obj->ir_code, als_comperator, obj->als_correct_factor);
        obj->ir_code = 0;
    }
	else
       printk("stk als test : als=%d, ir=%d, last_ir=%d\n",  als_data, obj->ir_code, obj->last_ir_code);
       ...
       // printk("stk test  1>>>>>>>>>>\n");		
		...
		else
		{
			...
			//	err = (obj->ps >= atomic_read(&obj->ps_high_thd_val))?0:1;
			    ...
			    printk("enable ps inital ps value is %d >>>\n",err);
#endif			
				if(err < 0)
				{
...
#else						
	obj->ps_high_thd_boot = obj->ps_stat_data[1] + obj->stk_ht_n_ct + STK_H_LT;
	obj->ps_low_thd_boot = obj->ps_stat_data[1] + obj->stk_lt_n_ct + STK_H_LT;
	atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
	atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
#endif
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
  ...
  //  printk("wingtech stk test  2 ps_enabled = %d >>>>>>>>>>\n",ps_enabled);	
#ifndef CALI_EVERY_TIME
...
#endif
	{
		//printk("wingtech stk test CALI_EVERY_TIME 2 ps_enabled =  >>>>>>>>>>\n");
		return 0;
	}

       ret = stk3x1x_master_recv(obj->client, 0x06, buf, 2);
	   if(ret < 0)
		 {
		   APS_ERR( "read High thd reg %s failed, err=0x%x", __func__, ret);
					return ret;	   
		}
	  high_thd = (buf[0] << 8) | buf[1];
      if(high_thd!= atomic_read(&obj->ps_high_thd_val))
	  {
		...
		if((obj->ps_distance_last == 1))
		{
          ...
          //  printk("stk test  3 ret= %d >>>>>>>>>>\n",ret);				
			if(ret == 0)
			{
				...
				if(word_data > 0)
				{   
			      //  printk("stk test  3 ret= %d %d %d>>>>>>>>>>\n",atomic_read(&obj->ps_high_thd_val),stk3x1x_obj->stk_ht_n_ct,obj->ps_thd_update);	
					ct_value = atomic_read(&obj->ps_high_thd_val) - stk3x1x_obj->stk_ht_n_ct;
					ret = stk3x1x_read_flag(obj->client, &flag);
				//	printk("stk test  4 word_data= %d,ct=%d,flag =%x, distance_last =%d >>>>>>>>>>\n",word_data,ct_value,flag,obj->ps_distance_last);	
					if((word_data < ct_value) && ((ct_value - word_data) > 2) && ((flag&STK_FLG_PSINT_MASK)==0) && (obj->ps_distance_last == 1) && (obj->ps_thd_update == false))
					{
						...
						stk3x1x_write_ps_high_thd(obj->client,  atomic_read(&obj->ps_high_thd_val));		
						stk3x1x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_low_thd_val));
						printk("%s: CTTRACKING set HT=%d, LT=%d\n", __func__, atomic_read(&obj->ps_high_thd_val), atomic_read(&obj->ps_low_thd_val));
						obj->psi = word_data;
						APS_LOG("%s: CTTRACKING update psi=%d\n", __func__, obj->psi);	
						if((atomic_read(&obj->ps_high_thd_val) + 100) < (obj->ps_high_thd_boot))
						{
							obj->ps_high_thd_boot = word_data + obj->stk_ht_n_ct + STK_H_LT;
							obj->ps_low_thd_boot = word_data + obj->stk_lt_n_ct + STK_H_LT;
							obj->boot_cali = 1;
...
static void stk_ps_tune0_work_func(struct work_struct *work)
{
	...
	printk("%s: ct>STK_PS_CT_LIMIT,%d \n", __func__,obj->tune_zero_init_proc);
#ifdef STK_GES		
	if(obj->ges_enabled)
	...
	if(obj->tune_zero_init_proc){
	 ...
	 printk("%s: ct>STK_PS_CT_LIMIT,obj->tune_zero_init_proc %d \n", __func__,obj->tune_zero_init_proc);
	}
	else{
		printk("%s: ct>STK_PS_CT_LIMIT,stk_ps_tune_zero_func_fae %d \n", __func__,obj->tune_zero_init_proc);
...
static enum hrtimer_restart stk_ps_tune0_timer_func(struct hrtimer *timer)
{
	...
	//schedule_work(&obj->stk_ps_tune0_work);
...
/* eint request */
	if (stk3x1x_obj->irq_node) {
		...
		if (request_irq(stk3x1x_obj->irq, stk3x1x_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
...
static ssize_t stk3x1x_show_ir(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "0x%04X %d\n", stk3x1x_obj->ir_code,stk3x1x_obj->als_correct_factor);     
}
...
{
#if 1
	APS_LOG("%s: als value = %d, als_correct_factor=%d\n", __func__, (als * obj->als_correct_factor / 1000), obj->als_correct_factor);
	return (als * obj->als_correct_factor / 1000);
#else
	int idx;
	int invalid = 0;
...
static int32_t stk3x1x_get_ir_value(struct stk3x1x_priv *obj, int32_t als_it_reduce)
{
	...
	uint8_t state_reg, w_reg, retry = 0;	
...
irs_err_i2c_rw:	
	//reset als data
	stk3x1x_clear_intr(obj->client, flag, STK_FLG_IR_RDY_MASK);	
	i2c_smbus_write_byte_data(obj->client, STK_ALSCTRL_REG, atomic_read(&obj->alsctrl_val));
	
	stk3x1x_master_recv(obj->client, obj->addr.state, &state_reg, 0x01);
	i2c_smbus_write_byte_data(obj->client, obj->addr.state, state_reg);
...
{
    int m_value = 0;
    m_value = (int)samplingPeriodNs / 1000 / 1000;
    APS_LOG("stk3x1x als set delay = (%d) ok.\n", m_value);
    return 0;
}
...
static int ps_batch(int flag, int64_t samplingPeriodNs,int64_t maxBatchReportLatencyNs)
{
    int m_value = 0;
    m_value = (int)samplingPeriodNs / 1000 / 1000;
    APS_LOG("stk3x1x ps set delay = (%d) ok.\n", m_value);
    return 0;
}
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(node != NULL)
    {
	    ...
	    if (err < 0) {
		    APS_ERR("get alsps info from dts failed\n");
		    goto exit;
	    }
    }
    else
    {
        APS_ERR("get mag dts node fail\n");
        err = -EFAULT;
		goto exit;
    }
    ...
    obj->irq_node = client->dev.of_node;  //Add by psensor interrupt
	atomic_set(&obj->state_val, 0);
	atomic_set(&obj->psctrl_val, 0x31);//0x31
	...
	obj->ps_thd_update = false;
#endif
	obj->als_correct_factor = STK_IRC_ALS_CWF_CORRECT; //1000
	...
	obj->stk_ps_tune0_wq = create_workqueue("stk_ps_tune0_wq");
...
/*----------------------------------------------------------------------------*/
extern char psensor_module_name[256];  //add by lanying.he adding psensor deviceinfo
//extern int proximity_initialise_status;  //added by lanying.he for robust
static int stk3x1x_local_init(void) 
{
    APS_FUN();
    ...
    sprintf(psensor_module_name,"STK3311-s34:sensortek:NA"); //add by lanying.he adding psensor deviceinfo
    //proximity_initialise_status = 1;//added by lanying.he for robust
	return 0;
}
...
static int __init stk3x1x_init(void)
{
	...
	printk("stk3x1x_init Out");
         APS_ERR("salsps_driver_add(&stk3x1x_init_info)\n");
	return 0;
}
/*----------------------------------------------------------------------------*/
