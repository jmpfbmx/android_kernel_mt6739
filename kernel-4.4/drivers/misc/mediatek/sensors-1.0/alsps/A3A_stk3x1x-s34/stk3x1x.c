...
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	110 //185 //150//200
	#define STK_LT_N_CT	   45 //80   //100
	#define STK_HT_N_CT	   100 //180 //150
#endif /* #ifdef STK_TUNE0 */

#define STK_H_PS		1200 //1500 //500  
#define STK_H_HT		300 //400
#define STK_H_LT		200 //350
...
void stk_als_ir_get_corr(struct stk3x1x_priv *obj, u32 als_data)
{
	...
	if(obj->ir_code)
	{
		obj->als_correct_factor = 1250; //690
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	if(obj->p_1x_r_bd_with_co == 0x07 || obj->p_19_r_bc == 0x03)
	{
		...
		if(als_data > 65535)
			als_data = 65535;
		printk("xiaobin.zhai 1 als_data:%d", als_data);
	}
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	u8 buf[2],psctl_reg,ledctl_reg,flag_reg;
	int ret, diff;
	...
	ret = stk3x1x_master_recv(obj->client, 0x01, &psctl_reg, 1);
	if(ret < 0)
    {
	    APS_ERR( "%s read psctl reg fail, err=0x%x", __func__, ret);
	    return ret;	   
	}
	ret = stk3x1x_master_recv(obj->client, 0x03, &ledctl_reg, 1);
	if(ret < 0)
    {
	    APS_ERR( "%s read ledctl reg fail, err=0x%x", __func__, ret);
	    return ret;	   
	}
	if(psctl_reg!=0x30||ledctl_reg!=0x3F)
	{
			
	APS_LOG("%s: stk test>>>>> reg check run \n", __FUNCTION__);
	if((ret= stk3x1x_write_state(obj->client, atomic_read(&obj->state_val))))
	{
		APS_ERR("write stete error: %d\n", ret);       
	}	
	
	psctl_reg = atomic_read(&obj->psctrl_val);
	if(obj->hw->polling_mode_ps == 1)
      psctl_reg &= 0x3F;
  
	if((ret= stk3x1x_write_ps(obj->client, psctl_reg)))
	{
		APS_ERR("write ps error: %d\n", ret);	       
	}
	if((ret= stk3x1x_write_als(obj->client, atomic_read(&obj->alsctrl_val))))
	{
		APS_ERR("write als error: %d\n", ret);	
	}	
	
	ret= stk3x1x_write_led(obj->client, obj->ledctrl_val);
	if(ret)
	{
		APS_ERR("write led error: %d\n", ret);	
	}	
	
	if((ret = stk3x1x_write_wait(obj->client, obj->wait_val)))
	{
		APS_ERR("write wait error: %d\n", ret);	
	}	
	...
	ret = stk3x1x_write_int(obj->client, obj->int_val);
	if(ret)
	{
		APS_ERR("write int mode error: %d\n", ret);	        
	}	
}
	ret = stk3x1x_master_recv(obj->client, 0x10, &flag_reg, 1);
	if(ret < 0)
    {
	    APS_ERR( "%s read flag reg fail, err=0x%x", __func__, ret);  
    }
	
    // printk("stk test >>>>>>> flag_reg is %x, distance_last is %d \n",flag_reg,obj->ps_distance_last);
    if((int)(flag_reg&0x01) != obj->ps_distance_last)
	{
        if(near_far_flag < 5)
			near_far_flag++ ;
		else{
		   ...
		   //printk("stk test >>>>>>> flag_reg is %x, distance_last is %d \n",flag_reg,obj->ps_distance_last);
		   stk_ps_int_handle(obj,flag_reg&0x01);
		   near_far_flag = 0;
		}
	}
...
//	obj->hw = get_cust_alsps_hw();
	...
	obj->ps_thd_update = false;
#endif
	obj->als_correct_factor = 1250; //1000
