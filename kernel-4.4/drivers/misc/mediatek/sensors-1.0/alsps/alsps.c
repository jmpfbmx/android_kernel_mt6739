...
/* *************************************************** */
//Begin added by lanying.he for XR5424729 on 2017/10/20 deviceinfo
char psensor_module_name[256]="LTR578:GUANGBAO:0X53";
static ssize_t  psensorinfo_show(struct device *dev,
                                  struct device_attribute *attr, char *buf)
{
     printk("Dump  %s \n",psensor_module_name);
...
static ssize_t psensorinfo_store(struct device* dev,
                                   struct device_attribute *attr, const char *buf, size_t count)
{
        return count;
}

DEVICE_ATTR(psensor,     S_IWUSR | S_IRUGO, psensorinfo_show, psensorinfo_store);
DEVICE_ATTR(lsensor,     S_IWUSR | S_IRUGO, psensorinfo_show, psensorinfo_store);
extern struct device* get_deviceinfo_dev(void);

static void Create_Psensorinfo_node_ForMMI(void)
{
       struct device * psensor;
       psensor=get_deviceinfo_dev();
       if (device_create_file(psensor, &dev_attr_psensor) < 0)
               printk("Failed to create device file(%s)!\n", dev_attr_psensor.attr.name);
        return;
}
static void Create_Lsensorinfo_node_ForMMI(void)
{
       struct device * lsensor;
       lsensor=get_deviceinfo_dev();
       if (device_create_file(lsensor, &dev_attr_lsensor) < 0)
               printk("Failed to create device file(%s)!\n", dev_attr_lsensor.attr.name);
        return;
}
//End added by lanying.he for XR5424729 on 2017/10/20 deviceinfo
//Begin added by lanying.he for XR5495833 on 2017/1031 robust
extern int proximity_initialise_status;
//End added by lanying.he for XR5495833 on 2017/1031 robust
static int alsps_probe(void)
{
	...
	//End added by lanying.he for XR5495833 on 2017/1031 robust
	//Begin: added by lanying.he for XR5424729 on 2017/10/20 deviceinfo
	Create_Psensorinfo_node_ForMMI();
	Create_Lsensorinfo_node_ForMMI();
	//End: added by lanying.he for XR5424729 on 2017/10/20 deviceinfo 
