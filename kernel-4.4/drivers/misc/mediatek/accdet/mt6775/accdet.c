...
static atomic_t g_accdet_first;
...
{
	int vol_val = pmic_get_auxadc_value(AUXADC_LIST_ACCDET);
	...
	ACCDET_DEBUG("%s %d offset:%d real vol:%d mv!\n",
		__func__, __LINE__, accdet_auxadc_offset, vol_val);
	return vol_val;
}
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	if (IS_ERR(accdet_pinctrl1)) {
		...
		ACCDET_INFO("Cannot find accdet accdet_pinctrl1!\n");
		return ret;
	}
	...
	if (node) {
		...
		ret = of_property_read_u32(node, "debounce", &headsetdebounce);
		if (ret < 0) {
		...
		ACCDET_ERROR("[Accdet] gpiopin:%d debounce:%d accdet_irq:%d accdet_eint_type:%d\n",
			gpiopin, headsetdebounce, accdet_irq, accdet_eint_type);
...
static void multi_key_detection(int current_status)
{
	...
	/*This delay is used for some headset report a AB = 0 when headset plug out*/
	/*tomer can set delay time according to their device performance*/
	/*mdelay(30);*/
#ifdef CONFIG_ACCDET_EINT_IRQ
	...
	case HOOK_SWITCH:
		if (current_status == 0) {
			...
			if (eint_accdet_sync_flag == 1) {
				...
				/* cable_type = HEADSET_NO_MIC; */
				/* accdet_status = HOOK_SWITCH; */
				ACCDET_DEBUG("[Accdet]1.HOOK_SWITCH state not change!\n");
				ACCDET_DEBUG("[Accdet]2. HOOK_SWITCH state not change!\n");
			} else {
				ACCDET_DEBUG("[Accdet] Headset has plugged out\n");
			}
			mutex_unlock(&accdet_eint_irq_sync_mutex);
...
static void accdet_pmic_Read_Efuse_HPOffset(void)
{
	...
	/* AD */
	tmp_val = (int)pmic_pwrap_read(REG_ACCDET_AD_CALI_1);
	efusevalue = (tmp_val>>RG_ACCDET_BIT_SHIFT)&ACCDET_CALI_MASK2;
	tmp_val = (int)pmic_pwrap_read(REG_ACCDET_AD_CALI_2);
	...
	tmp_val = (int)pmic_pwrap_read(REG_ACCDET_AD_CALI_3);
	efusevalue = efusevalue | ((tmp_val & 0x01) << RG_ACCDET_HIGH_BIT_SHIFT);
	...
	tmp_val = (int)pmic_pwrap_read(REG_ACCDET_AD_CALI_0);/* read HW reg */
	efusevalue = ((tmp_val>>RG_ACCDET_BIT_SHIFT)&ACCDET_CALI_MASK0);
	tmp_val = (int)pmic_pwrap_read(REG_ACCDET_AD_CALI_1);/* read HW reg */
...
static inline void accdet_init(void)
{
	...
	if (cur_eint_state == EINT_PIN_PLUG_OUT) {
		if (accdet_dts_data.eint_level_pol == IRQ_TYPE_LEVEL_HIGH) {
			...
			if (atomic_read(&g_accdet_first)) {/* set pmic eint default value */
...
#else
	pmic_pwrap_write(ACCDET_HW_MODE_DFF, 0x8000);/* Only use SW path */
#endif
...
/* wake lock */
	...
	if (ret == 0) {
		...
		ACCDET_INFO("[Accdet]accdet_get_dts_data err!\n");
	}
