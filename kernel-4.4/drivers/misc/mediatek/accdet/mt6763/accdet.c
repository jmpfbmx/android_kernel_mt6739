...
static TY_CFG_ACCDET_PARAM *accdet_cust_setting;
static TY_CFG_HEADSET_PARAM headset_dts_data;
...
static inline int accdet_set_debounce(TY_ACCDET_STATE state, unsigned int debounce);
...
static int accdet_set_debounce(TY_ACCDET_STATE state, unsigned int debounce)
{
	int ret = 0;
...
static int accdet_eint_func(int eint_id)
{
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		...
		if (g_cur_eint_state == EINT_PIN_PLUG_IN) {
			...
			ACCDET_INFO("[accdet_eint_func]plugin wait eint:eint0=%d;eint1=%d\n",
		...
		if ((g_cur_eint0_state|g_cur_eint1_state) == EINT_PIN_PLUG_OUT) {
			...
			ACCDET_INFO("[accdet_eint_func]plugout eint all:eint0=%d;eint1=%d\n",
				g_cur_eint0_state, g_cur_eint1_state);
		} else if ((g_cur_eint0_state&g_cur_eint1_state) == EINT_PIN_PLUG_OUT) {
			g_cur_eint_state = (g_cur_eint0_state&g_cur_eint1_state);
			ACCDET_INFO("[accdet_eint_func]plugout wait eint:eint0=%d;eint1=%d\n",
				g_cur_eint0_state, g_cur_eint1_state);
			ret = queue_work(accdet_eint_workqueue, &accdet_eint_work);
		} else {
			ACCDET_INFO("[accdet_eint_func]plugout err:eint0=%d;eint1=%d\n",
				g_cur_eint0_state, g_cur_eint1_state);
		}
	} else {
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	if (IS_ERR(accdet_pinctrl1)) {
		...
		dev_err(&accdet_device->dev, "[accdet]Cannot find accdet accdet_pinctrl1!\n");
		return ret;
	}
	...
	if (IS_ERR(pins_default)) {
		...
		/* dev_err(&accdet_device->dev, "[accdet]Cannot find accdet pinctrl default!\n"); */
	}

	pins_eint_int = pinctrl_lookup_state(accdet_pinctrl1, "state_eint_as_int");
	if (IS_ERR(pins_eint_int)) {
		ret = PTR_ERR(pins_eint_int);
		dev_err(&accdet_device->dev, "[accdet]Cannot find accdet pinctrl state_eint!\n");
		return ret;
	}
	...
	if (node) {
		...
		if (ret != 0)
			ACCDET_ERROR("[accdet_setup_eint]EINT IRQ LINE NOT AVAILABLE\n");
		else
			ACCDET_INFO("[accdet_setup_eint]irq=%d, deb=%d\n", g_accdet_irq, g_gpio_headset_deb);
	} else {
		ACCDET_ERROR("[accdet_setup_eint]%s can't find compatible node\n", __func__);
	}
	return 0;
}
...
/* need add clear mode bit----bxx */
		if (headset_dts_data.accdet_mic_mode == HEADSET_MODE_1)/* ACC mode*/
			pmic_pwrap_write(AUDENC_ANA_CON11, reg_val|0x0807);/* 0x0807 */
		else if (headset_dts_data.accdet_mic_mode == HEADSET_MODE_2) {/* Low cost mode without internal bias*/
			/* for test discharge quickly */
			/* pmic_pwrap_write(AUDENC_ANA_CON11, reg_val|RG_ACCDET_MODE_ANA11|(0x01<<10)); */
			pmic_pwrap_write(AUDENC_ANA_CON11, reg_val|RG_ACCDET_MODE_ANA11);/* 0x0887 */
			reg_val = pmic_pwrap_read(AUDENC_ANA_CON10);
			pmic_pwrap_write(AUDENC_ANA_CON10, reg_val&(~RG_AUD_MBIAS1_DC_SW_3P));/*0x0150*/
		} else if (headset_dts_data.accdet_mic_mode == HEADSET_MODE_6) {/* Low cost mode with internal bias*/
			pmic_pwrap_write(AUDENC_ANA_CON11, reg_val|RG_ACCDET_MODE_ANA11);/* 0x0887 */
			reg_val = pmic_pwrap_read(AUDENC_ANA_CON10);
			#ifdef CONFIG_HEADSET_SUPPORT_FIVE_POLE
			pmic_pwrap_write(AUDENC_ANA_CON10, reg_val|RG_MBIAS_MODE_6);/* 0x1F2 */
			#else
			 pmic_pwrap_write(AUDENC_ANA_CON10, reg_val|RG_AUD_MBIAS1_DC_SW_3P);
			#endif

			#if 0
			#ifdef CONFIG_FOUR_KEY_HEADSET/* 4-key, 2.7V, internal bias for mt6337*/
			reg_val = pmic_pwrap_read(AUDENC_ANA_CON10);/* for 4-key more accuracy */
...
static inline void accdet_init(void)
{
	...
	pwrap_write(AUDENC_ANA_CON11, pmic_pwrap_read(AUDENC_ANA_CON11)|RG_AUD_ACCDET_MBIAS1_EN);
	pmic_pwrap_write(AUDENC_ANA_CON10, pmic_pwrap_read(AUDENC_ANA_CON10)
...
#ifdef CONFIG_ACCDET_EINT_IRQ
#ifdef CONFIG_ACCDET_SUPPORT_EINT0
	pmic_register_interrupt_callback(INT_ACCDET_EINT, accdet_eint_int_handler);/* accdet eint0 */
