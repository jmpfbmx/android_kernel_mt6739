				  ...
				  unsigned int packet_type)
{
	...
	if (IS_CCMNI_LAN(dev)) {
		...
		if (br_dev) {
			...
			CCMNI_DBG_MSG(md_id, "%s can't find mdbr0\n", dev->name);
...
static inline int is_ack_skb(int md_id, struct sk_buff *skb)
{
	...
	if (packet_type == IPV6_VERSION) {
		...
		/* copy md tag into skb->tail and skb->len > 128B(md recv buf size) */
		...
		if (ntohs(iph->tot_len) <= 128 - sizeof(struct ccci_header) - count) {
...
static inline int arp_reply(int md_id, struct net_device *dev, struct ethhdr *eth, struct sk_buff *skb)
{
	...
	if (!new_skb) {
		CCMNI_DBG_MSG(md_id, "arp_reply: %s can't alloc new skb\n", dev->name);
		goto alloc_skb_fail;
	}
...
static int ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (tag->guard_pattern == MDT_TAG_PATTERN) {
		if (ccmni->md_id == MD_SYS1) {
			...
			CCMNI_DBG_MSG(ccmni->md_id, "%s: MD%d not support MDT tag\n",
	...
	if (ret == CCMNI_ERR_MD_NO_READY || ret == CCMNI_ERR_TX_INVAL) {
		...
		CCMNI_DBG_MSG(ccmni->md_id, "[TX]CCMNI%d send tx_pkt=%ld(ack=%d) fail: %d\n",
	...
	if (ccmni->tx_busy_cnt[is_ack] > 10) {
		CCMNI_DBG_MSG(ccmni->md_id, "[TX]CCMNI%d TX busy: tx_pkt=%ld(ack=%d) retry %ld times done\n",
...
tx_busy:
	if (unlikely(!(ctlb->ccci_ops->md_ability & MODEM_CAP_TXBUSY_STOP))) {
		if ((ccmni->tx_busy_cnt[is_ack]++) % 100 == 0)
			CCMNI_DBG_MSG(ccmni->md_id, "[TX]CCMNI%d TX busy: tx_pkt=%ld(ack=%d) retry_times=%ld\n",
				      ccmni->index, dev->stats.tx_packets, is_ack, ccmni->tx_busy_cnt[is_ack]);
	} else {
...
static int ccmni_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case SIOCSTXQSTATE:
		...
		if (likely(ccmni_tmp != NULL)) {
			CCMNI_DBG_MSG(ccmni->md_id, "SIOCSTXQSTATE: %s_state=0x%x, cnt=(%d, %d)\n",
				      dev->name, ifr->ifr_ifru.ifru_ivalue, atomic_read(&ccmni->usage),
				      atomic_read(&ccmni_tmp->usage));
		} else {
			CCMNI_DBG_MSG(ccmni->md_id, "SIOCSTXQSTATE: %s_state=0x%x, cnt=%d\n",
				      dev->name, ifr->ifr_ifru.ifru_ivalue, atomic_read(&ccmni->usage));
		}
		break;
...
static void ccmni_napi_poll_timeout(unsigned long data)
{
	...
	CCMNI_DBG_MSG(ccmni->md_id, "CCMNI%d lost NAPI polling\n", ccmni->index);
}


/********************ccmni driver register  ccci function********************/
static inline int ccmni_inst_init(int md_id, struct ccmni_instance *ccmni, struct net_device *dev)
{
	struct ccmni_ctl_block *ctlb = ccmni_ctl_blk[md_id];
	int ret = 0;
	...
	if (ret) {
		CCMNI_PR_ERR(md_id, "get ccmni%d channel fail\n", ccmni->index);
		return ret;
	}
...
static int ccmni_init(int md_id, struct ccmni_ccci_ops *ccci_info)
{
	...
	if ((ctlb->ccci_ops->md_ability & MODEM_CAP_CCMNI_IRAT) != 0) {
		...
		for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
			...
			memcpy(ccmni_irat_src, ctlb_irat_src->ccmni_inst[i], sizeof(struct ccmni_instance));
	...
	if (ctlb->ccci_ops->md_ability & MODEM_CAP_DIRECT_TETHERING) {
		...
		ret = register_netdev(dev);
		if (ret) {
			CCMNI_PR_ERR(md_id, "CCMNI%d register netdev fail: %d\n", i, ret);
			goto alloc_netdev_fail;
		}
	}
...
static void ccmni_exit(int md_id)
{
	...
	if (ctlb) {
		...
		for (i = 0; i < ctlb->ccci_ops->ccmni_num; i++) {
			ccmni = ctlb->ccmni_inst[i];
			if (ccmni) {
				CCMNI_DBG_MSG(md_id, "ccmni_exit: unregister ccmni%d dev\n", i);
				unregister_netdev(ccmni->dev);
				/* free_netdev(ccmni->dev); */
				ctlb->ccmni_inst[i] = NULL;
			}
...
static void ccmni_queue_state_callback(int md_id, int ccmni_idx, enum HIF_STATE state, int is_ack)
{
	...
	if (unlikely(ctlb == NULL)) {
		CCMNI_DBG_MSG(md_id, "invalid ccmni ctrl when ccmni%d_hif_sta=%d\n", ccmni_idx, state);
		return;
	}
...
static void ccmni_md_state_callback(int md_id, int ccmni_idx, enum MD_STATE state)
{
	...
	if (unlikely(ctlb == NULL)) {
		CCMNI_DBG_MSG(md_id, "invalid ccmni ctrl when ccmni%d_md_sta=%d\n", ccmni_idx, state);
		return;
	}
