...
int md_cd_get_modem_hw_info(struct platform_device *dev_ptr, struct ccci_dev_cfg *dev_cfg, struct md_hw_info *hw_info)
{
	...
	case 0:		/* MD_SYS1 */
		...
		if (!hw_info->ap_topclkgen_base) {
			CCCI_ERROR_LOG(-1, TAG, "md_cd_get_modem_hw_info:ioremap topclkgen base address fail\n");
			return -1;
		}
		break;
	default:
...
#ifdef CONFIG_MTK_QOS_SUPPORT
		pm_qos_add_request(&md_qos_vcore_request, PM_QOS_VCORE_OPP, VCORE_OPP_0);
#else
...
int md_cd_power_off(struct ccci_modem *md, unsigned int timeout)
{
	...
	case MD_SYS1:
		...
		reg_value |= ((1<<8)|(1<<9));
		ccci_write32(hw_info->ap_topclkgen_base, 0, reg_value);
		CCCI_BOOTUP_LOG(md->index, CORE, "md_cd_power_off: set md1_clk_mod =0x%x\n",
			     ccci_read32(hw_info->ap_topclkgen_base, 0));
