...
{
	struct ccci_smem_port *smem_port = container_of(timer, struct ccci_smem_port, notify_timer);

	ccci_md_send_ccb_tx_notify(smem_port->port->md_id, smem_port->core_id);
...
static void collect_ccb_info(int md_id, struct ccci_smem_port *smem_port)
{
	...
	for (i = SMEM_USER_CCB_START; i <= SMEM_USER_CCB_END; i++) { /* check current port is CCB or not */
		if (smem_port->user_id == i) {
			curr_size = 0;
			/* calculate length */
			for (j = 0; j < ccb_configs_len; j++) { /* search for first present */
				if (smem_port->user_id == ccb_configs[j].user_id)
					break;
			}
			smem_port->ccb_ctrl_offset = j;
			for ( ; j < ccb_configs_len; j++) { /* traverse to last present */
				if (smem_port->user_id != ccb_configs[j].user_id)
					break;
			...
			if (curr)
				curr->size = curr_size;
			CCCI_BOOTUP_LOG(md_id, TAG, "CCB user %d: ccb_ctrl_offset=%d, length=%d\n",
			...
			hrtimer_init(&smem_port->notify_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
			smem_port->notify_timer.function = smem_tx_timer_func;
			break;
		}
	}

	/* refresh all CCB users' address, except the first one, because user's size has been re-calculated above */
	if (SMEM_USER_CCB_END - SMEM_USER_CCB_START >= 1)
		for (i = SMEM_USER_CCB_START + 1; i <= SMEM_USER_CCB_END; i++) {
			curr = ccci_md_get_smem_by_user_id(md_id, i);
			prev = ccci_md_get_smem_by_user_id(md_id, i - 1);
			if (curr && prev) {
				curr->base_ap_view_phy = prev->base_ap_view_phy + prev->size;
...
void __iomem *get_smem_start_addr(int md_id, enum SMEM_USER_ID user_id, int *size_o)
{
	...
	if (smem_region) {
		...
		/* dbm addr returned to user should step over Guard pattern header */
...
long port_ccb_ioctl(struct port_t *port, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_CCB_CTRL_LEN:
		ret = put_user((unsigned int)ccb_ctl->size, (unsigned int __user *)arg);
		break;
	case CCCI_IOC_CCB_CTRL_INFO:
		if (copy_from_user(&ctrl_info, (void __user *)arg, sizeof(struct ccb_ctrl_info))) {
			CCCI_ERROR_LOG(md_id, TAG, "get ccb ctrl fail: copy_from_user fail!\n");
			ret = -EINVAL;
			break;
		}
		/*user id counts from ccb start*/
		if (ctrl_info.user_id + SMEM_USER_CCB_START > SMEM_USER_CCB_END) {
			CCCI_ERROR_LOG(md_id, TAG, "get ccb ctrl fail: user_id = %d!\n", ctrl_info.user_id);
			ret = -EINVAL;
			break;
		}
		/*get ctrl info by user id*/
		s_port = find_smem_port_by_user_id(md_id, ctrl_info.user_id + SMEM_USER_CCB_START);
		if (!s_port) {
			CCCI_ERROR_LOG(md_id, TAG, "get ccb port fail: user_id = %d!\n", ctrl_info.user_id);
			ret = -EINVAL;
			break;
		}
		...
		if (copy_to_user((void __user *)arg, &ctrl_info, sizeof(struct ccb_ctrl_info))) {
			CCCI_ERROR_LOG(md_id, TAG, "copy_to_user ccb ctrl failed !!\n");
			ret = -EINVAL;
			break;
		}
...
long port_smem_ioctl(struct port_t *port, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_GET_CCB_DEBUG_VAL:
		...
		if (debug_in.buffer_id == 0) {
			ptr = (char *)ccb_dhl->base_ap_view_vir + ccb_configs[0].dl_buff_size +
...
static long smem_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
...
int port_smem_init(struct port_t *port)
{
	...
	port->private_data = smem_port = kzalloc(sizeof(struct ccci_smem_port), GFP_KERNEL);
	/*user ID is from 0*/
