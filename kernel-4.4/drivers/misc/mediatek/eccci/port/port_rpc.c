	...
	case IPC_RPC_GET_RF_CLK_BUF_OP:
		{
			...
			if (count != CLKBUF_MAX_COUNT) {
				...
				CCCI_NORMAL_LOG(md_id, RPC, "RF_CLK_BUF*_DRIVING_CURR %d, %d, %d, %d, AfcDac: %d\n",
	...
	case IPC_RPC_GET_GPIO_ADC_OP:
		{
			...
			if (pkt_size == sizeof(struct ccci_rpc_gpio_adc_intput)) {
				...
				CCCI_BOOTUP_LOG(md_id, RPC, "IPC_RPC_GET_GPIO_ADC_OP request=%x\n",
								input->reqMask);
	...
	case IPC_RPC_USIM2NFC_OP:
		{
			...
			CCCI_DEBUG_LOG(md_id, RPC, "IPC_RPC_USIM2NFC_OP request=%x\n", input->lock_vsim1);
...
int port_rpc_recv_match(struct port_t *port, struct sk_buff *skb)
{
	...
	if (is_userspace_msg && (port->flags & PORT_F_WITH_CHAR_NODE)) {
		...
		if (is_userspace_msg == 0 && !(port->flags & PORT_F_WITH_CHAR_NODE)) {
