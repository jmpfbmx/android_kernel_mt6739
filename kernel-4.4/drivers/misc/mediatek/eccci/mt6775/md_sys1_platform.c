...
static void md1_pcore_sram_turn_on(struct ccci_modem *md)
{
	...
	CCCI_BOOTUP_LOG(md->index, TAG, "md1_pcore_sram reg: 0x%X, 0x%X, 0x%X, 0x%X\n",
		ccci_read32(base, 0x160), ccci_read32(base, 0x168), ccci_read32(base, 0x16C),
...
void md1_pll_init(struct ccci_modem *md)
{
	...
	cldma_write32(md_pll->md_top_Pll, 0x4, 0x02020E8D);
	...
	cldma_write32(md_pll->md_top_Pll, 0x40, 0x80213B14);
