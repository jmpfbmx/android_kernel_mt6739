...
/*4K alignment before MDM/META buffer*/
/*Each page need 32B alignment for MD MMU*/
/*UL : 1K*32 + 128   + 1K*16    +  128*10       +    128    +  128       +  128*2 + 128  */
/*DL : 1K*32 + 20K*6 + 0.5K*72  + (2*1M + 5*2M) + 0.5K*16   + */
/* 0.5K*48   +  128*2 + 128*DUMMY_PADDING_CNT  (padding for alignment here)  */
								  /*(+256K+128K*2)                                  */
/*Total : 64K  +  120K  +  52K  +  12M + 512K  +           8K +      24K + dummy(128*32= 4K)  = 12M + 784K*/

/*MDM  :  1K*16 +  65K*8  -> 536K*/
/*META :  65K*8 +  65K*8  -> 1040K*/

/*TOTAL DUMMY PAGE CNT should be multiple of 32 for 4K alignment*/
/*Update padding count when dummy page number changed*/
#define DUMMY_PAGE_SIZE (128)
#define DUMMY_PADDING_CNT (14)
...
/*remove useless AP EX page*/
/*#define AP_EX_PAGE_SIZE (1024 + MD_CACHELINE_ALIGN)*/
/*#define AP_EX_PAGE_NUM  (3)*/

/*Note : Modify this size will affect dhl frame size in this page*/
/*Minimum : 352B to reserve 256B for header frame */
#define MD_HW_PAGE_SIZE (512)

/*replace with HW page, need allocate 1 more page than MD total buffer size for PLS flush*/
/*#define MD_BUF1_PAGE_SIZE (128*1024)*/
#define MD_BUF1_PAGE_NUM  (72)
#define AP_BUF1_PAGE_SIZE (1024)
#define AP_BUF1_PAGE_NUM  (16)

/*must larger than internal RAM size*/
/*total: 1M, 2M, 256K, 2M, 2M, 128K, 128K, 2M, 2M, 1M*/
#define MD_BUF2_0_PAGE_SIZE (32*1024)
#define MD_BUF2_1_PAGE_SIZE (32*1024)
#define MD_BUF2_2_PAGE_SIZE (32*1024)
#define MD_BUF2_3_PAGE_SIZE (32*1024)
#define MD_BUF2_4_PAGE_SIZE (32*1024)
#define MD_BUF2_5_PAGE_SIZE (32*1024)
#define MD_BUF2_6_PAGE_SIZE (32*1024)
#define MD_BUF3_0_PAGE_SIZE (32*1024)
#define MD_BUF3_1_PAGE_SIZE (32*1024)
#define MD_BUF3_2_PAGE_SIZE (32*1024)

#define MD_BUF2_0_PAGE_NUM (32)
#define MD_BUF2_1_PAGE_NUM (64)
#define MD_BUF2_2_PAGE_NUM (8)
#define MD_BUF2_3_PAGE_NUM (64)
#define MD_BUF2_4_PAGE_NUM (64)
#define MD_BUF2_5_PAGE_NUM (4)
#define MD_BUF2_6_PAGE_NUM (4)
#define MD_BUF3_0_PAGE_NUM (64)
#define MD_BUF3_1_PAGE_NUM (64)
#define MD_BUF3_2_PAGE_NUM (32)

#define MD_HW_0_PAGE_NUM (16)
#define MD_HW_1_PAGE_NUM (48)
...
{SMEM_USER_CCB_DHL, P_CORE, CTRL_PAGE_SIZE, CTRL_PAGE_SIZE,
	CTRL_PAGE_SIZE*CTRL_PAGE_NUM, CTRL_PAGE_SIZE*CTRL_PAGE_NUM},               /* Ctrl */
{SMEM_USER_CCB_DHL, P_CORE, MD_EX_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_EX_PAGE_SIZE*MD_EX_PAGE_NUM, DUMMY_PAGE_SIZE},                   /* exception */
{SMEM_USER_CCB_DHL, P_CORE, MD_HW_PAGE_SIZE, AP_BUF1_PAGE_SIZE,
	(MD_HW_PAGE_SIZE*MD_BUF1_PAGE_NUM), AP_BUF1_PAGE_SIZE*AP_BUF1_PAGE_NUM},/*  PS */
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_0_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_0_PAGE_SIZE*MD_BUF2_0_PAGE_NUM, DUMMY_PAGE_SIZE},     /* L1 */
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_1_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_1_PAGE_SIZE*MD_BUF2_1_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_2_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_2_PAGE_SIZE*MD_BUF2_2_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_3_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_3_PAGE_SIZE*MD_BUF2_3_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_4_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_4_PAGE_SIZE*MD_BUF2_4_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_5_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_5_PAGE_SIZE*MD_BUF2_5_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF2_6_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF2_6_PAGE_SIZE*MD_BUF2_6_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF3_0_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF3_0_PAGE_SIZE*MD_BUF3_0_PAGE_NUM, DUMMY_PAGE_SIZE},    /* L2 */
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF3_1_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF3_1_PAGE_SIZE*MD_BUF3_1_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_BUF3_2_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_BUF3_2_PAGE_SIZE*MD_BUF3_2_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, MD_HW_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_HW_PAGE_SIZE*MD_HW_0_PAGE_NUM, DUMMY_PAGE_SIZE},                    /* HW logger  */
{SMEM_USER_CCB_DHL, P_CORE, MD_HW_PAGE_SIZE, DUMMY_PAGE_SIZE,
	MD_HW_PAGE_SIZE*MD_HW_1_PAGE_NUM, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, DUMMY_PAGE_SIZE, DUMMY_PAGE_SIZE,
	DUMMY_PAGE_SIZE, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, DUMMY_PAGE_SIZE, DUMMY_PAGE_SIZE,
	DUMMY_PAGE_SIZE, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_DHL, P_CORE, DUMMY_PAGE_SIZE, DUMMY_PAGE_SIZE,
	DUMMY_PAGE_SIZE*DUMMY_PADDING_CNT, DUMMY_PAGE_SIZE},
{SMEM_USER_CCB_MD_MONITOR, P_CORE, MD_MDM_PAGE_SIZE, AP_MDM_PAGE_SIZE,
	MD_MDM_PAGE_SIZE*MD_MDM_PAGE_NUM, AP_MDM_PAGE_SIZE*AP_MDM_PAGE_NUM},         /* MDM */
{SMEM_USER_CCB_META, P_CORE, MD_META_PAGE_SIZE, AP_META_PAGE_SIZE,
	MD_META_PAGE_SIZE*MD_META_PAGE_NUM, AP_META_PAGE_SIZE*AP_META_PAGE_NUM},   /* META */
};
