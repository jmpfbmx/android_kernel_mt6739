...
static void ccci_aed_v3(struct ccci_fsm_ee *mdee, unsigned int dump_flag, char *aed_str, int db_opt)
{
 ...
 err_exit1:
	if (dump_flag & CCCI_AED_DUMP_CCIF_REG) {
		...
		ccci_md_dump_info(mdee->md_id, DUMP_FLAG_CCIF_REG | DUMP_FLAG_CCIF,
...
static void mdee_output_debug_info_to_buf(struct ccci_fsm_ee *mdee, DEBUG_INFO_T *debug_info, char *ex_info)
{
	...
	case MD_EX_CLASS_ASSET:
		...
		CCCI_ERROR_LOG(md_id, FSM, "assert para0 = 0x%08x, para1 = 0x%08x, para2 = 0x%08x\n",
	...
	case MD_EX_CLASS_FATAL:
		...
		snprintf(ex_info, EE_BUF_LEN_UMOLY,
			 ...
			 debug_info->core_name, debug_info->dump_fatal.err_sec, debug_info->name,
		...
		CCCI_ERROR_LOG(md_id, FSM, "fatal error code 1,2,3 = [0x%08X, 0x%08X, 0x%08X]%s\n",
...
static EX_OVERVIEW_T *md_ee_get_buf_ptr(struct ccci_fsm_ee *mdee, u8 *buf_type)
{
	...
	if (((dumper->more_info == MD_EE_CASE_NORMAL) && (mdee->ee_info_flag&MD_EE_DUMP_IN_GPD))
		...
		*buf_type = MD_EE_DATA_IN_GPD;
		...
		CCCI_NORMAL_LOG(mdee->md_id, FSM, "parsing data from: GPD %p, %p\n", tar_ptr, dumper->ex_pl_info);
	} else {
...
static int mdee_set_core_name(int md_id, char *core_name, EX_OVERVIEW_T *ex_overview)
{
	...
	CCCI_NORMAL_LOG(md_id, FSM, "brief_info: core_name = %s", core_name);
	return core_id;
}
...
static void md_ee_set_fatal_para(EX_FATAL_V3_T *fatal_src, DUMP_INFO_FATAL *fata_tar)
{
	...
	if (fatal_src->error_section < ARRAY_SIZE(ee_err_sec_str))
		fata_tar->err_sec = ee_err_sec_str[fatal_src->error_section];
	else
		fata_tar->err_sec = "";
...
static void mdee_info_prepare_v3(struct ccci_fsm_ee *mdee)
{
	...
	CCCI_NORMAL_LOG(md_id, FSM, "mdee_info_prepare_v3, ee_case(0x%x)\n", dumper->more_info);
...
static void mdee_dumper_v3_dump_ee_info(struct ccci_fsm_ee *mdee, MDEE_DUMP_LEVEL level, int more_info)
{
	struct mdee_dumper_v3 *dumper = mdee->dumper_obj;
	int md_id = mdee->md_id;
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(mdee->md_id, SMEM_USER_RAW_MDCCCI_DBG);
	...
	if (level == MDEE_DUMP_LEVEL_BOOT_FAIL) {
		if (md_state == BOOT_WAITING_FOR_HS1) {
			ccci_md_dump_info(mdee->md_id, DUMP_MD_BOOTUP_STATUS, reg_value, 2);
			ccci_md_dump_info(mdee->md_id, DUMP_FLAG_CCIF, ccif_sram, 0);
			CCCI_MEM_LOG_TAG(md_id, FSM, "md_boot_stats0 /1 / bootuptrace:0x%X / 0x%X / 0x%X\n",
			...
			if ((reg_value[0] == 0) && (ccif_sram[0] == 0)) {
				...
				ccci_md_dump_info(mdee->md_id, DUMP_FLAG_REG, NULL, 0);
				msleep(10000);
				ccci_md_dump_info(mdee->md_id, DUMP_FLAG_REG, NULL, 0);
			}  else {
...
int mdee_dumper_v3_alloc(struct ccci_fsm_ee *mdee)
{
	...
	if (dumper == NULL) {
		CCCI_ERROR_LOG(md_id, FSM, "%s:alloc mdee_parser_v3 fail\n", __func__);
		return -1;
	}
