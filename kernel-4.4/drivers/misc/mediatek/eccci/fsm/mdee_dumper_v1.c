...
static void ccci_aed_v1(struct ccci_fsm_ee *mdee, unsigned int dump_flag, char *aed_str, int db_opt)
{
...
static void mdee_dumper_info_dump_v1(struct ccci_fsm_ee *mdee)
{
	...
	case MD_EX_TYPE_ASSERT:
		...
		CCCI_NORMAL_LOG(md_id, FSM, "para0 = %d, para1 = %d, para2 = %d\n",
	...
	case MD_EX_TYPE_EMI_CHECK:
		CCCI_NORMAL_LOG(md_id, FSM, "md_emi_check: %08X, %08X, %02d, %08X\n",
	...
	case DSP_EX_TYPE_EXCEPTION:
		CCCI_NORMAL_LOG(md_id, FSM, "exec unit = %s, code1:0x%08x\n",
	...
	case DSP_EX_FATAL_ERROR:
		...
		CCCI_NORMAL_LOG(md_id, FSM, "err_code0 = 0x%08x, err_code1 = 0x%08x\n",
...
static void mdee_dumper_info_prepare_v1(struct ccci_fsm_ee *mdee)
{
	...
	if ((md_id == MD_SYS3) ||
	(dumper->more_info == MD_EE_CASE_NORMAL && ccci_fsm_get_md_state(mdee->md_id) == BOOT_WAITING_FOR_HS1)) {
		ex_info = &dumper->ex_info;
		CCCI_DEBUG_LOG(md_id, FSM, "Parse ex info from ccci packages\n");
	} else {
		ex_info = (EX_LOG_T *)mdss_dbg->base_ap_view_vir;
		CCCI_DEBUG_LOG(md_id, FSM, "Parse ex info from shared memory\n");
	}
...
static void mdee_dumper_v1_set_ee_pkg(struct ccci_fsm_ee *mdee, char *data, int len)
{
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
...
static void mdee_dumper_v1_dump_ee_info(struct ccci_fsm_ee *mdee, MDEE_DUMP_LEVEL level, int more_info)
{
	struct mdee_dumper_v1 *dumper = mdee->dumper_obj;
	int md_id = mdee->md_id;
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(mdee->md_id, SMEM_USER_RAW_MDCCCI_DBG);
...
int mdee_dumper_v1_alloc(struct ccci_fsm_ee *mdee)
{
	...
	if (dumper == NULL) {
		CCCI_ERROR_LOG(md_id, FSM, "%s:alloc mdee_parser_v1 fail\n", __func__);
		return -1;
	}
