...
static int dev_char_open(struct inode *inode, struct file *file)
{
	...
	monitor_ctl = &ctl->monitor_ctl;
	CCCI_NORMAL_LOG(monitor_ctl->md_id, FSM, "monitor node open by %s\n", current->comm);
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	CCCI_NORMAL_LOG(monitor_ctl->md_id, FSM, "monitor close, clear_cnt=%d\n", clear_cnt);
	return 0;
}
...
static ssize_t dev_char_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	...
	if (skb) {
		read_len = skb->len;
		if (read_len > count || copy_to_user(buf, skb->data, read_len)) {
			CCCI_ERROR_LOG(monitor_ctl->md_id, FSM, "read on monitor, copy to user failed, %d/%zu\n",
				     read_len, count);
			ret = -EFAULT;
		}
		ccci_free_skb(skb);
...
int fsm_monitor_send_message(int md_id, CCCI_MD_MSG msg, u32 resv)
{
	...
	if (unlikely(in_interrupt())) {
		CCCI_ERROR_LOG(monitor_ctl->md_id, FSM, "sending virtual msg from IRQ context %ps\n",
...
{
	struct ccci_fsm_ctl *ctl = container_of(monitor_ctl, struct ccci_fsm_ctl, monitor_ctl);
	int ret = 0;
	...
	if (unlikely(!monitor_ctl->char_dev)) {
		CCCI_ERROR_LOG(monitor_ctl->md_id, FSM, "alloc fsm monitor char dev fail!!\n");
		return -1;
	}
	cdev_init(monitor_ctl->char_dev, &char_dev_fops);
	monitor_ctl->char_dev->owner = THIS_MODULE;
	ret = alloc_chrdev_region(&monitor_ctl->dev_n, monitor_ctl->md_id, 1, FSM_NAME);
	if (ret)
		CCCI_ERROR_LOG(monitor_ctl->md_id, FSM, "alloc_chrdev_region fail, ret=%d\n", ret);
	ret = cdev_add(monitor_ctl->char_dev, monitor_ctl->dev_n, 1);
	if (ret)
		CCCI_ERROR_LOG(monitor_ctl->md_id, FSM, "cdev_add fail, ret=%d\n", ret);

	ret = ccci_register_dev_node(fsm_monitor_name[monitor_ctl->md_id],
			MAJOR(monitor_ctl->dev_n), MINOR(monitor_ctl->dev_n));
	if (ret)
		CCCI_ERROR_LOG(monitor_ctl->md_id, FSM, "device_create fail, ret=%d\n", ret);
	return ret;
}
