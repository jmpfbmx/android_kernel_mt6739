...
void fsm_md_bootup_timeout_handler(struct ccci_fsm_ee *ee_ctl)
{
	...
	ccci_md_dump_info(ee_ctl->md_id, (DUMP_FLAG_QUEUE_0_1 | DUMP_MD_BOOTUP_STATUS | DUMP_FLAG_REG |
		DUMP_FLAG_CCIF_REG), NULL, 0);
...
void fsm_md_exception_stage(struct ccci_fsm_ee *ee_ctl, int stage)
{
	...
	if (stage == 0) { /* CCIF handshake just came in */
		...
		struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(ee_ctl->md_id, SMEM_USER_RAW_MDSS_DBG);

		CCCI_ERROR_LOG(md_id, FSM, "MD exception stage 1!\n");
#if defined(CONFIG_MTK_AEE_FEATURE)
		tracing_off();
#endif
		CCCI_MEM_LOG_TAG(md_id, FSM, "MD exception stage 1! ee=%x\n", ee_ctl->ee_info_flag);
		spin_lock_irqsave(&ee_ctl->ctrl_lock, flags);
		...
		if ((ee_info_flag & (MD_EE_SWINT_GET | MD_EE_MSG_GET | MD_EE_OK_MSG_GET)) ==
			...
			CCCI_ERROR_LOG(md_id, FSM, "Invalid MD_EX, ee_info=%x\n", ee_info_flag);
			goto _dump_done;
		}
		...
		if (ee_ctl->ops->dump_ee_info)
			ee_ctl->ops->dump_ee_info(ee_ctl, MDEE_DUMP_LEVEL_STAGE1, ee_case);
		...
		if (ee_case == MD_EE_CASE_NO_RESPONSE)
			ccci_md_dump_info(md_id, DUMP_FLAG_CCIF | DUMP_FLAG_CCIF_REG,
		...
		ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP, (void *)mem_layout->md_bank0.base_ap_view_vir,
					MD_IMG_DUMP_SIZE);
		...
		/* Dump CCB memory */
		ccci_md_dump_info(md_id, DUMP_FLAG_SMEM_CCB_CTRL | DUMP_FLAG_SMEM_CCB_DATA, NULL, 0);
...
_dump_done:
		...
		if (md_id == MD_SYS1 || ee_ctl->ee_case == MD_EE_CASE_NO_RESPONSE)
			md_dump_flag = DUMP_FLAG_REG;
		...
		/*parse & dump md ee info*/
		if (ee_ctl->ops->dump_ee_info)
			ee_ctl->ops->dump_ee_info(ee_ctl, MDEE_DUMP_LEVEL_STAGE2, ee_ctl->ee_case);

		/* Dump another modem if necessary*/
		if (ccci_fsm_get_md_state(GET_OTHER_MD_ID(md_id)) == BOOT_WAITING_FOR_HS2)
			ccci_md_dump_info(GET_OTHER_MD_ID(md_id), DUMP_FLAG_CCIF, NULL, 0);

		spin_lock_irqsave(&ee_ctl->ctrl_lock, flags);
		/* this flag should be the last action of a regular exception flow, clear flag for reset MD later */
		ee_ctl->ee_info_flag = 0;
		spin_unlock_irqrestore(&ee_ctl->ctrl_lock, flags);

		if (md_wdt_ee && md_id == MD_SYS3) {
			CCCI_ERROR_LOG(md_id, FSM, "trigger force assert after WDT EE\n");
			ccci_md_force_assert(md_id, MD_FORCE_ASSERT_BY_MD_WDT, NULL, 0);
		}
		CCCI_ERROR_LOG(md_id, FSM, "MD exception stage 2:end\n");
	}
}
...
void fsm_ee_message_handler(struct ccci_fsm_ee *ee_ctl, struct sk_buff *skb)
{
	...
	if (ccci_h->data[1] == MD_EX) {
		if (unlikely(ccci_h->reserved != MD_EX_CHK_ID)) {
			CCCI_ERROR_LOG(ee_ctl->md_id, FSM, "receive invalid MD_EX %x\n", ccci_h->reserved);
		} else {
			spin_lock_irqsave(&ee_ctl->ctrl_lock, flags);
			ee_ctl->ee_info_flag |= (MD_EE_FLOW_START | MD_EE_MSG_GET);
			spin_unlock_irqrestore(&ee_ctl->ctrl_lock, flags);
			ccci_port_send_msg_to_md(ee_ctl->md_id, CCCI_CONTROL_TX, MD_EX, MD_EX_CHK_ID, 1);
		...
		if (unlikely(ccci_h->reserved != MD_EX_REC_OK_CHK_ID)) {
			CCCI_ERROR_LOG(ee_ctl->md_id, FSM, "receive invalid MD_EX_REC_OK %x\n", ccci_h->reserved);
		} else {
			spin_lock_irqsave(&ee_ctl->ctrl_lock, flags);
			...
			if (ee_ctl->ops->set_ee_pkg)
				ee_ctl->ops->set_ee_pkg(ee_ctl, skb_pull(skb, sizeof(struct ccci_header)),
		...
		CCCI_ERROR_LOG(ee_ctl->md_id, FSM, "AP/MD driver version mis-match\n");
#ifdef CONFIG_MTK_AEE_FEATURE
		aed_md_exception_api(NULL, 0, NULL, 0, "AP/MD driver version mis-match\n", DB_OPT_DEFAULT);
#endif
	}
}
...
int fsm_check_ee_done(struct ccci_fsm_ee *ee_ctl, int timeout)
{
	...
	while (ccci_fsm_get_md_state(ee_ctl->md_id) == EXCEPTION) {
		if (ccci_port_get_critical_user(ee_ctl->md_id, CRIT_USR_MDLOG)) {
			CCCI_DEBUG_LOG(ee_ctl->md_id, FSM, "MD logger is running, waiting for EE dump done\n");
			is_ee_done = !(ee_ctl->ee_info_flag & MD_EE_FLOW_START) && ee_ctl->mdlog_dump_done;
		} else
		...
		if (loop_max && (count > loop_max)) {
			CCCI_ERROR_LOG(ee_ctl->md_id, FSM, "wait EE done timeout\n");
			return -1;
		}
	}
