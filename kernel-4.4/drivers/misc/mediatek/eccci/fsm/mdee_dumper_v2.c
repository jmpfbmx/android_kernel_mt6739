...
static void ccci_aed_v2(struct ccci_fsm_ee *mdee, unsigned int dump_flag, char *aed_str, int db_opt)
{
	...
	struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(mdee->md_id, SMEM_USER_RAW_MDSS_DBG);
 ...
 err_exit1:
	if (dump_flag & CCCI_AED_DUMP_CCIF_REG) {
		...
		ccci_md_dump_info(mdee->md_id, DUMP_FLAG_CCIF | DUMP_FLAG_CCIF_REG,
...
static void mdee_output_debug_info_to_buf(struct ccci_fsm_ee *mdee, DEBUG_INFO_T *debug_info, char *ex_info)
{
	int md_id = mdee->md_id;
	...
	case MD_EX_DUMP_ASSERT:
		...
		CCCI_ERROR_LOG(md_id, FSM, "para0 = %d, para1 = %d, para2 = %d\n",
	...
	case MD_EX_CC_C2K_EXCEPTION:
		...
		if (debug_info->fatal_error.err_code1 == 0x3104) {
			...
			snprintf(ex_info, EE_BUF_LEN_UMOLY, "%s%s, MD base = 0x%08X\n\n", ex_info,
				mdee->ex_mpu_string, (unsigned int)mem_layout->md_bank0.base_ap_view_phy);
	...
	case MD_EX_DUMP_EMI_CHECK:
		CCCI_ERROR_LOG(md_id, FSM, "md_emi_check: 0x%08X, 0x%08X, %02d, 0x%08X\n\n",
...
static void mdee_info_dump_v2(struct ccci_fsm_ee *mdee)
{
	...
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(mdee->md_id, SMEM_USER_RAW_MDCCCI_DBG);
	struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(mdee->md_id, SMEM_USER_RAW_MDSS_DBG);
	struct rtc_time tm;
	struct timeval tv = { 0 };
	...
	if (ex_info_temp == NULL) {
		CCCI_ERROR_LOG(md_id, FSM, "Fail alloc Mem for ex_info_temp!\n");
		goto err_exit;
	}
	...
	CCCI_ERROR_LOG(md_id, FSM, "Sync:%d%02d%02d %02d:%02d:%02d.%u(%02d:%02d:%02d.%03d(TZone))\n",
	...
	if (dumper->ex_core_num > 1) {
		CCCI_NORMAL_LOG(md_id, FSM, "%s+++++++%s", ex_info_temp, ex_info);
		snprintf(ex_info_temp, EE_BUF_LEN_UMOLY, "%smd%d:%s", ex_info_temp, md_id + 1, ex_info);
	...
	if (dumper->more_info == MD_EE_CASE_NORMAL && md_state == BOOT_WAITING_FOR_HS1) {
		...
		CCCI_MEM_LOG_TAG(md_id, FSM, "Dump MD exp smem_mdccci_debug_log\n");
		ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP, mdccci_dbg->base_ap_view_vir, mdccci_dbg->size);
		CCCI_MEM_LOG_TAG(md_id, FSM, "Dump MD exp smem_mdss_debug_log\n");
		ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP, mdss_dbg->base_ap_view_vir, 512);
		ccci_util_mem_dump(md_id, CCCI_DUMP_MEM_DUMP, (mdss_dbg->base_ap_view_vir + 6 * 1024), 2048);
		/*
...
err_exit:
	...
	if (debug_info && debug_info->type == MD_EX_TYPE_C2K_ERROR)
		...
		CCCI_NORMAL_LOG(md_id, FSM, "Not MD EMI violation, No need trigger DB\n");
...
static int mdee_md32_core_parse(int md_id, DEBUG_INFO_T *debug_info, EX_MD32_LOG_T *ex_md32LogInfo)
{
	...
	CCCI_ERROR_LOG(md_id, FSM, "md32 ex type(0x%x), name: %s\n", ee_type,
...
static void mdee_info_prepare_v2(struct ccci_fsm_ee *mdee)
{
	...
	CCCI_NORMAL_LOG(md_id, FSM, "ccci_md_exp_change, ee_case(0x%x)\n", dumper->more_info);

	if ((dumper->more_info == MD_EE_CASE_NORMAL) && (ccci_fsm_get_md_state(mdee->md_id) == BOOT_WAITING_FOR_HS1)) {
		debug_info = &dumper->debug_info[0];
	...
	ex_overview = (EX_OVERVIEW_T *) mdss_dbg->base_ap_view_vir;
	for (core_id = 0; core_id < MD_CORE_NUM; core_id++) {
		CCCI_DEBUG_LOG(md_id, FSM, "core_id(%x/%x): offset=%x, if_offender=%d, %s\n", (core_id + 1),
		...
		CCCI_NORMAL_LOG(md_id, FSM, "core_id(0x%x/%d), %s\n", core_id, off_core_num,
		     debug_info->core_name);
		...
		if (off_core_num == 1) {
			mdee->ex_type = ee_case;
			CCCI_ERROR_LOG(md_id, FSM, "set ee_type=%d\n", mdee->ex_type);
		}
	}
...
static void mdee_dumper_v2_set_ee_pkg(struct ccci_fsm_ee *mdee, char *data, int len)
{
	struct mdee_dumper_v2 *dumper = mdee->dumper_obj;
...
static void mdee_dumper_v2_dump_ee_info(struct ccci_fsm_ee *mdee, MDEE_DUMP_LEVEL level, int more_info)
{
	struct mdee_dumper_v2 *dumper = mdee->dumper_obj;
	int md_id = mdee->md_id;
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(mdee->md_id, SMEM_USER_RAW_MDCCCI_DBG);
...
int mdee_dumper_v2_alloc(struct ccci_fsm_ee *mdee)
{
	...
	if (dumper == NULL) {
		CCCI_ERROR_LOG(md_id, FSM, "%s:alloc mdee_parser_v2 fail\n", __func__);
		return -1;
	}
