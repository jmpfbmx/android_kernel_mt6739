...
{
	struct ccci_per_md *per_md_data = ccci_get_per_md_data(poller_ctl->md_id);
	...
	rem_nsec0 = (poller_ctl->latest_poll_start_time == 0 ?
			0 : do_div(poller_ctl->latest_poll_start_time, 1000000000));
	rem_nsec1 = (latest_isr_time == 0 ? 0 : do_div(latest_isr_time, 1000000000));
	rem_nsec2 = (latest_q0_isr_time == 0 ? 0 : do_div(latest_q0_isr_time, 1000000000));
	...
	CCCI_ERROR_LOG(md_id, FSM,
		"polling: start=%lu.%06lu, isr=%lu.%06lu,q0_isr=%lu.%06lu, q0_rx=%lu.%06lu\n",
			(unsigned long)poller_ctl->latest_poll_start_time, rem_nsec0 / 1000,
	...
	if (latest_q0_isr_time > latest_q0_rx_time) {
		CCCI_ERROR_LOG(md_id, FSM, "AP rx polling queue no work after isr, rx queue maybe blocked\n");
...
static int fsm_poll_main(void *data)
{
	...
	while (1) {
		...
		if (md_state != READY || ccci_port_get_critical_user(poller_ctl->md_id, CRIT_USR_MDLOG) != 1)
			goto next;
		...
		if (!ret) { /* timeout */
			...
			if (md_state == READY) {
				...
				if (assert_md_type == MD_FORCE_ASSERT_BY_MD_NO_RESPONSE)
					ccci_md_dump_info(poller_ctl->md_id, DUMP_FLAG_IRQ_STATUS, NULL, 0);
				ccci_md_dump_info(poller_ctl->md_id, DUMP_FLAG_QUEUE_0, NULL, 0);
				ccci_md_force_assert(poller_ctl->md_id, assert_md_type, NULL, 0);

				count = 0;
				while (count < FORCE_ASSERT_TIMEOUT / 200) {
					if (ccci_fsm_get_md_state(poller_ctl->md_id) == EXCEPTION) {
						count = 0;
						break;
					}
					count++;
				...
				if (count) {
					CCCI_ERROR_LOG(poller_ctl->md_id, FSM, "MD long time no response\n");
					ccci_md_dump_info(poller_ctl->md_id, DUMP_FLAG_QUEUE_0, NULL, 0);
					fsm_append_command(ctl, CCCI_COMMAND_MD_HANG, 0);
				}
			}
		}
next:
...
int fsm_poller_init(struct ccci_fsm_poller *poller_ctl)
{
	...
	poller_ctl->poll_thread = kthread_run(fsm_poll_main, poller_ctl, "ccci_poll%d", poller_ctl->md_id + 1);
	return 0;
}
...
int ccci_fsm_recv_status_packet(int md_id, struct sk_buff *skb)
{
	...
	CCCI_NORMAL_LOG(poller_ctl->md_id, FSM, "received MD status response %x\n", *(((u32 *)skb->data) + 2));
	/*ccci_util_cmpt_mem_dump(poller_ctl->md_id, CCCI_DUMP_REPEAT, skb->data, skb->len);*/
