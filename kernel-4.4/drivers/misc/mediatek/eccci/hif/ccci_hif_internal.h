...
struct ccci_hif_ops {
	/* must-have */
	int (*send_skb)(unsigned char hif_id, int qno, struct sk_buff *skb, int skb_from_pool, int blocking);
...
void ccci_md_dump_log_history(unsigned char md_id, struct ccci_hif_traffic *tinfo, int dump_multi_rec,
		int tx_queue_num, int rx_queue_num);
void ccci_md_add_log_history(struct ccci_hif_traffic *tinfo, DIRECTION dir, int queue_index,
		struct ccci_header *msg, int is_droped);

static inline void *ccci_hif_get_by_id(unsigned char hif_id)
{
	if (hif_id >= CCCI_HIF_NUM) {
		CCCI_ERROR_LOG(-1, CORE, "ccci_hif_get_by_id  hif_id = %u\n", hif_id);
		return NULL;
	} else
		return ccci_hif[hif_id];
}

static inline void ccci_hif_queue_status_notify(int md_id, int hif_id, int qno, int dir, int state)
{
...
static inline void ccci_reset_seq_num(struct ccci_hif_traffic *traffic_info)
{
	...
	memset(traffic_info->seq_nums[IN], -1, sizeof(traffic_info->seq_nums[IN]));
}
...
{
	if (ccci_h->channel >= ARRAY_SIZE(traffic_info->seq_nums[OUT]) || ccci_h->channel < 0) {
		CCCI_NORMAL_LOG(md_id, CORE, "ignore seq inc on channel %x\n", *(((u32 *) ccci_h) + 2));
...
static inline void ccci_md_check_rx_seq_num(unsigned char md_id,
		struct ccci_hif_traffic *traffic_info, struct ccci_header *ccci_h, int qno)
{
...
static inline void ccci_channel_update_packet_counter(unsigned long *logic_ch_pkt_cnt, struct ccci_header *ccci_h)
{
	if ((ccci_h->channel & 0xFF) < CCCI_MAX_CH_NUM)
		logic_ch_pkt_cnt[ccci_h->channel]++;
}

static inline void ccci_channel_dump_packet_counter(unsigned char md_id, struct ccci_hif_traffic *traffic_info)
{
	CCCI_REPEAT_LOG(md_id, CORE, "traffic(ch): tx:[%d]%ld, [%d]%ld, [%d]%ld rx:[%d]%ld, [%d]%ld, [%d]%ld\n",
...
static inline unsigned int ccci_md_get_seq_num(struct ccci_hif_traffic *traffic_info, DIRECTION dir, CCCI_CH ch)
{
