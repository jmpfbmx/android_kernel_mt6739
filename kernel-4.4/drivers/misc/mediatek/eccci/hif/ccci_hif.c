...
static inline int ccci_hif_napi_poll(unsigned char md_id, int rx_qno, struct napi_struct *napi, int weight)
{
	return 0;
}
	...
	int queue_index, struct ccci_header *msg, int is_droped)
{
	...
	if (dir == OUT) {
		memcpy(&tinfo->tx_history[queue_index][tinfo->tx_history_ptr[queue_index]].msg, msg,
		       sizeof(struct ccci_header));
		tinfo->tx_history[queue_index][tinfo->tx_history_ptr[queue_index]].tv = local_clock();
		tinfo->tx_history[queue_index][tinfo->tx_history_ptr[queue_index]].droped = is_droped;
		tinfo->tx_history_ptr[queue_index]++;
		tinfo->tx_history_ptr[queue_index] &= (PACKET_HISTORY_DEPTH - 1);
	}
	if (dir == IN) {
		memcpy(&tinfo->rx_history[queue_index][tinfo->rx_history_ptr[queue_index]].msg, msg,
		       sizeof(struct ccci_header));
		tinfo->rx_history[queue_index][tinfo->rx_history_ptr[queue_index]].tv = local_clock();
		tinfo->rx_history[queue_index][tinfo->rx_history_ptr[queue_index]].droped = is_droped;
		tinfo->rx_history_ptr[queue_index]++;
		tinfo->rx_history_ptr[queue_index] &= (PACKET_HISTORY_DEPTH - 1);
	}
#endif
}

void ccci_md_dump_log_history(unsigned char md_id, struct ccci_hif_traffic *tinfo,
	int dump_multi_rec, int tx_queue_num, int rx_queue_num)
{
	...
	if (dump_multi_rec) {
		for (i = 0; i < ((tx_queue_num <= MAX_TXQ_NUM) ? tx_queue_num : MAX_TXQ_NUM); i++) {
			CCCI_MEM_LOG_TAG(md_id, CORE, "dump txq%d packet history, ptr=%d\n", i,
			       tinfo->tx_history_ptr[i]);
			for (j = 0; j < PACKET_HISTORY_DEPTH; j++)
				ccci_md_dump_log_rec(md_id, &tinfo->tx_history[i][j]);
		}
		for (i = 0; i < ((rx_queue_num <= MAX_RXQ_NUM) ? rx_queue_num : MAX_RXQ_NUM); i++) {
			CCCI_MEM_LOG_TAG(md_id, CORE, "dump rxq%d packet history, ptr=%d\n", i,
			       tinfo->rx_history_ptr[i]);
			for (j = 0; j < PACKET_HISTORY_DEPTH; j++)
				ccci_md_dump_log_rec(md_id, &tinfo->rx_history[i][j]);
		}
	} else {
		CCCI_MEM_LOG_TAG(md_id, CORE, "dump txq%d packet history, ptr=%d\n", tx_queue_num,
		       tinfo->tx_history_ptr[tx_queue_num]);
		for (j = 0; j < PACKET_HISTORY_DEPTH; j++)
			ccci_md_dump_log_rec(md_id, &tinfo->tx_history[tx_queue_num][j]);
		CCCI_MEM_LOG_TAG(md_id, CORE, "dump rxq%d packet history, ptr=%d\n", rx_queue_num,
		       tinfo->rx_history_ptr[rx_queue_num]);
		for (j = 0; j < PACKET_HISTORY_DEPTH; j++)
			ccci_md_dump_log_rec(md_id, &tinfo->rx_history[rx_queue_num][j]);
	}
#endif
}
