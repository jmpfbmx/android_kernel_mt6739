...
static void md_ccif_sram_rx_work(struct work_struct *work)
{
	...
	*(((u32 *) ccci_h) + 2) = ccif_read32((((u32 *) dl_pkg) + 2), 0);
	...
	if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
		...
		CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "ccci_hif_ccif CCIF_MD wakeup source:(SRX_IDX/%d)(%u)\n",
...
static int ccif_rx_collect(struct md_ccif_queue *queue, int budget, int blocking, int *result)
{
	...
	while (1) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			md_ctrl->wakeup_count++;
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "ccci_hif_ccif CCIF_MD wakeup source:(%d/%d/%x)(%u)\n",
...
static void md_ccif_launch_work(struct md_ccif_ctrl *md_ctrl)
{
	...
	if (md_ctrl->channel_id & (1 << AP_MD_CCB_WAKEUP)) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			md_ctrl->wakeup_count++;
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "ccci_hif_ccif CCIF_MD wakeup source:(CCB)(%u)\n",
				md_ctrl->wakeup_count);
		}
		// End modified by zhuo.peng for task 5490699 on 2017/11/13
