...
#endif
#endif
		// Begin modified by zhuo.peng for task 5490699 on 2017/11/13
		/* check wakeup source */
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			md_ctrl->wakeup_count++;
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "ccci_hif_cldma1 CLDMA_MD wakeup source:(%d/%d/%x)(%u)\n",
...
static int cldma_gpd_bd_tx_collect(struct md_cd_queue *queue, int budget, int blocking)
{
	...
	while (1) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			md_ctrl->wakeup_count++;
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "ccci_hif_cldma2 CLDMA_AP wakeup source:(%d/%d)(%u)\n",
							queue->index, ccci_h->channel, md_ctrl->wakeup_count);
		}
		// End modified by zhuo.peng for task 5490699 on 2017/11/13
...
static int cldma_gpd_tx_collect(struct md_cd_queue *queue, int budget, int blocking)
{
	...
	while (1) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			md_ctrl->wakeup_count++;
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "ccci_hif_cldma3 CLDMA_AP wakeup source:(%d/%d)(%u)\n",
							queue->index, ccci_h->channel, md_ctrl->wakeup_count);
		}
		// End modified by zhuo.peng for task 5490699 on 2017/11/13
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "harvest Tx msg (%x %x %x %x) txq=%d len=%d\n",
