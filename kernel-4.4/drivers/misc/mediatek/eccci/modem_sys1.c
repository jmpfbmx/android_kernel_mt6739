...
static irqreturn_t md_cd_wdt_isr(int irq, void *data)
{
	...
	cldma_write32(md->l1_rgu_base, REG_L1RSTCTL_WDT_MODE, L1_WDT_MD_MODE_KEY);
...
static int md_cd_ccif_send(struct ccci_modem *md, int channel_id)
{
	...
	if (busy & (1 << channel_id))
		return -1;
	cldma_write32(md_info->ap_ccif_base, APCCIF_BUSY, 1 << channel_id);
	cldma_write32(md_info->ap_ccif_base, APCCIF_TCHNUM, channel_id);
	return 0;
}
...
{
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_MDCCCI_DBG);
...
static irqreturn_t md_cd_ccif_isr(int irq, void *data)
{
	...
	if (md_info->channel_id & (1 << AP_MD_CCB_WAKEUP)) {
		struct ccci_smem_region *ccb_ctl = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_CCB_CTRL);
		unsigned int *debug_addr = (unsigned int *)ccb_ctl->base_ap_view_vir;
		...
		ccci_port_queue_status_notify(md->index, CCIF_HIF_ID, AP_MD_CCB_WAKEUP, -1, RX_IRQ);
	}
#endif
	if (md_info->channel_id & (1<<AP_MD_PEER_WAKEUP))
		wake_lock_timeout(&md_info->peer_wake_lock, HZ);
...
static inline int md_sys1_sw_init(struct ccci_modem *md)
{
	...
	CCCI_BOOTUP_LOG(md->index, TAG, "md_sys1_sw_init, MD_WDT IRQ(%d) CCIF_IRQ %d\n",
						md->md_wdt_irq_id, md_info->ap_ccif_irq_id);
	ret = request_irq(md->md_wdt_irq_id, md_cd_wdt_isr, md->md_wdt_irq_flags, "MD_WDT", md);
	if (ret) {
	...
	ret = request_irq(md_info->ap_ccif_irq_id, md_cd_ccif_isr, md_info->ap_ccif_irq_flags, "CCIF0_AP", md);
	if (ret) {
...
static int md_cd_start(struct ccci_modem *md)
{
	...
	ret = ccci_get_md_check_hdr_inf(md->index, &md->per_md_data.img_info[IMG_MD], md->per_md_data.img_post_fix);
	if (ret < 0) {
	...
	ccci_reset_ccif_hw(md->index, AP_MD1_CCIF, md_info->ap_ccif_base, md_info->md_ccif_base);
...
{
	int smem_val;
	struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_MDSS_DBG);

	if (md->index != MD_SYS1)
		return 1;

	smem_val = *((int *)((long)mdss_dbg->base_ap_view_vir + CCCI_EE_OFFSET_EPOF_MD1));
...
static void md_cd_smem_sub_region_init(struct ccci_modem *md)
{
	...
	int i;
	struct ccci_smem_region *dbm = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_DBM);
...
static void config_ap_runtime_data_v2(struct ccci_modem *md, struct ap_query_md_feature *ap_feature)
{
	struct ccci_smem_region *runtime_data = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_RUNTIME_DATA);
	...
	ap_feature->set_md_mpu_start_addr = md->mem_layout.md_bank4_noncacheable_total.base_md_view_phy;
	ap_feature->set_md_mpu_total_size = md->mem_layout.md_bank4_noncacheable_total.size;
...
static void config_ap_runtime_data_v2_1(struct ccci_modem *md, struct ap_query_md_feature_v2_1 *ap_feature)
{
	...
	ap_feature->noncached_mpu_start_addr = md->mem_layout.md_bank4_noncacheable_total.base_md_view_phy;
	ap_feature->noncached_mpu_total_size = md->mem_layout.md_bank4_noncacheable_total.size;
	ap_feature->cached_mpu_start_addr = md->mem_layout.md_bank4_cacheable_total.base_md_view_phy;
	ap_feature->cached_mpu_total_size = md->mem_layout.md_bank4_cacheable_total.size;
	...
	unsigned int txqno, int skb_from_pool)
{
	...
	if (md->runtime_version < AP_MD_HS_V2) {
		CCCI_ERROR_LOG(md->index, TAG, "unsupported runtime version %d\n", md->runtime_version);
	...
	ret = ccci_hif_send_skb(CLDMA_HIF_ID, txqno, skb, skb_from_pool, 1);
#else
...
static void md_cd_dump_ccif_reg(struct ccci_modem *md)
{
	...
	for (idx = 0; idx < md->hw_info->sram_size / sizeof(u32); idx += 4) {
		CCCI_MEM_LOG_TAG(md->index, TAG,
				 ...
				 cldma_read32(md_info->ap_ccif_base + APCCIF_CHDATA,
					     (idx + 2) * sizeof(u32)),
				 cldma_read32(md_info->ap_ccif_base + APCCIF_CHDATA, (idx + 3) * sizeof(u32)));
	}
}
...
static int md_cd_dump_info(struct ccci_modem *md, MODEM_DUMP_FLAG flag, void *buff, int length)
{
	...
	if (flag & DUMP_FLAG_CCIF) {
		...
		for (i = 0; i < length / sizeof(unsigned int); i++) {
			*(dest_buff + i) = cldma_read32(md_info->ap_ccif_base,
	...
	if (flag & DUMP_FLAG_SMEM_EXP) {
		...
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, mdccci_dbg->base_ap_view_vir, mdccci_dbg->size);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, mdss_dbg->base_ap_view_vir, mdss_dbg->size);
	}
	if (flag & DUMP_FLAG_SMEM_CCISM) {
		struct ccci_smem_region *scp = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_CCISM_SCP);

		CCCI_MEM_LOG_TAG(md->index, TAG, "Dump CCISM share memory\n");
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, scp->base_ap_view_vir, scp->size);
	}
	if (flag & DUMP_FLAG_SMEM_CCB_CTRL) {
		...
		if (ccb_ctl) {
			...
			ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, ccb_ctl->base_ap_view_vir,
	...
	if (flag & DUMP_FLAG_SMEM_CCB_DATA) {
		...
		unsigned int *curr_p;
		struct ccci_smem_region *ccb_data = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_CCB_START);
		struct ccci_smem_region *dhl_raw = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_DHL);
	...
	if (flag & DUMP_FLAG_SMEM_MDSLP) {
		struct ccci_smem_region *low_pwr = ccci_md_get_smem_by_user_id(md->index, SMEM_USER_RAW_DBM);

		CCCI_MEM_LOG_TAG(md->index, TAG, "Dump MD SLP registers\n");
		ccci_cmpt_mem_dump(md->index, low_pwr->base_ap_view_vir, low_pwr->size);
	}
	if (flag & DUMP_FLAG_MD_WDT) {
		CCCI_MEM_LOG_TAG(md->index, TAG, "Dump MD RGU registers\n");
		md_cd_lock_modem_clock_src(1);
#ifdef BASE_ADDR_MDRSTCTL
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, md_info->md_rgu_base, 0x88);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, (md_info->md_rgu_base + 0x200), 0x5c);
#else
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, md_info->md_rgu_base, 0x30);
#endif
...
static ssize_t md_cd_control_store(struct ccci_modem *md, const char *buf, size_t count)
{
	...
	if (strncmp(buf, "md_dbg_dump=", size) == 0 && size < count - 1) {
		size = kstrtouint(buf+size, 16, &md->per_md_data.md_dbg_dump_flag);
		if (size)
...
static int ccci_modem_probe(struct platform_device *plat_dev)
{
	...
	snprintf(md->trm_wakelock_name, sizeof(md->trm_wakelock_name), "md%d_cldma_trm", md_id + 1);
	wake_lock_init(&md->trm_wake_lock, WAKE_LOCK_SUSPEND, md->trm_wakelock_name);
