...
int md_ccif_io_remap_md_side_register(struct ccci_modem *md)
{
	...
	case MD_SYS3:
		...
		/*CCIRQ reg */
		md_info->ccirq_base[0] = ioremap_nocache(L1_C2K_CCIRQ_BASE, 0x100);
		md_info->ccirq_base[1] = ioremap_nocache(C2K_L1_CCIRQ_BASE, 0x100);
		md_info->ccirq_base[2] = ioremap_nocache(PS_C2K_CCIRQ_BASE, 0x100);
		md_info->ccirq_base[3] = ioremap_nocache(C2K_PS_CCIRQ_BASE, 0x100);

		md_info->c2k_cgbr1_addr = ioremap_nocache(C2KSYS_BASE + C2K_CGBR1, 0x4);
		if (!md->hw_info) {
			/*todo: why it is null? */
			CCCI_ERROR_LOG(md->index, TAG, "md->hw_info is NULL!!!\n");
			md_info->md_rgu_base = ioremap_nocache(MD3_RGU_BASE, 0x40);
		} else
...
int md_ccif_let_md_go(struct ccci_modem *md)
{
	...
	case MD_SYS3:
		...
		ccif_write32(hw_info->infra_ao_base,
			     C2K_CONFIG, ccif_read32(hw_info->infra_ao_base, C2K_CONFIG) | (0x1 << 3));
		...
		ccif_write32(apmixed_base, MDPLL1_CON0, ccif_read32(apmixed_base, MDPLL1_CON0) & (~(0x1 << 9)));
		...
		/*step 4: wake up C2K */
#if 1
		ccif_write32(hw_info->toprgu_base,
			     TOP_RGU_WDT_SWSYSRST,
			     (ccif_read32
			      (hw_info->toprgu_base, TOP_RGU_WDT_SWSYSRST) | 0x88000000) & (~(0x1 << 15)));
#else
		...
		ccif_write32(hw_info->infra_ao_base,
			     ...
			     ccif_read32(hw_info->infra_ao_base, INFRA_AO_C2K_HANDSHAKE) | (0x1 << 1));
		...
		ccif_write32(hw_info->infra_ao_base,
			     ...
			     ccif_read32(hw_info->infra_ao_base, INFRA_TOPAXI_PROTECTEN_1) & (0xFE3FFFFF));
...
int md_ccif_power_on(struct ccci_modem *md)
{
	...
	if (ret == 0 && md->index != MD_SYS3) {
		/*disable MD WDT */
		ccif_write32(md_info->md_rgu_base, C2K_WDT_MD_MODE, C2K_WDT_MD_MODE_KEY);
	}
	return ret;
}
