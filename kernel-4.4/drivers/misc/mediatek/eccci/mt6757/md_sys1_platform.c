...
int md_cd_io_remap_md_side_register(struct ccci_modem *md)
{
	...
	md_info->md_bus_status = ioremap_nocache(MD_BUS_STATUS_BASE, MD_BUS_STATUS_LENGTH);
	md_info->md_topsm_status = ioremap_nocache(MD_TOPSM_STATUS_BASE, MD_TOPSM_STATUS_LENGTH);
...
void md_cd_dump_debug_register(struct ccci_modem *md)
{
	...
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_TOPSM)) {
		...
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP,
				md_info->md_topsm_status, MD_TOPSM_STATUS_LENGTH);
	}
	/* 3. PC Monitor */
	...
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_OST)) {
		...
		ccci_write32(md_reg->md_busreg1, R_DBGSYS_CLK_PMS_1, 0xE7C5); /* pre-action */
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, md_info->md_ost_status, MD_OST_STATUS_LENGTH);
	}
	/* 6. Bus status */
	if (per_md_data->md_dbg_dump_flag & (1 << MD_DBG_DUMP_BUS)) {
		...
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, md_info->md_bus_status, 0x38);
		ccci_util_mem_dump(md->index, CCCI_DUMP_MEM_DUMP, (md_info->md_bus_status + 0x100), 0x30);
...
static void md1_pcore_sram_turn_on(struct ccci_modem *md)
{
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(md->index, TAG, "MD_SRAM_PD_PSMCUSYS_SRAM_1 = 0x%X!= 0xFFFFFFFF\n", val);
		error_val = -1;
	}
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(md->index, TAG, "MD_SRAM_PD_PSMCUSYS_SRAM_2 = 0x%X!= 0xFFFFFFFF\n", val);
		error_val = -2;
	}
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(md->index, TAG, "MD_SRAM_PD_PSMCUSYS_SRAM_3 = 0x%X!= 0xFFFFFFFF\n", val);
		error_val = -3;
	}
	...
	if (val != golden_val) {
		CCCI_ERROR_LOG(md->index, TAG, "MD_SRAM_PD_PSMCUSYS_SRAM_4 = 0x%X!= 0xFFFFFFFA\n", val);
		error_val = -4;
	}

	if (error_val != 0) {
		CCCI_ERROR_LOG(md->index, TAG, "%s: error_code=%d, trigger AEE db\n", __func__, error_val);
#if defined(CONFIG_MTK_AEE_FEATURE)
...
void md1_pll_on(struct ccci_modem *md)
{
	...
	map_addr = (void __iomem *)(md->hw_info->ap_mixed_base);
...
int md_cd_power_on(struct ccci_modem *md)
{
	...
	ccci_write32(md_info->md_rgu_base, WDT_MD_MODE, WDT_MD_MODE_KEY);
	ccci_write32(md_info->l1_rgu_base, REG_L1RSTCTL_WDT_MODE, L1_WDT_MD_MODE_KEY);

#ifdef SET_EMI_STEP_BY_STAGE
	CCCI_BOOTUP_LOG(md->index, CORE, "set domain register\n");
	ccci_write32(md_info->md_pll_base->md_busreg1, 0xC4, 0x7);
	ccci_write32(md_info->md_pll_base->md_L1_a0, 0x220, 0x1);
	ccci_write32(md_info->md_pll_base->md_L1_a0, 0x220, 0x81);
#endif
...
int md_cd_let_md_go(struct ccci_modem *md)
{
	...
	ccci_write32(md_info->md_boot_slave_En, 0, 1);	/* make boot vector take effect */
	return 0;
}
...
void ccci_modem_restore_reg(struct ccci_modem *md)
{
	...
	ccci_write32(md_info->ap_ccif_base, APCCIF_CON, 0x01);	/* arbitration */
