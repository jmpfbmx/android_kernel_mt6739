...
static int mjc_open(struct inode *pInode, struct file *pFile)
{
	...
	ret = clk_prepare_enable(clk_SCP_SYS_MM0);
	if (ret) {
		/* print error log & error handling */
		MJCMSG("[ERROR] mjc_open() clk_SCP_SYS_MM0 is not enabled, ret = %d\n", ret);
	}
...
static int mjc_release(struct inode *pInode, struct file *pFile)
{
	...
	ret = clk_set_parent(clk_TOP_MUX_MJC, clk_TOP_SYSPLL_D5); /* clk_TOP_SYSPLL_D5 (218) */
	if (ret) {
		/* print error log & error handling */
...
static int mjc_probe(struct platform_device *pDev)
{
	...
	if (of_address_to_resource(node, 0, &res)) {
		MJCMSG("[ERROR] Unable to get dts resource\n");
		return -1;
	}
	...
	if (request_irq(gi4IrqID, (irq_handler_t) mjc_intr_dlr, IRQF_TRIGGER_NONE, MJC_DEVNAME, NULL)
	    < 0) {
		MJCMSG("[ERROR] mjc_probe() error to request mjc irq\n");
	} else {
	...
	if (IS_ERR(clk_MJC_METER)) {
		MJCMSG("[ERROR] Unable to devm_clk_get clk_MJC_METER\n");
		return PTR_ERR(clk_MJC_METER);
	}

	clk_SCP_SYS_MM0 = devm_clk_get(&pDev->dev, "mtcmos-mm0");
	if (IS_ERR(clk_SCP_SYS_MM0)) {
		MJCMSG("[ERROR] Unable to devm_clk_get clk_SCP_SYS_MM0\n");
		return PTR_ERR(clk_SCP_SYS_MM0);
	}
	...
	if (IS_ERR(clk_TOP_SYSPLL_D5)) {
		MJCMSG("[ERROR] Unable to devm_clk_get clk_TOP_SYSPLL_D5\n");
		return PTR_ERR(clk_TOP_SYSPLL_D5);
	}
