...
static int fstb_get_queue_fps(struct FSTB_FRAME_INFO *iter, long long interval)
{
	...
	for (j = i + 1; j < iter->queue_time_end; j++) {
		...
		if (iter->asfc_flag && (iter->queue_time_ts[j] - iter->queue_time_ts[j - 1]) < ASFC_THRESHOLD_NS)
	...
	if (iter->asfc_flag) {
		...
		fpsgo_systrace_c_fstb(iter->pid, (int)above_asfc_ths_count, "above_asfc_ths_count");
		fpsgo_systrace_c_fstb(iter->pid, queue_cnt, "queue_cnt");

		if (queue_cnt > 0) {
			do_div(result, queue_cnt);
			iter->check_asfc = result > ASFC_THRESHOLD_PERCENTAGE;
		} else if (queue_cnt == 0)
			iter->check_asfc = 0;

		fpsgo_systrace_c_fstb(iter->pid, (int)(iter->check_asfc), "check_asfc");
	}
...
static int cal_target_fps(struct FSTB_FRAME_INFO *iter)
{
	...
	if (iter->new_info == 1) {
		...
		if (iter->asfc_flag == 1 && (iter->queue_fps >= 33 || iter->check_asfc))
			iter->asfc_flag = 0;
