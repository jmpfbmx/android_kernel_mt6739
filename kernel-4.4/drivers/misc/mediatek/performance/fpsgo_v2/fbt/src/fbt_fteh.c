...
static struct fteh_loading *g_load_arr;
...
module_param(cap_th, int, S_IRUGO|S_IWUSR);
module_param(time_th, int, S_IRUGO|S_IWUSR);
module_param(loading_th, int, S_IRUGO|S_IWUSR);
module_param(sampling_period_MS, int, S_IRUGO|S_IWUSR);
module_param(start_th, int, S_IRUGO|S_IWUSR);
module_param(debounce_th, int, S_IRUGO|S_IWUSR);
module_param(enter_loading_th, int, S_IRUGO|S_IWUSR);
...
static int fteh_is_little_sleep_time(unsigned long long q2q_time, unsigned long long sleep_time)
{
...
static int fteh_update_dep_list_start(int pid)
{
	...
	if (g_load_arr_size == 0) {
		g_load_arr = (struct fteh_loading *)
				fpsgo_alloc_atomic(count * sizeof(struct fteh_loading));
		g_load_arr_size = count;
	} else if (g_load_arr_size < count) {
		fpsgo_free(g_load_arr, g_load_arr_size * sizeof(struct fteh_loading));
		g_load_arr = (struct fteh_loading *)
				fpsgo_alloc_atomic(count * sizeof(struct fteh_loading));
		g_load_arr_size = count;
	}
	...
	memset(g_load_arr, 0, g_load_arr_size * sizeof(struct fteh_loading));
	ret_size = fpsgo_fteh2xgf_get_dep_list(pid, g_load_valid_size, g_load_arr);
...
static int fteh_keep_active(int pid, unsigned long long q2q_time, unsigned long long sleep_time)
{
	if (g_load_valid_size <= 0 || !g_load_arr)
		goto debounce;
	...
	if (fteh_is_light_loading(loading_th, 0) && fteh_is_little_sleep_time(q2q_time, sleep_time)) {
		g_debounce_count = 0;
		goto keep;
	} else {
		g_debounce_count++;
...
int fpsgo_fbt2fteh_judge_ceiling(struct render_info *thread_info, unsigned int blc_wt)
{
	...
	if (thread_info->pid != g_cur_tracking_pid ||
		(g_load_ts && cur_time > g_load_ts && cur_time - g_load_ts > TIME_1S))
	...
	if (g_fteh_state == FTEH_ACTIVE)
		return fteh_keep_active(thread_info->pid, thread_info->Q2Q_time, thread_info->sleep_time);

	if (!(fteh_is_max_capacity(blc_wt) &&
		fteh_is_little_sleep_time(thread_info->Q2Q_time, thread_info->sleep_time))) {
		fteh_reset_state();
		return FTEH_CORRECT;
	}
