...
int lastpc_dump(char *buf, int len)
{
	...
	if (!plt->common) {
		pr_info("%s:%d: plt->common is NULL\n", __func__, __LINE__);
		return -ENODEV;
	}

	if (!plt->common->base) {
		pr_info("%s:%d: plt->common->base is NULL\n", __func__, __LINE__);
		return -ENODEV;
	}
	...
	pr_info("no dump function implemented\n");
	return 0;
}
...
static ssize_t lastpc_reboot_test_store(struct device_driver *driver, const char *buf, size_t count)
{
	...
	case 1:
		if (plt->ops->reboot_test(plt) != 0)
			pr_info("%s:%d: reboot test failed\n", __func__, __LINE__);
		break;
	default:
		break;
	}
...
static int __init lastpc_init(void)
{
	...
	ret = lastpc_start();
	if (ret) {
		pr_info("%s:%d: lastpc_start failed\n", __func__, __LINE__);
		return -ENODEV;
	}
	...
	if (ret) {
		pr_info("%s:%d: platform_driver_register failed\n", __func__, __LINE__);
		return -ENODEV;
	}

	ret = driver_create_file(&lastpc_drv.plt_drv.driver, &driver_attr_lastpc_dump);
	if (ret)
		pr_info("%s:%d: driver_create_file failed.\n", __func__, __LINE__);
