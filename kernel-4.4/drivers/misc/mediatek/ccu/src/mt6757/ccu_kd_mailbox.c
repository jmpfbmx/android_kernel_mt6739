...
mb_result mailbox_init(ccu_mailbox_t *apmcu_mb_addr, ccu_mailbox_t *ccu_mb_addr)
{
		...
		LOG_DBG("memclr _ccu_mailbox: %d\n", ccu_memclr(_ccu_mailbox, sizeof(ccu_mailbox_t)));
		LOG_DBG("memclr _apmcu_mailbox: %d\n", ccu_memclr(_apmcu_mailbox, sizeof(ccu_mailbox_t)));
...
static mb_result _mailbox_write_to_buffer(ccu_msg_t *task)
{
		...
		/*full when front=rear+1 (modulus add: rear+1 = rear+1 % CCU_MAILBOX_QUEUE_SIZE)*/
