...
u64  partition_type_array[PART_MAX_COUNT];
...
struct pt_resident new_part[PART_MAX_COUNT];
struct pt_resident lastest_part[PART_MAX_COUNT];
...
struct DM_PARTITION_INFO_PACKET pmtctl;
...
static long pmt_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	ret = read_pmt(uarg);
		break;
	case PMT_WRITE:
		pr_info("PMT IOCTL: PMT_WRITE (not support)\n");
		ret = -EINVAL;
#if 0
		if (copy_from_user(&pmtctl, uarg, sizeof(struct DM_PARTITION_INFO_PACKET))) {
			ret = -EFAULT;
			goto exit;
		}
...
static int read_pmt(void __user *arg)
{
	...
	if (copy_to_user(arg, &lastest_part, sizeof(struct pt_resident)*PART_MAX_COUNT))
		return -EFAULT;
	return 0;
}
...
void part_init_pmt(struct mtd_info *mtd, u8 *buf)
{
	...
	memset(&lastest_part, 0, PART_MAX_COUNT * sizeof(struct pt_resident));
...
{
	struct DM_PARTITION_INFO_PACKET *dm_part = (struct DM_PARTITION_INFO_PACKET *) buf;
...
void mtk_slc_blk_addr(u64 addr, u32 *blk_num, u32 *page_in_block)
{
	...
	u32 page_size = gn_devinfo.pagesize;
	u64 temp, temp1;

	temp = addr;
	temp1 = do_div(temp, (block_size & 0xFFFFFFFF));
	/*pr_info("%s temp 0x%llx temp1 0x%llx\n", __func__, temp, temp1);*/

	*blk_num = temp;
	*page_in_block = (u32)temp1 / page_size;
...
void mtk_slc_blk_addr(u64 addr, u32 *blk_num, u32 *page_in_block)
{
	...
	if (offset <= addr) {
		...
		*page_in_block = ((u32)temp1 % ((block_size/gn_devinfo.pagesize)/2));
		...
		pr_debug("[xiaolei] error :this is not slc mode block\n");
		WARN_ON(1);
		while (1)
			;
	}
...
int get_data_partition_info(struct nand_ftl_partition_info *info, struct mtk_nand_chip_info *cinfo)
{
	...
	for (i = 0; i < PART_MAX_COUNT; i++) {
		...
		if (!strcmp(g_exist_Partition[i].name, FTL_PARTITION_NAME)) {
			...
			temp = g_exist_Partition[i + 1].offset - g_exist_Partition[i].offset;
			pr_info("temp = %llu, gn_devinfo.blocksize = %u", temp, gn_devinfo.blocksize);
			do_div(temp, ((gn_devinfo.blocksize * 1024) & 0xFFFFFFFF));
			info->total_block = temp - 2;
			pr_info("info->total_block = %u", info->total_block);
