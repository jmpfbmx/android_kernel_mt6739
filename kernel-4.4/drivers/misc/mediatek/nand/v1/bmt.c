...
static struct bmt_struct bmt;		/* dynamic created global bmt table */
...
static void dump_bmt_info(struct bmt_struct *bmt)
{
	int i;
...
static u8 cal_bmt_checksum(struct phys_bmt_struct *phys_table, int bmt_size)
{
	...
	for (i = 0; i < bmt_size * sizeof(struct bmt_entry); i++)
		checksum += dat[i];
...
static bool valid_bmt_data(struct phys_bmt_struct *phys_table)
{
	...
	if (phys_table->header.checksum != checksum) {
		pr_info("BMT Data checksum error: %x %x\n", phys_table->header.checksum,
			checksum);
		return false;
	}
...
static void fill_nand_bmt_buffer(struct bmt_struct *bmt, u8 *dat, u8 *oob)
{
	...
	memcpy(phys_bmt->table, bmt->table, sizeof(struct bmt_entry) * bmt_block_count);
...
#ifdef CONFIG_MNTL_SUPPORT
	memcpy(dat + MAIN_SIGNATURE_OFFSET + sizeof(struct phys_bmt_struct),
		&(bmt->data_bmt), sizeof(struct data_bmt_struct));
#endif
	kfree(phys_bmt);
}
...
static int load_bmt_data(int start, int pool_size)
{
	...
	for (bmt_index = start + pool_size - 1; bmt_index >= start; bmt_index--) {
#if defined(CONFIG_MTK_TLC_NAND_SUPPORT)
		bmt_offset = (u64)(bmt_index&0xFFFFFFFF);
		bmt_offset = bmt_offset*(gn_devinfo.blocksize * 1024);
#else
		bmt_offset = OFFSET(bmt_index);
#endif
		if (nand_block_bad_bmt(bmt_offset)) {
...
#endif
		pr_debug("[load_bmt_data] datbuf 0x%lx oobbuf 0x%lx\n", (unsigned long)dat_buf, (unsigned long)oob_buf);
		...
		if (!valid_bmt_data(phys_table)) {
			pr_info("BMT data is not correct %d\n", bmt_index);
			continue;
		} else {
			...
			memcpy(bmt.table, phys_table->table, bmt.mapped_count * sizeof(struct bmt_entry));
#ifdef CONFIG_MNTL_SUPPORT
			memcpy(&bmt.data_bmt,
					(dat_buf + MAIN_SIGNATURE_OFFSET + sizeof(struct phys_bmt_struct)),
					sizeof(struct data_bmt_struct));
			pr_info("Data bmt bad_count:%d start_block:0x%x, end_block:0x%x\n", bmt.data_bmt.bad_count,
					bmt.data_bmt.start_block, bmt.data_bmt.end_block);
#endif

			pr_info("bmt found at block: %d, mapped block: %d\n", bmt_index,
				bmt.mapped_count);
			kfree(phys_table);
			return bmt_index;
		}
...
#if defined(CONFIG_MTK_TLC_NAND_SUPPORT)
		bmt_offset = (u64)(block&0xFFFFFFFF);
		bmt_offset = bmt_offset*(gn_devinfo.blocksize * 1024);
#else
		bmt_offset = OFFSET(block);
#endif
		if (nand_block_bad_bmt(bmt_offset)) {
...
static int migrate_from_bad(u64 offset, u8 *write_dat, u8 *write_oob)
{
	...
	if ((gn_devinfo.NAND_FLASH_TYPE == NAND_FLASH_TLC)
		...
		bRet = mtk_nand_write_tlc_block_hw(mtd_bmt, nand_chip_bmt, NULL,
...
struct bmt_struct *reconstruct_bmt(struct bmt_struct *bmt)
{
	int i;
	...
	memset(bmt->table, 0, bmt_block_count * sizeof(struct bmt_entry));
...
struct bmt_struct *init_bmt(struct nand_chip *chip, int size)
{
	...
	pr_info("bmt count: %d, system count: %d\n", bmt_block_count, system_block_count);

	/* set this flag, and unmapped block in pool will be erased. */
	pool_erased = 0;
	memset(bmt.table, 0, size * sizeof(struct bmt_entry));
