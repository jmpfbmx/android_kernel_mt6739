...
{
	pr_notice("[bq24196_shutdown] driver shutdown\n");
...
static ssize_t store_bq24196_access(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		pr_debug("[store_bq24196_access] buf is %s and size is %zu\n", buf, size);
...
static DEVICE_ATTR(bq24196_access, 0664, show_bq24196_access, store_bq24196_access);

static int bq24196_user_space_probe(struct platform_device *dev)
{
		int ret_device_file = 0;

	pr_notice("******** bq24196_user_space_probe!! ********\n");
...
static int __init bq24196_init(void)
{
	...
	if (i2c_add_driver(&bq24196_driver) != 0)
		pr_notice("[bq24196_init] failed to register bq24196 i2c driver.\n");
	else
		pr_notice("[bq24196_init] Success to register bq24196 i2c driver.\n");

	/*bq24196 user space access interface*/
	ret = platform_device_register(&bq24196_user_space_device);
	if (ret) {
		pr_notice("****[bq24196_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&bq24196_user_space_driver);
	if (ret) {
		pr_notice("****[bq24196_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
	return 0;
}
