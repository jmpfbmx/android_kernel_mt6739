			  ...
			  const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		buf += 2;
		if (!strncmp(buf, "mode", 4) && (sscanf(buf + 4, "%d %d", &pin, &mode) == 2))
			GPIOMSG("set mode(%3d, %d)=%d\n", pin, mode, mt_set_gpio_mode(pin, mode));
		else if (!strncmp(buf, "psel", 4) && (sscanf(buf + 4, "%d %d", &pin, &pullsel) == 2))
			GPIOMSG("set psel(%3d, %d)=%d\n", pin, pullsel, mt_set_gpio_pull_select(pin, pullsel));
		else if (!strncmp(buf, "dout", 4) && (sscanf(buf + 4, "%d %d", &pin, &dout) == 2))
			GPIOMSG("set dout(%3d, %d)=%d\n", pin, dout, mt_set_gpio_out(pin, dout));
		else if (!strncmp(buf, "pen", 3) && (sscanf(buf + 3, "%d %d", &pin, &pullen) == 2))
			GPIOMSG("set pen (%3d, %d)=%d\n", pin, pullen, mt_set_gpio_pull_enable(pin, pullen));
		else if (!strncmp(buf, "ies", 3) && (sscanf(buf + 3, "%d %d", &pin, &ies) == 2))
			GPIOMSG("set ies (%3d, %d)=%d\n", pin, ies, mt_set_gpio_ies(pin, ies));
		else if (!strncmp(buf, "dir", 3) && (sscanf(buf + 3, "%d %d", &pin, &dir) == 2))
...
#ifdef CONFIG_MTK_FPGA
		else if (sscanf(buf, "=%d:%d %d", &pin, &dout, &dir) == 3) {
			GPIOMSG("set dout(%3d, %d)=%d\n", pin, dout, mt_set_gpio_out(pin, dout));
			GPIOMSG("set dir (%3d, %d)=%d\n", pin, dir, mt_set_gpio_dir(pin, dir));
#else
		else if (sscanf(buf, "=%d:%d %d %d %d %d %d %d", &pin, &mode, &pullsel, &dout,
				&pullen, &dir, &ies, &smt) == 8) {
			GPIOMSG("set mode(%3d, %d)=%d\n", pin, mode, mt_set_gpio_mode(pin, mode));
			GPIOMSG("set psel(%3d, %d)=%d\n", pin, pullsel,
...
#ifdef MTK_MT6306_SUPPORT
	} else if (!strncmp(buf, "ww", 2)) {/* MT6306 GPIO */
		buf += 2;
		   if (sscanf(buf, "=%d:%d %d", &pin, &dout, &dir) == 3) {
		   ...
		   GPIOMSG("invalid format: '%s'", buf);
	} else if (!strncmp(buf, "wy", 2)) {/* MT6306 GPIO */
		buf += 2;
		if (sscanf(buf, "=%d:%d", &group, &on) == 2) {
		...
		buf += 2;
		if (!strncmp(buf, "s", 1) && (sscanf(buf + 1, "%d %d %d", &num, &src, &div) == 3))
			GPIOMSG("set num(%d, %d, %d)=%d\n", num, src, div, mt_set_clock_output(num, src, div));
	} else if (!strncmp(buf, "g", 1) && (sscanf(buf + 1, "%d", &num) == 1)) {
