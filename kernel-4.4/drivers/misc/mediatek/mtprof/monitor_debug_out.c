...
void mt_show_last_irq_counts(void)
{
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		printk_deferred
			("Last irq counts record at [%llu] ns\n",
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		for (irq = 0; irq < NR_IPI; irq++) {
			if (per_cpu(ipi_count_mon, cpu).ipis[irq] != 0)
				printk_deferred("(CPU#%d)IPI[%3d] = %8d\n", cpu, irq,
...
void mt_show_current_irq_counts(void)
{
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		...
		printk_deferred
			("Current irq counts record at [%llu] ns.(last at %llu, diff:+%llu ns)\n",
			t_cur, per_cpu(save_irq_count_time, cpu), usec_high(t_diff));
		printk_deferred
			("CPU%d state:%s\n", cpu, cpu_online(cpu) ? "online" : "offline");
		for (irq = 0; irq < nr_irqs && irq < MAX_NR_IRQS; irq++) {
			count = kstat_irqs_cpu(irq, cpu);
			if (count != 0)
				printk_deferred(" IRQ[%3d:%14s] = %8d, (+%d times in %lld us)\n", irq,
					isr_name(irq), count, count - per_cpu(irq_count_mon,
					cpu).irqs[irq], usec_high(t_diff));
		}
	}
#ifdef CONFIG_SMP
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		printk_deferred("Local IRQ on CPU#%d:\n", cpu);
		for (irq = 0; irq < NR_IPI; irq++) {
			count = __get_irq_stat(cpu, ipi_irqs[irq]);
			if (count != 0)
				printk_deferred
					(" IRQ[%2d:  IPI] = %8d,(+%d times in %lld us)\n", irq, count,
...
void mt_show_timer_info(void)
{
	...
	for_each_possible_cpu(cpu) {
		printk_deferred
			("[TimerISR#%d] last timer ISR start:%llu ns, end:%llu ns\n", cpu,
...
static void mt_aee_show_current_irq_counts(void)
{
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		...
		for (irq = 0; irq < NR_IPI; irq++) {
			...
			if (count != 0) {
				snprintf(wdt_sched_mon_log_buf, sizeof(wdt_sched_mon_log_buf),
					 " %d:IPI +%d(%d)\n", irq, count - per_cpu(ipi_count_mon,
										   cpu).ipis[irq],
					 count);
				aee_sram_fiq_log(wdt_sched_mon_log_buf);
				memset(wdt_sched_mon_log_buf, 0, sizeof(wdt_sched_mon_log_buf));
...
/****  Dump Stop Events ****/
		if (e_irq->cur_ts == 0)
			printk_deferred("[IRQ disable] last duration:%llu ns (s: %llu, e: %llu)\n\n",
				e_irq->last_te - e_irq->last_ts, e_irq->last_ts, e_irq->last_te);
		else {
			printk_deferred
				("[IRQ disable] cur_ts:%llu(elapsed:%llu ns),",
		...
		if (e_pmpt->cur_ts == 0)
			printk_deferred
				("[Preempt disable] last duration:%llu ns(s: %llu, e: %llu)\n\n",
				e_pmpt->last_te - e_pmpt->last_ts, e_pmpt->last_ts, e_pmpt->last_te);
		else {
			printk_deferred
				("[Preempt disable] cur_ts:%llu(elapsed:%llu ns)",
	...
	for_each_possible_cpu(cpu) {
		...
		if (b_tk->cur_event == 0) {
			...
			snprintf(wdt_sched_mon_log_buf, sizeof(wdt_sched_mon_log_buf),
				 ...
				 (int)b_tk->cur_count, (int)b_tk->last_count,
				 (void *)b_tk->cur_event, usec_high(b_tk->cur_ts),
		...
		if (b_hrt->cur_event == 0) {
			...
			snprintf(wdt_sched_mon_log_buf, sizeof(wdt_sched_mon_log_buf),
				 "[In HRTimer]\n Occurs: cur:%d, last:%d\n Current:%pS, Start:%lld(elapsed: %lld),",
				 (int)b_tk->cur_count, (int)b_tk->last_count,
				 (void *)b_hrt->cur_event, usec_high(b_hrt->cur_ts),
		...
		if (b_sft->cur_event == 0) {
			...
			snprintf(wdt_sched_mon_log_buf, sizeof(wdt_sched_mon_log_buf),
				 "[In SoftTimer]\n Occurs: cur:%d, last:%d\n Current:%pS, Start:%lld(elapsed: %lld),",
				 (int)b_sft->cur_count, (int)b_sft->last_count,
				 (void *)b_sft->cur_event, usec_high(b_sft->cur_ts),
