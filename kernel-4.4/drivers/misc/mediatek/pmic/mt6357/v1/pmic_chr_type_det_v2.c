...
//modified by shiqiang.chen.hz for defect 6208777 on 20180418, optimize the charger type detection.
	int check_count = 2;	//added by shiqiang.chen.hz for defect 5877305 on 20180117 , for USB insert slowly test .
...
#endif
//Begin modified by shiqiang.chen.hz for defect 5877305 on 20180117 , for USB insert slowly test .
#if 0
	hw_bc11_init();
	...
	do {
		...
		if((CHR_Type_num == NONSTANDARD_CHARGER) || (CHR_Type_num == CHARGER_UNKNOWN)) {
			check_count--;
//Begin modified by shiqiang.chen.hz for defect 6208777 on 20180418, optimize the charger type detection.
			if(check_count == 0)
				break;
			//pr_info("charger type:%d check_count:%d , delay 100ms retry check bc11.\n", CHR_Type_num,check_count);
			mdelay(100);
//End modified by shiqiang.chen.hz for defect 6208777 on 20180418, optimize the charger type detection.
		} else {
			//pr_info("charger type:%d check_count:%d , check ok.\n", CHR_Type_num,check_count);			
			break;
		}
		
	} while(check_count);
#endif
//End modified by shiqiang.chen.hz for defect 5877305 on 20180117 , for USB insert slowly test .
