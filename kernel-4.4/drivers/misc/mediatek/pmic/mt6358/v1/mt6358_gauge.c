...
static int fgauge_get_coulomb(struct gauge_device *gauge_dev, int *data)
{
	...
	if (reset == 0)
		...
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x0705, 0x1F05, 0x0); /*weiching0803*/
...
static int fgauge_reset_hw(struct gauge_device *gauge_dev)
{
	...
	while (val_car != 0x0) {
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x0600, 0x1F00, 0x0);
...
/* 6358 no need to switch SWCHR_POWER_PATH, only 56 57 */
	...
	bm_debug("[oam] read_hw_ocv_6358_plug_in (pchr) : adc_result_reg=%d, adc_result=%d, start_sel=%d, rdy=%d\n",
...
static int read_hw_ocv_6358_power_on(void)
{
	...
	adc_result = REG_to_MV_value(adc_result_reg);
	bm_debug("[oam] read_hw_ocv_6358_power_on (pchr) : adc_result_reg=%d, adc_result=%d, start_sel=%d, rdy=%d\n",
...
static int read_hw_ocv_6358_power_on_rdy(void)
{
	...
	bm_err("[read_hw_ocv_6358_power_on_rdy] pwron_PCHR_rdy %d\n", pon_rdy);
...
int read_hw_ocv(struct gauge_device *gauge_dev, int *data)
{
	...
	*data = _hw_ocv;
	...
	bm_debug("[read_hw_ocv] _hw_ocv_58_pon_rdy %d _hw_ocv_58_pon %d _hw_ocv_58_plugin %d _hw_ocv_chgin %d _sw_ocv %d now_temp %d now_thr %d\n",
		_hw_ocv_58_pon_rdy, _hw_ocv_58_pon, _hw_ocv_58_plugin, _hw_ocv_chgin, _sw_ocv, now_temp, now_thr);
...
int fgauge_set_coulomb_interrupt1_ht(struct gauge_device *gauge_dev, int car_value)
{
	...
	if (reset == 0) {
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x0001, 0x1F0F, 0x0);
	} else {
		ret = pmic_config_interface(MT6358_FGADC_CON1, 0x1F05, 0xFF0F, 0x0);
...
int fgauge_set_battery_cycle_interrupt(struct gauge_device *gauge_dev, int threshold)
{
	...
	pmic_set_register_value(PMIC_FG_N_CHARGE_LTH_15_00, (carReg & 0xffff));
...
static int mt6358_gauge_probe(struct platform_device *pdev)
{
	...
	info = devm_kzalloc(&pdev->dev, sizeof(struct mt6358_gauge), GFP_KERNEL);
	if (!info)
		return -ENOMEM;
