...
static int bif_set_cmd(u16 bif_cmd[], int bif_cmd_len)
{
	...
	for (i = 0; i < bif_cmd_len; i++) {
		ret = pmic_config_interface(MT6358_BIF_CON0 + con_index,
			bif_cmd[i], 0x07FF, 0);
		if (ret != 0) {
			chr_err("%s: failed, bif_cmd[%d]\n", __func__, i);
			return -EIO;
		}
		con_index += 0x2;
	}
...
static int bif_reset_irq(void)
{
	...
	if (reg_irq == 0)
		...
		ret = -EIO;
		chr_err("%s: failed, PMIC_BIF_IRQ_CLR = 0x%02X\n",
			__func__, reg_irq);
	}
...
static int bif_waitfor_slave(void)
{
	...
	/* Success */
	if (reg_irq == 1)
		chr_debug("%s: OK, retry_cnt = %d\n", __func__, retry_cnt);
	else { /* Failed */
		ret = -EIO;
		chr_err("%s: failed, PMIC_BIF_IRQ = 0x%02X, retry_cnt = %d\n",
...
_err:
	/* Failed */
	chr_err("%s: failed, retry_cnt = %d, ret = %d\n",
...
static int bif_write8(u16 addr, u8 *data)
{
	...
	era = (addr & 0xFF00) >> 8;
	wra = addr & 0x00FF;
	chr_debug("%s: ERA = 0x%02x, WRA = 0x%02x\n", __func__, era, wra);
...
_err:
	chr_err("%s: failed, retry_cnt = %d, ret = %d",
		__func__, retry_cnt, ret);
	chr_err("%s: bat_lost = %d, timeout = %d, total_valid = %d\n",
...
static int bif_read8(u16 addr, u8 *data)
{
	...
	era = (addr & 0xFF00) >> 8;
	rra = addr & 0x00FF;
	chr_debug("%s: ERA = 0x%02x, RRA = 0x%02x\n", __func__, era, rra);
...
_err:
	chr_err("%s: failed, retry_cnt = %d, ret = %d\n",
		__func__, retry_cnt, ret);
	chr_err("%s: failed, bat_lost = %d, timeout = %d, totoal_valid = %d\n",
		__func__, bat_lost, timeout, total_valid);
...
_err:
	chr_err("%s: failed, retry_cnt = %d, ret = %d\n",
		__func__, retry_cnt, ret);
	chr_err("%s: bat_lost = %d, timeout = %d, totoal_valid = %d\n",
		__func__, bat_lost, timeout, total_valid);
...
#define BIF_INT_REG MT6358_BM_TOP_INT_CON1
#define BIF_INT_REG_CLR PMIC_BM_TOP_INT_CON1_CLR
#define BIF_INT_REG_SET PMIC_BM_TOP_INT_CON1_SET
#define BIF_INT_SHIFT 4

#define BIF_CKPDN_REG MT6358_BM_TOP_CKPDN_CON0
...
static int bif_init(void)
{
	...
	/* Enable BIF clock */
	pmic_set_register_value(BIF_CKPDN_REG_CLR,
...
_err:
	chr_err("%s: failed, ret = %d", __func__, ret);
	return ret;
}
...
int mtk_bif_init(void)
{
	...
	if (ret == 0 && vbat != 0) {
		pr_info("%s: BIF battery detected\n", __func__);
		bif_exist = true;
		bif_checked = true;
		return ret;
	}
...
_err:
	chr_err("%s: BIF battery _NOT_ detected\n", __func__);
	bif_exist = false;
	bif_checked = true;
