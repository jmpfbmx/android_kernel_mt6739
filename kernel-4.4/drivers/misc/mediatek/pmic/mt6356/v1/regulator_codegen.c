...
static int pmic_ldo_vusb_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vusb don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vusb_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("ldo vusb don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vusb_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vusb don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vcama_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vcama don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vcama_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	...
	if (mreg->vol_cb != NULL)
		...
		pr_err("ldo vcama don't have vol_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vcama_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (mreg->da_vol_cb == NULL) {
		pr_err("ldo vcama don't have da_vol_cb\n");
		ret = -1;
	} else {
		ret = -1;
		selector = (mreg->da_vol_cb)();
...
static int pmic_ldo_vcamd_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vcamd don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vcamd_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("ldo vcamd don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vcamd_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vcamd don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vcamd_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	...
	if (mreg->vol_cb != NULL)
		...
		pr_err("ldo vcamd don't have vol_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vcamd_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (mreg->da_vol_cb == NULL) {
		pr_err("ldo vcamd don't have da_vol_cb\n");
		ret = -1;
	} else {
		ret = -1;
		selector = (mreg->da_vol_cb)();
...
static int pmic_ldo_vaux18_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vaux18 don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vaux18_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("ldo vaux18 don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vaux18_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vaux18 don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vsram_others_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vsram_others don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vsram_others_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		PMICLOG("ldo vsram_others should not be disable (use_count=%d)\n", rdev->use_count);
		ret = -1;
	} else {
		if (mreg->en_cb != NULL)
			ret = (mreg->en_cb)(0);
		else {
			pr_err("ldo vsram_others don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vsram_others_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vsram_others don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vsram_others_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	const struct regulator_desc *rdesc = rdev->desc;
	struct mtk_regulator *mreg;
	int ret = 0;
	...
	if (mreg->vol_cb != NULL)
		...
		pr_err("ldo vsram_others don't have vol_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vsram_others_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (mreg->da_vol_cb == NULL) {
		pr_err("ldo vsram_others don't have da_vol_cb\n");
		return -1;
	}
...
static int pmic_buck_vproc_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("buck vproc don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_buck_vproc_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("buck vproc don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_buck_vproc_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("buck vproc don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_buck_vproc_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	const struct regulator_desc *rdesc = rdev->desc;
	struct mtk_regulator *mreg;
	int ret = 0;
	...
	if (mreg->vol_cb != NULL)
		...
		pr_err("buck vproc don't have vol_cb\n");
		ret = -1;
	}
...
static int pmic_buck_vproc_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (mreg->da_vol_cb == NULL) {
		pr_err("buck vproc don't have da_vol_cb\n");
		return -1;
	}
...
static int pmic_ldo_vrf18_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vrf18 don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vrf18_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("ldo vrf18 don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vrf18_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vrf18 don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vdram_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vdram don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vdram_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("ldo vdram don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vdram_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vdram don't have da_en_cb\n");
		return -1;
	}
...
static int pmic_ldo_vdram_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	...
	if (mreg->vol_cb != NULL)
		...
		pr_err("ldo vdram don't have vol_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vdram_get_voltage_sel(struct regulator_dev *rdev)
{
	...
	if (mreg->da_vol_cb == NULL) {
		pr_err("ldo vdram don't have da_vol_cb\n");
		ret = -1;
	} else {
		ret = -1;
		selector = (mreg->da_vol_cb)();
...
static int pmic_ldo_vaud28_enable(struct regulator_dev *rdev)
{
	...
	if (mreg->en_cb != NULL)
		...
		pr_err("ldo vaud28 don't have en_cb\n");
		ret = -1;
	}
...
static int pmic_ldo_vaud28_disable(struct regulator_dev *rdev)
{
	...
	if (rdev->use_count == 0) {
		...
		if (mreg->en_cb != NULL)
			...
			pr_err("ldo vaud28 don't have enable callback\n");
			ret = -1;
		}
	}
...
static int pmic_ldo_vaud28_is_enabled(struct regulator_dev *rdev)
{
	...
	if (mreg->da_en_cb == NULL) {
		pr_err("ldo vaud28 don't have da_en_cb\n");
		return -1;
	}
...
struct mtk_regulator mt_bucks[] = {
	...
	REGULAR_VOLTAGE_REGULATOR_BUCK_GEN(vproc, buck, 500000, 1293750, 6250, 0, BUCK_VOL_EN, 1),
...
struct mtk_regulator mt_ldos[] = {
	...
	NON_REGULAR_VOLTAGE_REGULATOR_GEN(vibr, ldo, vibr_voltages, vibr_idx, LDO_VOL_EN, 1),
	...
	NON_REGULAR_VOLTAGE_REGULATOR_GEN(vcama, ldo, vcama_voltages, vcama_idx, LDO_VOL_EN, 1),
	...
	REGULAR_VOLTAGE_REGULATOR_LDO_GEN(vsram_gpu, ldo, 500000, 1293750, 6250, 0, LDO_VOL_EN, 1),
	...
	NON_REGULAR_VOLTAGE_REGULATOR_GEN(vdram, ldo, vdram_voltages, vdram_idx, LDO_VOL_EN, 1),
