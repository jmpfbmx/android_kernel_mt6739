...
static int mt6356_gauge_probe(struct platform_device *pdev)
{
	int ret = 0;
	struct mt6356_gauge *info;

	pr_err("%s: starts\n", __func__);

	info = devm_kzalloc(&pdev->dev, sizeof(struct mt6356_gauge), GFP_KERNEL);
	if (!info)
		return -ENOMEM;

	mt6356_parse_dt(info, &pdev->dev);
	platform_set_drvdata(pdev, info);

	/* Register charger device */
	info->gauge_dev = gauge_device_register(info->gauge_dev_name,
		&pdev->dev, info, &mt6356_gauge_ops, &info->gauge_prop);
...
static int mt6356_gauge_remove(struct platform_device *pdev)
{
	struct mt6356_gauge *mt = platform_get_drvdata(pdev);
...
static void mt6356_gauge_shutdown(struct platform_device *dev)
{
}


static const struct of_device_id mt6356_gauge_of_match[] = {
	{.compatible = "mediatek,mt6356_gauge",},
	{},
};

MODULE_DEVICE_TABLE(of, mt6356_gauge_of_match);

static struct platform_driver mt6356_gauge_driver = {
	.probe = mt6356_gauge_probe,
	.remove = mt6356_gauge_remove,
	.shutdown = mt6356_gauge_shutdown,
	.driver = {
		   .name = "mt6356_gauge",
		   .of_match_table = mt6356_gauge_of_match,
		   },
};

static int __init mt6356_gauge_init(void)
{
	return platform_driver_register(&mt6356_gauge_driver);
}
device_initcall(mt6356_gauge_init);

static void __exit mt6356_gauge_exit(void)
{
	platform_driver_unregister(&mt6356_gauge_driver);
}
module_exit(mt6356_gauge_exit);
