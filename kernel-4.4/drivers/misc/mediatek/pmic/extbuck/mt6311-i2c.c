...
int is_mt6311_exist(void)
{
	...
	/* get the MT6311 status which is recorded in MT6356 register at Preloader */
...
static int mt6311_read_device(unsigned char addr, unsigned int len, unsigned char *data)
{
	return i2c_smbus_read_i2c_block_data(new_client, addr, len, data);
}

static int mt6311_write_device(unsigned char addr, unsigned int len, unsigned char *data)
{
...
int mt6311_read_byte(unsigned char addr, unsigned char *data)
{
	...
	if (ret < 0) {
		pr_err(MT6311TAG "read Reg[0x%x] fail, ret = %d\n", addr, ret);
		return ret;
	}
...
int mt6311_write_byte(unsigned char addr, unsigned char data)
{
	...
	mutex_unlock(&mt6311_i2c_access);
	if (ret < 0) {
		pr_err(MT6311TAG "write Reg[0x%x]=0x%x fail, ret = %d\n", addr, data, ret);
		return ret;
	}
...
int mt6311_assign_bit(unsigned char reg, unsigned char mask, unsigned char data)
{
	...
	if (ret < 0) {
		pr_err(MT6311TAG "[%s] read fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x\n",
	...
	if (ret < 0) {
		pr_err(MT6311TAG "[%s] assign bit fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x\n",
			__func__, ret, reg, data, mask);
		goto OUT_ASSIGN;
	}
OUT_ASSIGN:
	...
	unsigned char mask, unsigned char shift)
{
	...
	tmp = regval;
	if (ret < 0) {
		pr_err(MT6311TAG "[%s] read fail(ret=%d), Reg[0x%x], mask=0x%x, shift=%d\n",
	...
	unsigned char mask, unsigned char shift)
{
	...
	tmp = regval;
	if (ret < 0) {
		pr_err(MT6311TAG "[%s] read fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x, shift=%d\n",
	...
	ret = mt6311_write_device(reg, 1, &tmp);
	if (ret < 0) {
		MT6311LOG("[%s] write fail(ret=%d), Reg[0x%x] data=0x%x, mask=0x%x, shift=%d\n",
			__func__, ret, reg, data, mask, shift);
		goto OUT_ASSIGN;
	}
OUT_ASSIGN:
...
static int mt6311_check_id(void)
{
	...
	if (ret < 0) {
		pr_err(MT6311TAG "MT6311 check id fail\n"); /* MT6311 IO fail */
		return -EIO;
	}

	if (data != MT6311_CID_CODE) {
		/*MT6311 CID(0x%02x) not match */
		pr_err(MT6311TAG "MT6311 id(0x%x) error\n", data);
		return -EINVAL;
	}
	return 0;
}
				   ...
				   const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		pr_err(MT6311TAG "[%s] size is %d, buf is %s\n", __func__, (int)size, buf);
		pvalue = (char *)buf;
		addr = strsep(&pvalue, " ");
		val = strsep(&pvalue, " ");
		if (addr)
		...
		if (val) {
			...
			pr_err(MT6311TAG "[%s] write mt6311 reg 0x%x with value 0x%x!\n",
			...
			pr_err(MT6311TAG "[%s] read mt6311 reg 0x%x with value 0x%x !\n",
				__func__, reg_address, g_reg_value_mt6311);
			pr_err(MT6311TAG "[%s] use \"cat mt6311_access\" to get value\n", __func__);
		}
	}
	return size;
#endif
