...
int extbuck_ipi_to_sspm(void *buffer, void *retbuf, unsigned char lock)
{
	...
	if (cmd == 0xf) {
		...
		aee_rr_rec_set_bit_pmic_ext_buck(1, 3);
	} else {
		aee_rr_rec_set_bit_pmic_ext_buck(cmd, 0);
	...
	*/
	ret_val =
		sspm_ipi_send_sync(IPI_ID_EXT_BUCK, IPI_OPT_POLLING,
				buffer, 1, retbuf, 1);

	if (cmd == 0xf)
		aee_rr_rec_set_bit_pmic_ext_buck(0, 3);
	else
