...
static int bif_set_cmd(u16 bif_cmd[], int bif_cmd_len)
{
	...
	for (i = 0; i < bif_cmd_len; i++) {
		ret = pmic_config_interface(MT6355_BIF_CON0 + con_index,
			bif_cmd[i], 0x07FF, 0);
		if (ret != 0) {
...
#define BIF_INT_REG MT6355_INT_CON0
#define BIF_INT_REG_CLR PMIC_INT_CON0_CLR
#define BIF_INT_REG_SET PMIC_INT_CON0_SET
#define BIF_INT_SHIFT 13

#define BIF_CKPDN_REG MT6355_TOP_CKPDN_CON0
#define BIF_CKPDN_REG_CLR PMIC_TOP_CKPDN_CON0_CLR
#define BIF_CKPDN_REG_SET PMIC_TOP_CKPDN_CON0_SET
#define BIF_1M_CK_PDN_SHIFT 4
#define BIF_X1_CK_PDN_SHIFT 5
#define BIF_X4_CK_PDN_SHIFT 6
#define BIF_X72_CK_PDN_SHIFT 7
...
_err:
	pr_err("%s: BIF battery _NOT_ detected\n", __func__);
	bif_exist = false;
	bif_checked = true;
