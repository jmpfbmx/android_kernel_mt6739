...
static int test_allocate_buffer(void)
{
	...
	ret = m4u_alloc_mva(client, M4U_PORT_DISP_OVL1, buffer_va, 0, bufferSize,
							...
							unsigned int w, unsigned int h, unsigned int color)
{
	...
	for (j = 0; j < h; j++) {
		for (i = 0; i < w; i++)
			*(unsigned int *)(start_addr + i * 4 + j * LCM_WIDTH * 4) = color;
	}
}

static int internal_test(unsigned long va, unsigned int w, unsigned int h, unsigned int odd)
{
	unsigned int i = 0;
