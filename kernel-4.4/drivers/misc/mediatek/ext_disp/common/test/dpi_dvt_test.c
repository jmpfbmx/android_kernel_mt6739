...
/* char *fill = NULL; */
	...
	if (resolution == HDMI_VIDEO_1280x720p_60Hz) {
		...
		if (IS_ERR(fd)) {
			pr_err("EXTD: open folder Gene_1280x720.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
			set_fs(fs);
			return -1;
			}
		...
		if (IS_ERR(fd)) {
			pr_err("EXTD: open folder PDA0026_720x480.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
			set_fs(fs);
			return -1;
			}
		...
		if (IS_ERR(fd)) {
			pr_err("EXTD: open folder Venice_1920x1080.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
			set_fs(fs);
			return -1;
		}
		...
		if (buffer_round == 3) {
			...
			if (IS_ERR(fd)) {
				pr_err("EXTD: open folder picture_4k.yuv fail !!!!! ret %ld\n", PTR_ERR(fd));
				set_fs(fs);
				return -1;
			}
			pos = 0;
...
int dvt_start_rdma_to_dpi(unsigned int resolution, unsigned int timeS, unsigned int caseNum)
{
	int ret = 0;
	int i;
	...
	case 1:
		{
			...
			dvt_dump_ext_dpi_parameters();
			dvt_start_rdma_to_dpi(HDMI_VIDEO_1920x1080p_30Hz, 20, 0x1);
			break;
		}
	case 2:
		{
