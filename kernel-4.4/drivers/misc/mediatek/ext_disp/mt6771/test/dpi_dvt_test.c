		...
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->init != 0)
					ddp_get_module_driver(cur_mod)->init(cur_mod, NULL);
			}
		}
		break;
	case POWER_ON:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->power_on != 0)
					ddp_get_module_driver(cur_mod)->power_on(cur_mod, NULL);
			}
		}
		break;
	case POWER_OFF:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->power_off != 0)
					ddp_get_module_driver(cur_mod)->power_off(cur_mod, NULL);
			}
		}
		break;
	case START:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->start != 0)
					ddp_get_module_driver(cur_mod)->start(cur_mod, NULL);
			}
		}
		break;
	case STOP:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->stop != 0)
					ddp_get_module_driver(cur_mod)->stop(cur_mod, NULL);
			}
		}
		break;
	case RESET:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->reset != 0)
					ddp_get_module_driver(cur_mod)->reset(cur_mod, NULL);
			}
		}
		break;
	case BYPASS:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->bypass != 0)
					ddp_get_module_driver(cur_mod)->bypass(cur_mod, 1);
			}
		}
		break;
	case TRIGGER:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->trigger != 0)
					ddp_get_module_driver(cur_mod)->trigger(cur_mod, NULL);
			}
		}
		break;
	case DEINIT:
		for (i = 0; i < mod_num; i++) {
			...
			if (cur_mod != DISP_MODULE_UNKNOWN
				...
				if (ddp_get_module_driver(cur_mod)->deinit != 0)
					ddp_get_module_driver(cur_mod)->deinit(cur_mod, NULL);
			}
		}
		break;
	default:
...
int dsi_init_OVL_param(unsigned int mode)
{
	...
	if (mode == RDMA_MODE_DIRECT_LINK) {
		extd_ovl_params.ovl_layer_scanned = 0;
		extd_ovl_params.dst_dirty = 1;
		extd_ovl_params.dst_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.dst_h = extd_dpi_params.dispif_config.dpi.height;
		extd_ovl_params.ovl_dirty = 1;
		...
		extd_ovl_params.ovl_config[0].src_x = 0;
		extd_ovl_params.ovl_config[0].src_y = 0;
		extd_ovl_params.ovl_config[0].src_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.ovl_config[0].src_h = extd_dpi_params.dispif_config.dpi.height;
		extd_ovl_params.ovl_config[0].src_pitch =
		...
		extd_ovl_params.ovl_config[0].dst_x = 0;
		extd_ovl_params.ovl_config[0].dst_y = 0;
		extd_ovl_params.ovl_config[0].dst_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.ovl_config[0].dst_h = extd_dpi_params.dispif_config.dpi.height;
		extd_ovl_params.ovl_config[0].ext_sel_layer = -1;
...
int ldvt_init_OVL_param(unsigned int mode)
{
	...
	if (mode == RDMA_MODE_DIRECT_LINK) {
		...
		extd_ovl_params.ovl_config[i].source = OVL_LAYER_SOURCE_MEM;
		extd_ovl_params.ovl_config[i].layer = i;
		extd_ovl_params.ovl_config[i].fmt = UFMT_BGR888;
		extd_ovl_params.ovl_config[i].addr = (unsigned long)ldvt_data_mva[i];
		...
		extd_ovl_params.ovl_config[i].src_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.ovl_config[i].src_h = extd_dpi_params.dispif_config.dpi.height;
		extd_ovl_params.ovl_config[i].src_pitch =
		    extd_dpi_params.dispif_config.dpi.width * 3 * 3;

		extd_ovl_params.ovl_config[i].dst_x = 0;
		extd_ovl_params.ovl_config[i].dst_y = 0;
		extd_ovl_params.ovl_config[i].dst_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.ovl_config[i].dst_h = extd_dpi_params.dispif_config.dpi.height;
		...
		extd_ovl_params.ovl_config[3].addr = (unsigned long)ldvt_data_mva[3];
		...
		extd_ovl_params.ovl_config[3].src_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.ovl_config[3].src_h = extd_dpi_params.dispif_config.dpi.height;
		...
		extd_ovl_params.ovl_config[3].dst_w = extd_dpi_params.dispif_config.dpi.width*3;
		extd_ovl_params.ovl_config[3].dst_h = extd_dpi_params.dispif_config.dpi.height;
...
int dvt_init_GAMMA_param(unsigned int mode)
{
	if (mode == RDMA_MODE_DIRECT_LINK) {
		...
		extd_gamma_params.dst_w = extd_dpi_params.dispif_config.dpi.width*3;
	...
	case HDMI_VIDEO_1080p_DSC_60Hz:
		{
			...
			extd_dpi_params.dispif_config.dpi.dsc_enable = 1;
			/* width/(slice_mode's slice) */
			extd_dpi_params.dispif_config.dpi.dsc_params.slice_width = 960;
			/* 32  8 */
			extd_dpi_params.dispif_config.dpi.dsc_params.slice_hight = 8;
			/* 128: 1/3 compress; 192: 1/2 compress */
			extd_dpi_params.dispif_config.dpi.dsc_params.bit_per_pixel = 128;
			...
			extd_dpi_params.dispif_config.dpi.dsc_params.rgb_swap = 0;
			extd_dpi_params.dispif_config.dpi.dsc_params.xmit_delay = 0x200;
			extd_dpi_params.dispif_config.dpi.dsc_params.dec_delay = 0x2e0;
			extd_dpi_params.dispif_config.dpi.dsc_params.scale_value = 0x20;
			extd_dpi_params.dispif_config.dpi.dsc_params.increment_interval = 0xed;
			extd_dpi_params.dispif_config.dpi.dsc_params.decrement_interval = 0xd;
			extd_dpi_params.dispif_config.dpi.dsc_params.nfl_bpg_offset = 0xdb7;  /* 0x667 */
			extd_dpi_params.dispif_config.dpi.dsc_params.slice_bpg_offset = 0x727; /* 0x1ca */
			extd_dpi_params.dispif_config.dpi.dsc_params.final_offset = 0x10f0;
			extd_dpi_params.dispif_config.dpi.dsc_params.line_bpg_offset = 0xc;
			extd_dpi_params.dispif_config.dpi.dsc_params.bp_enable = 0x0;
			extd_dpi_params.dispif_config.dpi.dsc_params.rct_on = 0x0;

			extd_dpi_params.dst_w = extd_dpi_params.dispif_config.dpi.width*3;  /*1920*/
			extd_dpi_params.dst_h = extd_dpi_params.dispif_config.dpi.height;  /*1080*/
...
/* OVL1_2L -> OVL1_2L_MOUT -> DISP_RDMA1 -> DISP_RDMA1_SOUT -> DPI0_SEL -> DPI0_SEL_SOUT -> DPI0_THP_LMT -> DPI0 */
int dvt_start_ovl1_2l_to_dpi(unsigned int resolution, unsigned int timeS)
{
	...
	ddp_driver_ovl.ioctl(DISP_MODULE_OVL1_2L, NULL, DDP_OVL_GOLDEN_SETTING, &gset_arg);
	...
	memcpy(extd_rdma_params.p_golden_setting_context, &temp_golden, sizeof(temp_golden));
...
int dpi_dvt_show_pattern(unsigned int pattern, unsigned int timeS)
{
	...
	if (pattern == 5) {
		DISP_REG_SET_FIELD(NULL, REG_FLD(8, 8), DISPSYS_DPI_BASE + 0xF00, 0xca);
		DISP_REG_SET_FIELD(NULL, REG_FLD(8, 16), DISPSYS_DPI_BASE + 0xF00, 0xbc);
		DISP_REG_SET_FIELD(NULL, REG_FLD(8, 24), DISPSYS_DPI_BASE + 0xF00, 0x3e);
	}
...
#if 0
int dvt_start_4K_rdma_to_dpi(unsigned int resolution, unsigned int timeS, unsigned int caseNum)
{int i;
	static int loop = 15;
	int ret = 0;
	...
	DISP_REG_SET(NULL, 1 * (DDP_REG_BASE_DISP_RDMA1 - DDP_REG_BASE_DISP_RDMA0)
				+ DDP_REG_BASE_DISP_RDMA0 + DISP_REG_RDMA_SHADOW_UPDATE, (0x1<<1)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_DSC + DISP_REG_DSC_SHADOW, (0x1<<1)|(0x1<<2));
	...
	if (caseNum == 0xA) {
		loop++;
		DPI_DVT_LOG_W("checksum loop %d sample checksum every %d frame\n", loop%2, (loop%2)+2);
		...
		cmdqRecWait(checksum_cmdq, CMDQ_EVENT_MUTEX1_STREAM_EOF);
		cmdqRecWrite(checksum_cmdq, DPI_PHY_ADDR + 0x048, 0x80000000, 0x80000000);
		cmdqRecWait(checksum_cmdq, CMDQ_EVENT_MUTEX1_STREAM_EOF);
		cmdqRecPoll(checksum_cmdq, DPI_PHY_ADDR + 0x048, 0x40000000, 0x40000000);
		cmdqRecBackupRegisterToSlot(checksum_cmdq, checksum_record, 0, DPI_PHY_ADDR + 0x048);
...
/* cmdqRecWait(checksum_cmdq, CMDQ_EVENT_MUTEX1_STREAM_EOF); */
			...
			cmdqRecWait(checksum_cmdq, CMDQ_EVENT_MUTEX1_STREAM_EOF);
			cmdqRecPoll(checksum_cmdq, DPI_PHY_ADDR + 0x048, 0x40000000, 0x40000000);
			cmdqRecBackupRegisterToSlot(checksum_cmdq, checksum_record, i, DPI_PHY_ADDR + 0x048);
		}
	}
...
/* int i = 0; */
	...
	DISP_REG_SET(NULL, 1 * (DDP_REG_BASE_DISP_RDMA1 - DDP_REG_BASE_DISP_RDMA0)
				+ DDP_REG_BASE_DISP_RDMA0 + DISP_REG_RDMA_SHADOW_UPDATE, (0x1<<1)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_DSC + DISP_REG_DSC_SHADOW, (0x1<<1)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_GAMMA1 - DDP_REG_BASE_DISP_GAMMA0
					+ DISP_REG_GAMMA_DEBUG, (0x1<<1)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_COLOR1 - DDP_REG_BASE_DISP_COLOR0
					+ DISP_COLOR_SHADOW_CTRL, (0x1<<0)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_CCORR1 - DDP_REG_BASE_DISP_CCORR0
					+ DISP_REG_CCORR_SHADOW, (0x1<<0)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_AAL1 - DDP_REG_BASE_DISP_AAL0
					+ DISP_AAL_SHADOW_CTL, (0x1<<0)|(0x1<<2));
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_DITHER1 - DDP_REG_BASE_DISP_DITHER0
					+ DISP_REG_DITHER_0, (0x1<<0)|(0x1<<2));

	DISP_REG_SET_FIELD(NULL, EN_FLD_BYPASS_SHADOW, DDP_REG_BASE_DISP_OVL1 + DISP_REG_OVL_EN, 0x1);
	DISP_REG_SET_FIELD(NULL, EN_FLD_RD_WRK_REG, DDP_REG_BASE_DISP_OVL1 + DISP_REG_OVL_EN, 0x1);
	DISP_REG_SET_FIELD(NULL, EN_FLD_BYPASS_SHADOW, DDP_REG_BASE_DISP_OVL1_2L + DISP_REG_OVL_EN, 0x1);
	DISP_REG_SET_FIELD(NULL, EN_FLD_RD_WRK_REG, DDP_REG_BASE_DISP_OVL1_2L + DISP_REG_OVL_EN, 0x1);


	DPI_DVT_LOG_W("set Mutex and connect path\n");
	dvt_connect_ovl1_2l_dpi(NULL);
	dvt_mutex_set_ovl1_2l_dpi(dvt_acquire_mutex(), NULL);
	ddp_driver_ovl.connect(DISP_MODULE_OVL1, DISP_MODULE_UNKNOWN, DISP_MODULE_OVL1_2L, 1, NULL);
	ddp_driver_ovl.connect(DISP_MODULE_OVL1_2L, DISP_MODULE_OVL1, DISP_MODULE_UNKNOWN, 1, NULL);
	...
	DISP_REG_SET(NULL, DISPSYS_COLOR1_BASE - DISPSYS_COLOR0_BASE + DISP_COLOR_CK_ON, 0x1);
	...
	gset_arg.dst_mod_type = DST_MOD_REAL_TIME;
	gset_arg.is_decouple_mode = 0;
	ddp_driver_ovl.ioctl(DISP_MODULE_OVL1, NULL, DDP_OVL_GOLDEN_SETTING, &gset_arg);
	ddp_driver_ovl.ioctl(DISP_MODULE_OVL1_2L, NULL, DDP_OVL_GOLDEN_SETTING, &gset_arg);
...
int dvt_init_WDMA_param(unsigned int resolution)
{
	...
	memcpy(&temp_golden, extd_rdma_params.p_golden_setting_context, sizeof(temp_golden));
...
int dvt_init_RDMA_param(unsigned int mode, unsigned int resolution)
{
	...
	case HDMI_VIDEO_1920x1080i_60Hz:
		extd_rdma_params.p_golden_setting_context->ext_dst_height = 1080;
	...
	case HDMI_VIDEO_2160p_DSC_24Hz:
	case HDMI_VIDEO_2160p_DSC_30Hz:
		extd_rdma_params.p_golden_setting_context->ext_dst_height = 2160;
...
/* #if defined(MHL_SII8338) || defined(MHL_SII8348) */
			...
			DPI_DVT_Context.output_video_resolution = HDMI_VIDEO_2160p_DSC_30Hz;
			dpi_clock = 99500;
...
/* char *fill = NULL; */
	...
	if (resolution == HDMI_VIDEO_1280x720p_60Hz) {
		...
		if (IS_ERR(fd)) {
			DPI_DVT_LOG_W("EXTD: open folder Gene_1280x720.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
			set_fs(fs);
			return -1;
			}
		...
		if (IS_ERR(fd)) {
			DPI_DVT_LOG_W("EXTD: open folder PDA0026_720x480.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
			set_fs(fs);
			return -1;
			}
		...
		fd = NULL;
		set_fs(fs);
	} else if (resolution == HDMI_VIDEO_1920x1080p_30Hz || resolution == HDMI_VIDEO_1920x1080p_60Hz) {
		fs = get_fs();
		set_fs(KERNEL_DS);
		fd = filp_open("/data/Venice_1920x1080.bmp", O_RDONLY, 0);
		if (IS_ERR(fd)) {
			DPI_DVT_LOG_W("EXTD: open folder Venice_1920x1080.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
			set_fs(fs);
			return -1;
		}
		...
		fd = NULL;
		set_fs(fs);
	} else if (resolution == HDMI_VIDEO_2160p_DSC_24Hz || resolution == HDMI_VIDEO_2160p_DSC_30Hz) {
#if 0
		if (buffer_round == 3) {
			fs = get_fs();
			set_fs(KERNEL_DS);
			...
			if (IS_ERR(fd)) {
				DPI_DVT_LOG_W("EXTD: open folder picture_4k.yuv fail !!!!! ret %ld\n", PTR_ERR(fd));
				set_fs(fs);
				return -1;
			}
			pos = 0;
			...
			if (IS_ERR(fd)) {
				DPI_DVT_LOG_W("EXTD: open folder picture_4k.bmp fail !!!!! ret %ld\n", PTR_ERR(fd));
				set_fs(fs);
				return -1;
			}
...
int dvt_allocate_outbuffer(unsigned int resolution, enum HW_MODULE_Type hw_type)
{
	...
	m4u_alloc_mva(client, M4U_PORT, (unsigned long int)out_va, NULL, hdmiBufferSize,
	M4U_PROT_READ | M4U_PROT_WRITE, 0, &out_mva);
...
int dvt_allocate_buffer(unsigned int resolution, enum HW_MODULE_Type hw_type)
{
	...
	if (resolution == HDMI_VIDEO_2160p_DSC_24Hz || resolution == HDMI_VIDEO_2160p_DSC_30Hz) {
		hdmiPixelSize =
		extd_dpi_params.dispif_config.dpi.width * 3 * extd_dpi_params.dispif_config.dpi.height;
/* 1280*2160	extd_dpi_params.dispif_config.dpi.width * extd_dpi_params.dispif_config.dpi.height; */
...
int ldvt_allocate_buffer(unsigned int resolution, enum HW_MODULE_Type hw_type)
{
	...
	for (i = 0; i < 4; i++) {
		...
		DPI_DVT_LOG_W("data_va %lx , client %p\n", ldvt_data_va[i], client_ldvt);

		m4u_alloc_mva(client_ldvt, M4U_PORT, (unsigned long int)ldvt_data_va[i], NULL, hdmiBufferSize,
		M4U_PROT_READ | M4U_PROT_WRITE, 0, &ldvt_data_mva[i]);
...
int dvt_start_rdma1_to_dpi(unsigned int resolution, unsigned int timeS, unsigned int caseNum)
{
	int ret = 0;
	int i;
...
int dvt_start_rdma_to_dpi_for_RGB2YUV(unsigned int resolution, unsigned int timeS, enum AviColorSpace_e format)
{
	int ret = 0;
...
static int dpi_dvt_testcase_6_yuv(unsigned int resolution, enum AviColorSpace_e format)
{
...
static int dpi_dvt_testcase_10_checkSum(unsigned int resolution)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if ((checksum[i] & 0xffffff) != (checksum[i+1] & 0xffffff))
			DPI_DVT_LOG_W("Error8888888888888888888888888888checksum is not the same!!\n");
	}
	DPI_DVT_LOG_W("checksum[%d] = %x", i, checksum[i] & 0xffffff);
...
static int ovl2mem_case19(unsigned int resolution)
{
	...
	DPI_DVT_LOG_W("top clock on\n");
	dvt_ddp_path_top_clock_on();
	DISP_REG_SET(NULL, DDP_REG_BASE_DISP_RDMA1 + DISP_REG_RDMA_SHADOW_UPDATE,
			    (0x1<<1)|(0x1<<2));
	DISP_REG_SET_FIELD(NULL, EN_FLD_BYPASS_SHADOW, DDP_REG_BASE_DISP_OVL1 + DISP_REG_OVL_EN, 0x1);
	DISP_REG_SET_FIELD(NULL, EN_FLD_RD_WRK_REG, DDP_REG_BASE_DISP_OVL1 + DISP_REG_OVL_EN, 0x1);
	DISP_REG_SET_FIELD(NULL, EN_FLD_BYPASS_SHADOW, DDP_REG_BASE_DISP_OVL1_2L + DISP_REG_OVL_EN, 0x1);
	...
	DPI_DVT_LOG_W("set Mutex and connect path\n");
	dvt_connect_ovl1_2l_wdma(NULL);
	dvt_mutex_set_ovl1_2l_wdma(HW_MUTEX, NULL);

	ddp_driver_ovl.connect(DISP_MODULE_OVL1, DISP_MODULE_UNKNOWN, DISP_MODULE_OVL1_2L, 1, NULL);
	ddp_driver_ovl.connect(DISP_MODULE_OVL1_2L, DISP_MODULE_OVL1, DISP_MODULE_UNKNOWN, 1, NULL);
	DISP_REG_SET_FIELD(NULL, DATAPATH_CON_FLD_BGCLR_IN_SEL,
						   DDP_REG_BASE_DISP_OVL1_2L + DISP_REG_OVL_DATAPATH_CON, 1);
	...
	ddp_driver_ovl.ioctl(DISP_MODULE_OVL0, NULL, DDP_OVL_GOLDEN_SETTING, &gset_arg);
	ddp_driver_ovl.ioctl(DISP_MODULE_OVL0_2L, NULL, DDP_OVL_GOLDEN_SETTING, &gset_arg);
	...
	DPI_DVT_LOG_W("set Mutex and disconnect path\n");
	dvt_disconnect_ovl1_2l_wdma(NULL);
...
/* dpi_dvt_testcase_10_checkSum(HDMI_VIDEO_2160p_DSC_30Hz); */
	...
	case 11:
		{
			dpi_dvt_testcase_11_ovl1_to_dpi(HDMI_VIDEO_1920x1080p_30Hz);
			msleep(500);
			dpi_dvt_testcase_11_ovl1_to_dpi(HDMI_VIDEO_1280x720p_60Hz);
			msleep(500);
			dpi_dvt_testcase_11_ovl1_to_dpi(HDMI_VIDEO_720x480p_60Hz);
			msleep(500);
			break;
		}
	case 18:
...
{
	unsigned int dpi_clock = 0;
	unsigned int  hsync_pulse_width = 0, hsync_back_porch = 0, hsync_front_porch = 0,
	    vsync_pulse_width = 0, vsync_back_porch = 0, vsync_front_porch = 0;
	unsigned int height = 0, width = 0;

	memset(&extd_dsi1_params, 0, sizeof(extd_dsi1_params));

	if (mode == DSI_VDO_MODE_BURST || mode == DSI_VDO_MODE_SYNC_EVENT || mode == DSI_VDO_MODE_SYNC_PLUSE) {
		...
		extd_dsi1_params.dispif_config.dsi.data_format.color_order = LCM_COLOR_ORDER_RGB;
		extd_dsi1_params.dispif_config.dsi.data_format.trans_seq = LCM_DSI_TRANS_SEQ_MSB_FIRST;
		extd_dsi1_params.dispif_config.dsi.data_format.padding = LCM_DSI_PADDING_ON_LSB;
		extd_dsi1_params.dispif_config.dsi.data_format.format = LCM_DSI_FORMAT_RGB888;
...
/* dpi_clock = 308 << 8; */
	...
	extd_dsi1_params.dispif_config.dsi.horizontal_sync_active = hsync_pulse_width;
	extd_dsi1_params.dispif_config.dsi.horizontal_backporch = hsync_back_porch;
	extd_dsi1_params.dispif_config.dsi.horizontal_frontporch = hsync_front_porch;
	extd_dsi1_params.dispif_config.dsi.horizontal_active_pixel = width;

	extd_dsi1_params.dispif_config.dsi.vertical_sync_active = vsync_pulse_width;
	extd_dsi1_params.dispif_config.dsi.vertical_backporch = vsync_back_porch;
	extd_dsi1_params.dispif_config.dsi.vertical_frontporch = vsync_front_porch;
