...
{
	memcpy(&external_display_util, util, sizeof(struct EXTERNAL_DISPLAY_UTIL_FUNCS));
}
...
static int _build_path_direct_link(void)
{
	...
	sPort.ePortID = M4U_PORT_UNKNOWN; /* todo: correct module */
...
static int _ext_disp_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER) &&
		...
		/* Qinglong removed full shadow register flow, need to double check */
...
int ext_disp_suspend(unsigned int session)
{
	...
	*/
	...
	if (disp_helper_get_option(DISP_OPT_SHADOW_REGISTER)) {
		...
		*/
			ddp_mutex_enable(dpmgr_path_get_mutex(pgc->dpmgr_handle),
			dpmgr_get_scenario(pgc->dpmgr_handle), DDP_VIDEO_MODE, NULL);
		} else {
		/*
		*	dpmgr_path_mutex_enable(pgc->dpmgr_handle, NULL);
		*	dpmgr_path_mutex_get(pgc->dpmgr_handle, NULL);
...
int ext_fence_release_callback(unsigned long userdata)
{
	...
	if (input_config_info & 0x100) {	/*input source is rdma*/
		if (ext_disp_get_ovl_req_status(pgc->session) == EXTD_OVL_REMOVED)
			ext_disp_path_change(EXTD_OVL_NO_REQ, pgc->session);
	} else {
		if (ext_disp_get_ovl_req_status(pgc->session) == EXTD_OVL_INSERTED)
			ext_disp_path_change(EXTD_OVL_NO_REQ, pgc->session);
	}
	...
	for (i = 0; i < EXTD_OVERLAY_CNT; i++) {
		...
		cmdqBackupReadSlot(pgc->ext_subtractor_when_free, i, &subtractor);
...
int ext_disp_trigger(int blocking, void *callback, unsigned int userdata, unsigned int session)
{
	...
	if (_should_trigger_interface()) {
		if (DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL && DISP_SESSION_DEV(session) == DEV_MHL + 1)
			_ext_disp_trigger(blocking, callback, atomic_read(&g_extd_trigger_ticket));
		else if (DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL
			&& DISP_SESSION_DEV(session) == DEV_EINK + 1)
			_ext_disp_trigger_EPD(blocking, callback, atomic_read(&g_extd_trigger_ticket));
	} else
...
int ext_disp_frame_cfg_input(struct disp_frame_cfg_t *cfg)
{
	...
	if (_should_config_ovl_input()) {
		...
		ret = _convert_disp_input_to_rdma(&(data_config->rdma_config), &(cfg->input_cfg[0]),
						data_config->dst_w, data_config->dst_h);
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		cmdqBackupReadSlot(pgc->ext_cur_config_fence, i, &ext_last_fence);
	...
	if (_should_config_ovl_input()) {
		cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_config,
						...
						0, disp_addr_convert(DISPSYS_OVL1_2L_BASE +
						DISP_REG_OVL_STA));
	} else {
		cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_config,
						pgc->ext_ovl_rdma_status_info,
