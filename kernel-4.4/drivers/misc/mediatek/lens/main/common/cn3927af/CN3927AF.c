	...
	if (*g_pAF_Opened == 2) {
		...
		//begin, add for bug 5780657 by zhijun.fu on 20180106
		 if(s4AF_WriteReg(0)<0)   //reset motor 
		 	pr_err("CN3927AF reset motor failed\n");
		  else 
		  	msleep(10); 
		//  if (s4AF_WriteReg(0x80)<0)   //power down  
		  //	pr_err("CN3927AFpower down failed\n");
		  //else 
		 // 	msleep(10);
...
int CN3927AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
