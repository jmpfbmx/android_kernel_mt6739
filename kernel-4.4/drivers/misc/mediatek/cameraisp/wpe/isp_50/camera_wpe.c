	...
	if (stus & WPE_INT_ST) {
		...
		LOG_ERR
		    (" WaitIRQ StatusErr, type:%d, userNum:%d, status:%d, whichReq:%d,ProcessID:0x%x, ReadReqIdx:%d\n",
		     type, userNumber, stus, whichReq, ProcessID, WPEInfo.ReadReqIdx);
	}
...
#define RegDump(start, end) {\
	...
	for (i = start; i <= end; i += 0x10) {\
	    ...
	    (unsigned int)(ISP_WPE_BASE + i+0x4), (unsigned int)WPE_RD32(ISP_WPE_BASE + i+0x4),\
	    (unsigned int)(ISP_WPE_BASE + i+0x8), (unsigned int)WPE_RD32(ISP_WPE_BASE + i+0x8),\
	    (unsigned int)(ISP_WPE_BASE + i+0xc), (unsigned int)WPE_RD32(ISP_WPE_BASE + i+0xc));\
	} \
}
...
static bool ConfigWPERequest(signed int ReqIdx)
{
	...
	if (g_WPE_ReqRing.WPEReq_Struct[ReqIdx].State == WPE_REQUEST_STATE_PENDING) {
		...
		for (j = 0; j < _SUPPORT_MAX_WPE_FRAME_REQUEST_; j++) {
			if (WPE_FRAME_STATUS_ENQUE ==
			    g_WPE_ReqRing.WPEReq_Struct[ReqIdx].WpeFrameStatus[j]) {
				g_WPE_ReqRing.WPEReq_Struct[ReqIdx].WpeFrameStatus[j] =
		...
		LOG_ERR("ConfigWPERequest state machine error!!, ReqIdx:%d, State:%d\n",
	...
	return MTRUE;
#else
	LOG_ERR("ConfigWPERequest don't support this mode.!!\n");
	return MFALSE;
#endif
}
...
static bool ConfigWPE(void)
{
	...
	for (k = 0; k < _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_; k++) {
		...
		if (g_WPE_ReqRing.WPEReq_Struct[i].State == WPE_REQUEST_STATE_PENDING) {
			g_WPE_ReqRing.WPEReq_Struct[i].State = WPE_REQUEST_STATE_RUNNING;
			for (j = 0; j < _SUPPORT_MAX_WPE_FRAME_REQUEST_; j++) {
				if (WPE_FRAME_STATUS_ENQUE ==
			...
			if (j != _SUPPORT_MAX_WPE_FRAME_REQUEST_) {
				 ...
				 */
				/* spin_unlock_irqrestore(&(WPEInfo.SpinLockIrq[WPE_IRQ_TYPE_INT_WPE_ST]), flags); */
				/* ConfigWPEHW(&g_Wpe_ReqRing.WPEReq_Struct[i].WpeFrameConfig[j]); */
				/* return MTRUE; */
				return MFALSE;
			}
			/*else {
			 *      g_WPE_ReqRing.WPEReq_Struct[i].State = WPE_REQUEST_STATE_RUNNING;
			 *      LOG_ERR("WPE Config State is wrong! HWProcessIdx(%d), State(%d)\n",
			 *      g_WPE_ReqRing.HWProcessIdx, g_WPE_ReqRing.WPEReq_Struct[i].State);
			 *      g_WPE_ReqRing.HWProcessIdx = (g_WPE_ReqRing.HWProcessIdx+1)
	...
	for (k = 0; k < _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_; k++) {
		...
		if (g_WPE_ReqRing.WPEReq_Struct[i].State == WPE_REQUEST_STATE_PENDING) {
			...
			g_WPE_ReqRing.HWProcessIdx =
			    (g_WPE_ReqRing.HWProcessIdx + 1) % _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_;
			/*} */
		}
	}
	if (k == _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_)
...
static bool UpdateWPE(pid_t *ProcessID)
{
	...
	for (i = g_WPE_ReqRing.HWProcessIdx; i < _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_; i++) {
		if (g_WPE_ReqRing.WPEReq_Struct[i].State == WPE_REQUEST_STATE_RUNNING) {
			...
			IRQ_LOG_KEEPER(WPE_IRQ_TYPE_INT_WPE_ST, m_CurrentPPB, _LOG_DBG,
				       "UpdateWPE idx j:%d\n", j);
			if (j != _SUPPORT_MAX_WPE_FRAME_REQUEST_) {
				next_idx = j + 1;
				g_WPE_ReqRing.WPEReq_Struct[i].WpeFrameStatus[j] =
				    WPE_FRAME_STATUS_FINISHED;
				if ((_SUPPORT_MAX_WPE_FRAME_REQUEST_ == (next_idx))
				    || ((_SUPPORT_MAX_WPE_FRAME_REQUEST_ > (next_idx))
					&& (WPE_FRAME_STATUS_EMPTY ==
					    g_WPE_ReqRing.WPEReq_Struct[i].
					    WpeFrameStatus[next_idx]))) {
					bFinishRequest = MTRUE;
					(*ProcessID) = g_WPE_ReqRing.WPEReq_Struct[i].processID;
					g_WPE_ReqRing.WPEReq_Struct[i].State =
			...
			/*else { */
			IRQ_LOG_KEEPER(WPE_IRQ_TYPE_INT_WPE_ST, m_CurrentPPB, _LOG_ERR,
				       "WPE State Machine is wrong! HWProcessIdx(%d), State(%d)\n",
				       g_WPE_ReqRing.HWProcessIdx,
				       g_WPE_ReqRing.WPEReq_Struct[i].State);
			g_WPE_ReqRing.WPEReq_Struct[i].State = WPE_REQUEST_STATE_FINISHED;
			g_WPE_ReqRing.HWProcessIdx =
			    (g_WPE_ReqRing.HWProcessIdx + 1) % _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_;
			break;
			/*} */
...
#if !BYPASS_REG
{
	...
	cmdqRecWrite(handle, WPE_CTL_MOD_EN_HW, pWPEConfig->WPE_CTL_MOD_EN, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DMA_EN_HW, pWPEConfig->WPE_CTL_DMA_EN, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_CTL_CFG_HW, pWPEConfig->WPE_CTL_CFG, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_FMT_SEL_HW, pWPEConfig->WPE_CTL_FMT_SEL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_INT_EN_HW, pWPEConfig->WPE_CTL_INT_EN, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_CTL_INT_STATUS_HW, pWPEConfig->WPE_CTL_INT_STATUS, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_INT_STATUSX_HW, pWPEConfig->WPE_CTL_INT_STATUSX,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_TDR_TILE_HW, pWPEConfig->WPE_CTL_TDR_TILE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_TDR_DBG_STATUS_HW, pWPEConfig->WPE_CTL_TDR_DBG_STATUS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_TDR_TCM_EN_HW, pWPEConfig->WPE_CTL_TDR_TCM_EN, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_SW_CTL_HW, pWPEConfig->WPE_CTL_SW_CTL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_SPARE0_HW, pWPEConfig->WPE_CTL_SPARE0, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_SPARE1_HW, pWPEConfig->WPE_CTL_SPARE1, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_SPARE2_HW, pWPEConfig->WPE_CTL_SPARE2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DONE_SEL_HW, pWPEConfig->WPE_CTL_DONE_SEL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DBG_SET_HW, pWPEConfig->WPE_CTL_DBG_SET, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DBG_PORT_HW, pWPEConfig->WPE_CTL_DBG_PORT, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DATE_CODE_HW, pWPEConfig->WPE_CTL_DATE_CODE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_PROJ_CODE_HW, pWPEConfig->WPE_CTL_PROJ_CODE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_WPE_DCM_DIS_HW, pWPEConfig->WPE_CTL_WPE_DCM_DIS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DMA_DCM_DIS_HW, pWPEConfig->WPE_CTL_DMA_DCM_DIS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_WPE_DCM_STATUS_HW, pWPEConfig->WPE_CTL_WPE_DCM_STATUS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DMA_DCM_STATUS_HW, pWPEConfig->WPE_CTL_DMA_DCM_STATUS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_WPE_REQ_STATUS_HW, pWPEConfig->WPE_CTL_WPE_REQ_STATUS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DMA_REQ_STATUS_HW, pWPEConfig->WPE_CTL_DMA_REQ_STATUS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_WPE_RDY_STATUS_HW, pWPEConfig->WPE_CTL_WPE_RDY_STATUS,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CTL_DMA_RDY_STATUS_HW, pWPEConfig->WPE_CTL_DMA_RDY_STATUS,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VGEN_CTL_HW, pWPEConfig->WPE_VGEN_CTL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_IN_IMG_HW, pWPEConfig->WPE_VGEN_IN_IMG, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_OUT_IMG_HW, pWPEConfig->WPE_VGEN_OUT_IMG, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_HORI_STEP_HW, pWPEConfig->WPE_VGEN_HORI_STEP, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_VERT_STEP_HW, pWPEConfig->WPE_VGEN_VERT_STEP, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_HORI_INT_OFST_HW, pWPEConfig->WPE_VGEN_HORI_INT_OFST,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_HORI_SUB_OFST_HW, pWPEConfig->WPE_VGEN_HORI_SUB_OFST,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_VERT_INT_OFST_HW, pWPEConfig->WPE_VGEN_VERT_INT_OFST,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_VERT_SUB_OFST_HW, pWPEConfig->WPE_VGEN_VERT_SUB_OFST,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VGEN_POST_CTL_HW, pWPEConfig->WPE_VGEN_POST_CTL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_POST_COMP_X_HW, pWPEConfig->WPE_VGEN_POST_COMP_X,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_POST_COMP_Y_HW, pWPEConfig->WPE_VGEN_POST_COMP_Y,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VGEN_MAX_VEC_HW, pWPEConfig->WPE_VGEN_MAX_VEC, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VFIFO_CTL_HW, pWPEConfig->WPE_VFIFO_CTL, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_CFIFO_CTL_HW, pWPEConfig->WPE_CFIFO_CTL, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_RWCTL_CTL_HW, pWPEConfig->WPE_RWCTL_CTL, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_CACHI_SPECIAL_FUN_EN_HW, pWPEConfig->WPE_CACHI_SPECIAL_FUN_EN,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_C24_TILE_EDGE_HW, pWPEConfig->WPE_C24_TILE_EDGE, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_MDP_CROP_X_HW, pWPEConfig->WPE_MDP_CROP_X, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MDP_CROP_Y_HW, pWPEConfig->WPE_MDP_CROP_Y, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_ISPCROP_CON1_HW, pWPEConfig->WPE_ISPCROP_CON1, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ISPCROP_CON2_HW, pWPEConfig->WPE_ISPCROP_CON2, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_PSP_CTL_HW, pWPEConfig->WPE_PSP_CTL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_PSP2_CTL_HW, pWPEConfig->WPE_PSP2_CTL, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_ADDR_GEN_SOFT_RSTSTAT_0_HW,
		     pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_0, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_BASE_ADDR_0_HW, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_0,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_OFFSET_ADDR_0_HW, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_0,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_STRIDE_0_HW, pWPEConfig->WPE_ADDR_GEN_STRIDE_0,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON_0_HW, pWPEConfig->WPE_CACHI_CON_0, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON2_0_HW, pWPEConfig->WPE_CACHI_CON2_0, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON3_0_HW, pWPEConfig->WPE_CACHI_CON3_0, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_CTRL_0_HW, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_0,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_STAT_0_HW, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_0,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_RSV1_0_HW, pWPEConfig->WPE_ADDR_GEN_RSV1_0,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_DEBUG_SEL_0_HW, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_0,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_ADDR_GEN_SOFT_RSTSTAT_1_HW,
		     pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_1, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_BASE_ADDR_1_HW, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_1,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_OFFSET_ADDR_1_HW, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_1,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_STRIDE_1_HW, pWPEConfig->WPE_ADDR_GEN_STRIDE_1,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON_1_HW, pWPEConfig->WPE_CACHI_CON_1, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON2_1_HW, pWPEConfig->WPE_CACHI_CON2_1, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON3_1_HW, pWPEConfig->WPE_CACHI_CON3_1, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_CTRL_1_HW, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_1,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_STAT_1_HW, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_1,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_RSV1_1_HW, pWPEConfig->WPE_ADDR_GEN_RSV1_1,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_DEBUG_SEL_1_HW, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_1,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_ADDR_GEN_SOFT_RSTSTAT_2_HW,
		     pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_BASE_ADDR_2_HW, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_2,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_OFFSET_ADDR_2_HW, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_2,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_STRIDE_2_HW, pWPEConfig->WPE_ADDR_GEN_STRIDE_2,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON_2_HW, pWPEConfig->WPE_CACHI_CON_2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON2_2_HW, pWPEConfig->WPE_CACHI_CON2_2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON3_2_HW, pWPEConfig->WPE_CACHI_CON3_2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_CTRL_2_HW, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_2,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_STAT_2_HW, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_2,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_RSV1_2_HW, pWPEConfig->WPE_ADDR_GEN_RSV1_2,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_DEBUG_SEL_2_HW, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_2,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_ADDR_GEN_SOFT_RSTSTAT_3_HW,
		     pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_3, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_BASE_ADDR_3_HW, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_3,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_OFFSET_ADDR_3_HW, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_3,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_STRIDE_3_HW, pWPEConfig->WPE_ADDR_GEN_STRIDE_3,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON_3_HW, pWPEConfig->WPE_CACHI_CON_3, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON2_3_HW, pWPEConfig->WPE_CACHI_CON2_3, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_CACHI_CON3_3_HW, pWPEConfig->WPE_CACHI_CON3_3, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_CTRL_3_HW, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_3,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_ERR_STAT_3_HW, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_3,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_RSV1_3_HW, pWPEConfig->WPE_ADDR_GEN_RSV1_3,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_ADDR_GEN_DEBUG_SEL_3_HW, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_3,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_DMA_SOFT_RSTSTAT_HW, pWPEConfig->WPE_DMA_SOFT_RSTSTAT,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_TDRI_BASE_ADDR_HW, pWPEConfig->WPE_TDRI_BASE_ADDR, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_TDRI_OFST_ADDR_HW, pWPEConfig->WPE_TDRI_OFST_ADDR, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_TDRI_XSIZE_HW, pWPEConfig->WPE_TDRI_XSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VERTICAL_FLIP_EN_HW, pWPEConfig->WPE_VERTICAL_FLIP_EN,
		     CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_DMA_SOFT_RESET_HW, pWPEConfig->WPE_DMA_SOFT_RESET, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_LAST_ULTRA_EN_HW, pWPEConfig->WPE_LAST_ULTRA_EN, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_SPECIAL_FUN_EN_HW, pWPEConfig->WPE_SPECIAL_FUN_EN, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_WPEO_BASE_ADDR_HW, pWPEConfig->WPE_WPEO_BASE_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_WPEO_OFST_ADDR_HW, pWPEConfig->WPE_WPEO_OFST_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_WPEO_XSIZE_HW, pWPEConfig->WPE_WPEO_XSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_YSIZE_HW, pWPEConfig->WPE_WPEO_YSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_STRIDE_HW, pWPEConfig->WPE_WPEO_STRIDE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_CON_HW, pWPEConfig->WPE_WPEO_CON, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_CON2_HW, pWPEConfig->WPE_WPEO_CON2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_CON3_HW, pWPEConfig->WPE_WPEO_CON3, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_CROP_HW, pWPEConfig->WPE_WPEO_CROP, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_MSKO_BASE_ADDR_HW, pWPEConfig->WPE_MSKO_BASE_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_MSKO_OFST_ADDR_HW, pWPEConfig->WPE_MSKO_OFST_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_MSKO_XSIZE_HW, pWPEConfig->WPE_MSKO_XSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_YSIZE_HW, pWPEConfig->WPE_MSKO_YSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_STRIDE_HW, pWPEConfig->WPE_MSKO_STRIDE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_CON_HW, pWPEConfig->WPE_MSKO_CON, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_CON2_HW, pWPEConfig->WPE_MSKO_CON2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_CON3_HW, pWPEConfig->WPE_MSKO_CON3, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_CROP_HW, pWPEConfig->WPE_MSKO_CROP, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VECI_BASE_ADDR_HW, pWPEConfig->WPE_VECI_BASE_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VECI_OFST_ADDR_HW, pWPEConfig->WPE_VECI_OFST_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VECI_XSIZE_HW, pWPEConfig->WPE_VECI_XSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VECI_YSIZE_HW, pWPEConfig->WPE_VECI_YSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VECI_STRIDE_HW, pWPEConfig->WPE_VECI_STRIDE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VECI_CON_HW, pWPEConfig->WPE_VECI_CON, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VECI_CON2_HW, pWPEConfig->WPE_VECI_CON2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VECI_CON3_HW, pWPEConfig->WPE_VECI_CON3, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VEC2I_BASE_ADDR_HW, pWPEConfig->WPE_VEC2I_BASE_ADDR,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VEC2I_OFST_ADDR_HW, pWPEConfig->WPE_VEC2I_OFST_ADDR,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VEC2I_XSIZE_HW, pWPEConfig->WPE_VEC2I_XSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC2I_YSIZE_HW, pWPEConfig->WPE_VEC2I_YSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC2I_STRIDE_HW, pWPEConfig->WPE_VEC2I_STRIDE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC2I_CON_HW, pWPEConfig->WPE_VEC2I_CON, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC2I_CON2_HW, pWPEConfig->WPE_VEC2I_CON2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC2I_CON3_HW, pWPEConfig->WPE_VEC2I_CON3, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VEC3I_BASE_ADDR_HW, pWPEConfig->WPE_VEC3I_BASE_ADDR,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VEC3I_OFST_ADDR_HW, pWPEConfig->WPE_VEC3I_OFST_ADDR,
		     CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_VEC3I_XSIZE_HW, pWPEConfig->WPE_VEC3I_XSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC3I_YSIZE_HW, pWPEConfig->WPE_VEC3I_YSIZE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC3I_STRIDE_HW, pWPEConfig->WPE_VEC3I_STRIDE, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC3I_CON_HW, pWPEConfig->WPE_VEC3I_CON, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC3I_CON2_HW, pWPEConfig->WPE_VEC3I_CON2, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC3I_CON3_HW, pWPEConfig->WPE_VEC3I_CON3, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_DMA_ERR_CTRL_HW, pWPEConfig->WPE_DMA_ERR_CTRL, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_WPEO_ERR_STAT_HW, pWPEConfig->WPE_WPEO_ERR_STAT, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_MSKO_ERR_STAT_HW, pWPEConfig->WPE_MSKO_ERR_STAT, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VECI_ERR_STAT_HW, pWPEConfig->WPE_VECI_ERR_STAT, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC2I_ERR_STAT_HW, pWPEConfig->WPE_VEC2I_ERR_STAT, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_VEC3I_ERR_STAT_HW, pWPEConfig->WPE_VEC3I_ERR_STAT, CMDQ_REG_MASK);
	cmdqRecWrite(handle, WPE_DMA_DEBUG_ADDR_HW, pWPEConfig->WPE_DMA_DEBUG_ADDR, CMDQ_REG_MASK);

	cmdqRecWrite(handle, WPE_DMA_DEBUG_SEL_HW, pWPEConfig->WPE_DMA_DEBUG_SEL, CMDQ_REG_MASK);
	...
	WPE_WR32(WPE_CTL_TDR_DBG_STATUS_REG, pWPEConfig->WPE_CTL_TDR_DBG_STATUS);
	...
	WPE_WR32(WPE_CTL_WPE_DCM_STATUS_REG, pWPEConfig->WPE_CTL_WPE_DCM_STATUS);
	WPE_WR32(WPE_CTL_DMA_DCM_STATUS_REG, pWPEConfig->WPE_CTL_DMA_DCM_STATUS);
	WPE_WR32(WPE_CTL_WPE_REQ_STATUS_REG, pWPEConfig->WPE_CTL_WPE_REQ_STATUS);
	WPE_WR32(WPE_CTL_DMA_REQ_STATUS_REG, pWPEConfig->WPE_CTL_DMA_REQ_STATUS);
	WPE_WR32(WPE_CTL_WPE_RDY_STATUS_REG, pWPEConfig->WPE_CTL_WPE_RDY_STATUS);
	WPE_WR32(WPE_CTL_DMA_RDY_STATUS_REG, pWPEConfig->WPE_CTL_DMA_RDY_STATUS);
	...
	WPE_WR32(WPE_VGEN_HORI_INT_OFST_REG, pWPEConfig->WPE_VGEN_HORI_INT_OFST);
	WPE_WR32(WPE_VGEN_HORI_SUB_OFST_REG, pWPEConfig->WPE_VGEN_HORI_SUB_OFST);
	WPE_WR32(WPE_VGEN_VERT_INT_OFST_REG, pWPEConfig->WPE_VGEN_VERT_INT_OFST);
	WPE_WR32(WPE_VGEN_VERT_SUB_OFST_REG, pWPEConfig->WPE_VGEN_VERT_SUB_OFST);

	WPE_WR32(WPE_VGEN_POST_CTL_REG, pWPEConfig->WPE_VGEN_POST_CTL);
	WPE_WR32(WPE_VGEN_POST_COMP_X_REG, pWPEConfig->WPE_VGEN_POST_COMP_X);
	WPE_WR32(WPE_VGEN_POST_COMP_Y_REG, pWPEConfig->WPE_VGEN_POST_COMP_Y);
	...
	WPE_WR32(WPE_CACHI_SPECIAL_FUN_EN_REG, pWPEConfig->WPE_CACHI_SPECIAL_FUN_EN);

	WPE_WR32(WPE_C24_TILE_EDGE_REG, pWPEConfig->WPE_C24_TILE_EDGE);
	...
	WPE_WR32(WPE_ADDR_GEN_SOFT_RSTSTAT_0_REG, pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_0);
	WPE_WR32(WPE_ADDR_GEN_BASE_ADDR_0_REG, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_0);
	WPE_WR32(WPE_ADDR_GEN_OFFSET_ADDR_0_REG, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_0);
	WPE_WR32(WPE_ADDR_GEN_STRIDE_0_REG, pWPEConfig->WPE_ADDR_GEN_STRIDE_0);
	...
	WPE_WR32(WPE_ADDR_GEN_ERR_CTRL_0_REG, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_0);
	WPE_WR32(WPE_ADDR_GEN_ERR_STAT_0_REG, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_0);
	WPE_WR32(WPE_ADDR_GEN_RSV1_0_REG, pWPEConfig->WPE_ADDR_GEN_RSV1_0);
	WPE_WR32(WPE_ADDR_GEN_DEBUG_SEL_0_REG, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_0);

	WPE_WR32(WPE_ADDR_GEN_SOFT_RSTSTAT_1_REG, pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_1);
	WPE_WR32(WPE_ADDR_GEN_BASE_ADDR_1_REG, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_1);
	WPE_WR32(WPE_ADDR_GEN_OFFSET_ADDR_1_REG, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_1);
	...
	WPE_WR32(WPE_ADDR_GEN_ERR_CTRL_1_REG, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_1);
	WPE_WR32(WPE_ADDR_GEN_ERR_STAT_1_REG, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_1);
	WPE_WR32(WPE_ADDR_GEN_RSV1_1_REG, pWPEConfig->WPE_ADDR_GEN_RSV1_1);
	WPE_WR32(WPE_ADDR_GEN_DEBUG_SEL_1_REG, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_1);

	WPE_WR32(WPE_ADDR_GEN_SOFT_RSTSTAT_2_REG, pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_2);
	WPE_WR32(WPE_ADDR_GEN_BASE_ADDR_2_REG, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_2);
	WPE_WR32(WPE_ADDR_GEN_OFFSET_ADDR_2_REG, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_2);
	WPE_WR32(WPE_ADDR_GEN_STRIDE_2_REG, pWPEConfig->WPE_ADDR_GEN_STRIDE_2);
	...
	WPE_WR32(WPE_ADDR_GEN_ERR_CTRL_2_REG, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_2);
	WPE_WR32(WPE_ADDR_GEN_ERR_STAT_2_REG, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_2);
	WPE_WR32(WPE_ADDR_GEN_RSV1_2_REG, pWPEConfig->WPE_ADDR_GEN_RSV1_2);
	WPE_WR32(WPE_ADDR_GEN_DEBUG_SEL_2_REG, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_2);

	WPE_WR32(WPE_ADDR_GEN_SOFT_RSTSTAT_3_REG, pWPEConfig->WPE_ADDR_GEN_SOFT_RSTSTAT_3);
	WPE_WR32(WPE_ADDR_GEN_BASE_ADDR_3_REG, pWPEConfig->WPE_ADDR_GEN_BASE_ADDR_3);
	WPE_WR32(WPE_ADDR_GEN_OFFSET_ADDR_3_REG, pWPEConfig->WPE_ADDR_GEN_OFFSET_ADDR_3);
	WPE_WR32(WPE_ADDR_GEN_STRIDE_3_REG, pWPEConfig->WPE_ADDR_GEN_STRIDE_3);
	...
	WPE_WR32(WPE_ADDR_GEN_ERR_CTRL_3_REG, pWPEConfig->WPE_ADDR_GEN_ERR_CTRL_3);
	WPE_WR32(WPE_ADDR_GEN_ERR_STAT_3_REG, pWPEConfig->WPE_ADDR_GEN_ERR_STAT_3);
	WPE_WR32(WPE_ADDR_GEN_RSV1_3_REG, pWPEConfig->WPE_ADDR_GEN_RSV1_3);
	WPE_WR32(WPE_ADDR_GEN_DEBUG_SEL_3_REG, pWPEConfig->WPE_ADDR_GEN_DEBUG_SEL_3);
...
static bool Check_WPE_Is_Busy(void)
{
	...
	if ((WPE_IS_BUSY == (Ctrl_Fsm & WPE_IS_BUSY)) || (WPE_START == (WPE_Start & WPE_START)))
		return MTRUE;
	...
	for (i = 0; i < _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_; i++) {
		LOG_INF
		    ("WPE Req:State:%d, procID:0x%08X, callerID:0x%08X, enqReqNum:%d, FraWRIdx:%d, FraRDIdx:%d\n",
		     g_WPE_ReqRing.WPEReq_Struct[i].State, g_WPE_ReqRing.WPEReq_Struct[i].processID,
...
static signed int WPE_ReadReg(struct WPE_REG_IO_STRUCT *pRegIo)
{
	...
	for (i = 0; i < pRegIo->Count; i++) {
		...
		if ((ISP_WPE_BASE + reg.Addr >= ISP_WPE_BASE)
		    && (ISP_WPE_BASE + reg.Addr < (ISP_WPE_BASE + WPE_REG_RANGE))
			&& ((reg.Addr & 0x3) == 0)) {
			reg.Val = WPE_RD32(ISP_WPE_BASE + reg.Addr);
		} else {
			LOG_ERR("Wrong address(0x%p)", (ISP_WPE_BASE + reg.Addr));
			reg.Val = 0;
		}
		/*  */
...
static signed int WPE_WaitIrq(struct WPE_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if (WaitIrq->Clear == WPE_IRQ_CLEAR_STATUS) {
		 ...
		 * ,WaitIrq->EventInfo.Clear,WaitIrq->Type,WPEInfo.IrqInfo.Status[WaitIrq->Type]);
		 */
		/* WPEInfo.IrqInfo.Status[WaitIrq->Type][WaitIrq->EventInfo.UserKey] &=
	...
	LOG_INF("before wait_event: WpeIrqCnt(0x%08X), WriteReqIdx(0x%08X), ReadReqIdx(0x%08X)\n",
		WPEInfo.IrqInfo.WpeIrqCnt, WPEInfo.WriteReqIdx, WPEInfo.ReadReqIdx);
#endif

	/* 2. start to wait signal */
	Timeout = wait_event_interruptible_timeout(WPEInfo.WaitQueueHead,
						   WPE_GetIRQState(WaitIrq->Type, WaitIrq->UserKey,
								   WaitIrq->Status, whichReq,
								   WaitIrq->ProcessID),
						   WPE_MsToJiffies(WaitIrq->Timeout));
	...
	if (Timeout == 0) {
		...
		LOG_ERR
		    ("Timeout!!! ERRRR WaitStatus(0x%08X), Timeout(%d),userKey(%d),whichReq(%d),ProcessID(%d)\n",
		     WaitIrq->Status, WaitIrq->Timeout, WaitIrq->UserKey, whichReq,
		     WaitIrq->ProcessID);
		LOG_ERR
		...
		/* Store irqinfo status in here to redeuce time of spin_lock_irqsave */
#ifdef __WPE_KERNEL_PERFORMANCE_MEASURE__
		mt_kernel_trace_begin("WPE_WaitIrq");
#endif
		...
		if (WaitIrq->Clear == WPE_IRQ_WAIT_CLEAR) {
...
#ifdef WPE_MULTIPROCESS_TIMEING_ISSUE
			WPEInfo.ReadReqIdx =
			    (WPEInfo.ReadReqIdx + 1) % _SUPPORT_MAX_WPE_FRAME_REQUEST_;
			...
			if (WaitIrq->Status & WPE_INT_ST) {
				...
				LOG_ERR("WPE_IRQ_WAIT_CLEAR Error, Type(%d), WaitStatus(0x%08X)",
...
static long WPE_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	if (pFile->private_data == NULL) {
		LOG_ERR("private_data is NULL,(process, pid, tgid)=(%s, %d, %d)", current->comm,
			current->pid, current->tgid);
		return -EFAULT;
	}
	/*  */
	...
	case WPE_DUMP_ISR_LOG:
		{
			...
			spin_unlock_irqrestore(&(WPEInfo.SpinLockIrq[WPE_IRQ_TYPE_INT_WPE_ST]),
					       flags);

			IRQ_LOG_PRINTER(WPE_IRQ_TYPE_INT_WPE_ST, currentPPB, _LOG_INF);
			IRQ_LOG_PRINTER(WPE_IRQ_TYPE_INT_WPE_ST, currentPPB, _LOG_ERR);
			break;
		}
	...
	case WPE_WRITE_REGISTER:
		{
			if (copy_from_user(&RegIo, (void *)Param, sizeof(struct WPE_REG_IO_STRUCT)) == 0) {
				...
				LOG_ERR("WPE_WRITE_REGISTER copy_from_user failed");
				Ret = -EFAULT;
			}
			break;
		}
	...
	case WPE_CLEAR_IRQ:
		{
			if (copy_from_user(&ClearIrq, (void *)Param, sizeof(struct WPE_CLEAR_IRQ_STRUCT))
				...
				if ((ClearIrq.Type >= WPE_IRQ_TYPE_AMOUNT) || (ClearIrq.Type < 0)) {
					Ret = -EFAULT;
				...
				spin_lock_irqsave(&(WPEInfo.SpinLockIrq[ClearIrq.Type]), flags);
				WPEInfo.IrqInfo.Status[ClearIrq.Type] &= (~ClearIrq.Status);
				spin_unlock_irqrestore(&(WPEInfo.SpinLockIrq[ClearIrq.Type]),
						       flags);
			} else {
				LOG_ERR("WPE_CLEAR_IRQ copy_from_user failed\n");
				Ret = -EFAULT;
			}
			break;
		}
			...
			if (copy_from_user(&enqueNum, (void *)Param, sizeof(int)) == 0) {
				if (WPE_REQUEST_STATE_EMPTY ==
					...
					spin_lock_irqsave(&
							  (WPEInfo.SpinLockIrq
							   [WPE_IRQ_TYPE_INT_WPE_ST]), flags);
					g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.WriteIdx].
					...
					spin_unlock_irqrestore(&
							       (WPEInfo.SpinLockIrq
								[WPE_IRQ_TYPE_INT_WPE_ST]), flags);
					if (enqueNum > _SUPPORT_MAX_WPE_FRAME_REQUEST_) {
						LOG_ERR
					...
					LOG_ERR
					    ("WPE Enque request state is not empty:%d, writeIdx:%d, readIdx:%d\n",
					     g_WPE_ReqRing.
					     WPEReq_Struct[g_WPE_ReqRing.WriteIdx].State,
					     g_WPE_ReqRing.WriteIdx, g_WPE_ReqRing.ReadIdx);
				}
			} else {
				LOG_ERR("WPE_WPE_EQNUE_NUM copy_from_user failed\n");
				Ret = -EFAULT;
			}
	...
	case WPE_ENQUE:
		{
			if (copy_from_user(&wpe_WpeConfig, (void *)Param, sizeof(struct WPE_Config))
				...
				if ((WPE_REQUEST_STATE_EMPTY ==
					...
					enqueReqNum)) {
					g_WPE_ReqRing.
					    WPEReq_Struct[g_WPE_ReqRing.WriteIdx].WpeFrameStatus
					    [g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.WriteIdx].
					     FrameWRIdx] = WPE_FRAME_STATUS_ENQUE;
					...
					if (g_WPE_ReqRing.WPEReq_Struct
					    [g_WPE_ReqRing.WriteIdx].FrameWRIdx ==
					    g_WPE_ReqRing.
					    WPEReq_Struct[g_WPE_ReqRing.WriteIdx].enqueReqNum) {
				...
				LOG_ERR("WPE_WPE_ENQUE copy_from_user failed\n");
				Ret = -EFAULT;
			}
	...
	case WPE_ENQUE_REQ:
		{
			if (copy_from_user(&wpe_WpeReq, (void *)Param, sizeof(struct WPE_Request)) == 0) {
				...
				if (wpe_WpeReq.m_ReqNum > _SUPPORT_MAX_WPE_FRAME_REQUEST_) {
					LOG_ERR("WPE Enque Num is bigger than enqueNum:%d\n",
						wpe_WpeReq.m_ReqNum);
					Ret = -EFAULT;
					goto EXIT;
				}
				...
				mutex_lock(&gWpeMutex);	/* Protect the Multi Process */
				...
				if (WPE_REQUEST_STATE_EMPTY ==
					...
					for (idx = 0; idx < wpe_WpeReq.m_ReqNum; idx++) {
						g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.
									    WriteIdx].WpeFrameStatus
			...
			if ((WPE_REQUEST_STATE_FINISHED ==
				...
				if (g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].
					...
					g_WPE_ReqRing.
					    ...
					    [g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].
					     FrameRDIdx++] = WPE_FRAME_STATUS_EMPTY;
				}
				if (g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].FrameRDIdx ==
				    g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.
								ReadIdx].enqueReqNum) {
					g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].State =
			...
			if (copy_from_user(&wpe_WpeReq, (void *)Param, sizeof(struct WPE_Request)) == 0) {
				...
				for (idx = 0; idx < dequeNum; idx++) {
					if (WPE_FRAME_STATUS_FINISHED ==
						...
						memcpy(&g_WpeDequeReq_Struct.WpeFrameConfig[idx],
						       &g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.
										    ReadIdx].WpeFrameConfig
						       [g_WPE_ReqRing.WPEReq_Struct
							[g_WPE_ReqRing.ReadIdx].FrameRDIdx],
						...
						LOG_ERR
						    ("Err!id(%d),deqNum(%d),RdId(%d),FraRDId(%d),WPEFraSta(%d)\n",
						     idx, dequeNum, g_WPE_ReqRing.ReadIdx,
						     g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.
										 ReadIdx].
						     FrameRDIdx,
						     g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.
										 ReadIdx].WpeFrameStatus
						     [g_WPE_ReqRing.
						      WPEReq_Struct[g_WPE_ReqRing.ReadIdx].
						      FrameRDIdx]);
					}
				}
				g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].State =
				...
				g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].FrameRDIdx = 0;
				g_WPE_ReqRing.WPEReq_Struct[g_WPE_ReqRing.ReadIdx].enqueReqNum = 0;
				g_WPE_ReqRing.ReadIdx =
				    (g_WPE_ReqRing.ReadIdx +
				     1) % _SUPPORT_MAX_WPE_REQUEST_RING_SIZE_;
				...
				LOG_ERR("WPE_CMD_WPE_DEQUE_REQ copy_from_user failed\n");
				Ret = -EFAULT;
			}
	...
	case WPE_DEQUE_DONE:
		{
			if (copy_from_user(&wpe_WpeReq, (void *)Param, sizeof(struct WPE_Request)) == 0) {
				...
				LOG_ERR("WPE_CMD_WPE_DEQUE_DONE copy_from_user failed\n");
				Ret = -EFAULT;
			}
...
static long WPE_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret;
	...
	case COMPAT_WPE_READ_REGISTER:
		{
			...
			if (err) {
				LOG_INF("compat_get_WPE_read_register_data error!!!\n");
				return err;
			}
			ret =
			...
			if (err) {
				LOG_INF("compat_put_WPE_read_register_data error!!!\n");
				return err;
			}
			return ret;
		}
	...
	case COMPAT_WPE_ENQUE_REQ:
		{
			...
			ret = filp->f_op->unlocked_ioctl(filp, WPE_ENQUE_REQ, (unsigned long)data);
	...
	case COMPAT_WPE_DEQUE_REQ:
		{
			...
			ret = filp->f_op->unlocked_ioctl(filp, WPE_DEQUE_REQ, (unsigned long)data);
	...
	case COMPAT_WPE_DEQUE_DONE:
		{
			...
			ret = filp->f_op->unlocked_ioctl(filp, WPE_DEQUE_DONE, (unsigned long)data);
			err = compat_put_WPE_deque_req_data(data32, data);
			if (err) {
				LOG_INF("COMPAT_WPE_WPE_DEQUE_REQ error!!!\n");
				return err;
...
static signed int WPE_open(struct inode *pInode, struct file *pFile)
{
	...
	if (WPEInfo.UserCount > 0) {
		...
		LOG_DBG("Curr UserCount(%d), (process, pid, tgid)=(%s, %d, %d), users exist",
			WPEInfo.UserCount, current->comm, current->pid, current->tgid);
		goto EXIT;
	} else {
		WPEInfo.UserCount++;
		spin_unlock(&(WPEInfo.SpinLockWPERef));
...
static signed int WPE_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	LOG_INF("WPE_mmap:pVma->vm_pgoff(0x%lx),pfn(0x%x),phy(0x%lx)\n",
		pVma->vm_pgoff, pfn, pVma->vm_pgoff << PAGE_SHIFT);
	LOG_INF("WPE_mmap:pVmapVma->vm_start(0x%lx),pVma->vm_end(0x%lx),length(0x%lx)\n",
		pVma->vm_start, pVma->vm_end, length);

	switch (pfn) {
	case WPE_BASE_HW:
		if (length > WPE_REG_RANGE) {
			LOG_ERR("mmap range error :module:0x%x length(0x%lx),WPE_REG_RANGE(0x%x)!",
...
static signed int WPE_probe(struct platform_device *pDev)
{
	...
	nr_WPE_devs += 1;
	_wpe_dev = krealloc(WPE_devs, sizeof(struct WPE_device) * nr_WPE_devs, GFP_KERNEL);
	if (!_wpe_dev) {
		dev_err(&pDev->dev, "Unable to allocate WPE_devs\n");
		return -ENOMEM;
	}
	WPE_devs = _wpe_dev;
	...
	if (!WPE_dev->regs) {
		dev_err(&pDev->dev,
			"Unable to ioremap registers, of_iomap fail, nr_WPE_devs=%d, devnode(%s).\n",
	...
	if (WPE_dev->irq > 0) {
		...
		for (i = 0; i < WPE_IRQ_TYPE_AMOUNT; i++) {
			if (strcmp(pDev->dev.of_node->name, WPE_IRQ_CB_TBL[i].device_name) == 0) {
				Ret =
				    request_irq(WPE_dev->irq,
						(irq_handler_t) WPE_IRQ_CB_TBL[i].isr_fp,
						irq_info[2],
						(const char *)WPE_IRQ_CB_TBL[i].device_name, NULL);
				if (Ret) {
					dev_err(&pDev->dev,
						"Unable to request IRQ, request_irq fail, nr_WPE_devs=%d, devnode(%s), irq=%d, ISR: %s\n",
						nr_WPE_devs, pDev->dev.of_node->name, WPE_dev->irq,
						WPE_IRQ_CB_TBL[i].device_name);
					return Ret;
				}

				LOG_INF("nr_WPE_devs=%d, devnode(%s), irq=%d, ISR: %s\n",
					nr_WPE_devs, pDev->dev.of_node->name, WPE_dev->irq,
					WPE_IRQ_CB_TBL[i].device_name);
				break;
			}
		...
		if (i >= WPE_IRQ_TYPE_AMOUNT) {
			LOG_INF("No corresponding ISR!!: nr_WPE_devs=%d, devnode(%s), irq=%d\n",
				nr_WPE_devs, pDev->dev.of_node->name, WPE_dev->irq);
		}
	...
	if (nr_WPE_devs == 1) {
...
#ifndef __WPE_EP_NO_CLKMGR__
#if !defined(CONFIG_MTK_LEGACY) && defined(CONFIG_COMMON_CLK) /*CCF*/
		wpe_clk.CG_IMGSYS_WPE_A = devm_clk_get(&pDev->dev, "WPE_CLK_IMG_WPE_A");
		...
		dev = device_create(pWPEClass, NULL, WPEDevNo, NULL, WPE_DEV_NAME);
		if (IS_ERR(dev)) {
			Ret = PTR_ERR(dev);
		...
		wake_lock_init(&WPE_wake_lock, WAKE_LOCK_SUSPEND, "WPE_lock_wakelock");

		for (i = 0; i < WPE_IRQ_TYPE_AMOUNT; i++)
			tasklet_init(WPE_tasklet[i].pWPE_tkt, WPE_tasklet[i].tkt_cb, 0);
...
#ifndef __WPE_EP_NO_CLKMGR__
#if !defined(CONFIG_MTK_LEGACY) && defined(CONFIG_COMMON_CLK) /*CCF*/
		wpe_clk.CG_IMGSYS_WPE_B = devm_clk_get(&pDev->dev, "WPE_CLK_IMG_WPE_B");
...
int WPE_pm_suspend(struct device *device)
{
	...
	LOG_DBG("calling %s()\n", __func__);
	LOG_DBG("WPE suspend g_u4EnableClockCount: %d, g_u4WpeCnt: %d", g_u4EnableClockCount, g_u4WpeCnt);
...
int WPE_pm_resume(struct device *device)
{
	...
	LOG_DBG("calling %s()\n", __func__);
	LOG_DBG("WPE resume g_u4EnableClockCount: %d, g_u4WpeCnt: %d", g_u4EnableClockCount, g_u4WpeCnt);
...
/*static int WPE_reg_write(struct file *file, const char __user *buffer, size_t count, loff_t *data)*/

static ssize_t wpe_reg_write(struct file *file, const char __user *buffer, size_t count,
			     loff_t *data)
{
	char desc[128];
	int len = 0;
...
static signed int __init WPE_Init(void)
{
	...
	/* proc_entry = proc_create("pll_test", S_IRUGO | S_IWUSR, isp_wpe_dir, &pll_test_proc_fops); */

	proc_entry = proc_create("wpe_dump", S_IRUGO, isp_wpe_dir, &WPE_dump_proc_fops);

	proc_entry = proc_create("wpe_reg", S_IRUGO | S_IWUSR, isp_wpe_dir, &WPE_reg_proc_fops);


	/* isr log */
	if (PAGE_SIZE <
	    ((WPE_IRQ_TYPE_AMOUNT * NORMAL_STR_LEN * ((DBG_PAGE + INF_PAGE + ERR_PAGE) + 1)) *
	     LOG_PPNUM)) {
		i = 0;
		while (i <
	...
	cmdqCoreRegisterCB(CMDQ_GROUP_WPE,
			   WPE_ClockOnCallback,
			   WPE_DumpCallback, WPE_ResetCallback, WPE_ClockOffCallback);
#endif
...
static irqreturn_t ISP_Irq_WPE(signed int Irq, void *DeviceId)
{
	...
	if (WPE_INT_ST == (WPE_INT_ST & WPEStatus)) {
		...
		if (bResulst == 1) {
			...
			WPEInfo.IrqInfo.ProcessID[WPE_PROCESS_ID_WPE] = ProcessID;
			WPEInfo.IrqInfo.WpeIrqCnt++;
			WPEInfo.ProcessID[WPEInfo.WriteReqIdx] = ProcessID;
			WPEInfo.WriteReqIdx =
			    (WPEInfo.WriteReqIdx + 1) % _SUPPORT_MAX_WPE_FRAME_REQUEST_;
#ifdef WPE_MULTIPROCESS_TIMEING_ISSUE
			/* check the write value is equal to read value ? */
			/* actually, it doesn't happen!! */
			if (WPEInfo.WriteReqIdx == WPEInfo.ReadReqIdx) {
				IRQ_LOG_KEEPER(WPE_IRQ_TYPE_INT_WPE_ST, m_CurrentPPB, _LOG_ERR,
					       "ISP_Irq_WPE Err!!, WriteReqIdx:0x%x, ReadReqIdx:0x%x\n",
					       WPEInfo.WriteReqIdx, WPEInfo.ReadReqIdx);
			}
#endif
	...
	IRQ_LOG_KEEPER(WPE_IRQ_TYPE_INT_WPE_ST, m_CurrentPPB, _LOG_INF,
		       "ISP_Irq_WPE:%d, reg 0x%x : 0x%x, bResulst:%d, wpeHWSta:0x%x, wpeIrqCnt:0x%x, WriteReqIdx:0x%x, ReadReqIdx:0x%x\n",
		       Irq, WPE_CTL_INT_STATUS_HW, WPEStatus, bResulst, WPEStatus,
		       WPEInfo.IrqInfo.WpeIrqCnt, WPEInfo.WriteReqIdx, WPEInfo.ReadReqIdx);
	/* IRQ_LOG_KEEPER(WPE_IRQ_TYPE_INT_WPE_ST, m_CurrentPPB, _LOG_INF, "DveHWSta:0x%x, WPEHWSta:0x%x,
