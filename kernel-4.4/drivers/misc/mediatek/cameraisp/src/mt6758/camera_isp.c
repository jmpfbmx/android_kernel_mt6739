...
#define LOG_PR_WARN(format, args...)    pr_warn(MyTag "[%s] " format, __func__, ##args)
#define LOG_PR_ERR(format, args...)    pr_err(MyTag "[%s] " format, __func__, ##args)
#define LOG_PR_ALERT(format, args...)    pr_alert(MyTag "[%s] " format, __func__, ##args)
...
#define AEE_DUMP_BY_USING_ION_MEMORY /*disable for EP*/
...
/*save ion fd*/
#define ENABLE_KEEP_ION_HANDLE /*disable for EP*/
...
static struct T_ION_TBL gION_TBL[ISP_DEV_NODE_NUM] = {
	...
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_CAM_A_IDX, (signed int *)G_WRDMA_IonCt[0], (signed int *)G_WRDMA_IonFd[0],
	(struct ion_handle **)G_WRDMA_IonHnd[0], (spinlock_t *)SpinLock_IonHnd[0]},
	{ISP_CAM_B_IDX, (signed int *)G_WRDMA_IonCt[1], (signed int *)G_WRDMA_IonFd[1],
	(struct ion_handle **)G_WRDMA_IonHnd[1], (spinlock_t *)SpinLock_IonHnd[1]},
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
	{ISP_DEV_NODE_NUM, NULL, NULL, NULL, NULL},
...
#define IRQ_LOG_KEEPER(irq, ppb, logT, fmt, ...) do {\
	...
	if (avaLen > 1) {\
		...
		if ('\0' != gSvLog[irq]._str[ppb][logT][str_leng - 1]) {\
			LOG_PR_ERR("log str over flow(%d)", irq);\
		} \
		while (*ptr++ != '\0') {        \
			(*ptr2)++;\
		}     \
...
static void ISP_DumpDmaDeepDbg(enum ISP_IRQ_TYPE_ENUM module)
{
	...
	dmaerr[_pdi_] = (unsigned int)ISP_RD32(CAM_REG_PDI_ERR_STAT(regModule));
	...
	if (regModule == ISP_CAM_B_IDX) {
		dmaerr[_eiso_] = (unsigned int)ISP_RD32(CAM_UNI_REG_B_EISO_ERR_STAT(ISP_UNI_A_IDX));
		dmaerr[_flko_] = (unsigned int)ISP_RD32(CAM_UNI_REG_B_FLKO_ERR_STAT(ISP_UNI_A_IDX));
		dmaerr[_rsso_] = (unsigned int)ISP_RD32(CAM_UNI_REG_B_RSSO_A_ERR_STAT(ISP_UNI_A_IDX));
		dmaerr[_rawi_] = (unsigned int)ISP_RD32(CAM_UNI_REG_B_RAWI_ERR_STAT(ISP_UNI_A_IDX));
	}
...
static signed int ISP_DumpReg(void)
{
	...
	/* hbin */
	LOG_INF("[%s] [0x%08X %08X],[0x%08X %08X]", (unsigned int)(ISP_TPIPE_ADDR + 0x4f0),
	...
	/* seninf1 */
	LOG_INF("[0x%08X %08X],[0x%08X %08X]", (unsigned int)(ISP_TPIPE_ADDR + 0x4008),
...
static signed int ISP_DumpDIPReg(void)
{
	...
	/*  */
	LOG_INF("- E.");
	LOG_INF("g_bDumpPhyISPBuf:(0x%x), g_pPhyISPBuffer:(0x%p), g_bIonBuf:(0x%x)\n", g_bDumpPhyISPBuf,
	...
	if (g_bDumpPhyISPBuf == MFALSE) {
		...
		if (g_pPhyISPBuffer != NULL) {
			for (i = 0; i < (ISP_DIP_PHYSICAL_REG_SIZE >> 2); i = i + 4) {
				...
				g_pPhyISPBuffer[i+3] = ISP_RD32(ISP_DIP_A_BASE + ((i+3)*4));
			}
		} else {
		...
		for (cmdqidx = 0; cmdqidx < 32 ; cmdqidx++) {
			if (ctrl_start & (0x1<<cmdqidx)) {
				g_cmdqaddr = ISP_RD32(ISP_DIP_A_BASE + 0x108 + (cmdqidx*12));
				break;
			}
		}
		...
		if ((g_CmdqBaseAddrInfo.MemPa != 0) && (g_CmdqBaseAddrInfo.MemVa != NULL)
			...
			if (copy_from_user(g_pKWCmdqBuffer, (void __user *)(OffsetAddr),
				...
				LOG_PR_ERR("cpy cmdq fail. cmdqaddr:0x%x, MemVa:0x%lx, MemPa:0x%x, offset:0x%x\n",
		...
		if ((g_TpipeBaseAddrInfo.MemPa != 0) && (g_TpipeBaseAddrInfo.MemVa != NULL)) {
			...
			if (copy_from_user(g_KWTpipeBuffer, (void __user *)(OffsetAddr),
				MAX_ISP_TILE_TDR_HEX_NO) != 0) {
				LOG_PR_ERR("cpy tpipe fail.tdriaddr:0x%x,MVa:0x%lx,MPa:0x%x,ofset:0x%x\n",
				g_tdriaddr, (uintptr_t)g_TpipeBaseAddrInfo.MemVa,
				g_TpipeBaseAddrInfo.MemPa, offset);
			}
	...
	LOG_INF("direct link:15020030(0x%x),g_bDumpPhyISPBuf:(0x%x),cmdqidx(0x%x),cmdqaddr(0x%x),tdriaddr(0x%x)\n",
		ISP_RD32(ISP_IMGSYS_CONFIG_BASE + 0x0030), g_bDumpPhyISPBuf, cmdqidx, g_cmdqaddr, g_tdriaddr);
...
static inline void Prepare_Enable_ccf_clock(void)
{
	...
	smi_bus_enable(SMI_LARB5, ISP_DEV_NAME);
	smi_bus_enable(SMI_LARB2, ISP_DEV_NAME);
	#endif

	ret = clk_prepare_enable(isp_clk.ISP_SCP_SYS_MM0);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_SCP_SYS_MM0 clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_SCP_SYS_ISP);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_SCP_SYS_ISP clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_SCP_SYS_CAM);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_SCP_SYS_CAM clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_IMG_DIP);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_IMG_DIP clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_IMG_RSC);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_IMG_RSC clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_CAM_CAMSYS);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_CAM_CAMSYS clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_CAM_CAMTG);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_CAM_CAMTG clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_CAM_SENINF);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_CAM_SENINF clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_CAM_CAMSV0);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_CAM_CAMSV0 clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_CAM_CAMSV1);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_CAM_CAMSV1 clock\n");

	ret = clk_prepare_enable(isp_clk.ISP_CAM_CAMSV2);
	if (ret)
		LOG_PR_ERR("cannot pre-en ISP_CAM_CAMSV2 clock\n");
}
...
static inline void Disable_Unprepare_ccf_clock(void)
{
	...
	smi_bus_disable(SMI_LARB5, ISP_DEV_NAME);
...
static inline void Prepare_Enable_cg_clock(void)
{
	...
	smi_clk_prepare(SMI_LARB6, ISP_DEV_NAME, 0);
	smi_clk_enable(SMI_LARB6, ISP_DEV_NAME, 0);
	#endif
...
static inline void Disable_Unprepare_cg_clock(void)
{
	...
	smi_clk_disable(SMI_LARB6, ISP_DEV_NAME, 0);
	smi_clk_unprepare(SMI_LARB6, ISP_DEV_NAME, 0);
	#endif
}
...
static inline void ISP_Reset(signed int module)
{
	...
	default:
		LOG_PR_ERR("Not support reset module:%d\n", module);
		break;
	}
...
static signed int ISP_ReadReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	case 0xFE:
		...
		break;
	default:
		LOG_PR_ERR("Unsupported module(%x) !!!\n", module);
		Ret = -EFAULT;
		goto EXIT;
	}
	...
	for (i = 0; i < pRegIo->Count; i++) {
		...
		if (((regBase + reg.Addr) < (regBase + PAGE_SIZE))) {
			...
			LOG_PR_ERR("Wrong address(0x%lx)\n", (unsigned long)(regBase + reg.Addr));
			reg.Val = 0;
		}
		/*  */
...
static signed int isp_allocbuf(struct isp_imem_memory *pMemInfo)
{
	...
	if (pMemInfo == NULL) {
		LOG_PR_ERR("pMemInfo is NULL!!\n");
		ret = -ENOMEM;
		goto isp_allocbuf_exit;
	}

	if (isp_p2_ion_client == NULL) {
		LOG_PR_ERR("isp_p2_ion_client is NULL!!\n");
		ret = -ENOMEM;
		goto isp_allocbuf_exit;
	}

	handle = ion_alloc(isp_p2_ion_client, pMemInfo->length, 0, ION_HEAP_MULTIMEDIA_MASK, 0);
	if (handle == NULL) {
		LOG_PR_ERR("fail to alloc ion buffer, ret=%d\n", ret);
		ret = -ENOMEM;
		goto isp_allocbuf_exit;
	}
	...
	if (pMemInfo->va == 0) {
		LOG_PR_ERR("fail to map va of buffer!\n");
		ret = -ENOMEM;
		goto isp_allocbuf_exit;
	}
	...
	ret = ion_kernel_ioctl(isp_p2_ion_client, ION_CMD_MULTIMEDIA, (unsigned long)&mm_data);
	if (ret) {
		LOG_PR_ERR("fail to config ion buffer, ret=%d\n", ret);
		ret = -ENOMEM;
		goto isp_allocbuf_exit;
	}
	...
	ret = ion_kernel_ioctl(isp_p2_ion_client, ION_CMD_SYSTEM, (unsigned long)&sys_data);
...
static signed int ISP_DumpBuffer(struct ISP_DUMP_BUFFER_STRUCT *pDumpBufStruct)
{
	...
	if (pDumpBufStruct->BytesofBufferSize > 0xFFFFFFFF) {
		LOG_PR_ERR("pDumpTuningBufStruct->BytesofBufferSize error");
		Ret = -EFAULT;
		goto EXIT;
	}
	/*  */
	if ((void __user *)(pDumpBufStruct->pBuffer) == NULL) {
		LOG_PR_ERR("NULL pDumpBufStruct->pBuffer");
		Ret = -EFAULT;
		goto EXIT;
	}
    /* Native Exception */
	...
	case ISP_DUMP_TPIPEBUF_CMD:
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pTpipeBuffer == NULL)
				...
				LOG_PR_ERR("g_pTpipeBuffer:0x%pK is not NULL!!", g_pTpipeBuffer);
		}
		if (g_pTpipeBuffer != NULL) {
			if (copy_from_user(g_pTpipeBuffer, (void __user *)(pDumpBufStruct->pBuffer),
				pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_PR_ERR("copy_from_user g_pTpipeBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_PR_ERR("g_pTpipeBuffer kmalloc failed, g_bIonBufAllocated:%d\n",
				g_bIonBufferAllocated);
		}
#else
		if (copy_from_user(g_TpipeBuffer, (void __user *)(pDumpBufStruct->pBuffer),
			pDumpBufStruct->BytesofBufferSize) != 0) {
			LOG_PR_ERR("copy_from_user g_TpipeBuffer failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
#endif
	...
	case ISP_DUMP_TUNINGBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > ISP_DIP_REG_SIZE) {
			LOG_PR_ERR("tuning buf size error, size:0x%x",
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pTuningBuffer == NULL)
				...
				LOG_PR_ERR("g_TuningBuffer:0x%pK is not NULL!!", g_pTuningBuffer);
		}
		if (g_pTuningBuffer != NULL) {
			if (copy_from_user(g_pTuningBuffer, (void __user *)(pDumpBufStruct->pBuffer),
				pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_PR_ERR("copy_from_user g_pTuningBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_PR_ERR("ERROR: g_TuningBuffer kmalloc failed\n");
		}
#else
		if (copy_from_user(g_TuningBuffer, (void __user *)(pDumpBufStruct->pBuffer),
			pDumpBufStruct->BytesofBufferSize) != 0) {
			LOG_PR_ERR("copy_from_user g_TuningBuffer failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
#endif
	...
	case ISP_DUMP_ISPVIRBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > ISP_DIP_REG_SIZE) {
			LOG_PR_ERR("vir isp buffer size error, size:0x%x",
				pDumpBufStruct->BytesofBufferSize);
			Ret = -EFAULT;
			goto EXIT;
		}
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pVirISPBuffer == NULL)
				...
				LOG_PR_ERR("g_pVirISPBuffer:0x%pK is not NULL!!", g_pVirISPBuffer);
		}
		if (g_pVirISPBuffer != NULL) {
			if (copy_from_user(g_pVirISPBuffer, (void __user *)(pDumpBufStruct->pBuffer),
				pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_PR_ERR("copy_from_user g_pVirISPBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_PR_ERR("ERROR: g_pVirISPBuffer kmalloc failed\n");
		}
#else
		if (copy_from_user(g_VirISPBuffer, (void __user *)(pDumpBufStruct->pBuffer),
			pDumpBufStruct->BytesofBufferSize) != 0) {
			LOG_PR_ERR("copy_from_user g_VirISPBuffer failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
#endif
	...
	case ISP_DUMP_CMDQVIRBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > MAX_ISP_CMDQ_BUFFER_SIZE) {
			LOG_PR_ERR("cmdq buffer size error, size:0x%x",
				pDumpBufStruct->BytesofBufferSize);
			Ret = -EFAULT;
			goto EXIT;
		}
		...
		if (g_bIonBufferAllocated == MFALSE) {
			if (g_pCmdqBuffer == NULL)
				...
				LOG_PR_ERR("g_pCmdqBuffer:0x%pK is not NULL!!", g_pCmdqBuffer);
		}
		if (g_pCmdqBuffer != NULL) {
			if (copy_from_user(g_pCmdqBuffer, (void __user *)(pDumpBufStruct->pBuffer),
				pDumpBufStruct->BytesofBufferSize) != 0) {
				LOG_PR_ERR("copy_from_user g_pCmdqBuffer failed\n");
				Ret = -EFAULT;
				goto EXIT;
			}
		} else {
			LOG_PR_ERR("ERROR: g_pCmdqBuffer kmalloc failed\n");
		}
#else
		...
		break;
	default:
		LOG_PR_ERR("error dump buffer cmd:%d", pDumpBufStruct->DumpCmd);
		break;
	}
...
static signed int ISP_SetMemInfo(struct ISP_MEM_INFO_STRUCT *pMemInfoStruct)
{
	...
	if ((void __user *)(pMemInfoStruct->MemVa) == NULL) {
		LOG_PR_ERR("NULL pMemInfoStruct->MemVa");
		Ret = -EFAULT;
		goto EXIT;
	}
	...
	case ISP_MEMORY_INFO_CMDQ_CMD:
		...
		break;
	default:
		LOG_PR_ERR("error set memory info cmd:%d", pMemInfoStruct->MemInfoCmd);
		break;
	}
	/*  */
EXIT:
...
static long ISP_Buf_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)Param, sizeof(struct ISP_BUFFER_CTRL_STRUCT)) == 0) {
		if (rt_buf_ctrl.module >= ISP_IRQ_TYPE_AMOUNT) {
			LOG_PR_ERR("[rtbc]not supported module:0x%x\n", rt_buf_ctrl.module);
			return -EFAULT;
		}

		if (pstRTBuf[rt_buf_ctrl.module] == NULL)  {
			LOG_PR_ERR("[rtbc]NULL pstRTBuf, module:0x%x\n", rt_buf_ctrl.module);
			return -EFAULT;
		}

		rt_dma = rt_buf_ctrl.buf_id;
		if (rt_dma >= _cam_max_) {
			LOG_PR_ERR("[rtbc]buf_id error:0x%x\n", rt_dma);
			return -EFAULT;
		}
		...
		case ISP_RT_BUF_CTRL_DMA_EN: {
			...
			for (z = 0; z < _cam_max_; z++) {
				if (get_user(array[z], (unsigned char *)pExt) == 0) {
					...
					LOG_PR_ERR("[rtbc][DMA_EN]:get_user failed(%d)", z);
					Ret = -EFAULT;
				}
				pExt++;
			}
...
static signed int ISP_P2_BufQue_CTRL_FUNC(struct ISP_P2_BUFQUE_STRUCT param)
{
	...
	case ISP_P2_BUFQUE_CTRL_ENQUE_FRAME:    /* signal that a specific buffer is enqueued */
		...
		P2_FramePack_List_Idx[property].start && (P2_FramePack_List_Idx[property].end !=  -1)) {
			LOG_PR_ERR("pty(%d), F/L(%d_%d,%d), dCQ(%d,%d), RF/C/L(%d,%d,%d), sts(%d,%d,%d)",
			...
			spin_unlock(&(SpinLock_P2FrameList));
			LOG_PR_ERR("p2 frame package list is full, enque Fail.");
			ret =  -EFAULT;
			return ret;
		}
		{
	...
	case ISP_P2_BUFQUE_CTRL_WAIT_DEQUE:    /* a dequeue thread is waiting to do dequeue */
		...
		spin_unlock(&(SpinLock_P2FrameList));
		if (idx ==  -1) {
			LOG_PR_ERR("Do not find match buffer (pty/pid/cid: %d/0x%x/0x%x) to deque!",
		...
		}
		{
			...
			if (restTime == 0) {
				LOG_PR_ERR("Wait Deque fail, idx(%d), pty(%d), pID(0x%x),cID(0x%x)",
				idx, param.property, param.processID, param.callerID);
				ret =  -EFAULT;
			} else if (restTime == -512) {
				LOG_PR_ERR("be stopped, restime(%d)", restTime);
				ret =  -EFAULT;
				break;
			}
		}
	...
	case ISP_P2_BUFQUE_CTRL_DEQUE_SUCCESS:             /* signal that a buffer is dequeued(success) */
		...
		if (idx2 ==  -1) {
			spin_unlock(&(SpinLock_P2FrameList));
			LOG_PR_ERR("ERRRRRRRRRRR findmatch index 2 fail (%d_0x%x_0x%x_%d, %d_%d)",
				param.property, param.processID, param.callerID,
		...
		if (idx ==  -1) {
			spin_unlock(&(SpinLock_P2FrameList));
			LOG_PR_ERR("ERRRRRRRRRRR findmatch index 1 fail (%d_0x%x_0x%x_%d, %d_%d)",
				param.property, param.processID, param.callerID,
				param.frameNum, param.cQIdx, param.dupCQIdx);
			ret =  -EFAULT;
			return ret;
	...
	case ISP_P2_BUFQUE_CTRL_WAIT_FRAME:             /* wait for a specific buffer */
		...
		*/
		...
		if (restTime == 0) {
			LOG_PR_ERR("could not find match buffer restTime(%d), pty/pID/cID (%d/0x%x/0x%x),idx(%d)",
		...
		}
		{
			...
			if (restTime == 0) {
				LOG_PR_ERR("Dequeue Buffer fail, rT(%d),idx(%d) pty(%d), pID(0x%x),cID(0x%x)\n",
	...
	case ISP_P2_BUFQUE_CTRL_CLAER_ALL:           /* free all recored dequeued buffer */
		...
		break;
	default:
		LOG_PR_ERR("do not support this ctrl cmd(%d)", param.ctrl);
		break;
	}
	return ret;
}
...
static signed int ISP_MARK_IRQ(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->Type >= ISP_IRQ_TYPE_AMOUNT) {
		LOG_PR_ERR("MARK_IRQ: type error(%d)", irqinfo->Type);
		return -EFAULT;
	}

	if (irqinfo->EventInfo.UserKey >= IRQ_USER_NUM_MAX || irqinfo->EventInfo.UserKey < 0) {
		LOG_PR_ERR("MARK_IRQ: userkey error(%d)", irqinfo->EventInfo.UserKey);
		return -EFAULT;
	}
	...
	if (irqinfo->EventInfo.St_type == SIGNAL_INT) {
		...
		LOG_PR_ERR("Not support DMA interrupt type(%d), Only support signal interrupt!!!",
...
static signed int ISP_GET_MARKtoQEURY_TIME(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->EventInfo.St_type == SIGNAL_INT) {
		...
		if (irqinfo->EventInfo.Status & IspInfo.IrqInfo.MarkedFlag[irqinfo->Type][irqinfo->EventInfo.UserKey]) {
			...
			LOG_PR_ERR("plz mark irq first, userKey/Type/Status (%d/%d/0x%x)",
...
static signed int ISP_FLUSH_IRQ(struct ISP_WAIT_IRQ_STRUCT *irqinfo)
{
	...
	if (irqinfo->Type >= ISP_IRQ_TYPE_AMOUNT) {
		LOG_PR_ERR("FLUSH_IRQ: type error(%d)", irqinfo->Type);
		return -EFAULT;
	}

	if (irqinfo->EventInfo.St_type >= ISP_IRQ_ST_AMOUNT) {
		LOG_PR_ERR("FLUSH_IRQ: st_type error(%d)", irqinfo->EventInfo.St_type);
		return -EFAULT;
	}

	if (irqinfo->EventInfo.UserKey >= IRQ_USER_NUM_MAX || irqinfo->EventInfo.UserKey < 0) {
		LOG_PR_ERR("FLUSH_IRQ: userkey error(%d)", irqinfo->EventInfo.UserKey);
		return -EFAULT;
	}
...
static signed int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT *WaitIrq)
{
	...
	if (WaitIrq->Type >= ISP_IRQ_TYPE_AMOUNT) {
		LOG_PR_ERR("WaitIrq: type error(%d)", WaitIrq->Type);
		return -EFAULT;
	}

	if (WaitIrq->EventInfo.St_type >= ISP_IRQ_ST_AMOUNT) {
		LOG_PR_ERR("WaitIrq: st_type error(%d)", WaitIrq->EventInfo.St_type);
		return -EFAULT;
	}

	if (WaitIrq->EventInfo.UserKey >= IRQ_USER_NUM_MAX || WaitIrq->EventInfo.UserKey < 0) {
		LOG_PR_ERR("WaitIrq: userkey error(%d)", WaitIrq->EventInfo.UserKey);
		return -EFAULT;
	}
...
#endif
		LOG_PR_ERR("marked, need p2 member to review\n");
	}
#endif
...
static struct ion_handle *ISP_ion_import_handle(struct ion_client *client, int fd)
{
	...
	if (IS_ERR(handle)) {
		LOG_PR_ERR("import ion handle failed!\n");
		return NULL;
	}
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	if (pFile->private_data == NULL) {
		LOG_PR_ERR("private_data is NULL,(process, pid, tgid)=(%s, %d, %d)\n",
			current->comm, current->pid, current->tgid);
		return -EFAULT;
	}
	/*  */
	...
	case ISP_GET_DMA_ERR:
		if (copy_from_user(&DebugFlag[0], (void *)Param, sizeof(unsigned int)) != 0) {
			LOG_PR_ERR("get module fail\n");
			Ret = -EFAULT;
		} else {
			if (DebugFlag[0] >= (ISP_IRQ_TYPE_AMOUNT)) {
				LOG_PR_ERR("module error(%d)\n", DebugFlag[0]);
				Ret = -EFAULT;
				break;
			}
...
#else
			if (DebugFlag[0] >= ISP_IRQ_TYPE_AMOUNT) {
				LOG_PR_ERR("cursof: error type(%d)\n", DebugFlag[0]);
				Ret = -EFAULT;
				break;
			}
	...
	case ISP_RESET_BY_HWMODULE: {
		if (copy_from_user(&module, (void *)Param, sizeof(unsigned int)) != 0) {
			LOG_PR_ERR("get hwmodule from user fail\n");
			Ret = -EFAULT;
		} else {
			ISP_Reset(module);
		}
	...
	case ISP_CLEAR_IRQ: {
		if (copy_from_user(&ClearIrq, (void *)Param, sizeof(struct ISP_CLEAR_IRQ_STRUCT)) == 0) {
			...
			if ((ClearIrq.EventInfo.UserKey >= IRQ_USER_NUM_MAX) || (ClearIrq.EventInfo.UserKey < 0)) {
				LOG_PR_ERR("errUserEnum(%d)", ClearIrq.EventInfo.UserKey);
				Ret = -EFAULT;
				goto EXIT;
			}
	...
	case ISP_REGISTER_IRQ_USER_KEY:
		if (copy_from_user(&RegUserKey, (void *)Param, sizeof(struct ISP_REGISTER_USERKEY_STRUCT)) == 0) {
			...
			LOG_PR_ERR("copy from user fail\n");
		}
	...
	case ISP_VF_LOG:
		if (copy_from_user(DebugFlag, (void *)Param, sizeof(unsigned int) * 3) == 0) {
			...
			case 1: {
				...
				switch (DebugFlag[1]) {
				case ISP_CAM_A_IDX:
					uni_dmao = ISP_RD32(CAM_UNI_REG_TOP_DMA_EN(ISP_UNI_A_IDX));
					hds2_sel = ((ISP_RD32(CAM_UNI_REG_TOP_PATH_SEL(ISP_UNI_A_IDX))) & 0x3);
					...
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAM_A_IDX);
					else
					...
					g1stSwP1Done[ISP_IRQ_TYPE_INT_CAM_A_ST] = MTRUE;
					gPrevSofTimestp[ISP_IRQ_TYPE_INT_CAM_A_ST] = 0;
					#endif
					break;
				case ISP_CAM_B_IDX:
					uni_dmao = ISP_RD32(CAM_UNI_REG_B_TOP_DMA_EN(ISP_UNI_A_IDX));
					hds2_sel = ((ISP_RD32(CAM_UNI_REG_B_TOP_PATH_SEL(ISP_UNI_A_IDX))) & 0x3);
					...
					memset((void *)&(IspInfo.TstpQInfo[ISP_IRQ_TYPE_INT_CAM_B_ST]), 0,
							sizeof(struct ISP_TIMESTPQ_INFO_STRUCT));
					g1stSwP1Done[ISP_IRQ_TYPE_INT_CAM_B_ST] = MTRUE;
					gPrevSofTimestp[ISP_IRQ_TYPE_INT_CAM_B_ST] = 0;
					#endif
			...
			case 0: {
				...
				case ISP_CAM_A_IDX:
					...
					if (vf & 0x1)
						...
						LOG_PR_ERR("module_%d: vf already disabled\n",
							ISP_CAM_A_IDX);
					break;
				case ISP_CAM_B_IDX:
			...
			case 11: {
				...
				case ISP_CAMSV0_IDX:
					...
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(ISP_CAMSV0_IDX));
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAMSV0_IDX);
					else
						ISP_WR32(CAMSV_REG_TG_VF_CON(ISP_CAMSV0_IDX), (vf+0x1));
				...
				case ISP_CAMSV1_IDX:
					...
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(ISP_CAMSV1_IDX));
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAMSV1_IDX);
					else
						ISP_WR32(CAMSV_REG_TG_VF_CON(ISP_CAMSV1_IDX), (vf+0x1));
					module = ISP_IRQ_TYPE_INT_CAMSV_0_ST;
					break;
				case ISP_CAMSV2_IDX:
					...
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(ISP_CAMSV2_IDX));
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAMSV2_IDX);
					else
						ISP_WR32(CAMSV_REG_TG_VF_CON(ISP_CAMSV2_IDX), (vf+0x1));
				...
				case ISP_CAMSV3_IDX:
					...
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(ISP_CAMSV3_IDX));
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAMSV3_IDX);
					else
						ISP_WR32(CAMSV_REG_TG_VF_CON(ISP_CAMSV3_IDX), (vf+0x1));
				...
				case ISP_CAMSV4_IDX:
					...
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(ISP_CAMSV4_IDX));
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAMSV4_IDX);
					else
						ISP_WR32(CAMSV_REG_TG_VF_CON(ISP_CAMSV4_IDX), (vf+0x1));
				...
				case ISP_CAMSV5_IDX:
					...
					vf = ISP_RD32(CAMSV_REG_TG_VF_CON(ISP_CAMSV5_IDX));
					if (vf & 0x1)
						LOG_PR_ERR("module_%d: vf already enabled\n",
							ISP_CAMSV5_IDX);
					else
						ISP_WR32(CAMSV_REG_TG_VF_CON(ISP_CAMSV5_IDX), (vf+0x1));
			...
			case 10: {
				...
				case ISP_CAMSV0_IDX:
					...
					if (vf & 0x1)
						...
						LOG_PR_ERR("module_%d: vf already disalbed\n",
							ISP_CAMSV0_IDX);
					break;
				case ISP_CAMSV1_IDX:
		...
		if (copy_from_user(DebugFlag, (void *)Param, sizeof(unsigned int) * 3) == 0) {
			...
			case ISP_IRQ_TYPE_INT_CAM_B_ST:
				...
				if (ISP_PopBufTimestamp(DebugFlag[0], dma_id, pTstp) != 0)
					LOG_PR_ERR("Get Buf sof timestamp fail");
	...
	case ISP_DFS_UPDATE:
		{
			...
			if (copy_from_user(&dfs_update, (void *)Param, sizeof(unsigned int)) == 0) {
				...
				LOG_PR_ERR("ISP_DFS_UPDATE copy_from_user failed\n");
				Ret = -EFAULT;
			}
		}
		break;
	case ISP_GET_SUPPORTED_ISP_CLOCKS:
		...
		if (ispclks.clklevelcnt > ISP_CLK_LEVEL_CNT) {
			LOG_PR_ERR("clklevelcnt is exceeded");
			Ret = -EFAULT;
			break;
		}
	...
	case ISP_ION_IMPORT:
		if (copy_from_user(&IonNode, (void *)Param, sizeof(struct ISP_DEV_ION_NODE_STRUCT)) == 0) {
			...
			if (!pIon_client) {
				LOG_PR_ERR("ion_import: invalid ion client!\n");
				Ret = -EFAULT;
				break;
			}

			if (IonNode.devNode >= ISP_DEV_NODE_NUM) {
				LOG_PR_ERR("[ISP_ION_IMPORT]devNode should be smaller than ISP_DEV_NODE_NUM");
				Ret = -EFAULT;
				break;
			}
			...
			if (ptbl->node != IonNode.devNode) {
				LOG_PR_ERR("ion_import: devNode not support(%d)!\n", IonNode.devNode);
				Ret = -EFAULT;
				break;
			}
			if (IonNode.dmaPort < 0 || IonNode.dmaPort >= _dma_max_wr_) {
				LOG_PR_ERR("ion_import: dmaport error:%d(0~%d)\n",
			...
			if (IonNode.memID <= 0) {
				LOG_PR_ERR("ion_import: dma(%d)invalid ion fd(%d)\n",
	...
	case ISP_ION_FREE:
		if (copy_from_user(&IonNode, (void *)Param, sizeof(struct ISP_DEV_ION_NODE_STRUCT)) == 0) {
			...
			if (!pIon_client) {
				LOG_PR_ERR("ion_free: invalid ion client!\n");
				Ret = -EFAULT;
				break;
			}

			if (IonNode.devNode >= ISP_DEV_NODE_NUM) {
				LOG_PR_ERR("[ISP_ION_FREE]devNode should be smaller than ISP_DEV_NODE_NUM");
				Ret = -EFAULT;
				break;
			}
			...
			ptbl = &gION_TBL[IonNode.devNode];
			if (ptbl->node != IonNode.devNode) {
				LOG_PR_ERR("ion_free: devNode not support(%d)!\n", IonNode.devNode);
				Ret = -EFAULT;
				break;
			}
			if (IonNode.dmaPort < 0 || IonNode.dmaPort >= _dma_max_wr_) {
				LOG_PR_ERR("ion_free: dmaport error:%d(0~%d)\n",
					IonNode.dmaPort, _dma_max_wr_);
				Ret = -EFAULT;
				break;
			}
			jump = IonNode.dmaPort*_ion_keep_max_;
			if (IonNode.memID <= 0) {
				LOG_PR_ERR("ion_free: invalid ion fd(%d)\n", IonNode.memID);
				Ret = -EFAULT;
				break;
			}
			...
			if (i == _ion_keep_max_) {
				spin_unlock(&(ptbl->pLock[IonNode.dmaPort]));
				LOG_PR_ERR("ion_free: can't find ion dev(%d)dma(%d)fd(%d) in list\n",
			...
			*/
			if (--ptbl->pIonCt[jump + i] > 0) {
				...
				spin_unlock(&(ptbl->pLock[IonNode.dmaPort]));
				LOG_PR_ERR("ion_free: free more than import (%d): dev(%d)dma(%d)i(%d)fd(%d)\n",
						ptbl->pIonCt[jump + i],
						IonNode.devNode, IonNode.dmaPort, i,
						IonNode.memID);
				Ret = -EFAULT;
			...
			LOG_PR_ERR("[ion free]copy_from_user failed\n");
			Ret = -EFAULT;
		}
		break;
	case ISP_ION_FREE_BY_HWMODULE:
		if (copy_from_user(&module, (void *)Param, sizeof(unsigned int)) == 0) {
			if (module >= ISP_DEV_NODE_NUM) {
				LOG_PR_ERR("[ISP_ION_FREE_BY_HWMODULE]module should be smaller than ISP_DEV_NODE_NUM");
				Ret = -EFAULT;
				break;
			}
			...
			LOG_PR_ERR("[ion free by module]copy_from_user failed\n");
			Ret = -EFAULT;
		}
		break;
	#endif
	case ISP_CQ_SW_PATCH: {
			...
			if ((Addr[0] != 0) && (Addr[1] != 0)) {
				...
				ISP_WR32(CAM_REG_CTL_UNI_B_DONE_SEL(ISP_CAM_A_IDX), 0x1);
				...
				ISP_WR32(CAM_REG_CTL_UNI_B_DONE_SEL(ISP_CAM_A_IDX), 0x0);
				Addr[0] = Addr[1] = 0;
				spin_unlock_irqrestore(&IspInfo.SpinLockIrq[ISP_IRQ_TYPE_INT_CAM_A_ST], flags);
			}
		}
	...
	case ISP_LARB_MMU_CTL: {
		...
		if (copy_from_user(&larbInfo, (void *)Param, sizeof(struct ISP_LARB_MMU_STRUCT)) != 0) {
			LOG_PR_ERR("copy_from_user LARB_MMU_CTL failed\n");
			Ret = -EFAULT;
			goto EXIT;
		}
		...
		case 6:
			break;
		default:
			LOG_PR_ERR("Wrong SMI_LARB port=%d\n", larbInfo.LarbNum);
			Ret = -EFAULT;
			goto EXIT;
		}

		if ((SMI_LARB_BASE[larbInfo.LarbNum] == NULL) || (larbInfo.regOffset >= 0x1000)) {
			LOG_PR_ERR("Wrong SMI_LARB port=%d base addr=%p offset=0x%x\n",
	...
	case ISP_GET_DUMP_INFO: {
		if (copy_to_user((void *)Param, &g_dumpInfo, sizeof(struct ISP_GET_DUMP_INFO_STRUCT)) != 0) {
			LOG_PR_ERR("ISP_GET_DUMP_INFO copy to user fail");
			Ret = -EFAULT;
		}
		break;
	}
	case ISP_DUMP_BUFFER: {
		if (copy_from_user(&DumpBufStruct, (void *)Param, sizeof(struct ISP_DUMP_BUFFER_STRUCT)) == 0) {
			...
			LOG_PR_ERR("ISP_DUMP_TUNING_BUFFER copy_from_user failed\n");
			Ret = -EFAULT;
		}
		break;
	}
	case ISP_SET_MEM_INFO: {
		if (copy_from_user(&MemInfoStruct, (void *)Param, sizeof(struct ISP_MEM_INFO_STRUCT)) == 0) {
			...
			LOG_PR_ERR("ISP_SET_MEM_INFO copy_from_user failed\n");
			Ret = -EFAULT;
		}
		break;
	}
...
EXIT:
	if (Ret != 0)
		LOG_PR_ERR("Fail, Cmd(%d), Pid(%d), (process, pid, tgid)=(%s, %d, %d)\n",
...
static long ISP_ioctl_compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_ISP_SET_MEM_INFO: {
		...
		ret = filp->f_op->unlocked_ioctl(filp, ISP_SET_MEM_INFO, (unsigned long)data);
		return ret;
	}
	case ISP_GET_DUMP_INFO:
	case ISP_WAIT_IRQ:
	...
	struct file *pFile)
{
	...
	if (isp_p2_ion_client == NULL)
		LOG_PR_ERR("invalid isp_p2_ion_client client!\n");
	...
	*/
	...
	case UNI_A_BASE_HW:
		if (length > ISP_REG_RANGE) {
			LOG_PR_ERR("mmap range error :module(0x%x) length(0x%lx),ISP_REG_RANGE(0x%lx)!\n",
	...
	case DIP_A_BASE_HW:
		if (length > ISP_REG_PER_DIP_RANGE) {
			LOG_PR_ERR("mmap range error :module(0x%x),length(0x%lx),ISP_REG_PER_DIP_RANGE(0x%lx)!\n",
	...
	case SENINF_BASE_HW:
		if (length > 0x8000) {
			LOG_PR_ERR("mmap range error :module(0x%x),length(0x%lx),SENINF_BASE_RANGE(0x%x)!\n",
				pfn, length, 0x4000);
			return -EAGAIN;
		}
		break;
	case MIPI_RX_BASE_HW:
		if (length > 0x6000) {
			LOG_PR_ERR("mmap range error :module(0x%x),length(0x%lx),MIPI_RX_RANGE(0x%x)!\n",
				pfn, length, 0x6000);
			return -EAGAIN;
		}
		break;
	case GPIO_BASE_HW:
		if (length > 0x1000) {
			LOG_PR_ERR("mmap range error :module(0x%x),length(0x%lx),GPIO_RX_RANGE(0x%x)!\n",
				pfn, length, 0x1000);
			return -EAGAIN;
		}
		break;
...
static inline signed int ISP_RegCharDev(void)
{
	...
	if ((Ret) < 0) {
		LOG_PR_ERR("Attatch file operation failed, %d\n", Ret);
		goto EXIT;
	}
	/*  */
EXIT:
...
static signed int ISP_probe(struct platform_device *pDev)
{
	...
	if (nr_isp_devs == 1) {
		...
		isp_clk.ISP_IMG_DIP = devm_clk_get(&pDev->dev, "ISP_CLK_IMG_DIP");
		isp_clk.ISP_IMG_RSC = devm_clk_get(&pDev->dev, "ISP_CLK_IMG_RSC");
		isp_clk.ISP_CAM_CAMSYS = devm_clk_get(&pDev->dev, "ISP_CLK_CAM");
		isp_clk.ISP_CAM_CAMTG = devm_clk_get(&pDev->dev, "ISP_CLK_CAMTG");
		isp_clk.ISP_CAM_SENINF = devm_clk_get(&pDev->dev, "ISP_CLK_CAM_SENINF");
...
)
{
	...
	case ISP_IRQ_TYPE_AMOUNT:
		LOG_PR_ERR("dev name is not found (%s)", moduleName);
		break;
	case ISP_IRQ_TYPE_INT_UNI_A_ST:
	default:
		/*don nothing*/
...
#endif
{
	...
	*ppStart = pPage + off;
	...
	return ret;
#else /* new file_operations style */
	LOG_PR_ERR("ISP_DumpRegToProc: Not implement");
	return 0;
#endif
}
...
#endif
{
	LOG_PR_ERR("ISP_RegDebug: Not implement");
	return 0;
}
...
#endif
{
	LOG_PR_ERR("CAMIO_DumpRegToProc: Not implement");
	return 0;
}
...
#endif
{
	LOG_PR_ERR("CAMIO_RegDebug: Not implement");
	return 0;
}
	...
	if (g_pPhyISPBuffer != NULL) {
		for (i = 0; i < (ISP_DIP_PHYSICAL_REG_SIZE >> 2); i = i + 4) {
			seq_printf(m, "(0x%08X,0x%08X)(0x%08X,0x%08X)(0x%08X,0x%08X)(0x%08X,0x%08X)\n",
					...
					DIP_A_BASE_HW+4*(i+2), (unsigned int)g_pPhyISPBuffer[i+2],
					DIP_A_BASE_HW+4*(i+3), (unsigned int)g_pPhyISPBuffer[i+3]);
		}
	} else {
		LOG_INF("g_pPhyISPBuffer:(0x%pK)\n", g_pPhyISPBuffer);
	}
...
static int isp_p2_dump_read(struct seq_file *m, void *v)
{
	...
	if (g_pVirISPBuffer != NULL) {
		for (i = 0; i < (ISP_DIP_REG_SIZE >> 2); i = i + 4) {
			seq_printf(m, "(0x%08X,0x%08X)(0x%08X,0x%08X)(0x%08X,0x%08X)(0x%08X,0x%08X)\n",
					DIP_A_BASE_HW+4*i, (unsigned int)g_pVirISPBuffer[i],
					DIP_A_BASE_HW+4*(i+1), (unsigned int)g_pVirISPBuffer[i+1],
					DIP_A_BASE_HW+4*(i+2), (unsigned int)g_pVirISPBuffer[i+2],
					DIP_A_BASE_HW+4*(i+3), (unsigned int)g_pVirISPBuffer[i+3]);
		}
	}
...
static signed int __init ISP_Init(void)
{
	...
	do {
		...
		for (i = 0; i < ARRAY_SIZE(SMI_LARB_BASE); i++) {
			...
			if (!node) {
				LOG_PR_ERR("find %s node failed!!!\n", comp_str);
				SMI_LARB_BASE[i] = 0;
				continue;
			}
			SMI_LARB_BASE[i] = of_iomap(node, 0);
			if (!SMI_LARB_BASE[i]) {
				LOG_PR_ERR("unable to map ISP_SENINF0_BASE registers!!!\n");
				break;
			}
	...
	if (!node) {
		LOG_PR_ERR("find mediatek,seninf1 node failed!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_PR_ERR("find mediatek,seninf2 node failed!!!\n");
		return -ENODEV;
	}
	ISP_SENINF1_BASE = of_iomap(node, 0);
	if (!ISP_SENINF1_BASE) {
		LOG_PR_ERR("unable to map ISP_SENINF1_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_PR_ERR("find mediatek,seninf3 node failed!!!\n");
		return -ENODEV;
	}
	ISP_SENINF2_BASE = of_iomap(node, 0);
	if (!ISP_SENINF2_BASE) {
		LOG_PR_ERR("unable to map ISP_SENINF2_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_PR_ERR("find mediatek,seninf4 node failed!!!\n");
		return -ENODEV;
	}
	ISP_SENINF3_BASE = of_iomap(node, 0);
	if (!ISP_SENINF3_BASE) {
		LOG_PR_ERR("unable to map ISP_SENINF3_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_PR_ERR("find mediatek,apmixed node failed!!!\n");
		return -ENODEV;
	}
	CLOCK_CELL_BASE = of_iomap(node, 0);
	if (!CLOCK_CELL_BASE) {
		LOG_PR_ERR("unable to map CLOCK_CELL_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!node) {
		LOG_PR_ERR("find mmsys_config node failed!!!\n");
		return -ENODEV;
	}
	ISP_MMSYS_CONFIG_BASE = of_iomap(node, 0);
	if (!ISP_MMSYS_CONFIG_BASE) {
		LOG_PR_ERR("unable to map ISP_MMSYS_CONFIG_BASE registers!!!\n");
		return -ENODEV;
	}
	...
	if (!isp_p2_dir) {
		LOG_PR_ERR("fail to mkdir /proc/isp_p2\n");
		return 0;
	}
	proc_entry = proc_create("isp_p2_dump", S_IRUGO, isp_p2_dir, &isp_p2_dump_proc_fops);
	proc_entry = proc_create("isp_p2_kedump", S_IRUGO, isp_p2_dir, &isp_p2_ke_dump_proc_fops);
		...
		if (uni_dma_en & 0x4) {
			if (module == ISP_CAM_A_IDX) {
				fbc_ctrl1[dma_arry_map[_eiso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_EISO_A_CTL1(ISP_UNI_A_IDX));
				fbc_ctrl2[dma_arry_map[_eiso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_EISO_A_CTL2(ISP_UNI_A_IDX));
			}
			if (module == ISP_CAM_B_IDX) {
				fbc_ctrl1[dma_arry_map[_eiso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_EISO_B_CTL1(ISP_UNI_A_IDX));
				fbc_ctrl2[dma_arry_map[_eiso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_EISO_B_CTL2(ISP_UNI_A_IDX));
			}
		} else {
		...
		if (uni_dma_en & 0x8) {
			if (module == ISP_CAM_A_IDX) {
				fbc_ctrl1[dma_arry_map[_rsso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_RSSO_A_CTL1(ISP_UNI_A_IDX));
				fbc_ctrl2[dma_arry_map[_rsso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_RSSO_A_CTL2(ISP_UNI_A_IDX));
			}
			if (module == ISP_CAM_B_IDX) {
				fbc_ctrl1[dma_arry_map[_rsso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_RSSO_B_CTL1(ISP_UNI_A_IDX));
				fbc_ctrl2[dma_arry_map[_rsso_]].Raw =
					ISP_RD32(CAM_UNI_REG_FBC_RSSO_B_CTL2(ISP_UNI_A_IDX));
			}
		} else {
...
static void ISP_GetDmaPortsStatus(enum ISP_DEV_NODE_ENUM reg_module, unsigned int *DmaPortsStats)
{
	...
	if (reg_module == ISP_CAM_A_IDX) {
		hds2_sel = (ISP_RD32(CAM_UNI_REG_TOP_PATH_SEL(ISP_UNI_A_IDX)) & 0x3);
		flk2_sel = ((ISP_RD32(CAM_UNI_REG_TOP_PATH_SEL(ISP_UNI_A_IDX)) >> 8) & 0x3);
	}
	if (reg_module == ISP_CAM_B_IDX) {
		hds2_sel = (ISP_RD32(CAM_UNI_REG_B_TOP_PATH_SEL(ISP_UNI_A_IDX)) & 0x3);
		flk2_sel = ((ISP_RD32(CAM_UNI_REG_B_TOP_PATH_SEL(ISP_UNI_A_IDX)) >> 8) & 0x3);
	}
					...
					unsigned int delayCheck)
{
	...
	case ISP_CAM_B_IDX:
		if (dma_id >= _cam_max_) {
			LOG_PR_ERR("LINE_%d ERROR: unsupported module:0x%x dma:%d\n",
	...
	if (_flko_ == dma_id) {
		...
		if ((flk2_sel == 1) && (module == ISP_CAM_B_IDX)) {
			uni_dma_en = ISP_RD32(CAM_UNI_REG_B_TOP_DMA_EN(ISP_UNI_A_IDX));
			if (uni_dma_en & 0x2) {
				fbc_ctrl1.Raw = ISP_RD32(CAM_UNI_REG_FBC_FLKO_B_CTL1(ISP_UNI_A_IDX));
				fbc_ctrl2.Raw = ISP_RD32(CAM_UNI_REG_FBC_FLKO_B_CTL2(ISP_UNI_A_IDX));
			}
		}
	}
	...
	unsigned int sec, unsigned int usec, unsigned int frmPeriod)
{
	...
	case ISP_IRQ_TYPE_INT_CAM_B_ST:
		...
		break;
	default:
		LOG_PR_ERR("Unsupport module:x%x\n", module);
		return -EFAULT;
	}
	...
	case ISP_IRQ_TYPE_INT_CAM_B_ST:
		...
		case _eiso_:
			...
			if (reg_module == ISP_CAM_B_IDX)
				fbc_ctrl2.Raw = ISP_RD32(CAM_UNI_REG_FBC_EISO_B_CTL2(ISP_UNI_A_IDX));
			break;
		case _lcso_:
		...
		case _rsso_:
			...
			if (reg_module == ISP_CAM_B_IDX)
				fbc_ctrl2.Raw = ISP_RD32(CAM_UNI_REG_FBC_RSSO_B_CTL2(ISP_UNI_A_IDX));
			break;
		case _aao_:
...
static int32_t ISP_WaitTimestampReady(unsigned int module, unsigned int dma_id)
{
	...
	if (wait_cnt == 0) {
		LOG_PR_ERR("ERROR: cam:%d dma:%d wait timestamp timeout!!!\n", module, dma_id);
		return -EFAULT;
	}
	#else
			...
			unsigned int usec, unsigned int frmPeriod)
{
	...
	if ((sec > time_prev1.sec) ||
		...
		if (dmao_mask)
			LOG_INF("ERROR: Cam:%d dma:%d current timestamp: cur: %d.%06d prev1: %d.%06d\n",
	...
	if ((time_prev1.sec > time_prev2.sec) ||
		...
		if (dmao_mask)
			LOG_INF("ERROR: Cam:%d dma:%d previous timestamp: prev1: %d.%06d prev2: %d.%06d\n",
	...
	if (IspInfo.TstpQInfo[module].Dmao[dma_id].PrevFbcWCnt != fbc_ctrl2.Bits.WCNT) {
		if (dmao_mask)
			LOG_INF("ERROR: Cam:%d dma:%d strange WCNT SW_HW: %d_%d\n",
				module, dma_id, IspInfo.TstpQInfo[module].Dmao[dma_id].PrevFbcWCnt,
				fbc_ctrl2.Bits.WCNT);
		IspInfo.TstpQInfo[module].Dmao[dma_id].PrevFbcWCnt = fbc_ctrl2.Bits.WCNT;
	}
		...
		unsigned long long refTimestp, unsigned long long prevTimestp)
{
	...
	while (target_wridx != curr_wridx) {
		...
		if (i > frmPeriod) {
			LOG_PR_ERR("Error: too many intpl in sub-sample period %d_%d\n",
	...
	if (IrqStatus & SOF_INT_ST) {
		...
		if (IspInfo.DebugMask & ISP_DBG_INT) {
			...
			if (IspInfo.TstpQInfo[module].DmaEnStatus[_rsso_])
				ISP_CompensateMissingSofTime(reg_module, module, _rsso_,
					sec, usec, frmPeriod);
			...
			if (FrameStatus[module] != CAM_FST_DROP_FRAME) {
				for (subFrm = 0; subFrm < frmPeriod; subFrm++) {
					...
					if (IspInfo.TstpQInfo[module].DmaEnStatus[_rsso_])
						ISP_PushBufTimestamp(module, _rsso_, sec, usec, frmPeriod);
