...
const struct ISR_TABLE IRQ_CB_TBL[ISP_IRQ_TYPE_AMOUNT] = {
	{ISP_Irq_CAM_A,     0,  "cama"}, /* Must be the same name with that in device node. */
	{ISP_Irq_CAM_B,     0,  "camb"},
	{ISP_Irq_DIP_A,     0,  "dip_a"},
	{ISP_Irq_CAMSV_0,   0,  "camsva"},
	{ISP_Irq_CAMSV_1,   0,  "camsvb"},
	{ISP_Irq_CAMSV_2,   0,  "camsvc"},
	{ISP_Irq_CAMSV_3,   0,  "camsvd"},
	{ISP_Irq_CAMSV_4,   0,  "camsve"},
	{ISP_Irq_CAMSV_5,   0,  "camsvf"},
	{NULL,              0,  "cam"} /* UNI */ /* QQ temp */
};
...
static struct ISP_GET_DUMP_INFO_STRUCT g_dumpInfo = {
	.extracmd = 0xFFFFFFFF,
	.imgi_baseaddr = 0xFFFFFFFF,
	.tdri_baseaddr = 0xFFFFFFFF,
	.dmgi_baseaddr = 0xFFFFFFFF
};
static struct ISP_MEM_INFO_STRUCT g_TpipeBaseAddrInfo = {
	.MemInfoCmd = 0x0,
	.MemPa = 0x0,
	.MemVa = NULL,
	.MemSizeDiff = 0x0
};
static struct ISP_MEM_INFO_STRUCT g_CmdqBaseAddrInfo = {
	.MemInfoCmd = 0x0,
	.MemPa = 0x0,
	.MemVa = NULL,
	.MemSizeDiff = 0x0
...
static unsigned int g_DmaErr_CAM[ISP_IRQ_TYPE_AMOUNT][nDMA_ERR] = {{0} };
...
static void ISP_DumpDmaDeepDbg(enum ISP_IRQ_TYPE_ENUM module)
{
	...
	dmaerr[0] = (unsigned int)ISP_RD32(CAM_REG_IMGO_ERR_STAT(regModule));
	dmaerr[1] = (unsigned int)ISP_RD32(CAM_REG_RRZO_ERR_STAT(regModule));
	dmaerr[2] = (unsigned int)ISP_RD32(CAM_REG_AAO_ERR_STAT(regModule));
	dmaerr[3] = (unsigned int)ISP_RD32(CAM_REG_AFO_ERR_STAT(regModule));
	dmaerr[4] = (unsigned int)ISP_RD32(CAM_REG_LCSO_ERR_STAT(regModule));
	dmaerr[5] = (unsigned int)ISP_RD32(CAM_REG_UFEO_ERR_STAT(regModule));
	dmaerr[6] = (unsigned int)ISP_RD32(CAM_REG_BPCI_ERR_STAT(regModule));
	dmaerr[7] = (unsigned int)ISP_RD32(CAM_REG_CACI_ERR_STAT(regModule));
	dmaerr[8] = (unsigned int)ISP_RD32(CAM_REG_LSCI_ERR_STAT(regModule));
	dmaerr[9] = (unsigned int)ISP_RD32(CAM_REG_PDO_ERR_STAT(regModule));


	dmaerr[10] = (unsigned int)ISP_RD32(CAM_UNI_REG_EISO_ERR_STAT(ISP_UNI_A_IDX));
	dmaerr[11] = (unsigned int)ISP_RD32(CAM_UNI_REG_FLKO_ERR_STAT(ISP_UNI_A_IDX));
	dmaerr[12] = (unsigned int)ISP_RD32(CAM_UNI_REG_RSSO_A_ERR_STAT(ISP_UNI_A_IDX));
	dmaerr[13] = (unsigned int)ISP_RD32(CAM_UNI_REG_RSSO_B_ERR_STAT(ISP_UNI_A_IDX));
	dmaerr[14] = (unsigned int)ISP_RD32(CAM_UNI_REG_RAWI_ERR_STAT(ISP_UNI_A_IDX));
	...
	g_DmaErr_CAM[module][3] |= dmaerr[3];
	g_DmaErr_CAM[module][4] |= dmaerr[4];
	g_DmaErr_CAM[module][5] |= dmaerr[5];
	g_DmaErr_CAM[module][6] |= dmaerr[6];
	g_DmaErr_CAM[module][7] |= dmaerr[7];
	g_DmaErr_CAM[module][8] |= dmaerr[8];
	g_DmaErr_CAM[module][9] |= dmaerr[9];
	g_DmaErr_CAM[module][10] |= dmaerr[10];
...
static signed int ISP_DumpSeninfReg(void)
{
	...
	LOG_INF("seninf:0008(0x%x)-0010(0x%x)-03a0(0x%x)-07a0(0x%x) 03b4(0x%x)-07b4(0x%x)-03b4(0x%x)-07b4(0x%x)\n",
		ISP_RD32(ISP_SENINF0_BASE + 0x0008), ISP_RD32(ISP_SENINF0_BASE + 0x0010),
		ISP_RD32(ISP_SENINF0_BASE + 0x03a0), ISP_RD32(ISP_SENINF0_BASE + 0x07a0),
		ISP_RD32(ISP_SENINF0_BASE + 0x03b4), ISP_RD32(ISP_SENINF0_BASE + 0x07b4),
		ISP_RD32(ISP_SENINF0_BASE + 0x03b4), ISP_RD32(ISP_SENINF0_BASE + 0x07b4));
	/*Sensor interface0 control*/
	LOG_INF("seninf:0200(0x%x)-0204(0x%x)-0a00(0x%x)-0a14(0x%x) 0a3c(0x%x)-0a44(0x%x)-0af0(0x%x)-0af4(0x%x)\n",
		ISP_RD32(ISP_SENINF0_BASE + 0x03b0), ISP_RD32(ISP_SENINF0_BASE + 0x03b4),
		ISP_RD32(ISP_SENINF0_BASE + 0x03b8), ISP_RD32(ISP_SENINF0_BASE + 0x03bc),
		ISP_RD32(ISP_SENINF0_BASE + 0x07b0), ISP_RD32(ISP_SENINF0_BASE + 0x07b4),
		ISP_RD32(ISP_SENINF0_BASE + 0x07b8), ISP_RD32(ISP_SENINF0_BASE + 0x07bc));
	/*Sensor interface mux*/
	LOG_INF("seninf:0100(0x%x)-0108(0x%x)-0114(0x%x)-0118(0x%x) 0500(0x%x)-0508(0x%x)-0514(0x%x)-0518(0x%x)\n",
		ISP_RD32(ISP_SENINF0_BASE + 0x0100), ISP_RD32(ISP_SENINF0_BASE + 0x0108),
		ISP_RD32(ISP_SENINF0_BASE + 0x0114), ISP_RD32(ISP_SENINF0_BASE + 0x0118),
		ISP_RD32(ISP_SENINF0_BASE + 0x0500), ISP_RD32(ISP_SENINF0_BASE + 0x0508),
		ISP_RD32(ISP_SENINF0_BASE + 0x0514), ISP_RD32(ISP_SENINF0_BASE + 0x0518));
	LOG_INF("seninf:0900(0x%x)-0908(0x%x)-0914(0x%x)-0918(0x%x) 0d00(0x%x)-0d08(0x%x)-0d14(0x%x)-0d18(0x%x)\n",
		ISP_RD32(ISP_SENINF0_BASE + 0x0900), ISP_RD32(ISP_SENINF0_BASE + 0x0908),
		ISP_RD32(ISP_SENINF0_BASE + 0x0914), ISP_RD32(ISP_SENINF0_BASE + 0x0918),
...
static void ISP_EnableClock(bool En)
{
	...
	if (En) {
		...
		case 0:
#ifdef EVEREST_EP_NO_CLKMGR  /*FPGA test*/
	...
	ISP_WR32(ISP_CAMSYS_CONFIG_BASE + 0x120, 0xFFFFEFCF);
...
static signed int ISP_ReadReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	if ((pRegIo->Count > (PAGE_SIZE/sizeof(unsigned int))) || (pRegIo->pData == NULL)) {
...
static signed int ISP_DumpBuffer(struct ISP_DUMP_BUFFER_STRUCT *pDumpBufStruct)
{
	...
	case ISP_DUMP_TPIPEBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > MAX_ISP_TILE_TDR_HEX_NO) {
			LOG_ERR("tpipe size error, size:0x%x", pDumpBufStruct->BytesofBufferSize);
			Ret = -EFAULT;
			goto EXIT;
		}
#ifdef AEE_DUMP_REDUCE_MEMORY
	...
	case ISP_DUMP_ISPVIRBUF_CMD:
		if (pDumpBufStruct->BytesofBufferSize > ISP_DIP_REG_SIZE) {
			LOG_ERR("vir isp buffer size error, size:0x%x", pDumpBufStruct->BytesofBufferSize);
			Ret = -EFAULT;
			goto EXIT;
		}
#ifdef AEE_DUMP_REDUCE_MEMORY
...
static bool gEnableSwTimestamp = 1;
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)Param, sizeof(struct ISP_BUFFER_CTRL_STRUCT)) == 0) {
		...
		case ISP_RT_BUF_CTRL_CLEAR:
			...
			case ISP_IRQ_TYPE_INT_CAM_B_ST:
				...
				memset((void *)g_DmaErr_CAM[rt_buf_ctrl.module], 0, sizeof(unsigned int)*nDMA_ERR_CAM);
				break;
...
static signed int __init ISP_Init(void)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,seiinfa");
	if (!node) {
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,seiinfb");
	if (!node) {
	...
	LOG_INF("Dummy reg buf for seninf 2/3");
	...
	m4u_register_fault_callback(M4U_PORT_ICEI, ISP_M4U_TranslationFault_callback, NULL);
#endif
...
void IRQ_INT_ERR_CHECK_CAM(unsigned int WarnStatus, unsigned int ErrStatus, enum ISP_IRQ_TYPE_ENUM module)
{
	...
	if (ErrStatus) {
		...
		/*SMI monitor*/
		/* temp marked for build error by Ryan
...
enum CAM_FrameST Irq_CAM_FrameStatus(enum ISP_DEV_NODE_ENUM module, enum ISP_IRQ_TYPE_ENUM irq_mod,
...
static int32_t ISP_CompensateMissingSofTime(enum ISP_DEV_NODE_ENUM reg_module,
			unsigned int module, unsigned int dma_id, unsigned int sec, unsigned int usec,
