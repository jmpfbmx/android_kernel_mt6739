...
#define T_STAMP_2_0		/* time stamp workaround method. (increase timestamp baseon*/
...
#define ISP_IRQ_P1_STATUS2_CQ0_DONE_ST               ((unsigned int)1<<12)
#define ISP_IRQ_P1_STATUS2_CQ0B_DONE_ST              ((unsigned int)1<<13)
#define ISP_IRQ_P1_STATUS2_CQ0C_DONE_ST              ((unsigned int)1<<14)
...
#define ISP_IRQ_P1_STATUS2_D_CQ0_DONE_ST               ((unsigned int)1<<12)
#define ISP_IRQ_P1_STATUS2_D_CQ0B_DONE_ST              ((unsigned int)1<<13)
#define ISP_IRQ_P1_STATUS2_D_CQ0C_DONE_ST              ((unsigned int)1<<14)
...
#define ISP_IRQ_P2_STATUS_IMG3BO_DONE_ST            ((unsigned int)1<<23)
#define ISP_IRQ_P2_STATUS_IMG3O_DONE_ST             ((unsigned int)1<<24)
#define ISP_IRQ_P2_STATUS_IMG3CO_DONE_ST            ((unsigned int)1<<25)
#define ISP_IRQ_P2_STATUS_FEO_DONE_ST               ((unsigned int)1<<26)
#define ISP_IRQ_P2_STATUS_DMA_ERR_ST                ((unsigned int)1<<27)
#define ISP_IRQ_P2_STATUS_GGM_COLLISION_ST          ((unsigned int)1<<28)
#define ISP_IRQ_P2_STATUS_PCA_COLLISION_ST          ((unsigned int)1<<29)
...
#define ISP_IRQ_STATUS3X_DMA_ERR_ST                ((unsigned int)1<<27)
#define ISP_IRQ_STATUS3X_GGM_COLLISION_ST          ((unsigned int)1<<28)
#define ISP_IRQ_STATUS3X_PCA_COLLISION_ST          ((unsigned int)1<<29)
...
struct ISP_IRQ_TIME_STRUCT {
	...
	unsigned int tLastSig2GetSig_sec;/* time period from latest occurring signal */
						/*to user try to wait and get the signal */
	unsigned int tLastSig2GetSig_usec;/* time period from latest occurring signal */
						/*to user try to wait and get the signal */
	int passedbySigcnt;	/* the count for the signal passed by  */
};

struct ISP_EIS_META_STRUCT {
	unsigned int tLastSOF2P1done_sec;	/* time stamp of the last closest occurring sof signal for pass1 done */
	unsigned int tLastSOF2P1done_usec;	/* time stamp of the last closest occurring sof signal for pass1 done */
};
...
enum ISP_ED_BUFQUE_CTRL_ENUM {
	...
	ISP_ED_BUFQUE_CTRL_WAKE_WAITFRAME,	/* 5,wake all slept users to check buffer is dequeued or not */
...
#define COMPAT_ISP_UPDATE_MAGNUM   _IOW(ISP_MAGIC, ISP_CMD_UPDATE_MAGNUM, compat_uint_t)

#define COMPAT_ISP_WAIT_IRQ             _IOW(ISP_MAGIC, ISP_CMD_WAIT_IRQ,      struct compat_ISP_WAIT_IRQ_STRUCT)
#define COMPAT_ISP_MARK_IRQ_REQUEST     _IOWR(ISP_MAGIC, ISP_CMD_MARK_IRQ_REQUEST, struct compat_ISP_WAIT_IRQ_STRUCT)
#define COMPAT_ISP_GET_MARK2QUERY_TIME  _IOWR(ISP_MAGIC, ISP_CMD_GET_MARK2QUERY_TIME, struct compat_ISP_WAIT_IRQ_STRUCT)
#define COMPAT_ISP_FLUSH_IRQ_REQUEST    _IOW(ISP_MAGIC, ISP_CMD_FLUSH_IRQ_REQUEST, struct compat_ISP_WAIT_IRQ_STRUCT)


#define COMPAT_ISP_READ_IRQ        _IOR(ISP_MAGIC, ISP_CMD_READ_IRQ,      struct compat_ISP_READ_IRQ_STRUCT)
...
#define COMPAT_ISP_REGISTER_IRQ_USER_KEY  _IOR(ISP_MAGIC, ISP_CMD_REGISTER_IRQ_USER_KEY,\
						struct compat_ISP_REGISTER_USERKEY_STRUCT)
