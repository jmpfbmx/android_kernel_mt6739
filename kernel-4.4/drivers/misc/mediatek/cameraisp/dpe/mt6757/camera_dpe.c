...
static inline void DPE_Prepare_ccf_clock(void)
{
	...
	ret = clk_prepare(dpe_clk.CG_IMGSYS_LAB5);
	if (ret)
		LOG_ERR("cannot prepare CG_IMGSYS_LAB5 clock\n");
...
static inline void DPE_Enable_ccf_clock(void)
{
	...
	ret = clk_enable(dpe_clk.CG_IMGSYS_LAB5);
	if (ret)
		LOG_ERR("cannot enable CG_IMGSYS_LAB5 clock\n");
...
static inline void DPE_Prepare_Enable_ccf_clock(void)
{
	...
	ret = clk_prepare_enable(dpe_clk.CG_IMGSYS_LAB5);
	if (ret)
		LOG_ERR("cannot prepare and enable CG_IMGSYS_LAB5 clock\n");
...
static inline void DPE_Unprepare_ccf_clock(void)
{
	...
	clk_unprepare(dpe_clk.CG_IMGSYS_LAB5);
...
static inline void DPE_Disable_ccf_clock(void)
{
	...
	clk_disable(dpe_clk.CG_IMGSYS_LAB5);
...
static inline void DPE_Disable_Unprepare_ccf_clock(void)
{
	...
	clk_disable_unprepare(dpe_clk.CG_IMGSYS_LAB5);
		...
		if (copy_from_user(&dpe_WmfeReq, (void *)Param, sizeof(struct DPE_WMFERequest)) ==
			...
			if (DPE_REQUEST_STATE_EMPTY ==
				...
				g_WMFE_ReqRing.WriteIdx = (WIdx + 1) % _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_;
					gWfmeCnt++;
					if ((gWfmeCnt == 1) && (g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState
						== DPE_REQUEST_STATE_PENDING)) {
						g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState
						= DPE_REQUEST_STATE_RUNNING;
						bWMFEFound = MTRUE;
					} else {
						LOG_INF("WMFE en req sta error!!, ReqIdx:%d,ReqState:%d,CfgCnt:%d\n",
						WmfeWriteIdx, g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState,
						gWfmeCnt);
					}
			} else {
...
static signed int DPE_probe(struct platform_device *pDev)
{
	...
	if (nr_DPE_devs == 1) {
		...
		dpe_clk.CG_SCP_SYS_DIS = devm_clk_get(&pDev->dev, "DPE_CG_SCP_SYS_DIS");
		dpe_clk.CG_MM_SMI_COMMON = devm_clk_get(&pDev->dev, "DPE_CG_MM_SMI_COMMON");
		dpe_clk.CG_SCP_SYS_ISP = devm_clk_get(&pDev->dev, "DPE_CG_SCP_SYS_ISP");
		dpe_clk.CG_IMGSYS_LAB5 = devm_clk_get(&pDev->dev, "DPE_CG_IMG_LARB5");
		...
		if (IS_ERR(dpe_clk.CG_IMGSYS_LAB5)) {
			LOG_ERR("cannot get CG_IMGSYS_LAB5 clock\n");
			return PTR_ERR(dpe_clk.CG_IMGSYS_LAB5);
		}
...
static void DPE_ScheduleWmfeWork(struct work_struct *data)
{
	...
	if (bResulst == MTRUE) {
		...
		if (gWfmeCnt > 0) {
			...
			if (WmfeWriteIdx < 0)
				WmfeWriteIdx = WmfeWriteIdx + _SUPPORT_MAX_DPE_REQUEST_RING_SIZE_;

			if (g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState == DPE_REQUEST_STATE_PENDING) {
				g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState = DPE_REQUEST_STATE_RUNNING;
				bFound = MTRUE;
			} else {
				LOG_INF("ScheduleWmfeWork  state machine error!!, WmfeWriteIdx:%d, RequestState:%d\n",
				WmfeWriteIdx, g_WMFE_ReqRing.WMFEReq_Struct[WmfeWriteIdx].RequestState);
			}
		}
	}
	...
	LOG_INF("WMFE:bFound:%d, WmfeWriteIdx:%d, WriteIdx:%d, gWfmeCnt:%d, RequestState:%d,HWIdx:%d\n",
		bFound, WmfeWriteIdx,
