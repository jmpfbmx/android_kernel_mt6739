...
static const struct i2c_device_id nxp_smartpa_i2c_id[] = {
	{ "speaker_amp", 0 },
	{ }
};
...
static int AudDrv_nxpspk_probe(struct platform_device *platform)
{
	...
	TfaI2CDMABuf_va = dma_alloc_coherent(&platform->dev, 4096, &TfaI2CDMABuf_pa, GFP_KERNEL);
	if (TfaI2CDMABuf_va == NULL) {
		pr_err("AudDrv_nxpspk_probe dma_alloc_coherent error\n");
		return -1;
	}
	pr_debug("AudDrv_nxpspk_probe dma_alloc_coherent TfaI2CDMABuf_va = %p\n", TfaI2CDMABuf_va);
...
{
	pr_debug("+AudDrv_nxpspk_remove\n");

	if (TfaI2CDMABuf_va != NULL) {
		dma_free_coherent(&platform->dev, 4096, TfaI2CDMABuf_va, TfaI2CDMABuf_pa);
...
static ssize_t AudDrv_nxpspk_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
	...
	if (count <= 8)
		...
		ret = nxp_i2c_master_send(new_client, (char *)TfaI2CDMABuf, count);
...
static ssize_t AudDrv_nxpspk_read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
	...
	if (count <= 8)
		...
		ret = nxp_i2c_master_recv(new_client, (char *)TfaI2CDMABuf, count);
		for (i = 0; i < count; i++)
