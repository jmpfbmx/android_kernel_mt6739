...
void switch_irtx_gpio(int mode)
{
	...
	if (mode >= (ARRAY_SIZE(irtx_gpio_cfg))) {
		pr_info("[IRTX][PinC]%s(%d) fail!! - parameter error!\n", __func__, mode);
		return;
	}

	if (IS_ERR(ppinctrl_irtx)) {
		pr_info("[IRTX][PinC]%s ppinctrl_irtx:%p is error! err:%ld\n",
	...
	if (mode == IRTX_GPIO_MODE_LED_SET) {
		...
		if (ret < 0) {
			pr_info("[IRTX] regulator_enable fail!!\n");
			return;
		}
	} else {
		ret = regulator_disable(mt_irtx_dev.buck);
		if (ret < 0) {
			pr_info("[IRTX] regulator_disable fail!!\n");
			return;
		}
	}
	...
	if (IS_ERR(pins_irtx)) {
		pr_info("[IRTX][PinC]%s pinctrl_lockup(%p, %s) fail!! ppinctrl:%p, err:%ld\n", __func__,
...
static int dev_char_close(struct inode *inode, struct file *file)
{
	...
	ret = put_ir_device();
	if (ret) {
		pr_info("[IRTX] device close without open\n");
		goto exit;
	}
...
static long dev_char_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IRTX_IOC_SET_IRTX_LED_EN:
		if (copy_from_user(&para, (void __user *)arg, sizeof(unsigned int))) {
			pr_info("[IRTX] IRTX_IOC_SET_IRTX_LED_EN: copy_from_user fail!\n");
			ret = -EFAULT;
		} else {
			/* en: bit 12; */
			/* gpio: bit 0-11 */
		...
		break;
	default:
		pr_info("[IRTX] unknown ioctl cmd 0x%x\n", cmd);
		ret = -ENOTTY;
		break;
	}
	return ret;
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (!wave_vir) {
		pr_info("[IRTX] alloc memory fail\n");
		return -ENOMEM;
	}
	ret = copy_from_user(wave_vir, buf, count);
	if (ret) {
		pr_info("[IRTX] write, copy from user fail %d\n", ret);
		goto exit;
	}
	/* invert bit */
...
static int irtx_probe(struct platform_device *plat_dev)
{
	...
	if (plat_dev->dev.of_node == NULL) {
		pr_info("[IRTX] irtx OF node is NULL\n");
		return -1;
	}
	...
	if (IS_ERR(mt_irtx_dev.ppinctrl_irtx)) {
		pr_info("[IRTX][PinC]cannot find pinctrl. ptr_err:%ld\n", PTR_ERR(mt_irtx_dev.ppinctrl_irtx));
	...
	if (mt_irtx_dev.buck == NULL) {
		pr_info("[IRTX] regulator_get fail!!\n");
		return -1;
	}

	ret = regulator_set_voltage(mt_irtx_dev.buck, 2800000, 2800000);
	if (ret < 0) {
		pr_info("[IRTX] regulator_set_voltage fail!!\n");
		return -1;
	}
	...
	if (!major) {
		...
		if (ret) {
			pr_info("[IRTX] alloc_chrdev_region fail ret=%d\n", ret);
			goto exit;
		} else {
		...
		if (ret) {
			pr_info("[IRTX] register_chrdev_region fail ret=%d\n", ret);
			goto exit;
		}
	}
	...
	if (!c_dev) {
		/* pr_info("[IRTX] kmalloc cdev fail\n"); */
		goto exit;
	}
	...
	if (ret) {
		pr_info("[IRTX] cdev_add fail ret=%d\n", ret);
		goto exit;
	}
	...
	if (IS_ERR(dev)) {
		ret = PTR_ERR(dev);
		pr_info("[IRTX] device_create fail ret=%d\n", ret);
		goto exit;
	}
...
static int __init irtx_init(void)
{
	...
	if (ret) {
		pr_info("[IRTX] irtx platform driver register fail %d\n", ret);
		goto exit;
	}
