...
int scp_set_pmic_vcore(unsigned int cur_freq)
{
	...
	if (cur_freq == CLK_OPP0) {
		...
		ret_vs = pmic_scp_set_vsram_vcore(856250);
		scp_cur_volt = 2;
	...
	if (ret_vc != 0 || ret_vs != 0) {
		ret = -1;
		pr_err("scp vcore/vsram setting error, (%d, %d)\n",
				ret_vc, ret_vs);
		WARN_ON(1);
	}
#endif
...
void scp_vcore_request(unsigned int clk_opp)
{
	...
	if (scp_expected_freq == CLK_OPP0)
		dvfsrc_set_scp_vcore_request(0x0);
	else if (scp_expected_freq == CLK_OPP1)
		dvfsrc_set_scp_vcore_request(0x1);
	else if (scp_expected_freq == CLK_OPP2)
...
void mt_pmic_sshub_init(void)
{
	...
	val[0] = pmic_get_register_value(PMIC_RG_BUCK_VCORE_SSHUB_ON);
	val[1] = pmic_get_register_value(PMIC_RG_BUCK_VCORE_SSHUB_MODE);
	val[2] = pmic_get_register_value(PMIC_RG_BUCK_VCORE_SSHUB_VOSEL);
	val[3] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_ON);
	val[4] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_MODE);
	val[5] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_VOSEL);
	pr_debug("Before: vcore=(0x%x,0x%x,0x%x), vsram=(0x%x,0x%x,0x%x)\n",
			val[0], val[1], val[2], val[3], val[4], val[5]);

	pmic_scp_set_vcore(625000);
	pmic_scp_set_vsram_vcore(856250);
	...
	val[4] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_MODE);
	val[5] = pmic_get_register_value(PMIC_RG_LDO_VSRAM_CORE_SSHUB_VOSEL);
	pr_debug("After: vcore=(0x%x,0x%x,0x%x), vsram=(0x%x,0x%x,0x%x)\n",
			val[0], val[1], val[2], val[3], val[4], val[5]);
#endif
}
