...
static void tinysys_time_sync(void)
{
	...
	do {
		...
		ret |= scp_ipi_send(IPI_SCP_TIMER, &ts_ctl, sizeof(struct timesync_ctrl_s), 0, SCP_B_ID);
		if (ret == DONE)
			break;
		timeout--;
		mdelay(5);
...
static int timesync_event(struct notifier_block *this, unsigned long event, void *ptr)
{
	...
	case SCP_EVENT_READY:
		if (is_scp_ready(SCP_A_ID) && is_scp_ready(SCP_B_ID))
			tinysys_time_sync();
		break;
	}
	return NOTIFY_DONE;
...
static int __init init_scp_timesync(void)
{
	...
	scp_B_register_notify(&timesync_notifier);
