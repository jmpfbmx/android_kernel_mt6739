...
static int pn553_compatible_test(struct pn553_dev *pn553_dev)
{
	...
	charsize = sizeof(send_reset);

	NFC_INFO("i2c_master_send test Start! \n");
 	ret = i2c_master_send(pn553_dev->client, &send_reset[0], charsize);
 	NFC_INFO("i2c_master_send test End, ret=%d! \n",ret);
	 // power off
	if (!IS_ERR(nfc_enb_low))
	{
		pinctrl_select_state(nfc_pinctrl, nfc_enb_low);
...
static int pn553_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	nfc_irq_pin_num = of_get_named_gpio(node, "deb-gpios", 0);
