...
void mtk_uart_set_auto_baud(struct mtk_uart *uart)
{
	...
	case MTK_SYSCLK_52:
		...
		break;
	default:
		dev_info(uart->port.dev, "SYSCLK = %ldMHZ doesn't support autobaud\n", uart->sysclk);
		return;
	}
...
void mtk_uart_power_up(struct mtk_uart *uart)
{
	...
	if (uart->poweron_count > 0) {
		...
		if (clk_en_ret) {
			pr_info("[UART%d][CCF]enable clk_uart_main failed. ret:%d, clk_main:%p\n", uart->nport,
			...
			if ((uart != console_port)
				...
				if (clk_en_ret) {
					pr_info("[UART%d][CCF]enable clk_uart_main failed. ret:%d, clk_dma:%p\n",
...
void switch_uart_gpio(int uartport, int gpioopid)
{
	...
	if ((uartport >= UART_NR) || (uartport > 3)) {
		pr_info("[UART%d][PinC]%s: port error!!\n", uartport, __func__);
		return;
	}

	ppinctrl = ppinctrl_uart[uartport];
	if (IS_ERR(ppinctrl)) {
		pr_info("[UART%d][PinC]%s get pinctrl fail!! err:%ld\n", uartport, __func__, PTR_ERR(ppinctrl));
		return;
	}
	...
	if (IS_ERR(pins_uart)) {
		pr_info("[UART%d][PinC]%s pinctrl_lockup(%d, %s) fail!! pctrl:%p, err:%ld\n", uartport, __func__,
...
{
#if defined(CONFIG_PM) && !defined(CONFIG_FPGA_EARLY_PORTING)
	int uart_gpio_op = 3;	/* UART TX CLEAR */
#endif
	int uartport = uart->nport;

	if (uartport > 3) {
		pr_info("[UART%d] %s fail!! port:%d", uartport, __func__, uartport);
		return;
	}
...
{
#if defined(CONFIG_PM) && !defined(CONFIG_FPGA_EARLY_PORTING)
	int uart_gpio_op = 2;	/* UART TX SET */
#endif
	int uartport = uart->nport;
...
{
#if defined(CONFIG_PM) && !defined(CONFIG_FPGA_EARLY_PORTING)
	int uart_gpio_op = 1;	/* UART RX Clear */
#endif
	int uartport = uart->nport;
...
{
#if defined(CONFIG_PM) && !defined(CONFIG_FPGA_EARLY_PORTING)
	int uart_gpio_op = 0;	/* UART RX SET */
#endif
	int uartport = uart->nport;
