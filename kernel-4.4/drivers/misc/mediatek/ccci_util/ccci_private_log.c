...
static struct buffer_node node_array[2][CCCI_DUMP_MAX+1] = {
	{
		...
		{&normal_ctlb[0], CCCI_NORMAL_BUF, CCCI_DUMP_ATTR_RING, CCCI_DUMP_NORMAL},
		{&repeat_ctlb[0], CCCI_REPEAT_BUF, CCCI_DUMP_ATTR_RING, CCCI_DUMP_REPEAT},
		{&reg_dump_ctlb[0], CCCI_REG_DUMP_BUF, CCCI_DUMP_ATTR_RING, CCCI_DUMP_MEM_DUMP},
...
void ccci_util_mem_dump(int md_id, int buf_type, void *start_addr, int len)
{
	...
	for (i = 0; i < _16_fix_num; i++) {
		ccci_dump_write(md_id, buf_type, 0, "%03X: %08X %08X %08X %08X\n",
...
int ccci_event_log(const char *fmt, ...)
{
	...
	if (wr_pose + write_len > ccci_event_buffer.buf_size) {
		...
		memcpy(ccci_event_buffer.buffer + wr_pose, temp_log, can_be_write);
		memcpy(ccci_event_buffer.buffer, temp_log + can_be_write, write_len - can_be_write);
	} else {
	...
	ccci_event_buffer.write_pos = (wr_pose + write_len)&(ccci_event_buffer.buf_size-1);
...
int ccci_event_log_cpy(char buf[], int size)
{
	...
	if (ccci_event_buffer.data_size > ccci_event_buffer.buf_size) {
		...
		cpy_size = ccci_event_buffer.buf_size > size ? size : ccci_event_buffer.buf_size;
		...
		memcpy(&buf[first_half], ccci_event_buffer.buffer, ccci_event_buffer.buf_size - first_half);
	} else {
		/* here using DATA size */
		cpy_size = ccci_event_buffer.data_size > size ? size : ccci_event_buffer.data_size;
