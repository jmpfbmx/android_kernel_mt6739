...
#define MMDVFS_SCENS_USING_NON_FORCE_KICKER ((1 << MMDVFS_SCEN_MHL) | (1 << SMI_BWC_SCEN_WFD))
...
static int mmdvfs_set_step_with_mmsys_clk(enum MTK_SMI_BWC_SCEN scenario, enum mmdvfs_voltage_enum step,
int mmsys_clk_mode);
static int mmdvfs_set_step_with_mmsys_clk_low_low(enum MTK_SMI_BWC_SCEN scenario, enum mmdvfs_voltage_enum step,
	...
	case SMI_BWC_SCEN_VR_SLOW:
		{
			...
			step = get_smvr_step(1, venc_size, 0, g_mmdvfs_cmd.sensor_fps);
#else
...
{
	int concurrency_hpm = g_mmdvfs_mgr->step_concurrency[MMDVFS_VOLTAGE_HIGH];
	int final_clk = MMSYS_CLK_MEDIUM;
...
#ifdef MMDVFS_O1
int mmdvfs_set_step_with_mmsys_clk_low_low(enum MTK_SMI_BWC_SCEN smi_scenario, enum mmdvfs_voltage_enum step,
	int mmsys_clk_mode_request, int enable_low_low)
{
	...
	g_mmdvfs_mgr->step_concurrency[MMDVFS_VOLTAGE_LOW_LOW] = concurrency_ulpm;
	...
	/* Config Non-force kicker, only WFD and MHL are using this type of kicker */
	...
	if (concurrency_hpm & MMDVFS_SCENS_USING_NON_FORCE_KICKER)
		...
		mmdvfs_vcorefs_request_dvfs_opp(kir_mm_non_force, OPPI_ULTRA_LOW_PWR);
	else
	...
	int mmsys_clk_mode_request, int enable_low_low)
{
	...
	if (final_step == MMDVFS_VOLTAGE_HIGH) {
		...
		if (final_step == MMDVFS_VOLTAGE_LOW_LOW) {
			...
			mmdvfs_vcorefs_request_dvfs_opp(KIR_MM, OPPI_ULTRA_LOW_PWR);
#else
			MMDVFSMSG("Doesn't support low_low mode in this veresion\n");
			mmdvfs_vcorefs_request_dvfs_opp(KIR_MM, OPPI_UNREQ);
...
static int handle_step_mmmclk_set(struct MTK_MMDVFS_CMD *cmd)
{
	...
	voltage_request = cmd->step & MMDVFS_IOCTL_CMD_STEP_FIELD_MASK;
	...
	if (voltage_request < 0 || voltage_request >= MMDVFS_VOLTAGE_COUNT ||
		...
		MMDVFSMSG("invalid step (%d) or mmclk (%d)\n", voltage_request, mmclk_request);
	}
	...
	case MTK_MMDVFS_CMD_TYPE_QUERY: { /* query with some parameters */
		{
			...
			if ((query_voltage >= 0) && (query_voltage <= MMDVFS_VOLTAGE_LOW_LOW))
				query_voltage_rank = voltage_rank_table[query_voltage];

			if ((current_voltage >= 0) && (current_voltage <= MMDVFS_VOLTAGE_LOW_LOW))
				current_voltage_rank = voltage_rank_table[current_voltage];
	...
	case MTK_MMDVFS_CMD_TYPE_GET:
		{
				...
				MMDVFSMSG("Current step query result: %d, %d, 0x%x\n",
				g_mmdvfs_current_step, current_mmsys_clk, cmd->ret);
		}
		break;
	case MTK_MMDVFS_CMD_TYPE_CONFIG:
		g_mmdvfs_mgr->is_boost_disable = cmd->boost_disable;

		if (cmd->ddr_type == MMDVFS_IOCTL_CMD_DDR_TYPE_AUTO_SELECT)
			mmdvfs_init_ddr();
		else
...
#ifdef MMDVFS_O1
	mmdvfs_set_step_with_mmsys_clk_low_low(scen, MMDVFS_VOLTAGE_DEFAULT_STEP, MMSYS_CLK_MEDIUM, 0);
#else /* MMDVFS_O1 */
	...
	case SMI_BWC_SCEN_VSS:
		{
			...
			if (vr_step == MMDVFS_VOLTAGE_HIGH || venc_step == MMDVFS_VOLTAGE_HIGH) {
				...
				if (vr_step == MMDVFS_VOLTAGE_LOW || venc_step == MMDVFS_VOLTAGE_LOW)
					mmdvfs_set_step_with_mmsys_clk(scen, MMDVFS_VOLTAGE_LOW,
						MMSYS_CLK_MEDIUM);
				else
					mmdvfs_set_step_with_mmsys_clk_low_low(scen, MMDVFS_VOLTAGE_LOW_LOW,
						MMSYS_CLK_MEDIUM, 1);
			}
#else /* MMDVFS_O1 */
			if (vr_step == MMDVFS_VOLTAGE_HIGH || venc_step == MMDVFS_VOLTAGE_HIGH) {
...
#ifdef MMDVFS_O1
		{
			enum mmdvfs_voltage_enum smvr_step = get_smvr_step(1, g_mmdvfs_info->video_record_size[0] *
			g_mmdvfs_info->video_record_size[1], 0, 120);
			if (smvr_step == MMDVFS_VOLTAGE_HIGH)
				mmdvfs_set_step_with_mmsys_clk_low_low(scen, MMDVFS_VOLTAGE_HIGH,
				MMSYS_CLK_HIGH, 1);
			else
				mmdvfs_set_step_with_mmsys_clk_low_low(scen, MMDVFS_VOLTAGE_LOW_LOW,
				MMSYS_CLK_MEDIUM, 1);
...
int venc_resolution, struct mmdvfs_context_struct *mmdvfs_mgr_cntx, int is_ui_idle){
	...
	/* WFD, HML and VP4K check, it is a specfial case which is not recorded with MTK_SMI_BWC_SCEN */
	...
	if (current_scenarios & ((1 << SMI_BWC_SCEN_VR) | (1 << SMI_BWC_SCEN_VSS))) {
		...
		if ((current_scenarios & (1 << SMI_BWC_SCEN_VENC)) == 0) {
			MMDVFSMSG("Didn't enter low low step in Camera scenario without VENC: 0x%x\n",
			current_scenarios);
			return 0;
		}
