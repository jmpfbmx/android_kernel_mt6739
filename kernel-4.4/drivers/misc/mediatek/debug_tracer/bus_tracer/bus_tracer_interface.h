...
struct bus_tracer_plt_operations {
	...
	int (*set_watchpoint_filter)(struct bus_tracer_plt *plt,
			struct watchpoint_filter, unsigned int tracer_id);
	/* setup bypass filter */
	int (*set_bypass_filter)(struct bus_tracer_plt *plt,
			struct bypass_filter, unsigned int tracer_id);
	/* setup id filter */
	int (*set_id_filter)(struct bus_tracer_plt *plt,
			struct id_filter, unsigned int tracer_id, unsigned int idf_id);
	/* setup rw filter */
	int (*set_rw_filter)(struct bus_tracer_plt *plt,
			struct rw_filter, unsigned int tracer_id);
	/* dump current setting of tracers */
	int (*dump_setting)(struct bus_tracer_plt *plt, char *buf, int len);
	/* if you want to do anything more than bus_tracer.c:bus_tracer_probe() */
	int (*probe)(struct bus_tracer_plt *plt, struct platform_device *pdev);
	/* if you want to do anything more than bus_tracer.c:bus_tracer_remove() */
	int (*remove)(struct bus_tracer_plt *plt, struct platform_device *pdev);
	/* if you want to do anything more than bus_tracer.c:bus_tracer_suspend() */
	int (*suspend)(struct bus_tracer_plt *plt, struct platform_device *pdev, pm_message_t state);
	/* if you want to do anything more than bus_tracer.c:bus_tracer_resume() */
...
struct tracer {
	...
	unsigned char at_id;
	unsigned char multi_user_etb_id;
