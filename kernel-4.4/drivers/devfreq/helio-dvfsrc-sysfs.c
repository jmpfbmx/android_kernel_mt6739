				 ...
				 char *buf)
{
	...
	p += snprintf(p, buff_end - p, "%-24s: 0x%08x, 0x%08x, 0x%08x, 0x%08x\n",
...
static DEVICE_ATTR(dvfsrc_enable, 0644, dvfsrc_enable_show, dvfsrc_enable_store);
...
{
	pm_qos_add_request(&dvfsrc_emi_request, PM_QOS_EMI_OPP, PM_QOS_EMI_OPP_DEFAULT_VALUE);
