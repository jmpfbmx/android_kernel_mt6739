...
void random_delay(struct ufs_hba *hba)
{
	...
	dev_notice(hba->dev, "%s: mvg_spoh reset delay time 0x%x\n", __func__, time);
	udelay(time);
}
...
void ufs_mtk_pltfrm_gpio_trigger_init(struct ufs_hba *hba)
{
	...
	dev_notice(hba->dev, "%s: trigger_gpio_init!!!\n",
				__func__);
}
#endif
...
int ufs_mtk_pltfrm_deepidle_check_h8(void)
{
	...
	if (ret) {
		...
		if (ret != -1)
			dev_notice(ufs_mtk_hba->dev, "ufshcd_dme_get 0x%x fail, ret = %d!\n", VENDOR_POWERSTATE, ret);
		return ret;
	}
...
int ufs_mtk_pltfrm_host_sw_rst(struct ufs_hba *hba, u32 target)
{
	...
	if (!ufs_mtk_mmio_base_infracfg_ao) {
		dev_notice(hba->dev, "ufs_mtk_host_sw_rst: failed, null ufs_mtk_mmio_base_infracfg_ao.\n");
		return 1;
	}
...
int ufs_mtk_pltfrm_parse_dt(struct ufs_hba *hba)
{
	...
	if (node_infracfg_ao) {
		...
		dev_notice(hba->dev, "error: node_infracfg_ao init fail\n");
	...
	if (node_pericfg) {
		...
		if (IS_ERR(*(void **)&ufs_mtk_mmio_base_pericfg)) {
			...
			dev_notice(hba->dev, "error: mmio_base_pericfg init fail\n");
			ufs_mtk_mmio_base_pericfg = NULL;
		}
	} else
		dev_notice(hba->dev, "error: node_pericfg init fail\n");
