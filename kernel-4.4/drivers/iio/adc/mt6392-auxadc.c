			      ...
			      struct iio_chan_spec const *chan)
{
	...
	if ((chan->channel == 5) || (chan->channel == 6)) {
		ret = regmap_update_bits(adc_dev->regmap, MT6392_TYPE_C_CTRL, 0xffff, 0);
		if (ret < 0) {
			mutex_unlock(&adc_dev->lock);
			return ret;
		}

		ret = regmap_update_bits(adc_dev->regmap, MT6392_TYPE_C_CC_SW_FORCE_MODE_ENABLE_0, 0xffff, 0x5B8);
		if (ret < 0) {
			mutex_unlock(&adc_dev->lock);
			return ret;
		}

		ret = regmap_update_bits(adc_dev->regmap, MT6392_TYPE_C_CC_SW_FORCE_MODE_VAL_0, 0xffff, 0x100);
		if (ret < 0) {
			mutex_unlock(&adc_dev->lock);
			return ret;
		}
		udelay(1000);
		if (chan->channel == 5) {
			/* But AUXADC_RQST0_SET is bit 6 */
			ret = regmap_update_bits(adc_dev->regmap, MT6392_TYPE_C_CC_SW_FORCE_MODE_VAL_0, 0xffff, 0x1f40);
			if (ret < 0) {
				mutex_unlock(&adc_dev->lock);
				return ret;
			}
		} else {
				ret = regmap_update_bits(adc_dev->regmap, MT6392_TYPE_C_CC_SW_FORCE_MODE_VAL_0,
					0xffff, 0x1fc0);
				if (ret < 0) {
					mutex_unlock(&adc_dev->lock);
				return ret;
		...
		ret = regmap_update_bits(adc_dev->regmap, MT6392_TYPE_C_CC_SW_FORCE_MODE_ENABLE_0, 0xffff, 0x05fc);
		if (ret < 0) {
			mutex_unlock(&adc_dev->lock);
			return ret;
		}
	...
	if ((chan->channel == 7) || (chan->channel == 8)) {
		if (chan->channel == 7) {
			/* DP channel */
			/* But AUXADC_RQST0_SET is bit 8 */
			ret = regmap_update_bits(adc_dev->regmap, MT6392_CHR_CON18, 0x0f00, 0x0700);
			if (ret < 0) {
				mutex_unlock(&adc_dev->lock);
				return ret;
			}
			...
			/* DM channel */
			ret = regmap_update_bits(adc_dev->regmap, MT6392_CHR_CON18, 0x0f00, 0x0b00);
			if (ret < 0) {
				mutex_unlock(&adc_dev->lock);
				return ret;
			}
	...
	/* AUXADC_RQST0 SET  */
	if ((chan->channel == 5) || (chan->channel == 7))
		ret = regmap_update_bits(adc_dev->regmap, MT6392_AUXADC_RQST0_SET,
			MT6392_AUXADC_RQST0_SET_MASK, (MT6392_AUXADC_RQST0_SET_VAL(0x1) << (chan->channel + 1)));
	else
		ret = regmap_update_bits(adc_dev->regmap, MT6392_AUXADC_RQST0_SET,
	...
	do {
		if ((chan->channel == 5) || (chan->channel == 7))
			ret = regmap_read(adc_dev->regmap, mt6392_auxadc_regs[chan->channel + 1], &regval);
		else
