...
void msdc_detect_bad_sd(struct msdc_host *host, u32 condition)
{
	...
	if (++(bad_sd_detecter[condition]) >= bad_sd_tolerance[condition]) {
		...
		pr_err("msdc1: detected bad sd, just power off for bad sd\n");
	}
}
			...
			if ((cmd->opcode == 13) || (cmd->opcode == 25)) {
				...
				if ((*rsp & R1_OUT_OF_RANGE)
				 && (host->hw->host_function != MSDC_SDIO)) {
					pr_err("[%s]: msdc%d XXX CMD<%d> arg<0x%.8x> resp<0x%.8x>, out of range\n",
						__func__, host->id, cmd->opcode, cmd->arg,
						*rsp);
				}
			}
...
static void msdc_fde_switch_config(struct msdc_host *host, u32 block_address, u32 dir)
{
	...
	if (aes_sw_reg & EMMC52_AES_SWITCH_VALID0) {
		...
		pr_err("MSDC: EMMC52_AES_SWITCH_VALID error in msdc reg\n");
		WARN_ON(1);
		return;
	}
	...
	if (dir == DMA_TO_DEVICE) {
		...
		while (MSDC_READ32(EMMC52_AES_SWST) & EMMC52_AES_SWITCH_START_ENC) {
			if (time_after(jiffies, polling_tmo)) {
				pr_err("msdc%d, error: triger AES ENC timeout!\n", host->id);
				WARN_ON(1);
			}
		}
	} else{
		...
		while (MSDC_READ32(EMMC52_AES_SWST) & EMMC52_AES_SWITCH_START_DEC) {
			if (time_after(jiffies, polling_tmo)) {
				pr_err("msdc%d, error: triger DEC AES DEC timeout!\n", host->id);
				WARN_ON(1);
			}
		}
	}
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (msdc_get_ccf_clk_pointer(pdev, host))
		...
		return 1;
#else
		pr_err("[MSDC]msdc_get_ccf_clk_pointer fail.\n");
#endif
