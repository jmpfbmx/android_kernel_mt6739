...
{
#ifndef CONFIG_MTK_MSDC_BRING_UP_WORKAROUND
...
void msdc_dump_ldo_sts(struct msdc_host *host)
{
	...
	case 0:
		...
		pr_err(" VEMC_EN=0x%x, VEMC_VOL=0x%x [4b'1011(3V)], VEMC_CAL=0x%x\n",
	...
	case 1:
		...
		pr_err(" VMC_EN=0x%x, VMC_VOL=0x%x [4b'0100(1V8),4b'1011(3V)], VMC_CAL=0x%x\n",
			ldo_en, ldo_vol, ldo_cal);
		...
		pr_err(" VMCH_EN=0x%x, VMCH_VOL=0x%x [4b'1011(3V)], VMCH_CAL=0x%x\n",
			ldo_en, ldo_vol, ldo_cal);
		break;
	default:
		break;
...
void msdc_power_calibration_init(struct msdc_host *host)
{
	if (host->hw->host_function == MSDC_EMMC) {
		...
		/* VMC calibration +60mV to SD3.0. According to SA's request. */
	}
}
...
void msdc_pmic_force_vcore_pwm(bool enable)
{
	...
	if (vcore_pmic_set_mode(enable))
		pr_err("[msdc]error: vcore_pmic_set_mode fail\n");
#endif
}
...
void msdc_dump_padctl_by_id(u32 id)
{
	...
	if (id == 0) {
		...
		pr_err(
		"MSDC0 SMT [0x%p] =0x%8x\tshould:32'b........ ........ ........ .11111..\n",
			MSDC0_GPIO_SMT, MSDC_READ32(MSDC0_GPIO_SMT));
		pr_err(
		"MSDC0 IES [0x%p] =0x%8x\tshould:32'b........ ........ 11111111 1111....\n",
		...
		pr_err(
		"MSDC0 TDSEL [0x%p] =0x%8x\tshould:32'b....0000 00000000 00000000 ........\n",
			MSDC0_GPIO_TDSEL, MSDC_READ32(MSDC0_GPIO_TDSEL));
		pr_err(
		"MSDC0 RDSEL [0x%p] =0x%8x\tshould:32'b....0000 00000000 00000000 0000....\n",
			MSDC0_GPIO_RDSEL, MSDC_READ32(MSDC0_GPIO_RDSEL));

		pr_err(
		"MSDC0 DRV   [0x%p] =0x%8x\tshould:32'b........ ...10110 11011011 01......\n",
		...
		pr_err("should 1.8v & sleep & awake: 32'b00000000 0000.... ........ ........\n");

		pr_err("MSDC1 RDSEL0  [0x%p] =0x%8x\n",
			MSDC1_GPIO_RDSEL, MSDC_READ32(MSDC1_GPIO_RDSEL));
		pr_err("should 1.8v & sleep & awake: 32'b00000000 00000000 00...... ........\n");

		pr_err(
		"MSDC1 DRV    [0x%p] =0x%8x\tshould: 32'b........ 11011011 0....... ........\n",
...
void msdc_set_rdsel_by_id(u32 id, u32 flag, u32 value)
{
	...
	if (id == 0) {
		if (flag == MSDC_TDRDSEL_CUST)
			...
			cust_val = 0;
		MSDC_SET_FIELD(MSDC0_GPIO_RDSEL, 0x3F << 4,
			cust_val);
		MSDC_SET_FIELD(MSDC0_GPIO_RDSEL, 0x3F << 10,
			cust_val);
		MSDC_SET_FIELD(MSDC0_GPIO_RDSEL, 0x3F << 16,
			cust_val);
		MSDC_SET_FIELD(MSDC0_GPIO_RDSEL, 0x3F << 22,
			cust_val);
		MSDC_SET_FIELD(MSDC0_GPIO_RDSEL1, 0x3F,
			cust_val);
	} else if (id == 1) {
		if (flag == MSDC_TDRDSEL_CUST)
			cust_val = value;
...
{
	if (id == 0) {
		MSDC_GET_FIELD(MSDC0_GPIO_TDSEL, 0xF << 8,
			*value);
	} else if (id == 1) {
...
{
	if (id == 0) {
		MSDC_GET_FIELD(MSDC0_GPIO_RDSEL, 0x3F << 4,
			*value);
	} else if (id == 1) {
...
int msdc_of_parse(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	if ((host->id == 0) && !device_rename(mmc->parent, "bootdevice"))
		pr_err("[msdc%d] device renamed to bootdevice.\n", host->id);
	else if ((host->id == 1) && !device_rename(mmc->parent, "externdevice"))
		pr_err("[msdc%d] device renamed to externdevice.\n", host->id);
	else if ((host->id == 0) || (host->id == 1))
		pr_err("[msdc%d] error: device renamed failed.\n", host->id);
