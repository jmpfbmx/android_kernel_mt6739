...
unsigned long long g_system_part_start;
unsigned long long g_system_part_end;
#endif
...
void msdc_get_cache_region(struct work_struct *work)
{
	...
	/* Begin added by yinghe for XR5424657 on 2017/12/26 */
	#ifdef CONFIG_TCT_BIGLOG
	if (msdc_get_part_info("system", &part)) {
		g_system_part_start = part.start_sect;
		g_system_part_end = g_system_part_start + part.nr_sects;
	} else {
		g_system_part_start = (sector_t)(-1);
		g_system_part_end = (sector_t)(-1);
		pr_err("There is no cache info\n");
	}
	pr_err("system(0x%llX~0x%llX)\n",g_system_part_start, g_system_part_end);
	#endif
	/* End added by yinghe for XR5424657 on 2017/12/26 */
