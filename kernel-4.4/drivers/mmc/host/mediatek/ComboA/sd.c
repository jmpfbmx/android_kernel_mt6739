...
/* Added by wzhang for task 5495833 (memory informatin node) on 2017/11/03 begin */
extern struct device* get_deviceinfo_dev(void);
static u8 *emmc_spt_tab[][5] = {
	/*:::*/
	{"150100514536334D42", "KMQE60013M_B318",		"Samsung",	"16G+2G"}, // 1st default 
	{"90014A484147346132", "H9TQ17ABJTCCUR",		"Hynix",	"16G+2G"},
	{"150100464536324D42", "KMFE60012M_B214",		"Samsung",	"16G+1G"},
	{"700100454841434438", "16EMCP08_EL3DT527",		"Kingston",	"16G+1G"}, 
	{"150100513331364242", "KMQ310006B_B419",		"Samsung",	"16G+1G"},	{NULL,NULL,NULL,NULL},
};

static ssize_t emmc_id_show(struct device *dev, 
		struct device_attribute *attr, char *buf)
{
	...
	//u32 emmc_cap;		/* emmc total capacity. */
	/* Calculate the emmc density(kb). */
	//u32 sector_count = *(u32 *)(card->ext_csd.raw_sectors);
	//u32 user_area_size = (sector_count >> 10) << 9;
	//
	//emmc_cap = user_area_size + 
	//		((card->part[0].size + card->part[1].size + card->part[2].size)>>10);
	...
	return sprintf(buf, "%s:Unknown:Unknown:Unknown\n", emmc_id);
}

static ssize_t emmc_id_store(struct device *dev, 
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

DEVICE_ATTR(eMMC, S_IWUSR | S_IRUGO, emmc_id_show, emmc_id_store);

static int create_memory_id_node(void)
{
	struct device *mem_dev;
	mem_dev = get_deviceinfo_dev();
	if(device_create_file(mem_dev, &dev_attr_eMMC) < 0) {
		printk("Failed to create memory device_info node file.\n");
	}
   	return 0;
}
/* Added by wzhang for task 5495833 (memory informatin node) on 2017/11/03 end */
