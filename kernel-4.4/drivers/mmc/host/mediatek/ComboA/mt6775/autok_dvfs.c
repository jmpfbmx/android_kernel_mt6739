...
void msdc_dump_autok(struct msdc_host *host, struct seq_file *m)
{
	...
	for (i = AUTOK_VCORE_LEVEL1; i >= AUTOK_VCORE_LEVEL0; i--) {
		...
		if (!m) {
			pr_notice("[AUTOK]CMD [EDGE:%d CMD_FIFO_EDGE:%d DLY1:%d DLY2:%d]\r\n",
				host->autok_res[i][0], host->autok_res[i][1],
				host->autok_res[i][5], host->autok_res[i][7]);
			pr_notice("[AUTOK]DAT [RDAT_EDGE:%d RD_FIFO_EDGE:%d WD_FIFO_EDGE:%d]\r\n",
				host->autok_res[i][2], host->autok_res[i][3],
				host->autok_res[i][4]);
			pr_notice("[AUTOK]DAT [LATCH_CK:%d DLY1:%d DLY2:%d]\r\n",
...
int msdc_vcorefs_get_hw_opp(struct msdc_host *host)
{
	...
	if (host->hw->host_function == MSDC_SD)
		vcore = AUTOK_VCORE_LEVEL0;
	else if (host->hw->host_function == MSDC_EMMC || host->use_hw_dvfs == 0) {
