...
#define msdc_dma_off()          { MSDC_SET_BIT32(MSDC_CFG, MSDC_CFG_PIO); \
					MSDC_CLR_BIT32(EMMC52_AES_SW, EMMC52_AES_EN); }
#else
...
void msdc_dump_register_core(struct msdc_host *host, struct seq_file *m)
{
	...
	if (!m)
		pr_notice("MSDC%d normal register\n", id);
...
void msdc_dump_dbg_register(struct msdc_host *host)
{
	...
	memset(buffer, 0, PRINTF_REGISTER_BUFFER_SIZE);
	pr_notice("MSDC debug register [set:out]\n");
	...
	u32 hs400_div_dis)
{
	...
	do {
		...
		msdc_retry(!(MSDC_READ32(MSDC_CFG) & MSDC_CFG_CKSTB), retry,
			cnt, host->id);
		if (retry == 0) {
			pr_notice("msdc%d host->onclock(%d)\n", host->id,
				host->core_clkon);
			pr_notice("msdc%d on clock failed ===> retry twice\n",
				host->id);
...
void msdc_reset_bad_sd_detecter(struct msdc_host *host)
{
	u32 i = 0;
...
void msdc_detect_bad_sd(struct msdc_host *host, u32 condition)
{
	...
	if (++(bad_sd_detecter[condition]) >= bad_sd_tolerance[condition]) {
		...
		pr_notice("msdc1: detected bad sd, just power off for bad sd\n");
	}
}
...
void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	pr_notice("msdc%d -> !!! Set<%dKHz> Source<%dKHz> -> sclk<%dKHz> timing<%d> mode<%d> div<%d> hs400_div_dis<%d>",
		...
		if (is_card_sdio(host) || (host->hw->flags & MSDC_SDIO_IRQ)) {
			...
			if (host->hw->host_function == MSDC_SD) {
				...
				if (host->mclk == HOST_MIN_MCLK) {
					host->block_bad_card = 1;
					pr_notice(
				"[%s]: msdc%d power off at clk %dhz set block_bad_card = %d\n",
...
static void msdc_pm(pm_message_t state, void *data)
{
	...
	if ((evt == PM_EVENT_SUSPEND) || (evt == PM_EVENT_USER_SUSPEND)) {
		if ((host->hw->host_function == MSDC_SDIO) &&
			...
			pr_notice("msdc%d -> MSDC Device Request Suspend",
				host->id);
		}
		...
		if ((host->hw->host_function == MSDC_SDIO) &&
			...
			pr_notice("msdc%d -> MSDC Device Request Resume",
				host->id);
		}
		msdc_gate_clock(host, 1);
	}
...
int msdc_pio_read(struct msdc_host *host, struct mmc_data *data)
{
	...
	while (1) {
		 ...
		 * and not continuous
		 */
		/* pr_notice("msdc0: kmap not continuous %x %x %x\n",
...
int msdc_pio_write(struct msdc_host *host, struct mmc_data *data)
{
	...
	while (1) {
		 ...
		 */
		/* pr_notice(ERR "msdc0:w kmap not continuous %x %x %x\n",
		 * left, kaddr[i], kaddr[i-1]);
		 */
		for (i = 0; i < totalpages; i++) {
			left = PAGE_SIZE;
...
check_fifo_end:
	...
	if (hmpage != NULL) {
		for (i = 0; i < totalpages; i++)
			kunmap(hmpage + i);
		pr_notice("msdc0 write unmap 0x%x:\n", left);
	}
	data->bytes_xfered += size;
...
static void msdc_dma_start(struct msdc_host *host)
{
	...
	/* make sure that anny write operation executed after the barrier
	 * will not become visible until all writes executed prior to
	 * the barrier are visible.
	*/
	mb();
}
...
static void msdc_fde_switch_config(struct msdc_host *host, u32 block_address, u32 dir)
{
	...
	/* 1. set ctr */
	aes_sw_reg = MSDC_READ32(EMMC52_AES_SW);
	...
	/* 2. enable AES path */
	MSDC_SET_BIT32(EMMC52_AES_SW, EMMC52_AES_EN);
...
static void msdc_fde(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (check_mmc_cmd1718(cmd->opcode) || check_mmc_cmd2425(cmd->opcode)) {
		if (mrq->bi_hw_fde) {
			if (!host->is_fde_init || (host->key_idx != mrq->bi_key_idx)) {
				/* fde init */
				mt_secure_call(MTK_SIP_KERNEL_HW_FDE_MSDC_CTL, (1 << 3), 4, 1);
				host->is_fde_init = true;
	...
	struct mmc_request *mrq)
{
	...
	pr_notice("[%s]: msdc%d, discard task id %d, CMD<%d> arg<0x%08x> rsp<0x%08x>",
	...
	struct mmc_request *mrq, int *retry)
{
	...
	do {
		...
		if (err) {
			...
			pr_notice("msdc%d waiting data transfer done\n",
				host->id);
...
int msdc_error_tuning(struct mmc_host *mmc,  struct mmc_request *mrq)
{
	...
	if (mrq)
		pr_notice("%s: host->need_tune : 0x%x CMD<%d>\n", __func__,
			host->need_tune, mrq->cmd->opcode);

	pr_notice("msdc%d saved device status: %x", host->id, host->device_status);
	/* clear device status */
	...
	msdc_pmic_force_vcore_pwm(true); /* set PWM mode */
	...
	if (ret) {
		...
		msdc_dump_clock_sts(host);
		msdc_dump_ldo_sts(host);
		pr_notice("msdc%d latest_INT_status<0x%.8x>\n", host->id,
...
int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if ((host->hw->host_function == MSDC_SDIO) && (mmc->doing_retune)) {
		pr_notice("msdc%d: mmc retune do nothing\n", host->id);
		return 0;
	}
#endif
...
static int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_180:
		msdc_ungate_clock(host);
		pr_notice("%s msdc%d set voltage to 1.8V\n", __func__, host->id);
		/* switch voltage */
