...
static void msdc_dump_host_state(struct seq_file *m, struct msdc_host *host)
{
	...
	MSDC_PRINFO_PROC_MSG(m, "Accumulated dma cnt: %u\n", host->dma_cnt);
	if (host->start_dma_time < host->stop_dma_time)
		MSDC_PRINFO_PROC_MSG(m, "No pending dma: last start %llu, last stop %llu\n",
			host->start_dma_time, host->stop_dma_time);

	if (host->tuning_in_progress)
		MSDC_PRINFO_PROC_MSG(m, "tuning_in_progress %d\n",
			host->tuning_in_progress);

	if ((host->core_clkon == 0) && host->stop_dma_time
	&& (host->start_dma_time > host->stop_dma_time)) {
		MSDC_PRINFO_PROC_MSG(m, "DMA pending with clock gated: start %llu, stop %llu\n",
			host->start_dma_time, host->stop_dma_time);
		dbg_msdc_dump_clock_sts(m, host);
		return;
	}

	if ((host->core_clkon == 1) && (host->start_dma_time > host->stop_dma_time)) {
		MSDC_PRINFO_PROC_MSG(m, "DMA pending DMA_CFG_SATUS(%d): start %llu, stop %llu\n",
	...
	uint type, uint compare)
{
...
#ifdef CONFIG_MTK_EMMC_CQ_SUPPORT
	is_cmdq_en = false;
	if (host_num == 0 && host_ctl->mmc->card && host_ctl->mmc->card->ext_csd.cmdq_mode_en) {
	...
	for (forIndex = 0; forIndex < MSDC_MULTI_BUF_LEN; forIndex++) {
		if (rPtr[forIndex] != wData[forIndex % wData_len]) {
			pr_notice("index[%d]\tW_buffer[0x%x]\tR_buffer[0x%x]\tfailed\n",
	...
	for (i = 0; i < count; i++) {
		...
		if (error) {
			if (!multi_thread)
				...
				pr_notice("[%s]: failed to write data, error=%d\n",
				__func__, error);
			goto skip_read;
		}
...
skip_read:
		if (!multi_thread)
			...
			pr_notice("== cpu[%d] pid[%d]: %s %d time compare ==\n",
				task_cpu(current), current->pid,
				(error ? "FAILED" : "FINISH"), i);
	...
	if (i == count) {
		if (!multi_thread)
			...
			pr_notice("pid[%d]: success to compare data for %d times\n",
				current->pid, count);
	}
...
void msdc_dump_gpd_bd(int id)
{
	...
	if (bd == NULL) {
		...
		for (i = 0; i < host->dma.sglen; i++) {
			pr_notice("%4d BD eol:0x%x, rsv0:0x%x, chksum:0x%x, rsv1:0x%x,blkpad:0x%x,dwpad:0x%x,rsv2:0x%x\n",
...
static int msdc_debug_proc_show(struct seq_file *m, void *v)
{
	...
	if (cmd == SD_TOOL_ZONE) {
		...
		mode = p3;
		host = mtk_msdc_host[id];
		pr_notice("[****AutoK test****]msdc host_id<%d> vcore<%d> mode<%d>\n",
			id, vcore, mode);
