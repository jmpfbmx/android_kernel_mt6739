...
void msdc_get_cache_region(struct work_struct *work)
{
	...
	if (likely(lp_hd_struct)) {
		...
		pr_notice("There is no cache info\n");
	}
	...
	if (likely(lp_hd_struct)) {
		...
		pr_notice("There is no userdata info\n");
	}

	pr_notice("cache(0x%llX~0x%llX, usrdata(0x%llX~0x%llX)\n",
...
struct gendisk *mmc_get_disk(struct mmc_card *card)
{
	...
	if (!card) {
		pr_notice("[%s:%d] card is NULL", __func__, __LINE__);
		return NULL;
	}
	md = dev_get_drvdata(&card->dev);
	if (!md || !md->disk) {
		pr_notice("[%s:%d] md or disk is NULL", __func__, __LINE__);
		return NULL;
	}
