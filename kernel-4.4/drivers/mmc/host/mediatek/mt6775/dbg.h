...
do {    \
	if ((DBG_EVT_##evt) & sd_debug_zone[host->id]) { \
		pr_notice(TAGMSDC"%d -> "fmt" <- %s() : L<%d> PID<%s><0x%x>\n", \
...
do { \
	if (print_nums == 0) { \
		...
		pr_notice(TAGMSDC"MSDC", TAG"%d -> "fmt" <- %s() : L<%d> " \
			"PID<%s><0x%x>\n", \
			host->id, ##args, __func__, __LINE__, \
			current->comm, current->pid); \
	} else { \
		...
		if ((msdc_print_end_time - msdc_print_start_time) >= \
			MAX_PRINT_PERIOD) { \
			pr_notcie(TAGMSDC"MSDC", TAG"%d -> "fmt" <- %s() : L<%d> " \
				"PID<%s><0x%x>\n", \
				host->id, ##args, __func__, __LINE__, \
				current->comm, current->pid); \
			print_nums = 0; \
		...
		if (print_nums <= MAX_PRINT_NUMS_OVER_PERIOD) { \
			pr_notcie(TAGMSDC"MSDC", TAG"%d -> "fmt" <- %s() : " \
				"L<%d> PID<%s><0x%x>\n", \
