				    ...
				    struct mmc_async_req *areq, int *error)
{
	...
	if (areq)
	/* Begin added by hailong.chen for task 6619660 avoid areq NULL point on 2018.07.18 */
	{
		mmc_pre_req(host, areq->mrq, !host->areq);
	/* Begin Added by bin.song.hz for task 5451394 (emmc perf) on 2017.12.9 */
#ifdef CONFIG_CHECK_EMMC_WRITE_DATA
	    if (areq->mrq->data->flags == MMC_DATA_WRITE) {
	        host->perf.wbytes_drv += ((areq->mrq->data->blocks) * (areq->mrq->data->blksz));
	        pr_err("[BLOCK_TAG] host->perf.wbytes_drv =%lld \n", host->perf.wbytes_drv);		            
	    } else if (areq->mrq->data->flags == MMC_DATA_READ) {
	        host->perf.rbytes_drv += ((areq->mrq->data->blocks) * (areq->mrq->data->blksz));
	        pr_err("[BLOCK_TAG] host->perf.rbytes_drv =%lld \n", host->perf.rbytes_drv);		    
	    }
#endif
	/* End Added by bin.song.hz for task 5451394 (emmc perf) on 2017.12.9 */
	}
	/* End added by hailong.chen for task 6619660 avoid areq NULL point on 2018.07.18 */
