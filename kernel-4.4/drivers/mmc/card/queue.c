...
extern unsigned long long g_system_part_start;
extern unsigned long long g_system_part_end;
...
static int mmc_prep_request(struct request_queue *q, struct request *req)
{
	...
	/* Begin added by yinghe for XR5424657 on 2017/12/26 */
	#ifdef CONFIG_TCT_BIGLOG
	if (req && mq && mmc_card_mmc(mq->card)) {
		if ((rq_data_dir(req) == WRITE) && (((blk_rq_pos(req)>=g_system_part_start)&&(blk_rq_pos(req)<g_system_part_end)))) {
			writeCount++;
			sprintf(writebuf, "part system been write %d times. writeAddr= 0x%lx sysStardAddr= 0x%llx sysEndAddr= 0x%llx", \
				writeCount, (unsigned long)blk_rq_pos(req), g_system_part_start, g_system_part_end );
			system_abnormal_write(writebuf,strlen(writebuf));
		}
	}
	#endif
	/* End added by yinghe for XR5424657 on 2017/12/26 */
