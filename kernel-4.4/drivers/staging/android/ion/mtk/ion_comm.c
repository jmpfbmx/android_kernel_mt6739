...
static int ion_comm_cache_pool(void *data)
{
	...
	struct ion_heap *movable_ion_heap = ion_drv_get_heap(g_ion_device,
	...
	while (1) {
		...
		wait_event_interruptible(ion_comm_wq, atomic_read(&ion_comm_event));
		req_cache_size = atomic_read(&ion_comm_event);
		cache_buffer = atomic_read(&ion_comm_cache_event);
		atomic_set(&ion_comm_event, 0);

		cached_size = ion_mm_heap_pool_size(movable_ion_heap,
		...
		buffer->heap = movable_ion_heap;
		buffer->flags = 3;
		if (cache_buffer == 0)
			buffer->flags = 0;
		buffer->size = req_cache_size;
		ion_mm_heap_cache_allocate(movable_ion_heap,
					   buffer,
					   buffer->size,
					   0,
					   0);
...
int ion_comm_init(void)
{
	...
	ion_comm_kthread = kthread_run(ion_comm_cache_pool, NULL, "%s", "ion_comm_pool");
...
void ion_comm_event_notify(bool cache, size_t len)
{
	...
	IONMSG("%s event %d, new len %zu\n", __func__, atomic_read(&ion_comm_event), len);
