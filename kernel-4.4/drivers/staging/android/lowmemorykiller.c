...
#define LOWMEM_P_STATE_OTHER	(0x4)
	...
	/*Begin modified by fengke in task 5446848 6011904 for weixin close 2018.02.27*/
	int minfree_limit = 0;
	/*Begin modified by fengke in task 5446848 6011904 for weixin close 2018.02.27*/
#ifdef CONFIG_SWAP
	...
	/*End modified by fengke in task 5446848 6011904 for weixin close 2018.02.27*/


#ifdef CONFIG_SWAP
	swap_pages = atomic_long_read(&nr_swap_pages);
/*Begin modified by fengke in task 5446848 6011904 for weixin close 2018.02.27*/
	minfree_limit = lowmem_minfree[array_size - 1] * 2;
	/*lowmem_print(1, "PERF: defined CONFIG_A3A minfree_limit = %d, other_file = %d, swap_pages = %d, total_swap_pages = %d .\n",
                        minfree_limit, other_file, swap_pages, total_swap_pages);*/
        /* More than 2/3 swap usage */
        if (swap_pages * 3 < total_swap_pages) {
            to_be_aggressive++;
            if (minfree_limit < other_file) {
                lowmem_print(1, "PERF: (minfree_limit = %d) < other_file = %d, so do not aggressive.\n", minfree_limit, other_file);
                to_be_aggressive--;
            }
        }
#else
