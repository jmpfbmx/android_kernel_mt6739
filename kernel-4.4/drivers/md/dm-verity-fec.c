...
//add 20180730 by xiaorong.ling.hz for task6695234 begin
#ifdef CONFIG_SYSTEMIMG_FEC_ENABLE
#include <linux/syscalls.h>
#endif

/* Begin add smart info */
#ifdef CONFIG_TCT_BIGLOG
#include <linux/smart_info.h>
#include <linux/time.h>
#include <linux/rtc.h>
#endif
//add 20180730 by xiaorong.ling.hz for task6695234 end
...
//add 20180730 by xiaorong.ling.hz for task6695234 begin
/*
 * write flag and block to metadata. indicate the system.img is corrupted
 */
#ifdef CONFIG_SYSTEMIMG_FEC_ENABLE
#define METADATA "/dev/block/platform/bootdevice/by-name/smart"
#define SYSTEMPATH "/dev/block/platform/bootdevice/by-name/system"
#define VENDORPATH "/dev/block/platform/bootdevice/by-name/vendor"


#define FLAG_OFFSET (0xF00000)
#define SYSTEM_LIST_OFFSET (0xF01000)
#define VENDOR_LIST_OFFSET (0xF03000)
//every count take 4bit, so max count is 4k for one partition,include count flag
#define MAX_COUNT 1024


void verity_handle_state_info(int fd)
{
    int flag = 0;
    //read flag
    sys_lseek(fd, FLAG_OFFSET, SEEK_SET);
    sys_read(fd, (char *)&flag, sizeof(flag));
    if(flag != 1){
        flag = 1;
        //set flag
        sys_lseek(fd, FLAG_OFFSET, SEEK_SET);
        sys_write(fd, (char *)&flag, sizeof(flag));
    }
}

void verity_handle_bad_block_list(int fd, u32 list_offset, u32 block_num)
 {
	u32 count = 0;//unsigned int
	u32 offset = list_offset;

	//read blocks list count
	sys_lseek(fd, offset, SEEK_SET);
	sys_read(fd, (char *)&count, sizeof(count));
	if((s32)count < 0 ){
		count =0;
	}
	if(count >= MAX_COUNT){//get the max count,just return and never record
		return;
	}
	//write the count
	count ++;
	sys_lseek(fd, offset, SEEK_SET);
	sys_write(fd, (char *)&count, sizeof(count));
	//write the block
	offset += sizeof(count) * count;
	sys_lseek(fd, offset, SEEK_SET);
	sys_write(fd, (char *)&block_num, sizeof(block_num));
	DMERR_LIMIT("set blocks count: %d", count);
}

bool is_bdev(dev_t bd_dev, const char *path)
{
    dev_t path_bd_dev;
    struct block_device * path_bdev = lookup_bdev(path);

    if (IS_ERR(path_bdev)) {
        return false;
    }

    path_bd_dev = path_bdev->bd_dev;
    bdput(path_bdev);
    return(bd_dev == path_bd_dev);
}

bool is_system_bdev(dev_t bd_dev)
{
    return(is_bdev(bd_dev, SYSTEMPATH));
}

bool is_vendor_bdev(dev_t bd_dev)
{
    return(is_bdev(bd_dev, VENDORPATH));
}


int verity_handle_bad_blocks(dev_t bd_dev, u32 block_num)
{
    int fd = sys_open(METADATA, O_RDWR, 0);
    if(fd < 0){
        DMERR_LIMIT("open %s failed.", METADATA);
        return -1;
    }

    verity_handle_state_info(fd);

    if(is_system_bdev(bd_dev)) {
        DMERR_LIMIT("verity_handle_bad_blocks SYSTEM block: 0x%08x", block_num);
        verity_handle_bad_block_list(fd, SYSTEM_LIST_OFFSET, block_num);
    } else if(is_vendor_bdev(bd_dev)) {
        DMERR_LIMIT("verity_handle_bad_blocks VENDOR block: 0x%08x", block_num);
        verity_handle_bad_block_list(fd, VENDOR_LIST_OFFSET, block_num);
    } else {
        DMERR_LIMIT("unknown bad block: 0x%08x", block_num);
    }
...
//Begin add smart info
#ifdef CONFIG_TCT_BIGLOG
#define SMARTLOG_TIME_FORMAT \
	"%04d-%02d-%02d %02d:%02d:%02d"
#define SMARTLOG_TIME_SIZE (20)
void set_smartlog_time(bool failed){
	u8 time_str[20] = {0};
	struct timeval tv = { 0 };
	struct rtc_time tm;
	...
	//caculate local time to tm
	rtc_time_to_tm(tv.tv_sec - sys_tz.tz_minuteswest * 60, &tm);
	snprintf(time_str, SMARTLOG_TIME_SIZE, SMARTLOG_TIME_FORMAT,
		tm.tm_year + 1900, tm.tm_mon +1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	//DMWARN_LIMIT("%s: set smartlog time: %s\n", __FUNCTION__, time_str);
	if(failed){//failed time
		fec_time_write(NULL, time_str, NULL);
	}else{//decode time
		fec_time_write(time_str, NULL, NULL);
	}
}
#endif
//End add smart info
		      ...
		      struct bvec_iter *iter)
{
	...
	#ifdef CONFIG_SYSTEMIMG_FEC_ENABLE
	if(fio->level == 0){
		verity_handle_bad_blocks(v->data_dev->bdev->bd_dev, block);
	}
	#endif
	//add 20180730 by xiaorong.ling.hz for task6695234 end
	...
	//add 20180730 by xiaorong.ling.hz for task6695234 begin
	#ifdef CONFIG_TCT_BIGLOG
	set_smartlog_time(false);//decode time
	#endif
	//add 20180730 by xiaorong.ling.hz for task6695234 end
	/*
	...
	if (r < 0) {
		...
		if (r < 0){
			//add 20180730 by xiaorong.ling.hz for task6695234 begin
			#ifdef CONFIG_TCT_BIGLOG
			set_smartlog_time(true);//decode time
			#endif
			//add 20180730 by xiaorong.ling.hz for task6695234 end
			goto done;
		}
