...
void wdt_dump_reg(void)
{
	...
	pr_info("MTK_WDT_LATCH2_CTL:0x%x\n", __raw_readl(MTK_WDT_LATCH_CTL2));
	pr_info("MTK_WDT_RESET_PROTECT:0x%x\n", __raw_readl(MTK_WDT_RESET_PROTECT));
	pr_info("MTK_WDT_DEBUG_CTL2:0x%x\n", __raw_readl(MTK_WDT_DEBUG_CTL2));
	pr_info("MTK_WDT_DEBUG_1_REG:0x%x\n", __raw_readl(MTK_WDT_DEBUG_1_REG));
	pr_info("MTK_WDT_DEBUG_2_REG:0x%x\n", __raw_readl(MTK_WDT_DEBUG_2_REG));
	pr_info("MTK_WDT_DEBUG_3_REG:0x%x\n", __raw_readl(MTK_WDT_DEBUG_3_REG));
	pr_info("MTK_WDT_DEBUG_4_REG:0x%x\n", __raw_readl(MTK_WDT_DEBUG_4_REG));
...
int mtk_wdt_swsysret_config(int bit, int set_value)
{
	...
	case MTK_WDT_SWSYS_RST_C2K_RST: /* c2k reset */
		...
		if (set_value == 0)
			wdt_sys_val &= ~MTK_WDT_SWSYS_RST_C2K_RST;
		break;
	case MTK_WDT_SWSYS_RST_IPU_CONN_RST: /* ipu conn reset */
		if (set_value == 1)
			wdt_sys_val |= MTK_WDT_SWSYS_RST_IPU_CONN_RST;
		if (set_value == 0)
			wdt_sys_val &= ~MTK_WDT_SWSYS_RST_IPU_CONN_RST;
		break;
	case MTK_WDT_SWSYS_RST_IPU_CORE1_RST: /* ipu core1 reset */
		if (set_value == 1)
			wdt_sys_val |= MTK_WDT_SWSYS_RST_IPU_CORE1_RST;
		if (set_value == 0)
			wdt_sys_val &= ~MTK_WDT_SWSYS_RST_IPU_CORE1_RST;
		break;
	case MTK_WDT_SWSYS_RST_IPU_CORE2_RST: /* ipu core2 reset */
		if (set_value == 1)
			wdt_sys_val |= MTK_WDT_SWSYS_RST_IPU_CORE2_RST;
		if (set_value == 0)
			wdt_sys_val &= ~MTK_WDT_SWSYS_RST_IPU_CORE2_RST;
		break;
	case MTK_WDT_SWSYS_RST_IPU_CORE3_RST: /* ipu core3 reset */
		if (set_value == 1)
			wdt_sys_val |= MTK_WDT_SWSYS_RST_IPU_CORE3_RST;
		if (set_value == 0)
			wdt_sys_val &= ~MTK_WDT_SWSYS_RST_IPU_CORE3_RST;
		break;
	}
...
static int mtk_wdt_probe(struct platform_device *dev)
{
	...
	pr_info("mtk_wdt_probe: ext_debugkey_eint=%d\n", ext_debugkey_io_eint);
