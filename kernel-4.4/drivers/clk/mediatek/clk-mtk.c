...
void __init mtk_clk_register_factors_pdn(const struct mtk_fixed_factor_pdn *clks,
		int num, struct clk_onecell_data *clk_data, void __iomem *base)
{
	...
	for (i = 0; i < num; i++) {
		...
		clk = mtk_clk_register_fixed_factor_pdn(NULL, ff->name, ff->parent_name,
				CLK_SET_RATE_PARENT, ff->mult, ff->div, ff->shift, ff->pd_reg, base);
