...
static DEFINE_SPINLOCK(mt6775_clk_lock);
...
void __iomem *ipu_vcore_base;
void __iomem *ipu_core0_base;
void __iomem *ipu_core1_base;
void __iomem *ipu_core2_base;
...
#define CLK_SCP_CFG_2		(cksys_base + 0x0408)
...
#define ARMPLLGP_TEST_CON2	(apmixed_base + 0x0064)
...
#define IPU_CONN_APB_CONT			(ipu_conn_base + 0x0010)
#define IPU_CONN_AXI_CONT			(ipu_conn_base + 0x0018)
#define IPU_CONN_AXI_CONT1			(ipu_conn_base + 0x001c)
#define IPU_CONN_AXI_CONT2			(ipu_conn_base + 0x0020)

#define IPU_VCORE_CG_CON		(ipu_vcore_base + 0x0000)
#define IPU_VCORE_CG_SET		(ipu_vcore_base + 0x0004)
#define IPU_VCORE_CG_CLR		(ipu_vcore_base + 0x0008)
...
#define IPU_CORE2_CG_CON		(ipu_core2_base + 0x0000)
#define IPU_CORE2_CG_SET		(ipu_core2_base + 0x0004)
#define IPU_CORE2_CG_CLR		(ipu_core2_base + 0x0008)
...
#define INFRA_CG0 0x06E9CF00	/* for non-CCF Bringup */
#define INFRA_CG1 0x0248CA76
#define INFRA_CG3 0xBEFFFF7F

#define PERI_CG0 0x7FFFFFFF		/* for non-CCF Bringup */
...
#define IPU_CONN_CG				0x0000003F
#define IPU_CONN_APB_CONT_CG	/* not used */
#define IPU_CONN_AXI_CONT_CG	/* not used */
#define IPU_CONN_AXI_CONT1_CG	/* not used */
#define IPU_CONN_AXI_CONT2_CG	/* not used */
#define IPU_VCORE_CG			0x00000007
#define IPU_CORE0_CG			0x00000003
#define IPU_CORE1_CG			0x00000003
#define IPU_CORE2_CG			0x00000003
...
#define CK_CFG_UPDATE		0x900
#define CK_CFG_UPDATE_1		0x904
...
static const struct mtk_fixed_factor top_divs[] __initconst = {
	FACTOR(CLK_TOP_ARMPLL2, "armpll2_ck", "armpll_l",
		1, 1),
	FACTOR(CLK_TOP_ARMPLL3, "armpll3_ck", "armpll_b",
		1, 1),
	...
	FACTOR(CLK_TOP_UNIVPLL, "univpll_ck", "univpll",
		1, 1),
	FACTOR(CLK_TOP_AD_USB_192M, "ad_usb_192m", "univpll",
		1, 1),
	FACTOR(CLK_TOP_AD_USB20_96M, "ad_usb20_96m", "ad_usb_192m",
		1, 1),
	FACTOR(CLK_TOP_USB_PHY48M, "usb_phy48m_ck", "ad_usb_192m",
		1, 1),
	...
	FACTOR(CLK_TOP_UNIVPLL_D52, "univpll_d52", "univpll1_d2",	/* Univpll / 2 / 2 / 13 == Univpll / 52 */
		1, 13),
	FACTOR(CLK_TOP_UNIVPLL_D104, "univpll_d104", "univpll1_d2",	/* Univpll / 2 / 2 / 26 == Univpll / 104 */
		1, 26),
	FACTOR(CLK_TOP_UNIVPLL_D208, "univpll_d208", "univpll1_d2",	/* Univpll / 2 / 2 / 52 == Univpll / 208 */
		1, 52),
	FACTOR(CLK_TOP_UNIVPLL_D416, "univpll_d416", "univpll1_d2",	/* Univpll / 2 / 2 / 104 == Univpll / 416 */
		1, 104),
	...
	FACTOR(CLK_TOP_MMPLL, "mmpll_ck", "mmpll",
		1, 1),
	FACTOR(CLK_TOP_MMPLL_D3, "mmpll_d3", "mmpll",
		1, 3),
	...
	FACTOR(CLK_TOP_EMIPLL, "emipll_ck", "emipll",
		1, 1),
	FACTOR(CLK_TOP_EMIPLL_D2, "emipll_d2", "emipll",
		1, 2),
	...
	FACTOR(CLK_TOP_AD_SYS_26M, "ad_sys_26m_ck", "clk26m", 1,
		1),

	FACTOR(CLK_TOP_AD_SYS_26M_D2, "ad_sys_26m_ck_d2", "clk26m", 1,
		2),
	FACTOR(CLK_TOP_AD_SYS_26M_D52, "ad_sys_26m_ck_d52", "clk26m", 1,
		52),
	FACTOR(CLK_TOP_CLK26M_BYP, "clk26m_byp", "clk26m", 1,
		1),
...
#define CLK_TOP_AXI_SEL_UPD_SHIFT			0
#define CLK_TOP_MEM_SEL_UPD_SHIFT			1
#define CLK_TOP_DDRPHYCFG_SEL_UPD_SHIFT			2
#define CLK_TOP_DPI0_SEL_UPD_SHIFT			4
#define CLK_TOP_MM_SEL_UPD_SHIFT			3
#define CLK_TOP_PWM_SEL_UPD_SHIFT			5
#define CLK_TOP_DISPPWM_SEL_UPD_SHIFT			6
#define CLK_TOP_VDEC_SEL_UPD_SHIFT			7
#define CLK_TOP_VENC_SEL_UPD_SHIFT			8
#define CLK_TOP_MFG_SEL_UPD_SHIFT			9
#define CLK_TOP_CAMTG_SEL_UPD_SHIFT			10
#define CLK_TOP_CAMTG2_SEL_UPD_SHIFT			11
#define CLK_TOP_CAMTG3_SEL_UPD_SHIFT			12
#define CLK_TOP_CAMTG4_SEL_UPD_SHIFT			13
#define CLK_TOP_I2C_SEL_UPD_SHIFT			14
#define CLK_TOP_UART_SEL_UPD_SHIFT			15
#define CLK_TOP_SPI_SEL_UPD_SHIFT			16
#define CLK_TOP_USB30_P0_SEL_UPD_SHIFT			17
#define CLK_TOP_MSDC50_0_HCLK_SEL_UPD_SHIFT		18
#define CLK_TOP_MSDC50_0_SEL_UPD_SHIFT			19
#define CLK_TOP_MSDC30_1_SEL_UPD_SHIFT			20
#define CLK_TOP_I3C_SEL_UPD_SHIFT			21
#define CLK_TOP_MSDC30_3_SEL_UPD_SHIFT			22
#define CLK_TOP_MSDC50_3_HCLK_SEL_UPD_SHIFT		23
#define CLK_TOP_AUDIO_SEL_UPD_SHIFT			24
#define CLK_TOP_AUD_INTBUS_SEL_UPD_SHIFT		25
#define CLK_TOP_PMICSPI_SEL_UPD_SHIFT			26
#define CLK_TOP_SCP_SEL_UPD_SHIFT			27
#define CLK_TOP_ATB_SEL_UPD_SHIFT			28
#define CLK_TOP_DSP_SEL_UPD_SHIFT			29
#define CLK_TOP_DSP1_SEL_UPD_SHIFT			30
#define CLK_TOP_DSP2_SEL_UPD_SHIFT			31

#define CLK_TOP_DSP3_SEL_UPD_SHIFT			0
#define CLK_TOP_AUD_1_SEL_UPD_SHIFT			1
#define CLK_TOP_AUD_2_SEL_UPD_SHIFT			2
#define CLK_TOP_AUD_ENGEN1_SEL_UPD_SHIFT		3
#define CLK_TOP_AUD_ENGEN2_SEL_UPD_SHIFT		4
#define CLK_TOP_DFP_MFG_SEL_UPD_SHIFT			5
#define CLK_TOP_CAM_SEL_UPD_SHIFT			6
#define CLK_TOP_IPU_IF_SEL_UPD_SHIFT			7
#define CLK_TOP_IMG_SEL_UPD_SHIFT			8
#define CLK_TOP_AES_UFSFDE_SEL_UPD_SHIFT		9
#define CLK_TOP_AUDIO_H_SEL_UPD_SHIFT			10
#define CLK_TOP_SSPM_SEL_UPD_SHIFT			11
#define CLK_TOP_UFS_CARD_SEL_UPD_SHIFT			12
#define CLK_TOP_BSI_SPI_SEL_UPD_SHIFT			13
#define CLK_TOP_DXCC_SEL_UPD_SHIFT			14
#define CLK_TOP_SENINF_SEL_UPD_SHIFT			15
#define CLK_TOP_DFP_SEL_UPD_SHIFT			16
#define CLK_TOP_MEM2_UPD_SHIFT				17

#define CLK_TOP_MUX_UPD_CTRL_REG0			0x900
#define CLK_TOP_MUX_UPD_CTRL_REG1			0x904

#if MT_CCF_BRINGUP
static const struct mtk_mux_clr_set_upd top_muxes[] __initconst = {
	/* CLK_CFG_0 */
	MUX_CLR_SET_UPD(CLK_TOP_AXI_SEL, "axi_sel", axi_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 0, 3, INVALID_MUX_GATE,
	CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_AXI_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MEM_SEL, "mem_sel", mem_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 8, 2, INVALID_MUX_GATE,
	CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MEM_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DDRPHYCFG_SEL, "ddrphycfg_sel", ddrphycfg_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 16, 1, INVALID_MUX_GATE,
	CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_DDRPHYCFG_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MM_SEL, "mm_sel", mm_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 24, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MM_SEL_UPD_SHIFT),

	/* CLK_CFG_1 */
	MUX_CLR_SET_UPD(CLK_TOP_DPI0_SEL, "dpi0_sel", dpi0_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 0, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_DPI0_SEL_UPD_SHIFT),


	MUX_CLR_SET_UPD(CLK_TOP_PWM_SEL, "pwm_sel", pwm_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_PWM_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DISPPWM_SEL, "disppwm_sel", disppwm_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 16, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_DISPPWM_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_VDEC_SEL, "vdec_sel", vdec_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 24, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_VDEC_SEL_UPD_SHIFT),

	/* CLK_CFG_2 */
	MUX_CLR_SET_UPD(CLK_TOP_VENC_SEL, "venc_sel", venc_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 0, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_VENC_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_MFG_SEL, "mfg_sel", mfg_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MFG_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_CAMTG_SEL, "camtg_sel", camtg_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 16, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_CAMTG_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_CAMTG2_SEL, "camtg2_sel", camtg2_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 24, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_CAMTG2_SEL_UPD_SHIFT),

	/* CLK_CFG_3 */
	MUX_CLR_SET_UPD(CLK_TOP_CAMTG3_SEL, "camtg3_sel", camtg3_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR,	0, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_CAMTG3_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_CAMTG4_SEL, "camtg4_sel", camtg4_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR,	8, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_CAMTG4_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_I2C_SEL, "i2c_sel", i2c_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR,	16, 1, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_I2C_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_UART_SEL, "uart_sel", uart_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR, 24, 1, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_UART_SEL_UPD_SHIFT),

	/* CLK_CFG_4 */
	MUX_CLR_SET_UPD(CLK_TOP_SPI_SEL, "spi_sel", spi_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 0, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_SPI_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_USB30_P0_SEL, "usb30_p0_sel", usb30_p0_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_USB30_P0_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_MSDC50_0_HCLK_SEL, "msdc50_0_hclk_sel", msdc50_0_hclk_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 16, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC50_0_HCLK_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_MSDC50_0_SEL, "msdc50_0_sel", msdc50_0_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 24, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MSDC50_0_SEL_UPD_SHIFT),

	/* CLK_CFG_5 */
	MUX_CLR_SET_UPD(CLK_TOP_MSDC30_1_SEL, "msdc30_1_sel", msdc30_1_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 0, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MSDC30_1_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_I3C_SEL, "i3c_sel", i3c_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_I3C_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_MSDC30_3_SEL, "msdc30_3_sel", msdc30_3_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 16, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MSDC30_3_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_MSDC50_3_HCLK_SEL, "msdc50_3_hsel_sel", msdc50_3_hsel_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 24, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC50_3_HCLK_SEL_UPD_SHIFT),

	/* CLK_CFG_6 */
	MUX_CLR_SET_UPD(CLK_TOP_AUDIO_SEL, "audio_sel", audio_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 0, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_AUDIO_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_AUD_INTBUS_SEL, "aud_intbus_sel", aud_intbus_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 8, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_AUD_INTBUS_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_PMICSPI_SEL, "pmicspi_sel", pmicspi_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 16, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_PMICSPI_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_SCP_SEL, "scp_sel", scp_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 24, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_SCP_SEL_UPD_SHIFT),

	/* CLK_CFG_7 */
	MUX_CLR_SET_UPD(CLK_TOP_ATB_SEL, "atb_sel", atb_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 0, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_ATB_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DSP_SEL, "dsp_sel", dsp_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 8, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_DSP_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_AUD_1_SEL, "aud_1_sel", aud_1_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 16, 1, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_AUD_1_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_AUD_2_SEL, "aud_2_sel", aud_2_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 24, 1, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_AUD_2_SEL_UPD_SHIFT),

	/* CLK_CFG_8 */
	MUX_CLR_SET_UPD(CLK_TOP_AUD_ENGEN1_SEL, "aud_engen1_sel", aud_engen1_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR,	0, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUD_ENGEN1_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_AUD_ENGEN2_SEL, "aud_engen2_sel", aud_engen2_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR,	8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUD_ENGEN2_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DFP_MFG_SEL, "dfp_mfg_sel", dfp_mfg_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR, 16, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_DFP_MFG_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_CAM_SEL, "cam_sel", cam_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR, 24, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_CAM_SEL_UPD_SHIFT),

	/* CLK_CFG_9 */
	MUX_CLR_SET_UPD(CLK_TOP_IPU_IF_SEL, "ipu_if_sel", ipu_if_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 0, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_IPU_IF_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_IMG_SEL, "img_sel", img_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 8, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_IMG_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_AES_UFSFDE_SEL, "aes_ufsfde_sel", aes_ufsfde_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 16, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AES_UFSFDE_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_AUDIO_H_SEL, "audio_h_sel", audio_h_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 24, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_AUDIO_H_SEL_UPD_SHIFT),

	/* CLK_CFG_10 */
	MUX_CLR_SET_UPD(CLK_TOP_SSPM_SEL, "sspm_sel", sspm_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 0, 3, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_SSPM_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_UFS_CARD_SEL, "ufs_card_sel", ufs_card_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_UFS_CARD_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_BSI_SPI_SEL, "bsi_spi_sel", bsi_spi_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 16, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_BSI_SPI_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DXCC_SEL, "dxcc_sel", dxcc_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 24, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_DXCC_SEL_UPD_SHIFT),

	/* CLK_CFG_11 */
	MUX_CLR_SET_UPD(CLK_TOP_SENINF_SEL, "seninf_sel", seninf_parents, CK_CFG_11,
	CK_CFG_11_SET, CK_CFG_11_CLR, 0, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_SENINF_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DFP_SEL, "dfp_sel", dfp_parents, CK_CFG_11,
	CK_CFG_11_SET, CK_CFG_11_CLR, 8, 2, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_DFP_SEL_UPD_SHIFT),

	/* CLK_CFG_12 */
	MUX_CLR_SET_UPD(CLK_TOP_DSP1_SEL, "dsp1_sel", dsp1_parents, CK_CFG_12,
	CK_CFG_12_SET, CK_CFG_12_CLR, 0,  4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_DSP1_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DSP2_SEL, "dsp2_sel", dsp2_parents, CK_CFG_12,
	CK_CFG_12_SET, CK_CFG_12_CLR, 8,  4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_DSP2_SEL_UPD_SHIFT),
	MUX_CLR_SET_UPD(CLK_TOP_DSP3_SEL, "dsp3_sel", dsp3_parents, CK_CFG_12,
	CK_CFG_12_SET, CK_CFG_12_CLR, 16, 4, INVALID_MUX_GATE, CLK_TOP_MUX_UPD_CTRL_REG1, CLK_TOP_DSP3_SEL_UPD_SHIFT),
...
#define CLK_TOP_AXI_SEL_PDN_SHIFT			7
#define CLK_TOP_MEM_SEL_PDN_SHIFT			15
#define CLK_TOP_DDRPHYCFG_SEL_PDN_SHIFT			23
#define CLK_TOP_MM_SEL_PDN_SHIFT			31
#define CLK_TOP_DPI0_SEL_PDN_SHIFT			7
#define CLK_TOP_PWM_SEL_PDN_SHIFT			15
#define CLK_TOP_DISPPWM_SEL_PDN_SHIFT			23
#define CLK_TOP_VDEC_SEL_PDN_SHIFT			31
#define CLK_TOP_VENC_SEL_PDN_SHIFT			7
#define CLK_TOP_MFG_SEL_PDN_SHIFT			15
#define CLK_TOP_CAMTG_SEL_PDN_SHIFT			23
#define CLK_TOP_CAMTG2_SEL_PDN_SHIFT			31
#define CLK_TOP_CAMTG3_SEL_PDN_SHIFT			7
#define CLK_TOP_CAMTG4_SEL_PDN_SHIFT			15
#define CLK_TOP_I2C_SEL_PDN_SHIFT			23
#define CLK_TOP_UART_SEL_PDN_SHIFT			31
#define CLK_TOP_SPI_SEL_PDN_SHIFT			7
#define CLK_TOP_USB30_P0_SEL_PDN_SHIFT			15
#define CLK_TOP_MSDC50_0_HCLK_SEL_PDN_SHIFT		23
#define CLK_TOP_MSDC50_0_SEL_PDN_SHIFT			31
#define CLK_TOP_MSDC30_1_SEL_PDN_SHIFT			7
#define CLK_TOP_I3C_SEL_PDN_SHIFT			15
#define CLK_TOP_MSDC30_3_SEL_PDN_SHIFT			23
#define CLK_TOP_MSDC50_3_HCLK_SEL_PDN_SHIFT		31
#define CLK_TOP_AUDIO_SEL_PDN_SHIFT			7
#define CLK_TOP_AUD_INTBUS_SEL_PDN_SHIFT		15
#define CLK_TOP_PMICSPI_SEL_PDN_SHIFT			23
#define CLK_TOP_SCP_SEL_PDN_SHIFT			31
#define CLK_TOP_ATB_SEL_PDN_SHIFT			7
#define CLK_TOP_DSP_SEL_PDN_SHIFT			15
#define CLK_TOP_AUD_1_SEL_PDN_SHIFT			23
#define CLK_TOP_AUD_2_SEL_PDN_SHIFT			31
#define CLK_TOP_AUD_ENGEN1_SEL_PDN_SHIFT		7
#define CLK_TOP_AUD_ENGEN2_SEL_PDN_SHIFT		15
#define CLK_TOP_DFP_MFG_SEL_PDN_SHIFT			23
#define CLK_TOP_CAM_SEL_PDN_SHIFT			31
#define CLK_TOP_IPU_IF_SEL_PDN_SHIFT			7
#define CLK_TOP_IMG_SEL_PDN_SHIFT			15
#define CLK_TOP_AES_UFSFDE_SEL_PDN_SHIFT		23
#define CLK_TOP_AUDIO_H_SEL_PDN_SHIFT			31
#define CLK_TOP_SSPM_SEL_PDN_SHIFT			7
#define CLK_TOP_UFS_CARD_SEL_PDN_SHIFT			15
#define CLK_TOP_BSI_SPI_SEL_PDN_SHIFT			23
#define CLK_TOP_DXCC_SEL_PDN_SHIFT			31
#define CLK_TOP_SENINF_SEL_PDN_SHIFT			7
#define CLK_TOP_DFP_SEL_PDN_SHIFT			15
#define CLK_TOP_DSP1_SEL_PDN_SHIFT			7
#define CLK_TOP_DSP2_SEL_PDN_SHIFT			15
#define CLK_TOP_DSP3_SEL_PDN_SHIFT			23


static const struct mtk_mux_clr_set_upd top_muxes[] __initconst = {
	/* CLK_CFG_0 */
	MUX_CLR_SET_UPD(CLK_TOP_AXI_SEL, "axi_sel", axi_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 0, 3, /* CLK_TOP_AXI_SEL_PDN_SHIFT */ INVALID_MUX_GATE,
	CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_AXI_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MEM_SEL, "mem_sel", mem_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 8, 2, /* CLK_TOP_MEM_SEL_PDN_SHIFT */ INVALID_MUX_GATE,
	CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_MEM_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DDRPHYCFG_SEL, "ddrphycfg_sel", ddrphycfg_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 16, 1, /* CLK_TOP_DDRPHYCFG_SEL_PDN_SHIFT */ INVALID_MUX_GATE,
	CLK_TOP_MUX_UPD_CTRL_REG0, CLK_TOP_DDRPHYCFG_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MM_SEL, "mm_sel", mm_parents, CK_CFG_0,
	CK_CFG_0_SET, CK_CFG_0_CLR, 24, 4, CLK_TOP_MM_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MM_SEL_UPD_SHIFT),

	/* CLK_CFG_1 */
	MUX_CLR_SET_UPD(CLK_TOP_DPI0_SEL, "dpi0_sel", dpi0_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 0, 3, CLK_TOP_DPI0_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_DPI0_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_PWM_SEL, "pwm_sel", pwm_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 8, 2, CLK_TOP_PWM_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_PWM_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DISPPWM_SEL, "disppwm_sel", disppwm_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 16, 2, CLK_TOP_DISPPWM_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_DISPPWM_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_VDEC_SEL, "vdec_sel", vdec_parents, CK_CFG_1,
	CK_CFG_1_SET, CK_CFG_1_CLR, 24, 4, CLK_TOP_VDEC_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_VDEC_SEL_UPD_SHIFT),

	/* CLK_CFG_2 */
	MUX_CLR_SET_UPD(CLK_TOP_VENC_SEL, "venc_sel", venc_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 0, 4, CLK_TOP_VENC_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_VENC_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MFG_SEL, "mfg_sel", mfg_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 8, 2, CLK_TOP_MFG_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MFG_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_CAMTG_SEL, "camtg_sel", camtg_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 16, 3, CLK_TOP_CAMTG_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_CAMTG_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_CAMTG2_SEL, "camtg2_sel", camtg2_parents, CK_CFG_2,
	CK_CFG_2_SET, CK_CFG_2_CLR, 24, 3, CLK_TOP_CAMTG2_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_CAMTG2_SEL_UPD_SHIFT),

	/* CLK_CFG_3 */
	MUX_CLR_SET_UPD(CLK_TOP_CAMTG3_SEL, "camtg3_sel", camtg3_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR,	0, 3, CLK_TOP_CAMTG3_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_CAMTG3_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_CAMTG4_SEL, "camtg4_sel", camtg4_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR,	8, 3, CLK_TOP_CAMTG4_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_CAMTG4_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_I2C_SEL, "i2c_sel", i2c_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR,	16, 1, CLK_TOP_I2C_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_I2C_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_UART_SEL, "uart_sel", uart_parents, CK_CFG_3,
	CK_CFG_3_SET, CK_CFG_3_CLR, 24, 1, CLK_TOP_UART_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_UART_SEL_UPD_SHIFT),

	/* CLK_CFG_4 */
	MUX_CLR_SET_UPD(CLK_TOP_SPI_SEL, "spi_sel", spi_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 0, 3, CLK_TOP_SPI_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_SPI_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_USB30_P0_SEL, "usb30_p0_sel", usb30_p0_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 8, 2, CLK_TOP_USB30_P0_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_USB30_P0_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MSDC50_0_HCLK_SEL, "msdc50_0_hclk_sel", msdc50_0_hclk_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 16, 3, CLK_TOP_MSDC50_0_HCLK_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC50_0_HCLK_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MSDC50_0_SEL, "msdc50_0_sel", msdc50_0_parents, CK_CFG_4,
	CK_CFG_4_SET, CK_CFG_4_CLR, 24, 3, CLK_TOP_MSDC50_0_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC50_0_SEL_UPD_SHIFT),

	/* CLK_CFG_5 */
	MUX_CLR_SET_UPD(CLK_TOP_MSDC30_1_SEL, "msdc30_1_sel", msdc30_1_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 0, 3, CLK_TOP_MSDC30_1_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC30_1_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_I3C_SEL, "i3c_sel", i3c_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 8, 2, CLK_TOP_I3C_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_I3C_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MSDC30_3_SEL, "msdc30_3_sel", msdc30_3_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 16, 4, CLK_TOP_MSDC30_3_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC30_3_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_MSDC50_3_HCLK_SEL, "msdc50_3_hsel_sel", msdc50_3_hsel_parents, CK_CFG_5,
	CK_CFG_5_SET, CK_CFG_5_CLR, 24, 2, CLK_TOP_MSDC50_3_HCLK_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_MSDC50_3_HCLK_SEL_UPD_SHIFT),

	/* CLK_CFG_6 */
	MUX_CLR_SET_UPD(CLK_TOP_AUDIO_SEL, "audio_sel", audio_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 0, 2, CLK_TOP_AUDIO_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_AUDIO_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_AUD_INTBUS_SEL, "aud_intbus_sel", aud_intbus_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 8, 3, CLK_TOP_AUD_INTBUS_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_AUD_INTBUS_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_PMICSPI_SEL, "pmicspi_sel", pmicspi_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 16, 3, CLK_TOP_PMICSPI_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_PMICSPI_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_SCP_SEL, "scp_sel", scp_parents, CK_CFG_6,
	CK_CFG_6_SET, CK_CFG_6_CLR, 24, 3, CLK_TOP_SCP_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_SCP_SEL_UPD_SHIFT),

	/* CLK_CFG_7 */
	MUX_CLR_SET_UPD(CLK_TOP_ATB_SEL, "atb_sel", atb_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 0, 2, CLK_TOP_ATB_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_ATB_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DSP_SEL, "dsp_sel", dsp_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 8, 4, CLK_TOP_DSP_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_DSP_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_AUD_1_SEL, "aud_1_sel", aud_1_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 16, 1, CLK_TOP_AUD_1_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUD_1_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_AUD_2_SEL, "aud_2_sel", aud_2_parents, CK_CFG_7,
	CK_CFG_7_SET, CK_CFG_7_CLR, 24, 1, CLK_TOP_AUD_2_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUD_2_SEL_UPD_SHIFT),


	/* CLK_CFG_8 */
	MUX_CLR_SET_UPD(CLK_TOP_AUD_ENGEN1_SEL, "aud_engen1_sel", aud_engen1_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR,	0, 2, CLK_TOP_AUD_ENGEN1_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUD_ENGEN1_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_AUD_ENGEN2_SEL, "aud_engen2_sel", aud_engen2_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR,	8, 2, CLK_TOP_AUD_ENGEN2_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUD_ENGEN2_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DFP_MFG_SEL, "dfp_mfg_sel", dfp_mfg_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR, 16, 2, CLK_TOP_DFP_MFG_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_DFP_MFG_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_CAM_SEL, "cam_sel", cam_parents, CK_CFG_8,
	CK_CFG_8_SET, CK_CFG_8_CLR, 24, 4, CLK_TOP_CAM_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_CAM_SEL_UPD_SHIFT),


	/* CLK_CFG_9 */
	MUX_CLR_SET_UPD(CLK_TOP_IPU_IF_SEL, "ipu_if_sel", ipu_if_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 0, 4, CLK_TOP_IPU_IF_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_IPU_IF_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_IMG_SEL, "img_sel", img_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 8, 4, CLK_TOP_IMG_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_IMG_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_AES_UFSFDE_SEL, "aes_ufsfde_sel", aes_ufsfde_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 16, 3, CLK_TOP_AES_UFSFDE_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AES_UFSFDE_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_AUDIO_H_SEL, "audio_h_sel", audio_h_parents, CK_CFG_9,
	CK_CFG_9_SET, CK_CFG_9_CLR, 24, 2, CLK_TOP_AUDIO_H_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_AUDIO_H_SEL_UPD_SHIFT),

	/* CLK_CFG_10 */
	MUX_CLR_SET_UPD(CLK_TOP_SSPM_SEL, "sspm_sel", sspm_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 0, 3, CLK_TOP_SSPM_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_SSPM_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_UFS_CARD_SEL, "ufs_card_sel", ufs_card_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 8, 2, CLK_TOP_UFS_CARD_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_UFS_CARD_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_BSI_SPI_SEL, "bsi_spi_sel", bsi_spi_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 16, 2, CLK_TOP_BSI_SPI_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_BSI_SPI_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DXCC_SEL, "dxcc_sel", dxcc_parents, CK_CFG_10,
	CK_CFG_10_SET, CK_CFG_10_CLR, 24, 2, CLK_TOP_DXCC_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_DXCC_SEL_UPD_SHIFT),

	/* CLK_CFG_11 */
	MUX_CLR_SET_UPD(CLK_TOP_SENINF_SEL, "seninf_sel", seninf_parents, CK_CFG_11,
	CK_CFG_11_SET, CK_CFG_11_CLR, 0, 2, CLK_TOP_SENINF_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_SENINF_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DFP_SEL, "dfp_sel", dfp_parents, CK_CFG_11,
	CK_CFG_11_SET, CK_CFG_11_CLR, 8, 2, CLK_TOP_DFP_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_DFP_SEL_UPD_SHIFT),

	/* CLK_CFG_12 */
	MUX_CLR_SET_UPD(CLK_TOP_DSP1_SEL, "dsp1_sel", dsp1_parents, CK_CFG_12,
	CK_CFG_12_SET, CK_CFG_12_CLR, 0,  4,  CLK_TOP_DSP1_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_DSP1_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DSP2_SEL, "dsp2_sel", dsp2_parents, CK_CFG_12,
	CK_CFG_12_SET, CK_CFG_12_CLR, 8,  4, CLK_TOP_DSP2_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG0,
	CLK_TOP_DSP2_SEL_UPD_SHIFT),

	MUX_CLR_SET_UPD(CLK_TOP_DSP3_SEL, "dsp3_sel", dsp3_parents, CK_CFG_12,
	CK_CFG_12_SET, CK_CFG_12_CLR, 16, 4, CLK_TOP_DSP3_SEL_PDN_SHIFT, CLK_TOP_MUX_UPD_CTRL_REG1,
	CLK_TOP_DSP3_SEL_UPD_SHIFT),
...
static const struct mtk_gate infra_clks[] __initconst = {
	...
	GATE_INFRA1(CLK_INFRA_SMI_L2C, "infra_smi_l2c",
		"mm_sel" /* check again, it isn't used anymore */, 22),
...
static const struct mtk_gate pericfg_clks[] __initconst = {
	...
	/* add */
	GATE_PERICFG0(CLK_PERICFG_RG_I2C2_IMM_BCLK, "pericfg_rg_i2c2_imm_bclk",
		"i2c_sel", 24),
	GATE_PERICFG0(CLK_PERICFG_RG_I2C3_IMM_BCLK, "pericfg_rg_i2c3_imm_bclk",
		"i2c_sel", 25),
	...
	/* PERICFG3 */
	GATE_PERICFG3(CLK_PERICFG_RG_USB_P1, "pericfg_rg_usb_p1",
		"usb30_p0_sel", 1),
	/* PERICFG4 */
...
static const struct mtk_gate vdec_clks[] __initconst = {
	/* VDEC0 */
	GATE_VDEC0_I(CLK_VDEC_CKEN, "vdec_cken", "vdec_sel", 0),	/* todo check: why shift = 0 */
	/* VDEC1 */
...
static const struct mtk_gate venc_clks[] __initconst = {
	GATE_VENC_I(CLK_VENC_CKE0, "venc_cke0", "venc_sel", 4),
	GATE_VENC_I(CLK_VENC_CKE1, "venc_cke1", "venc_sel", 8),
};
...
static const struct mtk_gate ipu_conn_clks[] __initconst = {
	GATE_IPU_CONN(CLK_IPU_CONN_IPU_CG, "ipu_conn_ipu", "dsp_sel", 0),
	GATE_IPU_CONN(CLK_IPU_CONN_AHB_CG, "ipu_conn_ahb", "dsp_sel", 1),
	GATE_IPU_CONN(CLK_IPU_CONN_AXI_CG, "ipu_conn_axi", "dsp_sel", 2),
	GATE_IPU_CONN(CLK_IPU_CONN_ISP_CG, "ipu_conn_isp", "dsp_sel", 3),
	GATE_IPU_CONN(CLK_IPU_CONN_CAM_ADL_CG, "ipu_conn_cam_adl", "dsp_sel", 4),
	GATE_IPU_CONN(CLK_IPU_CONN_IMG_ADL_CG, "ipu_conn_img_adl", "dsp_sel", 5),

	/* DCM. They shouldn't be touched. */
	GATE_IPU_CONN_APB(CLK_IPU_CONN_DAP_RX_CLOCK_CG_EN, "ipu_conn_dap_rx", "dsp_sel", 0),
	GATE_IPU_CONN_APB(CLK_IPU_CONN_APB2AXI_BRIDGE_GATED_PCLK_EN, "ipu_conn_apb2aix", "dsp_sel", 3),
	GATE_IPU_CONN_APB(CLK_IPU_CONN_APB2AHB_BRIDGE_GATED_PCLK_EN, "ipu_conn_apb2ahb", "dsp_sel", 20),

	GATE_IPU_CONN_AXI(CLK_IPU_CONN_IPU_CAB1TO2_CG_DISABLE, "ipu_conn_ipu_cab1to2", "dsp_sel", 6),
	GATE_IPU_CONN_AXI(CLK_IPU_CONN_IPU1_CAB1TO2_CG_DISABLE, "ipu_conn_ipu1_cab1to2", "dsp_sel", 13),
	GATE_IPU_CONN_AXI(CLK_IPU_CONN_IPU2_CAB1TO2_CG_DISABLE, "ipu_conn_ipu2_cab1to2", "dsp_sel", 20),

	GATE_IPU_CONN_AXI1(CLK_IPU_CONN_CAB3TO3_CG_DISABLE, "ipu_conn_cab3to3", "dsp_sel", 0),

	GATE_IPU_CONN_AXI2(CLK_IPU_CONN_CAB2TO1_CG_DISABLE, "ipu_conn_ipu_cab2to1", "dsp_sel", 14),
	GATE_IPU_CONN_AXI2(CLK_IPU_CONN_CAB3TO1_SLICE_CG_DISABLE, "ipu_conn_ipu1_cab3to1", "dsp_sel", 17),
};


/* IPU Vcore CGs */
static const struct mtk_gate_regs ipu_vcore_cg_regs = {
	.set_ofs = 0x4,
	.clr_ofs = 0x8,
	.sta_ofs = 0x0,
};

#define GATE_IPU_VCORE(_id, _name, _parent, _shift) {		\
		.id = _id,				\
		.name = _name,				\
		.parent_name = _parent,			\
		.regs = &ipu_vcore_cg_regs,			\
		.shift = _shift,			\
		.ops = &mtk_clk_gate_ops_setclr,	\
	}

static const struct mtk_gate ipu_vcore_clks[] __initconst = {
	GATE_IPU_VCORE(CLK_IPU_VCORE_AHB_CG, "ipu_vcore_ahb", "ipu_if_sel", 0),
	GATE_IPU_VCORE(CLK_IPU_VCORE_AXI_CG, "ipu_vcore_axi", "ipu_if_sel", 1),
	GATE_IPU_VCORE(CLK_IPU_VCORE_ADL_CG, "ipu_vcore_adl", "ipu_if_sel", 2),
};
...
static const struct mtk_gate ipu_core0_clks[] __initconst = {
	GATE_IPU_CORE0(CLK_IPU_CORE0_JTAG_CG, "ipu_core0_jtag", "dsp1_sel", 0),
	GATE_IPU_CORE0(CLK_IPU_CORE0_AXI_M_CG, "ipu_core0_axi", "dsp1_sel", 1),
	GATE_IPU_CORE0(CLK_IPU_CORE0_IPU_CG, "ipu_core0_ipu", "dsp1_sel", 2),
};
...
static const struct mtk_gate ipu_core1_clks[] __initconst = {
	GATE_IPU_CORE1(CLK_IPU_CORE1_JTAG_CG, "ipu_core1_jtag", "dsp2_sel", 0),
	GATE_IPU_CORE1(CLK_IPU_CORE1_AXI_M_CG, "ipu_core1_axi", "dsp2_sel", 1),
	GATE_IPU_CORE1(CLK_IPU_CORE1_IPU_CG, "ipu_core1_ipu", "dsp2_sel", 2),
};

/* IPU Core2 CGs */
static const struct mtk_gate_regs ipu_core2_cg_regs = {
	.set_ofs = 0x4,
	.clr_ofs = 0x8,
	.sta_ofs = 0x0,
};
#define GATE_IPU_CORE2(_id, _name, _parent, _shift) {	\
		.id = _id,			\
		.name = _name,			\
		.parent_name = _parent,		\
		.regs = &ipu_core2_cg_regs,		\
		.shift = _shift,		\
		.ops = &mtk_clk_gate_ops_setclr,	\
	}
static const struct mtk_gate ipu_core2_clks[] __initconst = {
	GATE_IPU_CORE2(CLK_IPU_CORE2_JTAG_CG, "ipu_core2_jtag", "dsp3_sel", 0),
	GATE_IPU_CORE2(CLK_IPU_CORE2_AXI_M_CG, "ipu_core2_axi", "dsp3_sel", 1),
	GATE_IPU_CORE2(CLK_IPU_CORE2_IPU_CG, "ipu_core2_ipu", "dsp3_sel", 2),
};
...
static const struct mtk_gate mm_clks[] __initconst = {
	...
	GATE_MM1(CLK_MM_DISP_RSZ, "mm_disp_rsz", "mm_sel",
		9),
	GATE_MM1(CLK_MM_MDP_CCORR, "mm_mdp_ccorr", "mm_sel",
		10),
	GATE_MM1(CLK_MM_MDP_AAL, "mm_mdp_aal", "mm_sel",
		11),
	GATE_MM1(CLK_MM_GALS_IPU12MM, "mm_gals_ipu12mm", "mm_sel",
		12),
	GATE_MM1(CLK_MM_GALS_CCU2MM, "mm_gals_ccu2mm", "mm_sel",
		13),
#if 0
	/* AO */
...
static void __init mtk_topckgen_init(struct device_node *node)
{
	...
	clk_writel(CLK_SCP_CFG_2, clk_readl(CLK_SCP_CFG_2) | (0x1 << 3));
	/*mtk_clk_enable_critical();*/


	/* CK_CFG_0 */
	/* CK_CFG_1 PWM7 PDN */
	clk_writel(cksys_base + CK_CFG_1_CLR, BIT(CLK_TOP_PWM_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_1_SET, BIT(CLK_TOP_PWM_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_PWM_SEL_UPD_SHIFT));

	/* CK_CFG_2 CAMTG PDN */
	clk_writel(cksys_base + CK_CFG_2_CLR, BIT(CLK_TOP_CAMTG_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_2_SET, BIT(CLK_TOP_CAMTG_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_CAMTG_SEL_UPD_SHIFT));

	/* CK_CFG_2 CAMTG2 PDN */
	clk_writel(cksys_base + CK_CFG_2_CLR, BIT(CLK_TOP_CAMTG2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_2_SET, BIT(CLK_TOP_CAMTG2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_CAMTG2_SEL_UPD_SHIFT));

	/* CK_CFG_3 CAMTG3 PDN */
	clk_writel(cksys_base + CK_CFG_3_CLR, BIT(CLK_TOP_CAMTG3_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_3_SET, BIT(CLK_TOP_CAMTG3_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_CAMTG3_SEL_UPD_SHIFT));

	/* CK_CFG_3 CAMTG4 PDN */
	clk_writel(cksys_base + CK_CFG_3_CLR, BIT(CLK_TOP_CAMTG4_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_3_SET, BIT(CLK_TOP_CAMTG4_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_CAMTG4_SEL_UPD_SHIFT));

	/* CK_CFG_4 USB  PDN */
	/* clk_writel(cksys_base + CK_CFG_4_CLR, BIT(CLK_TOP_USB30_P0_SEL_PDN_SHIFT)); */
	/* clk_writel(cksys_base + CK_CFG_4_SET, BIT(CLK_TOP_USB30_P0_SEL_PDN_SHIFT)); */
	/* clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_USB30_P0_SEL_UPD_SHIFT)); */

	/* CK_CFG_5 msdc30_3_sel  PDN */
	clk_writel(cksys_base + CK_CFG_5_CLR, BIT(CLK_TOP_MSDC30_3_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_5_SET, BIT(CLK_TOP_MSDC30_3_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_MSDC30_3_SEL_UPD_SHIFT));
	/* CK_CFG_5 msdc50_3_sel  PDN */
	clk_writel(cksys_base + CK_CFG_5_CLR, BIT(CLK_TOP_MSDC50_3_HCLK_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_5_SET, BIT(CLK_TOP_MSDC50_3_HCLK_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_MSDC50_3_HCLK_SEL_UPD_SHIFT));

	/* CK_CFG_7 atb_sel  PDN */
	clk_writel(cksys_base + CK_CFG_7_CLR, BIT(CLK_TOP_ATB_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_7_SET, BIT(CLK_TOP_ATB_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_ATB_SEL_UPD_SHIFT));
	/* CK_CFG_7 dsp_sel  PDN */
	clk_writel(cksys_base + CK_CFG_7_CLR, BIT(CLK_TOP_DSP_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_7_SET, BIT(CLK_TOP_DSP_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_DSP_SEL_UPD_SHIFT));
	/* CK_CFG_7 aud_1  PDN */
	clk_writel(cksys_base + CK_CFG_7_CLR, BIT(CLK_TOP_AUD_1_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_7_SET, BIT(CLK_TOP_AUD_1_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_AUD_1_SEL_UPD_SHIFT));
	/* CK_CFG_7 atb_sel  PDN */
	clk_writel(cksys_base + CK_CFG_7_CLR, BIT(CLK_TOP_AUD_2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_7_SET, BIT(CLK_TOP_AUD_2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_AUD_2_SEL_UPD_SHIFT));

	/* CK_CFG_8 aud_engen1  PDN */
	clk_writel(cksys_base + CK_CFG_8_CLR, BIT(CLK_TOP_AUD_ENGEN1_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_8_SET, BIT(CLK_TOP_AUD_ENGEN1_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_AUD_ENGEN1_SEL_UPD_SHIFT));
	/* CK_CFG_8 aud_engen2  PDN */
	clk_writel(cksys_base + CK_CFG_8_CLR, BIT(CLK_TOP_AUD_ENGEN2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_8_SET, BIT(CLK_TOP_AUD_ENGEN2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_AUD_ENGEN2_SEL_UPD_SHIFT));
	/* CK_CFG_8 dfp_mfg  PDN */
	clk_writel(cksys_base + CK_CFG_8_CLR, BIT(CLK_TOP_DFP_MFG_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_8_SET, BIT(CLK_TOP_DFP_MFG_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_DFP_MFG_SEL_UPD_SHIFT));

	/* CK_CFG_9 ipu_if  PDN */
	clk_writel(cksys_base + CK_CFG_9_CLR, BIT(CLK_TOP_IPU_IF_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_9_SET, BIT(CLK_TOP_IPU_IF_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_IPU_IF_SEL_UPD_SHIFT));
	/* CK_CFG_9 audio_h  PDN */
	clk_writel(cksys_base + CK_CFG_9_CLR, BIT(CLK_TOP_AUDIO_H_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_9_SET, BIT(CLK_TOP_AUDIO_H_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_AUDIO_H_SEL_UPD_SHIFT));


	/* CK_CFG_10 sspm  PDN */
	/* clk_writel(cksys_base + CK_CFG_10_CLR, BIT(CLK_TOP_SSPM_SEL_PDN_SHIFT)); */
	/* clk_writel(cksys_base + CK_CFG_10_SET, BIT(CLK_TOP_SSPM_SEL_PDN_SHIFT)); */
	/* clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_SSPM_SEL_UPD_SHIFT)); */
	/* CK_CFG_10 ufs_card  PDN */
	clk_writel(cksys_base + CK_CFG_10_CLR, BIT(CLK_TOP_UFS_CARD_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_10_SET, BIT(CLK_TOP_UFS_CARD_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_UFS_CARD_SEL_UPD_SHIFT));
	/* CK_CFG_10 bsi_spi  PDN */
	clk_writel(cksys_base + CK_CFG_10_CLR, BIT(CLK_TOP_BSI_SPI_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_10_SET, BIT(CLK_TOP_BSI_SPI_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_BSI_SPI_SEL_UPD_SHIFT));
	/* CK_CFG_10 dxcc  PDN */
	/* clk_writel(cksys_base + CK_CFG_10_CLR, BIT(CLK_TOP_DXCC_SEL_PDN_SHIFT)); */
	/* clk_writel(cksys_base + CK_CFG_10_SET, BIT(CLK_TOP_DXCC_SEL_PDN_SHIFT)); */
	/* clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_DXCC_SEL_UPD_SHIFT)); */

	/* CK_CFG_11 dfp PDN */
	clk_writel(cksys_base + CK_CFG_11_CLR, BIT(CLK_TOP_DFP_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_11_SET, BIT(CLK_TOP_DFP_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_DFP_SEL_UPD_SHIFT));

	/* CK_CFG_12 dsp1 PDN */
	clk_writel(cksys_base + CK_CFG_12_CLR, BIT(CLK_TOP_DSP1_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_12_SET, BIT(CLK_TOP_DSP1_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_DSP1_SEL_UPD_SHIFT));
	/* CK_CFG_12 dsp2 PDN */
	clk_writel(cksys_base + CK_CFG_12_CLR, BIT(CLK_TOP_DSP2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_12_SET, BIT(CLK_TOP_DSP2_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE, BIT(CLK_TOP_DSP2_SEL_UPD_SHIFT));
	/* CK_CFG_12 dsp3 PDN */
	clk_writel(cksys_base + CK_CFG_12_CLR, BIT(CLK_TOP_DSP3_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_12_SET, BIT(CLK_TOP_DSP3_SEL_PDN_SHIFT));
	clk_writel(cksys_base + CK_CFG_UPDATE_1, BIT(CLK_TOP_DSP3_SEL_UPD_SHIFT));
}
...
static const struct mtk_gate_regs apmixed_cg_con0_regs = {
	.set_ofs = 0x0,
	.clr_ofs = 0x0,
	.sta_ofs = 0x0,
};

static const struct mtk_gate_regs apmixed_cg_con5_regs = {
	.set_ofs = 0x14,
	.clr_ofs = 0x14,
	.sta_ofs = 0x14,
};

#define GATE_APMIXED_CON0(_id, _name, _parent, _shift) {	\
		.id = _id,				\
		.name = _name,				\
		.parent_name = _parent,			\
		.regs = &apmixed_cg_con0_regs,		\
		.shift = _shift,			\
		.ops = &mtk_clk_gate_ops_inv,		\
	}

#define GATE_APMIXED_CON5(_id, _name, _parent, _shift) {	\
		.id = _id,				\
		.name = _name,				\
		.parent_name = _parent,			\
		.regs = &apmixed_cg_con5_regs,		\
		.shift = _shift,			\
		.ops = &mtk_clk_gate_ops_inv,		\
	}

static const struct mtk_gate apmixed_clks[] __initconst = {
	GATE_APMIXED_CON0(CLK_APMIXED_MIPIC1_26M, "apmixed_mipic126m", "f_f26m_ck",
		12),
	GATE_APMIXED_CON0(CLK_APMIXED_MIPID1_26M, "apmixed_mipid126m", "f_f26m_ck",
		13),
	GATE_APMIXED_CON0(CLK_APMIXED_MIPID0_26M, "apmixed_mipid026m", "f_f26m_ck",
		14),
	GATE_APMIXED_CON5(CLK_APMIXED_APLL1_TUNER_EN, "apmixed_apll1_tuner_en", "f_f26m_ck",
		0),
	GATE_APMIXED_CON5(CLK_APMIXED_APLL2_TUNER_EN, "apmixed_apll2_tuner_en", "f_f26m_ck",
		1),
};


/* todo: FIXME: modify FMAX */
#define MT6775_PLL_FMAX		(3800UL * MHZ)

#define CON0_MT6775_RST_BAR	BIT(24)
...
#define PLL_B(_id, _name, _reg, _pwr_reg, _en_mask, _flags, _pcwbits,	\
		...
		.en_mask = _en_mask,					\
		.flags = _flags,					\
		.rst_bar_mask = CON0_MT6775_RST_BAR,			\
		.fmax = MT6775_PLL_FMAX,				\
		.pcwbits = _pcwbits,					\
		.pd_reg = _pd_reg,					\
		.pd_shift = _pd_shift,					\
...
#if 1 /* for the request at vdec_sel clkmux when doing MMDVFS */
	PLL(CLK_APMIXED_EMIPLL, "emipll", 0x0290, 0x029C, 0x00000001, 0,
		22, 0x0294, 28, 0, 0x0294, 0),
#endif
...
static void __init mtk_apmixedsys_init(struct device_node *node)
{
	...
	clk_writel(AP_PLL_CON8, clk_readl(AP_PLL_CON8) & 0xFFFFFFFE); /* enable MainPLL delay mode */

	/* For MT6775 only. Turn off ARMPLL CK on FREQ Meter path, suggested by DE for pwr saving.
	 * bit[22]: CK enable for AD_APLL1_CK_VCORE
	 * bit[20]: CK enable for AD_CCIPLL_CK_VCORE
	 * bit[18]: CK enable for AD_ARMPLL_B_CK_VCORE
	 * bit[16]: CK enable for AD_ARMPLL_L_CK_VCORE
	 */
	clk_writel(ARMPLLGP_TEST_CON2, clk_readl(ARMPLLGP_TEST_CON2) & ~(0x00550000));
...
#else
	clk_writel(CAMSYS_CG_SET, CAM_DISABLE_CG);
#endif
}
...
static void __init mtk_ipu_conn_init(struct device_node *node)
{
	...
	clk_writel(IPU_CONN_APB_CONT, clk_readl(IPU_CONN_APB_CONT) & ~(0x1 << 0 | 0x1 << 3 | 0x1 << 20));
	clk_writel(IPU_CONN_AXI_CONT, clk_readl(IPU_CONN_AXI_CONT) | 0x1 << 6 | 0x1 << 13 | 0x1 << 20);
	clk_writel(IPU_CONN_AXI_CONT1, clk_readl(IPU_CONN_AXI_CONT1) | 0x1 << 0);
	clk_writel(IPU_CONN_AXI_CONT2, clk_readl(IPU_CONN_AXI_CONT2) | 0x1 << 14 | 0x1 << 17);
#else
...
static void __init mtk_ipu_vcore_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	clk_data = mtk_alloc_clk_data(CLK_IPU_VCORE_NR_CLK);

	mtk_clk_register_gates(node, ipu_vcore_clks, ARRAY_SIZE(ipu_vcore_clks), clk_data);
...
#if MT_CCF_BRINGUP
	clk_writel(IPU_VCORE_CG_CLR, IPU_VCORE_CG);
#else
	clk_writel(IPU_VCORE_CG_SET, IPU_VCORE_CG);
#endif
}
CLK_OF_DECLARE(mtk_ipu_vcore, "mediatek,ipu_vcore", mtk_ipu_vcore_init);
...
CLK_OF_DECLARE(mtk_ipu_core0, "mediatek,ipu_core0", mtk_ipu_core0_init);
...
CLK_OF_DECLARE(mtk_ipu_core1, "mediatek,ipu_core1", mtk_ipu_core1_init);


static void __init mtk_ipu_core2_init(struct device_node *node)
{
	struct clk_onecell_data *clk_data;
	void __iomem *base;
	int r;
	...
	clk_data = mtk_alloc_clk_data(CLK_IPU_CORE2_NR_CLK);

	mtk_clk_register_gates(node, ipu_core2_clks, ARRAY_SIZE(ipu_core2_clks), clk_data);
...
#if MT_CCF_BRINGUP
	clk_writel(IPU_CORE2_CG_CLR, IPU_CORE2_CG);
#else
	clk_writel(IPU_CORE2_CG_SET, IPU_CORE2_CG);
#endif
}
CLK_OF_DECLARE(mtk_ipu_core2, "mediatek,ipu_core2", mtk_ipu_core2_init);
...
#else
	clk_writel(VDEC_CKEN_CLR, VDE_DISABLE_CG);
	clk_writel(LARB1_CKEN_CLR, LARB1_DISABLE_CG);
#endif
}
...
#else
	clk_writel(VENC_CG_CLR, VEN_DISABLE_CG);
#endif
}
...
unsigned int mt_get_ckgen_freq(unsigned int ID)
{
	...
	/* For MT6775 only. Turn on ARMPLL CK on FREQ Meter path. */
	clk_writel(ARMPLLGP_TEST_CON2, clk_readl(ARMPLLGP_TEST_CON2) | 0x00550000);
	...
	/* For MT6775 only. Turn off ARMPLL CK on FREQ Meter path. */
	clk_writel(ARMPLLGP_TEST_CON2, clk_readl(ARMPLLGP_TEST_CON2) & ~(0x00550000));
...
void mp_enter_suspend(int id, int suspend)
{
	...
	} else if (id == 1) { /* big core -- ARMPLL 3 */
		if (suspend) {
			clk_writel(AP_PLL_CON3, clk_readl(AP_PLL_CON3) & 0xf7fdff7f);
			clk_writel(AP_PLL_CON4, clk_readl(AP_PLL_CON4) & 0xf7ffff7f);
		} else {
			clk_writel(AP_PLL_CON3, clk_readl(AP_PLL_CON3) | 0x08020080);
			clk_writel(AP_PLL_CON4, clk_readl(AP_PLL_CON4) | 0x08000080);
		}
	}
}
...
void pll_if_on(void)
{
	...
	if (clk_readl(ARMPLL1_CON0) & 0x1)
		pr_debug("suspend warning: ARMPLL1 is on!!!\n");
#endif
	if (clk_readl(ARMPLL2_CON0) & 0x1)
		pr_debug("suspend warning: ARMPLL2 is on!!!\n");
	if (clk_readl(ARMPLL3_CON0) & 0x1)
		pr_debug("suspend warning: ARMPLL3 is on!!!\n");
	...
	if (clk_readl(GPUPLL_CON0) & 0x1)
		pr_notice("suspend warning: GPUPLL is on!!!\n");
	...
	if (clk_readl(EMIPLL_CON0) & 0x1)
		pr_debug("suspend warning: EMIPLL is on!!!\n");
}
...
void check_ven_clk_sts(void)
{
	/* confirm ven clk */
	#if 0
	pr_notice("[CCF] %s: CLK_CFG_1 = 0x%08x\r\n", __func__, clk_readl(CLK_CFG_1));
	pr_notice("[CCF] %s: CLK_CFG_2 = 0x%08x\r\n", __func__, clk_readl(CLK_CFG_2));
	pr_notice("[CCF] %s: VCODECPLL_CON1 = 0x%08x\r\n", __func__, clk_readl(UNIVPLL_CON0));
	pr_notice("[CCF] %s: VCODECPLL_CON0 = 0x%08x\r\n", __func__, clk_readl(MMPLL_CON0));
	pr_notice("[CCF] %s: VENC_CG_CON = 0x%08x\r\n", __func__, clk_readl(VENC_CG_CON));
	pr_notice("[CCF] %s: axi = %dkhz\r\n", __func__, mt_get_ckgen_freq(1));
	pr_notice("[CCF] %s: vcodecpll_d7 = %dkhz\r\n", __func__, mt_get_abist2_freq(22));
	pr_notice("[CCF] %s: mmpll_d6 = %dkhz\r\n", __func__, mt_get_abist2_freq(19));
	pr_notice("[CCF] %s: ven = %dkhz\r\n", __func__, mt_get_ckgen_freq(9));
	pr_notice("[CCF] %s: vde = %dkhz\r\n", __func__, mt_get_ckgen_freq(8));
	#endif
}
...
static int __init clk_mt6775_init(void)
{
	/*timer_ready = true;*/
	/*mtk_clk_enable_critical();*/
