...
#define SPI_ERR(fmt, args...) pr_err("spi.c:%5d: <%s>" fmt, __LINE__, __func__, ##args)
#ifdef SPI_DEBUG
...
u8 spi_uuid[6][16] = {{0x09, 0x15, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
...
#define SPI_4B_ALIGN 0x4
	...
	if ((mode == DMA_TRANSFER) || (mode == OTHER1)) {
		if ((ms->cur_transfer->tx_buf != NULL)
			...
			spi_writel(ms, SPI_TX_SRC_REG, cpu_to_le64(ms->cur_transfer->tx_dma) & SPI_32BIS_MASK);
			spi_writel(ms, SPI_TX_SRC_REG_64, (cpu_to_le64(ms->cur_transfer->tx_dma)
				>> SPI_32BIS_SHIFT) & SPI_32BIS_MASK);
	...
	if ((mode == DMA_TRANSFER) || (mode == OTHER2)) {
		if ((ms->cur_transfer->rx_buf != NULL)
			...
			spi_writel(ms, SPI_RX_DST_REG, cpu_to_le64(ms->cur_transfer->rx_dma) & SPI_32BIS_MASK);
			spi_writel(ms, SPI_RX_DST_REG_64, (cpu_to_le64(ms->cur_transfer->rx_dma)
				>> SPI_32BIS_SHIFT) & SPI_32BIS_MASK);
...
static int mt_spi_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (of_property_read_u32(pdev->dev.of_node, "dma_addrmask", &(ms->dma_addrmask))) {
			dev_err(&pdev->dev,
