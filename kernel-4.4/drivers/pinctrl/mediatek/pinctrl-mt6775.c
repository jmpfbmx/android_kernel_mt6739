		...
		int pin, bool enable, bool isup, unsigned int arg)
{
	...
	pr_info("mtk_pinctrl_set_gpio_pull, pin = %d, enab = %d, sel = %d\n",
		pin, enable, isup);
	ret = mtk_pinctrl_set_gpio_pupd_r1r0(pctl, pin, enable, isup, arg);
	if (ret != 0)
		mtk_pinctrl_set_gpio_pu_pd(pctl, pin, enable, isup, arg);
...
static const unsigned int mt6775_debounce_data[] = {
	128, 256, 512, 1024, 16384,
	32768, 65536, 131072, 262144, 524288
};

static unsigned int mt6775_spec_debounce_select(unsigned debounce)
{
	return mtk_gpio_debounce_select(mt6775_debounce_data,
		ARRAY_SIZE(mt6775_debounce_data), debounce);
}
...
static const struct mtk_pinctrl_devdata mt6775_pinctrl_data = {
	.pins = mtk_pins_mt6775,
	.npins = ARRAY_SIZE(mtk_pins_mt6775),
	...
	.spec_debounce_select = mt6775_spec_debounce_select,
...
{
	pr_info("pinctrl probe\n");
	return mtk_pctrl_init(pdev, &mt6775_pinctrl_data, NULL);
}
