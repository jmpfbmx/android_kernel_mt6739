...
/**
 * Add a XFRM output mark to message if required
 */
static bool add_output_mark(struct nlmsghdr *hdr, int buflen, u_int32_t output_mark)
{
	u_int32_t *out_xmrk;

	out_xmrk = netlink_reserve(hdr, buflen, XFRMA_OUTPUT_MARK, sizeof(*out_xmrk));
	if (!out_xmrk)
	{
		return FALSE;
	}
	*out_xmrk = output_mark;
	return TRUE;
}
		...
		traffic_selector_t* src_ts, traffic_selector_t* dst_ts)
{
	...
	DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY,
			"adding SAD entry with SPI %.8x and reqid {%u}  (mark %u/0x%08x)",
			ntohl(spi), reqid, mark.value, mark.mask);
		...
		case ENCR_CAMELLIA_CCM_ICV8:
			{
				...
				DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "  using encryption algorithm %N with key size %d",
	...
	DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY,
			"  using encryption key %B", &enc_key
			);
	...
	if (!add_output_mark(hdr, sizeof(request), 0x30000))
	{
		DBG0(DBG_KNL, "add XFRMA_OUTPUT_MARK failed");
	}
			...
			&& get_cust_setting_bool(ENCR_KEY_DISPLAY))
	{
		...
		if(sa->family == AF_INET)
			strcpy(ip_family, "IPv4");
		else if(sa->family == AF_INET6)
			strcpy(ip_family, "IPv6");

		chunk_to_hex(enc_key, enc_key_str, 1);
		chunk_to_hex(int_key, int_key_str, 1);

		if( get_log_file( &fp, ESP_SA_PREFIX) ) {

			fprintf(fp, "\"%s\",\"0x%.8x\",\"%H\",\"%H\",\"%N\",\"%s\",\"%N\",\"%s\"\n",
					"IPv4", ntohl(spi), src, dst,
					encryption_algorithm_names, enc_alg, enc_key_str,
					integrity_algorithm_names, int_alg, int_key_str
				   );

			fclose(fp);
		}
		else {
			DBG1(DBG_IKE, "ESP_SA table cannot be opened");
		}
...
#ifdef STRONGSWAN_UNIT_TEST
/* operation has no permisssion */
   DBG1(DBG_KNL, "SKIP setting IPSEC_POLICY on socket in UT");
   return TRUE;
#endif
...
#ifdef STRONGSWAN_UNIT_TEST
/*not yet implemented support for UT*/
#else
