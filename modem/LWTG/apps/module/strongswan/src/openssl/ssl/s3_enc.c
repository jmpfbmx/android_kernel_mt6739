...
//print master key and id session_id;
{
  ...
  for (i=0;i<SSL3_MASTER_SECRET_SIZE;i++)
  {
  	 if(out[i]=='\0' )
  	 	  break ;
 	 sprintf((char *)&dumpbuffer[i*2],"%02x",out[i]);
  	}
    dumpbuffer[i*2]='\0' ;
  
   LOGD("ssl(S3)=%p, MasterKey=%s ",s,dumpbuffer);
   ...
   memset(dumpbuffer,0,sizeof(dumpbuffer));	  
	  ...
	  for (i=0;i<SSL_MAX_SSL_SESSION_ID_LENGTH;i++)
	  {
	   ...
	   LOGD("ssl(S3)=%p, SessionID=%s ",s,dumpbuffer);
	   LOGD(" SessionID Length = %d", i);
   }else{
      LOGD("ssl(S3)=%p, Session=null ",s);
   }
	  //client_random
   ...
   memset(dumpbuffer,0,sizeof(dumpbuffer));
	...
	dumpbuffer[i*2]='\0' ;
	 ...
	 LOGD("ssl(S3)=%p, ClientRandom=%s ",s,dumpbuffer);
