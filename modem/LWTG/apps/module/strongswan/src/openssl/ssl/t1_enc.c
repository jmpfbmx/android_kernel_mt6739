...

{
  ...
  {
  	 if(s->session->master_key[i]=='\0' )
  	 	  break ;
  	 sprintf((char *)&dumpbuffer[i*2],"%02x",s->session->master_key[i]);
  	}
    dumpbuffer[i*2]='\0' ;
  
   LOGD("ssl(T1)=%p, MasterKey=%s ",s,dumpbuffer);
   ...
   //session_id
	memset(dumpbuffer,0,sizeof(dumpbuffer));
	  ...
	  {
		 if(s->session->session_id[i]=='\0' )
			  break ;
		 sprintf((char *)&dumpbuffer[i*2],"%02x",s->session->session_id[i]);
		}
		dumpbuffer[i*2]='\0' ;
	  
	   LOGD("ssl(T1)=%p, SessionID=%s ",s,dumpbuffer);
   ...
   memset(dumpbuffer,0,sizeof(dumpbuffer));
	  ...
	  {
		 if(s->s3->client_random[i]=='\0' )
			  break ;
		 sprintf((char *)&dumpbuffer[i*2],"%02x",s->s3->client_random[i]);
		}
		dumpbuffer[i*2]='\0' ;
	  
	   LOGD("ssl(T1)=%p, ClientRandom=%s ",s,dumpbuffer);
