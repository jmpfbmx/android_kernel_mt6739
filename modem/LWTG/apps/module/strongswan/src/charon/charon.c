...
#if (defined (STRONGSWAN_UNIT_TEST) && defined (DEBUG))
	if (level <= DEBUG)
#else
	if (level <= 1)
#endif
...
#endif
		fprintf(stderr, "%02i[%N] ", (int)level, debug_names, group);
#else
...
#ifdef ANDROID
//	lib->caps->set_uid(lib->caps, AID_VPN);		//	mask due to access right of property_set
//	lib->caps->set_uid(lib->caps, AID_SYSTEM);
//	lib->caps->set_gid(lib->caps, AID_SYSTEM);
#endif
	return TRUE;
}
...
int main(int argc, char *argv[])
{
	 ...
	 * Backtrace is not enabled in segv_handler.
	 * We need system backtrace log to analyze where it crashed.
