...
int IKEV2_UDP_PORT = 10500;
int CHARON_UDP_PORT = 10500;
int IKEV2_NATT_PORT = 14500;
int CHARON_NATT_PORT = 14500;
#endif
...
#ifdef STRONGSWAN_UNIT_TEST
	if (family == AF_INET6 && setsockopt(skt, SOL_IPV6, IPV6_V6ONLY, (void*)&on, sizeof(on)) < 0)
	{
...
#if defined (ANDROID)
		memset(&mark, 0, sizeof(mark_t));
		getsockopt(skt, SOL_SOCKET, SO_MARK, &mark.value, &len);
		DBG0(DBG_NET, "set protectFromVpn bit, fd: <%d>", skt);
		mark.value |= (0x1 << 17);
							...
							int *skt, int *skt_natt, char *label)
{
	...
	set_ut_st_ike_port(this->port);
	set_ut_st_ike_port_natt(this->natt);
	IKEV2_UDP_PORT = this->port;
	CHARON_UDP_PORT = this->port;
	IKEV2_NATT_PORT = this->natt;
	CHARON_NATT_PORT = this->natt;
	DBG1(DBG_NET, "open_socketpair successfully: ike port:%i and natt port:%i", this->port,this->natt);
#else
	*skt = open_socket(this, family, &this->port);
	if (*skt == -1)
	{
...
socket_default_socket_t *socket_default_socket_create()
{
	...
	if ((port = get_cust_setting_int(IKE_PORT)) > -1)
	{
		this->port = (u_int16_t)port;
		DBG1(DBG_NET, "IKE port: %u", this->port);
	}

	if ((port_natt = get_cust_setting_int(IKE_PORT_NATT)) > -1)
	{
		this->natt = (u_int16_t)port_natt;
		DBG1(DBG_NET, "IKE NATT port: %u", this->natt);
	}
