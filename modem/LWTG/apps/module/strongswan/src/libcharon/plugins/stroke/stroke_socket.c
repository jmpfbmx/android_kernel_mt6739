...
static void pop_end(stroke_msg_t *msg, const char* label, stroke_end_t *end)
{
	...
	DBG2(DBG_CFG, "  %simei=%s", label, end->imei);
...
static void stroke_add_conn(private_stroke_socket_t *this, stroke_msg_t *msg)
{
	...
	pop_string(msg, &msg->add_conn.iface);
	...
	DBG2(DBG_CFG, "  certreq_critical=%d", msg->add_conn.certreq_critical);
	DBG2(DBG_CFG, "  reauth_mbb=%d", msg->add_conn.reauth_mbb);
	DBG2(DBG_CFG, "  pcscf_restore=%d", msg->add_conn.pcscf_restore);
	DBG2(DBG_CFG, "  is_emergency=%d", msg->add_conn.is_emergency);
	DBG2(DBG_CFG, "  device_identity=%d", msg->add_conn.device_identity);
...
/**
 * data path status change handle
 */
static void stroke_data_path_status_notify(private_stroke_socket_t *this, stroke_msg_t *msg, FILE *out)
{
	pop_string(msg, &msg->data_path_status_notify.status);
	pop_string(msg, &msg->data_path_status_notify.if_name);
	DBG1(DBG_CFG, "received stroke: data_path_status_notify(%s), interface(%s)", msg->data_path_status_notify.status, msg->data_path_status_notify.if_name);

	this->control->data_path_status_notify(this->control, msg, out);
}

/**
 * enable/disable keep-alive
 */
static void stroke_wdrv_keepalive_notify(private_stroke_socket_t *this, stroke_msg_t *msg, FILE *out)
{
	pop_string(msg, &msg->wdrv_keepalive_notify.name);
	DBG1(DBG_CFG, "received stroke: wdrv_keepalive_notify(%d), name(%s)", msg->wdrv_keepalive_notify.enable, msg->wdrv_keepalive_notify.name);

	this->control->wdrv_keepalive_notify(this->control, msg, out);
}
...
static bool on_accept(private_stroke_socket_t *this, stream_t *stream)
{
	...
	switch (msg->type)
	{
		...
		case STR_DPD:
			...
			break;
		case STR_DATA_PATH_NOTIFY:
			stroke_data_path_status_notify(this, msg, out);
			break;
		case STR_WDRV_KEEPALIVE_NOTIFY:
			stroke_wdrv_keepalive_notify(this, msg, out);
			break;
		default:
