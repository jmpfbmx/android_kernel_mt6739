...
static ike_cfg_t *build_ike_cfg(private_stroke_config_t *this, stroke_msg_t *msg)
{
	...
	ike_cfg_t *ike_cfg;
	host_t *host;
	u_int16_t ikeport, ikeport_natt;
...
#ifdef STRONGSWAN_UNIT_TEST
/*update ike ports according reserved by socket_deafault_socket.c*/
	msg->add_conn.me.ikeport = get_ut_st_ike_port();
	msg->add_conn.me.ikeport_natt = get_ut_st_ike_port_natt();
	msg->add_conn.other.ikeport = get_ut_tc_ike_port();
#endif

	ikeport = msg->add_conn.me.ikeport;
	ikeport = (ikeport == IKEV2_UDP_PORT) ?
			   charon->socket->get_port(charon->socket, FALSE) : ikeport;
	ikeport_natt = msg->add_conn.me.ikeport_natt;
	ikeport_natt = (ikeport == IKEV2_NATT_PORT) ?
			   charon->socket->get_port(charon->socket, TRUE) : ikeport_natt;
	...
	ike_cfg->set_wod_options(ike_cfg, ikeport_natt, msg->add_conn.other.ikeport_natt);
								  ...
								  stroke_msg_t *msg, bool local, bool primary)
{
	...
	if (end->idr_apn)
	{
		identity = identification_create_from_string(end->idr_apn);
		cfg->add(cfg, AUTH_RULE_IDENTITY_APN, identity);
		DBG1(DBG_CFG, "IDr_APN: %s", end->idr_apn);
	}
	return cfg;
}
								  ...
								  stroke_msg_t *msg, ike_cfg_t *ike_cfg)
{
	...
	peer_cfg->set_wod_options(peer_cfg,
		msg->add_conn.me.imei, msg->add_conn.me.sourceif,
		msg->add_conn.me.cust_cp_pcscf4, msg->add_conn.me.cust_cp_pcscf6,
		msg->add_conn.me.cust_cp_imei, msg->add_conn.use_cfg_vip,
		msg->add_conn.wdrv_keepalive, msg->add_conn.keepalive_delay,
		msg->add_conn.oos_timeout, msg->add_conn.retrans_to,
		msg->add_conn.retrans_tries, msg->add_conn.retrans_base,
		msg->add_conn.no_init_contact, msg->add_conn.no_eap_only,
		msg->add_conn.hashandurl, msg->add_conn.ocsp,
		msg->add_conn.addrchg_reauth, msg->add_conn.fast_reauth,
		msg->add_conn.dpd.noreply, msg->add_conn.skip_check_id,
		msg->add_conn.skip_check_cert, msg->add_conn.server_nocert,
		msg->add_conn.liveness_check, msg->add_conn.certreq_critical,
		msg->add_conn.pcscf_restore, msg->add_conn.is_emergency,
		msg->add_conn.device_identity, msg->add_conn.reauth_mbb,
		msg->add_conn.detach_timeout);
	...
	if (msg->add_conn.me.pcscf)
	{
		enumerator_t *enumerator;
		char *token;

		enumerator = enumerator_create_token(msg->add_conn.me.pcscf, ",", " ");
		while (enumerator->enumerate(enumerator, &token))
		{
			host_t *pcscfip = NULL;

			if (streq(token, "%config4"))
			{
				pcscfip = host_create_any(AF_INET);
			}
			else if (streq(token, "%config6"))
			{
				pcscfip = host_create_any(AF_INET6);
			}
			else
			{
				pcscfip = host_create_from_string(token, 0);
				if (!pcscfip)
				{
					DBG1(DBG_CFG, "ignored invalid subnet token: %s", token);
				}
			...
			if (pcscfip)
			{
				peer_cfg->add_pcscf(peer_cfg, pcscfip);
			}
		}
		enumerator->destroy(enumerator);
	}

	if (msg->add_conn.me.intnetmask)
	{
		enumerator_t *enumerator;
		char *token;
		enumerator = enumerator_create_token(msg->add_conn.me.intnetmask, ",", " ");
		while (enumerator->enumerate(enumerator, &token))
		{
			host_t *ip = NULL;
			if (streq(token, "%config4"))
			{
				ip = host_create_any(AF_INET);
			}
			else if (streq(token, "%config6"))
			{
				ip = host_create_any(AF_INET6);
			}
			else
			{
				DBG1(DBG_CFG, "ignored invalid subnet token: %s", token);
			...
			if (ip)
			{
				peer_cfg->add_intnetmask(peer_cfg, ip);
			}
		}
		enumerator->destroy(enumerator);
	}

	if (msg->add_conn.me.intsubnet)
	{
		enumerator_t *enumerator;
		char *token;

		enumerator = enumerator_create_token(msg->add_conn.me.intsubnet, ",", " ");
		while (enumerator->enumerate(enumerator, &token))
		{
			host_t *ip = NULL;
			...
			if (ip)
			{
				peer_cfg->add_intsubnet(peer_cfg, ip);
			}
		}
		enumerator->destroy(enumerator);
	}
									...
									stroke_msg_t *msg)
{
	...
	child_cfg->set_wod_options(child_cfg, msg->add_conn.me.sourceif,
				msg->add_conn.me.force_tsi_64, msg->add_conn.me.force_tsi_full);
