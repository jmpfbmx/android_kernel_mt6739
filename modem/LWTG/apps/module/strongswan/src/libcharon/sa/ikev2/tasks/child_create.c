								   ...
								   bool no_dh, bool ike_auth)
{
	...
	if (this->initiator)
	{
		...
		if (this->config->use_force_tsi_full(this->config)) {
			enumerator_t *enumerator =
					this->ike_sa->get_virtual_ips(this->ike_sa);
			host_t *vip;
			while (enumerator->enumerate(enumerator, &vip)) {
				...
				{
					if ((src_ts->get_type(src_ts) == TS_IPV4_ADDR_RANGE
							&& vip->get_family(vip) == AF_INET)
						||(src_ts->get_type(src_ts) == TS_IPV6_ADDR_RANGE
							&& vip->get_family(vip) == AF_INET6))
					{
						src_ts->set_ts_single_addr(src_ts, vip);
					}
				}
...
*/
			...
			case TRAFFIC_SELECTOR_INITIATOR:
				...
				//store the tsi for rekey
				if (this->child_sa)
				{
					this->child_sa->set_rekey_tsi(this->child_sa, ts_payload->get_traffic_selectors(ts_payload));
					DBG1(DBG_IKE, "store tsi for rekey");
				}
				break;
			case TRAFFIC_SELECTOR_RESPONDER:
				ts_payload = (ts_payload_t*)payload;
				this->tsr = ts_payload->get_traffic_selectors(ts_payload);
				//store the tsr for rekey
				if (this->child_sa)
				{
					this->child_sa->set_rekey_tsr(this->child_sa, ts_payload->get_traffic_selectors(ts_payload));
					DBG1(DBG_IKE, "store tsr for rekey");
				}
	...
	private_child_create_t *this, message_t *message)
{
	...
	/*check if rekey request, if it is a rekey request, it need contain the init TS*/
	if(this->rekey)
	{
		DBG0(DBG_IKE, "build child_sa rekey request");
		if(this->tsi_rekey)
		{
			this->tsi = this->tsi_rekey;
			DBG0(DBG_IKE, "init rekey tsi from init tsi");
		}
		else
		{
			DBG0(DBG_IKE, "can not init rekey tsi from init tsi");
		}
		
		if(this->tsr_rekey)
		{
			this->tsr = this->tsr_rekey;
			DBG0(DBG_IKE, "init rekey tsr from init tsr");
		}
		else
		{
			DBG0(DBG_IKE, "can not init rekey tsr from init tsr");
		}
	}
	else
	{
	...
	//if rekey, the tsi/tsr will be destroy with child_sa ->destroy
	if(!this->rekey)
	{
		DBG0(DBG_IKE, "it is not rekey, destroy tsi & tsr");
		this->tsi->destroy_offset(this->tsi, offsetof(traffic_selector_t, destroy));
		this->tsr->destroy_offset(this->tsr, offsetof(traffic_selector_t, destroy));
	}
	...
	private_child_create_t *this, message_t *message)
{
	...
	if (select_and_install(this, no_dh, ike_auth) == SUCCESS)
	{
		if (!this->rekey)
		{	/* invoke the child_up() hook if we are not rekeying */
			this->ike_sa->set_status_code(this->ike_sa, PDN_STATUS_OK);
			this->ike_sa->set_sub_error_code(this->ike_sa, 0);
			DBG0(DBG_IKE,"reset status before notify success status to EPDGD");
			charon->bus->child_updown(charon->bus, this->child_sa, TRUE);
		}
	}
	else
...
child_create_t *child_create_create(ike_sa_t *ike_sa,
							...
							traffic_selector_t *tsi, traffic_selector_t *tsr, linked_list_t *tsi_rekey, linked_list_t *tsr_rekey)
{
	private_child_create_t *this;
