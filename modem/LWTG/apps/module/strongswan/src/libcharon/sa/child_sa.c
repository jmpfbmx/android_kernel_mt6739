...
struct private_child_sa_t {
	...
	/**
	 * traffic selectors for rekey initiators 
	 */
	linked_list_t *tsi_rekey;

	/**
	 * traffic selectors for rekey responders 
	 */
	linked_list_t *tsr_rekey;
};
...
METHOD(child_sa_t, set_rekey_tsi, void,
	   private_child_sa_t *this, linked_list_t *rekey_tsi)
{
	DBG1(DBG_IKE, "set_rekey_tsi");
	if (this->tsi_rekey)
	{
		this->tsi_rekey->destroy_offset(this->tsi_rekey, offsetof(traffic_selector_t, destroy));
	}
	this->tsi_rekey = rekey_tsi;
}

METHOD(child_sa_t, set_rekey_tsr, void,
	   private_child_sa_t *this, linked_list_t *rekey_tsr)
{
	DBG1(DBG_IKE, "set_rekey_tsr");
	if (this->tsr_rekey)
	{
		this->tsr_rekey->destroy_offset(this->tsr_rekey, offsetof(traffic_selector_t, destroy));
	}
	this->tsr_rekey = rekey_tsr;
}

METHOD(child_sa_t, get_rekey_tsi, linked_list_t *,
	   private_child_sa_t *this)
{
	DBG1(DBG_IKE, "get_rekey_tsi");
	return this->tsi_rekey;
}

METHOD(child_sa_t, get_rekey_tsr, linked_list_t *,
	   private_child_sa_t *this)
{
	DBG1(DBG_IKE, "get_rekey_tsr");
	return this->tsr_rekey;
}
	...
	linked_list_t *my_ts, linked_list_t *other_ts)
{
	...
	DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "adding %s %N SA",
         inbound ? "inbound" : "outbound", protocol_id_names, this->protocol);

	/* send SA down to the kernel */
	DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY,
         "  SPI 0x%.8x, src %H dst %H", ntohl(spi), src, dst);
	...
	if (inbound)
	{
		enumerator_t *enumerator_ts = my_ts->create_enumerator(my_ts);
		while (enumerator_ts->enumerate(enumerator_ts, &dst_ts))
		{
			if (dst_ts && dst_ts->get_type(dst_ts) == TS_IPV6_ADDR_RANGE)
			{
				DBG1(DBG_IKE, "origin inbound policy,dst : %R", dst_ts);
				if (this->config->use_force_tsi_64(this->config)) {
					dst_ts->set_ts_ipv6_range_64(dst_ts);
					DBG1(DBG_IKE, "inbound policy,dst : %R === %R", src_ts, dst_ts);
				}
			}
		}
		enumerator_ts->destroy(enumerator_ts);
	}
	else
	{
		enumerator_t *enumerator_ts = my_ts->create_enumerator(my_ts);
		while (enumerator_ts->enumerate(enumerator_ts, &src_ts))
		{
			if (src_ts && src_ts->get_type(src_ts) == TS_IPV6_ADDR_RANGE)
			{
				DBG1(DBG_IKE, "origin outbound policy,src : %R", src_ts);
				if (this->config->use_force_tsi_64(this->config)) {
					src_ts->set_ts_ipv6_range_64(src_ts);
					DBG1(DBG_IKE, "outbound policy,src : %R === %R", src_ts, dst_ts);
				}
			}
		}
		enumerator_ts->destroy(enumerator_ts);
	...
	if (status == SUCCESS &&
		IS_MDMI_SUPPORTED())
	{
		char enc_key_str[256];
		char int_key_str[256];
		char tmp[1024];

		chunk_to_hex(encr, enc_key_str, 1);
		chunk_to_hex(integ, int_key_str, 1);

		snprintf(tmp, sizeof(tmp), "%s,\"%08X\",\"%H\",\"%H\",%u,%zu,\"%s\",%u,\"%s\"",
				(src->get_family(src) == AF_INET)? "4" : "6",
				ntohl(spi), src, dst,
				enc_alg, encr.len * 8, enc_key_str,
				int_alg, int_key_str);
		mdmi_notify_wod(N_ESP_DECRYPT_ADD, get_name(this), (void *)tmp);
	}
...
static void reinstall_vip(host_t *vip, host_t *me)
{
	...
	if (hydra->kernel_interface->get_interface(hydra->kernel_interface,
											   me, &iface))
	{
		//	for MOBIKE, to mask del_ip and add_ip
		size_t netbits =
			(vip->get_family(vip) == AF_INET6 && vip->get_netbits(vip))
			? vip->get_netbits(vip) : -1;
		hydra->kernel_interface->del_ip(hydra->kernel_interface, vip, netbits, TRUE);
		hydra->kernel_interface->add_ip(hydra->kernel_interface, vip, netbits, iface);
		free(iface);
	}
}
	...
	if (this->config->install_policy(this->config))
	{
			...
			!other->ip_equals(other, this->other_addr))
		{
			...
			while (enumerator->enumerate(enumerator, &my_ts, &other_ts))
			{
				...
				/*
				 * If virtual inferface is used,
				 * no need to reinstall Virual IP for MOBIKE.
				 * EPDG interface is virtual and will not be changed.
				 */
				if (this->config->get_vif(this->config) == NULL)
				{
	   ...
	   private_child_sa_t *this)
{
	...
	if (this->my_spi)
	{
		...
		if (IS_MDMI_SUPPORTED())
		{
			...
			snprintf(tmp, sizeof(tmp), "\"%08X\"", ntohl(this->my_spi));
			mdmi_notify_wod(N_ESP_DECRYPT_DEL, get_name(this), (void *)tmp);
		}
	}
	if (this->other_spi)
	{
		...
		if (IS_MDMI_SUPPORTED())
		{
			...
			snprintf(tmp, sizeof(tmp), "\"%08X\"", ntohl(this->other_spi));
			mdmi_notify_wod(N_ESP_DECRYPT_DEL, get_name(this), (void *)tmp);
		}
	}
							 ...
							 child_cfg_t *config, u_int32_t rekey, bool encap)
{
	...
	INIT(this,
		.public = {
			...
			.set_rekey_tsi = _set_rekey_tsi,
			.set_rekey_tsr = _set_rekey_tsr,
			.get_rekey_tsi = _get_rekey_tsi,
			.get_rekey_tsr = _get_rekey_tsr,
