...
struct private_ike_mobike_t {
	...
	/**
	 * update keep-alive info when OOS ends
	 */
	bool oos_end_update;
};
...
static void process_payloads(private_ike_mobike_t *this, message_t *message)
{
	...
	while (enumerator->enumerate(enumerator, &payload))
	{
		...
		if (type != NAT_DETECTION_SOURCE_IP && type != NAT_DETECTION_DESTINATION_IP)
		{
			this->not_dpd = TRUE;
		}
	}
	   ...
	   private_ike_mobike_t *this, message_t *message)
{
	...
	else if (message->get_exchange_type(message) == INFORMATIONAL)
	{
		...
		{
			process_payloads(this, message);
			if (!this->not_dpd && this->ike_sa->get_oos(this->ike_sa))
			{
				this->ike_sa->set_oos(this->ike_sa, OOS_RESUME);
				DBG1(DBG_IKE, "============================= receive MOBIKE DPD reply, oos disabled.");
				this->oos_end_update = TRUE;
			}
		}
		if (this->natd)
		{
		...
		if (this->check)
		{
			...
			if (this->update)
			{
				...
				this->check = FALSE;
				if (this->oos_end_update)
				{
					/* Update keep alive info - address changed */
					this->ike_sa->update_keepalive_info(this->ike_sa, TRUE, TRUE);
					this->oos_end_update = FALSE;
				}
				return NEED_MORE;
			}
		}
		if (this->oos_end_update)
		{
			/* Update keep alive info - the same address */
			this->ike_sa->update_keepalive_info(this->ike_sa, TRUE, TRUE);
			this->oos_end_update = FALSE;
		}
		return SUCCESS;
