							...
							u_int mid, u_int retransmitted, array_t *packets)
{
	...
	lib->scheduler->schedule_job_ms(lib->scheduler, (job_t*)
			retransmit_job_create(seqnr, this->ike_sa->get_id(this->ike_sa), FALSE, 0), t);
	return NEED_MORE;
}
	...
	private_task_manager_t *this)
{
	...
	{
		message->destroy(message);
		return retransmit(this, this->initiating.seqnr, FALSE, 0);
	}
	if (keep)
...
static status_t build_response(private_task_manager_t *this, message_t *request)
{
	...
	{
		return retransmit(this, this->responding.seqnr, FALSE, 0);
	}
