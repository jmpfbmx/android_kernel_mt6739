...
typedef enum keepalive_job_state_t keepalive_job_state_t;
...
enum keepalive_job_state_t {
	KEEPALIVE_JOB_NONE,
	KEEPALIVE_JOB_CREATED,
	KEEPALIVE_JOB_EXECUTED,
};
...
struct ike_sa_t {
	 ...
	 * @param oos			retransmit job in oos
	...
	status_t (*retransmit) (ike_sa_t *this, u_int32_t message_id,
							bool oos, u_int32_t oos_seq_num);
	...
	/**
	 * Update keep alive info to WOD.
	 *
	 * @param enable		keep alive is enabled or not
	 * @param restart_orig		restart default software keep alive in this IKE_SA
	 */
	void (*update_keepalive_info) (ike_sa_t *this, bool enable, bool restart_orig);
	...
	enumerator_t* (*get_virtual_ips) (ike_sa_t *this);
	...
	/**
	 * Clear virtual IPs except my_vips in stored on this IKE_SA.
	 *
	 * @param local			TRUE to clear local addresses, FALSE for remote
	 * @param my_vips		list for my vips
	 */
	void (*clear_virtual_ips2) (ike_sa_t *this, bool local, linked_list_t *my_vips);
	/**
	...
	/**
	 * Add a dns server to use for this IKE_SA and its children.
	 *
	 * @param ip			IP to set as virtual IP
	 */
	void (*add_dns) (ike_sa_t *this, host_t *ip);

	/**
	 * Clear all dns stored on this IKE_SA.
	 *
 	 */
	void (*clear_dnss) (ike_sa_t *this);
	/**
	 * Create an enumerator over dnss.
	 *
	 * @return				enumerator over host_t*
	 */
	enumerator_t* (*create_dns_enumerator) (ike_sa_t *this);

	/**
	 * Add a pcscf server to use for this IKE_SA and its children.
	 *
	 * @param ip			IP to set as virtual IP
	 */
	void (*add_pcscf) (ike_sa_t *this, host_t *ip);

	/**
	 * Clear all pcscf stored on this IKE_SA.
	 *
 	 */
	void (*clear_pcscfs) (ike_sa_t *this);

	/**
	 * Create an enumerator over pcscfs.
	 *
	 * @return				enumerator over host_t*
	 */
	enumerator_t* (*create_pcscf_enumerator) (ike_sa_t *this);

	void (*add_intsubnet) (ike_sa_t *this, host_t *ip);
	void (*clear_intsubnets) (ike_sa_t *this);
	enumerator_t* (*create_intsubnet_enumerator) (ike_sa_t *this);
	void (*add_intnetmask) (ike_sa_t *this, host_t *ip);
	void (*clear_intnetmasks) (ike_sa_t *this);
	enumerator_t* (*create_intnetmask_enumerator) (ike_sa_t *this);

	void (*set_wdrv_keepalive) (ike_sa_t *this, bool enable);
	void (*set_keepalive_job_state) (ike_sa_t *this, keepalive_job_state_t state);
	bool (*get_oos) (ike_sa_t *this);
	void (*set_oos) (ike_sa_t *this, int oos);
	u_int32_t (*get_oos_seq_num) (ike_sa_t *this);
	int (*get_status_code) (ike_sa_t *this);
	void (*set_status_code) (ike_sa_t *this, int status_code);
	int (*get_sub_error_code) (ike_sa_t *this);
	void (*set_sub_error_code) (ike_sa_t *this, int sub_error_code);
	...
	*
	*/
	void (*set_backoff_timer) (ike_sa_t *this, int backoff_timer);

	/**
	*set the pcscf need notify flag 
	*/
	void (*set_pcscf_need_update) (ike_sa_t *this, bool need_update);

	/**
	*set need notify device_identity flag 
	*/
	void (*set_notify_device_identity) (ike_sa_t *this,bool need);

	/**
	*get backoff tmie setting
	*@return back off time value
	*
	*/
	int (*get_backoff_timer) (ike_sa_t *this);

	/**
	*get whether need notify pcscf update
	*@return TRUE or FALSE
	*
	*/
	bool (*get_pcscf_need_update) (ike_sa_t *this);

	/**
	*get whether need notify device_identity
	*@return TRUE or FALSE
	*
	*/
	bool (*get_notify_device_identity) (ike_sa_t *this);
