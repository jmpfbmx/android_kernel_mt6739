		...
		array_count(this->initiating.packets))
	{
			...
			!hydra->kernel_interface->get_data_path_status(hydra->kernel_interface))
		{
			...
			job = (job_t*)retransmit_job_create(this->initiating.mid,
												...
												TRUE,
												this->ike_sa->get_oos_seq_num(this->ike_sa));
			lib->scheduler->schedule_job(lib->scheduler, job, OOS_RETRANSMIT_TIMEOUT);
			return SUCCESS;
		}
	...
	if (this->initiating.type != EXCHANGE_TYPE_UNDEFINED)
	{
		...
		{	/* re-initiate deferred exchange */
			...
			return retransmit(this, this->initiating.mid,
							this->ike_sa->get_oos(this->ike_sa),
							this->ike_sa->get_oos_seq_num(this->ike_sa));
		}
		return SUCCESS;
	}
		...
		switch (message->get_exchange_type(message))
		{
			...
			case INFORMATIONAL:
			{
				...
				if (task == NULL)
				{
					...
					task = (task_t*)ike_dpd_create(this->ike_sa, FALSE);
				}
				array_insert(this->passive_tasks, ARRAY_TAIL, task);
				break;
			}
	...
	{	/* check for unsupported critical payloads */
		...
		while (enumerator->enumerate(enumerator, &payload))
		{
			...
			if (pl_type == NOTIFY)
			{
				...
				if(is_error_notify(notify) && notify_type != INVALID_KE_PAYLOAD)
				{
					if (this->ike_sa->get_status_code(this->ike_sa) == PDN_STATUS_OK)
					{
	...
	private_task_manager_t *this)
{
	...
	{
		queue_task(this, (task_t*)ike_dpd_create(this->ike_sa, TRUE));
	}
}
