...
static configuration_attribute_t *build_imei(peer_cfg_t *config)
{
	configuration_attribute_type_t type = config->get_cust_cp_imei(config);
	...
	if (len == 15 || len == 16)
	{
		bin[0] = (len == 15) ? 0x1 : 0x2;
		for (i = 0; i < len; i += 2)
		{
			if (i == 14 && len == 15)
			{
				bin[i / 2 + 1] = (imei[i] - 48) + (0xF << 4);
			}
			else
			{
				bin[i / 2 + 1] = (imei[i] - 48) + (imei[i + 1] << 4);
			}
		}
	}
	...
	switch (ca->get_type(ca))
	{
		...
		case INTERNAL_IP6_ADDRESS:
		{
			...
			else
			{
				...
				if (config->use_cfg_vip(config) && ip)
				{
					...
					while (enumerator->enumerate(enumerator, &host))
					{
						if(!host->is_anyaddr(host) && host->get_family(host) == AF_INET6)
						{
							...
							ip->destroy(ip); /* release assigned vip */
							ip =  host->clone(host);
						}
					}
					enumerator->destroy(enumerator);
		...
		default:
		{
			if (ca->get_type(ca) == config->get_cust_cp_pcscf4(config))
			{
				family = AF_INET;
				addr = ca->get_chunk(ca);
				if (addr.len == 0)
			...
			else if (ca->get_type(ca) == config->get_cust_cp_pcscf6(config))
			{
				addr = ca->get_chunk(ca);
				if (addr.len == 0)
				{
			...
			else if (ca->get_type(ca) == config->get_cust_cp_imei(config))
			{	/* Do nothing */
			}
			else
			{
	...
	private_ike_config_t *this, message_t *message)
{
	...
	{	/* in first IKE_AUTH only */
		...
		/* Add pcscf to cp request payload */
		...
		if (pcscfs->get_count(pcscfs))
		{
			...
			while (enumerator->enumerate(enumerator, &host))
			{
				cp->add_attribute(cp, build_pcscf(host, config));
			}
			enumerator->destroy(enumerator);
		}
		...
		if (config->get_cust_cp_imei(config) > 0 && config->get_imei(config))
		{
			if(!cp)
			{
				cp = cp_payload_create_type(CONFIGURATION, CFG_REQUEST);
			}
			cp->add_attribute(cp, build_imei(config));
		}

		/* Add netmask to cp request payload */
		...
		/* Add subnet to cp request payload */
