...
#define IKEV2_DECRYPTION_TABLE_PREFIX "ikev2_decryption_table"
      ...
      u_int16_t key_size, prf_plus_t *prf_plus)
{
   ...
   DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "Sk_ei secret %B", &key);
   this->sk_ei = chunk_create_clone(malloc(key.len), key);
   ...
   DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "Sk_er secret %B", &key);
   this->sk_er = chunk_create_clone(malloc(key.len), key);
      ...
      u_int16_t enc_size, u_int16_t int_alg, prf_plus_t *prf_plus)
{
   ...
   //	DBG4(DBG_IKE, "Sk_ai secret %B", &key);
    DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "Sk_ai secret %B", &key);
   this->sk_ai = chunk_create_clone(malloc(key.len), key);
   ...
   DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "Sk_ar secret %B", &key);
   this->sk_ar = chunk_create_clone(malloc(key.len), key);
...
METHOD(keymat_v2_t, derive_ike_keys, bool,
      ...
      pseudo_random_function_t rekey_function, chunk_t rekey_skd, const char *conn_name)
{
   chunk_t skeyseed, key, secret, full_nonce, fixed_nonce, prf_plus_seed;
   chunk_t spi_i, spi_r;
   prf_plus_t *prf_plus = NULL;
   u_int16_t alg, key_size, int_alg, enc_key_size;
   ...
   *((u_int64_t*)spi_r.ptr) = id->get_responder_spi(id);
   DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "SPI_i %B", &spi_i);
   DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "SPI_r %B", &spi_r);
   ...
   DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "SKEYSEED %B", &skeyseed);
   ...
   /* output IKEv2 keymat in wireshark format */
   //  libstrongswan/crypto/signers/signer.h
   //  libstrongswan/crypto/crypters/crypter.c
   {
      char spi_i_str[20], spi_r_str[20];
      char sk_ei_str[256], sk_er_str[256];
      char sk_ai_str[256], sk_ar_str[256];
      char keymat_str[1024];

      if(get_cust_setting_bool(WSHARK_KEY_DUMP)
        && get_cust_setting_bool(ENCR_KEY_DISPLAY))
      {
         FILE *fp;

         chunk_to_hex(spi_i, spi_i_str, 1);
         chunk_to_hex(spi_r, spi_r_str, 1);
         chunk_to_hex(this->sk_ei, sk_ei_str, 1);
         chunk_to_hex(this->sk_er, sk_er_str, 1);
         chunk_to_hex(this->sk_ai, sk_ai_str, 1);
         chunk_to_hex(this->sk_ar, sk_ar_str, 1);

         if (get_log_file( &fp, IKEV2_DECRYPTION_TABLE_PREFIX)) {
            if(encryption_algorithm_is_aead(alg)) {
               // To-Do
            }
            else {
               sprintf(keymat_str, "%s,%s,%s,%s,\"%N\",%s,%s,\"%N\"\n",
                           spi_i_str, spi_r_str,
                           sk_ei_str, sk_er_str, ikev2_encryption_algorithm_names, alg,
                           sk_ai_str, sk_ar_str, ikev2_integrity_algorithm_names, int_alg);
               fprintf(fp, keymat_str, enc_key_size);
            }
            fclose(fp);
         }
         else {
            DBG1(DBG_IKE, "IKEv2 decryption table cannot be opened");
         }
      }
      ...
      if (IS_MDMI_SUPPORTED())
      {
         ...
         snprintf(keymat_str, sizeof(keymat_str), "\"%s\",\"%s\",\"%s\",\"%s\",%u,%u,\"%s\",\"%s\",%u",
                     spi_i_str, spi_r_str,
                     sk_ei_str, sk_er_str, alg, enc_key_size,
                     sk_ai_str, sk_ar_str, int_alg);
         mdmi_notify_wod(N_IKE_DECRYPT_ADD, conn_name, (void *)keymat_str);
      }
   }
