...
ENUM_NEXT(notify_type_names, UNITY_LOAD_BALANCE, UNITY_LOAD_BALANCE, DPD_R_U_THERE_ACK,
	"UNITY_LOAD_BALANCE");
ENUM_NEXT(notify_type_names, USE_BEET_MODE, REACTIVATION_REQUESTED_CAUSE, UNITY_LOAD_BALANCE,
	"BEET_MODE",
	"PCSCF_REACTIVATION");
ENUM_NEXT(notify_type_names, ME_MEDIATION, RADIUS_ATTRIBUTE, REACTIVATION_REQUESTED_CAUSE,
	"ME_MEDIATION",
	"ME_ENDPOINT",
	"ME_CALLBACK",
	"ME_CONNECTID",
...
ENUM_NEXT(notify_type_names, BACKOFF_TIMER, BACKOFF_TIMER, RADIUS_ATTRIBUTE,
	"BACKOFF_TIMER");
ENUM_NEXT(notify_type_names, DEVICE_IDENTITY, DEVICE_IDENTITY, BACKOFF_TIMER,
	"DEVICE_IDENTITY");
ENUM_NEXT(notify_type_names, PCSCF_RESELECTION_SUPPORT, PCSCF_RESELECTION_SUPPORT, DEVICE_IDENTITY,
	"PCSCF_RECELECT_SUPPORRT");
ENUM_END(notify_type_names, PCSCF_RESELECTION_SUPPORT);
...
ENUM_NEXT(notify_type_short_names, UNITY_LOAD_BALANCE, UNITY_LOAD_BALANCE, DPD_R_U_THERE_ACK,
	"UNITY_LB");
ENUM_NEXT(notify_type_short_names, USE_BEET_MODE, REACTIVATION_REQUESTED_CAUSE, UNITY_LOAD_BALANCE,
	"BEET_MODE",
	"PCSCF_REACTIVATION");
ENUM_NEXT(notify_type_short_names, ME_MEDIATION, RADIUS_ATTRIBUTE, REACTIVATION_REQUESTED_CAUSE,
	"ME_MED",
	"ME_EP",
	"ME_CB",
	"ME_CID",
...
ENUM_NEXT(notify_type_short_names, BACKOFF_TIMER, BACKOFF_TIMER, RADIUS_ATTRIBUTE,
	"BO_TIMER");
ENUM_NEXT(notify_type_short_names, DEVICE_IDENTITY, DEVICE_IDENTITY, BACKOFF_TIMER,
	"DEVICE_IDENTITY");
ENUM_NEXT(notify_type_short_names, PCSCF_RESELECTION_SUPPORT, PCSCF_RESELECTION_SUPPORT, DEVICE_IDENTITY,
	"PCSCF_RECELECT_S");
ENUM_END(notify_type_short_names, PCSCF_RESELECTION_SUPPORT);
	...
	private_notify_payload_t *this)
{
	...
	switch (this->notify_type)
	{
		...
		case DEVICE_IDENTITY:
			...
			break;
		case PCSCF_RESELECTION_SUPPORT:
		case REACTIVATION_REQUESTED_CAUSE:
		default:
			/* TODO: verify */
			break;
	}
...
bool is_error_notify(notify_payload_t *notify)
{
	return (notify->get_notify_type(notify) < INITIAL_CONTACT);
}

bool is_status_notify(notify_payload_t *notify)
{
	return (notify->get_notify_type(notify) >= INITIAL_CONTACT);
}
