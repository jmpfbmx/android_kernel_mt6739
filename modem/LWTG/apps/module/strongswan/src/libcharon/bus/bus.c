...
METHOD(bus_t, child_update, void,
	private_bus_t *this, child_sa_t *child_sa, int type)
{
	enumerator_t *enumerator;
	ike_sa_t *ike_sa;
	entry_t *entry;
	...
	while (enumerator->enumerate(enumerator, &entry))
	{
		if (entry->calling || !entry->listener->child_update)
		{
			continue;
		}
		entry->calling++;
		keep = entry->listener->child_update(entry->listener,
											 ike_sa, child_sa, type);
		entry->calling--;
		if (!keep)
		{
			unregister_listener(this, entry, enumerator);
...
METHOD(bus_t, ike_update, void,
	private_bus_t *this, ike_sa_t *ike_sa, int type)
{
	enumerator_t *enumerator;

	/* a update event for IKE_SA implicitly downs all CHILD_SAs */
	...
	while (enumerator->enumerate(enumerator, (void**)&child_sa))
	{
		child_update(this, child_sa, type);
	}
	enumerator->destroy(enumerator);
}
