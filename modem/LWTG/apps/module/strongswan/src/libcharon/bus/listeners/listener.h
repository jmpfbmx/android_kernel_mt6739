...
struct listener_t {
	...
	/**
	 * Hook called when a CHILD_SA gets update.
	 *
	 * @param ike_sa	IKE_SA containing the handled CHILD_SA
	 * @param child_sa	CHILD_SA coming update
	 * @param type              update parameter
	 * @return			TRUE to stay registered, FALSE to unregister
	 */
	bool (*child_update)(listener_t *this, ike_sa_t *ike_sa,
						 child_sa_t *child_sa, int type);
