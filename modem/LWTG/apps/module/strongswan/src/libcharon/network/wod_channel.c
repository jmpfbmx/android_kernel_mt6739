	...
	switch (cmd)
	{
		...
		*/
		...
		case N_DNS:
			...
			break;
		case N_OOS:
			snprintf(tmp, MAX_BUF_LEN, "ipsecoos=%s,%i", sim_sn_info, (wo_oos_t)param);
			break;
...
void mdmi_notify_wod(wo_notify_cmd_t cmd, const char *conn_name, void *value)
{
	char rxbuf[MAX_BUF_LEN] = {0};
	char tmp[MAX_BUF_LEN] = {0};
	...
	DBG1(DBG_IKE, "mdmi_notify_wod:[%s]", tmp);
	wod_tcp_txrx(WOD_SOCK_NAME, NSID_ABSTRACT, tmp, rxbuf, MAX_BUF_LEN, &rxlen);
	free(sim_info);
}
