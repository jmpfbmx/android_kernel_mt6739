...
/*cannot use ut ipsec piddir because in UT path goes over 100 marks and it is too long*/
#define STROKE_SOCKET "charon.ctl"
#else
...
struct stroke_msg_t {
	...
	enum {
		...
		/* trigger DPD */
		STR_DPD,
		/* data path status notify */
		STR_DATA_PATH_NOTIFY,
		/* Wi-Fi driver keepalive notify */
		STR_WDRV_KEEPALIVE_NOTIFY,
	} type;
	...
	union {
		...
		/* data for STR_DATA_PATH_NOTIFY */
		struct {
			char *status;
			char *if_name;
		} data_path_status_notify;

		/* data for STR_WDRV_KEEPALIVE_NOTIFY */
		struct {
			char *name;
			int enable;
		} wdrv_keepalive_notify;
	};
