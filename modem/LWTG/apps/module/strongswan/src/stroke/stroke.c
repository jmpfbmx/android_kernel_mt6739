...
static int data_path_notify(char *status, char *name)
{
	stroke_msg_t msg;

	msg.length = offsetof(stroke_msg_t, buffer);
	msg.type = STR_DATA_PATH_NOTIFY;
	msg.data_path_status_notify.status = push_string(&msg, status);
	msg.data_path_status_notify.if_name = push_string(&msg, name);
	return send_stroke_msg(&msg);
}

static int wdrv_keepalive_notify(char *name, u_int enable)
{
	stroke_msg_t msg;

	msg.type = STR_WDRV_KEEPALIVE_NOTIFY;
	msg.length = offsetof(stroke_msg_t, buffer);
	msg.wdrv_keepalive_notify.name = push_string(&msg, name);
	msg.wdrv_keepalive_notify.enable = enable;
	return send_stroke_msg(&msg);
}
	...
	switch (token->kw)
	{
		...
		case STROKE_COUNTERS_RESET:
			res = counters(token->kw == STROKE_COUNTERS_RESET,
						   argc > 2 ? argv[2] : NULL);
			break;
		case STROKE_DATA_PATH:
			if (argc < 4)
			{
				exit_usage("\"data_path\" needs a status type and"
						"interface name");
			}
			res = data_path_notify(argv[2], argv[3]);
			break;
		case STROKE_WDRV_KEEPALIVE:
			if (argc < 4)
			{
				exit_usage("\"wdrv_keepalive\" needs a connection name and on/off status");
			}
			res = wdrv_keepalive_notify(argv[2], atoi(argv[3]));
			break;
		default:
			exit_usage(NULL);
	}
